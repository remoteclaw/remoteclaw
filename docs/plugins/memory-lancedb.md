...
Set `dimensions` for non-standard embedding models. RemoteClaw knows the
...
`memory_forget` query path, and `remoteclaw ltm search`. Auto-recall prefers the
...
```bash
remoteclaw ltm list
remoteclaw ltm search "project preferences"
...
By default, LanceDB data lives under `~/.remoteclaw/memory/lancedb`. Override the
path with `dbPath`:
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "~/.remoteclaw/memory/lancedb",
          embedding: {
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "s3://memory-bucket/remoteclaw",
          storageOptions: {
...
```bash
remoteclaw ltm stats
remoteclaw ltm search "recent preference"
```
