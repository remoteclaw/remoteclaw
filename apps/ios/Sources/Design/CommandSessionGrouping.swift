...
struct CommandSessionSection: Identifiable {
    ...
    let id: ID
    let title: String
    let entries: [RemoteClawChatSessionEntry]
    let showsHeader: Bool
}

enum CommandSessionGrouping {
    static func sections(
        from entries: [RemoteClawChatSessionEntry],
    ...
    static func previewOrder(_ entries: [RemoteClawChatSessionEntry]) -> [RemoteClawChatSessionEntry] {
    ...
    static func previewSelection(
        _ entries: [RemoteClawChatSessionEntry],
        currentKey: String,
        limit: Int = 3) -> [RemoteClawChatSessionEntry]
    {
    ...
    static func members(
        of group: String,
        in lists: [[RemoteClawChatSessionEntry]]) -> [RemoteClawChatSessionEntry]
    {
    ...
    static func activityTimestamp(_ entry: RemoteClawChatSessionEntry) -> Double {
        entry.lastActivityAt ?? entry.updatedAt ?? 0
    }

    private static func sortedByActivity(
        _ entries: [RemoteClawChatSessionEntry]) -> [RemoteClawChatSessionEntry]
    {
        entries.sorted { lhs, rhs in
            let left = self.activityTimestamp(lhs)
            let right = self.activityTimestamp(rhs)
