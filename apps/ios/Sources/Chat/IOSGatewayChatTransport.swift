...
struct IOSGatewayChatTransport: RemoteClawChatTransport {
    ...
    func createSession(
        ...
        label: String?,
        parentSessionKey: String?,
        worktree: Bool?) async throws -> RemoteClawChatCreateSessionResponse
    {
    ...
    func listSessions(
        limit: Int?,
        search: String?,
        archived: Bool) async throws -> RemoteClawChatSessionsListResponse
    {
    ...
    func forkSession(parentKey: String) async throws -> String {
        ...
        return try JSONDecoder().decode(RemoteClawChatCreateSessionResponse.self, from: response).key
    }
    ...
    func compactSession(sessionKey: String) async throws {
        ...
        try RemoteClawSessionsCompactResponse.requireSuccess(from: response)
    }
    ...
    func requestHistory(
        sessionKey: String,
        agentID: String? = nil,
        ifCurrentRoute expectedRoute: GatewayNodeSessionRoute?) async throws -> RemoteClawChatHistoryPayload
    {
    ...
    func listCommands(sessionKey: String) async throws -> [RemoteClawChatCommandChoice] {
        ...
        attachments: [RemoteClawChatAttachmentPayload]) async throws -> RemoteClawChatSendResponse
    {
        ...
        guard let route,
              let supportsRoutingContract = await gateway.supportsServerCapability(
                  .chatSendRoutingContract,
                  ifCurrentRoute: route)
        else { throw RemoteClawChatTransportSendError.notDispatched }
        ...
        let guardedContract = RemoteClawChatSessionRoutingContract.expectedValue(
    ...
    func sendMessage(
        ...
        thinking: String,
        idempotencyKey: String,
        attachments: [RemoteClawChatAttachmentPayload],
        ifCurrentRoute expectedRoute: GatewayNodeSessionRoute?,
        distinguishPreDispatchRouteChange: Bool = false) async throws -> RemoteClawChatSendResponse
    {
        let target = self.sessionTarget(for: sessionKey, overrideAgentID: agentID)
        let startLogMessage =
    ...
    private static func mapCommandChoice(_ entry: CommandEntry) -> RemoteClawChatCommandChoice {
        let sourceValue = (entry.source.value as? String)?
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
        let source: RemoteClawChatCommandChoice.Source = switch sourceValue {
        case "native":
            .command
        case "skill":
            .skill
        ...
        return RemoteClawChatCommandChoice(
            id: id,
            name: entry.name,
            textAliases: aliases,
