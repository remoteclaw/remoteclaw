...
describe("config io write prepare", () => {
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("remoteclaw config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('remoteclaw config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/remoteclaw.json",
        expectedConfigPath: "/tmp/remoteclaw.json",
  ...
  it("keeps plugin AJV defaults out of the persisted candidate", () => {
    ...
    const nextConfig: RemoteClawConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
  ...
  it("preserves root $schema during unrelated partial writes", () => {
    ...
    expect(persisted.$schema).toBe("https://remoteclaw.org/config.json");
    expect(persisted.gateway).toEqual({ mode: "local", port: 18789 });
  });

  it("does not preserve include-only $schema into the root persisted candidate", () => {
    const sourceConfig = {
      $schema: "https://remoteclaw.org/config-from-include.json",
      gateway: { mode: "local" },
    };
