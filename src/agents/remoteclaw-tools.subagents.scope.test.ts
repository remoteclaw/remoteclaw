...
} from "./remoteclaw-tools.subagents.test-harness.js";
...
describe("remoteclaw-tools: subagents scope isolation", () => {
  let storePath = "";
  ...
  beforeEach(() => {
    ...
    storePath = path.join(
      os.tmpdir(),
      `remoteclaw-subagents-scope-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
    setSubagentsConfigOverride({
