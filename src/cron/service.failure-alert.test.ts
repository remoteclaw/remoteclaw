...
describe("CronService failure alerts", () => {
  ...
  // Gutted in RemoteClaw fork — failure alert delivery path not fully wired
  it.skip("alerts after configured consecutive failures and honors cooldown", async () => {
  ...
  // Gutted in RemoteClaw fork — failure alert delivery path not fully wired
  it.skip("supports per-job failure alert override when global alerts are disabled", async () => {
    const store = await makeStorePath();
    const sendCronFailureAlert = vi.fn(async () => undefined);
    const runIsolatedAgentJob = vi.fn(async () => ({
      status: "error" as const,
  ...
  // Gutted in RemoteClaw fork — failure alert delivery path not fully wired
  it.skip("threads failure alert mode/accountId and skips best-effort jobs", async () => {
    const store = await makeStorePath();
    const sendCronFailureAlert = vi.fn(async () => undefined);
    const runIsolatedAgentJob = vi.fn(async () => ({
      status: "error" as const,
