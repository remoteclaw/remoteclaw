#!/usr/bin/env node
// Builds RemoteClaw packages and plugin SDK artifacts with cache-aware orchestration.
...
const PLUGIN_SDK_ENTRY_DTS_CACHE_ENV = [
  "REMOTECLAW_BUILD_PRIVATE_QA",
  "REMOTECLAW_PLUGIN_SDK_CANONICAL_DTS",
];
...
export const BUILD_ALL_STEPS = [
  ...
  },
  {
    ...
    env: {
      REMOTECLAW_PLUGIN_SDK_CANONICAL_DTS: "1",
    },
    cache: {
...
export const BUILD_ALL_PROFILE_STEP_ENV = {
  full: {
    tsdown: {
      REMOTECLAW_PRESERVE_CLI_STARTUP_METADATA: "1",
    },
  },
  ciArtifacts: {
    tsdown: {
...
export function buildAllUsage() {
  return [
    ...
    "",
    "Builds RemoteClaw artifacts for the selected profile.",
    "",
    "Profiles:",
...
export function resolveBuildAllStep(step, params = {}) {
  ...
  if (step.kind === "pnpm") {
    const nodeFallbackArgs =
      env.REMOTECLAW_BUILD_ALL_NO_PNPM === "1"
...
if (isMainModule()) {
  ...
  if (args?.help) {
    ...
    for (const step of resolveBuildAllSteps(args.profile)) {
      ...
      if (process.env.REMOTECLAW_BUILD_CACHE !== "0" && cacheState.fresh) {
