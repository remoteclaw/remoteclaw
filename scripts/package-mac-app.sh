...
/usr/libexec/PlistBuddy -c "Set :RemoteClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :RemoteClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "⏹  Stopping any running RemoteClaw"
killall -q RemoteClaw 2>/dev/null || true
