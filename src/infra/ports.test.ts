...
describeUnix("inspectPortUsage", () => {
  ...
  it("reports established gateway client connections from lsof", async () => {
      ...
      if (command === "ps") {
        ...
        if (argv.includes("command=")) {
          return {
            stdout:
              pid === "111"
                ? "node /tmp/newer-remoteclaw/dist/index.js logs --follow\n"
                : pid === "222"
                  ? "node /tmp/older-remoteclaw/dist/index.js gateway run\n"
    ...
    expect(result.connections[0]).toMatchObject({
      pid: 111,
      direction: "client",
      commandLine: "node /tmp/newer-remoteclaw/dist/index.js logs --follow",
    });
  ...
  it("deduplicates repeated lsof listener records for one process address", async () => {
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ps") {
        if (argv.includes("command=")) {
          return {
            stdout: "node /tmp/remoteclaw/dist/index.js gateway run\n",
            stderr: "",
            code: 0,
          };
        }
  ...
  it("reports multiple lsof socket records for one process", async () => {
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ps") {
        if (argv.includes("command=")) {
          return {
            stdout: "node /tmp/newer-remoteclaw/dist/index.js logs --follow\n",
            stderr: "",
            code: 0,
          };
        }
...
describe("inspectPortUsage on Windows", () => {
  it("reports established gateway client connections from netstat", async () => {
    ...
    expect(result.connections[0]?.commandLine).toContain("remoteclaw");
  });
