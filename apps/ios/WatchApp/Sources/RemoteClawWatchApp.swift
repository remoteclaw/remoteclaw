...
struct RemoteClawWatchApp: App {
    ...
    @State private var inboxStore = WatchInboxStore(
        requestNotificationAuthorization: !RemoteClawWatchApp.isScreenshotMode)
    @State private var receiver: WatchConnectivityReceiver?

    private static let screenshotModeDefaultsKey = "remoteclaw.watch.screenshotMode"
    private static let isScreenshotMode = ProcessInfo.processInfo.arguments.contains(
        "--remoteclaw-watch-screenshot-mode")
        || ProcessInfo.processInfo.environment["REMOTECLAW_WATCH_SCREENSHOT_MODE"] == "1"
        || UserDefaults.standard.bool(forKey: RemoteClawWatchApp.screenshotModeDefaultsKey)
    ...
    var body: some Scene {
        WindowGroup {
            WatchInboxView(
                ...
                .task {
                    if RemoteClawWatchApp.isScreenshotMode {
