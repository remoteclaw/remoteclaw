...
function createTempConfig(files: Record<string, string>): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-gateway-dispatch-config-"));
  tempDirs.push(dir);
  for (const [name, contents] of Object.entries(files)) {
    fs.writeFileSync(path.join(dir, name), contents);
  }
  return path.join(dir, "remoteclaw.json5");
}
  ...
  it("reads only gateway dispatch fields from JSON5 config with includes and env vars", () => {
    const configPath = createTempConfig({
      "gateway-base.json5": `{
        gateway: {
          port: 18888,
          auth: { mode: "token", token: "\${REMOTECLAW_GATEWAY_TOKEN}" },
        },
        models: { providers: { expensive: { apiKey: "\${MISSING_MODEL_KEY}" } } },
      }`,
      "remoteclaw.json5": `{
        $include: "./gateway-base.json5",
        env: { vars: { REMOTECLAW_GATEWAY_TOKEN: "inline-token" } },
        agents: {
    ...
    const env = { REMOTECLAW_CONFIG_PATH: configPath };
  ...
  it("loads only gateway credential shell env keys on explicit fallback", async () => {
    const configPath = createTempConfig({
      "remoteclaw.json5": `{
        ...
        gateway: { auth: { mode: "token", token: "\${REMOTECLAW_GATEWAY_TOKEN}" } },
      }`,
    });
    const env: NodeJS.ProcessEnv = { REMOTECLAW_CONFIG_PATH: configPath };
    shellEnvMocks.loadShellEnvFallback.mockImplementation(({ env: targetEnv }) => {
      targetEnv.REMOTECLAW_GATEWAY_TOKEN = "shell-token";
    });
    ...
    expect(shellEnvMocks.loadShellEnvFallback).toHaveBeenCalledWith({
      enabled: true,
      env,
      expectedKeys: ["REMOTECLAW_GATEWAY_TOKEN", "REMOTECLAW_GATEWAY_PASSWORD"],
      logger: console,
      timeoutMs: 123,
    });
