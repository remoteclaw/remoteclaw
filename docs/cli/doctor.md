...
- Doctor repairs missing bundled plugin runtime dependencies without writing into packaged global installs. For root-owned npm installs or hardened systemd units, set `REMOTECLAW_PLUGIN_STAGE_DIR` to a writable directory such as `/var/lib/remoteclaw/plugin-runtime-deps`; it can also be a path-list such as `/opt/remoteclaw/plugin-runtime-deps:/var/lib/remoteclaw/plugin-runtime-deps`, where earlier roots are read-only lookup layers and the final root is the repair target.
