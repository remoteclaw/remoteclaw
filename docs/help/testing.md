...
```bash
tmp=$(mktemp -d /tmp/remoteclaw-telegram-user.XXXXXX)
...
pnpm qa:telegram-user:crabbox -- start \
  --tdlib-url http://artifacts.remoteclaw.org/tdlib-v1.8.0-linux-x64.tgz \
...
- `run --session <file> -- <remote command>` runs an arbitrary command on the Crabbox and saves its output, for example `bash -lc 'source /tmp/remoteclaw-telegram-user-crabbox/env.sh && python3 /tmp/remoteclaw-telegram-user-crabbox/user-driver.py transcript --limit 20 --json'`.
...
- `pnpm test:live` now defaults to a quieter mode: it keeps `[live] ...` progress output, but suppresses the extra `~/.profile` notice and mutes gateway bootstrap logs/Bonjour chatter. Set `REMOTECLAW_LIVE_TEST_QUIET=0` if you want the full startup logs back.
