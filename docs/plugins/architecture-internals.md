...
metadata from the persisted installed plugin index, RemoteClaw also keeps a small
...
- Set `REMOTECLAW_DISABLE_INSTALLED_PLUGIN_MANIFEST_REGISTRY_CACHE=1` to disable
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-remoteclaw-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-remoteclaw-scopes` only when the header is explicitly present
  - if `x-remoteclaw-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-remoteclaw-scopes` header contract.
...
<Info>
`remoteclaw/plugin-sdk/channel-runtime` is deprecated — a compatibility shim for
...
```ts
import { buildMemorySystemPromptAddition } from "remoteclaw/plugin-sdk/core";
