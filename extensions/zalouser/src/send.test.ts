...
describe("zalouser send helpers", () => {
  ...
  it("uses env profile fallback and builds link command", async () => {
    ...
    const result = await sendLinkZalouser("thread-5", " https://remoteclaw.com ", {
      isGroup: true,
    });

    expect(mockRunZca).toHaveBeenCalledWith(
      ["msg", "link", "thread-5", "https://remoteclaw.com", "-g"],
      { profile: "env-profile" },
    );
  ...
  it("returns caught command errors", async () => {
    ...
    await expect(sendLinkZalouser("thread-6", "https://remoteclaw.com")).resolves.toEqual({
      ok: false,
