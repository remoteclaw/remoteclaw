...
```json5
{
  agent: { workspace: "~/.remoteclaw/workspace" },
  channels: { whatsapp: { allowFrom: ["+15555550123"] } },
}
```

Save to `~/.remoteclaw/remoteclaw.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.remoteclaw/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/remoteclaw" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.remoteclaw/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```
