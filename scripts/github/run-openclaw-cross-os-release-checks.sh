...
tsx_version="${REMOTECLAW_RELEASE_TSX_VERSION:-${TSX_VERSION:-4.21.0}}"
script_path="${REMOTECLAW_RELEASE_CHECKS_SCRIPT:-workflow/scripts/remoteclaw-cross-os-release-checks.ts}"
...
temp_root="${REMOTECLAW_RELEASE_TSX_TOOL_ROOT:-${RUNNER_TEMP:-${TMPDIR:-/tmp}}}"
if command -v cygpath >/dev/null 2>&1; then
  temp_root="$(cygpath -u "${temp_root}")"
fi

tool_dir="${REMOTECLAW_RELEASE_TSX_TOOL_DIR:-${temp_root}/remoteclaw-release-tsx-${tsx_version}}"
