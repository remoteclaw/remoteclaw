...
function extractStatusCode(err: unknown): number | null {
  ...
  // Only accept a real HTTP status: `parseInt` would happily read "404 Not
  // Found" or "4.5" as 404/4 and mis-route the error kind.
...
// `parseFloat` stops at the first non-numeric char, so "1500ms" would read as
// 1500 and a bogus header could stall the send queue. Require a clean decimal
// and cap it so the delay stays inside safe-integer milliseconds.
