...
import { normalizeTrimmedStringList } from "@remoteclaw/normalization-core/string-normalization";
...
function inferToolMeta(
  ...
  // caller-controlled detailMode is accepted for source compat but unused.
  _detailMode: "explain" | "raw" = "explain",
) {
  if (!name || !args) {
    return undefined;
  }
...
export function resolveChannelProgressDraftMaxLineChars(
  _entry: StreamingCompatEntry | null | undefined,
  defaultValue = DEFAULT_PROGRESS_DRAFT_MAX_LINE_CHARS,
): number {
  // ChannelStreamingProgressConfig exposes no per-line-char override in the fork;
  // the max-line-char limit is a fixed default (matches the plugin-sdk canonical).
  return defaultValue;
}
