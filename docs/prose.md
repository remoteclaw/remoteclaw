...
summary: "OpenProse is a markdown-first workflow format for multi-agent AI sessions. In RemoteClaw it ships as a plugin with a /prose slash command and a skill pack."
read_when:
  - You want to run or write .prose workflow files
  - You want to enable the OpenProse plugin
  - You need to understand how OpenProse maps to RemoteClaw primitives
---

OpenProse is a portable, markdown-first workflow format for orchestrating AI
sessions. In RemoteClaw it ships as a plugin that installs an OpenProse skill
...
<Steps>
  ...
  <Step title="Verify">
    ```bash
    remoteclaw plugins list | grep prose
    ```
...
| OpenProse concept         | RemoteClaw tool  |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review them before running. Use RemoteClaw tool
...
<CardGroup cols={2}>
  ...
  <Card title="Subagents" href="/tools/subagents" icon="users">
    RemoteClaw's native multi-agent coordination layer.
  </Card>
