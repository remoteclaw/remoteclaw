...
read_when:
  - Planning a broad RemoteClaw application modernization pass
...
Use this section to update the frontend-focused `SKILL.md` supplied with the modernization task. If adopting this guidance as a repo-local RemoteClaw skill, create `.agents/skills/remoteclaw-frontend/SKILL.md` first, keep the frontmatter that belongs in that target skill, then add or replace the body guidance with the following content.
