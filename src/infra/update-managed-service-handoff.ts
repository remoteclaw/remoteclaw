...
export function formatManagedServiceUpdateCommand(params?: {
  ...
  const args = ["remoteclaw", "update", "--yes"];
  if (params?.channel) {
...
function resolveGatewayServiceRecovery(
  ...
  if (supervisor === "launchd") {
    const label =
      env.REMOTECLAW_LAUNCHD_LABEL?.trim() ||
      resolveGatewayLaunchAgentLabel(env.REMOTECLAW_PROFILE);
...
function buildSystemdHandoffUnitName(handoffId: string | undefined): string {
  const suffix =
    ...
    "handoff";
  return `remoteclaw-update-${suffix}.scope`;
}
...
async function resolveHandoffSpawn(params: {
  ...
  if (!systemdRunPath) {
    throw new Error(
      "systemd-run is required to start the managed update handoff outside remoteclaw-gateway.service",
    );
  }
...
export async function startManagedServiceUpdateHandoff(params: {
  ...
  const env = {
    ...
    REMOTECLAW_UPDATE_RUN_HANDOFF: "1",
  };
...
export function buildManagedServiceHandoffUnavailableMessage(command: string): string {
  return [
    "RemoteClaw updates cannot safely run inside the live gateway process without a managed-service handoff.",
