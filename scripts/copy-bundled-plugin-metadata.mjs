...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "remoteclaw.plugin.json");
    ...
    if (packageJson.remoteclaw && "extensions" in packageJson.remoteclaw) {
      packageJson.remoteclaw = {
        ...packageJson.remoteclaw,
        extensions: rewritePackageExtensions(packageJson.remoteclaw.extensions),
        ...(typeof packageJson.remoteclaw.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.remoteclaw.setupEntry) }
          : {}),
      };
    }
