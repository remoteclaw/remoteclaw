...
describe("dependency denylist guardrails", () => {
  ...
  // Skipped: the fork's root package.json does not pin `axios` via
  // `overrides` or `pnpm.overrides` (upstream did). Re-enable if the fork
  // adopts an axios pin policy.
  it.skip("pins the axios override to an exact version", () => {
