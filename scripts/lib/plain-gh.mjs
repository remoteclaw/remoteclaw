...
export function resolvePlainGhBin(env = process.env) {
  if (env.REMOTECLAW_GH_BIN) {
    if (isExecutable(env.REMOTECLAW_GH_BIN)) {
      return env.REMOTECLAW_GH_BIN;
    }
    throw new Error(`REMOTECLAW_GH_BIN is not executable: ${env.REMOTECLAW_GH_BIN}`);
  }
