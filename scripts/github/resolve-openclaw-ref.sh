...
REMOTE_URL="${REMOTECLAW_REF_REMOTE:-https://github.com/remoteclaw/remoteclaw.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-remoteclaw-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves RemoteClaw branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve RemoteClaw ref: ${REF}" >&2
exit 1
