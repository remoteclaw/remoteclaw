...
function createPreflightArgs(params: {
  cfg: import("../../../../src/config/config.js").RemoteClawConfig;
...
describe("preflightDiscordMessage", () => {
        ...
        ref.conversationId === "user:user-1"
          ? createThreadBinding({
              ...
              metadata: {
                ...
                pluginRoot: "/Users/huntharo/github/remoteclaw-app-server",
              },
            })
          : null,
    });
  ...
  it("bypasses mention gating in bound threads for allowed bot senders", async () => {
    ...
    const result = await preflightDiscordMessage(
      createPreflightArgs({
        cfg: {
          ...DEFAULT_PREFLIGHT_CFG,
        } as import("../../../../src/config/config.js").RemoteClawConfig,
        discordConfig: {
          allowBots: true,
        } as DiscordConfig,
        data: createGuildEvent({
  ...
  it("allows bot messages with explicit mention when allowBots=mentions", async () => {
    ...
    const message = createDiscordMessage({
      ...
      channelId,
      content: "hi <@remoteclaw-bot>",
      mentionedUsers: [{ id: "remoteclaw-bot" }],
      author: {
        id: "relay-bot-1",
        bot: true,
        username: "Relay",
  ...
  it("uses attachment content_type for guild audio preflight mention detection", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hey remoteclaw");
