    ...
    try {
      ...
      try {
        const fallbackResult = await runWithModelFallback({
          ...
          run: async (provider: string, model: string, runOptions: Record<string, unknown>) => {
            ...
            const result = await runEmbeddedPiAgent({
              ...
              bootstrapPromptWarningSignature: Array.from(bootstrapPromptWarningSignaturesSeen)[
                bootstrapPromptWarningSignaturesSeen.size - 1
              ],
              onAgentEvent: (evt: { stream: string; data: { phase?: unknown } }) => {
