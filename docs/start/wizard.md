...
The setup wizard is the **recommended** way to set up RemoteClaw on macOS,
...
`remoteclaw dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`remoteclaw configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.remoteclaw/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `remoteclaw doctor` first.
</Note>
...
- Default workspaces follow `~/.remoteclaw/workspace-<agentId>`.
...
- CLI command reference: [`remoteclaw onboard`](/cli/onboard)
