...
Matrix is the Matrix channel plugin for RemoteClaw.
It uses the official `matrix-js-sdk` and supports DMs, rooms, threads, media, reactions, polls, location, and E2EE.

## Plugin required

Matrix is a plugin and is not bundled with core RemoteClaw.
...
Set `channels.matrix.streaming` to `"partial"` when you want RemoteClaw to send a single draft reply,
...
If you intentionally want to discard the current room-key backup and start a new backup baseline for future messages, use `remoteclaw matrix verify backup reset --yes`.
