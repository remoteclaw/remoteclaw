...
const TOOLING_ISOLATED_TEST_TARGET = "test/scripts/remoteclaw-e2e-instance.test.ts";
...
const INCLUDE_FILE_ENV_KEY = "REMOTECLAW_VITEST_INCLUDE_FILE";
const FS_MODULE_CACHE_PATH_ENV_KEY = "REMOTECLAW_VITEST_FS_MODULE_CACHE_PATH";
...
const TOOLING_SOURCE_TEST_TARGETS = new Map([
  ...
  [
    ...
    [
      ...
      "test/remoteclaw-npm-release-check.test.ts",
      "test/scripts/check-gateway-watch-regression.test.ts",
      "test/scripts/check-remoteclaw-package-tarball.test.ts",
      "test/scripts/remoteclaw-cross-os-release-checks.test.ts",
    ],
  ],
  [
  ...
  ["scripts/lib/remoteclaw-test-state.mjs", ["test/scripts/remoteclaw-test-state.test.ts"]],
  [
    "scripts/lib/workspace-bootstrap-smoke.mjs",
    ["test/release-check.test.ts", "test/remoteclaw-npm-release-check.test.ts"],
  ],
  [
  ...
  [
    ...
    [
      ...
      "test/remoteclaw-npm-release-check.test.ts",
      "test/remoteclaw-npm-postpublish-verify.test.ts",
  ...
  ["scripts/lib/remoteclaw-release-clawhub-plan.ts", ["test/plugin-clawhub-release.test.ts"]],
  [
  ...
  [
    "scripts/remoteclaw-npm-prepublish-verify.ts",
    ["test/remoteclaw-npm-prepublish-verify.test.ts"],
  ],
  [
    "scripts/remoteclaw-npm-postpublish-verify.ts",
    ["test/remoteclaw-npm-postpublish-verify.test.ts"],
  ],
  ["scripts/remoteclaw-npm-release-check.ts", ["test/remoteclaw-npm-release-check.test.ts"]],
  ["scripts/remoteclaw-prepack.ts", ["test/remoteclaw-prepack.test.ts"]],
  [
    "scripts/check-remoteclaw-package-tarball.mjs",
    ["test/scripts/check-remoteclaw-package-tarball.test.ts"],
  ],
  ...
  [
    "scripts/package-remoteclaw-for-docker.mjs",
    ["test/scripts/package-remoteclaw-for-docker.test.ts"],
  ],
  ...
  [
    ...
    ["test/scripts/docker-build-helper.test.ts", "test/scripts/remoteclaw-test-state.test.ts"],
  ],
  ...
  [
    "scripts/remoteclaw-performance-source-summary.mjs",
    ["test/scripts/remoteclaw-performance-source-summary.test.ts"],
  ],
...
const TOOLING_TEST_TARGETS = new Map([
  ...
  ["test/scripts/remoteclaw-test-state.test.ts", ["test/scripts/remoteclaw-test-state.test.ts"]],
...
const SOURCE_TEST_TARGETS = new Map([
  ...
  ["src/test-utils/remoteclaw-test-state.ts", ["src/test-utils/remoteclaw-test-state.test.ts"]],
  [
...
const VITEST_NO_OUTPUT_HEARTBEAT_ENV_KEY = "REMOTECLAW_VITEST_NO_OUTPUT_HEARTBEAT_MS";
...
export function buildFullSuiteVitestRunPlans(args, cwd = process.cwd()) {
  ...
  const parallelShardCount = parsePositiveInt(
    process.env.REMOTECLAW_TEST_PROJECTS_PARALLEL,
    "REMOTECLAW_TEST_PROJECTS_PARALLEL",
  );
  const expandToProjectConfigs =
    process.env.REMOTECLAW_TEST_PROJECTS_LEAF_SHARDS === "1" ||
    (Number.isFinite(parallelShardCount) && parallelShardCount > 1) ||
    shouldUseLocalFullSuiteParallelByDefault(process.env);
  return fullSuiteVitestShards.flatMap((shard) => {
    if (
      process.env.REMOTECLAW_TEST_SKIP_FULL_EXTENSIONS_SHARD === "1" &&
...
export function shouldUseLocalFullSuiteParallelByDefault(env = process.env) {
  ...
  return (
    env.REMOTECLAW_TEST_PROJECTS_SERIAL !== "1" &&
    env.CI !== "true" &&
...
export function resolveParallelFullSuiteConcurrency(specCount, envInput, hostInfo) {
  ...
  const override = parsePositiveInt(
    env.REMOTECLAW_TEST_PROJECTS_PARALLEL,
    "REMOTECLAW_TEST_PROJECTS_PARALLEL",
  );
  if (override !== null) {
    return Math.min(override, specCount);
  }
  if (env.REMOTECLAW_TEST_PROJECTS_SERIAL === "1") {
    return 1;
  }
  if (isCiLikeEnv(env)) {
    return 1;
  ...
  if (
    env.REMOTECLAW_TEST_PROJECTS_LEAF_SHARDS !== "1" &&
  ...
  return plans.map((plan, index) => {
    const includeFilePath = plan.includePatterns
      ? path.join(
          ...
          `remoteclaw-vitest-include-${process.pid}-${Date.now()}-${index}.json`,
        )
      : null;
    return {
      config: plan.config,
