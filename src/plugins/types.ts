...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). RemoteClaw uses this to expose
 ...
 *
 * Runs after RemoteClaw resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common RemoteClaw auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and RemoteClaw wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before RemoteClaw builds its generic
...
/**
 * Provider-owned stream wrapper hook after RemoteClaw applies its generic
 ...
 *
 * Return `true` or `false` to override RemoteClaw's built-in provider cache TTL
 ...
 *
 * Runs only after RemoteClaw exhausts normal env/profile/config auth resolution
 ...
 *
 * Runs after RemoteClaw loads the discovered model catalog and merges configured
...
export type ProviderPlugin = {
   ...
   *
   * RemoteClaw calls this only from async model resolution paths. After it
  ...
  /**
   * Provider-owned stream wrapper applied after generic RemoteClaw wrappers.
   *
   ...
   *
   * Called after RemoteClaw resolves the raw configured credential but before the
   ...
   * hint than RemoteClaw's generic auth-store guidance.
   */
   ...
   * `errorMessage` when RemoteClaw should surface a provider-specific hint for
   ...
   * merging. RemoteClaw deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * RemoteClaw uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `remoteclaw doctor`.
   *
   ...
   *
   * RemoteClaw calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep RemoteClaw's generic error text.
   */
  buildAuthDoctorHint?: (
...
export type WebSearchProviderPlugin = {
  ...
  getConfiguredCredentialValue?: (config?: RemoteClawConfig) => unknown;
  setConfiguredCredentialValue?: (configTarget: RemoteClawConfig, value: unknown) => void;
  applySelectionConfig?: (config: RemoteClawConfig) => RemoteClawConfig;
...
export type PluginFormat = "remoteclaw" | "bundle";
