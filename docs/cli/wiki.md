---
summary: "CLI reference for `remoteclaw wiki` (memory-wiki vault status, search, compile, lint, apply, bridge, and Obsidian helpers)"
read_when:
  - You want to use the memory-wiki CLI
  - You are documenting or changing `remoteclaw wiki`
title: "Wiki"
---
...
Use `remoteclaw wiki` when you want a compiled knowledge vault with:
...
remoteclaw wiki search "who should I ask about Teams?" --mode route-question
remoteclaw wiki get entity.alpha --from 1 --lines 80

remoteclaw wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

remoteclaw wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

remoteclaw wiki bridge import
remoteclaw wiki unsafe-local import

remoteclaw wiki obsidian status
remoteclaw wiki obsidian search "alpha"
remoteclaw wiki obsidian open syntheses/alpha-summary.md
remoteclaw wiki obsidian command workspace:quick-switcher
remoteclaw wiki obsidian daily
```
...
For one broad shared recall pass, prefer `remoteclaw memory search` when the
...
```bash
remoteclaw wiki search "bgroux" --mode find-person
remoteclaw wiki search "who knows Teams rollout?" --mode route-question
remoteclaw wiki search "maintainer-whois" --mode source-evidence
remoteclaw wiki search "strong route Teams" --mode raw-claim --json
```
...
remoteclaw wiki get syntheses/alpha-summary.md --from 1 --lines 80
```
...
`remoteclaw wiki` behavior is shaped by:
