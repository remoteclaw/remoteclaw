...
describe("gateway plugin HTTP auth boundary", () => {
  ...
  // Upstream v2026.3.2 — plugin HTTP auth enforcement changed
  test.skip("requires gateway auth for protected plugin route space and allows authenticated pass-through", async () => {
    const handlePluginRequest = vi.fn(async (req: IncomingMessage, res: ServerResponse) => {
      const pathname = new URL(req.url ?? "/", "http://localhost").pathname;
  ...
  test.skip("keeps wildcard plugin handlers ungated when auth enforcement predicate excludes their paths", async () => {
    const handlePluginRequest = vi.fn(async (req: IncomingMessage, res: ServerResponse) => {
      const pathname = new URL(req.url ?? "/", "http://localhost").pathname;
  ...
  test.skip("uses /api/channels auth by default while keeping wildcard handlers ungated with no predicate", async () => {
    const handlePluginRequest = vi.fn(async (req: IncomingMessage, res: ServerResponse) => {
      const pathname = new URL(req.url ?? "/", "http://localhost").pathname;
  ...
  test.skip("requires gateway auth for canonicalized /api/channels variants", async () => {
  ...
  test.skip("rejects unauthenticated plugin-channel fuzz corpus variants", async () => {
    const handlePluginRequest = createCanonicalizedChannelPluginHandler();
  ...
  test.skip("enforces auth before plugin handlers on encoded protected-path variants", async () => {
