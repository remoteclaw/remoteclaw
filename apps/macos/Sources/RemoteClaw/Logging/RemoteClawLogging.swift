...
enum RemoteClawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = RemoteClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = RemoteClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ...
        let label = RemoteClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct RemoteClawOSLogHandler: LogHandler {
...
struct RemoteClawFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = RemoteClawLogging.parseLabel(self.label)
