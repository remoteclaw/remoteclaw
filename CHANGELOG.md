...
- Install/update: allow package-manager installs from GitHub `main` via `remoteclaw update --tag main`, installer `--version main`, or direct npm/pnpm git specs.
...
- Browser/Chrome MCP: remove the legacy Chrome extension relay path, bundled extension assets, `driver: "extension"`, and `browser.relayBindHost`. Run `remoteclaw doctor --fix` to migrate host-local browser config to `existing-session` / `user`; Docker, headless, sandbox, and remote browser flows still use raw CDP. Thanks @vincentkoc.
...
- Plugins/install precedence: keep bundled plugins ahead of auto-discovered globals by default, but let an explicitly installed plugin record win its own duplicate-id tie so installed channel plugins load from `~/.remoteclaw/extensions` after `remoteclaw plugins install`.
...
- Browser/profiles: drop the auto-created `chrome-relay` browser profile; users who need the Chrome extension relay must now create their own profile via `remoteclaw browser create-profile`. (#45777) Thanks @odysseus0.
...
- Node/startup: remove leftover debug `console.log("node host PATH: ...")` that printed the resolved PATH on every `remoteclaw node run` invocation. (#46411)
