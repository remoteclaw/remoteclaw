...
- If clipboard and browser delivery fail, `remoteclaw dashboard` still prints the
  clean URL and tells you to use the token from `REMOTECLAW_GATEWAY_TOKEN` or
...
- **Shared-secret token source**: `gateway.auth.token` (or
  `REMOTECLAW_GATEWAY_TOKEN`); `remoteclaw dashboard` can pass it via URL fragment
...
- If `gateway.auth.token` is SecretRef-managed, `remoteclaw dashboard`
...
- If `gateway.auth.token` is configured as a SecretRef and is unresolved in your
  current shell, `remoteclaw dashboard` still prints a non-tokenized URL plus
  actionable auth setup guidance.
- **Shared-secret password**: use the configured `gateway.auth.password` (or
  `REMOTECLAW_GATEWAY_PASSWORD`). The dashboard does not persist passwords across
  reloads.
...
- Retrieve or supply the shared secret from the gateway host:
  ...
  - SecretRef-managed token: resolve the external secret provider or export
    `REMOTECLAW_GATEWAY_TOKEN` in this shell, then rerun `remoteclaw dashboard`
