...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["REMOTECLAW_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["REMOTECLAW_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["REMOTECLAW_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.REMOTECLAW_COMPATIBILITY_HOST_VERSION);
