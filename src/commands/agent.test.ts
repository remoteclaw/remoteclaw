...
describe("agentCommand", () => {
  ...
  it("honors explicit senderIsOwner for ingress runs", async () => {
    await withTempHome(async (home) => {
      ...
      const ingressCall = vi.mocked(runEmbeddedPiAgent).mock.calls.at(-1)?.[0] as
        | Record<string, unknown>
        | undefined;
  ...
  it("uses default fallback list for session model overrides", async () => {
    await withTempHome(async (home) => {
      ...
      const attempts = vi.mocked(runEmbeddedPiAgent).mock.calls.map((call) => ({
        provider: (call[0] as Record<string, unknown> | undefined)?.provider,
        model: (call[0] as Record<string, unknown> | undefined)?.model,
      }));
