...
TMP_STAGE_DIR="$(mktemp -d -p "$TMP_IMAGE_DIR" remoteclaw-image.XXXXXX)"
...
# do not need to be traversable by $REMOTECLAW_USER.
cat "$TMP_IMAGE" | run_as_user "$REMOTECLAW_USER" env HOME="$REMOTECLAW_HOME" podman load
rm -rf "$TMP_STAGE_DIR"
trap - EXIT
