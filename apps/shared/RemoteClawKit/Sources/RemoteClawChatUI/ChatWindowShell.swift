...
/// panel keeps using `RemoteClawChatView` directly; this shell is the full
/// window experience.
@MainActor
public struct RemoteClawChatWindowShell: View {
    @State private var viewModel: RemoteClawChatViewModel
    @State private var sessionQuery = ""
    @State private var isConfirmingClearHistory = false
    private let userAccent: Color?

    public init(viewModel: RemoteClawChatViewModel, userAccent: Color? = nil) {
...
private struct ChatContextUsageMenu: View {
    let usage: RemoteClawChatContextUsage
    let onCompact: () -> Void
...
private struct ChatSessionSidebar: View {
    ...
    @State private var sessionPendingDeletion: RemoteClawChatSessionEntry?
    ...
    private func row(for session: RemoteClawChatSessionEntry) -> some View {
        HStack(spacing: 6) {
            VStack(alignment: .leading, spacing: 2) {
                Text(ChatSessionSidebarModel.displayName(for: session))
                    .font(RemoteClawChatTypography.body(size: 13, weight: .medium, relativeTo: .body))
                    .lineLimit(1)
    ...
    private func rowSubtitle(for session: RemoteClawChatSessionEntry) -> String? {
