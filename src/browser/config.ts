...
import {
  ...
  DEFAULT_REMOTECLAW_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_REMOTECLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "remoteclaw" profile exists in the profiles map.
 ...
 *
 * Note: this is an RemoteClaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for remoteclaw/remoteclaw CDP).
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT_REMOTECLAW_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_REMOTECLAW_BROWSER_PROFILE_NAME]
        ? DEFAULT_REMOTECLAW_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "remoteclaw";
