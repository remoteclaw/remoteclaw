...
function resolveAuth(cfg: RemoteClawConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  ...
  const password =
    env.REMOTECLAW_GATEWAY_PASSWORD?.trim() ||
...
async function resolveGatewayPasswordSecretRef(
  ...
  const hasPasswordEnvCandidate = Boolean(
    env.REMOTECLAW_GATEWAY_PASSWORD?.trim() || env.CLAWDBOT_GATEWAY_PASSWORD?.trim(),
  );
  ...
  if (mode !== "password") {
    const hasTokenCandidate =
      Boolean(env.REMOTECLAW_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim()) ||
