...
const mocks = vi.hoisted(() => ({
  loadConfig: vi.fn<() => RemoteClawConfig>(),
...
function createTalkConfig(apiKey: unknown): RemoteClawConfig {
  return {
    talk: {
      provider: "acme",
      providers: {
...
describe("talk.speak handler", () => {
  ...
  it("uses the active runtime config snapshot instead of the raw config snapshot", async () => {
    ...
    mocks.synthesizeSpeech.mockImplementation(
      async ({ cfg }: { cfg: RemoteClawConfig; text: string; disableFallback: boolean }) => {
