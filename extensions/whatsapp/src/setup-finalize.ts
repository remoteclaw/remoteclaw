...
type WhatsAppConfig = NonNullable<NonNullable<RemoteClawConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: RemoteClawConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: RemoteClawConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: RemoteClawConfig, accountId: string): string {
  if (
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for RemoteClaw" },
    ],
  });
...
export async function finalizeWhatsAppSetup(params: {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      `Run \`${formatCliCommand("remoteclaw channels login")}\` later to link WhatsApp.`,
      "WhatsApp",
    );
  }
