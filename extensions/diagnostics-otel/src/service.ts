...
const PRELOADED_OTEL_SDK_ENV = "REMOTECLAW_OTEL_PRELOADED";
...
function assignOtelModelContentAttributes(
  ...
  if (policy.inputMessages) {
    assignOtelContentAttribute(
      attributes,
      "remoteclaw.content.input_messages",
      event.inputMessages,
    );
  }
  if (policy.outputMessages) {
    assignOtelContentAttribute(
      attributes,
      "remoteclaw.content.output_messages",
      event.outputMessages,
    );
  }
  if (policy.systemPrompt) {
    assignOtelContentAttribute(attributes, "remoteclaw.content.system_prompt", event.systemPrompt);
  }
}
...
function assignOtelToolContentAttributes(
  ...
  if (policy.toolInputs) {
    assignOtelContentAttribute(attributes, "remoteclaw.content.tool_input", event.toolInput);
  }
  if (policy.toolOutputs) {
    assignOtelContentAttribute(attributes, "remoteclaw.content.tool_output", event.toolOutput);
  }
}
...
function assignOtelLogEventAttributes(
  ...
  for (const rawKey in eventAttributes) {
    ...
    assignOtelLogAttribute(attributes, `remoteclaw.${key}`, eventAttributes[rawKey]);
  }
}
...
function addTraceAttributes(
  ...
  attributes["remoteclaw.traceId"] = normalized.traceId;
  if (normalized.spanId) {
    attributes["remoteclaw.spanId"] = normalized.spanId;
  }
  if (normalized.parentSpanId) {
    attributes["remoteclaw.parentSpanId"] = normalized.parentSpanId;
  }
  if (normalized.traceFlags) {
    attributes["remoteclaw.traceFlags"] = normalized.traceFlags;
  }
}
...
export function createDiagnosticsOtelService(): RemoteClawPluginService {
  ...
  return {
    ...
    async start(ctx) {
      ...
      const meter = metrics.getMeter("remoteclaw");
      const tracer = trace.getTracer("remoteclaw");

      const tokensCounter = meter.createCounter("remoteclaw.tokens", {
        unit: "1",
        description: "Token usage by type",
      });
      const costCounter = meter.createCounter("remoteclaw.cost.usd", {
        unit: "1",
        description: "Estimated model cost (USD)",
      });
      const durationHistogram = meter.createHistogram("remoteclaw.run.duration_ms", {
        unit: "ms",
        description: "Agent run duration",
      });
      const contextHistogram = meter.createHistogram("remoteclaw.context.tokens", {
        unit: "1",
        description: "Context window size and usage",
      });
      const webhookReceivedCounter = meter.createCounter("remoteclaw.webhook.received", {
        unit: "1",
        description: "Webhook requests received",
      });
      const webhookErrorCounter = meter.createCounter("remoteclaw.webhook.error", {
        unit: "1",
        description: "Webhook processing errors",
      });
      const webhookDurationHistogram = meter.createHistogram("remoteclaw.webhook.duration_ms", {
        unit: "ms",
        description: "Webhook processing duration",
      });
      const messageQueuedCounter = meter.createCounter("remoteclaw.message.queued", {
        unit: "1",
        description: "Messages queued for processing",
      });
      const messageProcessedCounter = meter.createCounter("remoteclaw.message.processed", {
        unit: "1",
        description: "Messages processed by outcome",
      });
      const messageDurationHistogram = meter.createHistogram("remoteclaw.message.duration_ms", {
        unit: "ms",
        description: "Message processing duration",
      });
      const queueDepthHistogram = meter.createHistogram("remoteclaw.queue.depth", {
        unit: "1",
        description: "Queue depth on enqueue/dequeue",
      });
      const queueWaitHistogram = meter.createHistogram("remoteclaw.queue.wait_ms", {
        unit: "ms",
        description: "Queue wait time before execution",
      });
      const laneEnqueueCounter = meter.createCounter("remoteclaw.queue.lane.enqueue", {
        unit: "1",
        description: "Command queue lane enqueue events",
      });
      const laneDequeueCounter = meter.createCounter("remoteclaw.queue.lane.dequeue", {
        unit: "1",
        description: "Command queue lane dequeue events",
      });
      const sessionStateCounter = meter.createCounter("remoteclaw.session.state", {
        unit: "1",
        description: "Session state transitions",
      });
      const sessionStuckCounter = meter.createCounter("remoteclaw.session.stuck", {
        unit: "1",
        description: "Sessions stuck in processing",
      });
      const sessionStuckAgeHistogram = meter.createHistogram("remoteclaw.session.stuck_age_ms", {
        unit: "ms",
      ...
      const modelCallDurationHistogram = meter.createHistogram(
        "remoteclaw.model_call.duration_ms",
        {
          unit: "ms",
      ...
      const toolExecutionDurationHistogram = meter.createHistogram(
        "remoteclaw.tool.execution.duration_ms",
        {
          unit: "ms",
          description: "Tool execution duration",
        },
      );
      const execProcessDurationHistogram = meter.createHistogram("remoteclaw.exec.duration_ms", {
        unit: "ms",
      ...
      if (logsEnabled) {
        ...
        const otelLogger = logProvider.getLogger("remoteclaw");
        recordLogRecord = (evt) => {
          try {
            const logLevelName = evt.level || "INFO";
            const severityNumber = logSeverityMap[logLevelName] ?? (9 as SeverityNumber);
            const attributes = Object.create(null) as Record<string, string | number | boolean>;
            assignOtelLogAttribute(attributes, "remoteclaw.log.level", logLevelName);
            if (evt.loggerName) {
              assignOtelLogAttribute(attributes, "remoteclaw.logger", evt.loggerName);
            }
            if (evt.loggerParents?.length) {
              assignOtelLogAttribute(
                attributes,
                "remoteclaw.logger.parents",
      ...
      const addRunAttrs = (
        ...
        if (evt.provider) {
          spanAttrs["remoteclaw.provider"] = evt.provider;
        }
        if (evt.model) {
          spanAttrs["remoteclaw.model"] = evt.model;
        }
        if (evt.channel) {
          spanAttrs["remoteclaw.channel"] = evt.channel;
        }
        if (evt.trigger) {
          spanAttrs["remoteclaw.trigger"] = evt.trigger;
        }
      };
      ...
      const paramsSummaryAttrs = (
        ...
        return {
          "remoteclaw.tool.params.kind": summary.kind,
          ...("length" in summary ? { "remoteclaw.tool.params.length": summary.length } : {}),
        };
      };
      ...
      const recordModelUsage = (evt: Extract<DiagnosticEventPayload, { type: "model.usage" }>) => {
        const attrs = {
          ...
          "remoteclaw.provider": evt.provider ?? "unknown",
          "remoteclaw.model": evt.model ?? "unknown",
        };

        const usage = evt.usage;
        if (usage.input) {
          tokensCounter.add(usage.input, { ...attrs, "remoteclaw.token": "input" });
        }
        if (usage.output) {
          tokensCounter.add(usage.output, { ...attrs, "remoteclaw.token": "output" });
        }
        if (usage.cacheRead) {
          tokensCounter.add(usage.cacheRead, { ...attrs, "remoteclaw.token": "cache_read" });
        }
        if (usage.cacheWrite) {
          tokensCounter.add(usage.cacheWrite, { ...attrs, "remoteclaw.token": "cache_write" });
        }
        if (usage.promptTokens) {
          tokensCounter.add(usage.promptTokens, { ...attrs, "remoteclaw.token": "prompt" });
        }
        if (usage.total) {
          tokensCounter.add(usage.total, { ...attrs, "remoteclaw.token": "total" });
        }
        ...
        if (evt.context?.limit) {
          contextHistogram.record(evt.context.limit, {
            ...attrs,
            "remoteclaw.context": "limit",
          });
        }
        if (evt.context?.used) {
          contextHistogram.record(evt.context.used, {
            ...attrs,
            "remoteclaw.context": "used",
          });
        }
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "remoteclaw.tokens.input": usage.input ?? 0,
          "remoteclaw.tokens.output": usage.output ?? 0,
          "remoteclaw.tokens.cache_read": usage.cacheRead ?? 0,
          "remoteclaw.tokens.cache_write": usage.cacheWrite ?? 0,
          "remoteclaw.tokens.total": usage.total ?? 0,
        };

        const span = spanWithDuration("remoteclaw.model.usage", spanAttrs, evt.durationMs);
        span.end();
      };
      ...
      const recordWebhookProcessed = (
        ...
        if (evt.chatId !== undefined) {
          spanAttrs["remoteclaw.chatId"] = String(evt.chatId);
        }
        const span = spanWithDuration("remoteclaw.webhook.processed", spanAttrs, evt.durationMs);
        span.end();
      };
      ...
      const recordWebhookError = (
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "remoteclaw.error": redactedError,
        };
        if (evt.chatId !== undefined) {
          spanAttrs["remoteclaw.chatId"] = String(evt.chatId);
        }
        const span = tracer.startSpan("remoteclaw.webhook.error", {
      ...
      const recordMessageProcessed = (
        ...
        if (evt.messageId !== undefined) {
          spanAttrs["remoteclaw.messageId"] = String(evt.messageId);
        }
        if (evt.reason) {
          spanAttrs["remoteclaw.reason"] = redactSensitiveText(evt.reason);
        }
        const span = spanWithDuration("remoteclaw.message.processed", spanAttrs, evt.durationMs);
      ...
      const recordSessionStuck = (
        ...
        const spanAttrs: Record<string, string | number> = { ...attrs };
        spanAttrs["remoteclaw.queueDepth"] = evt.queueDepth ?? 0;
        spanAttrs["remoteclaw.ageMs"] = evt.ageMs;
        const span = tracer.startSpan("remoteclaw.session.stuck", { attributes: spanAttrs });
      ...
      const recordRunCompleted = (
        ...
        const attrs: Record<string, string | number> = {
          "remoteclaw.outcome": evt.outcome,
          "remoteclaw.provider": evt.provider ?? "unknown",
          "remoteclaw.model": evt.model ?? "unknown",
        };
        if (evt.channel) {
          attrs["remoteclaw.channel"] = evt.channel;
        }
        durationHistogram.record(evt.durationMs, attrs);
        if (!tracesEnabled) {
          return;
        ...
        if (evt.errorCategory) {
          spanAttrs["remoteclaw.errorCategory"] = lowCardinalityAttr(evt.errorCategory, "other");
        }
        const span = spanWithDuration("remoteclaw.run", spanAttrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
      ...
      const modelCallMetricAttrs = (
        ...
        "remoteclaw.provider": evt.provider,
        "remoteclaw.model": evt.model,
        "remoteclaw.api": lowCardinalityAttr(evt.api),
        "remoteclaw.transport": lowCardinalityAttr(evt.transport),
      });
      ...
      const recordModelCallCompleted = (
        ...
        if (evt.api) {
          spanAttrs["remoteclaw.api"] = evt.api;
        }
        if (evt.transport) {
          spanAttrs["remoteclaw.transport"] = evt.transport;
        }
        ...
        const span = spanWithDuration("remoteclaw.model.call", spanAttrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      };
      ...
      const recordModelCallError = (
        ...
        modelCallDurationHistogram.record(evt.durationMs, {
          ...
          "remoteclaw.errorCategory": lowCardinalityAttr(evt.errorCategory, "other"),
        });
        if (!tracesEnabled) {
          return;
        }
      ...
      const recordToolExecutionCompleted = (
        ...
        const attrs = {
          "remoteclaw.toolName": evt.toolName,
        ...
        const span = spanWithDuration("remoteclaw.tool.execution", spanAttrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      };
      ...
      const recordToolExecutionError = (
        ...
        addRunAttrs(spanAttrs, evt);
        if (evt.errorCode) {
          spanAttrs["remoteclaw.errorCode"] = lowCardinalityAttr(evt.errorCode, "other");
        }
        assignOtelToolContentAttributes(
          spanAttrs,
          evt as unknown as Record<string, unknown>,
      ...
      const recordExecProcessCompleted = (
        ...
        const attrs: Record<string, string | number> = {
          "remoteclaw.exec.target": evt.target,
          "remoteclaw.exec.mode": evt.mode,
          "remoteclaw.outcome": evt.outcome,
        };
        if (evt.failureKind) {
          attrs["remoteclaw.failureKind"] = evt.failureKind;
        }
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...attrs,
          "remoteclaw.exec.command_length": evt.commandLength,
        };
        if (typeof evt.exitCode === "number") {
          spanAttrs["remoteclaw.exec.exit_code"] = evt.exitCode;
        }
        if (evt.exitSignal) {
          spanAttrs["remoteclaw.exec.exit_signal"] = lowCardinalityAttr(evt.exitSignal, "other");
        }
        if (evt.timedOut !== undefined) {
          spanAttrs["remoteclaw.exec.timed_out"] = evt.timedOut;
        }

        const span = spanWithDuration("remoteclaw.exec", spanAttrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
