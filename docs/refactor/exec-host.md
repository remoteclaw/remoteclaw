---
description: "Refactor plan: exec host routing, node approvals, and headless runner"
read_when:
...
- **Node host state:** `~/.remoteclaw/node.json` (node id + pairing token).
...
Path: `~/.remoteclaw/exec-approvals.json`
...
- Unix socket at `~/.remoteclaw/exec-approvals.sock` (0600).
