...
export function applyZaiProviderConfig(
  ...
  const seen = new Set(existingModels.map((m: Record<string, unknown>) => m.id));
  ...
  const baseUrl = params?.endpoint
    ...
    : (typeof ((existingProvider ?? {}) as Record<string, unknown>)?.baseUrl === "string"
        ? ((existingProvider ?? {}) as Record<string, unknown>).baseUrl
        : "") || resolveZaiBaseUrl();
...
export function applyKimiCodeProviderConfig(cfg: RemoteClawConfig): RemoteClawConfig {
  ...
  const kimiModels = buildKimiCodingProvider().models;
  const defaultModel = Array.isArray(kimiModels)
    ? (kimiModels[0] as Record<string, unknown>)
    : undefined;
...
export function applySyntheticProviderConfig(cfg: RemoteClawConfig): RemoteClawConfig {
  ...
  const mergedModels = [
    ...
    ...syntheticModels.filter(
      (model) =>
        !existingModels.some((existing: Record<string, unknown>) => existing.id === model.id),
    ),
  ];
  const { apiKey: existingApiKey, ...existingProviderRest } = (existingProvider ?? {}) as Record<
    string,
...
export function applyQianfanProviderConfig(cfg: RemoteClawConfig): RemoteClawConfig {
  ...
  const existingBaseUrl =
    typeof ((existingProvider ?? {}) as Record<string, unknown>)?.baseUrl === "string"
      ? (existingProvider!.baseUrl as string).trim()
      : "";
