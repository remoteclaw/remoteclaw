...
```
remoteclaw message <subcommand> [flags]
```
...
- `remoteclaw message` resolves the selected channel to its owning plugin when `--channel` or a channel-prefixed target is present; otherwise it loads configured channel plugins for default-channel inference.
...
- On cache miss, RemoteClaw will attempt a live directory lookup when the provider supports it.
...
```
remoteclaw message send --channel discord \
...
```bash
remoteclaw message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
remoteclaw message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
remoteclaw message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
remoteclaw message poll --channel msteams \
...
```
remoteclaw message react --channel slack \
...
```
remoteclaw message react --channel signal \
...
```
remoteclaw message send --channel telegram --target @mychat --message "Choose:" \
...
```
remoteclaw message send --channel telegram --target 123456789 --message "Open app:" \
