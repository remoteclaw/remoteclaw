...
RemoteClaw’s Gateway exposes a simple HTTP endpoint for invoking a single tool directly. It is always enabled and uses Gateway auth plus tool policy. Like the OpenAI-compatible `/v1/*` surface, shared-secret bearer auth is treated as trusted operator access for the whole gateway.
...
- Trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"` on a private ingress) honor `x-remoteclaw-scopes` when present and otherwise fall back to the normal operator default scope set.
...
- `x-remoteclaw-message-channel: <channel>` (example: `slack`, `telegram`)
- `x-remoteclaw-account-id: <accountId>` (when multiple accounts exist)
