  ...
  for (const [index, job] of jobs.entries()) {
    ...
    if (legacyJobIdIssue) {
      ...
      state.deps.log.warn(
        ...
        "cron: job used legacy jobId field; normalized id in memory (run remoteclaw doctor --fix to persist canonical shape)",
      );
    }
    ...
    if (typeof hydrated.sessionTarget !== "string") {
      ...
      if (defaulted) {
        ...
        if (!state.warnedInvalidSessionTarget.has(dedupeKey)) {
          state.warnedInvalidSessionTarget.add(dedupeKey);
          state.deps.log.warn(
