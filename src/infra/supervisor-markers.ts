...
const SUPERVISOR_HINTS = {
  launchd: ["REMOTECLAW_LAUNCHD_LABEL"],
...
function isCurrentGatewayLaunchdJob(env: NodeJS.ProcessEnv): boolean {
  const expectedLabel = resolveGatewayLaunchAgentLabel(env.REMOTECLAW_PROFILE);
  if (
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const serviceKind = env.REMOTECLAW_SERVICE_KIND?.trim();
