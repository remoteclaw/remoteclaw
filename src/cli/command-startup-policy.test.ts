...
describe("command-startup-policy", () => {
  ...
  it("matches plugin preload policy", () => {
    ...
    expect(
      shouldLoadPluginsForCommandPath({
        argv: ["node", "remoteclaw", "agent", "--json"],
        commandPath: ["agent"],
        jsonOutputMode: true,
      }),
    ).toBe(false);
    expect(
      shouldLoadPluginsForCommandPath({
        argv: ["node", "remoteclaw", "agent", "--json", "--local"],
        commandPath: ["agent"],
        jsonOutputMode: true,
      }),
    ).toBe(true);
  ...
  it("uses process env banner suppression when startup env is omitted", () => {
    const originalHideBanner = process.env.REMOTECLAW_HIDE_BANNER;
    try {
