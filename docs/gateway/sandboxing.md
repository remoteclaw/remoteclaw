---
summary: "How RemoteClaw sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
RemoteClaw can run **tools inside sandbox backends** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - By default, sandbox browser containers use a dedicated Docker network (`remoteclaw-sandbox-browser`) instead of the global `bridge` network.
  ...
  - noVNC observer access is password-protected by default; RemoteClaw emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
- Before `exec`, RemoteClaw syncs the local workspace into the OpenShell sandbox.
- After `exec`, RemoteClaw syncs the remote workspace back to the local workspace.
- File tools still operate through the sandbox bridge, but the local workspace remains the source of truth between turns.

Use this when:

- you edit files locally outside RemoteClaw and want those changes to show up in the sandbox automatically
...
- When the sandbox is first created, RemoteClaw seeds the remote workspace from the local workspace once.
...
- If you edit files on the host outside RemoteClaw after the seed step, the remote sandbox will **not** see those changes automatically.
...
- `remoteclaw sandbox list` shows OpenShell runtimes as well as Docker runtimes
- `remoteclaw sandbox recreate` deletes the current runtime and lets RemoteClaw recreate it on next use
...
- `"none"` (default): tools see a sandbox workspace under `~/.remoteclaw/sandboxes`.
...
RemoteClaw mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- RemoteClaw blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
...
Default Docker image: `remoteclaw-sandbox:bookworm-slim`
...
`remoteclaw-sandbox-common:bookworm-slim`.
...
- Use `remoteclaw sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
