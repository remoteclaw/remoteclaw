...
- Publish as `remoteclaw/plugin-sdk` (or export from core under `remoteclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `RemoteClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: RemoteClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: RemoteClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: RemoteClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: RemoteClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: RemoteClawConfig): string;
  };
};
```
...
- Add `api.runtime` to `RemoteClawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `remoteclawRuntime: ">=2026.2.0"`).
