...
function gatewayTokenConfig(config: RemoteClawConfig): RemoteClawConfig {
  return {
    ...config,
    gateway: {
      ...config.gateway,
...
function buildSnapshot(config: RemoteClawConfig): ConfigFileSnapshot {
  const raw = `${JSON.stringify(config, null, 2)}\n`;
  return buildTestConfigSnapshot({
    path: "/tmp/remoteclaw-startup-secrets-test.json",
    exists: true,
    raw,
    parsed: config,
    valid: true,
...
function preparedSnapshot(config: RemoteClawConfig): PreparedSecretsRuntimeSnapshot {
  return {
    sourceConfig: config,
    config,
    authStores: [],
...
describe("gateway startup config secret preflight", () => {
  const previousSkipChannels = process.env.REMOTECLAW_SKIP_CHANNELS;
  const previousSkipProviders = process.env.REMOTECLAW_SKIP_PROVIDERS;
