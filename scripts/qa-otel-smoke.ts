...
// Qa Otel Smoke script supports RemoteClaw repository automation.
...
const DEFAULT_DOCKER_COLLECTOR_IMAGE =
  process.env.REMOTECLAW_QA_OTEL_COLLECTOR_IMAGE || "otel/opentelemetry-collector:0.104.0";
...
const REQUIRED_METRIC_NAMES = ["remoteclaw.harness.duration_ms"] as const;
...
const MAX_OTLP_COMPRESSED_BODY_BYTES = readPositiveIntegerEnv(
  "REMOTECLAW_QA_OTEL_MAX_COMPRESSED_BODY_BYTES",
  2 * 1024 * 1024,
);
const MAX_OTLP_DECODED_BODY_BYTES = readPositiveIntegerEnv(
  "REMOTECLAW_QA_OTEL_MAX_DECODED_BODY_BYTES",
  8 * 1024 * 1024,
);
const MAX_CAPTURED_BODY_TEXT_BYTES = readPositiveIntegerEnv(
  "REMOTECLAW_QA_OTEL_MAX_CAPTURED_BODY_TEXT_BYTES",
  512 * 1024,
);
const QA_SUITE_TIMEOUT_MS = readPositiveIntegerEnv(
  "REMOTECLAW_QA_OTEL_SUITE_TIMEOUT_MS",
  10 * 60 * 1000,
);
const QA_SUITE_KILL_GRACE_MS = readPositiveIntegerEnv(
  "REMOTECLAW_QA_OTEL_SUITE_KILL_GRACE_MS",
  5000,
);
...
async function startDockerOtelCollector(
  ...
  const tempDir = await makeTempDir(path.join(osTmpdir(), "remoteclaw-otel-collector-"));
  const configPath = path.join(tempDir, "collector.yaml");
  const containerName = `remoteclaw-otel-smoke-${makeUuid()}`;
...
function remoteClawEntryArgs(): string[] {
  if (existsSync(path.join(process.cwd(), "scripts", "run-node.mjs"))) {
    return ["scripts/run-node.mjs"];
  }
  return ["remoteclaw.mjs"];
}

function spawnRemoteClaw(args: string[], env: NodeJS.ProcessEnv): ChildProcess {
  return spawn(process.execPath, [...remoteClawEntryArgs(), ...args], {
...
async function waitForChild(
  ...
  throw new Error(`remoteclaw qa suite timed out after ${timeoutMs}ms`);
}
...
function buildQaEnv(port: number): NodeJS.ProcessEnv {
  ...
  env.OTEL_SERVICE_NAME = "remoteclaw-qa-lab-otel-smoke";
  env.OTEL_SEMCONV_STABILITY_OPT_IN = "gen_ai_latest_experimental";
  env.REMOTECLAW_QA_SUITE_PROGRESS = env.REMOTECLAW_QA_SUITE_PROGRESS ?? "1";
  return env;
}
...
function isLatestGenAiModelCallSpan(span: CapturedSpan): boolean {
  ...
  return (
    ...
    typeof span.attributes["remoteclaw.provider"] === "string" &&
    typeof span.attributes["remoteclaw.model"] === "string"
  );
}
...
function assertSmoke(params: {
  ...
  if (spanNames.has("remoteclaw.model.call")) {
    failures.push("legacy remoteclaw.model.call span exported with GenAI semconv opt-in");
  }
  ...
  const contentKeys = [...attributeKeys].filter((key) => key.startsWith("remoteclaw.content."));
  ...
  const modelErrorSpans = modelSpans.filter((span) => {
    ...
    return (
      ...
      Object.hasOwn(span.attributes, "remoteclaw.errorCategory") ||
...
async function main() {
  ...
  try {
    ...
    const child = spawnRemoteClaw(buildQaArgs(options), buildQaEnv(exportPort));
