...
export function applyNonInteractiveGatewayConfig(params: {
  ...
  const envGatewayToken = normalizeGatewayTokenInput(process.env.REMOTECLAW_GATEWAY_TOKEN);
  ...
  // plaintext > ambient REMOTECLAW_GATEWAY_TOKEN > randomToken(). Ambient env
  ...
  if (authMode === "token") {
    if (gatewayTokenRefEnv) {
      ...
      if (!isValidEnvSecretRefId(gatewayTokenRefEnv)) {
        runtime.error(
          "Invalid --gateway-token-ref-env. Use an environment variable name like REMOTECLAW_GATEWAY_TOKEN.",
        );
        runtime.exit(1);
        return null;
      }
      ...
      if (!resolvedFromEnv) {
        runtime.error(
          `Environment variable "${gatewayTokenRefEnv}" is missing or empty. Export it first, then rerun ${formatCliCommand("remoteclaw onboard --non-interactive")}.`,
        );
        runtime.exit(1);
        return null;
      }
      nextConfig = {
        ...
        gateway: {
          ...
          auth: {
            ...
            token: {
              source: "env",
              provider: resolveDefaultSecretProviderAlias(
                nextConfig as Parameters<typeof resolveDefaultSecretProviderAlias>[0],
                "env",
                {
      ...
      // branch, an ambient REMOTECLAW_GATEWAY_TOKEN (or randomToken() fallback)
