...
describe("exec SecretRef id parity", () => {
  ...
  const sampledTargetsByClass = [
    ...
    .map((className) => {
      ...
      if (!selected) {
        throw new Error(`missing sampled target for class "${String(className)}"`);
      }
  ...
  for (const sample of sampledTargetsByClass) {
    it(`rejects traversal-segment exec ids for sampled class "${String(sample.className)}" (example: "${String(sample.id)}")`, () => {
      expect(
