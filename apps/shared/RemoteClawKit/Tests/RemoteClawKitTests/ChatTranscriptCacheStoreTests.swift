...
private func cacheMessage(
    ...
    text: String,
    timestamp: Double,
    idempotencyKey: String? = nil) -> RemoteClawChatMessage
{
...
private func cacheSessionEntry(key: String, updatedAt: Double) -> RemoteClawChatSessionEntry {
...
private func messageTexts(_ messages: [RemoteClawChatMessage]) -> [String] {
...
private func outboxCommand(
    ...
    thinking: String = "off",
    createdAt: Double = Date().timeIntervalSince1970) -> RemoteClawChatOutboxCommand
{
...
struct ChatTranscriptCacheStoreTests {
    @Test func `verified routing identity survives a cold store reopen`() async throws {
        ...
        let identity = try #require(RemoteClawChatSessionRoutingIdentity(
            scope: " Per-Sender ",
            mainSessionKey: " Work ",
            defaultAgentID: " Main "))
        let store = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")

        await store.storeSessionRoutingIdentity(identity)
        #expect(RemoteClawChatSQLiteTranscriptCache.loadSessionRoutingIdentity(
            databaseURL: url,
            gatewayID: "gw-a") == identity)

        let reopened = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
    ...
    @Test func `transcript keeps only most recent messages within bound`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let bound = RemoteClawChatSQLiteTranscriptCache.maxCachedMessagesPerSession
    ...
    @Test func `sessions list is bounded to most recently updated`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let bound = RemoteClawChatSQLiteTranscriptCache.maxCachedSessions
    ...
    @Test func `transcript eviction keeps most recent sessions`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let bound = RemoteClawChatSQLiteTranscriptCache.maxCachedTranscripts
    ...
    @Test func `transcripts are scoped per gateway identity`() async throws {
        let url = try makeDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let storeA = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let storeB = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-b")
    ...
    @Test func `reset retirement permits physical removal of all gateway rows`() async throws {
        ...
        await storeA.retire()
        await storeB.retire()
        RemoteClawChatSQLiteTranscriptCache.removeDatabaseFiles(at: url)
        await storeA.storeTranscript(
            sessionKey: "main",
            messages: [cacheMessage(role: "user", text: "late write", timestamp: 3)])

        let readerA = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let readerB = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-b")
    ...
    @Test func `removing one gateway database preserves another`() async throws {
        ...
        let storeA = RemoteClawChatSQLiteTranscriptCache(databaseURL: urlA, gatewayID: "gw-a")
        let storeB = RemoteClawChatSQLiteTranscriptCache(databaseURL: urlB, gatewayID: "gw-b")
        ...
        await storeA.retire()
        RemoteClawChatSQLiteTranscriptCache.removeDatabaseFiles(at: urlA)

        let readerA = RemoteClawChatSQLiteTranscriptCache(databaseURL: urlA, gatewayID: "gw-a")
    ...
    @Test func `v1 transcript cache migrates to durable outbox schema`() async throws {
        ...
        let migrated = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
    ...
    @Test func `v2 migration parks rows without a routing contract`() async throws {
        ...
        #expect(commands.map(\.lastError) == [
            RemoteClawChatSQLiteTranscriptCache.outboxUnknownTargetError,
            RemoteClawChatSQLiteTranscriptCache.outboxUnknownTargetError,
            RemoteClawChatSQLiteTranscriptCache.outboxUnconfirmedError,
            RemoteClawChatSQLiteTranscriptCache.outboxUnconfirmedError,
            RemoteClawChatSQLiteTranscriptCache.outboxUnknownTargetError,
            RemoteClawChatSQLiteTranscriptCache.outboxUnknownTargetError,
        ])
    ...
    @Test func `unknown schema preserves durable outbox bytes and fails closed`() async throws {
        ...
        let blocked = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        ...
        #expect(sqlite3_exec(
            raw,
            "PRAGMA user_version = \(RemoteClawChatSQLiteTranscriptCache.schemaVersion)",
            nil,
            nil,
            nil) == SQLITE_OK)
        sqlite3_close_v2(raw)
        let recovered = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
    ...
    @Test func `undecodable row is dropped and treated as miss`() async throws {
        ...
        let reader = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
...
struct ChatCommandOutboxStoreTests {
    ...
    @Test func `interrupted sending rows fail closed on recovery`() async throws {
        ...
        #expect(recovered.map(\.lastError) == [RemoteClawChatSQLiteTranscriptCache.outboxUnconfirmedError])
    ...
    @Test func `queued commands expire to failed at the staleness boundary`() async throws {
        ...
        let store = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let now = Date().timeIntervalSince1970
        let maxAge = RemoteClawChatSQLiteTranscriptCache.outboxCommandMaxAge
        #expect(await store.enqueueCommand(
        ...
        #expect(stale.lastError == RemoteClawChatSQLiteTranscriptCache.outboxExpiredError)
        #expect(fresh.status == .queued)
        #expect(fresh.lastError == nil)
        #expect(unconfirmed.status == .failed)
        #expect(unconfirmed.lastError == RemoteClawChatSQLiteTranscriptCache.outboxUnconfirmedError)
    }
    ...
    @Test func `enqueue refuses beyond the queue bound`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let bound = RemoteClawChatSQLiteTranscriptCache.maxQueuedCommands
    ...
    @Test func `attachment byte admission enforces command and gateway bounds`() {
        let commandBound = RemoteClawChatSQLiteTranscriptCache.maxAttachmentBytesPerCommand
        let gatewayBound = RemoteClawChatSQLiteTranscriptCache.maxQueuedAttachmentBytes

        #expect(RemoteClawChatSQLiteTranscriptCache.canEnqueueAttachmentBytes(
    ...
    @Test func `enqueue persists attachment bytes and refuses a full byte budget`() async throws {
        let url = try makeDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let attachment = RemoteClawChatOutboxAttachment(
            type: "file",
            mimeType: "audio/mp4",
        ...
        #expect(sqlite3_exec(
            ...
            SET attachment_bytes = \(RemoteClawChatSQLiteTranscriptCache.maxQueuedAttachmentBytes)
        ...
        let fullStore = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
