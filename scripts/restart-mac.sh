#!/usr/bin/env bash
# Reset RemoteClaw like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
source "${ROOT_DIR}/scripts/lib/restart-mac-gateway.sh"
APP_BUNDLE="${REMOTECLAW_APP_BUNDLE:-}"
APP_EXECUTABLE_RELATIVE_PATH="Contents/MacOS/RemoteClaw"
...
WAIT_FOR_LOCK=0
LOG_PATH="${REMOTECLAW_RESTART_LOG:-${TMPDIR:-/tmp}/remoteclaw-restart-${LOCK_KEY}.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${REMOTECLAW_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.remoteclaw/disable-launchagent"
ATTACH_ONLY=1
...
canonicalize_app_bundle() {
  ...
  if [[ ! -d "${APP_BUNDLE}" ]]; then
    fail "REMOTECLAW_APP_BUNDLE does not exist: ${APP_BUNDLE}"
  fi
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log "  rm ~/.remoteclaw/disable-launchagent"
      log ""
...
kill_all_remoteclaw() {
  ...
  [[ -z "$(remoteclaw_process_pids)" ]]
}

known_remoteclaw_executables() {
  if [[ -n "${APP_BUNDLE}" ]]; then
    printf '%s\n' "${APP_BUNDLE}/${APP_EXECUTABLE_RELATIVE_PATH}"
  fi
  printf '%s\n' \
    "${ROOT_DIR}/dist/RemoteClaw.app/${APP_EXECUTABLE_RELATIVE_PATH}" \
    "/Applications/RemoteClaw.app/${APP_EXECUTABLE_RELATIVE_PATH}" \
    "${DEBUG_PROCESS_PATTERN}" \
    "${LOCAL_PROCESS_PATTERN}" \
    "${RELEASE_PROCESS_PATTERN}"
}

remoteclaw_process_pids() {
  local pattern=""
  while IFS= read -r pattern; do
    [[ -n "${pattern}" ]] || continue
    process_pids_matching "${pattern}"
  done < <(known_remoteclaw_executables) | sort -u
}
...
stop_launch_agent
log "==> Killing existing RemoteClaw instances"
if ! kill_all_remoteclaw; then
  fail "RemoteClaw instances did not exit after cleanup attempts"
fi
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product RemoteClaw"
...
choose_app_bundle() {
  ...
  if [[ -d "${ROOT_DIR}/dist/RemoteClaw.app" ]]; then
    APP_BUNDLE="$(cd "${ROOT_DIR}/dist/RemoteClaw.app" && pwd -P)"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/RemoteClaw.app missing Sparkle after packaging"
    fi
    return 0
  fi

  if [[ -d "/Applications/RemoteClaw.app" ]]; then
    APP_BUNDLE="$(cd "/Applications/RemoteClaw.app" && pwd -P)"
    return 0
  fi

  fail "App bundle not found. Set REMOTECLAW_APP_BUNDLE to your installed RemoteClaw.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".remoteclaw", "remoteclaw.json"), "utf8");
