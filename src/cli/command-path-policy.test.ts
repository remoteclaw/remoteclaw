...
describe("command-path-policy", () => {
  ...
  it("defaults unknown command paths to network proxy routing", () => {
    ...
    expect(resolveCliNetworkProxyPolicy(["node", "remoteclaw", "tool", "image_generate"])).toBe(
      "bypass",
    );
  ...
  it("resolves mixed network proxy policies from argv-sensitive catalog entries", () => {
    ...
    expect(resolveCliNetworkProxyPolicy(["node", "remoteclaw", "skills", "check"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "remoteclaw", "skills", "list"])).toBe("bypass");
    expect(
