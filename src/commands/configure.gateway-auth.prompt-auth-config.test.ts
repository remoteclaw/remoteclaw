...
const noopPrompter = {
  select: vi.fn(async (params: { options: { value: string }[] }) => params.options[0]?.value ?? ""),
  text: vi.fn(async () => ""),
...
// Gutted in RemoteClaw fork — promptAuthConfig no longer calls promptAuthChoiceGrouped/applyAuthChoice/promptModelAllowlist
describe.skip("promptAuthConfig", () => {
  it("keeps Kilo provider models while applying allowlist defaults", async () => {
    const result = await runPromptAuthConfigWithAllowlist();
    expect(
      (
        (result.models?.providers?.kilocode as Record<string, unknown>)?.models as
  ...
  it("does not mutate provider model catalogs when allowlist is set", async () => {
    ...
    expect(
      (
        (result.models?.providers?.minimax as Record<string, unknown>)?.models as
          | Record<string, unknown>[]
          | undefined
      )?.map((model: Record<string, unknown>) => model.id),
    ).toEqual(["MiniMax-M2.5"]);
  });
});
