...
describe("path-resolve helpers (direct-import coverage attribution)", () => {
  const envSnapshot = captureEnv(["REMOTECLAW_STATE_DIR"]);
  let stateDir = "";

  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-path-direct-"));
  ...
  it("resolveAuthStorePath falls back to resolveRemoteClawAgentDir when agentDir is omitted", () => {
    // Omitting agentDir exercises the `agentDir ?? resolveRemoteClawAgentDir()`
    // nullish branch. With REMOTECLAW_STATE_DIR set to our tempdir, the
  ...
  it("resolveAuthStorePathForDisplay preserves a tilde-rooted path unchanged", () => {
    ...
    const tildeAgentDir = "~fake-remoteclaw-no-expand";
...
describe("ensureAuthStoreFile (direct-import coverage attribution)", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-path-ensure-"));
    process.env.REMOTECLAW_STATE_DIR = stateDir;
  });
