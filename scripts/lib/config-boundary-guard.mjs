...
export function collectDeprecatedInternalConfigApiViolations({
  ...
  for (const { filePath, relPath } of productionExtensionFiles) {
    ...
    const guards = [
      {
        pattern:
          /\b(?:import|export)\s+(?:type\s+)?\{[^}]*\bloadConfig\b[^}]*\}\s+from\s+["']remoteclaw\/plugin-sdk\/(?:config-runtime|memory-core-host-runtime-core)["']/,
        replacement:
  ...
  for (const { filePath, relPath } of repoFiles.filter(
    ...
    const guards = [
      {
        pattern:
          /\b(?:import|export)\s+(?:type\s+)?\{[\s\S]*?\b(?:loadConfig|writeConfigFile)\b[\s\S]*?\}\s+from\s+["']remoteclaw\/plugin-sdk\/(?:config-runtime|memory-core-host-runtime-core)["']/,
        replacement:
          "use getRuntimeConfig(), runtime.config.current(), or mutation helpers with afterWrite",
      },
      {
        pattern:
          /ReturnType<typeof import\(["']remoteclaw\/plugin-sdk\/(?:config-runtime|memory-core-host-runtime-core)["']\)\.(?:loadConfig|writeConfigFile)>/,
