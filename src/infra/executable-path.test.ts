...
describe("executable path helpers", () => {
  it("detects executable files and rejects directories or non-executables", async () => {
    const base = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-exec-path-"));
  ...
  it("does not treat drive-less rooted windows paths as cwd-relative executables", () => {
    ...
    expect(
      resolveExecutablePath(String.raw`:\Users\demo\AI\system\remoteclaw\git.exe`, {
        cwd: String.raw`C:\Users\demo\AI\system\remoteclaw`,
      }),
    ).toBeUndefined();
    expect(
      resolveExecutablePath(String.raw`:/Users/demo/AI/system/remoteclaw/git.exe`, {
