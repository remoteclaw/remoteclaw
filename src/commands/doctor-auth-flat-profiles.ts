...
function listAuthProfileRepairCandidates(cfg: RemoteClawConfig): AuthProfileRepairCandidate[] {
  const candidates = new Map<string, AuthProfileRepairCandidate>();
  addCandidate(candidates, resolveAgentDir(cfg, resolveDefaultAgentId(cfg)));
...
function ensureConfigAuthProfiles(config: RemoteClawConfig): Record<string, AuthProfileConfig> {
...
export async function maybeRepairLegacyFlatAuthProfileStores(params: {
  ...
  const noteLines = [
    ...
    ...awsSdkMarkerStores.map(
      (entry) =>
        `- ${shortenHomePath(entry.authPath)} contains aws-sdk profile markers that belong in remoteclaw.json auth.profiles.`,
    ),
  ];
  if (legacyStores.length > 0) {
    noteLines.push(
      `- The gateway expects the canonical version/profiles store; ${formatCliCommand("remoteclaw doctor --fix")} rewrites this legacy shape with a backup.`,
    );
  }
  if (awsSdkMarkerStores.length > 0) {
    noteLines.push(
      `- AWS SDK profile markers are routing metadata, not stored credentials; ${formatCliCommand("remoteclaw doctor --fix")} moves them to config with a backup.`,
    );
  }
  note(noteLines.join("\n"), "Auth profiles");
  ...
  for (const entry of legacyStores) {
    try {
      ...
      saveAuthProfileStore(entry.store, entry.agentDir);
      result.changes.push(
