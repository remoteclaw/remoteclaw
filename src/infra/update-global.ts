...
/** Supported package managers for RemoteClaw global install and update flows. */
...
const PRIMARY_PACKAGE_NAME = "remoteclaw";
...
const PNPM_REMOTECLAW_BUILD_ALLOWLIST_FLAG = `--allow-build=${PRIMARY_PACKAGE_NAME}`;
...
function isPnpmRemoteClawSourceInstallSpec(spec: string): boolean {
...
/**
 * Extracts a pinned installed version from package specs like `remoteclaw@1.2.3`.
...
/**
 * Verifies that a global package root looks like a packaged RemoteClaw install
  ...
  await Promise.all(
    BUNDLED_RUNTIME_SIDECAR_PATHS.map(async (relativePath) => {
      ...
      if (
        ...
        (await pathExists(path.join(packageRoot, pluginRoot, "remoteclaw.plugin.json")))
      ) {
...
async function resolvePortableGitPathPrepend(): Promise<string[]> {
  ...
  const portableGitRoot = path.join(localAppData, "RemoteClaw", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  if (isMainPackageTarget(target)) {
    return REMOTECLAW_MAIN_PACKAGE_SPEC;
  }
 ...
 * Used for staged installs where RemoteClaw creates the prefix itself.
 */
...
/** Resolves the RemoteClaw package root under a package manager's global root. */
...
/**
 * Detects an installed global RemoteClaw package by probing package-manager roots
...
/**
 * Builds the primary package-manager argv for a global RemoteClaw install.
...
export function globalInstallArgs(
  ...
  if (resolved.manager === "pnpm") {
    return [
      ...
      ...(isPnpmRemoteClawSourceInstallSpec(spec) ? [PNPM_REMOTECLAW_BUILD_ALLOWLIST_FLAG] : []),
      spec,
    ];
  }
  if (resolved.manager === "bun") {
