...
#
# Suggested cron: */30 * * * * /path/to/remoteclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send RemoteClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., remoteclaw-alerts)
...
STATE_FILE="$HOME/.remoteclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via RemoteClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use remoteclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via RemoteClaw to $NOTIFY_PHONE..."
            remoteclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! RemoteClaw is down. Run on the RemoteClaw host: ${SCRIPT_DIR}/mobile-reauth.sh" "urgent"
    exit 1
