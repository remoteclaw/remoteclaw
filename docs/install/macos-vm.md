...
# RemoteClaw on macOS VMs (Sandboxing)
...
Run RemoteClaw in a sandboxed macOS VM on your existing Apple Silicon Mac using [Lume](https://cua.ai/docs/lume).
...
1. Install Lume
2. `lume create remoteclaw --os macos --ipsw latest`
3. Complete Setup Assistant, enable Remote Login (SSH)
4. `lume run remoteclaw --no-display`
5. SSH in, install RemoteClaw, configure channels
6. Done
...
```bash
lume create remoteclaw --os macos --ipsw latest
```
...
```bash
nano ~/.remoteclaw/remoteclaw.json
```
...
```bash
lume stop remoteclaw
lume run remoteclaw --no-display
```

The VM runs in the background. RemoteClaw's daemon keeps the gateway running.

To check status:

```bash
ssh youruser@192.168.64.X "remoteclaw status"
```
...
This is the killer feature of running on macOS. Use [BlueBubbles](https://bluebubbles.app) to add iMessage to RemoteClaw.
...
```bash
lume stop remoteclaw
lume clone remoteclaw remoteclaw-golden
```

Reset anytime:

```bash
lume stop remoteclaw && lume delete remoteclaw
lume clone remoteclaw-golden remoteclaw
lume run remoteclaw --no-display
```
...
| VM IP not showing        | Wait for VM to fully boot, run `lume get remoteclaw` again                           |
| Lume command not found   | Add `~/.local/bin` to your PATH                                                      |
| WhatsApp QR not scanning | Ensure you're logged into the VM (not host) when running `remoteclaw channels login` |
