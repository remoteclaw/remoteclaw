...
describe("Control UI build chunking", () => {
  it("groups stable runtime dependencies into bounded chunks", () => {
    ...
    expect(
      controlUiManualChunk("/tmp/remoteclaw-pnpm-node-modules/dompurify/dist/purify.es.mjs"),
    ).toBe("markdown-runtime");
    expect(controlUiManualChunk("/tmp/remoteclaw-pnpm-node-modules/zod/v4/core/schemas.js")).toBe(
      "config-runtime",
    );
    expect(controlUiManualChunk("/tmp/remoteclaw-pnpm-node-modules/json5/dist/index.js")).toBe(
      "config-runtime",
    );
    expect(controlUiManualChunk("/tmp/remoteclaw-pnpm-node-modules/@noble/ed25519/index.js")).toBe(
      "gateway-runtime",
    );
