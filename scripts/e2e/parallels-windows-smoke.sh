...
VM_NAME="Windows 11"
SNAPSHOT_HINT="pre-remoteclaw-native-e2e-2026-03-12"
MODE="both"
OPENAI_API_KEY_ENV="OPENAI_API_KEY"
INSTALL_URL="https://remoteclaw.ai/install.ps1"
HOST_PORT="18426"
...
MINGIT_ZIP_NAME=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/remoteclaw-parallels-windows.XXXXXX)"
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-remoteclaw-native-e2e-2026-03-12"
  --mode <fresh|upgrade|both>
  --openai-api-key-env <var> Host env var name for OpenAI API key.
                             Default: OPENAI_API_KEY
  --install-url <url>        Installer URL for latest release. Default: https://remoteclaw.ai/install.ps1
...
guest_run_remoteclaw() {
  ...
  stdout_name="remoteclaw-stdout-$RANDOM-$RANDOM.log"
  stderr_name="remoteclaw-stderr-$RANDOM-$RANDOM.log"
...
try {
  ...
  \$proc = Start-Process -FilePath (Join-Path \$env:APPDATA 'npm\remoteclaw.cmd') -ArgumentList $args_literal -NoNewWindow -PassThru -RedirectStandardOutput \$stdout -RedirectStandardError \$stderr
  \$proc.WaitForExit()
  if (Test-Path \$stdout) {
    Get-Content \$stdout
  }
...
req = urllib.request.Request(
    ...
    headers={
        "User-Agent": "remoteclaw-parallels-smoke",
...
ensure_guest_git() {
  ...
  guest_exec cmd.exe /d /s /c "if exist \"%LOCALAPPDATA%\\RemoteClaw\\deps\\portable-git\" rmdir /s /q \"%LOCALAPPDATA%\\RemoteClaw\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "if not exist \"%LOCALAPPDATA%\\RemoteClaw\\deps\" mkdir \"%LOCALAPPDATA%\\RemoteClaw\\deps\""
  guest_exec cmd.exe /d /s /c "mkdir \"%LOCALAPPDATA%\\RemoteClaw\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "curl.exe -fsSL \"$mingit_url\" -o \"%TEMP%\\$MINGIT_ZIP_NAME\""
  guest_exec cmd.exe /d /s /c "tar.exe -xf \"%TEMP%\\$MINGIT_ZIP_NAME\" -C \"%LOCALAPPDATA%\\RemoteClaw\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "del /q \"%TEMP%\\$MINGIT_ZIP_NAME\" & set \"PATH=%LOCALAPPDATA%\\RemoteClaw\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\RemoteClaw\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\RemoteClaw\\deps\\portable-git\\usr\\bin;%PATH%\" && git.exe --version"
}
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/remoteclaw-parallels-windows-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
& (Join-Path \$env:APPDATA 'npm\remoteclaw.cmd') --version
EOF
)"
}
...
install_main_tgz() {
  ...
  guest_exec cmd.exe /d /s /c "set \"PATH=%LOCALAPPDATA%\\RemoteClaw\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\RemoteClaw\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\RemoteClaw\\deps\\portable-git\\usr\\bin;%PATH%\" && curl.exe -fsSL \"$tgz_url\" -o \"%TEMP%\\$temp_name\" && npm.cmd install -g \"%TEMP%\\$temp_name\" --no-fund --no-audit && \"%APPDATA%\\npm\\remoteclaw.cmd\" --version"
}

verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_run_remoteclaw "" "" "--version")"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  ...
  runner_name="remoteclaw-onboard-$RANDOM-$RANDOM.ps1"
  log_name="remoteclaw-onboard-$RANDOM-$RANDOM.log"
  done_name="remoteclaw-onboard-$RANDOM-$RANDOM.done"
...
try {
  \$remoteclaw = Join-Path \$env:APPDATA 'npm\remoteclaw.cmd'
  \$cmdLine = ('"{0}" onboard --non-interactive --mode local --auth-choice openai-api-key --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --accept-risk --json > "{1}" 2>&1' -f \$remoteclaw, \$log)
...
verify_gateway() {
  guest_run_remoteclaw "" "" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_run_remoteclaw "" "" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_run_remoteclaw "" "" gateway status --deep --require-rpc
    return
  fi
  guest_run_remoteclaw "" "" gateway status --deep
}

verify_turn() {
  guest_run_remoteclaw "" "" agent --agent main --message "Reply with exact ASCII text OK only." --json
}
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "remoteclaw-main-fresh.tgz" || return $?
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "remoteclaw-main-upgrade.tgz" || return $?
