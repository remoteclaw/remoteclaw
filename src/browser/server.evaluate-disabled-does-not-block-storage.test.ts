  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          remoteclaw: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.REMOTECLAW_GATEWAY_PORT;
    process.env.REMOTECLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.REMOTECLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.REMOTECLAW_GATEWAY_PASSWORD;
