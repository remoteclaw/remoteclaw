...
describe("package scripts", () => {
  ...
  // The fork gutted the live-cache regression runner (src/agents/live-cache-regression-runner +
  // live-cache-test-support), so it ships no test:live:cache script; and it runs no Windows CI lane
  // (no windows workflow, and the format-generated-module/run-with-env test files do not exist here),
  // so the upstream test:live:cache and test:windows:ci coverage assertions do not apply.
  it("uses the shipped package launcher for npm start", () => {
    expect(readPackageJson().scripts.start).toBe("node remoteclaw.mjs");
  });
});
