// Runs RemoteClaw package update checks, package steps, and restart handoff.
...
const UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE_ENV =
  "REMOTECLAW_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE";
const PREFLIGHT_TEMP_PREFIX =
  process.platform === "win32" ? "ocu-pf-" : "remoteclaw-update-preflight-";
...
const DEV_PREFLIGHT_LINT_ENV: NodeJS.ProcessEnv = {
  ...
  REMOTECLAW_LOCAL_CHECK_MODE: "throttled",
  REMOTECLAW_OXLINT_SHARDS_SERIAL: "1",
};
const DEV_PREFLIGHT_LINT_OPT_IN_ENV = "REMOTECLAW_UPDATE_PREFLIGHT_LINT";
  ...
  if (gitRoot && pkgRoot && (await pathsReferToSameLocation(gitRoot, pkgRoot))) {
    ...
    try {
      ...
      const doctorStep = await runStep(
        step("remoteclaw doctor", doctorArgv, gitRoot, {
