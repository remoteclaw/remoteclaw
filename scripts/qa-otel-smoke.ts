...
function assertSmoke(params: {
  ...
  const modelSpans = params.spans.filter((span) => span.name === "remoteclaw.model.call");
  ...
  if (modelSpans.length === 0) {
    failures.push("no remoteclaw.model.call span was exported");
  }
...
async function main() {
  ...
  let childExitCode = 1;
  try {
    const child = spawnRemoteClaw(buildQaArgs(options), buildQaEnv(port));
