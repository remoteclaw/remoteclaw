...
// Gutted in RemoteClaw fork — listSkillCommandsForAgents is a no-op stub
describe.skip("registerTelegramNativeCommands skill allowlist integration", () => {
  afterEach(async () => {
  ...
  it("registers only allowlisted skills for the bound agent menu", async () => {
    const workspaceDir = await makeWorkspace("remoteclaw-telegram-skills-");
    await writeSkill({
