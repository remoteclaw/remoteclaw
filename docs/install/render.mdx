---
summary: "Deploy RemoteClaw on Render with Infrastructure-as-Code"
read_when:
  - Deploying RemoteClaw to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy RemoteClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/remoteclaw/remoteclaw)
...
repository configures everything needed to run RemoteClaw:
...
services:
  - type: web
    ...
    envVars:
      - key: REMOTECLAW_GATEWAY_PORT
        value: "8080"
      - key: REMOTECLAW_STATE_DIR
        value: /data/.remoteclaw
      - key: REMOTECLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: REMOTECLAW_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: remoteclaw-data
      mountPath: /data
      sizeGB: 1
```
...
Connect using the `REMOTECLAW_GATEWAY_TOKEN` value that was auto-generated during deploy (find it in **Dashboard → your service → Environment**).
...
If you use the original RemoteClaw repository, Render will not auto-deploy your RemoteClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For RemoteClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
- Missing `REMOTECLAW_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `REMOTECLAW_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export your config via `remoteclaw backup create` in the Render shell.
