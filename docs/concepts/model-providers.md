...
- Provider plugins can inject model catalogs via `registerProvider({ catalog })`;
  RemoteClaw merges that output into `models.providers` before writing
  `models.json`.
...
Provider plugins can now own most provider-specific logic while RemoteClaw keeps
the generic inference loop.

Typical split:

- `auth[].run` / `auth[].runNonInteractive`: provider owns onboarding/login
  flows for `remoteclaw onboard`, `remoteclaw models auth`, and headless setup
...
That covers providers that still fit RemoteClaw's normal transports. A provider
...
- `openai/gpt-5.3-codex-spark` is intentionally suppressed in RemoteClaw because the live OpenAI API rejects it; Spark is treated as Codex-only
...
- Direct API-key models support the shared `/fast` toggle and `params.fastMode`; RemoteClaw maps that to Anthropic `service_tier` (`auto` vs `standard_only`)
...
- Caution: Gemini CLI OAuth in RemoteClaw is an unofficial integration. Some users have reported Google account restrictions after using third-party clients. Review Google terms and use a non-critical account if you choose to proceed.
- Gemini CLI OAuth is shipped as part of the bundled `google` plugin.
  - Enable: `remoteclaw plugins enable google`
...
- Hugging Face Inference example model: `huggingface/deepseek-ai/DeepSeek-R1`; CLI: `remoteclaw onboard --auth-choice huggingface-api-key`. See [Hugging Face (Inference)](/providers/huggingface).
...
`remoteclaw onboard` and the model picker. See [/providers/ollama](/providers/ollama)
