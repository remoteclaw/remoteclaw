...
describe("warnLegacyRemoteClawEnvVars", () => {
  ...
  it("warns with counts and prefixes instead of secret-shaped env names", () => {
    ...
    expect(message).toContain("replacing the legacy prefix with REMOTECLAW_");
    ...
    expect(options).toEqual({
      code: "REMOTECLAW_LEGACY_ENV_VARS",
      type: "DeprecationWarning",
    });
  });

  it("does not warn for current REMOTECLAW names", () => {
    warnLegacyRemoteClawEnvVars({ REMOTECLAW_GATEWAY_TOKEN: "token" });
  ...
  it("warns only once after a successful emit", () => {
    ...
    warnLegacyRemoteClawEnvVars({ MOLTBOT_GATEWAY_TOKEN: "old-token" });
  ...
  it("retries if emitWarning throws before the warning is emitted", () => {
    ...
    expect(() => warnLegacyRemoteClawEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" })).toThrow(
      "warning sink failed",
    );
    warnLegacyRemoteClawEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" });
