...
const RETAINED_RUNTIME_DEPS_MANIFEST = ".remoteclaw-runtime-deps.json";
...
const PLUGIN_ROOT_INSTALL_STAGE_DIR = ".remoteclaw-install-stage";
const BUNDLED_RUNTIME_DEPS_LOCK_DIR = ".remoteclaw-runtime-deps.lock";
...
const MIRRORED_PACKAGE_RUNTIME_DEP_PLUGIN_ID = "remoteclaw-core";
...
function formatRuntimeDepsLockTimeoutMessage(params: {
  ...
  return (
    ...
    `ownerFilePath=${params.owner.ownerFilePath}). If no RemoteClaw/npm install is running, remove the lock directory and retry.`
  );
}
...
export function isWritableDirectory(dir: string): boolean {
  ...
  try {
    probeDir = fs.mkdtempSync(path.join(dir, ".remoteclaw-write-probe-"));
...
function resolveBundledRuntimeDepsExternalBaseDirs(env: NodeJS.ProcessEnv): string[] {
  const explicit = env.REMOTECLAW_PLUGIN_STAGE_DIR?.trim();
  if (explicit) {
...
function resolveExternalBundledRuntimeDepsInstallRoots(params: {
  ...
  const packageKey = `remoteclaw-${version}-${createPathHash(packageRoot)}`;
...
function resolveExistingExternalBundledRuntimeDepsRoots(params: {
  ...
  for (const externalBaseDir of externalBaseDirs) {
    ...
    return packageKey.startsWith("remoteclaw-")
...
function replaceNodeModulesDir(targetDir: string, sourceDir: string): void {
  ...
  const tempDir = fs.mkdtempSync(path.join(parentDir, ".remoteclaw-runtime-deps-copy-"));
...
function linkNodeModulesDir(targetDir: string, sourceDir: string): boolean {
  ...
  const tempLink = path.join(
    parentDir,
    `.remoteclaw-runtime-deps-link-${process.pid}-${Date.now()}`,
  );
  try {
...
function readBundledPluginRuntimeDepsManifest(
  ...
  const manifest = readJsonObject(path.join(pluginDir, "remoteclaw.plugin.json"));
...
export function resolveBundledRuntimeDependencyPackageInstallRootPlan(
  ...
  if (
    ...
    env.REMOTECLAW_PLUGIN_STAGE_DIR?.trim() ||
...
function ensureNpmInstallExecutionManifest(installExecutionRoot: string): void {
  ...
  fs.writeFileSync(
    manifestPath,
    `${JSON.stringify({ name: "remoteclaw-runtime-deps-install", private: true }, null, 2)}\n`,
    "utf8",
  );
}
...
export function installBundledRuntimeDeps(params: {
  ...
  try {
    ...
    // Always make npm see an RemoteClaw-owned package root. The package-level
    ...
    const installEnv = createBundledRuntimeDepsInstallEnv(params.env, {
      cacheDir: path.join(installExecutionRoot, ".remoteclaw-npm-cache"),
    });
