...
/// Animated RemoteClaw mascot. Redraws the canonical 120x120 vector from
/// `ui/public/favicon.svg` so individual parts (claws, antennae, eyes) can
/// animate like the remoteclaw.org hero mark; the bundled PNG asset cannot.
/// Styling (palette, glow colors, float depth) follows the remoteclaw.org hero
/// (`src/pages/index.astro` + `Layout.astro` theme variables).
public struct RemoteClawMascotView: View {
    ...
    public var body: some View {
        let palette = RemoteClawMascotPalette.forScheme(self.colorScheme)
        if self.reduceMotion {
            RemoteClawMascotCanvas(pose: .still, palette: palette)
        } else {
            TimelineView(.animation(minimumInterval: 1.0 / 30.0)) { timeline in
                let pose = RemoteClawMascotPose.at(time: timeline.date.timeIntervalSinceReferenceDate)
                ...
                GeometryReader { proxy in
                    RemoteClawMascotCanvas(pose: pose, palette: palette)
                        .offset(
                            y: self.floats
    ...
    /// remoteclaw.org hero drop-shadow color (`--logo-glow` / `--logo-glow-hover`).
...
/// Body/antenna colors from the remoteclaw.org theme variables: `:root` (dark)
/// and `html[data-theme='light']` in `Layout.astro`. Eye colors are fixed in
/// the site markup and shared by both themes.
struct RemoteClawMascotPalette: Equatable {
    let gradientTop: Color
    let gradientBottom: Color
    let antenna: Color

    static let dark = RemoteClawMascotPalette(
    ...
    static let light = RemoteClawMascotPalette(
    ...
    static func forScheme(_ colorScheme: ColorScheme) -> RemoteClawMascotPalette {
        colorScheme == .light ? .light : .dark
    }
}

/// Part transforms for one animation frame. Mirrors the remoteclaw.org CSS
/// keyframes: float 4s, antenna wiggle 2s, eye blink 3s, claw snap 4s with
/// the right claw trailing by 0.2s.
struct RemoteClawMascotPose: Equatable {
    ...
    static let still = RemoteClawMascotPose()

    static func at(time: TimeInterval) -> RemoteClawMascotPose {
        // Float depth matches the hero: -8px on a 100px mark = 8% of the 120 box.
        RemoteClawMascotPose(
...
private struct RemoteClawMascotCanvas: View {
    let pose: RemoteClawMascotPose
    let palette: RemoteClawMascotPalette
    ...
    private static func draw(
        ...
        size: CGSize,
        pose: RemoteClawMascotPose,
        palette: RemoteClawMascotPalette)
    {
    ...
    private static func gradient(
        for path: Path,
        palette: RemoteClawMascotPalette) -> GraphicsContext.Shading
    {
