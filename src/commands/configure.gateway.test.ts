...
describe("promptGatewayConfig", () => {
  ...
  it("adds Tailscale origin to controlUi.allowedOrigins when tailscale serve is enabled", async () => {
    ...
    expect(result.config.gateway?.controlUi?.allowedOrigins).toContain("https://my-host.tail1234.ts.net");
  });
  ...
  it("does not duplicate Tailscale origin if already present", async () => {
    ...
    const tsOriginCount = origins.filter((origin) => origin.toLowerCase() === "https://my-host.tail1234.ts.net").length;
  ...
  it("formats IPv6 Tailscale fallback addresses as valid HTTPS origins", async () => {
    ...
    expect(result.config.gateway?.controlUi?.allowedOrigins).toContain("https://[fd7a:115c:a1e0::12]");
  });

  it("stores gateway token as SecretRef when token source is ref", async () => {
    const previous = process.env.REMOTECLAW_GATEWAY_TOKEN;
    process.env.REMOTECLAW_GATEWAY_TOKEN = "env-gateway-token";
    try {
      const { call, result } = await runGatewayPrompt({
        selectQueue: ["loopback", "token", "off", "ref"],
        textQueue: ["18789", "REMOTECLAW_GATEWAY_TOKEN"],
        authConfigFactory: ({ mode, token }) => ({ mode, token }),
      });
      ...
      if (previous === undefined) {
        ...
        process.env.REMOTECLAW_GATEWAY_TOKEN = previous;
      }
    }
  });
});
