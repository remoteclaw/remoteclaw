...
// Plugin Clawhub Owner Preflight script supports RemoteClaw repository automation.

import { readFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
import { collectClawHubRemoteClawOwnerErrors } from "./lib/plugin-clawhub-release.ts";
...
export async function runClawHubOwnerPreflight(argv: string[]) {
  ...
  const errors = await collectClawHubRemoteClawOwnerErrors({ plugins: candidates });
  if (errors.length > 0) {
    throw new Error(
      `ClawHub RemoteClaw package ownership preflight failed:\n${errors.map((error) => `- ${error}`).join("\n")}`,
    );
  }

  console.log(`ClawHub RemoteClaw owner preflight passed for ${candidates.length} candidate(s).`);
}
