...
/// Replays scripted gateway traffic against `RemoteClawChatViewModel` with deterministic
...
private final class ScriptedChatTransport: @unchecked Sendable, RemoteClawChatTransport {
    private actor State {
        var history: RemoteClawChatHistoryPayload
        var historyRequestCount = 0
        var sentRunIds: [String] = []

        init(history: RemoteClawChatHistoryPayload) {
            self.history = history
        }

        func setHistory(_ payload: RemoteClawChatHistoryPayload) {
            self.history = payload
        }

        func recordHistoryRequest() -> RemoteClawChatHistoryPayload {
    ...
    func setHistory(_ payload: RemoteClawChatHistoryPayload) async {
        ...
        attachments _: [RemoteClawChatAttachmentPayload]) async throws -> RemoteClawChatSendResponse
    {
        ...
        return RemoteClawChatSendResponse(runId: idempotencyKey, status: "pending")
    }
    ...
    {
        RemoteClawChatSessionsListResponse(ts: nil, path: nil, count: 0, defaults: nil, sessions: [])
    }
...
/// `__remoteclaw.idempotencyKey` metadata used for turn correlation.
    ...
    idempotencyKey: String? = nil) -> AnyCodable
{
    ...
    if let idempotencyKey {
        message["__remoteclaw"] = ["idempotencyKey": idempotencyKey]
    }
...
private func replaySessionMessageEvent(
    ...
    messageId: String) -> RemoteClawChatTransportEvent
{
    .sessionMessage(
        RemoteClawSessionMessageEventPayload(
            sessionKey: "main",
...
private func replayFinalEvent(
    runId: String,
    text: String,
    timestamp: Double) -> RemoteClawChatTransportEvent
{
    .chat(
...
private func replayAssistantDeltaEvent(
    ...
    seq: Int) -> RemoteClawChatTransportEvent
{
    .agent(
...
private struct StreamReplayHarness {
    ...
    static func bootstrapped(
        initialHistory: RemoteClawChatHistoryPayload = replayHistory()) async throws -> StreamReplayHarness
    {
        let transport = ScriptedChatTransport(history: initialHistory)
        let vm = await MainActor.run {
            RemoteClawChatViewModel(sessionKey: "main", transport: transport)
        }
    ...
    func converge(
        _ label: String,
        _ condition: @escaping @MainActor @Sendable (RemoteClawChatViewModel) -> Bool) async throws
    {
        let vm = self.vm
...
extension RemoteClawChatViewModel {
    fileprivate var replayAssistantRows: [RemoteClawChatMessage] {
        self.messages.filter { $0.role == "assistant" }
    }

    fileprivate func replayAssistantRows(text: String) -> [RemoteClawChatMessage] {
        self.replayAssistantRows.filter { message in
            message.content.compactMap(\.text).joined() == text
        }
    }

    fileprivate var replayUserRows: [RemoteClawChatMessage] {
