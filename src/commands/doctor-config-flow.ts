...
// import ... from "../infra/exec-command-resolution.js";
// oxlint-disable-next-line typescript/no-explicit-any
const resolveCommandResolutionFromArgv = (..._args: unknown[]) => undefined as any;
// Gutted in RemoteClaw fork (Middleware Boundary Principle)
// import ... from "../infra/exec-safe-bin-runtime-policy.js";
// oxlint-disable-next-line typescript/no-explicit-any
const listInterpreterLikeSafeBins = (..._args: unknown[]) => undefined as any;
// oxlint-disable-next-line typescript/no-explicit-any
const resolveMergedSafeBinProfileFixtures = (..._args: unknown[]) => undefined as any;
// Gutted in RemoteClaw fork (Middleware Boundary Principle)
// import ... from "../infra/exec-safe-bin-trust.js";
// oxlint-disable-next-line typescript/no-explicit-any
const getTrustedSafeBinDirs = (..._args: unknown[]) => undefined as any;
// oxlint-disable-next-line typescript/no-explicit-any
const isTrustedSafeBinPath = (..._args: unknown[]) => undefined as any;
// oxlint-disable-next-line typescript/no-explicit-any
const normalizeTrustedSafeBinDirs = (..._args: unknown[]) => undefined as any;
...
function stripUnknownConfigKeys(config: RemoteClawConfig): {
...
function noteOpencodeProviderOverrides(cfg: RemoteClawConfig) {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: RemoteClawConfig): Promise<{
  config: RemoteClawConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: RemoteClawConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: RemoteClawConfig): {
  config: RemoteClawConfig;
  changes: string[];
} {
...
function scanMutableAllowlistEntries(cfg: RemoteClawConfig): MutableAllowlistHit[] {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: RemoteClawConfig): {
  config: RemoteClawConfig;
  changes: string[];
} {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: RemoteClawConfig = baseCfg;
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("remoteclaw doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("remoteclaw doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("remoteclaw doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
