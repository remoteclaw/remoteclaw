...
<Tabs>
  ...
  <Tab title="Local checkout">
    ```bash
    remoteclaw plugins install ./path/to/local/mattermost-plugin
    ```
  </Tab>
</Tabs>
...
<Steps>
  <Step title="Ensure plugin is available">
    Current packaged RemoteClaw releases already bundle it. Older/custom installs can add it manually with the commands above.
  </Step>
  ...
  <Step title="Configure RemoteClaw and start the gateway">
    Minimal config:
...
Native slash commands are opt-in. When enabled, RemoteClaw registers `oc_*` slash commands via the Mattermost API and receives callback POSTs on the gateway HTTP server.
...
<AccordionGroup>
  <Accordion title="Behavior notes">
    ...
    - Command callbacks are validated with the per-command tokens returned by Mattermost when RemoteClaw registers `oc_*` commands.
    - RemoteClaw refreshes current Mattermost command registration before accepting each callback so stale tokens from deleted or regenerated slash commands stop being accepted without a gateway restart.
...
RemoteClaw resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), RemoteClaw sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When RemoteClaw sends to a Mattermost DM target and needs to resolve the direct channel first, it retries transient direct-channel creation failures by default.
...
<AccordionGroup>
  ...
  <Accordion title="Streaming behavior notes">
    - If the stream cannot be finalized in place (for example the post was deleted mid-stream), RemoteClaw falls back to sending a fresh final post so the reply is never lost.
...
Normal agent replies can also include semantic `presentation` payloads. RemoteClaw renders value buttons as Mattermost interactive buttons, keeps URL buttons visible in the message text, and downgrades select menus to readable text.
...
<AccordionGroup>
  ...
  <Accordion title="Config and reachability">
    ...
    - If `interactions.callbackBaseUrl` is omitted, RemoteClaw derives the callback URL from `gateway.customBindHost` + `gateway.port`, then falls back to `http://localhost:<port>`.
    - Reachability rule: the button callback URL must be reachable from the Mattermost server. `localhost` only works when Mattermost and RemoteClaw run on the same host/network namespace.
...
The Mattermost plugin includes a directory adapter that resolves channel and user names via the Mattermost API. This enables `#channel-name` and `@username` targets in `remoteclaw message send` and cron/webhook deliveries.
...
<AccordionGroup>
  ...
  <Accordion title="Native slash commands fail">
    - `Unauthorized: invalid command token.`: RemoteClaw did not accept the callback token. Typical causes:
    ...
    - If `callbackUrl` is omitted and logs warn that the callback resolved to `http://127.0.0.1:18789/...`, that URL is probably only reachable when Mattermost runs on the same host/network namespace as RemoteClaw. Set an explicit externally reachable `commands.callbackUrl` instead.
