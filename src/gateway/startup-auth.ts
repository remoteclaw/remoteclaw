...
function hasGatewayTokenCandidate(params: {
  ...
  const envToken =
    params.env.REMOTECLAW_GATEWAY_TOKEN?.trim() || params.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return true;
  }
  if (
...
function hasGatewayPasswordEnvCandidate(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.REMOTECLAW_GATEWAY_PASSWORD?.trim() || env.CLAWDBOT_GATEWAY_PASSWORD?.trim());
}
