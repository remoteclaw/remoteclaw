  ...
  return {
    ...
    splitSetupEntries: (raw: string) =>
      raw
        ...
        .filter(Boolean),
    setSetupChannelEnabled: (cfg: RemoteClawConfig, channel: string, enabled: boolean) => ({
      ...cfg,
      channels: {
        ...cfg.channels,
        [channel]: {
          ...(cfg.channels?.[channel as keyof NonNullable<RemoteClawConfig["channels"]>] as object),
          enabled,
        },
      },
    }),
...
describe("whatsapp setup wizard", () => {
  beforeEach(() => {
    ...
    hoisted.resolveWhatsAppAuthDir.mockReturnValue({ authDir: "/tmp/remoteclaw-whatsapp-test" });
  });
  ...
  it("forces wildcard allowFrom for open policy without allowFrom follow-up prompts", async () => {
    ...
    const result = await runConfigureWithHarness({
      harness,
      cfg: createWhatsAppRootAllowFromConfig() as RemoteClawConfig,
    });
