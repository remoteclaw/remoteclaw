...
# RemoteClaw CLI installer (non-interactive, no onboarding)
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://remoteclaw.org/install-cli.sh | bash -s -- [--json] [--prefix <path>] [--version <ver>] [--node-version <ver>] [--onboard]
...
resolve_remoteclaw_effective_home() {
  local remoteclaw_home="${REMOTECLAW_HOME:-}"
  if [[ -z "$remoteclaw_home" ]]; then
    echo "$HOME"
    return 0
  fi

  case "$remoteclaw_home" in
    \~)
      echo "$HOME"
      ;;
    \~/*)
      echo "${HOME}/${remoteclaw_home#~/}"
      ;;
    *)
      echo "$remoteclaw_home"
      ;;
  esac
}

REMOTECLAW_EFFECTIVE_HOME="$(resolve_remoteclaw_effective_home)"
PREFIX="${REMOTECLAW_PREFIX:-${HOME}/.remoteclaw}"
REMOTECLAW_VERSION="${REMOTECLAW_VERSION:-latest}"
DEFAULT_NODE_VERSION="24.15.0"
ARMV7_DEFAULT_NODE_VERSION="22.22.3"
NODE_VERSION="${REMOTECLAW_NODE_VERSION:-${DEFAULT_NODE_VERSION}}"
NODE_VERSION_REQUESTED=0
if [[ -n "${REMOTECLAW_NODE_VERSION:-}" ]]; then
...
INSTALL_METHOD="${REMOTECLAW_INSTALL_METHOD:-npm}"
GIT_DIR="${REMOTECLAW_GIT_DIR:-${REMOTECLAW_EFFECTIVE_HOME}/remoteclaw}"
GIT_UPDATE="${REMOTECLAW_GIT_UPDATE:-1}"
JSON=0
RUN_ONBOARD=0
SET_NPM_PREFIX=0
PNPM_CMD=()
...
Usage: install-cli.sh [options]
  ...
  --prefix <path>                     Install prefix (default: ~/.remoteclaw; use \$REMOTECLAW_PREFIX to override)
  --install-method, --method npm|git  Install via npm (default) or from a git checkout
  --npm                               Shortcut for --install-method npm
  --git, --github                     Shortcut for --install-method git
  --git-dir, --dir <path>             Checkout directory (default: ~/remoteclaw, or \$REMOTECLAW_HOME/remoteclaw)
  --version <ver>                     RemoteClaw version (default: latest)
...
Environment variables:
  REMOTECLAW_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  REMOTECLAW_INSTALL_METHOD=git|npm
  REMOTECLAW_HOME=...
  REMOTECLAW_PREFIX=...
  REMOTECLAW_VERSION=latest|next|<semver>
  REMOTECLAW_GIT_DIR=...
  REMOTECLAW_GIT_UPDATE=0|1
EOF
}
...
cleanup_legacy_submodules() {
  local repo_dir="${1:-${REMOTECLAW_GIT_DIR:-${REMOTECLAW_EFFECTIVE_HOME}/remoteclaw}}"
...
resolve_git_remoteclaw_ref() {
  ...
  case "$requested" in
    ""|latest)
      resolved_version="$("$(npm_bin)" view "remoteclaw" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
install_remoteclaw() {
  ...
  if [[ "${requested}" == "latest" ]]; then
    if ! env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "remoteclaw@latest"; then
      ...
      env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "remoteclaw@next"
      requested="next"
    fi
  else
    env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "remoteclaw@${requested}"
  fi

  mkdir -p "${PREFIX}/bin"
  rm -f "${PREFIX}/bin/remoteclaw"
  cat > "${PREFIX}/bin/remoteclaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec "${PREFIX}/tools/node/bin/node" "$(node_dir)/lib/node_modules/remoteclaw/dist/entry.js" "\$@"
EOF
  chmod +x "${PREFIX}/bin/remoteclaw"
  emit_json "{\"event\":\"step\",\"name\":\"remoteclaw\",\"status\":\"ok\",\"version\":\"${requested}\"}"
}
...
install_remoteclaw_from_git() {
  local repo_dir="$1"
  local repo_url="https://github.com/remoteclaw/remoteclaw.git"
  ...
  emit_json "{\"event\":\"step\",\"name\":\"remoteclaw\",\"status\":\"start\",\"method\":\"git\",\"repo\":\"${repo_url//\"/\\\"}\"}"
  if [[ -d "$repo_dir/.git" ]]; then
    log "Installing Remoteclaw from git checkout: ${repo_dir}"
  else
    log "Installing Remoteclaw from GitHub (${repo_url})..."
  fi
...
EOF
  chmod +x "${PREFIX}/bin/remoteclaw"
  emit_json "{\"event\":\"step\",\"name\":\"remoteclaw\",\"status\":\"ok\",\"method\":\"git\"}"
}

resolve_remoteclaw_version() {
  local version=""
  if [[ -x "${PREFIX}/bin/remoteclaw" ]]; then
    version="$("${PREFIX}/bin/remoteclaw" --version 2>/dev/null | head -n 1 | tr -d '\r')"
  fi
  echo "$version"
}
...
refresh_gateway_service_if_loaded() {
  local claw="${PREFIX}/bin/remoteclaw"
  if [[ ! -x "$claw" ]]; then
    return 0
  fi
  ...
  if ! "$claw" gateway restart >/dev/null 2>&1; then
    ...
    log "Warning: gateway service restart failed; continuing. Run: remoteclaw gateway restart"
    return 0
  fi
...
main() {
  ...
  if [[ "${REMOTECLAW_NO_ONBOARD:-0}" == "1" ]]; then
    RUN_ONBOARD=0
  fi
  ...
  if [[ "$INSTALL_METHOD" == "git" ]]; then
    install_remoteclaw_from_git "$GIT_DIR"
  ...
  local installed_version
  installed_version="$(resolve_remoteclaw_version)"
  if [[ -n "$installed_version" ]]; then
    emit_json "{\"event\":\"done\",\"ok\":true,\"version\":\"${installed_version//\"/\\\"}\"}"
    log "RemoteClaw installed (${installed_version})."
  else
    emit_json "{\"event\":\"done\",\"ok\":true}"
    log "RemoteClaw installed."
  fi

  if [[ "$RUN_ONBOARD" -eq 1 ]]; then
    "${PREFIX}/bin/remoteclaw" onboard
  fi
}

if [[ "${REMOTECLAW_INSTALL_CLI_SH_NO_RUN:-0}" != "1" ]]; then
  main "$@"
fi
