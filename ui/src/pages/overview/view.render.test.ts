...
import { renderOverview, type OverviewProps } from "../../ui/views/overview.ts";
...
describe("overview view rendering", () => {
  ...
  it("renders a dedicated scope-upgrade approval hint with the exact approve command", async () => {
    ...
    expect(container.textContent).toContain("remoteclaw devices approve req-123");
  });
  ...
  it("does not suggest preview-only latest approval when the request id is absent", async () => {
    ...
    expect(container.textContent).toContain("Scope upgrade pending approval.");
    expect(container.textContent).toContain("remoteclaw devices list");
    expect(container.textContent).not.toContain("remoteclaw devices approve --latest");
  });
});
