...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds REMOTECLAW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `remoteclaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.REMOTECLAW_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.REMOTECLAW_GATEWAY_TOKEN?.trim() || undefined;
}
...
async function auditGatewayRuntime(
  ...
  if (isBunRuntime(execPath)) {
    issues.push({
      ...
      message: "Gateway service uses Bun; RemoteClaw runtime state requires node:sqlite.",
      detail: execPath,
      level: "recommended",
    });
    return;
