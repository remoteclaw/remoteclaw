...
const DEFAULT_REMOTECLAW_TESTBOX_CLAIM_TTL_MINUTES = 12 * 60;
const TESTBOX_ID_PATTERN = /^tbx_[a-z0-9]+$/u;
const REMOTECLAW_TESTBOX_CLAIM_FILE = "remoteclaw-runner.json";
...
export function resolveBlacksmithTestboxStateDir({ env = process.env, homeDir } = {}) {
  if (env.REMOTECLAW_BLACKSMITH_TESTBOX_STATE_DIR) {
    return env.REMOTECLAW_BLACKSMITH_TESTBOX_STATE_DIR;
  }
...
export function resolveRemoteClawTestboxClaimPath({ testboxId, env = process.env, homeDir } = {}) {
  const stateDir = resolveBlacksmithTestboxStateDir({ env, homeDir });
  return path.join(stateDir, testboxId, REMOTECLAW_TESTBOX_CLAIM_FILE);
}

export function evaluateRemoteClawTestboxClaim({
  testboxId,
  cwd,
  env = process.env,
  exists = fs.existsSync,
  ...
  const claimPath = resolveRemoteClawTestboxClaimPath({ testboxId, env, homeDir });
  const expectedRepoRoot = path.resolve(cwd || process.cwd());
  const maxAgeMinutes = parsePositiveInteger(
    env.REMOTECLAW_TESTBOX_CLAIM_TTL_MINUTES,
    DEFAULT_REMOTECLAW_TESTBOX_CLAIM_TTL_MINUTES,
  );
  const problems = [];

  if (!exists(claimPath)) {
    problems.push(
      `RemoteClaw Testbox claim missing for ${testboxId}: expected ${claimPath}. ` +
  ...
  try {
    ...
    problems.push(`RemoteClaw Testbox claim is unreadable for ${testboxId}: ${error.message}`);
  }

  const claimedRepoRoot = claim?.repoRoot ? path.resolve(claim.repoRoot) : "";
  if (!claimedRepoRoot) {
    problems.push(`RemoteClaw Testbox claim is missing repoRoot for ${testboxId}: ${claimPath}`);
  } else if (claimedRepoRoot !== expectedRepoRoot) {
    problems.push(
      `RemoteClaw Testbox claim repo mismatch for ${testboxId}: claimed ${claimedRepoRoot}, current ${expectedRepoRoot}. ` +
  ...
  if (!Number.isFinite(claimedAtMs)) {
    problems.push(`RemoteClaw Testbox claim is missing claimedAt for ${testboxId}: ${claimPath}`);
  } else {
    const ageMinutes = Math.floor((now().getTime() - claimedAtMs) / 60000);
    if (ageMinutes > maxAgeMinutes) {
      problems.push(
        `RemoteClaw Testbox claim is stale for ${testboxId}: ${ageMinutes}m old, limit ${maxAgeMinutes}m. ` +
...
export function writeRemoteClawTestboxClaim({
  testboxId,
  cwd,
  env = process.env,
  homeDir,
