...
run_ref_onboard() {
  guest_current_user_exec \
    ...
    "$GUEST_NODE_BIN" "$GUEST_REMOTECLAW_ENTRY" onboard \
    --non-interactive \
    --mode local \
...
verify_gateway() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_REMOTECLAW_ENTRY" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_REMOTECLAW_ENTRY" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_REMOTECLAW_ENTRY" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_REMOTECLAW_ENTRY" gateway status --deep
}

verify_turn() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_REMOTECLAW_ENTRY" agent --agent main --message ping --json
}
