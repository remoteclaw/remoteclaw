...
private final class TestChatTransport: @unchecked Sendable, RemoteClawChatTransport {
    ...
    func requestHistory(sessionKey: String) async throws -> RemoteClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? RemoteClawChatHistoryPayload(
    ...
    func listSessions(limit _: Int?) async throws -> RemoteClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? RemoteClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: RemoteClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = RemoteClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = RemoteClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { RemoteClawChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { RemoteClawChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
