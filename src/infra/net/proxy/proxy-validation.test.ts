...
describe("proxy validation", () => {
  ...
  function writeTempCa(contents = "proxy-ca"): string {
    const dir = mkdtempSync(path.join(os.tmpdir(), "remoteclaw-proxy-validation-ca-"));
    tempDirs.push(dir);
  ...
  it("resolves proxy URL overrides before config and REMOTECLAW_PROXY_URL", () => {
    const result = resolveProxyValidationConfig({
      ...
      env: {
        REMOTECLAW_PROXY_URL: "http://env-proxy.example:3128",
      },
    });
  ...
  it("resolves config proxy URLs before REMOTECLAW_PROXY_URL", () => {
    const result = resolveProxyValidationConfig({
      config: {
        enabled: true,
        proxyUrl: "http://config-proxy.example:3128",
  ...
  it("uses REMOTECLAW_PROXY_URL when enabled config has no URL", () => {
    const result = resolveProxyValidationConfig({
      config: { enabled: true },
      env: {
        REMOTECLAW_PROXY_URL: "http://env-proxy.example:3128",
  ...
  it("reports disabled proxy config when only REMOTECLAW_PROXY_URL is present", async () => {
    const fetchCheck = vi.fn();
    ...
    expect(result).toEqual({
      ...
      config: {
        ...
        proxyUrl: "http://env-proxy.example:3128",
        source: "env",
        errors: ["proxy validation requires proxy.enabled to be true for REMOTECLAW_PROXY_URL"],
      },
      checks: [],
    });
  });
  ...
  it("reports missing URL when proxy validation is enabled without an effective URL", () => {
    ...
    expect(result.errors).toEqual([
      "proxy validation requires proxy.proxyUrl, --proxy-url, or REMOTECLAW_PROXY_URL",
    ]);
  });
  ...
  it("reports disabled proxy config as an actionable validation problem", async () => {
    ...
    expect(result).toEqual({
      ...
      config: {
        ...
        source: "disabled",
        errors: [
          "proxy validation requires proxy.enabled=true with proxy.proxyUrl or REMOTECLAW_PROXY_URL, or --proxy-url",
        ],
      },
      checks: [],
    });
  ...
  it("does not load proxy CA files for plain HTTP proxy validation", async () => {
    const missingCaFile = path.join(os.tmpdir(), "remoteclaw-missing-http-proxy-validation-ca.pem");
    const fetchCheck = vi.fn().mockResolvedValue({ ok: true, status: 200 });
  ...
  it("fails closed before probing when proxy CA file cannot be loaded", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "remoteclaw-proxy-validation-missing-ca-"));
    tempDirs.push(dir);
    const fetchCheck = vi.fn();
