...
<AccordionGroup>
  <Accordion title="Dedicated number (recommended)">
    ...
    - separate WhatsApp identity for RemoteClaw
...
- if `messages.responsePrefix` is unset, self-chat replies default to `[{identity.name}]` or `[remoteclaw]`
...
<AccordionGroup>
  ...
  <Accordion title="Credential paths and legacy compatibility">
    - current auth path: `~/.remoteclaw/credentials/whatsapp/<accountId>/creds.json`
    - backup file: `creds.json.bak`
    - legacy default auth in `~/.remoteclaw/credentials/` is still recognized/migrated for default-account flows
  </Accordion>

  <Accordion title="Logout behavior">
    `remoteclaw channels logout --channel whatsapp [--account <id>]` clears WhatsApp auth state for that account.
...
<AccordionGroup>
  ...
  <Accordion title="Group messages unexpectedly ignored">
    ...
    - duplicate keys in `remoteclaw.json` (JSON5): later entries override earlier ones, so keep a single `groupPolicy` per scope
