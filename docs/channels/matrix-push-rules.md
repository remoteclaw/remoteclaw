...
If no pushers come back, fix normal Matrix push delivery for this account before continuing.
  ...
  <Step title="Install the override push rule">
    RemoteClaw marks finalized text-only preview edits with `content["com.remoteclaw.finalized_preview"] = true`. Install a rule that matches that marker plus the bot MXID as sender:
