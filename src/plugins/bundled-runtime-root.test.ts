...
function makeTempRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-bundled-runtime-root-"));
  tempRoots.push(root);
  return root;
}
...
describe("prepareBundledPluginRuntimeRoot", () => {
  it("materializes plugin-owned root chunks in external mirrors", () => {
    ...
    const env = { ...process.env, REMOTECLAW_PLUGIN_STAGE_DIR: stageDir };
    fs.mkdirSync(pluginRoot, { recursive: true });
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "remoteclaw", version: "2026.4.24", type: "module" }),
      "utf8",
    );
    fs.writeFileSync(
  ...
  it("does not copy staged runtime mirror dist files onto themselves", () => {
    const stageDir = makeTempRoot();
    const installRoot = path.join(stageDir, "remoteclaw-2026.4.26-alpha");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "remoteclaw", version: "2026.4.26", type: "module" }),
      "utf8",
    );
