...
const ROOT_IMPORT_PATTERN = /["']remoteclaw\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']remoteclaw\/plugin-sdk\/compat["']/;
const LEGACY_BROAD_SUBPATH_PATTERNS = [
  {
    pattern: /["']remoteclaw\/plugin-sdk\/channel-runtime["']/,
    label: "remoteclaw/plugin-sdk/channel-runtime",
  },
  {
    pattern: /["']remoteclaw\/plugin-sdk\/config-runtime["']/,
    label: "remoteclaw/plugin-sdk/config-runtime",
  },
] as const;
...
function main() {
  ...
  if (
    ...
    if (
      ...
      console.error(
        "Use focused remoteclaw/plugin-sdk/<domain> subpaths for bundled plugins; root, compat, and broad runtime barrels are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
