...
struct ChatMarkdownRenderer: View {
    ...
    struct InlineMathTypography {
        static let body = Self(size: RemoteClawChatTypography.bodySize, relativeTo: .body)
    ...
    private var linkColor: Color {
        self.context == .user ? self.textColor : RemoteClawChatTheme.accent
    }
}
        ...
        ForEach(self.images, id: \.id) { item in
            if let img = item.image {
                ...
                Text(item.label.isEmpty ? "Image" : item.label)
                    .font(RemoteClawChatTypography.footnote)
