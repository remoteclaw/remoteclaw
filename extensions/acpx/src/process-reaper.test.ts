...
import {
  cleanupRemoteClawOwnedAcpxProcessTree,
  isRemoteClawLeaseAwareAcpxProcessCommand,
  isRemoteClawOwnedAcpxProcessCommand,
  reapStaleRemoteClawOwnedAcpxOrphans,
  type AcpxProcessInfo,
} from "./process-reaper.js";

const WRAPPER_ROOT = "/tmp/remoteclaw-state/acpx";
const CODEX_WRAPPER_COMMAND = `node ${WRAPPER_ROOT}/codex-acp-wrapper.mjs`;
const CODEX_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_WRAPPER_COMMAND} ${REMOTECLAW_ACPX_LEASE_ID_ARG} lease-1 ${REMOTECLAW_GATEWAY_INSTANCE_ID_ARG} gateway-1`;
const CLAUDE_WRAPPER_COMMAND = `node ${WRAPPER_ROOT}/claude-agent-acp-wrapper.mjs`;
const PLUGIN_DEPS_CODEX_COMMAND =
  "node /tmp/remoteclaw/plugin-runtime-deps/node_modules/@zed-industries/codex-acp/bin/codex-acp.js";
...
describe("process reaper", () => {
  ...
  it("only treats generated wrappers as launch-lease aware", () => {
    ...
    expect(
      isRemoteClawLeaseAwareAcpxProcessCommand({ command: LOCAL_NODE_MODULES_CODEX_COMMAND }),
    ).toBe(false);
    expect(isRemoteClawLeaseAwareAcpxProcessCommand({ command: PLUGIN_DEPS_CODEX_COMMAND })).toBe(
      false,
    );
  });

  it("recognizes RemoteClaw plugin-runtime-deps ACP adapter children", () => {
    expect(isRemoteClawOwnedAcpxProcessCommand({ command: PLUGIN_DEPS_CODEX_COMMAND })).toBe(true);
    expect(isRemoteClawOwnedAcpxProcessCommand({ command: "npx @zed-industries/codex-acp" })).toBe(
      false,
    );
  });

  it("recognizes plugin-local ACP adapter package paths without trusting arbitrary installs", () => {
    expect(isRemoteClawOwnedAcpxProcessCommand({ command: LOCAL_NODE_MODULES_CODEX_COMMAND })).toBe(
      true,
    );
    expect(
      isRemoteClawOwnedAcpxProcessCommand({
  ...
  it("kills an owned recorded process tree children first", async () => {
    ...
    const result = await cleanupRemoteClawOwnedAcpxProcessTree({
      rootPid: 100,
  ...
  it("does not kill a reused same-root wrapper pid with a different lease identity", async () => {
    const { deps, killed } = cleanupDeps([
      {
        pid: 113,
        ppid: 1,
        command: `${CODEX_WRAPPER_COMMAND} ${REMOTECLAW_ACPX_LEASE_ID_ARG} other-lease ${REMOTECLAW_GATEWAY_INSTANCE_ID_ARG} gateway-1`,
      },
    ]);
    ...
    expect(result).toEqual({
      inspectedPids: [113],
      terminatedPids: [],
      skippedReason: "not-remoteclaw-owned",
    });
  ...
  it("does not kill a reused pid when the live command is not RemoteClaw-owned", async () => {
  ...
  it("skips non-owned recorded process trees", async () => {
    ...
    expect(result.skippedReason).toBe("not-remoteclaw-owned");
    expect(killed).toStrictEqual([]);
  });

  it("reaps stale RemoteClaw-owned wrapper and adapter orphans on startup", async () => {
    const { deps, killed } = cleanupDeps([
    ...
    const result = await reapStaleRemoteClawOwnedAcpxOrphans({
      wrapperRoot: WRAPPER_ROOT,
      deps,
    });
