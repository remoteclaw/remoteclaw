...
read_when:
  - You need a beginner-friendly overview of RemoteClaw logging
...
The date uses the gateway host's local timezone. When `/tmp/remoteclaw` is unsafe
or unavailable (and always on Windows), RemoteClaw uses a user-scoped
`remoteclaw-<uid>` directory under the OS temp dir instead. Dated log files are
pruned after 24 hours.

Each file rotates when the next write would exceed `logging.maxFileBytes`
(default: 100 MB). RemoteClaw keeps up to five numbered archives beside the
active file, such as `remoteclaw-YYYY-MM-DD.1.log`, and keeps writing to a fresh
active log instead of suppressing diagnostics.

You can override the path in `~/.remoteclaw/remoteclaw.json`:

```json
{
  "logging": {
    "file": "/path/to/remoteclaw.log"
  }
}
```
...
or times out before `logs.tail` answers, `remoteclaw logs` falls back to the
configured Gateway file log automatically. Explicit `--url` targets do not use
this fallback. `remoteclaw logs --follow` is stricter: on Linux it uses the active
...
```bash
remoteclaw channels logs --channel whatsapp
```
...
```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/remoteclaw/remoteclaw-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
`REMOTECLAW_DEBUG_MODEL_TRANSPORT`, so basic model transport hygiene is visible
without debug flags.

### Trace correlation

File logs are JSONL. When a log call carries a valid diagnostic trace context,
RemoteClaw writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
RemoteClaw can redact sensitive tokens before they hit console output, file logs,
...
policy. RemoteClaw still redacts safety-boundary payloads that can be shown to UI
...
- **Diagnostics flags** — targeted debug-log flags that route extra logs to
  ...
  or via the `REMOTECLAW_DIAGNOSTICS=...` env override. Full guide:
...
- **Gateway not reachable?** Run `remoteclaw doctor` first.
