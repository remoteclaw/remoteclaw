...
describe("server-context hot-reload profiles", () => {
  ...
  it("forProfile hot-reloads newly added profiles from config", async () => {
    // Start with only remoteclaw profile
    ...
    // 2. Simulate adding a new profile to config (like user editing remoteclaw.json)
  ...
  it("forProfile refreshes existing profile config after loadConfig cache updates", async () => {
    ...
    cfgProfiles.remoteclaw = { cdpPort: 19999, color: "#FF4500" };
    cachedConfig = null;
    ...
    expect(state.resolved.profiles.remoteclaw?.cdpPort).toBe(19999);
  });
  ...
  it("marks existing runtime state for reconcile when profile invariants change", async () => {
    const cfg = loadConfig();
    const resolved = resolveBrowserConfig(cfg.browser, cfg);
    const remoteclawProfile = resolveProfile(resolved, "remoteclaw");
    expect(remoteclawProfile).toBeTruthy();
    ...
    const runtime = state.profiles.get("remoteclaw");
