...
describe("cli debug timing", () => {
  it("does not emit timing lines unless REMOTECLAW_DEBUG_TIMING enables a mode", () => {
    const writer = vi.fn();
  ...
  it("emits readable timing lines with REMOTECLAW_DEBUG_TIMING=1", () => {
    const writer = vi.fn();
    const timing = createCliDebugTiming({
      command: "models list",
      env: { REMOTECLAW_DEBUG_TIMING: "1" },
      writer,
    });
    ...
    expect(writer.mock.calls.map(([line]) => String(line))).toEqual([
      'RemoteClaw CLI debug timing: "models list"',
      expect.stringMatching(/\s+\d+ms\s+\+\d+ms "start" detail="ready"/),
      expect.stringMatching(/\s+\d+ms\s+\+\d+ms "sync" duration=\d+ms/),
    ]);
  });

  it("emits parseable timing JSON lines with REMOTECLAW_DEBUG_TIMING=json", async () => {
    const writer = vi.fn();
    const timing = createCliDebugTiming({
      command: "models list",
      env: { REMOTECLAW_DEBUG_TIMING: "json" },
      writer,
    });
