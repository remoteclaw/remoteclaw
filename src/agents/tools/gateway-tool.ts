...
import { isRemoteClawOwnerOnlyCoreToolName } from "./owner-only-tools.js";
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `remoteclaw security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as RemoteClawConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as RemoteClawConfig);
...
export function createGatewayTool(opts?: {
  ...
  return {
    label: "Gateway",
    name: "gateway",
    ownerOnly: isRemoteClawOwnerOnlyCoreToolName("gateway"),
    description:
