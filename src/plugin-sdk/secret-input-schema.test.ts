...
describe("plugin-sdk secret input schema", () => {
  ...
  it("accepts plaintext and valid refs", () => {
    ...
    expect(schema.safeParse({ source: "env", provider: "default", id: "OPENAI_API_KEY" }).success).toBe(true);
    expect(schema.safeParse({ source: "file", provider: "filemain", id: "/providers/openai/apiKey" }).success).toBe(
      true,
    );
