...
final class MotionService: MotionServicing {
    func activities(params: RemoteClawMotionActivityParams) async throws -> RemoteClawMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[RemoteClawMotionActivityEntry], Error>) in
        ...
        return RemoteClawMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: RemoteClawPedometerParams) async throws -> RemoteClawPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<RemoteClawPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = RemoteClawPedometerPayload(
