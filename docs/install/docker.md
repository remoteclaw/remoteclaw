...
- Containerized Gateway (full RemoteClaw in Docker)
...
- `REMOTECLAW_INSTALL_DOCKER_CLI` — build arg passthrough for local image builds (`1` installs Docker CLI in the image). `docker-setup.sh` sets this automatically when `REMOTECLAW_SANDBOX=1` for local builds.
- `REMOTECLAW_DOCKER_SOCKET` — override Docker socket path (default: `DOCKER_HOST=unix://...` path, else `/var/run/docker.sock`)
...
- `REMOTECLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0` — disable container browser hardening flags
  `--disable-3d-apis`, `--disable-software-rasterizer`, `--disable-gpu` when you need
  WebGL/3D compatibility.
- `REMOTECLAW_BROWSER_DISABLE_EXTENSIONS=0` — keep extensions enabled when browser
  flows require them (default keeps extensions disabled in sandbox browser).
- `REMOTECLAW_BROWSER_RENDERER_PROCESS_LIMIT=<N>` — set Chromium renderer process
...
- Need the URL again? Run `docker compose run --rm remoteclaw-cli dashboard --no-open`.
...
```bash
export REMOTECLAW_SANDBOX=1
export REMOTECLAW_DOCKER_SOCKET=/run/user/1000/docker.sock
./docker-setup.sh
```
...
- If `Dockerfile.sandbox` is missing, the script prints a warning and continues;
  build `remoteclaw-sandbox:bookworm-slim` with `scripts/sandbox-setup.sh` if
  needed.
- For non-local `REMOTECLAW_IMAGE` values, the image must already contain Docker
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/remoteclaw/remoteclaw/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
```
...
See [`ClawDock` Helper README](https://github.com/remoteclaw/remoteclaw/blob/main/scripts/shell-helpers/README.md) for details.

### Manual flow (compose)

```bash
docker build -t remoteclaw:local -f Dockerfile .
docker compose run --rm remoteclaw-cli onboard
docker compose up -d remoteclaw-gateway
```

Note: run `docker compose ...` from the repo root. If you enabled
`REMOTECLAW_EXTRA_MOUNTS` or `REMOTECLAW_HOME_VOLUME`, the setup script writes
...
`REMOTECLAW_EXTRA_MOUNTS` before running `docker-setup.sh`. This accepts a
comma-separated list of Docker bind mounts and applies them to both
`remoteclaw-gateway` and `remoteclaw-cli` by generating `docker-compose.extra.yml`.

Example:

```bash
export REMOTECLAW_EXTRA_MOUNTS="$HOME/.codex:/home/node/.codex:ro,$HOME/github:/home/node/github:rw"
./docker-setup.sh
```
...
- If you edit `REMOTECLAW_EXTRA_MOUNTS`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
volume via `REMOTECLAW_HOME_VOLUME`. This creates a Docker volume and mounts it at
...
```bash
export REMOTECLAW_HOME_VOLUME="remoteclaw_home"
./docker-setup.sh
```
...
- If you change `REMOTECLAW_HOME_VOLUME`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
libraries), set `REMOTECLAW_DOCKER_APT_PACKAGES` before running `docker-setup.sh`.
...
```bash
export REMOTECLAW_DOCKER_APT_PACKAGES="ffmpeg build-essential"
./docker-setup.sh
```
...
```bash
export REMOTECLAW_DOCKER_APT_PACKAGES="git curl jq"
./docker-setup.sh
```
...
`REMOTECLAW_DOCKER_APT_PACKAGES` instead of using `--with-deps` at runtime.

4. **Persist Playwright browser downloads**:

- Set `PLAYWRIGHT_BROWSERS_PATH=/home/node/.cache/ms-playwright` in
  `docker-compose.yml`.
- Ensure `/home/node` persists via `REMOTECLAW_HOME_VOLUME`, or mount
  `/home/node/.cache/ms-playwright` via `REMOTECLAW_EXTRA_MOUNTS`.

### Permissions + EACCES

The image runs as `node` (uid 1000). If you see permission errors on
`/home/node/.remoteclaw`, make sure your host bind mounts are owned by uid 1000.

Example (Linux host):

```bash
sudo chown -R 1000:1000 /path/to/remoteclaw-config /path/to/remoteclaw-workspace
```
...
```bash
docker compose run --rm remoteclaw-cli channels login
```

Telegram (bot token):

```bash
docker compose run --rm remoteclaw-cli channels add --channel telegram --token "<token>"
```

Discord (bot token):

```bash
docker compose run --rm remoteclaw-cli channels add --channel discord --token "<token>"
```
...
- Agent workspace access: `workspaceAccess: "none"` (default) uses `~/.remoteclaw/sandboxes`
...
- `user` must be root for `apt-get` (omit `user` or set `user: "0:0"`).
  RemoteClaw auto-recreates containers when `setupCommand` (or docker config) changes
  unless the container was **recently used** (within ~5 minutes). Hot containers
  log a warning with the exact `remoteclaw sandbox recreate ...` command.
...
This builds `remoteclaw-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `remoteclaw-sandbox-common:bookworm-slim`. To use it:

```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "remoteclaw-sandbox-common:bookworm-slim" } },
    },
  },
}
```
...
This builds `remoteclaw-sandbox-browser:bookworm-slim` using
...
- Browser containers default to a dedicated Docker network (`remoteclaw-sandbox-browser`) instead of global `bridge`.
...
- noVNC observer access is password-protected by default; RemoteClaw provides a short-lived observer token URL that serves a local bootstrap page and keeps the password in URL fragment (instead of URL query).
- Browser container startup defaults are conservative for shared/container workloads, including:
  ...
  - The three graphics hardening flags above are optional. If your workload needs
    WebGL/3D, set `REMOTECLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0` to run without
  ...
  - Extension behavior is controlled by `--disable-extensions` and can be disabled
    (enables extensions) via `REMOTECLAW_BROWSER_DISABLE_EXTENSIONS=0` for
    extension-dependent pages or extensions-heavy workflows.
  - `--renderer-process-limit=2` is also configurable with
    `REMOTECLAW_BROWSER_RENDERER_PROCESS_LIMIT`; set `0` to let Chromium choose its
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-remoteclaw-browser" } },
    },
  },
}
```
...
```bash
docker build -t my-remoteclaw-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-remoteclaw-sbx" } },
    },
  },
}
```
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/remoteclaw/remoteclaw/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.
- Container not running: it will auto-create per session on demand.
- Permission errors in sandbox: set `docker.user` to a UID:GID that matches your
  mounted workspace ownership (or chown the workspace folder).
- Custom tools not found: RemoteClaw runs commands with `sh -lc` (login shell), which
