...
@MainActor
@Observable
public final class RemoteClawChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [RemoteClawChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [RemoteClawChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [RemoteClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [RemoteClawChatPendingToolCall] = []
    public private(set) var sessions: [RemoteClawChatSessionEntry] = []
    private let transport: any RemoteClawChatTransport
    private var sessionDefaults: RemoteClawChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: RemoteClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any RemoteClawChatTransport,
    ...
    public var sessionChoices: [RemoteClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [RemoteClawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: RemoteClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [RemoteClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: RemoteClawChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: RemoteClawChatMessage) -> RemoteClawChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> RemoteClawChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return RemoteClawChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: RemoteClawChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: RemoteClawChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: RemoteClawChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [RemoteClawChatMessage],
        incoming: [RemoteClawChatMessage]) -> [RemoteClawChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [RemoteClawChatMessage]) -> [RemoteClawChatMessage] {
    ...
    private static func dedupeKey(for message: RemoteClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [RemoteClawChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> RemoteClawChatAttachmentPayload in
    ...
    private func placeholderSession(key: String) -> RemoteClawChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = RemoteClawChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleChatEvent(_ chat: RemoteClawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: RemoteClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = RemoteClawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> RemoteClawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
