...
- **Cron (isolated)** is a scheduled **background run** — creates a task record tracked in `remoteclaw tasks`.
...
- **Task tracking**: isolated jobs create [background task](/automation/tasks) records visible in `remoteclaw tasks` and `remoteclaw tasks audit`.
...
| [Tasks](/automation/tasks) | No task record                  | Task record (silent)     | Task record (visible in `remoteclaw tasks`)     |
