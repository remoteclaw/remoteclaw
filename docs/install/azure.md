...
<Steps>
  ...
  <Step title="Set deployment variables">
    ...
    RG="rg-remoteclaw"
    LOCATION="westus2"
    VNET_NAME="vnet-remoteclaw"
    VNET_PREFIX="10.40.0.0/16"
    VM_SUBNET_NAME="snet-remoteclaw-vm"
    VM_SUBNET_PREFIX="10.40.2.0/24"
    BASTION_SUBNET_PREFIX="10.40.1.0/26"
    NSG_NAME="nsg-remoteclaw-vm"
    VM_NAME="vm-remoteclaw"
    ADMIN_USERNAME="remoteclaw"
    BASTION_NAME="bas-remoteclaw"
    BASTION_PIP_NAME="pip-remoteclaw-bastion"
    ```
...
<Steps>
  ...
  <Step title="Install RemoteClaw (in the VM shell)">
    ```bash
    curl -fsSL https://remoteclaw.org/install.sh -o /tmp/install.sh
    bash /tmp/install.sh
    rm -f /tmp/install.sh
    ```

    The installer installs Node LTS and dependencies if not already present, installs RemoteClaw, and launches the onboarding wizard. See [Install](/install) for details.
