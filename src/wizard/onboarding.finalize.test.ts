...
describe("finalizeOnboardingWizard", () => {
  ...
  // so the SecretRef password cannot be resolved and falls back to "".
  it("resolves gateway password SecretRef for probe and TUI", async () => {
    const previous = process.env.REMOTECLAW_GATEWAY_PASSWORD;
    process.env.REMOTECLAW_GATEWAY_PASSWORD = "resolved-gateway-password";
    ...
    // Gutted in RemoteClaw fork: password stays "" because resolveSecretRefString
    // is a no-op stub that always returns undefined.
