  ...
  if (!channel) {
    return {
      warning:
        ...
        'Configure Matrix, then rerun "remoteclaw doctor --fix" or restart the gateway.',
    };
  }
  if (requiresExplicitMatrixDefaultAccount(params.cfg)) {
    return {
      warning:
        ...
        'Set "channels.matrix.defaultAccount" to the intended target account before rerunning "remoteclaw doctor --fix" or restarting the gateway.',
    };
  }
  ...
  if (!target) {
    ...
    return {
      warning:
        ...
        'Start the gateway once with a working Matrix login, or rerun "remoteclaw doctor --fix" after cached credentials are available.',
    };
  }
