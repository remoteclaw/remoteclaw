...
describe("config defaults", () => {
  ...
  // Skipped: the fork removed the LLM-provider integration layer (gutted
  // `@mariozechner/pi-*` and provider-specific defaults). `provider-policy.js`
  // exists as a stub; `applyContextPruningDefaults` no longer delegates to it
  ...
  // Skipped: the fork's `applyAgentDefaults` no longer materializes the
  // `agents.defaults.maxConcurrent` / `subagents.maxConcurrent` keys when no
  // agent block exists — agent concurrency lives in the CLI-runtime layer
  // (replacement for the gutted in-process executor), not in config defaults.
  it.skip("fills missing agent concurrency defaults", () => {
