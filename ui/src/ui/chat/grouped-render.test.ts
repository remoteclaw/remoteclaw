...
vi.mock("../views/agents-utils.ts", () => ({
  agentLogoUrl: () => "/remoteclaw-logo.svg",
}));
...
function clearDeleteConfirmSkip() {
  try {
    getSafeLocalStorage()?.removeItem("remoteclaw:skipDeleteConfirm");
  } catch {
    /* noop */
  }
}
...
describe("grouped chat rendering", () => {
  ...
  it("renders allowed transcript images and skips blocked/non-image media", () => {
    ...
    let container = renderUserMedia({
      ...
      role: "user",
      content: "",
      MediaPath: "/tmp/remoteclaw/user-upload.png",
      timestamp: Date.now(),
    });
    expect(
      container.querySelector<HTMLImageElement>(".chat-message-image")?.getAttribute("src"),
    ).toBe(
      "/remoteclaw/__remoteclaw__/assistant-media?source=%2Ftmp%2Fremoteclaw%2Fuser-upload.png&token=session-token",
    );
    ...
    container = renderUserMedia({
      ...
      role: "user",
      content: "",
      MediaPaths: ["/tmp/remoteclaw/first.png", "/tmp/remoteclaw/second.jpg"],
    ...
    expect(
      ...
      "/remoteclaw/__remoteclaw__/assistant-media?source=%2Ftmp%2Fremoteclaw%2Ffirst.png&token=session-token",
      "/remoteclaw/__remoteclaw__/assistant-media?source=%2Ftmp%2Fremoteclaw%2Fsecond.jpg&token=session-token",
    ]);
    ...
    container = renderUserMedia({
      ...
      role: "user",
      content: "",
      MediaPath: "/tmp/remoteclaw/user-upload.pdf",
  ...
  it("renders verified local assistant attachments through the Control UI media route", async () => {
    ...
    const renderMessage = () =>
      renderAssistantMessage(
        container,
        {
          ...
          role: "assistant",
          content:
            "Local image\nMEDIA:/tmp/remoteclaw/test image.png\nMEDIA:/tmp/remoteclaw/test-doc.pdf",
          timestamp: Date.now(),
        },
        {
          showToolCalls: false,
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "/remoteclaw/__remoteclaw__/assistant-media?source=%2Ftmp%2Fremoteclaw%2Ftest+image.png&token=session-token&meta=1",
    ...
    expect(image?.getAttribute("src")).toBe(
      "/remoteclaw/__remoteclaw__/assistant-media?source=%2Ftmp%2Fremoteclaw%2Ftest+image.png&token=session-token",
    );
    expect(docLink?.getAttribute("href")).toBe(
      "/remoteclaw/__remoteclaw__/assistant-media?source=%2Ftmp%2Fremoteclaw%2Ftest-doc.pdf&token=session-token",
    );
  ...
  it("rechecks local assistant attachment availability when the auth token changes", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      "/remoteclaw/__remoteclaw__/assistant-media?source=%2Ftmp%2Fremoteclaw%2Ftest+image.png&meta=1",
      expect.objectContaining({ credentials: "same-origin", method: "GET" }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "/remoteclaw/__remoteclaw__/assistant-media?source=%2Ftmp%2Fremoteclaw%2Ftest+image.png&token=fresh-token&meta=1",
      expect.objectContaining({ credentials: "same-origin", method: "GET" }),
    );
    expect(container.querySelector(".chat-message-image")).not.toBeNull();
  ...
  it("preserves same-origin assistant attachments without local preview rewriting", () => {
    ...
    renderAssistantMessage(
      container,
      {
        ...
        role: "assistant",
        content:
          "Inline\nMEDIA:/media/inbound/test-image.png\nMEDIA:/__remoteclaw__/media/test-doc.pdf",
        timestamp: Date.now(),
      },
      {
        showToolCalls: false,
    ...
    expect(docLink?.getAttribute("href")).toBe("/__remoteclaw__/media/test-doc.pdf");
    expect(container.textContent).not.toContain("Unavailable");
  });
  ...
  it("allows platform-specific local assistant attachments inside preview roots", async () => {
    ...
    const cases = [
      renderCase({
        roots: ["C:\\tmp\\remoteclaw"],
        message: {
          id: "assistant-windows-file-url",
          role: "assistant",
          content: "Windows image\nMEDIA:file:///C:/tmp/remoteclaw/test%20image.png",
          timestamp: Date.now(),
        },
        expectedUrl:
          "/remoteclaw/__remoteclaw__/assistant-media?source=%2FC%3A%2Ftmp%2Fremoteclaw%2Ftest%2520image.png&meta=1",
      }),
      renderCase({
        ...
        expectedUrl:
          "/remoteclaw/__remoteclaw__/assistant-media?source=C%3A%5CUsers%5CTest%5CPictures%5Ctest+image.png&meta=1",
      }),
      renderCase({
        ...
        expectedUrl:
          "/remoteclaw/__remoteclaw__/assistant-media?source=%7E%2FPictures%2Ftest+image.png&meta=1",
      }),
    ];
  ...
  it("routes inline canvas blocks through the scoped canvas host when available", () => {
    ...
    renderAssistantMessage(
        ...
        content: [
          { type: "text", text: "Rendered inline." },
          {
            ...
            preview: {
              ...
              url: "/__remoteclaw__/canvas/documents/cv_inline_scoped/index.html",
              preferredHeight: 320,
            },
          },
        ],
      ...
      {
        canvasHostUrl: "http://127.0.0.1:19003/__remoteclaw__/cap/cap_123",
      },
    );

    const iframe = container.querySelector(".chat-tool-card__preview-frame");
    expect(iframe?.getAttribute("src")).toBe(
      "http://127.0.0.1:19003/__remoteclaw__/cap/cap_123/__remoteclaw__/canvas/documents/cv_inline_scoped/index.html",
    );
  });
        ...
        content: [
          ...
          { type: "text", text: "This item is ready." },
          {
            ...
            preview: {
              ...
              url: "/__remoteclaw__/canvas/documents/cv_canvas_live_history/index.html",
              preferredHeight: 420,
            },
  ...
  it("renders hidden assistant_message canvas results with the configured sandbox", () => {
          ...
          toolMessages: [
            {
              ...
              content: JSON.stringify({
                ...
                view: {
                  ...
                  url: `/__remoteclaw__/canvas/documents/cv_inline_${params.suffix}/index.html`,
                  title: "Inline demo",
    ...
    expect(iframe?.getAttribute("src")).toBe(
      "/__remoteclaw__/canvas/documents/cv_inline_default/index.html",
    );
        ...
        toolMessages: [
          {
            ...
            content: JSON.stringify({
              ...
              view: {
                ...
                url: "/__remoteclaw__/canvas/documents/cv_inline_visible/index.html",
                title: "Inline demo",
                preferred_height: 360,
              },
              presentation: {
