...
enum WatchMessagingPayloadCodec {
    private static let durableSnapshotTypes = [
        RemoteClawWatchPayloadType.appSnapshot.rawValue,
        RemoteClawWatchPayloadType.execApprovalSnapshot.rawValue,
    ]
        ...
        gatewayStableID: String?) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": RemoteClawWatchPayloadType.notify.rawValue,
            "id": id,
            "title": params.title,
            "body": params.body,
            "priority": params.priority?.rawValue ?? RemoteClawNotificationPriority.active.rawValue,
            "sentAtMs": self.nowMs(),
        ]
    ...
    static func encodeExecApprovalItem(_ item: RemoteClawWatchExecApprovalItem) -> [String: Any] {
        var payload: [String: Any] = [
            "id": item.id,
    ...
    static func encodeExecApprovalPromptPayload(
        _ message: RemoteClawWatchExecApprovalPromptMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": RemoteClawWatchPayloadType.execApprovalPrompt.rawValue,
    ...
    static func encodeExecApprovalResolvedPayload(
        _ message: RemoteClawWatchExecApprovalResolvedMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": RemoteClawWatchPayloadType.execApprovalResolved.rawValue,
    ...
    static func encodeExecApprovalExpiredPayload(
        _ message: RemoteClawWatchExecApprovalExpiredMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": RemoteClawWatchPayloadType.execApprovalExpired.rawValue,
            "approvalId": message.approvalId,
    ...
    static func encodeExecApprovalSnapshotPayload(
        _ message: RemoteClawWatchExecApprovalSnapshotMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": RemoteClawWatchPayloadType.execApprovalSnapshot.rawValue,
    ...
    static func encodeAppSnapshotPayload(
        _ message: RemoteClawWatchAppSnapshotMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": RemoteClawWatchPayloadType.appSnapshot.rawValue,
    ...
    static func encodeChatCompletionPayload(
        _ message: RemoteClawWatchChatCompletionMessage) -> [String: Any]
    {
        [
    ...
    {
        guard (payload["type"] as? String) == RemoteClawWatchPayloadType.reply.rawValue else {
            return nil
        }
    ...
    {
        guard (payload["type"] as? String) == RemoteClawWatchPayloadType.execApprovalResolve.rawValue else {
            return nil
        }
        guard let approvalId = nonEmpty(payload["approvalId"] as? String),
              let rawDecision = nonEmpty(payload["decision"] as? String),
              let decision = RemoteClawWatchExecApprovalDecision(rawValue: rawDecision)
        else {
            return nil
        }
        let replyId = self.nonEmpty(payload["replyId"] as? String) ?? UUID().uuidString
    ...
    {
        guard (payload["type"] as? String) == RemoteClawWatchPayloadType.execApprovalSnapshotRequest.rawValue else {
            return nil
        }
    ...
    {
        guard (payload["type"] as? String) == RemoteClawWatchPayloadType.appSnapshotRequest.rawValue else {
            return nil
        }
        let requestId = self.nonEmpty(payload["requestId"] as? String) ?? UUID().uuidString
        let sentAtMs = (payload["sentAtMs"] as? Int) ?? (payload["sentAtMs"] as? NSNumber)?.intValue
    ...
    {
        guard (payload["type"] as? String) == RemoteClawWatchPayloadType.appCommand.rawValue else {
            return nil
        }
        guard let rawCommand = nonEmpty(payload["command"] as? String),
              let command = RemoteClawWatchAppCommand(rawValue: rawCommand)
        else {
            return nil
        }
