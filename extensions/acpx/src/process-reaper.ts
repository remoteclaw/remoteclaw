...
const REMOTECLAW_PLUGIN_DEPS_MARKER = "/plugin-runtime-deps/";
...
function liveCommandMatchesLeaseIdentity(params: {
  ...
  return (
    commandOptionEquals(parts, REMOTECLAW_ACPX_LEASE_ID_ARG, params.expectedLeaseId) &&
    commandOptionEquals(parts, REMOTECLAW_GATEWAY_INSTANCE_ID_ARG, params.expectedGatewayInstanceId)
  );
}
...
export function isRemoteClawOwnedAcpxProcessCommand(params: {
  ...
  if (!normalized.includes(REMOTECLAW_PLUGIN_DEPS_MARKER)) {
    return false;
  }
...
export async function cleanupRemoteClawOwnedAcpxProcessTree(params: {
  ...
  // that this PID still belongs to an RemoteClaw-owned wrapper, fail closed to
