...
Prompt caching means the model provider can reuse unchanged prompt prefixes (usually system/developer instructions and other stable context) across turns instead of re-processing them every time. RemoteClaw normalizes provider usage into `cacheRead` and `cacheWrite` where the upstream API exposes those counters directly.
...
- Anthropic native Messages responses expose both `cache_read_input_tokens` and `cache_creation_input_tokens`, so RemoteClaw can show both `cacheRead` and `cacheWrite`.
...
- Prompt caching is automatic on supported recent models. RemoteClaw does not need to inject block-level cache markers.
- RemoteClaw uses `prompt_cache_key` to keep cache routing stable across turns. Direct OpenAI hosts use `prompt_cache_retention: "24h"` when `cacheRetention: "long"` is selected.
...
- OpenAI responses expose cached prompt tokens via `usage.prompt_tokens_details.cached_tokens` (or `input_tokens_details.cached_tokens` on Responses API events). RemoteClaw maps that to `cacheRead`.
...
For `openrouter/anthropic/*` model refs, RemoteClaw injects Anthropic
...
handles provider-side prompt caching automatically. RemoteClaw does not inject
...
If you repoint the model at an arbitrary OpenAI-compatible proxy URL, RemoteClaw
...
- Direct Gemini transport (`api: "google-generative-ai"`) reports cache hits
  through upstream `cachedContentTokenCount`; RemoteClaw maps that to `cacheRead`.
- When `cacheRetention` is set on a direct Gemini model, RemoteClaw automatically
...
- This is separate from Anthropic/OpenAI prompt-prefix caching. For Gemini,
  RemoteClaw manages a provider-native `cachedContents` resource rather than
  injecting cache markers into the request.

### Gemini CLI usage

- Gemini CLI `stream-json` output can surface cache hits through `stats.cached`;
  RemoteClaw maps that to `cacheRead`. Legacy `--output-format json` overrides use
  the same usage normalization.
- If the CLI omits a direct `stats.input` value, RemoteClaw derives input tokens
  from `stats.input_tokens - stats.cached`.
- This is usage normalization only. It does not mean RemoteClaw is creating
  Anthropic/OpenAI-style prompt-cache markers for Gemini CLI.

## System-prompt cache boundary

RemoteClaw splits the system prompt into a **stable prefix** and a **volatile
...
RemoteClaw also keeps several cache-sensitive payload shapes deterministic before
...
RemoteClaw keeps one combined live cache regression gate for repeated prefixes, tool turns, image turns, MCP-style tool transcripts, and an Anthropic no-cache control.
...
- If you need request tracing, log request IDs and rate-limit headers separately from cache metrics. RemoteClaw's current cache-trace output is focused on prompt/session shape and normalized token usage rather than raw provider response headers.
