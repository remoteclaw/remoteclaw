...
function isRemoteChangedGateEnv(env) {
  return (
    env.REMOTECLAW_CHECK_CHANGED_REMOTE_CHILD === "1" ||
    env.REMOTECLAW_CHANGED_LANES_RAW_SYNC === "1"
  );
}
...
export function resolveOxlintShardConcurrency({
  ...
  const explicitConcurrency = resolvePositiveEnvInt(env, "REMOTECLAW_OXLINT_SHARD_CONCURRENCY");
...
export function resolveShardHeartbeatMs(env) {
  return resolveNonNegativeEnvInt(
    env,
    "REMOTECLAW_OXLINT_SHARD_HEARTBEAT_MS",
...
export function resolveShardTimeoutMs(env) {
  return resolveNonNegativeEnvInt(
    env,
    "REMOTECLAW_OXLINT_SHARD_TIMEOUT_MS",
...
export function resolveShardKillGraceMs(env) {
  return resolveNonNegativeEnvInt(
    env,
    "REMOTECLAW_OXLINT_SHARD_KILL_GRACE_MS",
