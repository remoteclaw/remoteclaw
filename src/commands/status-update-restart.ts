...
export function formatUpdateRestartStatusValue(
  ...
  if (payload.status === "error") {
    return warn(
      `failed · ${reason ?? "restart failed"} · run remoteclaw gateway status --deep${age}`,
    );
  }
  ...
  if (payload.status === "skipped") {
    if (reason === CONTROL_PLANE_UPDATE_HANDOFF_STARTED_REASON) {
      ...
      return warn(`handoff running · gateway restart pending · run remoteclaw update status${age}`);
    }
    if (reason === CONTROL_PLANE_UPDATE_RESTART_HEALTH_PENDING_REASON) {
      ...
      return warn(
        `restart pending health verification · run remoteclaw gateway status --deep${age}`,
      );
    }
...
export function formatUpdateRestartActionLines(
  ...
  if (payload.status === "error") {
    return [
      "Update restart failed; run remoteclaw gateway status --deep.",
      "If the service is down, run remoteclaw gateway restart or remoteclaw gateway install --force.",
    ];
  }
  const reason = readReason(payload);
  if (
    ...
    return [
      "Update restart is still pending; run remoteclaw update status --json for handoff state.",
      "If it stays pending, run remoteclaw gateway status --deep.",
    ];
  }
  return [];
}
