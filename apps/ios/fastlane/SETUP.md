...
APP_STORE_CONNECT_KEYCHAIN_SERVICE=remoteclaw-app-store-connect-key
...
```bash
APP_STORE_CONNECT_APP_IDENTIFIER=org.remoteclaw.app
# or
...
Tip: run `scripts/ios-team-id.sh --require-canonical` from repo root to verify the canonical RemoteClaw iOS team (`FWJYW4S8P8`) is available locally. Fastlane uses the same canonical-only path when `IOS_DEVELOPMENT_TEAM` is missing, and rejects non-canonical teams for release archives.
...
For local/manual iOS builds that stay on direct APNs, configure the gateway host separately with `REMOTECLAW_APNS_TEAM_ID`, `REMOTECLAW_APNS_KEY_ID`, and either `REMOTECLAW_APNS_PRIVATE_KEY_P8` or `REMOTECLAW_APNS_PRIVATE_KEY_PATH`. Those gateway runtime env vars are separate from Fastlane's `.env`.
...
The screenshot lane runs the app with `--remoteclaw-screenshot-mode`, which enters the built-in connected screenshot fixture instead of pairing with a live gateway. By default it chooses one available large iPhone simulator and one available 13-inch iPad simulator from the installed Xcode runtime; override devices with a comma-separated `REMOTECLAW_SNAPSHOT_DEVICES` value when the requested simulators exist locally.
...
- confirm `apps/ios/build/app-store/RemoteClaw-<version>.ipa` exists
...
- The release flow regenerates `apps/ios/RemoteClaw.xcodeproj` from `apps/ios/project.yml` before archiving
...
- App Store release uses `RemoteClawPushMode=appStore`, which derives the canonical production hosted relay, production APNs, production relay profile, and `appleStrict` proof. The release lane rejects custom production relay URL overrides.
