...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".remoteclaw-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + brand-orange). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateRemoteClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_REMOTECLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_REMOTECLAW_BROWSER_COLOR).toUpperCase();
