...
- Npm tarball onboarding/channel/agent smoke: `pnpm test:docker:npm-onboard-channel-agent` installs the packed RemoteClaw tarball globally in Docker, configures OpenAI via env-ref onboarding plus Telegram by default, verifies doctor repairs activated plugin runtime deps, and runs one mocked OpenAI agent turn. Reuse a prebuilt tarball with `REMOTECLAW_CURRENT_PACKAGE_TGZ=/path/to/remoteclaw-*.tgz`, skip the host rebuild with `REMOTECLAW_NPM_ONBOARD_HOST_BUILD=0`, or switch channel with `REMOTECLAW_NPM_ONBOARD_CHANNEL=discord`.
