...
// Rtt script supports RemoteClaw repository automation.
...
function usage() {
  return [
    "Usage: pnpm rtt <remoteclaw@spec> [--package-tgz PATH] [--provider mock-openai|live-frontier] [--credential-source env|convex] [--credential-role maintainer|ci] [--runs N] [--samples N] [--sample-timeout-ms N] [--timeout-ms N] [--harness-root PATH] [--output PATH]",
    "",
    "Examples:",
    "  pnpm rtt remoteclaw@main --package-tgz .artifacts/package/remoteclaw.tgz",
    "  pnpm rtt remoteclaw@beta",
    "  pnpm rtt remoteclaw@2026.4.30",
    "  pnpm rtt remoteclaw@latest --provider live-frontier",
  ].join("\n");
}
...
function parseArgs(argv: string[]) {
  ...
  return {
    spec: validateRemoteClawPackageSpec(spec),
    options: {
      packageTgz,
      credentialRole,
      credentialSource,
...
async function main() {
  ...
  if (spec === "remoteclaw@main" && !options.packageTgz) {
    throw new Error("remoteclaw@main requires --package-tgz.");
  }
  const version =
    spec === "remoteclaw@main"
