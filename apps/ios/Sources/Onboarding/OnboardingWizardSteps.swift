...
private struct OnboardingActivationCanvas<Content: View>: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            .background(RemoteClawBrand.activationCanvasGradient.ignoresSafeArea())
        }
    }
}
...
private struct OnboardingHeroGlyph: View {
    var body: some View {
        RemoteClawActivationGlyph(size: 78)
    }
}
...
private typealias OnboardingPrimaryButtonStyle = RemoteClawPrimaryActionButtonStyle
...
private enum OnboardingIntroPanelStyle {
    ...
    static let panelFill = RemoteClawBrand.activationNeutralSurface
    static let iconFill = RemoteClawBrand.activationNeutralInsetSurface
    static let stroke = RemoteClawBrand.activationNeutralStroke
}
...
private struct OnboardingSafetyRow: View {
    ...
    var body: some View {
        HStack(spacing: OnboardingIntroPanelStyle.contentSpacing) {
            OnboardingIntroIcon(
                symbol: self.symbol,
                tint: RemoteClawBrand.activationPrimaryAction)
            ...
            HStack(alignment: .top, spacing: OnboardingIntroPanelStyle.contentSpacing) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("Security notice")
                        ...
                        .foregroundStyle(.primary)
                    (
                        Text("The connected RemoteClaw agent can use device capabilities you enable.")
                            + Text(verbatim: " ")
                            + Text(
...
private struct OnboardingCommandChip: View {
    ...
    private let command = "remoteclaw qr"
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Button {
                ...
                Image(systemName: self.didCopy ? "checkmark" : "doc.on.doc")
                    .font(RemoteClawType.subheadSemiBold)
                    .foregroundStyle(
                        self.didCopy ? RemoteClawBrand.activationPrimaryAction : Color.secondary.opacity(0.56))
        ...
        .background {
            RoundedRectangle(cornerRadius: 20, style: .continuous)
                .fill(RemoteClawBrand.activationNeutralSurface)
        }
        .overlay {
            RoundedRectangle(cornerRadius: 20, style: .continuous)
                .stroke(RemoteClawBrand.activationNeutralStroke, lineWidth: 0.5)
        }
    }
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                OnboardingIntroPanel {
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        OnboardingSafetyRow(
                            symbol: "message.fill",
                            title: "Use RemoteClaw from your phone")
                    }
                }
                .padding(.top, 44)
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                VStack(spacing: 36) {
                            ...
                            if self.isConnecting {
                                HStack(spacing: 8) {
                                    ProgressView()
                                        ...
                                        .tint(RemoteClawBrand.activationPrimaryActionText)
                                    Text("Connecting…")
                                        .font(RemoteClawType.subheadSemiBold)
                                }
                            } else {
                    ...
                    VStack(spacing: 14) {
                        ...
                        .buttonStyle(RemoteClawSecondaryActionButtonStyle(height: 54, shadowOpacity: 0.018))
                        .disabled(self.isConnecting)
                    }
                }
                .padding(.top, 46)
        ...
        OnboardingActivationCanvas {
            VStack(spacing: 0) {
                ...
                ZStack(alignment: .bottomTrailing) {
                    RemoteClawActivationGlyph(size: 86)
                        .shadow(color: RemoteClawBrand.activationGlow.opacity(0.18), radius: 12, x: 0, y: 6)

                    Image(systemName: "checkmark")
                        .font(RemoteClawType.headlineBold)
                        .foregroundStyle(.white)
                        .frame(width: 30, height: 30)
                        .background {
                            Circle()
                                .fill(RemoteClawBrand.ok)
                        }
                        .overlay {
                            Circle()
...
struct OnboardingModeIcon: View {
    ...
    var body: some View {
        Image(systemName: self.symbol)
            .font(RemoteClawType.subheadSemiBold)
            .foregroundStyle(self.selected ? RemoteClawBrand.activationPrimaryActionText : .secondary)
            .frame(width: 34, height: 34)
            .background {
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .fill(self.selected ? RemoteClawBrand.activationPrimaryGradient : RemoteClawBrand
                        .activationNeutralGradient)
                    .shadow(
                        color: self.selected ? RemoteClawBrand.activationGlow.opacity(0.18) : .clear,
                        radius: 5,
                        x: 0,
                        y: 2)
            }
            .overlay {
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .stroke(
                        self.selected ? Color.white.opacity(0.30) : RemoteClawBrand.activationNeutralStroke,
                        lineWidth: 0.5)
            }
    }
}
        ...
        Button(action: self.action) {
            HStack(spacing: 12) {
                ...
                Image(systemName: self.selected ? "checkmark.circle.fill" : "circle")
                    ...
                    .foregroundStyle(
                        self.selected
                            ? RemoteClawBrand.activationPrimaryAction
