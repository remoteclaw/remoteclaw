...
export const zh_CN: TranslationMap = {
  ...
  nodes: {
    pairing: {
      ...
      review: "审核",
      waiting: "官方 RemoteClaw 移动应用在扫描后会自动连接。",
      help: "配对帮助",
      manageDevices: "管理设备",
    },
    binding: {
  ...
  worktrees: {
    title: "托管的 Worktrees",
    subtitle: "由 RemoteClaw 拥有的隔离代码库检出。",
    cleanNow: "立即清理",
    name: "名称",
    repo: "代码库",
    branch: "分支",
  ...
  overview: {
    ...
    cards: {
      ...
      modelAuthAttentionExpiredDesc: "{providers} — 使用 remoteclaw models auth 重新认证",
  ...
  login: {
    ...
    failure: {
      ...
      authRequired: {
        ...
        stepPaste: "粘贴 remoteclaw dashboard --no-open 提供的令牌，或输入已配置的密码。",
        stepGenerate:
          "如果未配置令牌，请在 Gateway 主机上运行 remoteclaw doctor --generate-gateway-token。",
        stepConnect: "更新凭据后再次点击 Connect。",
      },
      authFailed: {
        title: "认证不匹配",
        summary: "提供的凭据被拒绝。最常见原因是令牌已过期，或令牌来自另一个 Gateway URL。",
        stepDashboard: "运行 remoteclaw dashboard --no-open 并打开新的 URL，或粘贴其中的令牌。",
      ...
      pairing: {
        ...
        stepList: "在 Gateway 主机上运行 remoteclaw devices list。",
        stepApproveId: "批准此请求：remoteclaw devices approve {requestId}。",
      ...
      protocol: {
        ...
        stepDashboard:
          "使用 remoteclaw dashboard 重新打开提供的 dashboard，确保 UI 和 Gateway 来自同一安装。",
        stepDevUi: "如果使用 pnpm ui:dev，请基于当前 checkout 重新构建或重启开发 UI。",
        stepRestart: "更新 RemoteClaw 后重启 Gateway，使其提供当前协议。",
      },
      network: {
        title: "无法连接",
        summary: "浏览器无法完成 Gateway 连接。重试凭据前请检查目标和传输方式。",
        stepGateway: "使用 remoteclaw status 或 remoteclaw gateway run 确认 Gateway 正在运行。",
        stepUrl: "检查 WebSocket URL；当 Gateway 位于 HTTPS/Tailscale Serve 后面时使用 wss://。",
        stepDashboard:
          "使用 remoteclaw dashboard --no-open 重新打开 dashboard，以重新复制当前 URL 和认证详情。",
      },
    },
  },
  connection: {
