...
@MainActor
final class RemoteClawSnapshotUITests: XCTestCase {
    ...
    private func launchApp(for target: ScreenshotTarget) {
        ...
        app.launchArguments += [
            "--remoteclaw-screenshot-mode",
            "--remoteclaw-initial-tab",
            target.initialTab,
            "--remoteclaw-initial-destination",
            target.initialDestination,
            "--remoteclaw-sidebar-visibility",
            "hidden",
        ]
        app.launch()
        self.app = app
