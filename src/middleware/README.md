...
| ------------------------ | --------- | ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cold_start_ms`          | Histogram | All                     | First valid NDJSON line on the configured `ndjsonStream` (`stdout` for Gemini/Codex/OpenCode, `stderr` for Claude); blank or malformed lines do not count | Wall time from immediately before the `spawn()` syscall to the first valid NDJSON line. Includes process-creation cost; the CLI is "warm" once the first parseable line arrives.                                  |
| `inflight_subprocesses`  | Gauge     | All                     | Increment inside the outer `try` of `execute()` (so synchronous `spawn()` throws still balance); decrement in the matching `finally`                      | Number of concurrent CLI subprocesses for this backend, shared across all runtime instances. Balanced on every termination path (normal completion, exception, generator break, abort).                           |
| `mcp_config_setup_ms`    | Histogram | Gemini, Codex, OpenCode | Per-runtime `*McpConfigManager.setup()` wrapped via `timedMcpSetup()`                                                                                     | Time spent staging the per-runtime MCP config (Codex: merge `[mcp_servers.*]` TOML sections; Gemini/OpenCode: merge a `mcpServers` JSON field). Covers read existing + merge + write.                             |
| `mcp_config_teardown_ms` | Histogram | Gemini, Codex, OpenCode | Per-runtime `*McpConfigManager.teardown()` wrapped via `timedMcpTeardown()`                                                                               | Time spent restoring the original MCP config (or removing the file the manager created).                                                                                                                          |
