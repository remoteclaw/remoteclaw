...
let triggerRemoteClawRestart: typeof import("./restart.js").triggerRemoteClawRestart;
...
beforeAll(async () => {
  ...
  ({ triggerRemoteClawRestart } = await import("./restart.js"));
});
  ...
  it("kills stale gateway pids discovered on the gateway port", () => {
    ...
    spawnSyncMock
      .mockReturnValueOnce({
        error: undefined,
        status: 0,
        stdout: [`p${stalePidA}`, "cremoteclaw", `p${stalePidB}`, "cremoteclaw-gateway"].join("\n"),
      })
      .mockReturnValue({
        error: undefined,
        status: 1,
  ...
  it("uses explicit port override when provided", () => {
    ...
    spawnSyncMock
      .mockReturnValueOnce({
        error: undefined,
        status: 0,
        stdout: [`p${stalePid}`, "cremoteclaw"].join("\n"),
      })
      .mockReturnValue({
        error: undefined,
        status: 1,
  ...
  it("continues when launchctl bootstrap reports the service is already loaded", () => {
    ...
    expect(result).toEqual({
      ...
      tried: [
        ...
        `launchctl kickstart gui/${uid}/org.remoteclaw.gateway`,
      ],
    });
  });
});
