...
describe("plugin-sdk qa-runner-runtime", () => {
  ...
  it("loads bundled plugin test APIs with the private QA source tree override", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "remoteclaw-qa-test-api-root-");
    resolveRemoteClawPackageRootSync.mockReturnValue(sourceRoot);
    ...
    expect(loadBundledPluginPublicSurfaceModuleSync).toHaveBeenCalledWith({
      ...
      env: expect.objectContaining({
        REMOTECLAW_ENABLE_PRIVATE_QA_CLI: "1",
        REMOTECLAW_BUNDLED_PLUGINS_DIR: path.join(sourceRoot, "extensions"),
      }),
    });
  });
  ...
  it("prefers the source bundled tree for private qa discovery in repo checkouts", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "remoteclaw-qa-runner-root-");
    resolveRemoteClawPackageRootSync.mockReturnValue(sourceRoot);
  ...
  it("fails when runtime registrations include an undeclared command", async () => {
    ...
    expect(() => module.listQaRunnerCliContributions()).toThrow(
      'QA runner plugin "qa-matrix" exported "extra" from runtime-api.js but did not declare it in remoteclaw.plugin.json',
    );
  });
});
