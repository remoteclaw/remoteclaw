...
describe("resolveHeartbeatPrompt", () => {
  it("uses default or trimmed override prompts", async () => {
    const cases = [
      { cfg: {} as RemoteClawConfig, expected: HEARTBEAT_PROMPT },
      {
        cfg: {
    ...
    for (const testCase of cases) {
      expect(await resolveHeartbeatPrompt(testCase.cfg)).toBe(testCase.expected);
    }
  });
});
  ...
  it("resolves configured and forced session key overrides", async () => {
    ...
    try {
      const cases = typedCases<{
        ...
        peerId: string;
        message: string;
        applyOverride: (params: { cfg: RemoteClawConfig; sessionKey: string }) => void;
      ...
      }>([
        {
          ...
          message: "Group alert",
          applyOverride: ({ cfg, sessionKey }: { cfg: RemoteClawConfig; sessionKey: string }) => {
