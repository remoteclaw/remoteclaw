...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredRemoteClawTmpDir: pluginApiMocks.resolvePreferredRemoteClawTmpDir,
...
function createApi(params?: {
  runtime?: RemoteClawPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as RemoteClawPluginApi["runtime"],
...
function createChannelRuntime(
  ...
  } as unknown as RemoteClawPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredRemoteClawTmpDir(), { recursive: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the RemoteClaw iOS app:");
    expect(text).toContain("![RemoteClaw pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the RemoteClaw iOS app:");
