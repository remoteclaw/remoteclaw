  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "remoteclaw-gateway.service"]);
...
describe("readSystemdServiceRuntime", () => {
  it("surfaces systemd cgroup metrics and KillMode", async () => {
    ...
    expect(runtime).toEqual({
      ...
      systemd: {
        unit: "remoteclaw-gateway.service",
        killMode: "process",
        tasksCurrent: 807,
...
describe("splitArgsPreservingQuotes", () => {
  ...
  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('remoteclaw --path "C:\\\\Program Files\\\\RemoteClaw"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["remoteclaw", "--path", "C:\\\\Program Files\\\\RemoteClaw"]);
