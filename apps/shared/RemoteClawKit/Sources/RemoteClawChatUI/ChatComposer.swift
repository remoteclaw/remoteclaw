...
private struct CleanChatComposerSurface: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        content
            ...
            .overlay(
                RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                    .strokeBorder(RemoteClawChatTheme.composerBorder, lineWidth: 1))
        #else
...
struct RemoteClawChatComposer: View {
    ...
    let voiceNoteControl: RemoteClawChatVoiceNoteControl?
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            if let voiceNoteControl, voiceNoteControl.recorder.isRecording {
                RemoteClawVoiceNoteRecordingRow(recorder: voiceNoteControl.recorder)
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 5) {
                    ...
                    if self.viewModel.showsModelPicker {
                        self.modelPicker
                        if self.viewModel.modelSelectionID != RemoteClawChatViewModel.defaultModelSelectionID {
                            self.modelPinButton
                        }
                    }
                }
    ...
    private func contextUsageIndicator(_ fraction: Double) -> some View {
        ...
        let color = fraction >= 0.8 ? RemoteClawChatTheme.warning : RemoteClawChatTheme.muted
        return ZStack {
            Circle()
                .stroke(RemoteClawChatTheme.muted.opacity(0.2), lineWidth: 2)
            Circle()
    ...
    private var thinkingPicker: some View {
                ...
                set: { next in self.viewModel.selectThinkingLevel(next) }))
        {
            ForEach(self.viewModel.thinkingLevelOptions) { option in
                Text(option.label)
                    .font(RemoteClawChatTypography.captionSemiBold)
                    .tag(option.id)
            }
        }
        .labelsHidden()
    ...
    private var modelPicker: some View {
                ...
                set: { next in self.viewModel.selectModel(next) }))
        {
            Text(self.viewModel.defaultModelLabel)
                .font(RemoteClawChatTypography.captionSemiBold)
                .tag(RemoteClawChatViewModel.defaultModelSelectionID)
    ...
    private func modelOptions(_ models: [RemoteClawChatModelChoice]) -> some View {
                ...
                set: { next in self.viewModel.switchSession(to: next) }))
        {
            ForEach(self.viewModel.sessionChoices, id: \.key) { session in
                Text(session.displayName ?? session.key)
                    .font(RemoteClawChatTypography.mono(size: 12, relativeTo: .caption))
                    .tag(session.key)
            }
        }
        .labelsHidden()
                ...
                { (att: RemoteClawPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            ...
                            Text("Voice note")
                                .font(RemoteClawChatTypography.caption)
                            if let durationSeconds = att.durationSeconds {
                                Text(remoteClawVoiceNoteDurationLabel(durationSeconds))
                                    .font(RemoteClawChatTypography.caption)
                                    .foregroundStyle(.secondary)
                            }
                        } else {
                    ...
                    .padding(.vertical, 5)
                    .background(RemoteClawChatTheme.accent.opacity(0.08))
                    .clipShape(Capsule())
                }
            }
        }
    ...
    private func compactTalkButton(_ talkControl: RemoteClawChatTalkControl) -> some View {
        Button {
            ...
            Image(systemName: talkControl.isEnabled ? "stop.fill" : "waveform")
                .font(RemoteClawChatTypography.body(size: 14, weight: .semibold, relativeTo: .subheadline))
                ...
                .background {
                    Circle()
                        .fill(talkControl.isEnabled
                            ...
                            : AnyShapeStyle(RemoteClawChatTheme.accent))
    ...
    private var connectionPill: some View {
        HStack(spacing: 6) {
            ...
            Text(self.connectionStatusText)
                .font(RemoteClawChatTypography.caption2)
                .foregroundStyle(.secondary)
        }
        ...
        ZStack(alignment: self.editorOverlayAlignment) {
            if self.viewModel.input.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                Text(self.placeholderText)
                    .font(RemoteClawChatTypography.body)
    ...
    private func slashCommandRow(
        _ command: RemoteClawChatCommandChoice,
        isHighlighted: Bool) -> some View
    {
        HStack(alignment: .top, spacing: 0) {
            VStack(alignment: .leading, spacing: 3) {
                Text(command.displayInvocation)
                    .font(RemoteClawChatTypography.mono(
                        size: 15,
                        weight: .semibold,
    ...
    private func selectSlashCommand(_ command: RemoteClawChatCommandChoice) {
                ...
                Button {
                    ...
                    if self.viewModel.isAborting {
                        ...
                        Image(systemName: "stop.fill")
                            .font(RemoteClawChatTypography.display(size: 13, weight: .semibold, relativeTo: .caption))
                    }
                }
                .buttonStyle(.plain)
                .foregroundStyle(.white)
