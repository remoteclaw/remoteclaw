...
agent can inspect, including `remoteclaw.json`, `auth-profiles.json`, `.env`, or
...
`remoteclaw secrets audit --check` reports no plaintext secret residue.
</Warning>
...
- legacy plaintext residue has been scrubbed from `remoteclaw.json`,
  `auth-profiles.json`, `.env`, and generated `models.json` files
- `remoteclaw secrets audit --check` is clean after the migration
...
<AccordionGroup>
  <Accordion title="Examples of inactive surfaces">
    ...
    - `gateway.auth.token` SecretRef is inactive for startup auth resolution when `REMOTECLAW_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
<AccordionGroup>
  ...
  <Accordion title="Exec provider">
    ...
    - Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). RemoteClaw validates the resolved target path.
    ...
    - Plugin-managed exec providers can use `pluginIntegration` instead of
      copied `command`/`args`. RemoteClaw resolves the current command details
...
```json5
{
  secrets: {
    providers: {
      xai_key_file: {
        source: "file",
        path: "~/.remoteclaw/secrets/xai-api-key.txt",
        mode: "singleValue",
      },
    },
  },
...
<AccordionGroup>
      ...
      secrets: {
        providers: {
          onepassword_openai: {
            ...
            args: ["read", "op://Personal/RemoteClaw QA API Key/password"],
            passEnv: ["HOME"],
            jsonOnly: false,
          },
        },
  ...
  <Accordion title="Bitwarden Secrets Manager (`bws`)">
    ...
    `scripts/secrets/remoteclaw-bws-resolver.mjs`; install or copy it to an absolute
    ...
    ```json5
    {
      secrets: {
        providers: {
          bws: {
            source: "exec",
            command: "/usr/local/bin/remoteclaw-bws-resolver.mjs",
        ...
        providers: {
          openai: {
            ...
            apiKey: {
              source: "exec",
              provider: "bws",
              id: "remoteclaw/providers/openai/apiKey",
            },
          },
        },
      },
    ...
    SecretRef id contract, such as `remoteclaw/providers/openai/apiKey`; env-var
    ...
    ```bash
    remoteclaw secrets audit --allow-exec
    ```
      ...
      secrets: {
        providers: {
          vault_openai: {
            ...
            allowSymlinkCommand: true, // required for Homebrew symlinked binaries
            trustedDirs: ["/opt/homebrew"],
            args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/remoteclaw"],
  ...
  <Accordion title="password-store (`pass`)">
    ...
    `/usr/local/bin/remoteclaw-pass-resolver`. The `#!/usr/bin/env node` shebang
    ...
    ```json5
    {
      secrets: {
        providers: {
          pass_store: {
            source: "exec",
            command: "/usr/local/bin/remoteclaw-pass-resolver",
    ...
    ```bash
    remoteclaw secrets audit --check
    remoteclaw secrets audit --allow-exec
    ```
...
- The redaction sentinel `__REMOTECLAW_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.

Warning and audit signals:

- `SECRETS_REF_OVERRIDES_PLAINTEXT` (runtime warning)
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `remoteclaw.json` refs)
...
<Steps>
  ...
  <Step title="Configure and apply SecretRefs">
    ```bash
    remoteclaw secrets configure --apply
    ```
  </Step>
  <Step title="Re-audit">
    ```bash
...
<AccordionGroup>
  <Accordion title="secrets audit">
    ...
    - plaintext values at rest (`remoteclaw.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
    - plaintext sensitive provider header residues in generated `models.json` entries
    - unresolved refs
    - precedence shadowing (`auth-profiles.json` taking priority over `remoteclaw.json` refs)
    - legacy residues (`auth.json`, OAuth reminders)

    Exec note:

    - By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
    - Use `remoteclaw secrets audit --allow-exec` to execute exec providers during audit.
  ...
  <Accordion title="secrets configure">
    ...
    - lets you select supported secret-bearing fields in `remoteclaw.json` plus `auth-profiles.json` for one agent scope
    ...
    - `remoteclaw secrets configure --providers-only`
    - `remoteclaw secrets configure --skip-provider-setup`
    - `remoteclaw secrets configure --agent <id>`
  ...
  <Accordion title="secrets apply">
    ...
    ```bash
    remoteclaw secrets apply --from /tmp/remoteclaw-secrets-plan.json
    remoteclaw secrets apply --from /tmp/remoteclaw-secrets-plan.json --allow-exec
    remoteclaw secrets apply --from /tmp/remoteclaw-secrets-plan.json --dry-run
    remoteclaw secrets apply --from /tmp/remoteclaw-secrets-plan.json --dry-run --allow-exec
    ```
...
<Warning>
RemoteClaw intentionally does not write rollback backups containing historical plaintext secret values.
</Warning>
