...
read_when:
  ...
  - You are changing plugin behavior across RemoteClaw and Codex harness turns
---
...
Codex mode is not RemoteClaw with a different model call underneath. Codex owns more of
the native model loop, and RemoteClaw adapts its plugin, tool, session, and
diagnostic surfaces around that boundary.

RemoteClaw still owns channel routing, session files, visible message delivery,
RemoteClaw dynamic tools, approvals, media delivery, and a transcript mirror.
...
explicit RemoteClaw compatibility route keeps the normal RemoteClaw system prompt even
...
according to the active Codex thread config. RemoteClaw starts and resumes native
Codex threads with Codex's built-in personality disabled so workspace
personality files and RemoteClaw agent identity stay authoritative. Lightweight
RemoteClaw runs still preserve their existing project-doc suppression. RemoteClaw
developer instructions cover RemoteClaw runtime concerns such as source-channel
delivery, RemoteClaw dynamic tools, ACP delegation, adapter context, and the
active agent workspace profile files. RemoteClaw skill catalogs and tool-routed
...
When an RemoteClaw session is attached to an existing Codex thread, the next turn
...
normally, and RemoteClaw posts the final text to the source conversation. Set
...
Codex heartbeat turns also get `heartbeat_respond` in the searchable RemoteClaw
...
| Layer                                 | Owner                      | Purpose                                                             |
| ------------------------------------- | -------------------------- | ------------------------------------------------------------------- |
| RemoteClaw plugin hooks               | RemoteClaw                 | Product/plugin compatibility across RemoteClaw and Codex harnesses. |
| Codex app-server extension middleware | RemoteClaw bundled plugins | Per-turn adapter behavior around RemoteClaw dynamic tools.          |
...
Codex's app-server reviewer and RemoteClaw's approval bridge handle real
...
Codex-level controls. They are not exposed as RemoteClaw plugin hooks in the v1
contract.
...
RemoteClaw can mirror selected events, but it cannot rewrite the native Codex
...
to the matching app-server approval. If an RemoteClaw `before_tool_call` hook
returns `requireApproval` while the native payload sets report approval mode
(`remoteclaw_approval_mode` is `"report"`), the native hook relay records the
plugin approval requirement and returns no native decision. When Codex sends the
app-server approval request for the same tool use, RemoteClaw opens the plugin
approval prompt and maps the decision back to Codex. Codex `PermissionRequest`
events are a separate approval path and can still route through RemoteClaw
...
notifications and RemoteClaw adapter state, not native Codex hook commands.
RemoteClaw's `before_compaction`, `after_compaction`, `llm_input`, and
...
They do not invoke RemoteClaw plugin hooks.
...
| RemoteClaw channel routing and delivery       | Supported                                                                        | Telegram, Discord, Slack, WhatsApp, iMessage, and other channels stay outside the model runtime.                                                                                                                                                                                                                                                                                                                                                                                          |
| RemoteClaw dynamic tools                      | Supported                                                                        | Codex asks RemoteClaw to execute these tools, so RemoteClaw stays in the execution path.                                                                                                                                                                                                                                                                                                                                                                                                  |
| Prompt and context plugins                    | Supported                                                                        | RemoteClaw projects RemoteClaw-specific prompt/context into the Codex turn while leaving Codex-owned base, model, and configured project-doc prompts in the native Codex lane. RemoteClaw disables Codex's built-in personality for native threads so agent workspace personality files remain authoritative. Native Codex developer instructions accept only command guidance explicitly scoped to `codex_app_server`; legacy global command hints remain for non-Codex prompt surfaces. |
...
| Dynamic tool hooks                            | Supported                                                                        | `before_tool_call`, `after_tool_call`, and tool-result middleware run around RemoteClaw-owned dynamic tools.                                                                                                                                                                                                                                                                                                                                                                              |
...
| Native permission policy                      | Supported through Codex app-server approvals and compatibility native hook relay | Codex app-server approval requests route through RemoteClaw after Codex review. The `PermissionRequest` native hook relay is opt-in for native approval modes because Codex emits it before guardian review.                                                                                                                                                                                                                                                                              |
| App-server trajectory capture                 | Supported                                                                        | RemoteClaw records the request it sent to app-server and the app-server notifications it receives.                                                                                                                                                                                                                                                                                                                                                                                        |

Not supported in Codex runtime v1:

| Surface                                             | V1 boundary                                                                                                                                       | Future path                                                                               |
| --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Native tool argument mutation                       | Codex native pre-tool hooks can block, but RemoteClaw does not rewrite Codex-native tool arguments.                                               | Requires Codex hook/schema support for replacement tool input.                            |
| Editable Codex-native transcript history            | Codex owns canonical native thread history. RemoteClaw owns a mirror and can project future context, but should not mutate unsupported internals. | Add explicit Codex app-server APIs if native thread surgery is needed.                    |
| `tool_result_persist` for Codex-native tool records | That hook transforms RemoteClaw-owned transcript writes, not Codex-native tool records.                                                           | Could mirror transformed records, but canonical rewrite needs Codex support.              |
| Rich native compaction metadata                     | RemoteClaw can request native compaction, but does not receive a stable kept/dropped list, token delta, completion summary, or summary payload.   | Needs richer Codex compaction events.                                                     |
| Compaction intervention                             | RemoteClaw does not let plugins or context engines veto, rewrite, or replace native Codex compaction.                                             | Add Codex pre/post compaction hooks if plugins need to veto or rewrite native compaction. |
| Byte-for-byte model API request capture             | RemoteClaw can capture app-server requests and notifications, but Codex core builds the final OpenAI API request internally.                      | Needs a Codex model-request tracing event or debug API.                                   |
...
RemoteClaw remembers that exact provider/session/tool input/cwd fingerprint for a
...
default `messages.queue.mode: "steer"`, RemoteClaw batches steer-mode chat
...
case, RemoteClaw waits for the active run to finish before starting the prompt.
...
harness, RemoteClaw also calls Codex app-server `feedback/upload` for relevant
...
error. The completed diagnostics reply lists the channels, RemoteClaw session ids,
Codex thread ids, and local `codex resume <thread-id>` commands for the threads
that were sent.

If you deny or ignore the approval, RemoteClaw does not print those Codex ids and
...
to Codex app-server. RemoteClaw does not run preflight compaction for Codex turns,
does not replace Codex compaction with context-engine compaction, and does not
fall back to RemoteClaw or public OpenAI summarization when native Codex
compaction cannot be started. RemoteClaw keeps a transcript mirror for channel
...
RemoteClaw returns after starting that native operation. It does not wait for
completion, impose a separate RemoteClaw timeout, restart the shared Codex
app-server, or record the operation as an RemoteClaw-completed compaction.

When a context engine requests Codex thread-bootstrap projection, RemoteClaw
...
RemoteClaw continues to own media delivery and media provider selection. Image,
...
through the normal RemoteClaw delivery path. Media generation does not require the legacy runtime.
When Codex emits a native image-generation item with a `savedPath`, RemoteClaw
