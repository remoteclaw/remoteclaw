...
RemoteClaw can talk to you on any chat app you already use. Each channel connects via the Gateway.
...
- [iMessage](/channels/imessage) - Native macOS integration via the `imsg` bridge on a signed-in Mac (or SSH wrapper when the Gateway runs elsewhere), including private API actions for replies, tapbacks, effects, attachments, and group management. Preferred for new RemoteClaw iMessage setups when host permissions and Messages access fit.
...
- Channels can run simultaneously; configure multiple and RemoteClaw will route per chat.
