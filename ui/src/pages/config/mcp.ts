...
import { redactSensitiveUrlLikeString } from "@remoteclaw/net-policy/redact-sensitive-url";
...
function renderServerRow(props: McpViewProps, server: McpServerRow) {
  ...
  const probeCommand = `remoteclaw mcp probe ${quotedName}`;
  const loginCommand = `remoteclaw mcp login ${quotedName}`;
  return html`
    ...
    <section class="mcp-page">
      ...
      <section class="card mcp-command-card">
        ...
        <div class="mcp-command-card__grid">
          <code>remoteclaw mcp status --verbose</code>
          <code>remoteclaw mcp doctor --probe</code>
          <code>remoteclaw mcp login &lt;name&gt;</code>
          <code>remoteclaw mcp reload</code>
        </div>
      </section>
      ...
      <section class="card mcp-server-list">
        ...
        ${
          rows.length
            ...
            : html`
                <div class="data-table-empty-state">No MCP servers configured.</div>
              `
        }
      </section>
