    ...
    var body: some View {
        HStack(alignment: .center, spacing: 12) {
            Image(systemName: self.item.icon)
                ...
                .background {
                    RoundedRectangle(cornerRadius: RemoteClawRadius.sm, style: .continuous)
                ...
                HStack(alignment: .firstTextBaseline, spacing: 8) {
                    ...
                    if self.item.isPinned {
                        Image(systemName: "pin.fill")
                            .font(RemoteClawType.caption2Medium)
...
struct CommandSessionActionsModifier: ViewModifier {
    ...
    let session: RemoteClawChatSessionEntry
...
extension View {
    func commandSessionActions(
        session: RemoteClawChatSessionEntry,
        categories: [String],
...
struct CommandViewMoreRow: View {
    var body: some View {
        Label("View More", systemImage: "chevron.right")
            .font(RemoteClawType.subheadBold)
            .foregroundStyle(RemoteClawBrand.accent)
    ...
    var body: some View {
        HStack(spacing: 10) {
            Image(systemName: self.icon)
                .font(RemoteClawType.captionBold)
                .foregroundStyle(RemoteClawBrand.ok)
                .frame(width: 30, height: 30)
                .background {
                    RoundedRectangle(cornerRadius: RemoteClawRadius.xs, style: .continuous)
                        .fill(RemoteClawBrand.ok.opacity(0.10))
                }
