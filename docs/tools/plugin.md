...
A plugin is just a **small code module** that extends RemoteClaw with extra
...
1. **Manifest + discovery**
   ...
   `remoteclaw.plugin.json` plus package metadata first.
...
2. read `remoteclaw.plugin.json` and package metadata
...
Each plugin must include a `remoteclaw.plugin.json` file in its root. If a path
...
Channel plugins can advertise onboarding metadata via `remoteclaw.channel` and
...
3. **Wizard integration**
   `wizard.onboarding` adds an entry to `remoteclaw onboard`.
...
- `wizard.onboarding` adds a provider choice to `remoteclaw onboard`.
