...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> RemoteClawDeviceStatusPayload {
        ...
        return RemoteClawDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> RemoteClawDeviceInfoPayload {
        ...
        return RemoteClawDeviceInfoPayload(
    ...
    private func batteryStatus() -> RemoteClawBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: RemoteClawBatteryState = switch device.batteryState {
        ...
        return RemoteClawBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> RemoteClawThermalStatusPayload {
        let state: RemoteClawThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return RemoteClawThermalStatusPayload(state: state)
    }

    private func storageStatus() -> RemoteClawStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return RemoteClawStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
