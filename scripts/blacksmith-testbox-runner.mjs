...
import {
  ...
  evaluateRemoteClawTestboxClaim,
  resolveTestboxId,
  writeRemoteClawTestboxClaim,
...
export function resolveTestboxSyncTimeoutMs(env = process.env) {
  const raw = env.REMOTECLAW_TESTBOX_SYNC_TIMEOUT_MS;
      ...
      () => {
        ...
        stderr.write(
          `Blacksmith Testbox sync produced no post-sync output for ${syncTimeoutMs}ms; terminating local runner. ` +
            "Rerun with REMOTECLAW_TESTBOX_SYNC_TIMEOUT_MS=0 to disable this guard.\n",
        );
        timedOut = true;
        syncingSince = 0;
        child.kill?.("SIGTERM");
...
export async function runBlacksmithTestboxRunner({
  ...
  if (!testboxId) {
    stderr.write(
      "Missing Testbox id. Pass `--id <tbx_id>` or set REMOTECLAW_TESTBOX_ID from this session's warmup output.\n",
    );
    return 2;
  }
  ...
  if (shouldClaim) {
    const claim = writeRemoteClawTestboxClaim({ cwd: root, env, testboxId });
    stdout.write(`RemoteClaw Testbox claim written: ${testboxId} -> ${claim.claimPath}\n`);
  } else {
    const claimResult = evaluateRemoteClawTestboxClaim({
      cwd: root,
      env,
      testboxId,
    });
    if (!claimResult.ok) {
      ...
      stderr.write(
        "Refusing to run a Testbox that was not claimed by this RemoteClaw checkout. Run:\n" +
          "  blacksmith testbox warmup ci-check-testbox.yml --ref main --idle-timeout 90\n" +
  ...
  if (blacksmithArgs.length === 0) {
    stdout.write(`Testbox local key and RemoteClaw claim ok: ${testboxId}\n`);
    return 0;
  }
