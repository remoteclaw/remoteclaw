...
`plugins install` registers and enables the plugin, so no separate `remoteclaw plugins enable matrix` step is needed. The plugin still does nothing until you configure the channel below. See [Plugins](/tools/plugin) for general plugin behavior and install rules.
...
If matching `MATRIX_*` env vars already exist and the selected account has no saved auth, the wizard offers an env-var shortcut. To resolve room names before saving an allowlist, run `remoteclaw channels resolve --channel matrix "Project Room"`. When E2EE is enabled, the wizard writes the config and runs the same bootstrap as [`remoteclaw matrix encryption setup`](#encryption-and-verification).
...
RemoteClaw cannot tell at invite time whether an invited room is a DM or a group, so all invites — including DM-style invites — go through `autoJoin` first. `dm.policy` only applies later, after the bot has joined and the room has been classified.
...
Matrix stores cached credentials under `~/.remoteclaw/credentials/matrix/`:

- default account: `credentials.json`
- named accounts: `credentials-<account>.json`

When cached credentials exist there, RemoteClaw treats Matrix as configured even if the access token is not in the config file — that covers setup, `remoteclaw doctor`, and channel-status probes.
...
Matrix reply streaming is opt-in. `streaming` controls how RemoteClaw delivers the in-flight assistant reply; `blockStreaming` controls whether each completed block is preserved as its own Matrix message.
...
All `remoteclaw matrix` commands accept `--verbose` (full diagnostics), `--json` (machine-readable output), and `--account <id>` (multi-account setups). Output is concise by default with quiet internal SDK logging. The examples below show the canonical form; add the flags as needed.

### Enable encryption

```bash
remoteclaw matrix encryption setup
```
...
```bash
printf '%s\n' "$MATRIX_RECOVERY_KEY" | remoteclaw matrix verify device --recovery-key-stdin
```
...
```bash
remoteclaw matrix verify self
```

`verify self` waits for `Cross-signing verified: yes` before it exits successfully. Use `--timeout-ms <ms>` to tune the wait.

The literal-key form `remoteclaw matrix verify device "<recovery-key>"` is also accepted, but the key ends up in your shell history.
...
If the homeserver requires UIA to upload cross-signing keys, RemoteClaw tries no-auth first, then `m.login.dummy`, then `m.login.password` (requires `channels.matrix.password`).
...
printf '%s\n' "$MATRIX_RECOVERY_KEY" | remoteclaw matrix verify backup restore --recovery-key-stdin
```
...
```bash
remoteclaw matrix verify list
```

Lists pending verification requests for the selected account.

```bash
remoteclaw matrix verify request --own-user
remoteclaw matrix verify request --user-id @ops:example.org --device-id ABCDEF
```

Sends a verification request from this RemoteClaw account. `--own-user` requests self-verification (you accept the prompt in another Matrix client of the same user); `--user-id`/`--device-id`/`--room-id` target someone else. `--own-user` cannot be combined with the other targeting flags.
...
| Command                                      | Purpose                                                             |
| -------------------------------------------- | ------------------------------------------------------------------- |
| `remoteclaw matrix verify accept <id>`       | Accept an inbound request                                           |
| `remoteclaw matrix verify start <id>`        | Start the SAS flow                                                  |
| `remoteclaw matrix verify sas <id>`          | Print the SAS emoji or decimals                                     |
| `remoteclaw matrix verify confirm-sas <id>`  | Confirm that the SAS matches what the other client shows            |
| `remoteclaw matrix verify mismatch-sas <id>` | Reject the SAS when the emoji or decimals do not match              |
| `remoteclaw matrix verify cancel <id>`       | Cancel; takes optional `--reason <text>` and `--code <matrix-code>` |
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    ...
    Startup also runs a conservative crypto bootstrap pass that reuses the current secret storage and cross-signing identity. If bootstrap state is broken, RemoteClaw attempts a guarded repair even without `channels.matrix.password`; if the homeserver requires password UIA, startup logs a warning and stays non-fatal. Already-owner-signed devices are preserved.
  ...
  <Accordion title="Verification notices">
    ...
    Incoming requests from another Matrix client are tracked and auto-accepted. For self-verification, RemoteClaw starts the SAS flow automatically and confirms its own side once emoji verification is available — you still need to compare and confirm "They match" in your Matrix client.
  ...
  <Accordion title="Deleted or invalid Matrix device">
    If `verify status` says the current device is no longer listed on the homeserver, create a new RemoteClaw Matrix device. For password login:
...
remoteclaw matrix account add \
  ...
  --password '<password>' \
  --device-name RemoteClaw-Gateway
```

    For token auth, create a fresh access token in your Matrix client or admin UI, then update RemoteClaw:
...
```
  ...
  <Accordion title="Device hygiene">
    Old RemoteClaw-managed devices can accumulate. List and prune:
...
```
  ...
  <Accordion title="Crypto store">
    ...
    Encrypted runtime state lives under `~/.remoteclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/` and includes the sync store, crypto store, recovery key, IDB snapshot, thread bindings, and startup verification state. When the token changes but the account identity stays the same, RemoteClaw reuses the best existing root so prior state remains visible.
...
You can pass both options in one call. Matrix accepts `mxc://` avatar URLs directly; when you pass `http://` or `https://`, RemoteClaw uploads the file first and stores the resolved `mxc://` URL into `channels.matrix.avatarUrl` (or the per-account override).
...
`dm.sessionScope` decides how Matrix DM rooms map to RemoteClaw sessions:
...
- DM user-target reuse only kicks in when the current session metadata proves the same DM peer on the same Matrix account; otherwise RemoteClaw falls back to normal user-scoped routing.
...
When RemoteClaw detects a Matrix DM room colliding with another DM room on the same shared session, it posts a one-time `m.notice` in that room pointing to the `/focus` escape hatch and suggesting a `dm.sessionScope` change. The notice only appears when thread bindings are enabled.
...
If an unapproved Matrix user keeps messaging you before approval, RemoteClaw reuses the same pending pairing code and may send a reminder reply after a short cooldown instead of minting a new code.

See [Pairing](/channels/pairing) for the shared DM pairing flow and storage layout.

## Direct room repair

If direct-message state drifts out of sync, RemoteClaw can end up with stale `m.direct` mappings that point at old solo rooms instead of the live DM. Inspect the current mapping for a peer:
...
- If you have multiple accounts and one is literally named `default`, RemoteClaw uses it implicitly even when `defaultAccount` is unset.
...
- When RemoteClaw promotes a single-account config to multi-account during repair or setup, it preserves the existing named account if one exists or `defaultAccount` already points at one. Only Matrix auth/bootstrap keys move into the promoted account; shared delivery-policy keys stay at the top level.
...
RemoteClaw keeps internal session keys canonical for storage, so those lowercase
