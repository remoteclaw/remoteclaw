...
describe("resolveRemoteClawAgentDir", () => {
      ...
      withEnv(
        {
          ...
          REMOTECLAW_AGENT_DIR: undefined,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveRemoteClawAgentDir();
  ...
  it("honors REMOTECLAW_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          REMOTECLAW_STATE_DIR: undefined,
          REMOTECLAW_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveRemoteClawAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when REMOTECLAW_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers REMOTECLAW_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          REMOTECLAW_STATE_DIR: undefined,
          REMOTECLAW_AGENT_DIR: primaryOverride,
