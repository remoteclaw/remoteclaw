...
remoteclaw security audit --auth password --password <password>
...
Run `remoteclaw doctor --fix` to rotate a persisted reused `hooks.token`, then update external hook senders to use the new token.
...
- Warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `remoteclaw.browserConfigEpoch`) and recommends `remoteclaw sandbox recreate --browser --all`.
