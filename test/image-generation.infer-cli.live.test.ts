...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.REMOTECLAW_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through remoteclaw infer", () => {
    const outputBase = path.join(os.tmpdir(), `remoteclaw-infer-image-${process.pid}.png`);
