...
remoteclaw agent --agent ops --message-file ./task.md
remoteclaw agent --agent ops --model openai/gpt-5.4 --message "Summarize logs"
remoteclaw agent --session-key agent:ops:incident-42 --message "Summarize status"
remoteclaw agent --agent ops --session-key incident-42 --message "Summarize status"
...
remoteclaw agent --agent ops --message "Run locally" --local
```
...
- Slash commands (for example `/compact`) cannot run through `--message`. The CLI rejects them and points you at the first-class command instead (`remoteclaw sessions compact <key>` for compaction).
