...
export OPENCLAW_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.remoteclaw/remoteclaw.json' | tr -d '\n'
)"
...
- Use the `remoteclaw` wrapper for guest `message send/read`; `node remoteclaw.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/remoteclaw-parallels-smoke.*`
...
- host inbound nonce appears in `remoteclaw message read` output
