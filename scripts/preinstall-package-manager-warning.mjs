...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[remoteclaw] warning: detected ${packageManager} for install lifecycle.`,
    "[remoteclaw] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[remoteclaw] prefer: corepack pnpm install",
  ].join("\n");
}
