...
function collectConfiguredPluginEntryIds(config: RemoteClawConfig): string[] {
...
function resolveRegister(mod: RemoteClawPluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: RemoteClawPluginModule;
  try {
    mod = getJiti(setupSource)(setupSource) as RemoteClawPluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: RemoteClawPluginModule }).default ?? mod);
