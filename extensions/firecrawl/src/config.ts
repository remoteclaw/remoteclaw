...
import { normalizeSecretInput } from "remoteclaw/plugin-sdk/provider-auth";
...
type WebSearchConfig = NonNullable<RemoteClawConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<RemoteClawConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: RemoteClawConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: RemoteClawConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: RemoteClawConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: RemoteClawConfig): FirecrawlFetchConfig {
...
export function resolveFirecrawlApiKey(cfg?: RemoteClawConfig): string | undefined {
...
export function resolveFirecrawlBaseUrl(cfg?: RemoteClawConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: RemoteClawConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: RemoteClawConfig, override?: number): number {
