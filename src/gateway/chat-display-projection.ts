...
import { asOptionalRecord as readRecord } from "@remoteclaw/normalization-core/record-coerce";
import { normalizeOptionalString } from "@remoteclaw/normalization-core/string-coerce";
import { REMOTECLAW_RUNTIME_CONTEXT_CUSTOM_TYPE } from "../agents/internal-runtime-context.js";
...
function buildMessageToolVisibleReplyMirror(
  ...
  const transcriptMeta = readRecord((pending.completionAnchor ?? pending.anchor)["__remoteclaw"]);
  if (transcriptMeta) {
    mirror["__remoteclaw"] = { ...transcriptMeta };
  }
  return mirror;
}
...
function readTtsSupplementMarker(
  ...
  const marker = message.remoteclawTtsSupplement;
...
function isDisplayHiddenProjectedMessage(message: Record<string, unknown>): boolean {
  ...
  return message.role === "custom" && message.customType === REMOTECLAW_RUNTIME_CONTEXT_CUSTOM_TYPE;
}
...
function remoteclawAssistantModel(message: Record<string, unknown>): string | undefined {
  return message.role === "assistant" &&
    message.provider === "remoteclaw" &&
...
function isDuplicateAcpGatewayInjectedMessage(
  ...
  if (
    remoteclawAssistantModel(previousVisible) !== "acp-runtime" ||
    remoteclawAssistantModel(current) !== "gateway-injected"
  ) {
    return false;
  }
