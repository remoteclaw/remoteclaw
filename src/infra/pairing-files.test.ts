...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/remoteclaw-state", "devices")).toEqual({
      dir: path.join("/tmp/remoteclaw-state", "devices"),
      pendingPath: path.join("/tmp/remoteclaw-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/remoteclaw-state", "devices", "paired.json"),
    });
  });
