...
Both show up under `remoteclaw plugins`, but only native OpenClaw plugins execute
...
remoteclaw plugins install <path>                 # copy a local file/dir into ~/.openclaw/extensions/<id>
...
remoteclaw plugins install @openclaw/voice-call # install from npm
remoteclaw plugins install @openclaw/voice-call --pin # store exact resolved name@version
...
`remoteclaw plugins list` shows the top-level format as `openclaw` or `bundle`.
...
- `remoteclaw plugins install <npm-spec>` uses `npm pack`, extracts into `~/.openclaw/extensions/<id>/`, and enables it in config.
