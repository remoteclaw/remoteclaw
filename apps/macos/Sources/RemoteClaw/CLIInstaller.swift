...
enum CLIInstaller {
    ...
    enum Status: Equatable {
        ...
        var message: String {
            ...
            case let .ready(_, version):
                "RemoteClaw Gateway \(version) is ready."
            case .missing:
                "RemoteClaw Gateway is not installed yet."
            case .unusable:
                "The RemoteClaw Gateway could not be verified. Setup will repair it."
        ...
        fileManager: FileManager) -> [String]
    {
        ...
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("remoteclaw").path
    ...
    static func managedExecutableLocation() -> String {
        URL(fileURLWithPath: self.installPrefix())
            .appendingPathComponent("bin/remoteclaw")
            .path
    }
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async -> Bool {
        ...
        await statusHandler("Installing remoteclaw CLI…")
        guard let installerURL = Bundle.main.url(forResource: "install-cli", withExtension: "sh") else {
            await statusHandler("Install failed: installer resource is missing. Reinstall RemoteClaw.")
            return false
        }
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed remoteclaw \($0)." } ?? "Installed remoteclaw."
    ...
    private static func installPrefix() -> String {
        FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent(".remoteclaw")
            .path
    }
