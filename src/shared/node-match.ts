...
export type NodeMatchCandidate = {
  ...
  connected?: boolean;
  /** Client id used to prefer current RemoteClaw nodes over legacy migration ties. */
  clientId?: string;
};
...
function pickPreferredLegacyMigrationMatch(
  ...
  // During Clawdbot -> RemoteClaw migration, a unique current client should win only
