    ...
    var body: some View {
        VStack(alignment: .leading, spacing: self.style == .sheet ? 16 : 12) {
            HStack(alignment: .top, spacing: 12) {
                Image(systemName: self.iconSystemName)
                    ...
                    .foregroundStyle(self.requestIsPending ? RemoteClawBrand.warn : RemoteClawBrand.accent)
                    .frame(width: 28, height: 28)

                VStack(alignment: .leading, spacing: 6) {
                    Text(self.titleText)
                        .font(self.style == .sheet ? RemoteClawType.title3SemiBold : RemoteClawType.headline)
        ...
        .background {
            if self.style == .card || self.style == .sheet {
                RoundedRectangle(cornerRadius: RemoteClawRadius.md, style: .continuous)
                    .fill(.thinMaterial)
            }
        }
        .overlay {
            if self.style == .card || self.style == .sheet {
                RoundedRectangle(cornerRadius: RemoteClawRadius.md, style: .continuous)
                    .stroke(RemoteClawBrand.accent.opacity(0.20), lineWidth: 1)
            }
        }
