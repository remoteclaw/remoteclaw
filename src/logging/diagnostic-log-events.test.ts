...
// NOTE (RemoteClaw fork): upstream's logger→diagnostics transport
// (buildDiagnosticLogRecord + attachDiagnosticEventTransport in src/logging/logger.ts,
// with its redaction/bounding/attribute-filtering machinery) is NOT adopted in this
// fork — the diagnostic-events.ts sink was kept but the logger emission side was never
// ported. Skipped pending a dedicated port of the logger diagnostic transport
// (keep-layer follow-up FU-13).
describe.skip("diagnostic log events", () => {
