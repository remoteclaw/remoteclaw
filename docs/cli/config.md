...
```bash
ssh remoteclaw-host 'remoteclaw config patch --stdin --dry-run' < ./remoteclaw.patch.json5
ssh remoteclaw-host 'remoteclaw config patch --stdin' < ./remoteclaw.patch.json5
```
...
After `remoteclaw config validate` is passing, you can use the local TUI to have an embedded agent compare the active config against the docs while you validate each change from the same terminal:
