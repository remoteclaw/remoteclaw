...
describe("handleReset", () => {
  it("uses active profile paths for destructive reset targets", async () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-reset-profile-"));
    const profileStateDir = path.join(homeDir, ".remoteclaw-work");
    const defaultStateDir = path.join(homeDir, ".remoteclaw");
    const profileConfigPath = path.join(profileStateDir, "remoteclaw.json");
...
describe("moveToTrash", () => {
  it("uses fs-safe trash instead of resolving a PATH trash command", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-trash-helper-"));
  ...
  it("canonicalizes a symlinked parent before calling fs-safe trash", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-trash-parent-link-"));
    const lexicalParent = path.join(testRoot, "state-link");
    const realParent = path.join(testRoot, "state-real");
    const targetPath = path.join(lexicalParent, "remoteclaw.json");
    const sourcePath = path.join(realParent, "remoteclaw.json");
