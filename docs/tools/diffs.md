...
Set plugin-wide defaults in `~/.remoteclaw/remoteclaw.json`:
...
- Artifacts are stored under the temp subfolder: `$TMPDIR/remoteclaw-diffs`.
...
1. `browser.executablePath` in RemoteClaw config.
