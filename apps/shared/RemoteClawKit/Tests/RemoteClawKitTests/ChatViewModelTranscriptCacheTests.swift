...
private func cachedMessage(role: String, text: String, timestamp: Double) -> RemoteClawChatMessage {
...
private func visibleTexts(_ vm: RemoteClawChatViewModel) async -> [String] {
    await MainActor.run { vm.messages.map { $0.content.compactMap(\.text).joined() } }
}

private func cachedSessionEntry(key: String, updatedAt: Double) -> RemoteClawChatSessionEntry {
...
private actor TestTranscriptCache: RemoteClawChatTranscriptCache {
    private var transcripts: [String: [RemoteClawChatMessage]]
    private var sessions: [RemoteClawChatSessionEntry]
    private let loadSessionsHook: (@Sendable () async -> Void)?
    private(set) var storedTranscriptSessionKeys: [String] = []
    private(set) var storedTranscripts: [[RemoteClawChatMessage]] = []
    private(set) var storedSessionsCallCount = 0

    init(
        transcripts: [String: [RemoteClawChatMessage]] = [:],
...
private final class GatedHistoryChatTransport: @unchecked Sendable, RemoteClawChatTransport {
    private let historyGate: AsyncStream<Void>.Continuation?
    private let historyGateStream: AsyncStream<Void>?
    private let historyResult: @Sendable (String, Int) throws -> RemoteClawChatHistoryPayload
    ...
    init(
        gated: Bool,
        historyResult: @escaping @Sendable (String, Int) throws -> RemoteClawChatHistoryPayload)
    {
        ...
        var eventCont: AsyncStream<RemoteClawChatTransportEvent>.Continuation!
    ...
    {
        RemoteClawChatSendResponse(runId: idempotencyKey, status: "accepted")
    }
...
struct ChatViewModelTranscriptCacheTests {
    @Test func `cold open paints cached transcript then live history replaces it`() async throws {
        ...
        let vm = await MainActor.run {
            RemoteClawChatViewModel(sessionKey: "main", transport: transport, transcriptCache: cache)
        }
