...
const shouldRunAutoDoctor = (deps, autoDoctorAttempted) =>
  ...
  !AUTO_DOCTOR_DISABLE_VALUES.has(
    String(deps.env.REMOTECLAW_GATEWAY_WATCH_AUTO_DOCTOR ?? "").toLowerCase(),
  );
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.REMOTECLAW_WATCH_MODE = "1";
  childEnv.REMOTECLAW_WATCH_SESSION = watchSession;
  ...
  return await new Promise((resolve, reject) => {
    ...
    const runAutoDoctorAndRestart = () => {
      ...
      logWatcher(
        "Gateway exited early; running `remoteclaw doctor --fix --non-interactive` once.",
        deps,
      );
