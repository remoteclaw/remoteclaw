...
describe("gateway cli backend live helpers", () => {
  afterEach(() => {
    ...
    delete process.env.REMOTECLAW_SKIP_CANVAS_HOST;
    delete process.env.REMOTECLAW_SKIP_BROWSER_CONTROL_SERVER;
    delete process.env.REMOTECLAW_BUNDLED_PLUGINS_DIR;
    delete process.env.REMOTECLAW_TEST_MINIMAL_GATEWAY;
  ...
  it("applies and restores live env including minimal gateway mode", async () => {
    ...
    process.env.REMOTECLAW_SKIP_CHANNELS = "old-channels";
    process.env.REMOTECLAW_SKIP_PROVIDERS = "old-providers";
    process.env.REMOTECLAW_SKIP_GMAIL_WATCHER = "old-gmail";
    process.env.REMOTECLAW_SKIP_CRON = "old-cron";
    process.env.REMOTECLAW_SKIP_CANVAS_HOST = "old-canvas";
    process.env.REMOTECLAW_SKIP_BROWSER_CONTROL_SERVER = "old-browser";
    process.env.REMOTECLAW_BUNDLED_PLUGINS_DIR = "old-bundled";
    process.env.REMOTECLAW_TEST_MINIMAL_GATEWAY = "old-minimal";
    ...
    expect(process.env.REMOTECLAW_SKIP_CHANNELS).toBe("1");
    expect(process.env.REMOTECLAW_SKIP_PROVIDERS).toBe("1");
    expect(process.env.REMOTECLAW_SKIP_GMAIL_WATCHER).toBe("1");
    expect(process.env.REMOTECLAW_SKIP_CRON).toBe("1");
    expect(process.env.REMOTECLAW_SKIP_CANVAS_HOST).toBe("1");
    expect(process.env.REMOTECLAW_SKIP_BROWSER_CONTROL_SERVER).toBe("1");
    expect(process.env.REMOTECLAW_BUNDLED_PLUGINS_DIR).toBe("old-bundled");
    expect(process.env.REMOTECLAW_TEST_MINIMAL_GATEWAY).toBe("1");
    expect(process.env.ANTHROPIC_API_KEY).toBeUndefined();
    expect(process.env.ANTHROPIC_API_KEY_OLD).toBeUndefined();

    restoreCliBackendLiveEnv(snapshot);

    expect(process.env.REMOTECLAW_SKIP_CHANNELS).toBe("old-channels");
    expect(process.env.REMOTECLAW_SKIP_PROVIDERS).toBe("old-providers");
    expect(process.env.REMOTECLAW_SKIP_GMAIL_WATCHER).toBe("old-gmail");
    expect(process.env.REMOTECLAW_SKIP_CRON).toBe("old-cron");
    expect(process.env.REMOTECLAW_SKIP_CANVAS_HOST).toBe("old-canvas");
    expect(process.env.REMOTECLAW_SKIP_BROWSER_CONTROL_SERVER).toBe("old-browser");
    expect(process.env.REMOTECLAW_BUNDLED_PLUGINS_DIR).toBe("old-bundled");
    expect(process.env.REMOTECLAW_TEST_MINIMAL_GATEWAY).toBe("old-minimal");
  ...
  it("defaults the model switch probe to Claude Sonnet -> Opus", async () => {
    ...
    delete process.env.REMOTECLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE;
  ...
  it("lets env disable the model switch probe", async () => {
    ...
    process.env.REMOTECLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE = "0";
  ...
  it("allows live env overrides for fresh and resume CLI args", async () => {
    ...
    process.env.REMOTECLAW_LIVE_CLI_BACKEND_ARGS = JSON.stringify([
      "exec",
      "--sandbox",
      "danger-full-access",
    ]);
    process.env.REMOTECLAW_LIVE_CLI_BACKEND_RESUME_ARGS = JSON.stringify([
      "exec",
      "resume",
      "{sessionId}",
      "-c",
