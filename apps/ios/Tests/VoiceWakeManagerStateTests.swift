...
@Suite(.serialized) struct VoiceWakeManagerStateTests {
    ...
    @Test @MainActor func `handle recognition callback dispatches command`() async throws {
        ...
        manager.triggerWords = ["remoteclaw"]
        manager.isEnabled = true
        ...
        let transcript = "remoteclaw hello"
        let triggerRange = try #require(transcript.range(of: "remoteclaw"))
        let helloRange = try #require(transcript.range(of: "hello"))
        let segments = [
            WakeWordSegment(text: "remoteclaw", start: 0.0, duration: 0.2, range: triggerRange),
