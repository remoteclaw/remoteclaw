...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in RemoteClaw. See [Webhooks](/automation/cron-jobs#webhooks) or use `remoteclaw webhooks` for Gmail helper commands.
...
Hook packs are standard npm packages that export one or more hooks via `remoteclaw.hooks` in
...
Hook packs can ship dependencies; they will be installed under `~/.remoteclaw/hooks/<id>`.
...
```typescript
{
  ...
  cfg: RemoteClawConfig            // Current gateway config
}
```
...
Runs after the built-in install security scan and before installation continues. RemoteClaw fires this hook for interactive skill installs as well as plugin bundle, package, and single-file installs.

Default behavior differs by target type:

- Plugin installs fail closed on built-in scan `critical` findings and scan errors unless the operator explicitly uses `remoteclaw plugins install --dangerously-force-unsafe-install`.
...
- **`origin`**: Normalized install origin when available (for example `remoteclaw-bundled`, `remoteclaw-workspace`, `plugin-bundle`, `plugin-package`, or `plugin-file`)
...
```json
{
  ...
  "sourcePath": "/var/folders/.../remoteclaw-plugin-acme-audit/package",
  "sourcePathKind": "directory",
  "origin": "plugin-package",
  "request": {
    "kind": "plugin-npm",
    "mode": "install",
    "requestedSpecifier": "@acme/remoteclaw-plugin-audit@1.4.2"
  },
  "builtinScan": {
    "status": "ok",
    "scannedFiles": 12,
  ...
  "plugin": {
    "pluginId": "acme-audit",
    "contentType": "package",
    "packageName": "@acme/remoteclaw-plugin-audit",
