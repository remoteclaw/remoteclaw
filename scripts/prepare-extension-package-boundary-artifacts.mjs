...
const ROOT_SHIMS_MAX_OLD_SPACE_SIZE =
  process.env.REMOTECLAW_ROOT_SHIMS_MAX_OLD_SPACE_SIZE?.trim() || "8192";
...
export function resolveBoundaryEntryShimRequiredOutputs(env = process.env) {
  const entries =
    env.REMOTECLAW_BUILD_PRIVATE_QA === "1" ? pluginSdkEntrypoints : publicPluginSdkEntrypoints;
  return [
...
export function resolveBoundaryRootShimsTimeoutMs(env = process.env) {
  const raw = env.REMOTECLAW_PLUGIN_SDK_BOUNDARY_ROOT_SHIMS_TIMEOUT_MS?.trim();
  if (!raw) {
    return 300_000;
  }
  return parsePositiveInt(raw, "REMOTECLAW_PLUGIN_SDK_BOUNDARY_ROOT_SHIMS_TIMEOUT_MS");
}
