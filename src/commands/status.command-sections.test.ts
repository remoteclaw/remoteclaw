...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:remoteclaw security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:remoteclaw security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:remoteclaw status --all",
      "  Need to debug live? cmd:remoteclaw logs --follow",
      "  Fix reachability first: cmd:remoteclaw gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:remoteclaw devices approve req-123)",
      "muted(Fallback: cmd:remoteclaw devices approve --latest)",
      "muted(Inspect: cmd:remoteclaw devices list)",
    ]);
  });
