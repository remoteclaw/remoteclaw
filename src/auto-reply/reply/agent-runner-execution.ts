...
export async function runAgentTurnWithFallback(params: {
  ...
  // If the run completed but with a context overflow error that wasn't
  // recovered from (e.g. compaction reset already attempted), surface the
  // error to the user instead of silently returning an empty response.
  // See #26905: Slack DM sessions silently swallowed messages when context
  // overflow errors were returned as embedded error payloads.
  const finalRunError = runResult?.error;
  const hasPayloadText = runResult?.payloads?.some((p) => p.text?.trim());
  if (finalRunError && isContextOverflowError(finalRunError) && !hasPayloadText) {
    return {
      kind: "final",
      payload: {
