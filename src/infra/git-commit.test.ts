...
async function makeFakeRemoteClawPackage(root: string) {
...
describe("git commit resolution", () => {
  ...
  it("does not walk out of the remoteclaw package into a host repo", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "remoteclaw", version: "2026.3.10" }),
      "utf-8",
    );
