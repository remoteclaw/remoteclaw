---
summary: "Delegate architecture: running RemoteClaw as a named agent on behalf of an organization"
...
Goal: run RemoteClaw as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an RemoteClaw agent that:
...
RemoteClaw's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of RemoteClaw's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.remoteclaw/workspace-delegate",
  tools: {
...
- Cron run history: `~/.remoteclaw/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.remoteclaw/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through RemoteClaw's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
remoteclaw agents add delegate
```

This creates:

- Workspace: `~/.remoteclaw/workspace-delegate`
- State: `~/.remoteclaw/agents/delegate/agent`
- Sessions: `~/.remoteclaw/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.remoteclaw/workspace" },
      {
        id: "delegate",
        workspace: "~/.remoteclaw/workspace-delegate",
        tools: {
...
~/.remoteclaw/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.remoteclaw/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.remoteclaw/workspace-org",
        agentDir: "~/.remoteclaw/agents/org-assistant/agent",
