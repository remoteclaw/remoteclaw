...
- legacy channel route key and comparable-target helper aliases while
  plugins move to `remoteclaw/plugin-sdk/channel-route`
...
- channel `showConfigured` / `showInSetup` metadata while channel packages
  move to `remoteclaw.channel.exposure`
...
- persisted plugin registry disable and install-migration env flags while
  repair flows migrate operators to `remoteclaw plugins registry --refresh`
...
The plugin inspector should live outside the core RemoteClaw repo as a
...
machine-readable output in CI annotations. RemoteClaw core should expose
contracts and fixtures the inspector can consume, but should not publish the
inspector binary from the main `remoteclaw` package.

### Maintainer acceptance lane

Use Crabbox-backed Blacksmith Testbox for the installable-package acceptance
lane when validating the external inspector against RemoteClaw plugin
packages. Run it from a clean RemoteClaw checkout after the package is built:
