...
function createHandoffEnv(): NodeJS.ProcessEnv {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-restart-handoff-"));
  tempDirs.push(dir);
  return {
    ...process.env,
...
function handoffPath(env: NodeJS.ProcessEnv): string {
  return path.join(env.REMOTECLAW_STATE_DIR ?? "", GATEWAY_SUPERVISOR_RESTART_HANDOFF_FILENAME);
}
...
describe("gateway restart handoff", () => {
  ...
  it("does not follow an existing handoff-path symlink when writing", () => {
    const env = createHandoffEnv();
    const targetPath = path.join(env.REMOTECLAW_STATE_DIR ?? "", "attacker-target.txt");
