...
If you turn off only the plugin, the bundled browser CLI (`remoteclaw browser`),
...
- Default profile is `remoteclaw` (RemoteClaw-managed standalone browser). Use `defaultProfile: "user"` to opt into the signed-in user browser.
...
- If you set `nodeHost.browserProxy.allowProfiles`, RemoteClaw treats it as a least-privilege boundary: only allowlisted profiles can be targeted, and persistent profile create/delete routes are blocked on the proxy surface.
...
CDP connection URLs over HTTPS and WebSocket. RemoteClaw can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let RemoteClaw
...
- **HTTP(S) endpoints** — RemoteClaw calls `/json/version` to discover the
...
RemoteClaw can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when RemoteClaw attaches.
...
- `remoteclaw doctor` migrates old extension-based browser config and checks that
...
- RemoteClaw does not launch the browser for this driver; it attaches to an
  existing session only.
- RemoteClaw uses the official Chrome DevTools MCP `--autoConnect` flow here. If
  `userDataDir` is set, RemoteClaw passes it through to target that explicit
