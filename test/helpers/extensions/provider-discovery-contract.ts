...
function installDiscoveryHooks(state: DiscoveryState) {
  beforeEach(async () => {
    vi.resetModules();
    vi.doMock("remoteclaw/plugin-sdk/agent-runtime", async () => {
    ...
    vi.doMock("remoteclaw/plugin-sdk/provider-auth", async () => {
      const actual = await vi.importActual<object>("remoteclaw/plugin-sdk/provider-auth");
      return {
        ...actual,
        ensureAuthProfileStore: ensureAuthProfileStoreMock,
        listProfilesForProvider: listProfilesForProviderMock,
    ...
    vi.doMock("remoteclaw/plugin-sdk/provider-setup", async () => {
