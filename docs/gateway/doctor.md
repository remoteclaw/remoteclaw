...
Cron job store migrations are handled by `remoteclaw doctor --fix`.
...
migrated via `remoteclaw doctor`. Talk provider/provider-map normalization now
...
- **Multiple state dirs**: warns when multiple `~/.remoteclaw` folders exist across
  home directories or when `REMOTECLAW_STATE_DIR` points elsewhere (history can
...
`remoteclaw doctor --fix` / `remoteclaw doctor --repair` mode. External plugins still
use `remoteclaw plugins install` / `remoteclaw plugins update`; doctor does not
...
startup continues. In read-only mode (`remoteclaw doctor` without `--fix`) this check
is skipped entirely.
...
- **Legacy workspace dirs**: warns when `~/remoteclaw` or other legacy workspace directories
...
- If the shell profile uses a slow dynamic completion pattern
  (`source <(remoteclaw completion ...)`), doctor upgrades it to the faster
  cached file variant.
...
- `remoteclaw doctor --repair` applies recommended fixes without prompts.
