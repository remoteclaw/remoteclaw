...
let loadRemoteClawPluginsMock: ReturnType<typeof vi.fn>;
...
function createWebFetchEnv(overrides?: Partial<NodeJS.ProcessEnv>) {
  return {
    REMOTECLAW_HOME: "/tmp/remoteclaw-home",
    ...overrides,
    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/firecrawl/remoteclaw.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
      ...
      },
      {
        ...
        manifestPath: "/tmp/noise/remoteclaw.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
...
describe("resolvePluginWebFetchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadRemoteClawPluginsMock = vi
      .spyOn(loaderModule, "loadRemoteClawPlugins")
      .mockImplementation(() => {
  ...
  it("does not force a fresh snapshot load when the same web-provider load is already in flight", () => {
    ...
    loadRemoteClawPluginsMock.mockImplementation(() => {
  ...
  it("invalidates web-fetch snapshot memoization when the active registry workspace changes", () => {
    ...
    expect(loadRemoteClawPluginsMock).toHaveBeenCalledTimes(2);
  });
});
