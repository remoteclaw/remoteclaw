...
function expectUnknownChannelRemovalPrompt(unsafeChannel: string, label: string) {
  ...
  expect(confirm).toHaveBeenCalledWith(
    expect.objectContaining({
      message: `Delete ${label} configuration from ~/.remoteclaw/remoteclaw.json?`,
    }),
  );
...
describe("removeChannelConfigWizard", () => {
  ...
  it("lists configured channels from remoteclaw.json even when no plugins are loaded", async () => {
  ...
  it("deletes the selected channel block from remoteclaw.json", async () => {
    ...
    expect(confirm).toHaveBeenCalledWith(
      expect.objectContaining({
        message: "Delete Telegram configuration from ~/.remoteclaw/remoteclaw.json?",
      }),
    );
  ...
  it("deletes a real channel block named done", async () => {
    ...
    expect(confirm).toHaveBeenCalledWith(
      expect.objectContaining({
        message: "Delete done configuration from ~/.remoteclaw/remoteclaw.json?",
      }),
    );
  ...
  it("sanitizes known channel labels before rendering prompts", async () => {
    ...
    expect(confirm).toHaveBeenCalledWith(
      expect.objectContaining({
        message: "Delete Telegram\\nBot configuration from ~/.remoteclaw/remoteclaw.json?",
      }),
    );
    expect(note).toHaveBeenCalledWith(
