...
import { warnLegacyRemoteClawEnvVars } from "./env-deprecation.js";
...
export async function resolveGatewayRuntimeConfig(params: {
  ...
  if (!isLoopbackHost(bindHost) && !hasSharedSecret && authMode !== "trusted-proxy") {
    throw new Error(
      `refusing to bind gateway to ${bindHost}:${params.port} without auth (set gateway.auth.token/password, or set REMOTECLAW_GATEWAY_TOKEN/REMOTECLAW_GATEWAY_PASSWORD; legacy CLAWDBOT_* and MOLTBOT_* environment variables are ignored)`,
    );
  }
  if (
    controlUiEnabled &&
