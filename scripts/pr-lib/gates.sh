...
prepare_gates() {
  ...
  if [ "$reuse_gates" = "true" ]; then
    ...
    if [ "$docs_only" = "true" ]; then
      ...
      prepare_unit_fast_batch_target_ms="${REMOTECLAW_PREPARE_TEST_UNIT_FAST_BATCH_TARGET_MS:-5000}"
      echo "Running pnpm test with REMOTECLAW_TEST_UNIT_FAST_BATCH_TARGET_MS=$prepare_unit_fast_batch_target_ms for shorter-lived unit-fast workers."
      run_quiet_logged \
        "pnpm test" \
        ".local/gates-test.log" \
        env REMOTECLAW_TEST_UNIT_FAST_BATCH_TARGET_MS="$prepare_unit_fast_batch_target_ms" pnpm test
