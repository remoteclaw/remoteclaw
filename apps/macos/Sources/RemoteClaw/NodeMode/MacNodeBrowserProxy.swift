...
actor MacNodeBrowserProxy {
    ...
    private struct RequestParams: Decodable {
        let method: String?
        let path: String?
        let query: [String: RemoteClawProtocol.AnyCodable]?
        let body: RemoteClawProtocol.AnyCodable?
        let timeoutMs: Int?
        let profile: String?
    }
    ...
    private static func unavailableError(endpoint: Endpoint, cause: Error) -> NSError {
        ...
        In remote mode, browser control is owned by the CLI node-host; start `remoteclaw node start` \
