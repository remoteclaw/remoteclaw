...
beforeEach(async () => {
  ...
  vi.unstubAllGlobals();
  await loadRemoteClawToolsForTest();
});
...
describe("nodes camera_snap", () => {
  ...
  it("omits inline base64 image blocks when model has no vision", async () => {
    ...
    expect(expectFirstMediaUrl(result)).toMatch(/remoteclaw-camera-snap-front-.*\.jpg$/);
  });
...
describe("nodes photos_latest", () => {
  ...
  it("returns MEDIA paths and no inline images when model has no vision", async () => {
    ...
    expect(expectFirstMediaUrl(result)).toMatch(/remoteclaw-camera-snap-.*\.jpg$/);
  });
...
async function loadRemoteClawToolsForTest(): Promise<void> {
  vi.resetModules();
