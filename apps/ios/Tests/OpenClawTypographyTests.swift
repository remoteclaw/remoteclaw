...
struct RemoteClawTypographyTests {
    @Test func `session controls use branded typography`() throws {
        ...
        #expect(support.contains(".font(RemoteClawType.subhead)"))
        #expect(support.contains(".font(RemoteClawType.subheadSemiBold)"))
        #expect(commandCenter.contains("Toggle(isOn: self.$showArchived)"))
        #expect(commandCenter.contains("Text(\"Show Archived\")"))
        #expect(commandCenter.contains(".font(RemoteClawType.captionMedium)"))
    }

    @Test func `bundled fonts load from app bundle`() {
        for name in RemoteClawType.registeredPostScriptNames {
    ...
    @Test func `app extensions register bundled branded fonts`() throws {
        ...
        for targetName in ["RemoteClawActivityWidget", "RemoteClawWatchApp"] {
            let target = try Self.extract(
                project,
                from: "  \(targetName):",
                to: targetName == "RemoteClawActivityWidget" ? "  RemoteClawWatchApp:" : "  RemoteClawTests:")
    ...
    @Test func `extension text surfaces use branded typography helpers`() throws {
        ...
        #expect(activitySource.contains("RemoteClawActivityType.subheadSemiBold"))
        #expect(activitySource.contains("RemoteClawActivityType.subheadBold"))
        #expect(activitySource.contains("RemoteClawActivityType.caption"))
    ...
    @Test func `UIKit typography refreshes when Dynamic Type changes`() throws {
        ...
        let typographySource = try String(
            contentsOf: Self.sourceURL("Design/RemoteClawTypography.swift"),
            encoding: .utf8)

        #expect(appSource.contains("UIContentSizeCategory.didChangeNotification"))
        #expect(appSource.contains("RemoteClawType.refreshUIKitAppearance(in: Self.connectedWindows())"))
    ...
    @Test func `listed iOS app surfaces enforce branded control typography`() throws {
        let proComponents = try String(
            contentsOf: Self.sourceURL("Design/RemoteClawProComponents.swift"),
            encoding: .utf8)
        ...
        let channels = try String(
            ...
            encoding: .utf8)
        let docs = try String(contentsOf: Self.sourceURL("Design/RemoteClawDocsScreen.swift"), encoding: .utf8)
        ...
        let chatTypography = try String(
            contentsOf: Self.iosRootURL()
                ...
                .appendingPathComponent("shared/RemoteClawKit/Sources/RemoteClawChatUI/ChatTypography.swift"),
            encoding: .utf8)
        let chatMessageViews = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/RemoteClawKit/Sources/RemoteClawChatUI/ChatMessageViews.swift"),
            encoding: .utf8)

        #expect(proComponents.contains(".font(RemoteClawType.subheadSemiBold)"))
        ...
        #expect(chatTab.contains(".font(RemoteClawType.body)"))
        ...
        #expect(quickSetup.contains(".font(RemoteClawType.subheadSemiBold)"))
        ...
        #expect(gatewayProblem.contains(".font(RemoteClawType.subheadSemiBold)"))
        ...
        #expect(onboardingSteps.contains(".font(RemoteClawType.subheadSemiBold)"))
        ...
        #expect(onboardingWizard.contains(".font(RemoteClawType.subheadSemiBold)"))
        ...
        let onboardingSecureOption = try Self.extract(
            ...
            to: ".tag(true)")
        #expect(onboardingUnencryptedOption.contains(".font(RemoteClawType.captionSemiBold)"))
        #expect(onboardingSecureOption.contains(".font(RemoteClawType.captionSemiBold)"))

        #expect(settingsSections.contains(".font(RemoteClawType.body)"))
        #expect(settingsSections.contains("self.settingsToggle(\"Show Talk Control\", isOn: self.$talkButtonEnabled)"))
        #expect(settingsSections.contains("RemoteClawToggleIndicator(isOn: isOn.wrappedValue)"))
        #expect(settingsSections.contains("TextField(\"Default Share Instruction\""))
        #expect(settingsSections.contains(".font(RemoteClawType.subhead)"))
        ...
        #expect(settingsSections.contains(".font(RemoteClawType.subheadSemiBold)"))
        ...
        let settingsSecureOption = try Self.extract(
            ...
            from: "Text(\"Secure (TLS)\")",
            to: ".tag(true)")
        #expect(settingsUnencryptedOption.contains(".font(RemoteClawType.captionSemiBold)"))
        #expect(settingsSecureOption.contains(".font(RemoteClawType.captionSemiBold)"))
        ...
        #expect(privacyAccess.contains(".font(RemoteClawType.footnoteSemiBold)"))
        ...
        for source in [agentDestinations, dreaming, instances, channels, docs] {
            #expect(source.contains(".font(RemoteClawType.body)"))
        }

        #expect(chatMessageViews.contains("font: RemoteClawChatTypography.body"))
        #expect(chatMessageViews.contains("RemoteClawChatTypography.callout.italic()"))
    ...
    private static func activityWidgetSourceURL() -> URL {
        self.iosRootURL().appendingPathComponent("ActivityWidget/RemoteClawLiveActivity.swift")
    }

    private static func activityWidgetTypographySourceURL() -> URL {
        self.iosRootURL().appendingPathComponent("ActivityWidget/RemoteClawActivityTypography.swift")
    }
    ...
    private static func appSourceURL() -> URL {
        self.sourceURL("RemoteClawApp.swift")
    }
    ...
    private static func swiftSourcesForTypographyAudit() throws -> [URL] {
        let roots = [
            ...
            self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/RemoteClawKit/Sources/RemoteClawChatUI"),
        ]
    ...
    private static func unbrandedTextCallOffenders() throws -> [String] {
        let fontTokens = ["RemoteClawType", "RemoteClawChatTypography"]
    ...
    private static func hasAllowedBrandedFontParameter(_ window: String, in url: URL) -> Bool {
        ...
        case "apps/ios/Sources/Design/RemoteClawProComponents.swift":
            window.contains(".font(self.titleFont)") || window.contains(".font(self.subtitleFont)")
        case "apps/shared/RemoteClawKit/Sources/RemoteClawChatUI/ChatMarkdownRenderer.swift":
