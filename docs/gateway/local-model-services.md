...
`models.providers.<id>.localService` lets RemoteClaw start a provider-owned local
model server on demand. It is provider-level config: when the selected model
belongs to that provider, RemoteClaw probes the service, starts the process if the
...
3. If the probe succeeds, RemoteClaw uses the existing server.
4. If the probe fails, RemoteClaw starts `command` with `args`.
5. RemoteClaw polls readiness until `readyTimeoutMs` expires.
6. The model request is sent through the normal provider transport.
7. If RemoteClaw started the process and `idleStopMs` is positive, the process is
   stopped after the last in-flight request has been idle for that long.

RemoteClaw does not install launchd, systemd, Docker, or a daemon for this. The
server is a child process of the RemoteClaw process that first needed it.
...
- `env`: optional environment variables merged over the RemoteClaw process
  environment.
- `healthUrl`: readiness URL. If omitted, RemoteClaw appends `/models` to
  `baseUrl`, so `http://127.0.0.1:8000/v1` becomes
  `http://127.0.0.1:8000/v1/models`.
- `readyTimeoutMs`: startup readiness deadline. Default: `120000`.
- `idleStopMs`: idle shutdown delay for RemoteClaw-started processes. `0` or
  omitted keeps the process alive until RemoteClaw exits.
...
- One RemoteClaw process manages the child it started. Another RemoteClaw process
