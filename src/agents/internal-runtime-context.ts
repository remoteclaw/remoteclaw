...
export const MODULE_ATTESTATIONS = {
  ...
  relocateCurrentRuntimeContextCarrierToTail: "live",
} as const;

export const INTERNAL_RUNTIME_CONTEXT_BEGIN = "<<<BEGIN_REMOTECLAW_INTERNAL_CONTEXT>>>";
export const INTERNAL_RUNTIME_CONTEXT_END = "<<<END_REMOTECLAW_INTERNAL_CONTEXT>>>";
...
export function relocateCurrentRuntimeContextCarrierToTail<T>(messages: T[]): T[] {
  ...
  if (lastIndex < 0 || !messages.some(isRemoteClawRuntimeContextCustomMessage)) {
    return messages;
  }
  ...
  while (
    ...
    isRemoteClawRuntimeContextCustomMessage(messages[firstNonCarrierFromEnd])
  ) {
    firstNonCarrierFromEnd -= 1;
  }
  const rest = messages.filter((message) => !isRemoteClawRuntimeContextCustomMessage(message));
  ...
  const carriers = messages.filter(isRemoteClawRuntimeContextCustomMessage);
