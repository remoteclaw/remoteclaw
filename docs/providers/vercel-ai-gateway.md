...
- RemoteClaw auto-discovers the Gateway `/v1/models` catalog, so `/models vercel-ai-gateway`
...
```bash
remoteclaw onboard --auth-choice ai-gateway-api-key
```
...
RemoteClaw accepts Vercel Claude shorthand model refs and normalizes them at
runtime:
