import Foundation
import Testing
@testable import RemoteClawMacCLI
...
struct ConfigureRemoteCommandTests {
    @Test @MainActor func `configure remote writes ssh config and app defaults`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("remoteclaw-configure-remote-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        let defaultSuites = ["org.remoteclaw.mac", "org.remoteclaw.mac.debug"]
        let keys = [
            "remoteclaw.connectionMode",
            "remoteclaw.remoteTarget",
            "remoteclaw.onboardingSeen",
            "remoteclaw.onboardingVersion",
            "remoteclaw.remoteCliPath",
        ]
        ...
        try await TestIsolation.withIsolatedState(env: ["REMOTECLAW_CONFIG_PATH": configURL.path]) {
            ...
            for (_, defaults) in defaultsBySuite {
                #expect(defaults.string(forKey: "remoteclaw.connectionMode") == "remote")
                #expect(defaults.string(forKey: "remoteclaw.remoteTarget") == "alice@gateway.example")
                #expect(defaults.bool(forKey: "remoteclaw.onboardingSeen") == true)
                #expect(defaults.string(forKey: "remoteclaw.remoteCliPath") == "/opt/homebrew/bin/remoteclaw")
            }
        }
    }
    ...
    @Test @MainActor func `configure remote preserves existing optional credentials when flags omitted`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("remoteclaw-configure-remote-preserve-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote can write direct private url`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("remoteclaw-configure-direct-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote rejects plaintext public prefix bypass`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("remoteclaw-configure-direct-reject-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        _ = await TestIsolation.withIsolatedState(env: ["REMOTECLAW_CONFIG_PATH": configURL.path]) {
            #expect(throws: Error.self) {
