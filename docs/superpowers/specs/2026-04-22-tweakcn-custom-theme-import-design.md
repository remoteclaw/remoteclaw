...
Add exactly one browser-local custom Control UI theme slot that can be imported from a tweakcn share link. The existing built-in theme families remain `claw`, `knot`, and `dash`. The new `custom` family behaves like a normal RemoteClaw theme family and supports `light`, `dark`, and `system` mode when the imported tweakcn payload includes both light and dark token sets.
...
Add a fourth theme family id, `custom`, to the Control UI theme model. The `custom` family becomes selectable only when a valid tweakcn import is present. The imported payload is normalized into an RemoteClaw-specific custom theme record and stored in browser local storage with the rest of the UI settings.

At runtime, RemoteClaw renders a managed `<style>` tag that defines the resolved custom CSS variable blocks:
...
- `light` and `dark` are already normalized RemoteClaw token maps, not raw tweakcn payloads.
...
- Create or update one stable `<style id="remoteclaw-custom-theme">` tag in `document.head`.
...
Do not mirror tweakcn variables blindly. Normalize a bounded subset into RemoteClaw tokens and derive the rest in a helper.
...
### Tokens derived for RemoteClaw

The importer derives RemoteClaw-only variables from the imported base colors:
...
Font stack strings are imported if present, but RemoteClaw does not load remote font assets in v1. If the imported stack references fonts that are unavailable in the browser, normal fallback behavior applies.
...
- map a valid tweakcn payload into normalized RemoteClaw light and dark token maps
