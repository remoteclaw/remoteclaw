...
if (
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.REMOTECLAW_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.REMOTECLAW_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.REMOTECLAW_NODE_OPTIONS_READY)) {
      return false;
    }
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `RemoteClaw ${VERSION} (${commit})` : `RemoteClaw ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
