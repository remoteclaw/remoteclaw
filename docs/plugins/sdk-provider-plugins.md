...
summary: "Step-by-step guide to building a model provider plugin for RemoteClaw"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to RemoteClaw
...
(LLM) to RemoteClaw. By the end you will have a provider with a model catalog,
API key auth, and dynamic model resolution.

<Info>
  If you have not built any RemoteClaw plugin before, read
...
<Steps>
  <Step title="Package and manifest">
    ...
    The manifest declares `providerAuthEnvVars` so RemoteClaw can detect
  ...
  <Step title="Register the provider">
    ...
    `remoteclaw onboard --acme-ai-api-key <key>` and select
    ...
    ```typescript
    import { defineSingleProviderPluginEntry } from "remoteclaw/plugin-sdk/provider-entry";
    ...
    `remoteclaw/plugin-sdk/provider-onboard`. The narrowest helpers are
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    <Accordion title="All available provider hooks">
      RemoteClaw calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    RemoteClaw classifies this as a **hybrid-capability** plugin. This is the
...
```
extensions/acme-ai/
├── package.json              # remoteclaw.providers metadata
├── remoteclaw.plugin.json      # Manifest with providerAuthEnvVars
