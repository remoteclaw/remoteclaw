...
summary: "How the RemoteClaw plugin SDK is organized, which subpaths are stable, and how to choose the right import"
read_when:
  - You are starting a new RemoteClaw plugin
...
The RemoteClaw plugin SDK is split into **small public subpaths** under
`remoteclaw/plugin-sdk/<subpath>`.
...
- Use focused imports such as `remoteclaw/plugin-sdk/plugin-entry`.
- Do not import the root `remoteclaw/plugin-sdk` barrel in new code.
- Do not import `remoteclaw/extension-api` in new code.
...
- Use `api.runtime.*` for trusted in-process helpers that RemoteClaw injects at
  registration time.
...
```ts
import { definePluginEntry, type RemoteClawPluginApi } from "remoteclaw/plugin-sdk/plugin-entry";
import { createProviderApiKeyAuthMethod } from "remoteclaw/plugin-sdk/provider-auth";
import { applyProviderConfigWithDefaultModel } from "remoteclaw/plugin-sdk/provider-onboard";
import { buildSecretInputSchema } from "remoteclaw/plugin-sdk/secret-input";
