...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: RemoteClawProtocol.AnyCodable(["ok": RemoteClawProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: RemoteClawProtocol.AnyCodable(["ok": RemoteClawProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = RemoteClawProtocol.AnyCodable([
            "runId": RemoteClawProtocol.AnyCodable("run-1"),
            "sessionKey": RemoteClawProtocol.AnyCodable("main"),
            "state": RemoteClawProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: RemoteClawProtocol.AnyCodable(["a": RemoteClawProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
