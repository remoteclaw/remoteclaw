...
remoteclaw sessions --store ./tmp/sessions.json
...
`remoteclaw sessions` and the Gateway `sessions.list` RPC are bounded by default
...
`remoteclaw sessions tail` renders recent trajectory JSONL events as compact
...
- Scope note: `remoteclaw sessions cleanup` maintains session stores,
...
Reclaim context budget for a wedged or oversized session. `remoteclaw sessions
...
<Note>
`remoteclaw agent --message '/compact ...'` is **not** a compaction path. Slash
