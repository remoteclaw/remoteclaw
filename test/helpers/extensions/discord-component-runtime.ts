...
async function createConversationRuntimeMock(
  importOriginal: () => Promise<typeof import("remoteclaw/plugin-sdk/conversation-runtime")>,
) {
...
vi.mock("remoteclaw/plugin-sdk/security-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("remoteclaw/plugin-sdk/security-runtime")>();
  return {
    ...actual,
...
export function resetDiscordComponentRuntimeMocks() {
  ...
  resolvePluginConversationBindingApprovalMock.mockReset().mockResolvedValue({
    ...
    binding: {
      bindingId: "binding-1",
      pluginId: "remoteclaw-codex-app-server",
      pluginName: "RemoteClaw App Server",
