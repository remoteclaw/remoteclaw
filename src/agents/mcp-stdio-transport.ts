/**
 * RemoteClaw stdio transport wrapper for MCP server subprocesses.
 */
...
export const MODULE_ATTESTATIONS = {
  RemoteClawStdioClientTransport: "live",
} as const;

export type RemoteClawStdioServerParameters = {
  command: string;
  args?: string[];
...
export class RemoteClawStdioClientTransport implements Transport {
  onclose?: () => void;
  ...
  async start(): Promise<void> {
    if (this.process) {
      throw new Error(
        "RemoteClawStdioClientTransport already started; Client.connect() starts transports automatically.",
      );
    }
