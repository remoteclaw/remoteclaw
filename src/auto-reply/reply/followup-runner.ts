...
const resolveRunModelFallbacksOverride = (..._args: unknown[]) => undefined as unknown;
const lookupContextTokens = (..._args: unknown[]) => undefined as number | undefined;
const DEFAULT_CONTEXT_TOKENS = 128_000;
const runWithModelFallback = async (..._args: unknown[]) => ({
  result: undefined,
  provider: "",
  model: "",
  attempts: [] as { provider: string; model: string; error: string; reason: string }[],
});
const runEmbeddedPiAgent = async (..._args: unknown[]) => undefined as unknown;
...
// Gutted in RemoteClaw fork (Middleware Boundary Principle)
const resolveRunAuthProfile = (..._args: unknown[]) => ({}) as Record<string, unknown>;
import {
...
export function createFollowupRunner(params: {
  ...
  return async (queued: FollowupRun) => {
    try {
      ...
      const followupAgentMeta = (runResult as Record<string, unknown>)?.meta as
        | Record<string, unknown>
        | undefined;
      const followupAgentMetaInner = (followupAgentMeta?.agentMeta ?? undefined) as
        | Record<string, unknown>
        | undefined;
      const usage = followupAgentMetaInner?.usage;
      const promptTokens = followupAgentMetaInner?.promptTokens;
      const modelUsed =
        (followupAgentMetaInner?.model as string | undefined) ?? fallbackModel ?? defaultModel;
      ...
      if (storePath && sessionKey) {
        await persistRunSessionUsage({
          storePath,
          sessionKey,
          usage: usage as import("../../agents/usage.js").NormalizedUsage | undefined,
          lastCallUsage: followupAgentMetaInner?.lastCallUsage as
            | import("../../agents/usage.js").NormalizedUsage
            | undefined,
          promptTokens: promptTokens as number | undefined,
          modelUsed,
      ...
      const payloadArray = ((runResult as Record<string, unknown>)?.payloads ?? []) as Array<{
        text?: string;
        mediaUrl?: string;
        mediaUrls?: string[];
      }>;
      ...
      const sanitizedPayloads = payloadArray.flatMap(
      ...
      const dedupedPayloads = filterMessagingToolDuplicates({
        ...
        sentTexts: (runResultRecord.messagingToolSentTexts as string[]) ?? [],
      });
      const mediaFilteredPayloads = filterMessagingToolMediaDuplicates({
        payloads: dedupedPayloads,
        sentMediaUrls: (runResultRecord.messagingToolSentMediaUrls as string[]) ?? [],
      });
      const suppressMessagingToolReplies = shouldSuppressMessagingToolReplies({
        ...
        messagingToolSentTargets: (runResultRecord.messagingToolSentTargets ?? []) as Record<
          string,
          unknown
        >[],
