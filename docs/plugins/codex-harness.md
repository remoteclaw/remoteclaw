---
summary: "Run RemoteClaw embedded agent turns through the bundled Codex app-server harness"
title: "Codex harness"
read_when:
  - You want to use the bundled Codex app-server harness
  - You need Codex harness config examples
  - You want Codex-only deployments to fail instead of falling back to RemoteClaw
---

The bundled `codex` plugin lets RemoteClaw run embedded OpenAI agent turns
through Codex app-server instead of the built-in RemoteClaw harness.
...
app-server execution. RemoteClaw still owns chat channels, session files, model
selection, RemoteClaw dynamic tools, approvals, media delivery, and the visible
transcript mirror.
...
When no RemoteClaw sandbox is active, RemoteClaw starts Codex app-server threads
...
RemoteClaw dynamic tools continue through the app-server `item/tool/call` bridge.
Active RemoteClaw sandboxing and restricted tool policies disable native code mode
entirely unless you opt into the experimental sandbox exec-server path.

This Codex-native feature is separate from
[RemoteClaw code mode](/reference/code-mode), which is an opt-in QuickJS-WASI
runtime for generic RemoteClaw runs with a different `exec` input shape.
...
- Codex auth available through `remoteclaw models auth login --provider openai`,
...
Most users who want Codex in RemoteClaw want this path: sign in with a
...
harness options in RemoteClaw config, and use the CLI only for Codex auth:

| Need                                   | Set                                                                              | Where                                |
| -------------------------------------- | -------------------------------------------------------------------------------- | ------------------------------------ |
| Enable the harness                     | `plugins.entries.codex.enabled: true`                                            | RemoteClaw config                    |
| Keep an allowlisted plugin install     | Include `codex` in `plugins.allow`                                               | RemoteClaw config                    |
| Route OpenAI agent turns through Codex | `agents.defaults.model` or `agents.list[].model` as `openai/gpt-*`               | RemoteClaw agent config              |
| Sign in with ChatGPT/Codex OAuth       | `remoteclaw models auth login --provider openai`                                 | CLI auth profile                     |
| Add API-key backup for Codex runs      | `openai:*` API-key profile listed after subscription auth in `auth.order.openai` | CLI auth profile + RemoteClaw config |
| Fail closed when Codex is unavailable  | Provider or model `agentRuntime.id: "codex"`                                     | RemoteClaw model/provider config     |
| Use direct OpenAI API traffic          | Provider or model `agentRuntime.id: "remoteclaw"` with normal OpenAI auth        | RemoteClaw model/provider config     |
...
Codex compacts through its native app-server thread state, so RemoteClaw ignores
those local summarizer overrides at runtime and `remoteclaw doctor --fix` removes
...
`agents.defaults.compaction.provider`. `remoteclaw doctor --fix` migrates the old
...
the bound thread. RemoteClaw does not wait for completion, impose an RemoteClaw
...
turns. The API key is only an auth fallback, not a request to switch to RemoteClaw or
...
- Do not use legacy Codex GPT refs in config. Run `remoteclaw doctor --fix` to
...
- `agentRuntime.id: "remoteclaw"` opts a provider or model into the RemoteClaw
...
| Direct OpenAI API-key traffic through RemoteClaw     | Provider or model `agentRuntime.id: "remoteclaw"` and normal OpenAI auth | `/status` shows RemoteClaw runtime      | Use only when RemoteClaw is intentional |
| Legacy config                                        | legacy Codex GPT refs                                                    | `remoteclaw doctor --fix` rewrites it   | Do not write new config this way        |
...
When an RemoteClaw sandbox is active for the session, RemoteClaw disables Codex
...
through RemoteClaw sandbox-backed dynamic tools such as `sandbox_exec` and
`sandbox_process` when the normal exec/process tools are available.

Use normalized RemoteClaw exec mode when you want Codex native auto-review before
...
For Codex app-server sessions, RemoteClaw maps `tools.exec.mode: "auto"` to Codex
...
In `tools.exec.mode: "auto"`, RemoteClaw does not preserve legacy unsafe Codex
...
works, but `tools.exec.mode: "auto"` is the normalized RemoteClaw surface.
...
supports RemoteClaw text commands.
...
- `/codex resume <thread-id>` attaches the current RemoteClaw session to an
...
1. Ordered OpenAI auth profiles for the agent, preferably under
   `auth.order.openai`. Run `remoteclaw doctor --fix` to migrate older
...
When RemoteClaw sees a ChatGPT subscription-style Codex auth profile, it removes
...
For local stdio app-server launches, RemoteClaw sets `CODEX_HOME` to a per-agent
...
default. RemoteClaw preserves the normal process `HOME`; Codex-run subprocesses
...
RemoteClaw removes `CODEX_HOME` and `HOME` from this list during local launch
normalization: `CODEX_HOME` stays per-agent, and `HOME` stays inherited so
subprocesses can use normal user-home state.

Codex dynamic tools default to `searchable` loading. RemoteClaw does not expose
...
RemoteClaw integration tools such as messaging, media, cron, browser, nodes,
gateway, `heartbeat_respond`, and `web_search` are available through Codex tool
search under the `remoteclaw` namespace, keeping the initial model context
smaller.
...
RemoteClaw or ACP delegation is still available through the `remoteclaw` dynamic
...
| -------------------------- | -------------- | ------------------------------------------------------------------------------------------ |
| `codexDynamicToolsLoading` | `"searchable"` | Use `"direct"` to put RemoteClaw dynamic tools directly in the initial Codex tool context. |
| `codexDynamicToolsExclude` | `[]`           | Additional RemoteClaw dynamic tool names to omit from Codex app-server turns.              |
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after RemoteClaw builds its inherited environment. RemoteClaw keeps per-agent `CODEX_HOME` and inherited `HOME` for local launches.                                                                                               |
| `codeModeOnly`                                | `false`                                                | Opt into Codex's code-mode-only tool surface. RemoteClaw dynamic tools remain registered with Codex so nested `tools.*` calls return through the app-server `item/tool/call` bridge.                                                                                                                                 |
| `requestTimeoutMs`                            | `60000`                                                | Timeout for app-server control-plane calls.                                                                                                                                                                                                                                                                          |
| `turnCompletionIdleTimeoutMs`                 | `60000`                                                | Quiet window after Codex accepts a turn or after a turn-scoped app-server request while RemoteClaw waits for `turn/completed`.                                                                                                                                                                                       |
| `postToolRawAssistantCompletionIdleTimeoutMs` | `300000`                                               | Completion-idle and progress guard used after a tool handoff, native tool completion, or post-tool raw assistant progress while RemoteClaw waits for `turn/completed`. Use this for trusted or heavy workloads where post-tool synthesis can legitimately stay quiet longer than the final assistant release budget. |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start/resume. Guardian defaults prefer `"workspace-write"` when allowed, otherwise `"read-only"`. When an RemoteClaw sandbox is active, `danger-full-access` turns use Codex `workspace-write` with network access derived from the RemoteClaw sandbox egress setting.      |
| `approvalsReviewer`                           | `"user"` or an allowed guardian reviewer               | Use `"auto_review"` to let Codex review native approval prompts when allowed, otherwise `guardian_subagent` or `user`. `guardian_subagent` remains a legacy alias.                                                                                                                                                   |
| `serviceTier`                                 | unset                                                  | Optional Codex app-server service tier. `"priority"` enables fast-mode routing, `"flex"` requests flex processing, `null` clears the override, and legacy `"fast"` is accepted as `"priority"`.                                                                                                                      |
| `experimental.sandboxExecServer`              | `false`                                                | Preview opt-in that registers an RemoteClaw sandbox-backed Codex environment with Codex app-server 0.132.0 or newer so native Codex execution can run inside the active RemoteClaw sandbox.                                                                                                                          |
...
After Codex accepts a turn, and after RemoteClaw responds to a turn-scoped
...
quiet for `appServer.turnCompletionIdleTimeoutMs`, RemoteClaw best-effort
...
longer post-tool idle budget: after RemoteClaw returns an `item/tool/call`
...
without `turn/completed`, RemoteClaw best-effort interrupts the native turn and
...
timeouts still retire the stuck app-server client and release the RemoteClaw
...
- `REMOTECLAW_CODEX_APP_SERVER_MODE=yolo|guardian`
...
- Codex owns native compaction. RemoteClaw keeps a transcript mirror for channel
  ...
  it does not replace Codex compaction with an RemoteClaw or context-engine
  summarizer.
...
**RemoteClaw uses the built-in harness instead of Codex:** make sure the model ref is
...
Ask affected collaborators to run this read-only command on their RemoteClaw host:

```bash
(
  pattern='openai/gpt-5\.[45]|openai[-]codex|agentRuntime(\.id)?|harnessRuntime|Runtime: OpenAI Codex|legacy OpenAI Codex prefix|resolveSelectedOpenAIRuntimeProvider|candidateProvider[": ]+openai|status[": ]+401|Incorrect API key|No API key|api-key path|API-key path|OAuth'

  if ls /tmp/remoteclaw/remoteclaw-*.log >/dev/null 2>&1; then
    grep -E -i -n "$pattern" /tmp/remoteclaw/remoteclaw-*.log 2>/dev/null || true
  else
    journalctl --user -u remoteclaw-gateway --since today --no-pager 2>/dev/null \
...
**Legacy Codex model refs config remains:** run `remoteclaw doctor --fix`.
...
the Codex thread is still trying to use a native hook relay id that RemoteClaw no
...
app-server or RemoteClaw Gateway so old threads are dropped and native hook
