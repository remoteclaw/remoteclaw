...
const TOOLING_EXCLUDED_TESTS = new Set([
  ...boundaryTestFiles,
  "test/scripts/remoteclaw-e2e-instance.test.ts",
]);
...
export function resolveVitestNodeArgs(env = process.env) {
  if (isTruthyEnvValue(env.REMOTECLAW_VITEST_ENABLE_MAGLEV)) {
    return [];
  }
...
export function resolveVitestCliEntry({
  ...
  try {
    ...
    if (isMissingVitestResolveError(error)) {
      ...
      wrappedError.code = "REMOTECLAW_MISSING_VITEST";
      throw wrappedError;
    }
    throw error;
  }
...
function shouldApplyNativeWorkerBudget(env) {
  ...
  return (
    env.REMOTECLAW_TEST_PROJECTS_SERIAL === "1" || resolveExplicitVitestWorkerBudget(env) !== null
  );
}
...
function resolveExplicitVitestWorkerBudget(env) {
  return parsePositiveInt(env.REMOTECLAW_VITEST_MAX_WORKERS ?? env.REMOTECLAW_TEST_WORKERS);
}
  ...
  try {
    return fsImpl.readdirSync(extensionsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fsImpl.existsSync(path.join(pluginDir, "remoteclaw.plugin.json"))
      );
    });
  } catch {
    return false;
...
export function resolveLinkedSourceBundledPluginsEnv(
  ...
  if (env.REMOTECLAW_BUNDLED_PLUGINS_DIR?.trim()) {
    return {};
  }
  ...
  return {
    REMOTECLAW_BUNDLED_PLUGINS_DIR: extensionsDir,
    REMOTECLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR:
      env.REMOTECLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR?.trim() || "1",
  };
}
...
function main(argv = process.argv.slice(2), env = process.env) {
  ...
  try {
    ...
    if (error instanceof Error && error.code === "REMOTECLAW_MISSING_VITEST") {
