...
read_when:
  - Adding or modifying `remoteclaw infer` commands
...
For end-to-end provider checks, prefer `remoteclaw infer ...` once lower-level
...
remoteclaw infer model run --prompt "Summarize this changelog entry" --model openai/gpt-5.4 --json
remoteclaw infer model run --prompt "Describe this image in one sentence" --file ./photo.jpg --model google/gemini-2.5-flash --json
remoteclaw infer model run --prompt "Use more reasoning here" --thinking high --json
remoteclaw infer model providers --json
remoteclaw infer model inspect --name gpt-5.5 --json
```
...
```bash
remoteclaw infer model run --local --model anthropic/claude-sonnet-4-6 --prompt "Reply with exactly: pong" --json
remoteclaw infer model run --local --model cerebras/zai-glm-4.7 --prompt "Reply with exactly: pong" --json
remoteclaw infer model run --local --model google/gemini-2.5-flash --prompt "Reply with exactly: pong" --json
...
- Use `model run --gateway` when you need to test Gateway routing, agent-runtime setup, or Gateway-managed provider state while keeping the model input raw. Use `remoteclaw agent` or chat surfaces when you want the full agent context, tools, memory, and session transcript.
...
remoteclaw infer image generate --model openai/gpt-image-2 --quality low --openai-moderation low --prompt "low-cost draft poster" --json
...
For generated media commands, `outputs` contains files written by RemoteClaw. Use
