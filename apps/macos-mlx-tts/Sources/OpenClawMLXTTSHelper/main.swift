...
enum RemoteClawMLXTTSHelper {
    static func main() async {
        do {
            ...
            FileHandle.standardError.write(Data("remoteclaw-mlx-tts: \(error)\n".utf8))
            exit(1)
        }
    }
    ...
    private enum Usage: Error, CustomStringConvertible {
        ...
        var description: String {
            switch self {
            case .requested:
                "usage: remoteclaw-mlx-tts --text <text> --output <wav> [--model <hf-repo>] [--language <id>] [--voice <name>]"
