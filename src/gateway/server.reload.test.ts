...
describe("gateway hot reload", () => {
  ...
  async function writeAuthProfileEnvRefStore() {
    const stateDir = process.env.REMOTECLAW_STATE_DIR;
    if (!stateDir) {
      throw new Error("REMOTECLAW_STATE_DIR is not set");
    }
      ...
      `${JSON.stringify(
        {
          ...
          profiles: {
            missing: {
              type: "api_key",
              provider: "openai",
              keyRef: { source: "env", provider: "default", id: "MISSING_REMOTECLAW_AUTH_REF" },
            },
          },
  ...
  it.skip("fails startup when required secret refs are unresolved", async () => {
  ...
  it.skip("fails startup when auth-profile secret refs are unresolved", async () => {
    await writeAuthProfileEnvRefStore();
    delete process.env.MISSING_REMOTECLAW_AUTH_REF;
    try {
      await expect(withGatewayServer(async () => {})).rejects.toThrow(
        'Environment variable "MISSING_REMOTECLAW_AUTH_REF" is missing or empty.',
      );
    } finally {
  ...
  it.skip("emits one-shot degraded and recovered system events during secret reload transitions", async () => {
    await writeEnvRefConfig();
  ...
  it.skip("serves secrets.reload immediately after startup without race failures", async () => {
    await writeEnvRefConfig();
    process.env.OPENAI_API_KEY = "sk-startup";
