...
function projectGatewayDispatchConfig(value: unknown): RemoteClawConfig {
  if (!isPlainRecord(value)) {
    return {};
  }
  ...
  return projected as RemoteClawConfig;
}

// Main session keys are process-local; Gateway dispatch always sees the canonical main key.
function applyGatewayDispatchSessionDefaults(config: RemoteClawConfig): RemoteClawConfig {
...
function resolveGatewayDispatchEnvVars(config: unknown, env: NodeJS.ProcessEnv): unknown {
  if (isPlainRecord(config) && Object.hasOwn(config, "env")) {
    applyConfigEnvVars(config as RemoteClawConfig, env);
  }
