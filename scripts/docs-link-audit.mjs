...
export function prepareMirroredDocsDir(sourceDir = DOCS_DIR, options = {}) {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-docs-link-audit-"));
  try {
...
export function prepareAnchorAuditDocsDir(sourceDir = DOCS_DIR) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-docs-anchor-audit-"));
  try {
