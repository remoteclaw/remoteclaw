...
- Explicit `remoteclaw infer image describe --file <path> --model <provider/model>` (alias: `remoteclaw capability image describe`) runs that image-capable provider/model directly, including Ollama refs such as `ollama/qwen2.5vl:7b` when a matching image-capable model is configured under `models.providers.ollama.models[]`.
- If `<capability>.enabled` is not `false` but no models are configured, RemoteClaw tries the active reply model when its provider supports the capability.

### Auto-detect (default)

When `tools.media.<capability>.enabled` is not `false` and no models are configured, RemoteClaw tries these in order and stops at the first working option:
...
<Steps>
  ...
  <Step title="Antigravity CLI (image/video only)">
    First installed `agy` or `antigravity` binary (override with `REMOTECLAW_ANTIGRAVITY_CLI`), sandboxed against the media's directory.
  </Step>
</Steps>
...
Set `capabilities` on a `models[]` entry to restrict it to specific media types. For shared lists, RemoteClaw infers defaults per bundled provider:
...
- `parakeet-mlx`: with `--output-dir`, RemoteClaw reads `<output-dir>/<media-basename>.txt` when the output format is `txt` or unspecified; other formats fall back to stdout.
...
- If a PDF falls back to rendered page images, RemoteClaw forwards those images to vision-capable reply models and keeps the placeholder `[PDF content rendered to images]` in the file block.
