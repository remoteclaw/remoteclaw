 ...
 * Mirrors the real `remoteclaw config <sub>` surface so the message only ever
...
const CONFIGURE_NON_TTY_HINT = [
  ...
  `  ${formatCliCommand("remoteclaw config set <path> <value>")}  write a config entry`,
  `  ${formatCliCommand("remoteclaw config get <path>")}          read a config entry`,
  `  ${formatCliCommand("remoteclaw config patch")}              apply a JSON patch`,
  `  ${formatCliCommand("remoteclaw config validate")}           validate configuration`,
].join("\n");
...
export async function configureCommandFromSectionsArg(
  ...
  // Fail closed once at the shared entry: both `remoteclaw configure` and the
  // no-subcommand `remoteclaw config` route here, so a single guard keeps them
  ...
  if (invalid.length > 0) {
    runtime.error(
      `Invalid --section: ${invalid.join(", ")}. Expected one of: ${CONFIGURE_WIZARD_SECTIONS.join(", ")}. Run ${formatCliCommand("remoteclaw configure")} without --section to use the full wizard.`,
    );
    runtime.exit(1);
    return;
  }
