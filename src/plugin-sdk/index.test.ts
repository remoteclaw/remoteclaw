...
describe("plugin-sdk exports", () => {
  ...
  it("emits importable bundled subpath entries", { timeout: 240_000 }, async () => {
    const outDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-plugin-sdk-build-"));
    const fixtureDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-plugin-sdk-consumer-"));
...
`,
      ...
      const packageDir = path.join(fixtureDir, "remoteclaw");
      ...
      await fs.symlink(packageDir, path.join(consumerDir, "node_modules", "remoteclaw"), "dir");
      await fs.writeFile(
        consumerEntry,
        [
