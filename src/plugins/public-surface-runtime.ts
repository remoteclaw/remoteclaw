...
// `areBundledPluginsDisabled` is no longer exported from ./bundled-dir; the fork
// keeps the env check as a file-local helper (matching manifest-metadata-scan.ts).
...
function resolveExplicitEnvBundledPluginsDir(env: NodeJS.ProcessEnv): string | undefined {
  const envOverride = env.REMOTECLAW_BUNDLED_PLUGINS_DIR?.trim();
  if (!envOverride) {
    return undefined;
  }
