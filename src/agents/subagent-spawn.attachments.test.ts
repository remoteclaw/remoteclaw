...
let previousConfigPath = process.env.REMOTECLAW_CONFIG_PATH;
...
describe("spawnSubagentDirect filename validation", () => {
  beforeEach(() => {
    ...
    workspaceDirOverride = fs.mkdtempSync(
      path.join(os.tmpdir(), `remoteclaw-subagent-attachments-${process.pid}-${Date.now()}-`),
    );
    configPathOverride = path.join(workspaceDirOverride, "remoteclaw.test.json");
    ...
    process.env.REMOTECLAW_CONFIG_PATH = configPathOverride;
  });
  ...
  it("removes materialized attachments when lineage patching fails", async () => {
    ...
    const attachmentsRoot = path.join(workspaceDirOverride, ".remoteclaw", "attachments");
