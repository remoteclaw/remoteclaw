...
describe("native Gateway protocol levels", () => {
  it("match the TypeScript source of truth", async () => {
    ...
    const androidPath =
      "apps/android/app/src/main/java/org/remoteclaw/app/gateway/GatewayProtocol.kt";
  ...
  it("uses the min constant for native connect compatibility ranges", async () => {
    const swiftConnectFiles = [
      "apps/shared/RemoteClawKit/Sources/RemoteClawKit/GatewayChannel.swift",
      "apps/macos/Sources/RemoteClawMacCLI/WizardCommand.swift",
    ];
    ...
    const androidPath =
      "apps/android/app/src/main/java/org/remoteclaw/app/gateway/GatewaySession.kt";
    const android = await readRepoFile(androidPath);
    assertPattern(
      android,
      androidPath,
