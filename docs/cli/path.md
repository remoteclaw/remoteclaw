...
RemoteClaw state is spread across human-edited markdown, commented JSONC
...
`remoteclaw path` gives those callers a stable address instead of a one-off
...
Skip `remoteclaw path` for ordinary whole-file edits, rich config migrations, or
