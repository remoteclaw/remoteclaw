...
describe("createConfigScopedPromiseLoader", () => {
  ...
  it("caches loads by config object", async () => {
    const firstConfig = { plugins: { load: { disabled: true } } } as RemoteClawConfig;
    const secondConfig = { plugins: { load: { disabled: false } } } as RemoteClawConfig;
    const load = vi.fn(async (config?: RemoteClawConfig) =>
  ...
  it("clears default and config-scoped entries", async () => {
    ...
    const loader = createConfigScopedPromiseLoader(
      async (owner?: RemoteClawConfig) => `${owner ? "config" : "default"}-${++calls}`,
    );
