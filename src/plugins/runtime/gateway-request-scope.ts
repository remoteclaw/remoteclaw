...
const PLUGIN_RUNTIME_GATEWAY_REQUEST_SCOPE_KEY: unique symbol = Symbol.for(
  "remoteclaw.pluginRuntimeGatewayRequestScope",
);

const pluginRuntimeGatewayRequestScope = resolveGlobalSingleton<AsyncLocalStorage<PluginRuntimeGatewayRequestScope>>(
 ...
 */
export function withPluginRuntimeGatewayRequestScope<T>(scope: PluginRuntimeGatewayRequestScope, run: () => T): T {
 ...
 */
export function getPluginRuntimeGatewayRequestScope(): PluginRuntimeGatewayRequestScope | undefined {
