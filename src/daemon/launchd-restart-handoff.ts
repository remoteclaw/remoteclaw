...
function resolveLaunchAgentLabel(env?: Record<string, string | undefined>): string {
  const envLabel = env?.REMOTECLAW_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return envLabel;
  }
  return resolveGatewayLaunchAgentLabel(env?.REMOTECLAW_PROFILE);
}
...
export function isCurrentProcessLaunchdServiceLabel(
  ...
  const configuredLabel = env.REMOTECLAW_LAUNCHD_LABEL?.trim();
