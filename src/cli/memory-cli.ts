...
async function scanMemoryFiles(
  ...
  let listedOk = false;
  try {
    listed = listMemoryFiles(workspaceDir, resolvedExtraPaths) as string[];
    listedOk = true;
  } catch (err) {
    const code = (err as NodeJS.ErrnoException).code;
    if (dirReadable !== null) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ...
          ["remoteclaw memory status --deep", "Probe embedding provider readiness."],
          ["remoteclaw memory index --force", "Force a full reindex."],
          ['remoteclaw memory search "meeting notes"', "Quick search using positional query."],
          [
            'remoteclaw memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["remoteclaw memory status --json", "Output machine-readable JSON (good for scripts)."],
