---
summary: "Run RemoteClaw on local LLMs (LM Studio, vLLM, LiteLLM, custom OpenAI endpoints)"
read_when:
...
| LiteLLM / OAI-proxy / custom OpenAI-compatible proxy | You front another model API and need RemoteClaw to treat it as OpenAI       |
...
If `api` is omitted on a custom provider with a `baseUrl`, RemoteClaw defaults to
...
- hidden RemoteClaw attribution headers (`originator`, `version`, `User-Agent`)
...
- Some local models emit standalone bracketed tool requests as text, such as
  `[tool_name]` followed by JSON and `[END_TOOL_REQUEST]`. RemoteClaw promotes
...
- If a model emits JSON, XML, or ReAct-style text that looks like a tool call
  but the provider did not emit a structured invocation, RemoteClaw leaves it as
...
- If tools appear as assistant text instead of running, for example raw JSON,
  ...
  It overrides RemoteClaw's default proxy value of `tool_choice: "auto"`.
...
5. **Past that, the bottleneck is upstream.** If the backend still fails only on larger RemoteClaw runs after lean mode and `supportsTools: false`, the remaining issue is usually upstream model or server capacity — context window, GPU memory, kv-cache eviction, or a backend bug. It is not RemoteClaw's transport layer at that point.
...
- Local server says `terminated`, `ECONNRESET`, or closes the stream mid-turn?
  RemoteClaw records a low-cardinality `model.call.error.failureKind` plus the
