...
function makeTempDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-local-heavy-check-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("local-heavy-check-runtime", () => {
  ...
  it("keeps explicit tsgo flags and Go env overrides intact", () => {
    const { args, env } = applyLocalTsgoPolicy(
      ...
      makeEnv({
        GOGC: "80",
        GOMEMLIMIT: "5GiB",
        REMOTECLAW_TSGO_PPROF_DIR: "/tmp/profile",
      }),
    );
  ...
  it("reclaims stale local heavy-check locks from dead pids", () => {
    ...
    const lockDir = path.join(commonDir, "remoteclaw-local-checks", "heavy-check.lock");
