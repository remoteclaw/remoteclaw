...
private func outboxTestCommand(id: String, text: String, createdAt: Double) -> RemoteClawChatOutboxCommand {
...
private func userTexts(_ vm: RemoteClawChatViewModel) async -> [String] {
    await MainActor.run {
        vm.messages
            .filter { $0.role == "user" }
...
private final class OutboxTestTransport: @unchecked Sendable, RemoteClawChatTransport {
    let state: OutboxTransportState
    private let sessions: [RemoteClawChatSessionEntry]
    ...
    init(
        ...
        sessions: [RemoteClawChatSessionEntry] = [],
    ...
    func listCommands(sessionKey _: String) async throws -> [RemoteClawChatCommandChoice] {
    ...
    private func requestHistory(
        sessionKey: String,
        agentID: String?,
        expectedRoute: Int?) async throws -> RemoteClawChatHistoryPayload
    {
        ...
        if let stale = await state.staleHistoryRows {
            ...
            return RemoteClawChatHistoryPayload(
        ...
        let durableUserRows = keys.indices.compactMap { index -> AnyCodable? in
            ...
            return AnyCodable([
                ...
                "__remoteclaw": ["idempotencyKey": "\(key):user"],
            ] as [String: Any])
        }
        return RemoteClawChatHistoryPayload(
            sessionKey: sessionKey,
    ...
    private func sendMessage(
        ...
        thinking: String,
        idempotencyKey: String,
        expectedRoute: Int?) async throws -> RemoteClawChatSendResponse
    {
        if let expectedRoute, await state.routeGeneration != expectedRoute {
            throw RemoteClawChatTransportSendError.notDispatched
        }
        ...
        if await self.state.sendRoutingChanged {
            throw GatewayResponseError(
                ...
                details: [
                    "reason": AnyCodable(RemoteClawChatSessionRoutingContract.changedErrorReason),
                ])
        }
        if await self.state.sendRejects {
            // Gateway responded but refused to start the run.
            return RemoteClawChatSendResponse(runId: idempotencyKey, status: "error")
        }
        ...
        return RemoteClawChatSendResponse(runId: idempotencyKey, status: "accepted")
    }

    func acquireOutboxRouteLease() async -> RemoteClawChatTransportRouteLeaseResult {
        if let routeUnavailableReason {
            return .unavailable(reason: routeUnavailableReason)
        }
        if !self.requiresRoutingContract {
            let transport = self
            return .available(RemoteClawChatTransportRouteLease(
    ...
    func listSessions(
        limit _: Int?,
        search _: String?,
        archived _: Bool) async throws -> RemoteClawChatSessionsListResponse
    {
        RemoteClawChatSessionsListResponse(
            ts: nil,
            path: nil,
...
private func outboxSessionEntry(
    key: String,
    thinkingLevels: [String]) -> RemoteClawChatSessionEntry
{
    RemoteClawChatSessionEntry(
        ...
        model: nil,
        contextTokens: nil,
        thinkingLevels: thinkingLevels.map { RemoteClawChatThinkingLevelOption(id: $0, label: $0) })
}

private func makeOutboxViewModel(
    transport: OutboxTestTransport,
    outbox: any RemoteClawChatCommandOutbox,
    transcriptCache: (any RemoteClawChatTranscriptCache)? = nil,
    retryDelaysMs: [UInt64] = [1, 1],
    sessionKey: String = "main",
    activeAgentID: String? = "main",
    sessionRoutingContract: String? = "per-sender|main|main") async -> RemoteClawChatViewModel
{
    await MainActor.run {
...
private func sendWhileOffline(_ vm: RemoteClawChatViewModel, text: String) async throws {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
@MainActor
private func queuedStateCount(_ vm: RemoteClawChatViewModel) -> Int {
...
private actor DelayingOutbox: RemoteClawChatCommandOutbox {
    private nonisolated let base: RemoteClawChatSQLiteTranscriptCache
    ...
    init(base: RemoteClawChatSQLiteTranscriptCache) {
        self.base = base
    }

    nonisolated func changes() -> AsyncStream<RemoteClawChatOutboxChange> {
        self.base.changes()
    }
...
private actor SnapshotHoldingOutbox: RemoteClawChatCommandOutbox {
    private nonisolated let base: RemoteClawChatSQLiteTranscriptCache
...
private actor CancellationHoldingOutbox: RemoteClawChatCommandOutbox {
    private nonisolated let base: RemoteClawChatSQLiteTranscriptCache
...
struct ChatViewModelOutboxTests {
    @Test func `offline send queues durably and renders queued row`() async throws {
        ...
        let store = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
    ...
    @Test func `unsupported gateway keeps queued work and surfaces upgrade action`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        let message = RemoteClawChatTransportUpgradeMessage.routingContract
    ...
    @Test func `inert outbox does not capability gate healthy live chat`() async throws {
        ...
        let transport = OutboxTestTransport(
            healthy: true,
            routeUnavailableReason: RemoteClawChatTransportUpgradeMessage.routingContract)
        let vm = await makeOutboxViewModel(transport: transport, outbox: store)
        ...
        parked.lastError = RemoteClawChatSQLiteTranscriptCache.outboxChangedTargetError
    ...
    @Test func `changed default agent parks command visibly for retry`() async throws {
        ...
        #expect(await store.loadCommands().first?.lastError ==
            RemoteClawChatSQLiteTranscriptCache.outboxChangedTargetError)
    ...
    @Test func `atomic gateway routing rejection parks without retrying`() async throws {
        ...
        let command = try #require(await store.loadCommands().first)
        #expect(command.lastError == RemoteClawChatSQLiteTranscriptCache.outboxChangedTargetError)
    ...
    @Test func `failed alias row remains reachable after owner change`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        #expect(await store.enqueueCommand(RemoteClawChatOutboxCommand(
            id: "c-old-failure",
            sessionKey: "main",
            ...
            status: .failed,
            retryCount: 1,
            lastError: RemoteClawChatSQLiteTranscriptCache.outboxExpiredError)))
        let transport = OutboxTestTransport(healthy: false)
        let vm = await makeOutboxViewModel(
            transport: transport,
            outbox: store,
    ...
    @Test func `ownerless global retry stays failed without a selected agent`() async throws {
        ...
        #expect(await store.enqueueCommand(RemoteClawChatOutboxCommand(
            ...
            status: .failed,
            retryCount: 0,
            lastError: RemoteClawChatSQLiteTranscriptCache.outboxUnknownTargetError)))
        let transport = OutboxTestTransport(healthy: false)
        let vm = await makeOutboxViewModel(
            transport: transport,
            outbox: store,
    ...
    @Test func `delete race preserves a turn already proven by canonical history`() async throws {
        ...
        let canonicalMessage = RemoteClawChatMessage(
            role: "user",
    ...
    @Test func `gateway rejections burn attempts then fail terminally and support tap retry`() async throws {
        ...
        #expect(failed.retryCount == RemoteClawChatViewModel.maxOutboxSendAttempts)
    ...
    @Test func `unavailable terminal write drops health instead of advancing FIFO`() async throws {
        ...
        #expect(await store.enqueueCommand(RemoteClawChatOutboxCommand(
            ...
            createdAt: Date().timeIntervalSince1970,
            status: .queued,
            retryCount: RemoteClawChatViewModel.maxOutboxSendAttempts - 1,
            lastError: "rejected")))
        let outbox = DelayingOutbox(base: store)
    ...
    @Test func `ambiguous live transport failure requires explicit retry`() async throws {
        ...
        #expect(preserved.lastError == RemoteClawChatSQLiteTranscriptCache.outboxUnconfirmedError)
    ...
    @Test func `tap retry refreshes createdAt so an expired command can resend`() async throws {
        ...
        let staleCreatedAt = Date().timeIntervalSince1970 -
            RemoteClawChatSQLiteTranscriptCache.outboxCommandMaxAge - 60
        ...
        #expect(await store.loadCommands().map(\.lastError) == [
            RemoteClawChatSQLiteTranscriptCache.outboxExpiredError,
        ])
    ...
    @Test func `background canonical alias event confirms by idempotency key`() async throws {
        ...
        transport.emit(.sessionMessage(RemoteClawSessionMessageEventPayload(
    ...
    @Test func `full queue refuses enqueue and keeps the draft`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = RemoteClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        for index in 0..<RemoteClawChatSQLiteTranscriptCache.maxQueuedCommands {
        ...
        #expect(await userTexts(vm).isEmpty)
        #expect(await store.loadCommands().count == RemoteClawChatSQLiteTranscriptCache.maxQueuedCommands)
    }
    ...
    @Test func `queued send transport failure fails closed until explicit retry`() async throws {
        ...
        let command = try #require(await store.loadCommands().first)
        #expect(command.lastError == RemoteClawChatSQLiteTranscriptCache.outboxUnconfirmedError)
        #expect(command.retryCount == 0)
        #expect(await transport.state.sentIdempotencyKeys.isEmpty)
