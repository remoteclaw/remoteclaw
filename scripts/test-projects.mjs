...
const TIMINGS_FILE_ENV_KEY = "REMOTECLAW_TEST_PROJECTS_TIMINGS_PATH";
const TIMINGS_DISABLE_ENV_KEY = "REMOTECLAW_TEST_PROJECTS_TIMINGS";
...
async function main() {
  ...
  const isExplicitParallelMultiConfigRun =
    Boolean(process.env.REMOTECLAW_TEST_PROJECTS_PARALLEL) &&
    runSpecs.length > 1 &&
    !runSpecs.some((spec) => spec.watchMode);
  ...
  if (isParallelShardRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        ...
        !process.env.REMOTECLAW_VITEST_MAX_WORKERS &&
        !process.env.REMOTECLAW_TEST_WORKERS &&
