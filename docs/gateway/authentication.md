...
Anthropic setup-token auth is still available in RemoteClaw as a supported token
path. Anthropic staff has since told us that RemoteClaw-style Claude CLI usage is
...
- RemoteClaw therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
When provider auth is removed through the Gateway control plane, RemoteClaw deletes
...
Use `remoteclaw models auth login --provider <id> --profile-id <profileId>` for
