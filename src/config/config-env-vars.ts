...
function collectConfigEnvVarsByTarget(cfg?: RemoteClawConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: RemoteClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: RemoteClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}
...
export function applyConfigEnvVars(cfg: RemoteClawConfig, env: NodeJS.ProcessEnv = process.env): void {
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.REMOTECLAW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
