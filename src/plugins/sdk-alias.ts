...
function hasTrustedRemoteClawRootIndicator(params: {
  packageRoot: string;
  ...
  const hasRemoteClawBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("remoteclaw")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.remoteclaw === "string");
  const hasRemoteClawEntrypoint = fs.existsSync(path.join(params.packageRoot, "remoteclaw.mjs"));
  return hasCliEntryExport || hasRemoteClawBin || hasRemoteClawEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedRemoteClawRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedRemoteClawRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  return hasTrustedRemoteClawRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveRemoteClawPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedRemoteClawRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["remoteclaw/plugin-sdk", "@remoteclaw/plugin-sdk"] as const;
...
function readBundledPluginPackageName(packageJsonPath: string): string | null {
  try {
    ...
    return name.startsWith("@remoteclaw/") ? name : null;
  } catch {
    return null;
  }
}
...
function shouldIncludePrivateLocalOnlyPluginSdkSubpaths() {
  return process.env.REMOTECLAW_ENABLE_PRIVATE_QA_CLI === "1";
}
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.prototype.hasOwnProperty.call(aliasMap, `remoteclaw/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  const result: Record<string, string> = {
    ...(extensionApiAlias
      ? { "remoteclaw/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
...
export function isBundledPluginExtensionPath(params: {
  ...
  const roots = [
    ...
    path.join(params.remoteClawPackageRoot, "extensions"),
    path.join(params.remoteClawPackageRoot, "dist", "extensions"),
    path.join(params.remoteClawPackageRoot, "dist-runtime", "extensions"),
