...
<AccordionGroup>
  ...
  <Accordion title="How do I authenticate the dashboard on localhost vs remote?">
    ...
    **Not on localhost:**
    ...
    - **Identity-aware reverse proxy**: keep the Gateway behind a trusted proxy, configure `gateway.auth.mode: "trusted-proxy"`, then open the proxy URL. The proxy must reach the Gateway from a non-loopback address; same-host (loopback-source) proxies are not supported.
