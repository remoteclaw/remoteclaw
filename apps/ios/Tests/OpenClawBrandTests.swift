...
struct RemoteClawBrandTests {
    @Test func `brand colors meet text contrast in both appearances`() {
        let foregroundColors = [
            ("accent", RemoteClawBrand.uiAccentForeground),
            ("accentHot", RemoteClawBrand.uiAccentHotForeground),
            ("ok", RemoteClawBrand.uiOK),
            ("warn", RemoteClawBrand.uiWarn),
            ("danger", RemoteClawBrand.uiDanger),
            ("info", RemoteClawBrand.uiInfo),
        ]
        ...
        for style in [UIUserInterfaceStyle.light, .dark] {
            ...
            let pillBackground = Self.composite(
                RemoteClawBrand.uiAccentForeground,
                alpha: style == .dark ? 0.12 : 0.08,
                over: .secondarySystemGroupedBackground,
                traits: traits)
            #expect(Self.contrastRatio(RemoteClawBrand.uiAccentForeground, pillBackground, traits: traits) >= 4.5)
            #expect(Self.contrastRatio(RemoteClawBrand.uiAccent, .white, traits: traits) >= 4.5)
        }
    }
