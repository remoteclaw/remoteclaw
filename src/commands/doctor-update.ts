...
async function detectRemoteClawGitCheckout(root: string): Promise<"git" | "not-git" | "unknown"> {
...
/** Offers to update RemoteClaw before doctor when running interactively from an updatable install. */
export async function maybeOfferUpdateBeforeDoctor(params: {
  ...
  const updateInProgress = isTruthyEnvValue(process.env.REMOTECLAW_UPDATE_IN_PROGRESS);
  ...
  if (git === "not-git") {
    note(
      [
        ...
        `Run \`${formatCliCommand("remoteclaw update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`,
      ].join("\n"),
      "Update",
    );
  }
