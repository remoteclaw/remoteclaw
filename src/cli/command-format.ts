...
const CLI_PREFIX_RE = /^(?:pnpm|npm|bunx|npx)\s+remoteclaw\b|^remoteclaw\b/;
const CONTAINER_FLAG_RE = /(?:^|\s)--container(?:\s|=|$)/;
const PROFILE_FLAG_RE = /(?:^|\s)--profile(?:\s|=|$)/;
const DEV_FLAG_RE = /(?:^|\s)--dev(?:\s|$)/;
const UPDATE_COMMAND_RE = /^(?:(?:pnpm|npm|bunx|npx)\s+remoteclaw|remoteclaw)\b.*\supdate(?:\s|$)/;
...
export function formatCliCommand(
  ...
  const rawContainer = env.REMOTECLAW_CONTAINER_HINT?.trim();
  const container = rawContainer && CONTAINER_HINT_RE.test(rawContainer) ? rawContainer : undefined;
  const profile = normalizeProfileName(env.REMOTECLAW_PROFILE);
