...
describe("canvas host", () => {
  ...
  it("injects live reload script", () => {
    ...
    expect(out).toContain("remoteclawCanvasA2UIAction");
    expect(out).toContain("remoteclawSendUserAction");
  });
  ...
  it("serves A2UI scaffold and blocks traversal/symlink escapes", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.remoteclawA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__remoteclaw__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      ...
      expect(js).toContain("remoteclaw-a2ui-host");
