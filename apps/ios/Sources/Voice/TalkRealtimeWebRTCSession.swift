...
final class TalkRealtimeWebRTCSession: NSObject {
    private static let logger = Logger(subsystem: "org.remoteclaw.app", category: "TalkRealtimeWebRTC")
    private static let consultToolName = "remoteclaw_agent_consult"
    private static let controlToolName = "remoteclaw_agent_control"
    private static let defaultOfferURL = "https://api.openai.com/v1/realtime/calls"
    private static let mediaStreamID = "remoteclaw-ios-realtime"
    private static let audioTrackID = "remoteclaw-ios-audio"
    ...
    private func handleToolDone(_ event: TalkRealtimeServerEvent) {
        ...
        self.delegate?.realtimeSession(
            self,
            didChangeStatus: name == Self.controlToolName ? "Updating RemoteClaw" : "Asking RemoteClaw")
    ...
    private func submitConsultToolCall(callId: String, argsJSON: String) async {
        ...
        let statusTask = Task { @MainActor [weak self] in
            ...
            guard let self, !Task.isCancelled, !self.stopped else { return }
            self.delegate?.realtimeSession(self, didChangeStatus: "Still asking RemoteClaw")
        }
        defer {
            statusTask.cancel()
        ...
        do {
            ...
            self.delegate?.realtimeSession(self, didChangeStatus: "RemoteClaw unavailable")
            let fallbackMessage = [
                "RemoteClaw consult did not finish quickly enough.",
                "Give a brief spoken fallback from the realtime conversation",
                "and ask the user to try again if they need RemoteClaw-specific context.",
            ].joined(separator: " ")
    ...
    private func submitControlToolCall(callId: String, argsJSON: String) async {
        ...
        do {
            ...
            let res = try await gateway.request(
                ...
                paramsJSON: json,
                timeoutSeconds: Self.toolCallTimeoutSeconds)
            let message = Self.controlResultMessage(from: res) ?? "RemoteClaw updated the active run."
            ...
            self.submitToolResult(callId: callId, result: [
                "error": "RemoteClaw could not update the active run.",
            ])
        }
        guard !Task.isCancelled, !self.stopped else { return }
        if !self.assistantAudioActive {
    ...
    private static func controlParams(sessionKey: String, argsJSON: String) throws -> [String: Any] {
        ...
        guard let text else {
            throw NSError(domain: "TalkRealtimeWebRTC", code: 20, userInfo: [
                NSLocalizedDescriptionKey: "RemoteClaw control tool call missing text",
            ])
        }
        ...
        return try await withThrowingTaskGroup(of: String.self) { group in
            group.addTask { [runId, currentSessionKey] in
                for await evt in stream {
                    ...
                    if chatEvent.state == "final" {
                        return RemoteClawChatEventText.assistantText(from: chatEvent) ?? "RemoteClaw finished with no text."
                    }
                    if chatEvent.state == "aborted" {
                        throw NSError(domain: "TalkRealtimeWebRTC", code: 9, userInfo: [
                            NSLocalizedDescriptionKey: "RemoteClaw realtime tool call aborted",
                        ])
                    }
                    if chatEvent.state == "error" {
                        throw NSError(domain: "TalkRealtimeWebRTC", code: 10, userInfo: [
                            NSLocalizedDescriptionKey: "RemoteClaw realtime tool call failed",
                        ])
                    }
                }
                throw NSError(domain: "TalkRealtimeWebRTC", code: 11, userInfo: [
                    NSLocalizedDescriptionKey: "RemoteClaw realtime tool event stream ended",
                ])
            }
            ...
            group.addTask {
                ...
                throw NSError(domain: "TalkRealtimeWebRTC", code: 12, userInfo: [
                    NSLocalizedDescriptionKey: "RemoteClaw realtime tool call timed out",
                ])
            }
            guard let result = try await group.next() else {
                throw NSError(domain: "TalkRealtimeWebRTC", code: 13, userInfo: [
                    NSLocalizedDescriptionKey: "RemoteClaw realtime tool call did not finish",
                ])
            }
            group.cancelAll()
            return result
        ...
        timeoutSeconds: Int) async throws -> String
    {
        ...
        while Date() < deadline {
            ...
            case "error":
                throw NSError(domain: "TalkRealtimeWebRTC", code: 14, userInfo: [
                    NSLocalizedDescriptionKey: wait.error ?? "RemoteClaw realtime tool call failed",
                ])
            case "aborted", "cancelled", "canceled":
                throw NSError(domain: "TalkRealtimeWebRTC", code: 15, userInfo: [
                    NSLocalizedDescriptionKey: wait.stopReason ?? "RemoteClaw realtime tool call aborted",
                ])
            case "timeout":
                break
            default:
        ...
        throw NSError(domain: "TalkRealtimeWebRTC", code: 16, userInfo: [
            NSLocalizedDescriptionKey: "RemoteClaw realtime tool call timed out in \(phase)",
        ])
    }
        ...
        timeoutSeconds: Int) async throws -> AgentWaitResponse
    {
        ...
        guard let json = String(data: data, encoding: .utf8) else {
            throw NSError(domain: "TalkRealtimeWebRTC", code: 17, userInfo: [
                NSLocalizedDescriptionKey: "Failed to encode RemoteClaw wait request",
            ])
        }
        ...
        since: Double) async throws -> String?
    {
        ...
        guard let json = String(data: data, encoding: .utf8) else {
            throw NSError(domain: "TalkRealtimeWebRTC", code: 18, userInfo: [
                NSLocalizedDescriptionKey: "Failed to encode RemoteClaw history request",
            ])
        }
        let response = try await gateway.request(method: "chat.history", paramsJSON: json, timeoutSeconds: 15)
        let history = try JSONDecoder().decode(RemoteClawChatHistoryPayload.self, from: response)
        let messages = history.messages ?? []
        let decoded: [RemoteClawChatMessage] = messages.compactMap { item in
