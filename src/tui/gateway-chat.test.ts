...
describe("resolveGatewayConnection", () => {
  ...
  it.each([
    ...
    const result = await withEnvAsync({ REMOTECLAW_GATEWAY_TOKEN: "env-token" }, async () => {
  ...
  it("falls back to REMOTECLAW_GATEWAY_TOKEN when config token is missing", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });
  ...
  it("prefers REMOTECLAW_GATEWAY_PASSWORD over remote password fallback", async () => {
    ...
    const gatewayPasswordEnv = "REMOTECLAW_GATEWAY_PASSWORD"; // pragma: allowlist secret
