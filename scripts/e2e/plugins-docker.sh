...
IMAGE_NAME="remoteclaw-plugins-e2e"
...
if [ -f dist/index.mjs ]; then
  REMOTECLAW_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  REMOTECLAW_ENTRY="dist/index.js"
else
  echo "Missing dist/index.(m)js (build output):"
  ls -la dist || true
  exit 1
fi
export REMOTECLAW_ENTRY

home_dir=$(mktemp -d "/tmp/remoteclaw-plugins-e2e.XXXXXX")
export HOME="$home_dir"
BUNDLED_PLUGIN_ROOT_DIR="extensions"
REMOTECLAW_PLUGIN_HOME="$HOME/.remoteclaw/$BUNDLED_PLUGIN_ROOT_DIR"
...
start_gateway() {
  local log_file="$1"
  : > "$log_file"
  node "$REMOTECLAW_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured \
    >"$log_file" 2>&1 &
  gateway_pid=$!
...
wait_for_gateway_health() {
  for _ in $(seq 1 120); do
    if node "$REMOTECLAW_ENTRY" gateway health \
...
run_gateway_chat_json() {
  ...
  node - <<'NODE' "$REMOTECLAW_ENTRY" "$session_key" "$message" "$output_file" "$timeout_ms"
...
JS
  cat > "$dir/remoteclaw.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/remoteclaw.plugin.json" "$id"
const fs = require("node:fs");
...
demo_plugin_root="$REMOTECLAW_PLUGIN_HOME/$demo_plugin_id"
...
JS
cat > "$demo_plugin_root/remoteclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$REMOTECLAW_ENTRY" plugins list --json > /tmp/plugins.json
node "$REMOTECLAW_ENTRY" plugins inspect demo-plugin --json > /tmp/plugins-inspect.json
...
pack_dir="$(mktemp -d "/tmp/remoteclaw-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@remoteclaw/demo-plugin-tgz",
  "version": "0.0.1",
  "remoteclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/remoteclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
node "$REMOTECLAW_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$REMOTECLAW_ENTRY" plugins list --json > /tmp/plugins2.json
node "$REMOTECLAW_ENTRY" plugins inspect demo-plugin-tgz --json > /tmp/plugins2-inspect.json
...
dir_plugin="$(mktemp -d "/tmp/remoteclaw-plugin-dir.XXXXXX")"
...
JS
cat > "$dir_plugin/remoteclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
node "$REMOTECLAW_ENTRY" plugins install "$dir_plugin"
node "$REMOTECLAW_ENTRY" plugins list --json > /tmp/plugins3.json
node "$REMOTECLAW_ENTRY" plugins inspect demo-plugin-dir --json > /tmp/plugins3-inspect.json
...
file_pack_dir="$(mktemp -d "/tmp/remoteclaw-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@remoteclaw/demo-plugin-file",
  "version": "0.0.1",
  "remoteclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/remoteclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
node "$REMOTECLAW_ENTRY" plugins install "file:$file_pack_dir/package"
node "$REMOTECLAW_ENTRY" plugins list --json > /tmp/plugins4.json
node "$REMOTECLAW_ENTRY" plugins inspect demo-plugin-file --json > /tmp/plugins4-inspect.json
...
bundle_root="$REMOTECLAW_PLUGIN_HOME/$bundle_plugin_id"
...
const configPath = path.join(process.env.HOME, ".remoteclaw", "remoteclaw.json");
...
gateway_log="/tmp/remoteclaw-plugin-command-e2e.log"
start_gateway "$gateway_log"
wait_for_gateway_health

echo "Testing /plugin install with auto-restart..."
slash_install_dir="$(mktemp -d "/tmp/remoteclaw-plugin-slash-install.XXXXXX")"
cat > "$slash_install_dir/package.json" <<'JSON'
{
  "name": "@remoteclaw/slash-install-plugin",
  "version": "0.0.1",
  "remoteclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$slash_install_dir/remoteclaw.plugin.json" <<'JSON'
{
  "id": "slash-install-plugin",
  "configSchema": {
    "type": "object",
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "remoteclaw/fixture-marketplace"
    }
  }
}
JSON

node "$REMOTECLAW_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
node "$REMOTECLAW_ENTRY" plugins install marketplace-shortcut@claude-fixtures
node "$REMOTECLAW_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$REMOTECLAW_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
node "$REMOTECLAW_ENTRY" plugins inspect marketplace-shortcut --json > /tmp/plugins-marketplace-shortcut-inspect.json
node "$REMOTECLAW_ENTRY" plugins inspect marketplace-direct --json > /tmp/plugins-marketplace-direct-inspect.json
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
node "$REMOTECLAW_ENTRY" plugins update marketplace-shortcut --dry-run
node "$REMOTECLAW_ENTRY" plugins update marketplace-shortcut
node "$REMOTECLAW_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
node "$REMOTECLAW_ENTRY" plugins inspect marketplace-shortcut --json > /tmp/plugins-marketplace-updated-inspect.json
