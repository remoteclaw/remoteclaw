...
It is a **library guardrail** for trusted RemoteClaw code that receives untrusted path names, not a sandbox. Host filesystem permissions, OS users, containers, and the agent/tool policy still define the real blast radius.

## Default: no Python helper

RemoteClaw sets the fs-safe POSIX Python helper to **off** by default:
...
RemoteClaw only changes the _default_. An explicit setting always wins:
...
With the helper off, RemoteClaw still gets fs-safe's Node-only guardrails:
...
This covers RemoteClaw's normal threat model: trusted gateway code handling untrusted model/plugin/channel path input inside a single trusted operator boundary.
...
That narrows same-UID race windows where another process swaps a parent directory between validation and mutation — defense in depth on hosts where untrusted local processes can modify the same directories RemoteClaw operates in.
...
- Core code should use the fs-safe wrappers under `src/infra/*` so RemoteClaw's process policy applies consistently.
