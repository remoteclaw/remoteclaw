...
describe("gateway codex harness live helpers", () => {
  it("accepts the current codex status prose from the live harness", () => {
    const text =
      "RemoteClaw is running on `openai/gpt-5.5` with low reasoning/text settings. Context is at `22k/272k` tokens, no compactions, and the current session is `agent:dev:live-codex-harness`.";
  ...
  it("accepts current status prose that reports session context without the session id", () => {
    const text = [
      "RemoteClaw is running on `openai/gpt-5.5` with low reasoning/text settings.",
      "",
  ...
  it("accepts current status prose that reports healthy session context without the session id", () => {
    const text = [
      ...
      "",
      "Session context is healthy: `22k/272k` tokens used, `0` compactions, `53%` cache hit. Current workspace is `/tmp/remoteclaw-live-codex-harness/workspace/dev`.",
    ].join("\n");

    expect(isExpectedCodexStatusCommandText(text)).toBe(true);
  });

  it("accepts current app-server status prose without the RemoteClaw prefix", () => {
    const text = [
      "Status: running on `openai/gpt-5.5` in `/tmp/remoteclaw-live-codex-harness/workspace/dev`.",
      "",
  ...
  it("accepts the OpenAI Codex status card emitted by the GPT-5.5 Docker harness", () => {
    const text = [
      "RemoteClaw 2026.4.30-beta.1 is running on `openai/gpt-5.5`.",
      "",
      "Session is healthy:",
  ...
  it("rejects status prose for a different codex session", () => {
    const text =
      "RemoteClaw is running on `openai/gpt-5.5` with low reasoning/text settings. Context is at `22k/272k` tokens, no compactions, and the current session is `agent:dev:other`.";
  ...
  it("accepts the GPT-5.5 Docker harness shell fallback", () => {
    const text = [
      ...
      "",
      "Current session model from RemoteClaw status is `openai/gpt-5.5`.",
    ].join("\n");
