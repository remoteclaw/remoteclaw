...
const TMUX_CWD_ENV_KEY = "REMOTECLAW_GATEWAY_WATCH_CWD";
const TMUX_CWD_OPTION_KEY = "@remoteclaw.gateway_watch.cwd";
...
const resolveGatewayWatchBenchmarkArgs = ({ args = [], env = process.env } = {}) => {
  ...
  if (benchmarkFlagSeen) {
    ...
    nextEnv.REMOTECLAW_TRACE_SYNC_IO ??= "0";
    if (nextEnv.REMOTECLAW_TRACE_SYNC_IO === "1") {
...
export const resolveGatewayWatchTmuxSessionName = ({ args = [], env = process.env } = {}) => {
  ...
  const port = env.REMOTECLAW_GATEWAY_PORT || readArgValue(args, "--port");
  const parts = [
    "remoteclaw",
    "gateway",
    "watch",
...
export const buildGatewayWatchTmuxCommand = ({
  ...
  const childEnv = [
    "env",
    ...colorEnv.options,
    `REMOTECLAW_GATEWAY_WATCH_TMUX_CHILD=1`,
    `REMOTECLAW_GATEWAY_WATCH_SESSION=${sessionName}`,
...
const log = (stderr, message) => {
  stderr.write(`[remoteclaw] ${message}\n`);
};
...
const shouldAttachTmux = ({ env, stdinIsTTY, stdoutIsTTY }) => {
  const raw = String(env.REMOTECLAW_GATEWAY_WATCH_ATTACH ?? "").toLowerCase();
...
export const runGatewayWatchTmuxMain = (params = {}) => {
  ...
  if (TMUX_DISABLE_VALUES.has((deps.env.REMOTECLAW_GATEWAY_WATCH_TMUX ?? "").toLowerCase())) {
  ...
  if (deps.env.REMOTECLAW_GATEWAY_WATCH_TMUX_CHILD === "1") {
    return runForegroundWatcher({
      args: deps.args,
      cwd: deps.cwd,
      env: deps.env,
