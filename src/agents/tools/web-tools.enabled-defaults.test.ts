...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
      ...
      if (idx === 1) {
        return new Response(
          JSON.stringify({
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "remoteclaw" }),
                      },
                    },
                  ],
                },
            ...
            search_results: [
              { title: "RemoteClaw", url: "https://remoteclaw.ai/docs", content: "docs" },
            ],
          }),
    ...
    const result = await tool?.execute?.("call-1", { query: "latest remoteclaw release" });
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://remoteclaw.ai/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://remoteclaw.ai/docs"]);
