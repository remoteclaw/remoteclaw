...
RemoteClaw Release Publish command only after everything is green.
...
Options:
  ...
  --npm-preflight-run <id>            Reuse successful RemoteClaw NPM Release preflight run.
...
export function parseArgs(argv) {
  ...
  if (options.pluginPublishScope === "selected") {
    throw new Error(
      "--plugin-publish-scope selected is only for plugin-only repair publishes; release candidates publish RemoteClaw with --plugin-publish-scope all-publishable",
    );
  }
...
function githubApiTimeoutMs() {
  const raw = process.env.REMOTECLAW_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS;
  if (!raw) {
    return DEFAULT_GITHUB_API_TIMEOUT_MS;
  }
  const value = Number(raw);
  if (!Number.isFinite(value) || value <= 0) {
    throw new Error("REMOTECLAW_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS must be a positive number");
  }
...
export function buildPublishCommand(options) {
  ...
  return [
    ...
    "workflow",
    "run",
    "remoteclaw-release-publish.yml",
    "--repo",
    options.repo,
    "--ref",
    options.workflowRef,
...
async function runTelegramIfNeeded(options, artifactName) {
  ...
  const dispatchedRunId = dispatchWorkflow(options.repo, workflowFile, options.workflowRef, {
    package_spec: `remoteclaw@${options.tag.replace(/^v/u, "")}`,
...
async function main() {
  ...
  if (!options.npmPreflightRunId && !options.skipDispatch) {
    const workflowFile = "remoteclaw-npm-release.yml";
    const before = await beforeRunIds(options.repo, workflowFile);
    const dispatchedRunId = dispatchWorkflow(options.repo, workflowFile, options.workflowRef, {
      tag: options.tag,
      preflight_only: "true",
    ...
    options.npmPreflightRunId =
      dispatchedRunId ||
      (await findNewRunId(options.repo, workflowFile, "RemoteClaw NPM Release", before));
  }
  ...
  const npmRun = await waitForSuccessfulRun(options.repo, options.npmPreflightRunId, {
    workflowName: "RemoteClaw NPM Release",
    workflowRef: options.workflowRef,
  });
  ...
  const npmArtifactName = await downloadResolvedArtifact(
    options.repo,
    options.npmPreflightRunId,
    `remoteclaw-npm-preflight-${options.tag}`,
    "remoteclaw-npm-preflight-",
    npmDir,
  );
