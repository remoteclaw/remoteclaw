...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: RemoteClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: RemoteClawKit.AnyCodable]()) {
                $0[$1.key] = RemoteClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: RemoteClawProtocol.AnyCodable?) -> [String: RemoteClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: RemoteClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: RemoteClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: RemoteClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("remoteclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("remoteclaw.control.agent")
}
