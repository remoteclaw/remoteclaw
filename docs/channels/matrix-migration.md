---
summary: "How RemoteClaw upgrades the previous Matrix plugin in place, including encrypted-state recovery limits and manual recovery steps."
read_when:
...
- the plugin stays `@remoteclaw/matrix`
- the channel stays `matrix`
- your config stays under `channels.matrix`
- cached credentials stay under `~/.remoteclaw/credentials/matrix/`
- runtime state stays under `~/.remoteclaw/matrix/`
...
When the gateway starts, and when you run [`remoteclaw doctor --fix`](/gateway/doctor), RemoteClaw tries to repair old Matrix state automatically.
Before any actionable Matrix migration step mutates on-disk state, RemoteClaw creates or reuses a focused recovery snapshot.

When you use `remoteclaw update`, the exact trigger depends on how RemoteClaw is installed:
...
- if you use `remoteclaw update --no-restart`, startup-backed Matrix migration is deferred until you later run `remoteclaw doctor --fix` and restart the gateway

Automatic migration covers:

- creating or reusing a pre-migration snapshot under `~/Backups/remoteclaw-migrations/`
...
- RemoteClaw writes a marker file at `~/.remoteclaw/matrix/migration-snapshot.json` after a successful snapshot so later startup and repair passes can reuse the same archive.
- These automatic Matrix migration snapshots back up config + state only (`includeWorkspace: false`).
- If Matrix only has warning-only migration state, for example because `userId` or `accessToken` is still missing, RemoteClaw does not create the snapshot yet because no Matrix mutation is actionable.
- If the snapshot step fails, RemoteClaw skips Matrix migration for that run instead of mutating state without a recovery point.

About multi-account upgrades:

- the oldest flat Matrix store (`~/.remoteclaw/matrix/bot-storage.json` and `~/.remoteclaw/matrix/crypto/`) came from a single-store layout, so RemoteClaw can only migrate it into one resolved Matrix account target
...
RemoteClaw cannot automatically recover:
...
1. Update RemoteClaw and the Matrix plugin normally.
   Prefer plain `remoteclaw update` without `--no-restart` so startup can finish the Matrix migration immediately.
2. Run:
...
6. If RemoteClaw tells you a recovery key is needed, run the command for the matching account:

   ```bash
   printf '%s\n' "$MATRIX_RECOVERY_KEY" | remoteclaw matrix verify backup restore --recovery-key-stdin
   printf '%s\n' "$MATRIX_RECOVERY_KEY_ASSISTANT" | remoteclaw matrix verify backup restore --recovery-key-stdin --account assistant
   ```
...
7. If this device is still unverified, run the command for the matching account:
   ...
   printf '%s\n' "$MATRIX_RECOVERY_KEY_ASSISTANT" | remoteclaw matrix verify device --recovery-key-stdin --account assistant
   ```
...
3. If a backup decryption key is found, RemoteClaw writes it into the new recovery-key flow and marks room-key restore as pending.
4. On the next Matrix startup, RemoteClaw restores backed-up room keys into the new crypto store automatically.

If the old store reports room keys that were never backed up, RemoteClaw warns instead of pretending recovery succeeded.
...
- Meaning: RemoteClaw created a recovery archive before mutating Matrix state.
- What to do: keep the printed archive path until you confirm migration succeeded.

`Matrix migration snapshot reused before applying Matrix upgrades.`

- Meaning: RemoteClaw found an existing Matrix migration snapshot marker and reused that archive instead of creating a duplicate backup.
- What to do: keep the printed archive path until you confirm migration succeeded.

`Legacy Matrix state detected at ... but channels.matrix is not configured yet.`

- Meaning: old Matrix state exists, but RemoteClaw cannot map it to a current Matrix account because Matrix is not configured.
...
- Meaning: RemoteClaw found old state, but it still cannot determine the exact current account/device root.
...
- Meaning: RemoteClaw found one shared flat Matrix store, but it refuses to guess which named Matrix account should receive it.
...
- Meaning: the new account-scoped location already has a sync or crypto store, so RemoteClaw did not overwrite it automatically.
...
- Meaning: RemoteClaw tried to move old Matrix state but the filesystem operation failed.
- What to do: inspect filesystem permissions and disk state, then rerun `remoteclaw doctor --fix`.

`Legacy Matrix encrypted state detected at ... but channels.matrix is not configured yet.`

- Meaning: RemoteClaw found an old encrypted Matrix store, but there is no current Matrix config to attach it to.
- What to do: configure `channels.matrix`, then rerun `remoteclaw doctor --fix` or restart the gateway.

`Legacy Matrix encrypted state detected at ... but the account-scoped target could not be resolved yet (need homeserver, userId, and access token for channels.matrix...).`

- Meaning: the encrypted store exists, but RemoteClaw cannot safely decide which current account/device it belongs to.
...
- Meaning: RemoteClaw found one shared flat legacy crypto store, but it refuses to guess which named Matrix account should receive it.
- What to do: set `channels.matrix.defaultAccount` to the intended account, then rerun `remoteclaw doctor --fix` or restart the gateway.

`Matrix migration warnings are present, but no on-disk Matrix mutation is actionable yet. No pre-migration snapshot was needed.`

- Meaning: RemoteClaw detected old Matrix state, but the migration is still blocked on missing identity or credential data.
- What to do: finish Matrix login or config setup, then rerun `remoteclaw doctor --fix` or restart the gateway.

`Legacy Matrix encrypted state was detected, but the Matrix plugin helper is unavailable. Install or repair @remoteclaw/matrix so RemoteClaw can inspect the old rust crypto store before upgrading.`
...
- Meaning: RemoteClaw found a helper file path that escapes the plugin root or fails plugin boundary checks, so it refused to import it.
...
- Meaning: RemoteClaw refused to mutate Matrix state because it could not create the recovery snapshot first.
- What to do: resolve the backup error, then rerun `remoteclaw doctor --fix` or restart the gateway.

`Failed migrating legacy Matrix client storage: ...`

- Meaning: the Matrix client-side fallback found old flat storage, but the move failed. RemoteClaw now aborts that fallback instead of silently starting with a fresh store.
...
`Legacy Matrix encrypted state for account "..." has backed-up room keys, but no local backup decryption key was found. Ask the operator to run "remoteclaw matrix verify backup restore --recovery-key-stdin" after upgrade if they have the recovery key.`

- Meaning: backup exists, but RemoteClaw could not recover the recovery key automatically.
- What to do: run `printf '%s\n' "$MATRIX_RECOVERY_KEY" | remoteclaw matrix verify backup restore --recovery-key-stdin`.

`Failed inspecting legacy Matrix encrypted state for account "..." (...): ...`

- Meaning: RemoteClaw found the old encrypted store, but it could not inspect it safely enough to prepare recovery.
- What to do: rerun `remoteclaw doctor --fix`. If it repeats, keep the old state directory intact and recover using another verified Matrix client plus `printf '%s\n' "$MATRIX_RECOVERY_KEY" | remoteclaw matrix verify backup restore --recovery-key-stdin`.

`Legacy Matrix backup key was found for account "...", but .../recovery-key.json already contains a different recovery key. Leaving the existing file unchanged.`

- Meaning: RemoteClaw detected a backup key conflict and refused to overwrite the current recovery-key file automatically.
...
- What to do: run `remoteclaw matrix verify backup status`, then retry with `printf '%s\n' "$MATRIX_RECOVERY_KEY" | remoteclaw matrix verify backup restore --recovery-key-stdin` if needed.

### Manual recovery messages

`Backup key is not loaded on this device. Run 'remoteclaw matrix verify backup restore' to load it and restore old room keys.`

- Meaning: RemoteClaw knows you should have a backup key, but it is not active on this device.
- What to do: run `remoteclaw matrix verify backup restore`, or set `MATRIX_RECOVERY_KEY` and run `printf '%s\n' "$MATRIX_RECOVERY_KEY" | remoteclaw matrix verify backup restore --recovery-key-stdin` if needed.

`Store a recovery key with 'remoteclaw matrix verify device --recovery-key-stdin', then run 'remoteclaw matrix verify backup restore'.`

- Meaning: this device does not currently have the recovery key stored.
- What to do: set `MATRIX_RECOVERY_KEY`, run `printf '%s\n' "$MATRIX_RECOVERY_KEY" | remoteclaw matrix verify device --recovery-key-stdin`, then restore the backup.

`Backup key mismatch on this device. Re-run 'remoteclaw matrix verify device --recovery-key-stdin' with the matching recovery key.`
...
current backup baseline with `remoteclaw matrix verify backup reset --yes`. When the
...
`Backup trust chain is not verified on this device. Re-run 'remoteclaw matrix verify device --recovery-key-stdin'.`
...
- Meaning: RemoteClaw could apply the recovery key, but Matrix still has not
...
- What to do: run `remoteclaw matrix verify self`, accept the request in another
  ...
  `printf '%s\n' "$MATRIX_RECOVERY_KEY" | remoteclaw matrix verify bootstrap --recovery-key-stdin --force-reset-cross-signing`
...
- What to do: verify the device first, then recheck with `remoteclaw matrix verify backup status`.

`Matrix crypto backend cannot load backup keys from secret storage. Verify this device with 'remoteclaw matrix verify device --recovery-key-stdin' first.`

- Meaning: this device cannot restore from secret storage until device verification is complete.
- What to do: run `printf '%s\n' "$MATRIX_RECOVERY_KEY" | remoteclaw matrix verify device --recovery-key-stdin` first.
...
printf '%s\n' "$MATRIX_RECOVERY_KEY" | remoteclaw matrix verify backup restore --recovery-key-stdin --verbose
```
