...
/// (`~/Library/Application Support/RemoteClaw/chat-cache.sqlite`), matching the
/// existing RemoteClaw app-support layout. macOS has no per-file Data Protection
...
enum MacChatTranscriptCache {
    struct Context {
        let store: RemoteClawChatSQLiteTranscriptCache
        let routingIdentity: RemoteClawChatSessionRoutingIdentity?
    }
    ...
    @MainActor private static var storesByGatewayID: [String: RemoteClawChatSQLiteTranscriptCache] = [:]
    ...
    ///   session data: distinct profiles (distinct `REMOTECLAW_STATE_DIR`) never
    ...
    static func currentGatewayID() -> String? {
        ...
        return self.gatewayID(
            mode: mode,
            localStateDir: RemoteClawConfigFile.stateDirURL(),
    ...
    @MainActor
    static func make() -> RemoteClawChatSQLiteTranscriptCache? {
    ...
    static func makeContext() -> Context? {
        ...
        let databaseURL = base.appendingPathComponent("RemoteClaw/chat-cache.sqlite", isDirectory: false)
        return Context(
            store: self.store(databaseURL: databaseURL, gatewayID: gatewayID),
            routingIdentity: RemoteClawChatSQLiteTranscriptCache.loadSessionRoutingIdentity(
                databaseURL: databaseURL,
                gatewayID: gatewayID))
    }

    @MainActor
    static func store(databaseURL: URL, gatewayID: String) -> RemoteClawChatSQLiteTranscriptCache {
        if let store = storesByGatewayID[gatewayID] {
            return store
        }
        let store = RemoteClawChatSQLiteTranscriptCache(databaseURL: databaseURL, gatewayID: gatewayID)
