...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like RemoteClaw.
...
```bash
remoteclaw onboard --auth-choice openai-api-key
# or non-interactive
remoteclaw onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. RemoteClaw forwards both through the `openai/*` Responses path.
RemoteClaw intentionally suppresses the stale `openai/gpt-5.3-codex-spark` row,
because direct OpenAI API calls reject it in live traffic.

RemoteClaw does **not** expose `openai/gpt-5.3-codex-spark` on the direct OpenAI
API path. `pi-ai` still ships a built-in row for that model, but live OpenAI API
requests currently reject it. Spark is treated as Codex-only in RemoteClaw.
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. RemoteClaw
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

If your Codex account is entitled to Codex Spark, RemoteClaw also supports:

- `openai-codex/gpt-5.3-codex-spark`

RemoteClaw treats Codex Spark as Codex-only. It does not expose a direct
`openai/gpt-5.3-codex-spark` API-key path.

RemoteClaw also preserves `openai-codex/gpt-5.3-codex-spark` when `pi-ai`
...
RemoteClaw uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), RemoteClaw also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. RemoteClaw enables it by default for
...
RemoteClaw, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
RemoteClaw exposes a shared fast-mode toggle for both `openai/*` and
`openai-codex/*` sessions:

- Chat/UI: `/fast status|on|off`
- Config: `agents.defaults.models["<provider>/<model>"].params.fastMode`

When fast mode is enabled, RemoteClaw applies a low-latency OpenAI profile:
...
`baseUrl` on `api.openai.com`), RemoteClaw now auto-enables OpenAI server-side
