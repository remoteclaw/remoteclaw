...
- Same path: run the app from a fixed location (for RemoteClaw, `dist/RemoteClaw.app`).
...
```bash
sudo tccutil reset Accessibility org.remoteclaw.mac
sudo tccutil reset ScreenCapture org.remoteclaw.mac
...
Workaround: move files into the RemoteClaw workspace (`~/.remoteclaw/workspace`) if you want to avoid per-folder grants.
