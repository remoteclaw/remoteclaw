...
- Workspace + bootstrap files (`AGENTS.md`, `SOUL.md`, `TOOLS.md`,
  ...
  - Lowercase root `memory.md` is never injected. It is legacy repair input
    for `remoteclaw doctor --fix`, which migrates it into `MEMORY.md`.
...
`toolResultMaxChars` is unset by default, so RemoteClaw derives the live
...
- `/usage off|tokens|full` -> appends a per-response usage footer to every
  ...
  - `/usage full` shows compact model/context/cost details; estimated cost
    appears only when RemoteClaw has usage metadata and local pricing for the
    active model. Custom `messages.usageTemplate` layouts can include
    token/cache fields.
- `/usage cost` -> local cost summary from RemoteClaw session logs.
...
RemoteClaw falls back to matching OAuth/API-key credentials from auth
...
Provider prompt caching only applies within the cache TTL window. RemoteClaw
...
RemoteClaw sizes GA-capable Claude 4.x models such as Opus 4.8, Opus 4.7, Opus
...
(`sk-ant-oat-*`), RemoteClaw preserves the OAuth-required Anthropic beta
