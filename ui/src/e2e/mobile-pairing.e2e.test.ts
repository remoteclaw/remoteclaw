...
describeControlUiE2e("Control UI mobile pairing mocked Gateway E2E", () => {
  beforeAll(async () => {
    if (!chromiumAvailable) {
      throw new Error(
        `Playwright Chromium is not installed or cannot start at ${chromiumExecutablePath}. Run \`pnpm --dir ui exec playwright install --with-deps chromium\`, or set REMOTECLAW_UI_E2E_ALLOW_MISSING_CHROMIUM=1 only when intentionally skipping this lane.`,
      );
    }
  ...
  it("opens pairing from the app shell and Quick Settings", async () => {
    ...
    try {
      ...
      const dialog = page.getByRole("dialog", { name: "RemoteClaw mobile" });
      const qr = page.getByAltText("RemoteClaw mobile pairing QR code");
      ...
      expect(
        await page.getByText("Official RemoteClaw mobile apps connect automatically").isVisible(),
      ).toBe(true);
