...
describe("tool image sanitizing", () => {
  ...
  async function withUnavailableImageBackend<T>(fn: () => Promise<T>): Promise<T> {
    const previousBackend = process.env.REMOTECLAW_IMAGE_BACKEND;
    process.env.REMOTECLAW_IMAGE_BACKEND = unavailableImageBackend;
    try {
      return await fn();
    } finally {
      if (previousBackend === undefined) {
        delete process.env.REMOTECLAW_IMAGE_BACKEND;
      } else {
        process.env.REMOTECLAW_IMAGE_BACKEND = previousBackend;
      }
    }
  }
