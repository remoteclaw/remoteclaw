...
import { autoMigrateLegacyStateDir, resetAutoMigrateLegacyStateDirForTest } from "./state-migrations.js";
...
describe("legacy state dir auto-migration", () => {
  it("follows legacy symlink when it points at another legacy dir (clawdbot -> moldbot)", async () => {
    ...
    const targetMarker = path.join(root, ".remoteclaw", "marker.txt");
  ...
  it("skips state-dir migration when REMOTECLAW_STATE_DIR is explicitly set", async () => {
    const root = await makeTempRoot();
