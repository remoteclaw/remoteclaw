...
public struct RemoteClawChatView: View {
    ...
    private let talkControl: RemoteClawChatTalkControl?
    private let voiceNoteControl: RemoteClawChatVoiceNoteControl?
    private let speech: RemoteClawChatSpeechController?
    ...
    public init(
        ...
        talkControl: RemoteClawChatTalkControl? = nil,
        voiceNoteControl: RemoteClawChatVoiceNoteControl? = nil,
        speech: RemoteClawChatSpeechController? = nil)
    {
    ...
    @ViewBuilder
    private func messageRow(for msg: RemoteClawChatMessage) -> some View {
    ...
    private func isListenable(_ msg: RemoteClawChatMessage) -> Bool {
    ...
    private func speechChipIsPreparing(
        _ speech: RemoteClawChatSpeechController,
        messageID: UUID) -> Bool?
    {
        switch speech.phase {
    ...
    @ViewBuilder
    private func copyMessageButton(for message: RemoteClawChatMessage) -> some View {
    ...
    private var jumpToLatestButton: some View {
        Button {
            ...
            Label("Jump to latest", systemImage: "arrow.down")
                .font(RemoteClawChatTypography.body(size: 16, weight: .semibold, relativeTo: .callout))
                .padding(.horizontal, 13)
                .padding(.vertical, 8)
        }
        .buttonStyle(.plain)
        .foregroundStyle(RemoteClawChatTheme.assistantText)
        .background(
            Capsule()
    ...
    private func primaryText(in message: RemoteClawChatMessage) -> String {
        ...
        return RemoteClawChatMessage.displayText(
...
private struct ChatAssistantIntroCard: View {
    let text: String
    let prompts: [RemoteClawChatView.StarterPrompt]
    let onPrompt: (RemoteClawChatView.StarterPrompt) -> Void
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            ForEach(self.prompts) { prompt in
                Button {
                    ...
                    HStack(spacing: 8) {
                        Text(prompt.title)
                            .font(RemoteClawChatTypography.body(size: 15, weight: .semibold, relativeTo: .callout))
                            .multilineTextAlignment(.leading)
                        Spacer(minLength: 8)
    ...
    var body: some View {
        ...
        ContentUnavailableView {
            Label(self.title, systemImage: self.systemImage)
                .font(RemoteClawChatTypography.headline)
        } description: {
            Text(self.message)
                .font(RemoteClawChatTypography.body)
        } actions: {
            if let actionTitle, let action {
                Button(action: action) {
                    Text(actionTitle)
                        .font(RemoteClawChatTypography.body(size: 15, weight: .semibold, relativeTo: .subheadline))
                }
