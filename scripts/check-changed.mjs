...
const CANVAS_A2UI_NATIVE_RESOURCE_PATH_RE =
  /^(?:pnpm-lock\.yaml$|apps\/shared\/RemoteClawKit\/Sources\/RemoteClawKit\/Resources\/CanvasA2UI\/|extensions\/canvas\/(?:package\.json$|scripts\/bundle-a2ui\.mjs$|src\/host\/a2ui(?:\/(?:index\.html|a2ui\.bundle\.js|\.bundle\.hash)$|-app\/))|scripts\/(?:bundle-a2ui|sync-native-a2ui)\.mjs$)/u;
const CORE_OXLINT_TS_CONFIG = "tsconfig.oxlint.core.json";
...
export function createChangedCheckChildEnv(baseEnv = process.env) {
  ...
  return {
    ...
    REMOTECLAW_TEST_HEAVY_CHECK_LOCK_HELD: "1",
    REMOTECLAW_TSGO_HEAVY_CHECK_LOCK_HELD: "1",
  };
}
...
export function shouldDelegateChangedCheckToCrabbox(argv = [], env = process.env) {
  if (isTruthyEnvFlag(env.REMOTECLAW_CHECK_CHANGED_REMOTE_CHILD)) {
    return false;
  }
...
export function createChangedCheckPlan(result, options = {}) {
  ...
  if (lanes.liveDockerTooling) {
    ...
    addCommand("live Docker scheduler dry run", "node", ["scripts/test-docker-all.mjs"], {
      ...baseEnv,
      REMOTECLAW_DOCKER_ALL_DRY_RUN: "1",
      REMOTECLAW_DOCKER_ALL_LIVE_MODE: "only",
    });
  }
...
function ensureCorepackPnpmShimDir() {
  ...
  const dir = mkdtempSync(path.join(tmpdir(), "remoteclaw-corepack-pnpm-"));
