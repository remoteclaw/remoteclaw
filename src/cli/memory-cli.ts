...
type MemoryManagerPurpose = string | undefined;
...
function resolveAgentIds(cfg: ReturnType<typeof loadConfig>, agent?: string): string[] {
  ...
  if (list.length > 0) {
    return list.map((entry: { id?: string }) => entry.id).filter((id): id is string => Boolean(id));
  }
  return [resolveDefaultAgentId(cfg)];
}

function formatExtraPaths(workspaceDir: string, extraPaths: string[]): string[] {
  return normalizeExtraMemoryPaths(workspaceDir, extraPaths).map((entry: string) =>
...
async function withMemoryManagerForAgent(params: {
  ...
  await withManager<MemoryManager>({
    getManager: () =>
      getMemorySearchManager(managerParams) as Promise<{
...
async function scanMemoryFiles(
  ...
  let listedOk = false;
  try {
    listed = listMemoryFiles(workspaceDir, resolvedExtraPaths);
    listedOk = true;
  } catch (err) {
    const code = (err as NodeJS.ErrnoException).code;
    if (dirReadable !== null) {
  ...
  for (const agentId of agentIds) {
    ...
    await withMemoryManagerForAgent({
      ...
      run: async (manager) => {
        ...
        let embeddingProbe: Awaited<ReturnType<typeof manager.probeEmbeddingAvailability>>;
              ...
              async (update, progress) => {
                try {
                  await syncFn({
                    ...
                    progress: (syncUpdate: { completed: number; total: number; label: string }) => {
                      update({
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["remoteclaw memory status", "Show index and provider status."],
          ...
          run: async (manager) => {
            try {
              ...
              if (opts.verbose) {
                ...
                const warn = (text: string) => colorize(rich, theme.warn, text);
                const label = (text: string) => muted(`${text}:`);
                const sourceLabels = (status.sources ?? []).map((source: string) =>
