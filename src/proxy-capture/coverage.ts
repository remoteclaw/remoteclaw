...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[remoteclaw proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[remoteclaw proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`remoteclaw proxy coverage\` for details.`,
  );
}
