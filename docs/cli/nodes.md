---
summary: "CLI reference for `remoteclaw nodes` (status, pairing, invoke, camera/canvas/screen/location/notify)"
read_when:
...
remoteclaw nodes status --last-connected 24h
...
These commands drive the gateway-owned `node.pair.*` store, separate from device pairing (`remoteclaw devices approve`) that gates the node's WS `connect` handshake. See [Nodes](/nodes) for how the two relate.
...
```bash
remoteclaw nodes invoke --node <id> --command system.which --params '{"name":"uname"}'
```
...
```bash
remoteclaw nodes notify --node <id> --title "Build" --body "Done" --priority timeSensitive
remoteclaw nodes push --node <id> --title "RemoteClaw" --environment sandbox
remoteclaw nodes location get --node <id> --accuracy precise
remoteclaw nodes screen record --node <id> --duration 10s --fps 10 --out ./clip.mp4
```
...
- `push` sends an APNs test push to an iOS node. Options: `--title <text>` (default `RemoteClaw`), `--body <text>`, `--environment <sandbox|production>` to override the detected APNs environment.
...
Camera and Canvas commands have their own docs: [Camera nodes](/nodes/camera), [Canvas](/platforms/mac/canvas). Canvas is implemented by the bundled experimental Canvas plugin; core keeps `remoteclaw nodes canvas` as a compatibility mount point.
