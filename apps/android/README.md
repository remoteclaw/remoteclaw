## RemoteClaw Android App

RemoteClaw Android is the officially released Google Play app. It connects to an RemoteClaw Gateway as a companion node for chat, voice, approvals, screen, and device-aware automation.
...
```bash
ANDROID_SCREENSHOT_AVD=RemoteClaw_QA_API35 pnpm android:screenshots
```
...
- Play build: `remoteclaw-<version>-play-release.aab`
- Third-party build: `remoteclaw-<version>-third-party-release.apk`

`pnpm android:bundle:release` is an alias for the same Fastlane archive lane.

Regular final and correction RemoteClaw releases publish the signed third-party APK as `RemoteClaw-Android.apk` with a checksum manifest and GitHub Actions provenance. `.github/workflows/android-release.yml` is the only automated GitHub Release upload path; `RemoteClaw Release Publish` dispatches it while the canonical release is still a draft and blocks publication until the uploaded asset contract verifies.

The protected `android-release` environment supplies `MATCH_PASSWORD`; the repository's read-only GitHub App token checks out encrypted material from `remoteclaw/apps-signing`. The workflow builds the exact release tag, refuses to replace different existing bytes, and re-downloads the APK for checksum, certificate, and provenance verification.
...
```bash
pnpm remoteclaw gateway --port 18789 --verbose
```
...
- Canvas web content already supports live reload when loaded from Gateway `__remoteclaw__/canvas/` (see `docs/platforms/android.md`).
...
Current RemoteClaw Android implication:
...
7) Canvas host is enabled and reachable from the device for remote Canvas checks (do not run gateway with `REMOTECLAW_SKIP_CANVAS_HOST=1`; startup logs should include `canvas host mounted at .../__remoteclaw__/`).
...
```bash
remoteclaw devices list
remoteclaw devices approve --latest   # preview only; copy the requestId from output
remoteclaw devices approve <requestId>
```
...
- `pairing required` before tests start:
  - list pending requests (`remoteclaw devices list`), then approve with the exact ID (`remoteclaw devices approve <requestId>`) and rerun.
