...
var RemoteClawA2UIHost = class extends i$6 {
	static properties = {
		surfaces: { state: true },
    ...
    :host {
      ...
      box-sizing: border-box;
      padding:
        var(--remoteclaw-a2ui-inset-top, 0px)
        var(--remoteclaw-a2ui-inset-right, 0px)
        var(--remoteclaw-a2ui-inset-bottom, 0px)
        var(--remoteclaw-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--remoteclaw-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--remoteclaw-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--remoteclaw-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--remoteclaw-a2ui-empty-top, var(--remoteclaw-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  `;
	connectedCallback() {
		...
		for (const eventName of ["remoteclaw:a2ui-action-status"]) globalThis.addEventListener(eventName, this.#statusListener);
		this.#syncSurfaces();
	}
	disconnectedCallback() {
		super.disconnectedCallback();
		if (this.#statusListener) {
			for (const eventName of ["remoteclaw:a2ui-action-status"]) globalThis.removeEventListener(eventName, this.#statusListener);
	...
	#handleA2UIAction(evt) {
		...
		const handler = globalThis.webkit?.messageHandlers?.remoteclawCanvasA2UIAction ?? globalThis.remoteclawCanvasA2UIAction;
		if (handler?.postMessage) try {
			if (handler === globalThis.remoteclawCanvasA2UIAction) handler.postMessage(JSON.stringify({ userAction }));
...
if (!customElements.get("remoteclaw-a2ui-host")) customElements.define("remoteclaw-a2ui-host", RemoteClawA2UIHost);
