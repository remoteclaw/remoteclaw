  ...
  if (typeof window === "undefined") {
    vi.stubGlobal(
      ...
      value == null
        ...
        : ({ __REMOTECLAW_CONTROL_UI_BASE_PATH__: value } as Window & typeof globalThis),
    );
    return;
  }
  if (value == null) {
    delete window.__REMOTECLAW_CONTROL_UI_BASE_PATH__;
    return;
  }
  Object.defineProperty(window, "__REMOTECLAW_CONTROL_UI_BASE_PATH__", {
    value,
    writable: true,
    configurable: true,
  });
...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("uses configured base path and normalizes trailing slash", async () => {
    ...
    setControlUiBasePath(" /remoteclaw/ ");

    const { loadSettings } = await import("../ui/storage.ts");
    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/remoteclaw"));
  });
  ...
  it("infers base path from nested pathname when configured base path is not set", async () => {
    setTestLocation({
      ...
      pathname: "/apps/remoteclaw/chat",
    });

    const { loadSettings } = await import("../ui/storage.ts");
    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/apps/remoteclaw"));
  });
  ...
  it("ignores and scrubs legacy persisted tokens", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("remoteclaw.control.settings.v1") ?? "{}")).toEqual({
      gatewayUrl: "wss://gateway.example:8443/remoteclaw",
      sessionKey: "agent",
  ...
  it("loads the current-tab token from sessionStorage", async () => {
    ...
    const { loadSettings, saveSettings } = await import("../ui/storage.ts");
    saveSettings({
      gatewayUrl: "wss://gateway.example:8443/remoteclaw",
      token: "session-token",
      sessionKey: "main",
