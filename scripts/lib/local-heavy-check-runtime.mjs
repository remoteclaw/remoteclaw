...
export function applyLocalTsgoPolicy(args, env, hostResources) {
  ...
  if (defaultProjectRun) {
    ...
    insertBeforeSeparator(
      nextArgs,
      "--tsBuildInfoFile",
      nextEnv.REMOTECLAW_TSGO_BUILD_INFO_FILE ?? DEFAULT_LOCAL_TSGO_BUILD_INFO_FILE,
    );
  }
  ...
  if (nextEnv.REMOTECLAW_TSGO_PPROF_DIR && !hasFlag(nextArgs, "--pprofDir")) {
    insertBeforeSeparator(nextArgs, "--pprofDir", nextEnv.REMOTECLAW_TSGO_PPROF_DIR);
  }
...
export function shouldAcquireLocalHeavyCheckLockForOxlint(
  ...
  if (env.REMOTECLAW_OXLINT_FORCE_LOCK === "1") {
    return true;
  }
...
export function shouldAcquireLocalHeavyCheckLockForTsgo(args, env = process.env) {
  if (env.REMOTECLAW_TSGO_FORCE_LOCK === "1") {
    return true;
  }
...
export function acquireLocalHeavyCheckLockSync(params) {
  ...
  const locksDir = path.join(commonDir, "remoteclaw-local-checks");
  const lockDir = path.join(locksDir, `${params.lockName ?? "heavy-check"}.lock`);
  const ownerPath = path.join(lockDir, "owner.json");
  const timeoutMs = readPositiveInt(
    env.REMOTECLAW_HEAVY_CHECK_LOCK_TIMEOUT_MS,
    DEFAULT_LOCK_TIMEOUT_MS,
    "REMOTECLAW_HEAVY_CHECK_LOCK_TIMEOUT_MS",
  );
  const pollMs = readPositiveInt(
    env.REMOTECLAW_HEAVY_CHECK_LOCK_POLL_MS,
    DEFAULT_LOCK_POLL_MS,
    "REMOTECLAW_HEAVY_CHECK_LOCK_POLL_MS",
  );
  const progressMs = readPositiveInt(
    env.REMOTECLAW_HEAVY_CHECK_LOCK_PROGRESS_MS,
    DEFAULT_LOCK_PROGRESS_MS,
    "REMOTECLAW_HEAVY_CHECK_LOCK_PROGRESS_MS",
  );
  const staleLockMs = readPositiveInt(
    env.REMOTECLAW_HEAVY_CHECK_STALE_LOCK_MS,
    DEFAULT_STALE_LOCK_MS,
    "REMOTECLAW_HEAVY_CHECK_STALE_LOCK_MS",
  );
...
function readLocalCheckMode(env, defaultMode) {
  const raw = env.REMOTECLAW_LOCAL_CHECK_MODE?.trim().toLowerCase();
