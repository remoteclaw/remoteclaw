...
describe("gateway session utils", () => {
  ...
  test("resolveSessionStoreKey falls back to main when agents.list is missing", () => {
    // Post-#1581, resolveMainSessionKey hard-fails when agents.list is empty
    // ("no agents configured"). Seed a minimal single-agent list so the
    // main-alias path resolves; the test still asserts the "main" agent / mainKey
    // fallback behavior. resolveSessionStoreKey takes cfg directly (no schema
    // validation), so no workspace field is required.
    const cfg = {
      session: { mainKey: "work" },
      agents: { list: [{ id: "main" }] },
    } as RemoteClawConfig;
