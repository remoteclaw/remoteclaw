...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.REMOTECLAW_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "RemoteClaw";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(RemoteClaw\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw = process.env.REMOTECLAW_MDNS_HOSTNAME?.trim() || "remoteclaw";
    const hostname =
      hostnameRaw
        .replace(/\.local$/i, "")
        .split(".")[0]
