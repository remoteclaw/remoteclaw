...
source_repo="${SOURCE_REPO:-${GITHUB_REPOSITORY:-remoteclaw/remoteclaw}}"
...
pack_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/remoteclaw-clawhub-pack.XXXXXX")"
cleanup() {
  rm -rf "${pack_dir}"
}
trap cleanup EXIT
...
build_package_runtime() {
  if [[ "${REMOTECLAW_PLUGIN_NPM_RUNTIME_BUILD:-1}" == "0" || "${REMOTECLAW_PLUGIN_NPM_RUNTIME_BUILD:-1}" == "false" ]]; then
...
for attempt in $(seq 1 "${REMOTECLAW_CLAWHUB_PUBLISH_ATTEMPTS:-8}"); do
  ...
  echo "ClawHub publish hit a rate limit; retrying (${attempt}/${REMOTECLAW_CLAWHUB_PUBLISH_ATTEMPTS:-8})." >&2
  sleep "${REMOTECLAW_CLAWHUB_PUBLISH_RETRY_DELAY_SECONDS:-60}"
done

echo "ClawHub publish failed after ${REMOTECLAW_CLAWHUB_PUBLISH_ATTEMPTS:-8} attempts." >&2
exit 1
