...
If you believe you've found a security issue in RemoteClaw, please report it privately.

## Reporting

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [remoteclaw/remoteclaw](https://github.com/remoteclaw/remoteclaw)
...
- Tested version details (RemoteClaw version and/or commit SHA).
- Reproducible PoC against latest `main` or latest released version.
- Demonstrated impact tied to RemoteClaw's documented trust boundaries.
- For exposed-secret reports: proof the credential is RemoteClaw-owned (or grants access to RemoteClaw-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at RemoteClaw. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

RemoteClaw is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
RemoteClaw does **not** model one gateway as a multi-tenant, adversarial user boundary.

- Authenticated Gateway callers are treated as trusted operators for that gateway instance.
- Session identifiers (`sessionKey`, session IDs, labels) are routing controls, not per-user authorization boundaries.
- If one operator can view data from another operator on the same gateway, that is expected in this trust model.
- RemoteClaw can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need RemoteClaw, use one VPS (or host/OS user boundary) per user.
...
- This is expected in RemoteClaw's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of RemoteClaw's trusted computing base for a gateway.
...
- Using RemoteClaw in ways that the docs recommend not to
...
RemoteClaw security guidance assumes:

- The host where RemoteClaw runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.remoteclaw` state/config (including `remoteclaw.json`) is effectively a trusted operator.
...
RemoteClaw's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
RemoteClaw separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when RemoteClaw can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the RemoteClaw process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

RemoteClaw uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/remoteclaw` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/remoteclaw` (or `remoteclaw-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the RemoteClaw-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use RemoteClaw temp helpers (`resolvePreferredRemoteClawTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
...
- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  ...
  - CLI: `remoteclaw gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - RemoteClaw keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `remoteclaw security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__remoteclaw__/canvas/`, `/__remoteclaw__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__remoteclaw__/canvas/`, `/__remoteclaw__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

RemoteClaw requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running RemoteClaw in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v remoteclaw-data:/app/data \
  remoteclaw/remoteclaw:latest
```
