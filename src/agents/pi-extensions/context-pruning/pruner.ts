...
function estimateMessageChars(message: AgentMessage): number {
  ...
  if (message.role === "assistant") {
    ...
    for (const b of message.content) {
      ...
      if (
        (b.type as string) === "thinking" &&
        typeof (b as unknown as Record<string, unknown>).thinking === "string"
      ) {
        chars += ((b as unknown as Record<string, unknown>).thinking as string).length;
      }
      if (b.type === "toolCall") {
        try {
