...
// Hugging Face local-app CLI contract tests cover RemoteClaw's snippet-facing commands.
import { createServer, type IncomingMessage, type ServerResponse } from "node:http";
import { afterEach, describe, expect, it } from "vitest";
import {
  createRemoteClawTestInstance,
  type RemoteClawTestInstance,
} from "./helpers/remoteclaw-test-instance.js";
...
const instances: RemoteClawTestInstance[] = [];
...
describe("Hugging Face RemoteClaw local-app CLI contract", () => {
  it.each(HF_LOCAL_APP_CASES)(
    ...
    async ({ providerId, modelId }) => {
      ...
      const instance = await createRemoteClawTestInstance({
...
async function handleFakeOpenAiRequest(
  ...
  writeSse(res, {
    id: "chatcmpl-hf-remoteclaw-contract",
