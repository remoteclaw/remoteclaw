...
describe("connectGateway", () => {
  ...
  it("routes exec approval requested events into the queue", () => {
    ...
    expect(host.execApprovalQueue[0]?.request.command).toBe(
      'ls | grep "stuff" | python -c \'print("hi")\'',
    );
  });

  it("removes resolved approvals from the exec approval queue", () => {
    const { host, client } = connectHostGateway();
  ...
  it("logs and drops stale queued chat abort failures after reconnect", async () => {
    ...
    expect(host.pendingAbort).toBeNull();
    expect(warn).toHaveBeenCalledWith("[remoteclaw] pending abort failed:", error);
    warn.mockRestore();
  });
