...
# Build with: docker build --build-arg REMOTECLAW_INSTALL_DOCKER_CLI=1 ...
# Adds ~50MB. Only the CLI is installed — no Docker daemon.
# Required for agents.defaults.sandbox to function in Docker deployments.
ARG REMOTECLAW_INSTALL_DOCKER_CLI=""
ARG REMOTECLAW_DOCKER_GPG_FINGERPRINT="9DC858229FC7DD38854AE2D88D81803C0EBFCD88"
RUN if [ -n "$REMOTECLAW_INSTALL_DOCKER_CLI" ]; then \
      apt-get update && \
