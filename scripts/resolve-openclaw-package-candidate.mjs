...
export const REMOTECLAW_PACKAGE_SPEC_RE =
  /^remoteclaw@(beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-beta\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-remoteclaw-package-candidate.mjs --source <ref|npm|url|artifact> --output-dir <dir> [options]
...
export function validateRemoteClawPackageSpec(spec) {
  if (!REMOTECLAW_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be remoteclaw@beta, remoteclaw@latest, or an exact RemoteClaw release version; got: ${spec}`,
    );
  }
}
...
async function resolveCandidate(options) {
  ...
  try {
    if (options.source === "ref") {
      ...
      validateRemoteClawPackageSpec(options.packageSpec);
