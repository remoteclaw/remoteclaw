...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-remoteclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-remoteclaw#general");
  });
  ...
  it.skip("loadSessionStore auto-migrates legacy provider keys to channel keys", async () => {
    const mainSessionKey = "agent:main:main";
