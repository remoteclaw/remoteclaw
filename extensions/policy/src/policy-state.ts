...
import { coerceSecretRef } from "remoteclaw/plugin-sdk/secret-ref-runtime";
import {
  ...
  return Object.entries(configuredChannels(cfg))
    ...
    .map(([id, value]) => {
      const entry: {
        ...
        id,
        provider: id,
        source: `oc://remoteclaw.config/channels/${id}`,
      };
  ...
  return Object.entries(configuredMcpServers(cfg))
    ...
    .map(([id, value]) => {
      const entry: {
        ...
        source: `oc://remoteclaw.config/mcp/servers/${ocPathSegment(id)}`,
      };
      if (isRecord(value)) {
...
export function scanPolicyModelProviders(
  ...
  return Object.keys(configuredModelProviders(cfg))
    ...
    .map((id) => ({
      ...
      source: `oc://remoteclaw.config/models/providers/${id}`,
    }));
}
...
export function scanPolicyModelRefs(
  ...
  if (isRecord(cfg.agents)) {
    collectModelRefsFromRecord(refs, cfg.agents, "oc://remoteclaw.config/agents");
...
export function scanPolicyNetwork(cfg: Record<string, unknown>): readonly PolicyNetworkEvidence[] {
  return [
    networkBooleanEvidence(
      ...
      "oc://remoteclaw.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "browser-private-network-legacy",
      ["browser", "ssrfPolicy", "allowPrivateNetwork"],
      "oc://remoteclaw.config/browser/ssrfPolicy/allowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-private-network",
      ["tools", "web", "fetch", "ssrfPolicy", "dangerouslyAllowPrivateNetwork"],
      "oc://remoteclaw.config/tools/web/fetch/ssrfPolicy/dangerouslyAllowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-private-network-legacy",
      ["tools", "web", "fetch", "ssrfPolicy", "allowPrivateNetwork"],
      "oc://remoteclaw.config/tools/web/fetch/ssrfPolicy/allowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-rfc2544-benchmark-range",
      ["tools", "web", "fetch", "ssrfPolicy", "allowRfc2544BenchmarkRange"],
      "oc://remoteclaw.config/tools/web/fetch/ssrfPolicy/allowRfc2544BenchmarkRange",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-ipv6-unique-local-range",
      ["tools", "web", "fetch", "ssrfPolicy", "allowIpv6UniqueLocalRange"],
      "oc://remoteclaw.config/tools/web/fetch/ssrfPolicy/allowIpv6UniqueLocalRange",
    ),
...
export function scanPolicyGatewayExposure(
  ...
  if (bind === "custom" && hasCustomBindHost) {
    entries.push({
      ...
      kind: "bind",
      source: "oc://remoteclaw.config/gateway/customBindHost",
  ...
  entries.push({
    ...
    kind: "auth",
    source: "oc://remoteclaw.config/gateway/auth/mode",
  ...
  entries.push({
    ...
    kind: "authRateLimit",
    source: "oc://remoteclaw.config/gateway/auth/rateLimit",
  ...
  pushGatewayBooleanEvidence(
    ...
    "controlUi",
    controlUi.enabled,
    "oc://remoteclaw.config/gateway/controlUi/enabled",
  );
  pushGatewayBooleanEvidence(
    entries,
    ...
    "oc://remoteclaw.config/gateway/controlUi/allowInsecureAuth",
  );
  pushGatewayBooleanEvidence(
    entries,
    ...
    "oc://remoteclaw.config/gateway/controlUi/dangerouslyDisableDeviceAuth",
  );
  pushGatewayBooleanEvidence(
    entries,
    ...
    "oc://remoteclaw.config/gateway/controlUi/dangerouslyAllowHostHeaderOriginFallback",
  );
  ...
  if (typeof tailscale.mode === "string") {
    entries.push({
      ...
      kind: "tailscale",
      source: "oc://remoteclaw.config/gateway/tailscale/mode",
  ...
  if (tailscale.mode === "serve" && tailscale.preserveFunnel === true) {
    entries.push({
      ...
      kind: "tailscale",
      source: "oc://remoteclaw.config/gateway/tailscale/preserveFunnel",
      value: "funnel",
    });
  }
  ...
  if (gateway.mode === "remote") {
    entries.push({
      ...
      kind: "remote",
      source: "oc://remoteclaw.config/gateway/mode",
      value: "remote",
    });
    if (typeof remote.url === "string" && remote.url.trim() !== "") {
      entries.push({
        ...
        kind: "remote",
        source: "oc://remoteclaw.config/gateway/remote/url",
        value: true,
      });
    }
  }
...
export function scanPolicyAgentWorkspace(
  ...
  pushAgentWorkspaceEvidence(entries, {
    ...
    tools: defaultTools,
    inheritedTools: {},
    workspaceSourceBase: "oc://remoteclaw.config/agents/defaults",
    inheritedWorkspaceSourceBase: "oc://remoteclaw.config/agents/defaults",
    toolsSourceBase: "oc://remoteclaw.config/tools",
    inheritedToolsSourceBase: "oc://remoteclaw.config/tools",
  });
  ...
  list.forEach((agent, index) => {
    ...
    pushAgentWorkspaceEvidence(entries, {
      ...
      workspaceSourceBase: `oc://remoteclaw.config/agents/list/#${index}`,
      inheritedWorkspaceSourceBase: "oc://remoteclaw.config/agents/defaults",
      toolsSourceBase: `oc://remoteclaw.config/agents/list/#${index}/tools`,
      inheritedToolsSourceBase: "oc://remoteclaw.config/tools",
    });
  });
...
export function scanPolicyToolPosture(
  ...
  pushToolPostureEvidence(entries, {
    ...
    sandbox: defaultSandbox,
    inheritedSandbox: {},
    sourceBase: "oc://remoteclaw.config/tools",
    inheritedSourceBase: "oc://remoteclaw.config/tools",
  });
  ...
  list.forEach((agent, index) => {
    ...
    pushToolPostureEvidence(entries, {
      ...
      inheritedSandbox: defaultSandbox,
      sourceBase: `oc://remoteclaw.config/agents/list/#${index}/tools`,
      inheritedSourceBase: "oc://remoteclaw.config/tools",
    });
  });
  ...
  return Object.entries(profiles)
    ...
    .map(([id, value]) => {
      const entry: {
        ...
        id,
        source: `oc://remoteclaw.config/auth/profiles/${ocPathSegment(id)}`,
...
function scanPolicySecretProviders(cfg: Record<string, unknown>): readonly PolicySecretEvidence[] {
  ...
  return Object.entries(providers).map(([id, value]) => {
    ...
    const entry: {
      ...
      id,
      kind: "provider",
      source: `oc://remoteclaw.config/secrets/providers/${ocPathSegment(id)}`,
    };
...
function configPathSource(path: readonly string[]): string {
  return `oc://remoteclaw.config/${path.map(ocPathSegment).join("/")}`;
}
  ...
  const sandboxModeSource =
    explicitSandboxMode !== undefined
      ...
      : inheritedSandboxMode !== undefined
        ...
        : "oc://remoteclaw.config/agents/defaults/sandbox/mode";
    ...
    source:
      explicitWorkspaceAccess !== undefined
        ...
        : inheritedWorkspaceAccess !== undefined
          ...
          : "oc://remoteclaw.config/agents/defaults/sandbox/workspaceAccess",
    scope: params.scope,
...
function pushGatewayHttpEndpointEvidence(
  ...
  const source = `oc://remoteclaw.config/gateway/http/endpoints/${endpoint}`;
...
function collectModelRefsFromAgentAllowlist(
  ...
  if (isRecord(defaults) && isRecord(defaults.models)) {
    collectModelRefsFromModelMap(
      refs,
      defaults.models,
      "oc://remoteclaw.config/agents/defaults/models",
    );
  }
  ...
  for (const [index, agent] of list.entries()) {
    ...
    collectModelRefsFromModelMap(
      refs,
      agent.models,
      `oc://remoteclaw.config/agents/list/#${index}/models`,
    );
  }
}
