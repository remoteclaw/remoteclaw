...
describe("Control UI Vite config", () => {
  it("resolves root tsconfig package aliases for source imports", () => {
    expect(findStringAlias("@remoteclaw/net-policy/ip")?.replacement).toBe(
  ...
  it("resolves Control UI dev-server source aliases for internal packages", () => {
    ...
    expect(
      aliases.find((alias) => alias.find === "@remoteclaw/normalization-core/string-coerce"),
    )?.toEqual({
  ...
  it("keeps specific tsconfig aliases ahead of broad package aliases", () => {
    ...
    const netPolicyIpIndex = aliases.findIndex(
      (alias) => alias.find === "@remoteclaw/net-policy/ip",
    );
    const netPolicyPackageIndex = aliases.findIndex(
      (alias) => alias.find === "@remoteclaw/net-policy",
    );
    ...
    const broadRemoteClawWildcardIndex = aliases.findIndex(
    ...
    expect(broadRemoteClawWildcardIndex).toBeGreaterThanOrEqual(0);
    expect(netPolicyIpIndex).toBeLessThan(netPolicyPackageIndex);
    expect(netPolicyWildcardIndex).toBeLessThan(broadRemoteClawWildcardIndex);
  });
