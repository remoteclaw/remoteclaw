...
private let appDefaultsSuites = ["org.remoteclaw.mac", "org.remoteclaw.mac.debug"]
...
func runConfigureRemote(_ args: [String]) {
    do {
        ...
        if opts.help {
            print("""
            remoteclaw-mac configure-remote

            Usage:
              remoteclaw-mac configure-remote --ssh-target <user@host[:port]> [--local-port <port>]
              ...
              remoteclaw-mac configure-remote --direct-url <ws://host:port|wss://host> [--token <token>]
            ...
            Options:
              ...
              --project-root <p>  Remote RemoteClaw checkout for CLI commands.
              --cli-path <path>   Remote remoteclaw executable or entrypoint.
              --json              Emit JSON.
              -h, --help          Show help.
            """)
            return
    ...
    defaultsSuites: [String]) throws -> ConfigureRemoteOutput
{
    ...
    let configURL = remoteClawConfigURL()
...
private func remoteClawConfigURL() -> URL {
    if let raw = ProcessInfo.processInfo.environment["REMOTECLAW_CONFIG_PATH"],
       !raw.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    {
        return URL(fileURLWithPath: NSString(string: raw).expandingTildeInPath)
    }
    return FileManager().homeDirectoryForCurrentUser.appendingPathComponent(".remoteclaw/remoteclaw.json")
}
...
private func writeAppDefaults(opts: ConfigureRemoteOptions, target: String, suites: [String]) {
    for suite in suites {
        ...
        defaults.set("remote", forKey: "remoteclaw.connectionMode")
        setDefaultString(defaults, key: "remoteclaw.remoteTarget", value: target)
        defaults.set(true, forKey: "remoteclaw.onboardingSeen")
        defaults.set(appOnboardingVersion, forKey: "remoteclaw.onboardingVersion")
        setDefaultStringIfProvided(defaults, key: "remoteclaw.remoteIdentity", value: opts.identity)
        setDefaultStringIfProvided(defaults, key: "remoteclaw.remoteProjectRoot", value: opts.projectRoot)
        setDefaultStringIfProvided(defaults, key: "remoteclaw.remoteCliPath", value: opts.cliPath)
...
private func printConfigureRemoteOutput(_ output: ConfigureRemoteOutput, json: Bool) {
    ...
    print("RemoteClaw macOS Remote Config")
