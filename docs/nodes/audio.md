...
- **Media understanding (audio)**: If audio understanding is enabled (or auto-detected), RemoteClaw:
...
RemoteClaw auto-detects in this order and stops at the first working option:
...
- CLI `args` should use `{{MediaPath}}` for the local audio file path. Run `remoteclaw doctor --fix` to migrate deprecated `{input}` placeholders from older `audio.transcription.command` configs.
...
When `requireMention: true` is set for a group chat, RemoteClaw now transcribes audio **before** checking for mentions. This allows voice notes to be processed even when they contain mentions.

**How it works:**

1. If a voice message has no text body and the group requires mentions, RemoteClaw performs a "preflight" transcription.
