...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.REMOTECLAW_DIAGNOSTICS)) &&
    typeof env.REMOTECLAW_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.REMOTECLAW_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  const normalized = {
    schemaVersion: REMOTECLAW_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.REMOTECLAW_DIAGNOSTICS_RUN_ID ? { runId: env.REMOTECLAW_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.REMOTECLAW_DIAGNOSTICS_ENV ? { envName: env.REMOTECLAW_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.REMOTECLAW_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }
