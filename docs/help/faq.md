...
<AccordionGroup>
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    - If a direct edit breaks startup or hot reload, the Gateway restores the last-known-good config and saves the rejected file as `remoteclaw.json.clobbered.*`.
    - The main agent receives a boot warning after recovery so it does not blindly write the bad config again.

    Recover:

    - Check `remoteclaw logs --follow` for `Config auto-restored from last-known-good`, `Config write rejected:`, or `config reload restored last-known-good config`.
    - Inspect the newest `remoteclaw.json.clobbered.*` or `remoteclaw.json.rejected.*` beside the active config.
    - Keep the active restored config if it works, then copy only the intended keys back with `remoteclaw config set` or `config.patch`.
    - Run `remoteclaw config validate` and `remoteclaw doctor`.
