...
const nixEnv: NodeJS.ProcessEnv = { REMOTECLAW_NIX_MODE: "1" };
...
describe("plugin hardlink policy", () => {
  ...
  it("does not treat REMOTECLAW_NIX_MODE as enough by itself", () => {
    expect(
      shouldRejectHardlinkedPluginFiles({
        origin: "config",
        rootDir: "/tmp/plugin",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      expect(isNixStorePluginRoot("/nix/store/abc-remoteclaw-plugin")).toBe(true);
      expect(isNixStorePluginRoot("/tmp/nix/store/abc-remoteclaw-plugin")).toBe(false);
      expect(
        shouldRejectHardlinkedPluginFiles({
          origin: "config",
          rootDir: "/nix/store/abc-remoteclaw-plugin",
          env: nixEnv,
        }),
      ).toBe(false);
    },
