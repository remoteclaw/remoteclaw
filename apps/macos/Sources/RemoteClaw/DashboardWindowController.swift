...
final class DashboardWindowController: NSWindowController, WKNavigationDelegate, NSWindowDelegate {
    ...
    private static func installNativeChromeScript(into userContentController: WKUserContentController) {
        let css = """
        html.remoteclaw-native-macos {
          --remoteclaw-native-titlebar-height: 50px;
        }
        @media (min-width: 700px) {
          html.remoteclaw-native-macos .sidebar-shell {
            padding-top: max(14px, var(--remoteclaw-native-titlebar-height)) !important;
          }
          html.remoteclaw-native-macos .sidebar-shell__header {
        ...
        (() => {
          try {
            if (document.getElementById("remoteclaw-native-macos-chrome")) return;
            const style = document.createElement("style");
            style.id = "remoteclaw-native-macos-chrome";
            style.textContent = \(Self.jsStringLiteral(css));
            document.documentElement.classList.add("remoteclaw-native-macos");
        ...
        auth: DashboardWindowAuth)
    {
        ...
        (() => {
          try {
            ...
            Object.defineProperty(window, "__REMOTECLAW_NATIVE_CONTROL_AUTH__", {
              value: \(json),
              configurable: true,
            });
          } catch {}
