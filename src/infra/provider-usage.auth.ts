...
async function resolveOAuthToken(params: {
  ...
  for (const profileId of deduped) {
    ...
    try {
      ...
      if (resolved) {
        const resolvedAuth = resolved as { apiKey?: string };
        let token = resolvedAuth.apiKey;
        if (params.provider === "google-gemini-cli") {
          const parsed = parseGoogleToken(resolvedAuth.apiKey ?? "");
          token = parsed?.token ?? resolvedAuth.apiKey;
        }
        return {
          provider: params.provider,
          token: token ?? "",
