...
- `gateway.auth.mode: "trusted-proxy"`: delegate browser/user auth to an identity-aware reverse proxy and trust identity headers from `gateway.trustedProxies` (see [Trusted Proxy Auth](/gateway/trusted-proxy-auth)). This mode requires a **non-loopback** proxy source: trusted-proxy auth rejects loopback and host-interface sources unconditionally, so a same-host reverse proxy cannot satisfy it and there is no opt-in override. Internal same-host callers can use `gateway.auth.password` as a local direct fallback; `gateway.auth.token` remains mutually exclusive with trusted-proxy mode.
