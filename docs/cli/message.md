...
`remoteclaw message` resolves channel SecretRefs before running the action,
...
```bash
remoteclaw message broadcast --targets <target...> [--channel all] [--message <text>] [--media <url>] [--dry-run]
```
