...
- `hooks.token` reuses an active Gateway shared-secret auth value (`gateway.auth.token` / `REMOTECLAW_GATEWAY_TOKEN` or `gateway.auth.password` / `REMOTECLAW_GATEWAY_PASSWORD`)
