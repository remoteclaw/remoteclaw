...
describe("cdp.helpers", () => {
  ...
  it("adds relay header for known relay ports", async () => {
    ...
    process.env.REMOTECLAW_GATEWAY_TOKEN = "test-gateway-token";
    try {
      ...
      if (prev === undefined) {
        ...
        process.env.REMOTECLAW_GATEWAY_TOKEN = prev;
      }
    }
  });
});
...
describe("browser server-context listKnownProfileNames", () => {
  it("includes configured and runtime-only profile names", () => {
    ...
    if (!remoteclaw) {
      throw new Error("expected remoteclaw profile");
    }
    ...
    const state: BrowserServerState = {
      ...
      profiles: new Map([
        [
          "stale-removed",
          {
            profile: { ...remoteclaw, name: "stale-removed" },
            running: null,
          },
        ],
      ]),
