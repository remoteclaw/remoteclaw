...
describe("RemoteClawStdioClientTransport", () => {
  afterEach(() => {
    vi.useRealTimers();
    spawnMock.mockReset();
  ...
  it("starts stdio MCP servers in a disposable process group on POSIX", async () => {
    // Detached POSIX process groups let RemoteClaw clean up child tool servers
  ...
  it("reports stderr pipe errors without an unhandled error crash", async () => {
    ...
    const transport = new RemoteClawStdioClientTransport({ command: "npx", stderr: "pipe" });
    const onerror = vi.fn();
