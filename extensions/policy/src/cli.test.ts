...
import { clearConfigCache } from "remoteclaw/plugin-sdk/runtime-config-snapshot";
...
describe("policy commands", () => {
  ...
  it("links policy findings to evidence and policy requirement refs", async () => {
    ...
    vi.stubEnv("REMOTECLAW_CONFIG_PATH", configPath);
    await fs.writeFile(
      configPath,
      JSON.stringify({
        plugins: {
  ...
  it("fails closed when the RemoteClaw config is invalid", async () => {
    const configPath = join(workspaceDir, "remoteclaw.jsonc");
    vi.stubEnv("REMOTECLAW_CONFIG_PATH", configPath);
