...
struct ChatMathBlockView: View {
    ...
    @ScaledMetric(relativeTo: .body) private var fontSize: CGFloat = RemoteClawChatTypography.bodySize
...
struct ChatMarkdownTableView: View {
    ...
    private func cell(_ text: String, column: Int, isHeader: Bool) -> some View {
        Text(self.inlineMarkdown(text))
            .font(isHeader ? RemoteClawChatTypography.footnoteSemiBold : RemoteClawChatTypography.footnote)
            .foregroundStyle(RemoteClawChatTheme.assistantText)
            .textSelection(.enabled)
    }
