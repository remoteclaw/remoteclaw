...
This is the deterministic model-bound layer stack RemoteClaw can snapshot for the Codex happy path. It uses a pinned Codex `gpt-5.5` prompt fixture generated from Codex's model catalog/cache shape, then adds the Codex permission developer text, simulated RemoteClaw workspace bootstrap config instructions, RemoteClaw developer instructions, turn-scoped collaboration-mode instructions when RemoteClaw provides them, turn input, and the RemoteClaw dynamic tool catalog. Codex can still add runtime-owned context such as native workspace `AGENTS.md`, environment context, memories, app/plugin instructions, and built-in collaboration-mode instructions inside the Codex runtime.
...
### User: Codex Config Instructions (RemoteClaw Workspace Bootstrap Context)

```text
RemoteClaw loaded these user-editable workspace files. Treat them as project/user context. Codex loads AGENTS.md natively, so AGENTS.md is not repeated here.
...
````text
You are running inside RemoteClaw. Use RemoteClaw dynamic tools for RemoteClaw-specific integrations such as messaging, cron, sessions, media, gateway, and nodes when available.

Preserve the user's existing channel/session context. If sending a channel reply, use the RemoteClaw messaging tool instead of describing that you would reply.
...
```json
{
  "schema": "remoteclaw.inbound_meta.v2",
  "account_id": "primary",
  "channel": "telegram",
  "provider": "telegram",
  "surface": "telegram",
...
```text
This is an RemoteClaw heartbeat turn. Apply these instructions only to this heartbeat wake; ordinary chat turns should stay in Codex Default mode.
