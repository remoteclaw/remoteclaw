...
2. Verify the read, watch, send, and RPC surfaces before changing RemoteClaw config:
   ...
   Replace `42` with a real chat id from `imsg chats`. Sending requires Automation permission for Messages.app. If RemoteClaw will run through SSH, run these commands through the same SSH wrapper or user context that RemoteClaw will use.
...
The reply cache lives at `~/.remoteclaw/state/imessage/reply-cache.jsonl` (mode `0600`, parent dir `0700`). It is safe to delete if you want a clean slate.
