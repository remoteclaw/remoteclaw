...
const DEFAULT_CLI_NAME = "remoteclaw";

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(remoteclaw)\b/;

/** Resolve the displayed CLI binary name from argv, falling back to `remoteclaw`. */
...
/** Replace a leading `remoteclaw` command prefix with the active CLI name. */
