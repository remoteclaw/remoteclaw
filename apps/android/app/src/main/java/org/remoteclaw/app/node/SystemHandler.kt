...
import org.remoteclaw.app.mainActivityPendingIntent
...
private const val NOTIFICATION_CHANNEL_BASE_ID = "remoteclaw.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "RemoteClaw Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "RemoteClaw Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "RemoteClaw Active")
      }
