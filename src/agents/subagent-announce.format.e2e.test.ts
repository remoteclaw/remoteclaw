...
describe("subagent announce formatting", () => {
  ...
  it("retries reading subagent output when early lifecycle completion had no text", async () => {
    ...
    // readLatestAssistantReply is called at least twice: once initially (returns undefined),
    // then again inside readLatestAssistantReplyWithRetry (returns text).
    expect(readLatestAssistantReplyMock.mock.calls.length).toBeGreaterThanOrEqual(2);
    const call = agentSpy.mock.calls[0]?.[0] as { params?: { message?: string } };
