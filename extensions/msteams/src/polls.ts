...
import { isRecord, normalizeOptionalString } from "remoteclaw/plugin-sdk/string-coerce-runtime";
...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["remoteclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["remoteclaw", "pollId"]) ??
    readNestedString(value, ["remoteclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "remoteclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "remoteclaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          remoteclawPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "remoteclaw poll vote",
          displayText: "Vote recorded",
          value: { remoteclawPollId: pollId, pollId },
        },
      },
    ],
  };
