...
function parsePluginSdkSubpath(specifier) {
  if (!specifier.startsWith("remoteclaw/plugin-sdk/")) {
    return null;
  }
  const subpath = specifier.slice("remoteclaw/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced remoteclaw/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced remoteclaw/plugin-sdk/<subpath> must exist in the public package exports.",
  );
