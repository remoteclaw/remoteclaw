  ...
  try {
    ...
    if (tokenRef) {
      ...
      throw new SecretRefUnavailableError(
        `extension relay requires a resolved gateway token, but gateway.auth.token SecretRef is unavailable (${refLabel}). Set REMOTECLAW_GATEWAY_TOKEN or resolve your secret provider.`,
      );
    }
...
export async function probeAuthenticatedRemoteClawRelay(params: {
  ...
  try {
    ...
    return browserName === REMOTECLAW_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
