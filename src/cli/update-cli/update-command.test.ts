...
describe("resolveGatewayInstallEntrypointCandidates", () => {
  it("prefers index.js before legacy entry.js", () => {
    expect(resolveGatewayInstallEntrypointCandidates("/tmp/remoteclaw-root")).toEqual([
      path.join("/tmp/remoteclaw-root", "dist", "index.js"),
      path.join("/tmp/remoteclaw-root", "dist", "index.mjs"),
      path.join("/tmp/remoteclaw-root", "dist", "entry.js"),
      path.join("/tmp/remoteclaw-root", "dist", "entry.mjs"),
    ]);
  });
});
...
describe("resolveGatewayInstallEntrypoint", () => {
  it("prefers dist/index.js over dist/entry.js when both exist", async () => {
    const root = "/tmp/remoteclaw-root";
