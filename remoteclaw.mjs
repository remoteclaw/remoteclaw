...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `remoteclaw: Node.js v${MIN_NODE_VERSION}+ is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["remoteclaw: missing dist/entry.(m)js (build output)."];
  ...
  lines.push("For releases, use `npm install -g remoteclaw@latest`.");
  return lines.join("\n");
};
