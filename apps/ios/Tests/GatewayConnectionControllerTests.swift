...
@Suite(.serialized) struct GatewayConnectionControllerTests {
    ...
    @Test @MainActor func loadLastConnectionReadsSavedValues() {
        let prior = KeychainStore.loadString(service: "org.remoteclaw.app.gateway", account: "lastConnection")
        defer {
            if let prior {
                _ = KeychainStore.saveString(
                    prior,
                    service: "org.remoteclaw.app.gateway",
                    account: "lastConnection")
            } else {
                _ = KeychainStore.delete(service: "org.remoteclaw.app.gateway", account: "lastConnection")
            }
        }
        _ = KeychainStore.delete(service: "org.remoteclaw.app.gateway", account: "lastConnection")
