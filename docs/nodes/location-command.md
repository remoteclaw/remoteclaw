...
- `location.get` is a node command, invoked via `node.invoke` or `remoteclaw nodes location get`.
- Off by default.
...
- CLI: `remoteclaw nodes location get --node <id>`.
- Agent guidelines: only call when the user enabled location and understands the scope.

## UX copy (suggested)

- Off: "Location sharing is disabled."
- While Using: "Only when RemoteClaw is open."
- Always: "Allow requested location checks while RemoteClaw is in the background."
