...
- `remoteclaw gateway status` shows `Runtime: running`, `Connectivity probe: ok`, and a `Capability: ...` line.
- `remoteclaw doctor` reports no blocking config/service issues.
- `remoteclaw channels status --probe` shows live per-account transport status and, where supported, probe/audit results such as `works` or `audit ok`.
...
- `Update restart` in `remoteclaw status` / `remoteclaw status --all`. Pending or
  failed handoffs include the next command to run.
- `plugin load failed: dependency tree corrupted; run remoteclaw doctor --fix`
  under Channels. That means the channel config still exists, but plugin
  registration failed before the channel could load.
- provider 401s after re-auth. `remoteclaw doctor --fix` checks for stale
...
Use this when a gateway service unexpectedly stops after an update, or logs show that one `remoteclaw` binary is older than the version that last wrote `remoteclaw.json`.

RemoteClaw stamps config writes with `meta.lastTouchedVersion`. Read-only commands can still inspect a config written by a newer RemoteClaw, but process and service mutations refuse to continue from an older binary. Blocked actions include gateway service start, stop, restart, uninstall, forced service reinstall, service-mode gateway startup, and `gateway --force` port cleanup.

```bash
which remoteclaw
remoteclaw --version
remoteclaw gateway status --deep
remoteclaw config get meta.lastTouchedVersion
```

<Steps>
  <Step title="Fix PATH">
    Fix `PATH` so `remoteclaw` resolves to the newer install, then rerun the action.
  </Step>
  ...
  <Step title="Remove stale wrappers">
    Remove stale system package or old wrapper entries that still point at an old `remoteclaw` binary.
  </Step>
</Steps>

<Warning>
For intentional downgrade or emergency recovery only, set `REMOTECLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1` for the single command. Leave it unset for normal operation.
</Warning>

## Protocol mismatch after rollback

Use this when logs keep printing `protocol mismatch` after you downgrade or roll back RemoteClaw. This means an older Gateway is running, but a newer local client process is still trying to reconnect with a protocol range that the older Gateway cannot speak.
...
- `Established clients:` in `remoteclaw gateway status --deep` or `Gateway clients` in `remoteclaw doctor --deep`. This lists active TCP clients connected to the Gateway port, including PIDs and command lines when the OS allows it.
- A client process whose command line points at the newer RemoteClaw install or wrapper you rolled back from.

Fix:

1. Stop or restart the stale RemoteClaw client process shown by `gateway status --deep`.
2. Restart apps or wrappers that embed RemoteClaw, such as local dashboards, editors, app-server helpers, or long-running `remoteclaw logs --follow` shells.
3. Re-run `remoteclaw gateway status --deep` or `remoteclaw doctor --deep` and confirm the stale client PID is gone.
...
RemoteClaw treats every skill root as a containment boundary. A symlink under
`~/.agents/skills`, `<workspace>/.agents/skills`, `<workspace>/skills`, or
`~/.remoteclaw/skills` is skipped when its real target resolves outside that root
...
remoteclaw config get skills.load
```
...
Do not assume this is always an RemoteClaw configuration issue. The response can
...
block. Prefer a narrowly scoped allow or skip rule for the API path RemoteClaw
...
- RemoteClaw model runs fail only on normal agent turns
...
remoteclaw infer model run --model <provider/model> --prompt "hi" --json
remoteclaw logs --follow
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `incomplete turn detected ... stopReason=stop payloads=0` → the backend completed the Chat Completions request but returned no user-visible assistant text for that turn. RemoteClaw retries replay-safe empty OpenAI-compatible turns once; persistent failures usually mean the backend is emitting empty/non-text content or suppressing final-answer text.
    - direct tiny requests succeed, but RemoteClaw agent runs fail with backend/model crashes (for example Gemma on some `inferrs` builds) → RemoteClaw transport is likely already correct; the backend is failing on the larger agent-runtime prompt shape.
  ...
  <Accordion title="Fix options">
    ...
    3. Set `compat.supportsTools: false` for models/backends that cannot handle RemoteClaw's tool schema surface reliably.
    ...
    5. If tiny direct requests keep passing while RemoteClaw agent turns still crash inside the backend, treat it as an upstream server/model limitation and file a repro there with the accepted payload shape.
  </Accordion>
</AccordionGroup>
...
If `curl` returns RemoteClaw HTML, the Gateway is working and the remaining issue
...
does not leave the service running, run `remoteclaw gateway start` and recheck
`remoteclaw gateway status`.
...
- `remoteclaw devices rotate --scope ...` can only request operator scopes that the caller session already holds
...
remoteclaw logs --follow
remoteclaw doctor
remoteclaw gateway status --deep   # also scan system-level services
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    - `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `remoteclaw onboard --mode local` / `remoteclaw setup` to restamp the expected local-mode config. If you are running RemoteClaw via Podman, the default config path is `~/.remoteclaw/remoteclaw.json`.
    ...
    - `System-level RemoteClaw gateway service detected` from doctor → a systemd system unit exists while the user-level service is missing. Remove or disable the duplicate before allowing doctor to install a user service, or set `REMOTECLAW_SERVICE_REPAIR_POLICY=external` if the system unit is the intended supervisor.
    - `Gateway service port does not match current gateway config` → the installed supervisor still pins the old `--port`. Run `remoteclaw doctor --fix` or `remoteclaw gateway install --force`, then restart the gateway service.
...
```bash
ls ~/.remoteclaw/logs/stability/ | tail -5
remoteclaw gateway stability --bundle latest
pmset -g log | grep -iE "sleep|wake|maintenance" | tail -50
launchctl print gui/$UID/org.remoteclaw.gateway | grep -E "state|last exit|runs"
```

Look for:

- One or more `*-uncaught_exception.json` bundles in `~/.remoteclaw/logs/stability/` with `error.code` set to a transient network code such as `ENETDOWN`, `ENETUNREACH`, `EHOSTUNREACH`, or `ECONNREFUSED`.
...
- `runs` count incrementing across the day with no corresponding `received SIG*; shutting down` line in `~/Library/Logs/remoteclaw/gateway.log`: clean shutdowns log a signal; transient crashes do not.
...
3. **Add a liveness watchdog** so a future crash burst that gets parked by launchd is caught quickly:
   ...
   state=$(launchctl print gui/$UID/org.remoteclaw.gateway 2>/dev/null | awk -F'= ' '/state =/ {print $2; exit}')
...
remoteclaw logs --follow
remoteclaw gateway stability --bundle latest
remoteclaw gateway diagnostics export
```
...
- `critical memory pressure bundle written` appears shortly before restart → RemoteClaw captured a pre-OOM stability bundle. Inspect it with `remoteclaw gateway stability --bundle latest`.
- `memory pressure: level=critical ... memoryPressureSnapshot=disabled` appears in gateway logs → RemoteClaw detected critical memory pressure, but the pre-OOM stability snapshot is off.
...
- A timestamped `remoteclaw.json.rejected.*` file beside the active config
- A timestamped `remoteclaw.json.clobbered.*` file if `doctor --fix` repaired a broken direct edit
- RemoteClaw keeps the latest 32 `.clobbered.*` files for each config path and rotates older ones

<AccordionGroup>
  <Accordion title="What happened">
    - The config did not validate during startup, hot reload, or an RemoteClaw-owned write.
    - Gateway startup fails closed instead of rewriting `remoteclaw.json`.
    - Hot reload skips invalid external edits and keeps the current runtime config active.
    - RemoteClaw-owned writes reject invalid/destructive payloads before commit and save `.rejected.*`.
    - `remoteclaw doctor --fix` owns repair. It can remove non-JSON prefixes or restore the last-known-good copy while preserving the rejected payload as `.clobbered.*`.
    - When many repairs happen for one config path, RemoteClaw rotates older `.clobbered.*` files so the newest repaired payload is still available.
  ...
  <Accordion title="Common signatures">
    ...
    - `.rejected.*` exists → an RemoteClaw-owned config write failed schema or clobber checks before commit.
    ...
    - `missing-meta-vs-last-good`, `gateway-mode-missing-vs-last-good`, or `size-drop-vs-last-good:*` → an RemoteClaw-owned write was rejected because it lost fields or size compared with the last-known-good backup.
    - `Config last-known-good promotion skipped` → the candidate contained redacted secret placeholders such as `***`.

  </Accordion>
  <Accordion title="Fix options">
    1. Run `remoteclaw doctor --fix` to let doctor repair prefixed/clobbered config or restore last-known-good.
    2. Copy only the intended keys from `.clobbered.*` or `.rejected.*`, then apply them with `remoteclaw config set` or `config.patch`.
    3. Run `remoteclaw config validate` before restarting.
...
Use this when `remoteclaw gateway probe` reaches something, but still prints a warning block.
...
- `multiple reachable gateway identities detected` → distinct gateways answered, or RemoteClaw could not prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is treated as one gateway with multiple transports, even when transport ports differ.
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank, comment, header, fence, or empty-checklist scaffolding, so RemoteClaw skips the model call.
...
<AccordionGroup>
  ...
  <Accordion title="Element / screenshot / upload signatures">
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `remoteclaw browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
- [Browser (RemoteClaw-managed)](/tools/browser)
