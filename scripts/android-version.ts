// Android Version script supports RemoteClaw repository automation.
...
function main(argv = process.argv.slice(2)): number {
  ...
  if (options.format === "shell") {
    process.stdout.write(
      [
        `REMOTECLAW_ANDROID_VERSION_NAME=${version.canonicalVersion}`,
        `REMOTECLAW_ANDROID_VERSION_CODE=${version.versionCode}`,
      ].join("\n") + "\n",
    );
  } else {
