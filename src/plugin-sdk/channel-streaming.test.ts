...
describe("channel-streaming", () => {
  ...
  it("keeps compacted raw progress lines from leaking unmatched markdown backticks", () => {
    const line = formatChannelProgressDraftLine(
      {
        ...
        args: {
          command:
            "node scripts/check-something-with-a-very-long-path /tmp/remoteclaw/some/really/deep/path/that/keeps/going/and/going/index.ts --flag value",
        },
      },
      { detailMode: "raw" },
    );
    ...
    // RemoteClaw fork: tool-display always resolves in summary mode (detailMode
    // is accepted for source-compat but unused), so the compacted detail differs
    // from upstream's raw mode. The invariant under test — no unbalanced markdown
    // backticks leak from the compacted line — holds either way.
    expect(text).toBe("Shelling\n🛠️ Exec: run node script…h/that/keeps/going/and/going/ind…");
