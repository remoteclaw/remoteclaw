---
title: "Memory"
summary: "How RemoteClaw memory works (workspace files + automatic memory flush)"
read_when:
...
RemoteClaw memory is **plain Markdown in the agent workspace**. The files are the
...
`~/.remoteclaw/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.

## Memory tools

RemoteClaw exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, RemoteClaw triggers a **silent,
...
RemoteClaw can build a small vector index over `MEMORY.md` and `memory/*.md` so
...
- Configure memory search under `agents.defaults.memorySearch` (not top-level
  `memorySearch`).
- Uses remote embeddings by default. If `memorySearch.provider` is not set, RemoteClaw auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. RemoteClaw
...
BM25 + vectors + reranking. Markdown stays the source of truth; RemoteClaw shells
...
- The gateway runs QMD in a self-contained XDG home under
  `~/.remoteclaw/agents/<agentId>/qmd/` by setting `XDG_CONFIG_HOME` and
  `XDG_CACHE_HOME`.
...
- The gateway writes a self-contained QMD home under
  `~/.remoteclaw/agents/<agentId>/qmd/` (config + cache + sqlite DB).
...
- Searches run via `memory.qmd.searchMode` (default `qmd search --json`; also
  ...
  QMD build, RemoteClaw retries with `qmd query`. If QMD fails or the binary is
  missing, RemoteClaw automatically falls back to the builtin SQLite manager so
  memory tools keep working.
- RemoteClaw does not expose QMD embed batch-size tuning today; batch behavior is
  controlled by QMD itself.
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  expansion) on the first `qmd query` run.
  - RemoteClaw sets `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` automatically when it runs QMD.
  - If you want to pre-download models manually (and warm the same index RemoteClaw
    uses), run a one-off query with the agent’s XDG dirs.

    RemoteClaw’s QMD state lives under your **state dir** (defaults to `~/.remoteclaw`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
    RemoteClaw uses:

    ```bash
    # Pick the same state dir RemoteClaw uses
...
- When `scope` denies a search, RemoteClaw logs a warning with the derived
  `channel`/`chatType` so empty results are easier to debug.
- Snippets sourced outside the workspace show up as
  `qmd/<collection>/<relative-path>` in `memory_search` results; `memory_get`
  understands that prefix and reads from the configured QMD collection root.
- When `memory.qmd.sessions.enabled = true`, RemoteClaw exports sanitized session
  transcripts (User/Assistant turns) into a dedicated QMD collection under
  `~/.remoteclaw/agents/<id>/qmd/sessions/`, so `memory_search` can recall recent
...
- If `memorySearch.multimodal.enabled = true`, RemoteClaw also indexes supported image/audio files under `extraPaths` only. Default memory roots (`MEMORY.md`, `memory.md`, `memory/**/*.md`) stay Markdown-only.
- Symlinks are ignored (files or directories).

### Multimodal memory files (Gemini image + audio)

RemoteClaw can index image and audio files from `memorySearch.extraPaths` when using Gemini embedding 2:
...
> RemoteClaw will automatically reindex when it detects a model or dimension change.
...
- Index storage: per-agent SQLite at `~/.remoteclaw/memory/<agentId>.sqlite` (configurable via `agents.defaults.memorySearch.store.path`, supports `{agentId}` token).
- Freshness: watcher on `MEMORY.md` + `memory/` marks the index dirty (debounce 1.5s). Sync is scheduled on session start, on search, or on an interval and runs asynchronously. Session transcripts use delta thresholds to trigger background sync.
- Reindex triggers: the index stores the embedding **provider/model + endpoint fingerprint + chunking params**. If any of those change, RemoteClaw automatically resets and reindexes the entire store.

### Hybrid search (BM25 + vector)

When enabled, RemoteClaw combines:

- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, RemoteClaw falls back to vector-only search.
...
RemoteClaw can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
- Session logs live on disk (`~/.remoteclaw/agents/<agentId>/sessions/*.jsonl`). Any process/user with filesystem access can read them, so treat disk access as the trust boundary. For stricter isolation, run agents under separate OS users or hosts.
...
When the sqlite-vec extension is available, RemoteClaw stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, RemoteClaw logs the
