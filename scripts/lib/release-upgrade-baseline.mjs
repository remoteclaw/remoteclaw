...
export function compareRemoteClawVersions(leftVersion, rightVersion) {
  const left = parseVersion(leftVersion);
  const right = parseVersion(rightVersion);
  if (!left || !right) {
    throw new Error(`cannot compare RemoteClaw versions: ${leftVersion} ${rightVersion}`);
  }
...
function normalizePublishedVersions(publishedVersions) {
  return [...new Set(publishedVersions.map((version) => String(version).trim()).filter(Boolean))]
    ...
    .toSorted((left, right) => compareRemoteClawVersions(right, left));
}

export function resolveDefaultReleaseUpgradeBaseline(candidateVersion, publishedVersions) {
  const candidate = parseVersion(candidateVersion);
  if (!candidate) {
    throw new Error(`invalid candidate RemoteClaw version: ${candidateVersion}`);
  }

  const versions = normalizePublishedVersions(publishedVersions);
  const older = versions.find(
    (version) => compareRemoteClawVersions(version, candidate.version) < 0,
  );
  if (older) {
    return `remoteclaw@${older}`;
  }
...
function readPublishedVersions(args) {
  ...
  const raw = execFileSync("npm", ["view", "remoteclaw", "versions", "--json", "--silent"], {
    encoding: "utf8",
    stdio: ["ignore", "pipe", "inherit"],
  });
  const parsed = JSON.parse(raw);
  if (!Array.isArray(parsed)) {
    throw new Error("npm returned a non-array remoteclaw versions payload");
  }
  return parsed;
}
