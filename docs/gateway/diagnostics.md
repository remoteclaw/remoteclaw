...
RemoteClaw can build a local diagnostics `.zip` for bug reports: sanitized Gateway
...
2. RemoteClaw sends a preamble and asks for one explicit exec approval, which runs
   `remoteclaw gateway diagnostics export --json`. Do not approve diagnostics via
   an allow-all rule.
3. After approval, RemoteClaw replies with the local bundle path, manifest
   summary, privacy notes, and relevant session ids.

In group chats, an owner can still run `/diagnostics`, but RemoteClaw sends the
...
approval also covers an OpenAI feedback upload for the Codex threads RemoteClaw
...
approval, the reply lists channels, RemoteClaw session ids, Codex thread ids, and
...
| ----------------------- | --------------------------------------------------------------------------------- | -------------------------------------------------- |
| `--output <path>`       | `$REMOTECLAW_STATE_DIR/logs/support/remoteclaw-diagnostics-<timestamp>-<pid>.zip` | Write to a specific zip path (or directory).       |
