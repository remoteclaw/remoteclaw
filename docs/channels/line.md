...
LINE connects to RemoteClaw via the LINE Messaging API. The plugin runs as a webhook
...
LINE ships as a bundled plugin in current RemoteClaw releases, so normal
...
If npm reports the RemoteClaw-owned package as deprecated or missing, use a
current packaged RemoteClaw build or a local checkout until the npm package train
catches up.

Local checkout (when running from a git repo):

```bash
remoteclaw plugins install ./path/to/local/line-plugin
```
...
```bash
remoteclaw pairing list line
remoteclaw pairing approve line <CODE>
```
...
- Inbound media is saved under `~/.remoteclaw/media/inbound/` before it is passed
...
Outbound media URLs must be public HTTPS URLs. RemoteClaw validates the target hostname before handing the URL to LINE and rejects loopback, link-local, and private-network targets.
