...
describe("wrapNoteMessage", () => {
  ...
  it("preserves long Windows paths without inserting spaces/newlines", () => {
    ...
    const input = "C:\\\\State\\\\RemoteClaw\\\\bin\\\\remoteclaw.exe";
  ...
  it("widens note output columns so clack does not re-wrap copy-sensitive lines", () => {
    const wrapped = wrapNoteMessage(
      [
        ...
        "- ~/.remoteclaw/agents/main/sessions/9c2acae5-841f-4aea-936b-fdb513b60202.jsonl.lock pid=86519 (alive) age=2m47s stale=no",
      ].join("\n"),
      { columns: 80 },
    );
    const writes: string[] = [];
    ...
    expect(rendered).toContain(
      "- ~/.remoteclaw/agents/main/sessions/9c2acae5-841f-4aea-936b-fdb513b60202.jsonl.lock",
    );
  });
