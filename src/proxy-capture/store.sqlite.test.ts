...
function makeStore() {
  const root = mkdtempSync(path.join(os.tmpdir(), "remoteclaw-proxy-capture-"));
...
describe("DebugProxyCaptureStore", () => {
  it("keeps the cached store open until the last lease releases", () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "remoteclaw-proxy-capture-lease-"));
    cleanupDirs.push(root);
