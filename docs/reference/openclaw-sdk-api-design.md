---
summary: "Reference design for the public RemoteClaw App SDK API, event taxonomy, artifacts, approvals, and package structure"
title: "RemoteClaw App SDK API design"
sidebarTitle: "App SDK API design"
read_when:
  - You are implementing the proposed public RemoteClaw app SDK
  - You need the draft namespace, event, result, artifact, approval, or security contract for the app SDK
  - You are comparing Gateway protocol resources with the high-level RemoteClaw App SDK wrapper
---

This page is the detailed API reference design for the public
[RemoteClaw App SDK](/concepts/remoteclaw-sdk). It is intentionally separate from
the [Plugin SDK](/plugins/sdk-overview).

<Note>
  `@remoteclaw/sdk` is the external app/client package for talking to the
  Gateway. `remoteclaw/plugin-sdk/*` is the in-process plugin authoring contract.
...
2. A high-level ergonomic wrapper with `RemoteClaw`, `Agent`, `Session`, `Run`,
...
```typescript
type RemoteClawEvent = {
  version: 1;
  id: string;
  ts: number;
  type: RemoteClawEventType;
  runId?: string;
  sessionId?: string;
  sessionKey?: string;
  taskId?: string;
...
- source: RemoteClaw, MCP, plugin, channel, runtime, or app
- schema summary
- approval policy
...
| Package                   | Purpose                                                       |
| ------------------------- | ------------------------------------------------------------- |
| `@remoteclaw/sdk`         | Public high-level SDK and generated low-level Gateway client. |
| `@remoteclaw/sdk-react`   | Optional React hooks for dashboards and app builders.         |
| `@remoteclaw/sdk-testing` | Test helpers and fake Gateway server for app integrations.    |

The repo already has `remoteclaw/plugin-sdk/*` for plugins. Keep that namespace
...
4. High-level `RemoteClaw`, `Agent`, `Session`, `Run`, `Task`, and `Artifact`
   wrappers.
...
- [RemoteClaw App SDK](/concepts/remoteclaw-sdk)
