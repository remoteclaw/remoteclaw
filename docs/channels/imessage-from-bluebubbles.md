...
BlueBubbles support was removed. RemoteClaw supports iMessage through `imsg` only. This guide is for migrating old `channels.bluebubbles` configs to `channels.imessage`; there is no other supported migration path.
...
5. With the Gateway stopped, enable `channels.imessage`, then run `remoteclaw channels status --probe --channel imessage`.
...
`imsg` is a local macOS CLI for Messages. RemoteClaw starts `imsg rpc` as a child process and talks JSON-RPC over stdin/stdout. There is no HTTP server, webhook URL, background daemon, launch agent, or port to expose.
...
- Linux builds can inspect a copied `chat.db`, but cannot send, watch the live Mac database, or drive Messages.app. For RemoteClaw iMessage, run `imsg` on the signed-in Mac or through an SSH wrapper to that Mac.
...
2. Verify the read, watch, send, and RPC surfaces before changing RemoteClaw config:

   ```bash
   imsg chats --limit 10 --json | jq -s
   imsg history --chat-id 42 --limit 10 --attachments --json | jq -s
   imsg watch --chat-id 42 --reactions --json
   imsg send --chat-id 42 --text "RemoteClaw imsg test"
   imsg rpc --help
   ```

   Replace `42` with a real chat id from `imsg chats`. Sending requires Automation permission for Messages.app. If RemoteClaw will run through SSH, run these commands through the same SSH wrapper or user context that RemoteClaw will use. If reads/probes work but sends fail with AppleEvents `-1743`, check whether Automation landed on `/usr/libexec/sshd-keygen-wrapper`; see [SSH wrapper sends fail with AppleEvents -1743](/channels/imessage#ssh-wrapper-sends-fail-with-appleevents-1743).
...
4. After you add an enabled `channels.imessage` config, verify the bridge through RemoteClaw:
...
5. Snapshot your config:
   ...
   ```bash
   cp ~/.remoteclaw/remoteclaw.json5 ~/.remoteclaw/remoteclaw.json5.bak
   ```
...
7. **Remove the BlueBubbles server and config** once iMessage DMs, groups, and actions are verified. RemoteClaw will not use `channels.bluebubbles`.
...
| Threaded reply (`reply_to_guid`)                           | ✅                                  | ✅ (closes [#51892](https://github.com/remoteclaw/remoteclaw/issues/51892))                                                 |
| Tapback (`react`)                                          | ✅                                  | ✅                                                                                                                          |
| Edit / unsend (macOS 13+ recipients)                       | ✅                                  | ✅                                                                                                                          |
| Send with screen effect                                    | ✅                                  | ✅ (closes part of [#9394](https://github.com/remoteclaw/remoteclaw/issues/9394))                                           |
...
| Catchup of inbound messages received while gateway is down | ✅ (webhook replay + history fetch) | ✅ (opt-in via `channels.imessage.catchup.enabled`; closes [#78649](https://github.com/remoteclaw/remoteclaw/issues/78649)) |
...
The reply cache lives in SQLite plugin state. `remoteclaw doctor --fix` imports and archives the old `imessage/reply-cache.jsonl` sidecar when present.
