...
function main() {
  ...
  if (offenders.length > 0) {
    console.error("Bundled plugin entrypoints must not import monolithic remoteclaw/plugin-sdk.");
    for (const file of offenders.toSorted()) {
      const relative = path.relative(process.cwd(), file) || file;
      console.error(`- ${relative}`);
    }
    console.error("Use remoteclaw/plugin-sdk/<channel> for channel plugins or /core for others.");
    process.exit(1);
  }
