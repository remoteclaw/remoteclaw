...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export OPENCLAW_IMAGE="ghcr.io/remoteclaw/remoteclaw:latest"
    ./docker-setup.sh
    ```
...
```bash
docker compose exec remoteclaw-gateway node dist/index.js health --token "$OPENCLAW_GATEWAY_TOKEN"
```
...
Docker Compose bind-mounts `OPENCLAW_CONFIG_DIR` to `/home/node/.remoteclaw` and
`OPENCLAW_WORKSPACE_DIR` to `/home/node/.remoteclaw/workspace`, so those paths
...
<AccordionGroup>
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export OPENCLAW_HOME_VOLUME="remoteclaw_home"`
