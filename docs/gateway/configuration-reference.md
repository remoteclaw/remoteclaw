...
- `REMOTECLAW_ALLOW_INSECURE_PRIVATE_WS=1`: client-side process-environment
  break-glass override that allows plaintext `ws://` to trusted private-network
  IPs; default remains loopback-only for plaintext. There is no `remoteclaw.json`
...
- `stuckSessionAbortMs`: no-progress age threshold in ms before eligible stalled active work may be abort-drained for recovery. When unset, RemoteClaw uses the safer extended embedded-run window of at least 10 minutes and 5x `stuckSessionWarnMs`.
