...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@remoteclaw/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    ...
    { packageName: "@remoteclaw/matrix", id: "quietchat", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      resolveSafeInstallDir({
        baseDir: "/tmp/plugins",
        id: "@remoteclaw/matrix",
        invalidNameMessage: "invalid plugin name",
      }),
    ).toEqual({
      ok: true,
      path: path.join("/tmp/plugins", "@remoteclaw__matrix"),
    });
  });
