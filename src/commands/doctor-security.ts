    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("remoteclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("remoteclaw config set gateway.auth.mode token")}`,
            ]
          : [
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("remoteclaw config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
