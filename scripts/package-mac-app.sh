...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/remoteclaw/remoteclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
plist_set_string_required "$APP_ROOT/Contents/Info.plist" RemoteClawBuildTimestamp "$BUILD_TS"
plist_set_string_required "$APP_ROOT/Contents/Info.plist" RemoteClawGitCommit "$GIT_COMMIT"
...
if [ -d "$REMOTECLAWKIT_BUNDLE" ]; then
  ...
  echo "ERROR: RemoteClawKit resource bundle not found at $REMOTECLAWKIT_BUNDLE" >&2
  exit 1
fi
...
stop_packaged_app_if_running() {
  ...
  echo "ERROR: Packaged RemoteClaw bundle did not exit: ${pids[*]}" >&2
  return 1
}
