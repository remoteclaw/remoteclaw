...
final class VoiceNoteRecorderTests: XCTestCase {
    ...
    func testDurationLabelBoundsMalformedHistoryValues() {
        XCTAssertEqual(remoteClawVoiceNoteDurationLabel(.infinity), "0:00")
        XCTAssertEqual(remoteClawVoiceNoteDurationLabel(.nan), "0:00")
        XCTAssertEqual(remoteClawVoiceNoteDurationLabel(1e100), "3:00")
        XCTAssertEqual(remoteClawVoiceNoteDurationLabel(-1), "0:00")
    }

    @MainActor
    func testRecordButtonRequiresAttachmentInput() {
        let recorder = RemoteClawVoiceNoteRecorder(capture: FakeVoiceNoteAudioCapture())
        let control = RemoteClawChatVoiceNoteControl(recorder: recorder, isTalkActive: false)
        let button = RemoteClawVoiceNoteButton(
            control: control,
            compact: false,
