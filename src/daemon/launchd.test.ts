...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 org.remoteclaw.gateway\n";
...
describe("launchd install", () => {
  ...
  it("rewrites the plist before bootstrap during restart fallback", async () => {
    ...
    state.files.set(
      ...
      [
        ...
        "  <dict>",
        "    <key>Label</key>",
        "    <string>org.remoteclaw.gateway</string>",
  ...
  it("stops LaunchAgent by disabling relaunch before stopping the process", async () => {
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/org.remoteclaw.gateway`;
    expect(state.launchctlCalls).toContainEqual(["disable", serviceId]);
    expect(state.launchctlCalls).toContainEqual(["stop", "org.remoteclaw.gateway"]);
  ...
  it("treats already-unloaded services as successfully stopped without bootout fallback", async () => {
    ...
    expect(state.launchctlCalls).toContainEqual([
      "disable",
      `${typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501"}/org.remoteclaw.gateway`,
    ]);
    expect(state.launchctlCalls.some((call) => call[0] === "bootout")).toBe(false);
    expect(output).toContain("Stopped LaunchAgent");
...
describe("resolveLaunchAgentPlistPath", () => {
  it.each([
    {
      ...
      env: { HOME: "/Users/test" },
      expected: "/Users/test/Library/LaunchAgents/org.remoteclaw.gateway.plist",
    },
    {
      ...
      expected: "/Users/test/Library/LaunchAgents/org.remoteclaw.jbphoenix.plist",
    },
    {
      ...
      env: {
        ...
        REMOTECLAW_LAUNCHD_LABEL: "com.custom.label",
      },
      expected: "/Users/test/Library/LaunchAgents/com.custom.label.plist",
    },
    {
      name: "trims whitespace from REMOTECLAW_LAUNCHD_LABEL",
      env: {
        HOME: "/Users/test",
        REMOTECLAW_LAUNCHD_LABEL: "  com.custom.label  ",
      },
    ...
    {
      name: "ignores empty REMOTECLAW_LAUNCHD_LABEL and falls back to profile",
      env: {
        HOME: "/Users/test",
        REMOTECLAW_PROFILE: "myprofile",
        REMOTECLAW_LAUNCHD_LABEL: "   ",
      },
      expected: "/Users/test/Library/LaunchAgents/org.remoteclaw.myprofile.plist",
    },
  ])("$name", ({ env, expected }) => {
  ...
  it("rejects invalid launchd labels that contain path separators", () => {
    expect(() =>
      resolveLaunchAgentPlistPath({
        HOME: "/Users/test",
        REMOTECLAW_LAUNCHD_LABEL: "../evil/label",
      }),
