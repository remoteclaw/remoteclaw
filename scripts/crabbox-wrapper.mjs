...
const ignoreRepoBinary = process.env.REMOTECLAW_CRABBOX_WRAPPER_IGNORE_REPO_BINARY === "1";
...
const remoteChangedGateEnv = [
  "REMOTECLAW_CHECK_CHANGED_REMOTE_CHILD=1",
  "REMOTECLAW_CHANGED_LANES_RAW_SYNC=1",
  "CI=1",
];
...
function shouldRequireBrokeredAws(commandArgs, providerName) {
  if (process.env.REMOTECLAW_CRABBOX_ALLOW_DIRECT_AWS === "1") {
    return false;
  }
...
function enforceBrokeredAws(commandArgs, providerName) {
  ...
  console.error(
    [
      "[crabbox] provider=aws requires a configured Crabbox broker for RemoteClaw proof.",
      "[crabbox] run `crabbox login --url https://crabbox.remoteclaw.org --provider aws`, then retry.",
      "[crabbox] for intentional direct AWS provider debugging, set REMOTECLAW_CRABBOX_ALLOW_DIRECT_AWS=1.",
    ].join("\n"),
  );
  process.exit(2);
}
...
function remoteGitBootstrapForChangedGate(changedGateBase) {
  ...
  return [
    "remoteclaw_changed_gate_base=${REMOTECLAW_CHANGED_GATE_BASE:-" + quotedBase + "};",
    'if ! command -v git >/dev/null 2>&1; then echo "git is required for RemoteClaw remote changed-gate sync" >&2; exit 2; fi;',
    'remoteclaw_changed_gate_remote_base="$(git rev-parse --verify refs/remotes/origin/main 2>/dev/null || true)";',
    'if ! git status --short >/dev/null 2>&1 || [ "$remoteclaw_changed_gate_remote_base" != "$remoteclaw_changed_gate_base" ]; then',
    "rm -rf .git;",
    "git init -q;",
    "git remote add origin https://github.com/remoteclaw/remoteclaw.git 2>/dev/null || git remote set-url origin https://github.com/remoteclaw/remoteclaw.git;",
    'git fetch -q --depth=1 origin "$remoteclaw_changed_gate_base:refs/remotes/origin/main";',
    "git reset --mixed --quiet refs/remotes/origin/main;",
    "git add -A;",
    "if ! git diff --cached --quiet; then git -c user.name=RemoteClaw -c user.email=ci@remoteclaw.local commit -q --no-gpg-sign -m remote-changed-gate-tree; fi;",
    "fi",
  ].join(" ");
}
...
function remoteWindowsHydratedNodeModulesBootstrap() {
  return [
    "$remoteclawModulesDir = $env:PNPM_CONFIG_MODULES_DIR",
    "if ($remoteclawModulesDir) {",
    'if (-not (Test-Path $remoteclawModulesDir)) { throw "PNPM_CONFIG_MODULES_DIR does not exist: $remoteclawModulesDir" }',
    '$remoteclawWorkspaceModules = Join-Path (Get-Location).Path "node_modules"',
    '$remoteclawSelfModules = Join-Path $remoteclawModulesDir "node_modules"',
    'if (-not (Test-Path $remoteclawSelfModules)) { cmd /c mklink /J "$remoteclawSelfModules" "$remoteclawModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link hydrated pnpm node_modules" } }',
    'if (-not (Test-Path $remoteclawWorkspaceModules)) { cmd /c mklink /J "$remoteclawWorkspaceModules" "$remoteclawModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link workspace node_modules" } }',
    "}",
  ].join("; ");
}
...
function remotePosixJsEnvBootstrap() {
  return [
    "remoteclaw_crabbox_env() {",
    "remoteclaw_env_args=();",
    "remoteclaw_env_ignore=0;",
    "remoteclaw_env_path_seen=0;",
    'while [ "$#" -gt 0 ]; do',
    'case "$1" in',
    '-i|--ignore-environment) remoteclaw_env_ignore=1; remoteclaw_env_args+=("$1"); shift ;;',
    '-S|--split-string|-S*|--split-string=*) command env "${remoteclaw_env_args[@]}" "$@"; return ;;',
    '-[!-]*i*) remoteclaw_env_ignore=1; remoteclaw_env_args+=("$1"); shift ;;',
    '-u|--unset|-C|--chdir) remoteclaw_env_args+=("$1"); shift; if [ "$#" -gt 0 ]; then remoteclaw_env_args+=("$1"); shift; fi ;;',
    '--unset=*|--chdir=*) remoteclaw_env_args+=("$1"); shift ;;',
    'PATH=*) if [ "$remoteclaw_env_ignore" = "1" ]; then remoteclaw_env_args+=("PATH=${REMOTECLAW_CRABBOX_BOOTSTRAP_PATH:-$PATH}:${1#PATH=}"); else remoteclaw_env_args+=("$1"); fi; remoteclaw_env_path_seen=1; shift ;;',
    '[A-Za-z_]*=*) remoteclaw_env_args+=("$1"); shift ;;',
    '--) remoteclaw_env_args+=("--"); shift; break ;;',
    "*) break ;;",
    "esac;",
    "done;",
    'if [ "$remoteclaw_env_ignore" = "1" ] && [ "$remoteclaw_env_path_seen" = "0" ]; then remoteclaw_env_args+=("PATH=${REMOTECLAW_CRABBOX_BOOTSTRAP_PATH:-$PATH}"); fi;',
    'command env "${remoteclaw_env_args[@]}" "$@";',
    "};",
  ];
}

function remoteAwsMacosJsBootstrap({ packageManager = false, bun = false } = {}) {
  const nodeVersion = process.env.REMOTECLAW_CRABBOX_MACOS_NODE_VERSION?.trim() || "24.15.0";
  const bootstrap = [
    "remoteclaw_crabbox_bootstrap_macos_js() {",
    'tool_root="${REMOTECLAW_CRABBOX_MACOS_TOOLCHAIN_DIR:-$HOME/.remoteclaw-crabbox-toolchain}";',
    ...
    'macos_locale="${REMOTECLAW_CRABBOX_MACOS_LOCALE:-en_US.UTF-8}";',
    ...
    'ready_marker="$node_dir/.remoteclaw-crabbox-node-ready";',
  ...
  if (bun) {
    bootstrap.push(
      ...
      'bun_ready_marker="$bun_root/.remoteclaw-crabbox-bun-ready";',
  ...
  bootstrap.push('export REMOTECLAW_CRABBOX_BOOTSTRAP_PATH="$PATH";');
  bootstrap.push("};", "remoteclaw_crabbox_bootstrap_macos_js");
  return bootstrap.join(" ");
}

function remoteWsl2JsBootstrap({ packageManager = false } = {}) {
  const nodeVersion = process.env.REMOTECLAW_CRABBOX_WSL2_NODE_VERSION?.trim() || "24.15.0";
  const bootstrap = [
    "remoteclaw_crabbox_bootstrap_wsl2_js() {",
    'tool_root="${REMOTECLAW_CRABBOX_WSL2_TOOLCHAIN_DIR:-$HOME/.remoteclaw-crabbox-toolchain}";',
    `node_version=${shellQuote(nodeVersion)};`,
    'arch="$(uname -m)";',
  ...
  bootstrap.push('export REMOTECLAW_CRABBOX_BOOTSTRAP_PATH="$PATH";');
  bootstrap.push("};", "remoteclaw_crabbox_bootstrap_wsl2_js");
  return bootstrap.join(" ");
}
...
function scopedAwsMacosEnvCommand(commandArgs) {
  ...
  return {
    ...
    bun: needsBun,
    shellCommand: `remoteclaw_crabbox_env ${shellJoin(commandArgs.slice(1))}`,
  };
}
...
function shellCommandWithEnvShim(command, eligibleSegments) {
  ...
  for (const segment of shellCommandSegmentsWithBounds(command)) {
    ...
    rewritten += "remoteclaw_crabbox_env";
...
function prepareRemoteWsl2JsBootstrapScript(commandArgs, providerName) {
  ...
  const scriptRoot = mkdtempSync(resolve(tmpdir(), "remoteclaw-crabbox-wsl2-script-"));
...
function remoteAwsMacosSwiftBootstrap() {
  return [
    "remoteclaw_crabbox_require_macos_swift_62() {",
    'remoteclaw_xcode="";',
    'for remoteclaw_candidate in /Applications/Xcode_26.1.app /Applications/Xcode_26*.app /Applications/Xcode-26*.app; do if [ -d "$remoteclaw_candidate" ]; then remoteclaw_xcode="$remoteclaw_candidate"; fi; done;',
    'if [ -n "$remoteclaw_xcode" ]; then remoteclaw_developer="$remoteclaw_xcode/Contents/Developer"; if [ ! -d "$remoteclaw_developer" ]; then remoteclaw_developer="$remoteclaw_xcode"; fi; sudo xcode-select -s "$remoteclaw_developer" || return 1; fi;',
    'remoteclaw_swift_version="$(swift --version 2>&1)" || { status=$?; printf "%s\\n" "$remoteclaw_swift_version" >&2; return "$status"; };',
    'printf "%s\\n" "$remoteclaw_swift_version" >&2;',
    'remoteclaw_swift_major_minor="$(printf "%s\\n" "$remoteclaw_swift_version" | sed -nE "s/.*Apple Swift version ([0-9]+)\\.([0-9]+).*/\\1 \\2/p" | head -n 1)";',
    'if [ -z "$remoteclaw_swift_major_minor" ]; then echo "[crabbox] RemoteClaw macOS app proof requires Swift tools 6.2+; unable to parse swift --version." >&2; return 2; fi;',
    "set -- $remoteclaw_swift_major_minor;",
    'if [ "$1" -lt 6 ] || { [ "$1" -eq 6 ] && [ "$2" -lt 2 ]; }; then',
    'echo "[crabbox] RemoteClaw macOS app proof requires Swift tools 6.2+ (Xcode 26.x)." >&2;',
    'echo "[crabbox] current Swift is $1.$2; select/install Xcode 26.x or use a Blacksmith macOS runner with Xcode_26.1.app." >&2;',
    "return 2;",
    "fi;",
    'remoteclaw_xcodebuild_version="$(xcodebuild -version 2>&1)" || { printf "%s\\n" "$remoteclaw_xcodebuild_version" >&2; echo "[crabbox] RemoteClaw macOS app proof requires Xcode 26.x; active developer directory does not provide usable xcodebuild." >&2; return 2; };',
    'printf "%s\\n" "$remoteclaw_xcodebuild_version" >&2;',
    'remoteclaw_xcode_major="$(printf "%s\\n" "$remoteclaw_xcodebuild_version" | sed -nE "s/^Xcode ([0-9]+)(\\..*)?$/\\1/p" | head -n 1)";',
    'if [ "$remoteclaw_xcode_major" != "26" ]; then echo "[crabbox] RemoteClaw macOS app proof requires Xcode 26.x; current xcodebuild is ${remoteclaw_xcode_major:-unknown}." >&2; return 2; fi;',
    "};",
    "remoteclaw_crabbox_require_macos_swift_62",
  ].join(" ");
}
...
function prepareAwsMacosScriptStdinBootstrap(commandArgs, providerName) {
  ...
  const scriptRoot = mkdtempSync(resolve(tmpdir(), "remoteclaw-crabbox-macos-script-"));
  const scriptPath = resolve(scriptRoot, "script.sh");
...
function createAwsMacosScriptStdinWrapper(script) {
  ...
  return [
    ...
    'tmp_script="$(mktemp "${TMPDIR:-/tmp}/remoteclaw-crabbox-script.XXXXXX")" || exit $?',
    'cleanup_remoteclaw_crabbox_script() { rm -f "$tmp_script"; }',
    "trap cleanup_remoteclaw_crabbox_script EXIT",
...
function uniqueHereDocDelimiter(script) {
  let index = 0;
  for (;;) {
    const delimiterLocal = `REMOTECLAW_CRABBOX_SCRIPT_${index}`;
...
function defaultFullCheckoutSyncRoot() {
  const home = homedir();
  if (home) {
    return resolve(home, ".cache", "remoteclaw", "crabbox-sync");
  }
  return resolve(tmpdir(), "remoteclaw-crabbox-sync");
}

function fullCheckoutSyncRoot() {
  const configured = process.env.REMOTECLAW_CRABBOX_SYNC_TMPDIR?.trim();
...
function assertFullCheckoutSyncDisk(root) {
  ...
  throw new Error(
    [
      ...
      "set REMOTECLAW_CRABBOX_SYNC_TMPDIR to a roomier filesystem or lower REMOTECLAW_CRABBOX_SYNC_MIN_FREE_BYTES if you know this checkout fits",
    ].join("; "),
  );
}
...
function prepareFullCheckoutForSync(options = {}) {
  ...
  const dir = mkdtempSync(resolve(syncRoot, "remoteclaw-crabbox-sync-"));
  let active = false;
  ...
  if (!satisfiesMinimumCrabboxVersion(version.text, minimumBlacksmithCrabboxVersion)) {
    console.error(
      [
        ...
        "[crabbox] if using ../crabbox, rebuild it: version=$(git -C ../crabbox describe --tags --always --dirty | sed 's/^v//') && go build -C ../crabbox -trimpath -ldflags \"-s -w -X github.com/remoteclaw/crabbox/internal/cli.version=${version}\" -o bin/crabbox ./cmd/crabbox",
      ].join("\n"),
    );
    process.exit(2);
  }
...
if (
  ...
  if (isAwsMacosRemoteTarget(normalizedArgs, provider)) {
    ...
    console.error(
      `[crabbox] warning: provider=aws raw boxes may lack Node/Corepack/pnpm/Bun for ${runtimeEntrypoint}; hydrate first (${hydrate}) or pass --provider blacksmith-testbox for RemoteClaw CI-like proof; not switching providers automatically`,
    );
  }
}
...
if (
  ...
  console.error(
    "[crabbox] provider=docker enabling host Docker socket pass-through for RemoteClaw Docker tests",
  );
}
if (
  isLocalContainerProvider(provider) &&
  ...
  childEnv.CRABBOX_LOCAL_CONTAINER_WORK_ROOT = "/tmp/remoteclaw-crabbox-docker-work";
  console.error(
    "[crabbox] provider=docker using short host-visible work root for RemoteClaw Docker tests",
  );
}
...
function resolveChildKillGraceMs(env) {
  if (!env.VITEST || !env.REMOTECLAW_TEST_CRABBOX_CHILD_KILL_GRACE_MS) {
    return 5_000;
  }
  const value = Number.parseInt(env.REMOTECLAW_TEST_CRABBOX_CHILD_KILL_GRACE_MS, 10);
  return Number.isFinite(value) && value >= 0 ? value : 5_000;
}

function resolveMetadataProbeTimeoutMs(env) {
  if (!env.VITEST || !env.REMOTECLAW_TEST_CRABBOX_METADATA_PROBE_TIMEOUT_MS) {
    return CRABBOX_METADATA_PROBE_TIMEOUT_MS;
  }
  const value = Number.parseInt(env.REMOTECLAW_TEST_CRABBOX_METADATA_PROBE_TIMEOUT_MS, 10);
