// Verifies installed packages can bootstrap the default RemoteClaw workspace files.
...
export function createWorkspaceBootstrapSmokeEnv(env, homeDir, overrides = {}) {
  ...
  return {
    ...
    REMOTECLAW_NO_ONBOARD: "1",
    REMOTECLAW_SUPPRESS_NOTES: "1",
...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "remoteclaw-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "remoteclaw.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".remoteclaw", "workspace");
