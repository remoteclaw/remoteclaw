...
vi.mock("remoteclaw/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("remoteclaw/plugin-sdk/file-lock")>(
    "remoteclaw/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
describe("Matrix IndexedDB persistence lock ordering", () => {
  ...
  it("captures the snapshot after the file lock is acquired", async () => {
    ...
    const dbName = "remoteclaw-matrix-test::matrix-sdk-crypto";
    await seedDatabase({
      name: dbName,
      storeName: "sessions",
    ...
    await persistIdbToDisk({ snapshotPath, databasePrefix: "remoteclaw-matrix-test" });
