/**
 * High-level lifecycle management for RemoteClaw's operator-managed network
 * proxy routing.
 *
 * RemoteClaw does not spawn or configure the filtering proxy. When enabled, it
...
const PROXY_ACTIVE_KEYS = ["REMOTECLAW_PROXY_ACTIVE", "REMOTECLAW_PROXY_LOOPBACK_MODE"] as const;
...
function captureProxyEnv(): ProxyEnvSnapshot {
  return {
    ...
    REMOTECLAW_PROXY_ACTIVE: process.env["REMOTECLAW_PROXY_ACTIVE"],
    REMOTECLAW_PROXY_LOOPBACK_MODE: process.env["REMOTECLAW_PROXY_LOOPBACK_MODE"],
  };
}
...
function applyProxyEnv(proxyUrl: string, loopbackMode: ProxyLoopbackMode): void {
  ...
  process.env["REMOTECLAW_PROXY_LOOPBACK_MODE"] = loopbackMode;
...
function resolveProxyUrl(config: ProxyConfig | undefined): string {
  const candidate = config?.proxyUrl?.trim() || process.env["REMOTECLAW_PROXY_URL"]?.trim();
  if (!candidate) {
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl " +
        "or REMOTECLAW_PROXY_URL to an http:// forward proxy.",
    );
  }
...
export function ensureInheritedManagedProxyRoutingActive(): void {
  if (process.env["REMOTECLAW_PROXY_ACTIVE"] !== "1") {
    return;
  }
