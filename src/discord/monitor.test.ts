...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of RemoteClaw")).toBe("friends-of-remoteclaw");
  ...
  it("matches ids or names", () => {
    ...
    expect(allowListMatches(allow, { name: "friends-of-remoteclaw" })).toBe(true);
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    ...
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of RemoteClaw"),
      guildEntries,
    });
