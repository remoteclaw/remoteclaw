...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "remoteclaw.internal", expected: "remoteclaw.internal." },
    { value: "remoteclaw.internal.", expected: "remoteclaw.internal." },
    { value: "  remoteclaw.internal  ", expected: "remoteclaw.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { REMOTECLAW_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { REMOTECLAW_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
    ...
    },
    {
      ...
      params: {
        env: { REMOTECLAW_WIDE_AREA_DOMAIN: "foo/bar" } as NodeJS.ProcessEnv,
      },
      expected: null,
    },
  ...
  it("builds valid zone paths under the DNS config directory", () => {
    ...
    const zonePath = getWideAreaZonePath("remoteclaw.internal.");

    expect(zonePath).toBe(path.join(dnsDir, "remoteclaw.internal.db"));
    expect(path.relative(dnsDir, zonePath)).toBe("remoteclaw.internal.db");
  });
});
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("remoteclaw.internal."),
      changed: false,
    });
    expect(writeSpy).not.toHaveBeenCalled();
  });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("remoteclaw.internal."),
      expectedZoneText,
      "utf-8",
    );
  });
