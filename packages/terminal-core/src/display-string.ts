...
/** Resolve the underlying OS home before applying RemoteClaw overrides. */
...
/** Resolve raw home with REMOTECLAW_HOME tilde expansion. */
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.REMOTECLAW_HOME?.trim();
  return explicitHome ? { home, prefix: "$REMOTECLAW_HOME" } : { home, prefix: "~" };
}
...
/** Replace the effective home path with "~" or "$REMOTECLAW_HOME" for terminal display. */
