...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-remoteclaw-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-remoteclaw-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-remoteclaw-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function resolveBrowserFetchOperatorHint(url: string): string {
  ...
  return isLocal
    ? `Restart the RemoteClaw gateway (RemoteClaw.app menubar, or \`${formatCliCommand("remoteclaw gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      appendBrowserToolModelHint(
        `Can't reach the RemoteClaw browser control service (timed out after ${timeoutMs}ms). ${operatorHint}`,
      ),
    );
  }
  return new Error(
    appendBrowserToolModelHint(
      `Can't reach the RemoteClaw browser control service. ${operatorHint} (${msg})`,
    ),
  );
}
