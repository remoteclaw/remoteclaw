...
describe("finalizeOnboardingWizard", () => {
  ...
  it("resolves gateway password SecretRef for probe and TUI", async () => {
    ...
    process.env.REMOTECLAW_GATEWAY_PASSWORD = "resolved-gateway-password"; // pragma: allowlist secret
  ...
  it("does not persist resolved SecretRef token in daemon install plan", async () => {
    ...
    // Gateway install token resolution is gutted in this fork — the fork's
    // onboarding does not invoke resolveGatewayInstallToken. The remaining
    // assertions still verify that the build plan omits the token.
