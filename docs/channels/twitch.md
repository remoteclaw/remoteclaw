...
Twitch chat support via IRC connection. RemoteClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
<Steps>
  ...
  <Step title="Configure the token">
    - Env: `REMOTECLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "remoteclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use REMOTECLAW_TWITCH_ACCESS_TOKEN env var)
...
<Tabs>
  <Tab title="Env var (default account only)">
    ```bash
    REMOTECLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
    ```
  </Tab>
  <Tab title="Config">
    ```json5
