...
/**
 * Return a path under `~/.remoteclaw/qqbot`, creating it on demand.
 */
export function getQQBotDataDir(...subPaths: string[]): string {
  const dir = path.join(getHomeDir(), ".remoteclaw", "qqbot", ...subPaths);
...
/**
 * Return a path under `~/.remoteclaw/media/qqbot`, creating it on demand.
 *
 * Unlike `getQQBotDataDir`, this lives under RemoteClaw's core media allowlist so
...
export function getQQBotMediaDir(...subPaths: string[]): string {
  const dir = path.join(getHomeDir(), ".remoteclaw", "media", "qqbot", ...subPaths);
  if (!fs.existsSync(dir)) {
    fs.mkdirSync(dir, { recursive: true });
  }
  return dir;
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const workspaceRoot = path.join(homeDir, ".remoteclaw", "workspace", "qqbot");
