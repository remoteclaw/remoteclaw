...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  // Explicit REMOTECLAW_STATE_DIR overrides take precedence (including
  // per-test mkdtemp dirs in this module's test suite).
  if (env.REMOTECLAW_STATE_DIR?.trim()) {
    return resolveStateDir(env);
  }
  // Default test isolation: per-pid tmpdir, no bleed into real ~/.remoteclaw.
  // Use resolvePreferredRemoteClawTmpDir + string concat (mirrors
  ...
  if (env.VITEST || env.NODE_ENV === "test") {
    const name = "remoteclaw-vitest-" + process.pid;
    return path.join(resolvePreferredRemoteClawTmpDir(), name);
  }
  // Canonical RemoteClaw state dir: honors `~` expansion + legacy/new
