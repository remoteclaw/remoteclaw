...
describe("completion-runtime", () => {
  ...
  it("installs PowerShell completion into the concrete profile path", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-completion-state-bob's-"));

    try {
      await withEnvAsync({ HOME: homeDir, REMOTECLAW_STATE_DIR: stateDir }, async () => {
        const cachePath = resolveCompletionCachePath("powershell", "remoteclaw");
        await fs.mkdir(path.dirname(cachePath), { recursive: true });
        await fs.writeFile(cachePath, "# powershell completion\n", "utf-8");

        await installCompletion("powershell", true, "remoteclaw");

        const profilePath = resolveCompletionProfilePath("powershell");
        const profile = await fs.readFile(profilePath, "utf-8");
        expect(profile).toBe(`# RemoteClaw Completion\n. '${cachePath.replace(/'/g, "''")}'\n`);
      });
    } finally {
