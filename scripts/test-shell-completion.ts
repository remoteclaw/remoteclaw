 ...
 * `remoteclaw update`. Use it to verify the completion installation flow
...
\`remoteclaw update\`, \`remoteclaw doctor\`, and \`remoteclaw onboard\`.
...
async function main() {
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`remoteclaw completion --install\` later to enable.`));
    return;
  }
