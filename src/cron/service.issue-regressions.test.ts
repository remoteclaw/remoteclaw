...
describe("Cron issue regressions", () => {
  ...
  it.skip("#24355: one-shot job retries on transient error, then succeeds", async () => {
    const store = await makeStorePath();
  ...
  it.skip("#24355: one-shot job disabled after max transient retries", async () => {
    const store = await makeStorePath();
    const scheduledAt = Date.parse("2026-02-06T10:00:00.000Z");
  ...
  it.skip("#24355: one-shot job respects cron.retry config", async () => {
    const store = await makeStorePath();
    const scheduledAt = Date.parse("2026-02-06T10:00:00.000Z");
  ...
  it.skip("#24355: deleteAfterRun:true one-shot job is deleted after successful retry", async () => {
    const store = await makeStorePath();
    const scheduledAt = Date.parse("2026-02-06T10:00:00.000Z");
  ...
  it("respects abort signals while retrying main-session wake-now heartbeat runs", async () => {
    ...
    const state = createCronServiceState({
      cronEnabled: true,
      storePath: "/tmp/remoteclaw-cron-abort-test/jobs.json",
      log: noopLogger,
      nowMs: () => Date.now(),
      enqueueSystemEvent,
      requestHeartbeatNow,
