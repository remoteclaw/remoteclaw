...
This iOS app is super-alpha and internal-use only. It connects to an RemoteClaw Gateway as a `role: node` on iPhone and iPad.
...
The iOS app is not a Codex Computer Use backend. Computer Use and `cua-driver mcp` are macOS desktop-control paths; iOS exposes device capabilities as RemoteClaw node commands through the gateway. Agents can drive the iPhone or iPad canvas, camera, screen, location, voice, and other node capabilities with `node.invoke`, subject to iOS foreground/background limits.
