...
function hasConfiguredEmbeddedHarnessRuntime(cfg: RemoteClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: RemoteClawConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: RemoteClawConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: RemoteClawConfig, plugin: PluginManifestRecord): boolean {
...
function hasConfiguredWebSearchPluginEntry(cfg: RemoteClawConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: RemoteClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: RemoteClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: RemoteClawConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: RemoteClawConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: RemoteClawConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: RemoteClawConfig): boolean {
  const entries = cfg.plugins?.entries;
...
function hasPluginEntries(cfg: RemoteClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithEntries(cfg: RemoteClawConfig): boolean {
  return Array.isArray(cfg.plugins?.allow) && cfg.plugins.allow.length > 0 && hasPluginEntries(cfg);
}

function hasConfiguredProviderModelOrHarness(cfg: RemoteClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function configMayNeedPluginManifestRegistry(cfg: RemoteClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: RemoteClawConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
