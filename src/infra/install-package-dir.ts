...
const STAGED_NPM_PROJECT_CONFIG_PREFIX = ".remoteclaw-install-hidden-npmrc-";
...
export async function installPackageDir(params: {
  ...
  try {
    ...
    stageDir = await fs.mkdtemp(path.join(installBaseRealPath, ".remoteclaw-install-stage-"));
  ...
  if (params.mode === "update" && (await fileExists(canonicalTargetDir))) {
    const backupRoot = path.join(installBaseRealPath, ".remoteclaw-install-backups");
