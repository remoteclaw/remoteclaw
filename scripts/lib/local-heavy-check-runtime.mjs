...
export function acquireLocalHeavyCheckLockSync(params) {
  ...
  const pollMs = readPositiveInt(env.REMOTECLAW_HEAVY_CHECK_LOCK_POLL_MS, DEFAULT_LOCK_POLL_MS);
...
function readLocalCheckMode(env) {
  ...
  // into full-speed runs explicitly with REMOTECLAW_LOCAL_CHECK_MODE=full.
  return "throttled";
}
