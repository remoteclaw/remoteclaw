...
export async function installChromeExtension(opts?: {
  ...
  if (!hasManifest(src)) {
    throw new Error("Bundled Chrome extension is missing. Reinstall RemoteClaw and try again.");
  }
      ...
      defaultRuntime.error(
        info(
          [
            ...
            `- Pin “RemoteClaw Browser Relay”, then click it on the tab (badge shows ON)`,
            "",
            `${theme.muted("Docs:")} ${formatDocsLink("/tools/chrome-extension", "docs.remoteclaw.org/tools/chrome-extension")}`,
          ].join("\n"),
        ),
      );
    });
        ...
        defaultRuntime.error(
          danger(
            [
              ...
              `Docs: ${formatDocsLink("/tools/chrome-extension", "docs.remoteclaw.org/tools/chrome-extension")}`,
            ].join("\n"),
          ),
        );
        defaultRuntime.exit(1);
