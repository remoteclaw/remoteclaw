    ...
    try {
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "remoteclaw", messages: [{ role: "user", content: "hi" }] },
          { "x-remoteclaw-session-key": "agent:main:subagent:spoofed" },
        );
        expect(res.status).toBe(400);
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
        expect(json.error?.message).toBe(
          "`x-remoteclaw-session-key` cannot use reserved internal session namespaces.",
        );
        expect(agentCommand).toHaveBeenCalledTimes(0);
      }
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from RemoteClaw.");
      }
    ...
    try {
      ...
      const res = await fetch(`http://127.0.0.1:${port}/v1/chat/completions`, {
        ...
        headers: {
          ...
          "content-type": "application/json",
          "x-remoteclaw-scopes": "operator.approvals",
        },
        body: JSON.stringify({
          model: "remoteclaw",
          messages: [{ role: "user", content: "hi" }],
