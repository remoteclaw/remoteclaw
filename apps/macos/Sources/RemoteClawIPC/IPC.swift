...
public struct CanvasShowResult: Codable, Sendable {
    /// Session directory on disk (e.g. `~/Library/Application Support/RemoteClaw/canvas/<session>/`).
...
public let controlSocketPath: String = {
    ...
    return home
        .appendingPathComponent("Library/Application Support/RemoteClaw/control.sock")
        .path
}()
