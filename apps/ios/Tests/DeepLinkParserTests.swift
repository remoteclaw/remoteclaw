...
@Suite struct DeepLinkParserTests {
    ...
    @Test func parseRejectsEmptyMessage() {
        let url = URL(string: "remoteclaw://agent?message=%20%20%0A")!
        #expect(DeepLinkParser.parse(url) == nil)
    }
    ...
    @Test func parseGatewayLinkParsesCommonFields() {
        let url = URL(
            string: "remoteclaw://gateway?host=remoteclaw.local&port=18789&tls=1&token=abc&password=def")!
        #expect(
            DeepLinkParser.parse(url) == .gateway(
                .init(
                    host: "remoteclaw.local",
                    port: 18789,
                    tls: true,
                    bootstrapToken: nil,
                    token: "abc",
    ...
    @Test func parseGatewayLinkRejectsInsecureNonLoopbackWs() {
        let url = URL(
            string: "remoteclaw://gateway?host=attacker.example&port=18789&tls=0&token=abc")!
        #expect(DeepLinkParser.parse(url) == nil)
    }

    @Test func parseGatewayLinkAllowsPrivateLanWs() {
        let url = URL(
            string: "remoteclaw://gateway?host=remoteclaw.local&port=18789&tls=0&token=abc")!
        #expect(
            DeepLinkParser.parse(url) == .gateway(
                .init(
                    host: "remoteclaw.local",
    ...
    @Test func parseGatewayLinkRejectsInsecurePrefixBypassHost() {
        let url = URL(
            string: "remoteclaw://gateway?host=127.attacker.example&port=18789&tls=0&token=abc")!
        #expect(DeepLinkParser.parse(url) == nil)
    }

    @Test func parseGatewayLinkRejectsInvalidPort() {
        let url = URL(string: "remoteclaw://gateway?host=gateway.example.com&port=70000&tls=1")!
        #expect(DeepLinkParser.parse(url) == nil)
    }

    @Test func parseGatewayLinkRejectsMalformedPort() {
        let url = URL(string: "remoteclaw://gateway?host=gateway.example.com&port=not-a-port&tls=1")!
        #expect(DeepLinkParser.parse(url) == nil)
    }
    ...
    @Test func parseGatewaySetupCodeAllowsPrivateLanWs() {
        let payload = #"{"url":"ws://remoteclaw.local:18789","bootstrapToken":"tok"}"#
        let link = GatewayConnectDeepLink.fromSetupCode(setupCode(from: payload))
