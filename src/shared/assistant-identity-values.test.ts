...
describe("shared/assistant-identity-values", () => {
  ...
  it("trims values and preserves strings within the limit", () => {
    ...
    expect(coerceIdentityValue("  RemoteClaw  ", 10)).toBe("RemoteClaw");
  });

  it("truncates overlong trimmed values at the exact limit", () => {
    expect(coerceIdentityValue("  RemoteClaw Assistant  ", 10)).toBe("RemoteClaw");
  });

  it("returns an empty string when truncating to a zero-length limit", () => {
    expect(coerceIdentityValue("  RemoteClaw  ", 0)).toBe("");
    expect(coerceIdentityValue("  RemoteClaw  ", -1)).toBe("RemoteCla");
  });
});
