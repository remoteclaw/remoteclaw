# RemoteClaw iOS (Super Alpha)

This iOS app is super-alpha and internal-use only. The first public App Store release targets iPhone and connects to an RemoteClaw Gateway as a `role: node`.
...
- `xcodegen`
- `fastlane`
- Apple account signed into Xcode for the canonical RemoteClaw team (`FWJYW4S8P8`)
- Fastlane Apple Developer Portal session for the canonical RemoteClaw team when creating bundle IDs or enabling services
- Release-owner access to the encrypted signing repo password (`MATCH_PASSWORD`)
- App Store Connect app already created for `org.remoteclaw.app`
...
- Local development uses the canonical `org.remoteclaw.app*` bundle IDs when the RemoteClaw team is available, and unique `org.remoteclaw.app.test.*` bundle IDs only for non-canonical fallback teams.
- App Store release uses canonical `org.remoteclaw.app*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/AppStoreRelease.xcconfig`.
...
- App Store release also switches the app to `RemoteClawPushMode=appStore`, which derives relay transport, official distribution, the canonical production relay, production APNs, production relay profile, `appleStrict` proof, and the App-Attest-capable entitlement file.
...
- App Store release builds use the canonical hosted relay at `https://ios-push-relay.remoteclaw.org`.
...
3. Confirm the App Store Connect app and Apple Developer identifiers/capabilities exist for:
   - `org.remoteclaw.app`
   - `org.remoteclaw.app.share`
   - `org.remoteclaw.app.activitywidget`
   - `org.remoteclaw.app.watchkitapp`
...
7. Expected outputs after a successful run:
   - `apps/ios/build/app-store/RemoteClaw-<version>.ipa`
   - `apps/ios/build/app-store/RemoteClaw-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/RemoteClaw.entitlements` derives `aps-environment` from the active build configuration/signing override.
- App Attest relay builds use `apps/ios/Sources/RemoteClawAppAttest.entitlements`; local/direct builds do not require App Attest provisioning.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual Debug builds default to `RemoteClawPushMode=localSandbox`, direct APNs registration, and a development `aps-environment` entitlement. Local/manual Release builds default to `RemoteClawPushMode=localProduction` and direct production APNs registration.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `REMOTECLAW_APNS_TEAM_ID`, `REMOTECLAW_APNS_KEY_ID`, and either `REMOTECLAW_APNS_PRIVATE_KEY_P8` or `REMOTECLAW_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.remoteclaw/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `REMOTECLAW_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to sandbox APNs through `RemoteClawPushMode=localSandbox`; Release builds default to production APNs through `RemoteClawPushMode=localProduction`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `remoteclaw.json`. `REMOTECLAW_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine RemoteClaw official builds and to ensure a
...
The iOS app is not a Codex Computer Use backend. Computer Use and `cua-driver mcp` are macOS desktop-control paths; iOS exposes device capabilities as RemoteClaw node commands through the gateway. Agents can drive the iPhone canvas, camera, screen, location, voice, and other node capabilities with `node.invoke`, subject to iOS foreground/background limits.
