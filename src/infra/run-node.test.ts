...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-run-node-"));
  try {
...
describe("run-node script", () => {
    ...
    async () => {
      await withTempDir(async (tmp) => {
        ...
        const exitCode = await runNodeMain({
          ...
          env: {
            ...process.env,
            REMOTECLAW_FORCE_BUILD: "1",
            REMOTECLAW_RUNNER_LOG: "0",
          },
          spawn,
  ...
  it("skips rebuilding when dist is current and the source tree is clean", async () => {
    await withTempDir(async (tmp) => {
      ...
      await fs.writeFile(packageJsonPath, '{"name":"remoteclaw-test"}\n', "utf-8");
      ...
      expect(exitCode).toBe(0);
      expect(spawnCalls).toEqual([[process.execPath, "remoteclaw.mjs", "status"]]);
    });
  });
