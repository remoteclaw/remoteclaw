...
describe("channel plugin catalog", () => {
  ...
  it("preserves plugin ids when they differ from channel ids", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-channel-catalog-state-"));
  ...
  it("uses the provided env for external catalog path resolution", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-catalog-home-"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@remoteclaw/env-demo-channel",
            remoteclaw: {
              channel: {
                id: "env-demo-channel",
              ...
              install: {
                npmSpec: "@remoteclaw/env-demo-channel",
              },
            },
          },
        ],
  ...
  it("uses the provided env for default catalog paths", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@remoteclaw/default-env-demo",
            remoteclaw: {
              channel: {
                id: "default-env-demo",
              ...
              install: {
                npmSpec: "@remoteclaw/default-env-demo",
              },
            },
          },
        ],
  ...
  it("includes bundled metadata-only channel entries even when the runtime entrypoint is omitted", () => {
    const packageRoot = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-bundled-catalog-"));
    ...
    fs.writeFileSync(
      path.join(bundledDir, "remoteclaw.plugin.json"),
    ...
    expect(entry?.install.npmSpec).toBe("@remoteclaw/whatsapp");
    expect(entry?.pluginId).toBe("whatsapp");
  });

  it("includes shipped official channel catalog entries when bundled metadata is omitted", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-official-catalog-"));
...
async function listDirectoryEntriesWithDefaults(listFn: DirectoryListFn, cfg: RemoteClawConfig) {
  return await listFn({
    cfg,
    accountId: "default",
    query: null,
