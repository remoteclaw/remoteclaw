...
Voice calls for RemoteClaw via a plugin. Supports outbound notifications and
...
- Use `remoteclaw voicecall ...` or the `voice_call` tool
...
`remoteclaw voicecall setup --json` for scripts.
...
- If older configs still use `provider: "log"`, `twilio.from`, or legacy `streaming.*` OpenAI keys, run `remoteclaw doctor --fix` to rewrite them.
...
- Runtime fallback still accepts those old voice-call keys for now, but the rewrite path is `remoteclaw doctor --fix` and the compat shim is temporary.
...
- Voice Call exposes the shared `remoteclaw_agent_consult` realtime tool by
  default. The realtime model can call it when the caller asks for deeper
  reasoning, current information, or normal RemoteClaw tools.
...
Legacy keys are still auto-migrated by `remoteclaw doctor --fix`:
