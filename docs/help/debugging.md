...
RemoteClaw keeps `src/cli/debug-timing.ts` as a small helper for local
...
`remoteclaw models list`, a temporary patch in
...
```bash
REMOTECLAW_DEBUG_TIMING=1 pnpm remoteclaw models list --all --provider moonshot
```
...
```bash
REMOTECLAW_DEBUG_TIMING=json pnpm remoteclaw models list --all --provider moonshot \
