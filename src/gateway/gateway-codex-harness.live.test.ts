...
const CODEX_HARNESS_LIVE = isTruthyEnvValue(process.env.REMOTECLAW_LIVE_CODEX_HARNESS);
const CODEX_HARNESS_DEBUG = isTruthyEnvValue(process.env.REMOTECLAW_LIVE_CODEX_HARNESS_DEBUG);
const CODEX_HARNESS_IMAGE_PROBE = isTruthyEnvValue(
  process.env.REMOTECLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE,
);
const CODEX_HARNESS_MCP_PROBE = isTruthyEnvValue(process.env.REMOTECLAW_LIVE_CODEX_HARNESS_MCP_PROBE);
const CODEX_HARNESS_AUTH_MODE =
  process.env.REMOTECLAW_LIVE_CODEX_HARNESS_AUTH === "api-key" ? "api-key" : "codex-auth";
...
function snapshotEnv(): EnvSnapshot {
  return {
    agentRuntime: process.env.REMOTECLAW_AGENT_RUNTIME,
    configPath: process.env.REMOTECLAW_CONFIG_PATH,
    gatewayToken: process.env.REMOTECLAW_GATEWAY_TOKEN,
...
function restoreEnv(snapshot: EnvSnapshot): void {
  restoreEnvVar("REMOTECLAW_AGENT_RUNTIME", snapshot.agentRuntime);
  restoreEnvVar("REMOTECLAW_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("REMOTECLAW_GATEWAY_TOKEN", snapshot.gatewayToken);
...
describeLive("gateway live (Codex harness)", () => {
  it(
    ...
    async () => {
      const modelKey = process.env.REMOTECLAW_LIVE_CODEX_HARNESS_MODEL ?? DEFAULT_CODEX_MODEL;
      const { clearRuntimeConfigSnapshot } = await import("../config/config.js");
      const { startGatewayServer } = await import("./server.js");

      const previousEnv = snapshotEnv();
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-live-codex-harness-"));
      const stateDir = path.join(tempDir, "state");
      const workspace = await createLiveWorkspace(tempDir);
      const configPath = path.join(tempDir, "remoteclaw.json");
      const token = `test-${randomUUID()}`;
      const port = await getFreeGatewayPort();

      clearRuntimeConfigSnapshot();
      process.env.REMOTECLAW_AGENT_RUNTIME = "codex";
      process.env.REMOTECLAW_AGENT_HARNESS_FALLBACK = "none";
