...
describe("/session idle and /session max-age", () => {
  ...
  it.skip("sets idle timeout for the focused session", async () => {
    vi.useFakeTimers();
  ...
  it.skip("shows active idle timeout when no value is provided", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-02-20T00:00:00.000Z"));
  ...
  it.skip("sets max age for the focused session", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-02-20T00:00:00.000Z"));
