...
import { parseReleaseVersion } from "../remoteclaw-npm-release-check.ts";
...
export const REMOTECLAW_PLUGIN_NPM_REPOSITORY_URL = "https://github.com/remoteclaw/remoteclaw";
...
export function collectPublishablePluginPackageErrors(
  ...
  const installNpmSpec = normalizeOptionalString(packageJson.remoteclaw?.install?.npmSpec);
  const repositoryUrl =
  ...
  if (repositoryUrl !== REMOTECLAW_PLUGIN_NPM_REPOSITORY_URL) {
    errors.push(
      `package.json repository.url must be "${REMOTECLAW_PLUGIN_NPM_REPOSITORY_URL}" so npm provenance can validate GitHub trusted publishing; found "${repositoryUrl || "<missing>"}".`,
    );
  }
  if (!packageVersion) {
  ...
  if (extensions.some((entry) => typeof entry !== "string" || !entry.trim())) {
    errors.push("remoteclaw.extensions must contain only non-empty strings.");
  }
  if (!installNpmSpec) {
    errors.push("remoteclaw.install.npmSpec must be a non-empty string for publishable plugins.");
  }
  errors.push(
...
export function collectPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    publishable.push({
      ...
      installNpmSpec: normalizeOptionalString(packageJson.remoteclaw?.install?.npmSpec),
    });
  }
...
function isPluginVersionPublished(packageName: string, version: string): boolean {
  const tempDir = mkdtempSync(join(tmpdir(), "remoteclaw-plugin-npm-view-"));
