...
extension SettingsProTab {
        ...
        color: Color) -> some View
    {
        Section {
            HStack(spacing: 12) {
                ...
                Spacer(minLength: 8)
                Text(value)
                    .font(RemoteClawType.subheadMedium)
    ...
    func refreshLocationPermissionSummary(desiredMode modeOverride: RemoteClawLocationMode? = nil) {
        let mode = modeOverride ?? RemoteClawLocationMode(rawValue: self.locationModeRaw) ?? .off
        ...
        Task {
            ...
            let latestMode = modeOverride ?? RemoteClawLocationMode(rawValue: self.locationModeRaw) ?? .off
            self.locationPermissionSummary = LocationPermissionSummary(
        ...
        previous: String) async
    {
        ...
        if granted {
            ...
            self.refreshLocationPermissionSummary(
                desiredMode: RemoteClawLocationMode(rawValue: previous) ?? .off)
        }
    }
    ...
    func handleNotificationAction() {
        ...
        if PushBuildConfig.current.usesRemoteClawHostedRelay {
    ...
    var gatewayStatusColor: Color {
        if self.appModel.isAppleReviewDemoModeEnabled { return RemoteClawBrand.accent }
        return self.gatewayConnected ? RemoteClawBrand.ok : .secondary
    }
    ...
    var approvalEmptyDetail: String {
        ...
        if self.notificationsNeedAttention {
            return "Foreground approvals still appear while RemoteClaw is connected."
        }
    ...
    var gatewayTalkConfigColor: Color {
        ...
        return self.appModel.talkMode.gatewayTalkConfigLoaded ? RemoteClawBrand.ok : .secondary
    }
    ...
    var notificationStatusDetail: String {
        ...
        case .allowed:
            "RemoteClaw can show approval prompts and event alerts when the app is not active."
        case .notAllowed:
        ...
        case .unknown:
            "RemoteClaw cannot determine the current notification permission state."
        }
    }
    ...
    var notificationRelayDetail: String {
        if PushBuildConfig.current.usesRemoteClawHostedRelay {
            ...
            } ?? "ios-push-relay.remoteclaw.org"
            return """
            This build uses RemoteClaw's hosted push relay at \(host) for notification \
            delivery data.
            """
        }
        return "This build is not configured to use RemoteClaw's hosted push relay."
    }

    var notificationRelayDisclosureMessage: String {
        "Enabling this sends delivery data through RemoteClaw's hosted push relay."
    }
}
