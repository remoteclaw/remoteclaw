...
<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), RemoteClaw supersedes the previous pending entry with a new `requestId`. Run `remoteclaw devices list` right before approval to get the current id.
</Note>

Approval behavior:

- If the device is already paired and requests broader scopes or role, RemoteClaw keeps the existing approval and creates a new pending upgrade request. Compare `Requested` vs `Approved` in `remoteclaw devices list`, or preview with `--latest`, before approving.
...
Paperclip agents connecting through the `remoteclaw_gateway` adapter go through the same first-run device pairing approval as any other new client. If Paperclip reports `remoteclaw_gateway_pairing_required`, approve the pending device and retry.

```bash
remoteclaw devices approve --latest
```

The preview prints the exact `remoteclaw devices approve <requestId>` command; verify the details, then rerun that command with the request ID to approve it. For a remote gateway or explicit credentials, pass the same options while previewing and approving:
