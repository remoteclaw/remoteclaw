...
- If you install plugins from npm (`remoteclaw plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.remoteclaw/extensions/<pluginId>/` (or `$OPENCLAW_STATE_DIR/extensions/<pluginId>/`).
...
Assume anything under `~/.remoteclaw/` (or `$OPENCLAW_STATE_DIR/`) may contain secrets or private data:
