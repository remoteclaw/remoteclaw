// Validates the current runtime against RemoteClaw's Node engine floor.
...
/** Returns whether a detected runtime meets RemoteClaw's minimum runtime contract. */
...
/** Checks a Node version label against RemoteClaw's supported Node version range. */
...
export function assertSupportedRuntime(
  ...
  const requirement =
    details.kind === "bun"
      ? "remoteclaw cannot run under Bun because the runtime does not provide node:sqlite."
      : "remoteclaw requires Node >=22.22.3 <23, >=24.15.0 <25, or >=25.9.0.";
  const retryHint =
    details.kind === "bun"
      ? "Run RemoteClaw with Node; Bun remains supported for installs and package scripts."
      : "Upgrade Node and re-run remoteclaw.";
