...
describe("argv-invocation", () => {
  it("resolves root help and empty command path", () => {
    expect(resolveCliArgvInvocation(["node", "remoteclaw", "--help"])).toEqual({
  ...
  it("resolves command path and primary with root options", () => {
    expect(
      resolveCliArgvInvocation(["node", "remoteclaw", "--profile", "work", "gateway", "status"]),
    ).toEqual({
      argv: ["node", "remoteclaw", "--profile", "work", "gateway", "status"],
