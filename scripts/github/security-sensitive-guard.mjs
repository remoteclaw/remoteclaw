...
export const securitySensitiveGuardMarker = "<!-- remoteclaw:security-sensitive-guard -->";
...
const securityTeamSlug = process.env.REMOTECLAW_SECURITY_TEAM_SLUG ?? "remoteclaw-secops";
...
export function renderSecuritySensitiveAwarenessComment(changes) {
  return [
    ...
    "- If this PR intentionally needs the change, a repository admin or member of `@remoteclaw/remoteclaw-secops` must approve the exact head SHA.",
  ].join("\n");
}
...
export function renderAuthorizedSecuritySensitiveComment(override) {
  const lines = [
    ...
    "",
    "This PR includes security-sensitive file changes. A repository admin or member of `@remoteclaw/remoteclaw-secops` authorized this exact head SHA with `/allow-security-sensitive-change`.",
    "",
...
export function renderTrustedSecuritySensitiveComment({ actor, headSha, changes }) {
  return [
    ...
    "",
    "This PR includes security-sensitive file changes. The guard is informational because the PR author is a repository admin or a member of `@remoteclaw/remoteclaw-secops`.",
    "",
...
export function renderBlockedSecuritySensitiveComment({ headSha, changes }) {
  return [
    ...
    "",
    "RemoteClaw does not accept security-sensitive file changes through PRs unless a repository admin or security explicitly authorizes the current head SHA.",
    "",
    "Detected security-sensitive changes:",
    ...renderChangedFileLines(changes),
    "",
    "If this PR intentionally needs these changes, ask a repository admin or member of `@remoteclaw/remoteclaw-secops` to comment:",
    "",
    "```text",
...
export function githubApi(token, options = {}) {
  return createGitHubApi(token, {
    ...options,
    userAgent: "remoteclaw-security-sensitive-guard",
  });
}
...
async function main() {
  ...
  const explicitSecurityApprovers = securityApproverSet(process.env.REMOTECLAW_SECURITY_APPROVERS);
  const trustedCommentAuthors = securitySensitiveGuardCommentAuthors(
    process.env.REMOTECLAW_SECURITY_SENSITIVE_GUARD_COMMENT_BOTS,
  );
  ...
  const mode = process.env.REMOTECLAW_SECURITY_SENSITIVE_GUARD_MODE ?? "enforce";
