...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "remoteclaw", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "remoteclaw", version: "2.3.4" });
  ...
  it("ignores non-remoteclaw package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
