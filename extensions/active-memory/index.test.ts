...
import type { RemoteClawPluginApi } from "remoteclaw/plugin-sdk/plugin-entry";
...
describe("active-memory plugin", () => {
  ...
  const api: any = {
    ...
    runtime: {
      agent: {
        runEmbeddedPiAgent,
        session: {
          resolveStorePath: vi.fn(() => "/tmp/remoteclaw-session-store.json"),
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-active-memory-test-"));
    configFile = {
      plugins: {
        entries: {
          "active-memory": {
    ...
    plugin.register(api as unknown as RemoteClawPluginApi);
  });
  ...
  it("keeps subagent transcripts off disk by default by using a temp session file", async () => {
    const mkdtempSpy = vi
      .spyOn(fs, "mkdtemp")
      .mockResolvedValue("/tmp/remoteclaw-active-memory-temp");
    ...
    expect(runEmbeddedPiAgent.mock.calls.at(-1)?.[0]?.sessionFile).toBe(
      "/tmp/remoteclaw-active-memory-temp/session.jsonl",
    );
    expect(rmSpy).toHaveBeenCalledWith("/tmp/remoteclaw-active-memory-temp", {
      recursive: true,
      force: true,
    });
  });
