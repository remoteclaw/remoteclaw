# fastlane setup (RemoteClaw Android)
...
Create a Google Play service account JSON key with Google Play Developer API access, then grant that service account access to the RemoteClaw app in Play Console.
...
```bash
GOOGLE_PLAY_PACKAGE_NAME=org.remoteclaw.app
```
...
REMOTECLAW_ANDROID_UPLOAD_KEYSTORE=<path-to-upload-keystore.jks> \
REMOTECLAW_ANDROID_SIGNING_PROPERTIES=<path-to-android-signing.properties> \
...
```properties
REMOTECLAW_ANDROID_STORE_PASSWORD=<store-password>
REMOTECLAW_ANDROID_KEY_ALIAS=<upload-key-alias>
REMOTECLAW_ANDROID_KEY_PASSWORD=<key-password>
```
...
- Stable GitHub Release APK publication is separate from Google Play: `RemoteClaw Release Publish` dispatches `.github/workflows/android-release.yml`, whose protected `android-release` environment provides `MATCH_PASSWORD`; the repository GitHub App reads the encrypted signing repo.
