...
`remoteclaw cron run` returns as soon as the manual run is queued. Successful responses include `{ ok: true, enqueued: true, runId }`. Use `remoteclaw cron runs --id <job-id>` to follow the eventual outcome.

<Note>
`remoteclaw cron run <job-id>` force-runs by default. Use `--due` to keep the older "only run if due" behavior.
</Note>
