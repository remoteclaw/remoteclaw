...
export const FIELD_HELP: Record<string, string> = {
  ...
  "wizard.lastRunVersion":
    "RemoteClaw version recorded at the time of the most recent wizard run on this config. Use this when diagnosing behavior differences across version-to-version setup changes.",
  ...
  "gateway.handshakeTimeoutMs":
    "Pre-auth Gateway WebSocket handshake timeout in milliseconds. Use higher values on loaded or low-powered hosts where local clients can connect during startup warmup. REMOTECLAW_HANDSHAKE_TIMEOUT_MS still takes precedence.",
  ...
  "gateway.tailscale.serviceName":
    'Optional Tailscale Service name for Serve mode, such as "svc:remoteclaw". The value must use Tailscale\'s svc:<dns-label> format. When set, RemoteClaw passes it to tailscale serve --service and reports the derived Service URL.',
  "gateway.tailscale.preserveFunnel":
    "When mode='serve' and an externally configured Tailscale Funnel route already covers the gateway port, skip re-applying tailscale serve on startup. Lets operators keep Funnel exposure managed outside RemoteClaw without losing it across gateway restarts.",
  "gateway.remote":
  ...
  "talk.realtime.instructions":
    "Additional system instructions appended to RemoteClaw's built-in realtime Talk prompt. Use this for voice style, tone, and other provider-facing realtime behavior while keeping agent-consult guidance intact.",
  ...
  "talk.realtime.consultRouting":
    "Gateway relay fallback for final user transcripts when the realtime provider skips remoteclaw_agent_consult. provider-direct preserves provider replies; force-agent-consult routes through RemoteClaw.",
  ...
  "agents.list[].runtime":
    "Optional runtime descriptor for this agent. Use embedded for default RemoteClaw execution or acp for external ACP harness defaults.",
  "agents.list[].runtime.type":
    'Runtime type for this agent: "embedded" (default RemoteClaw runtime) or "acp" (ACP harness defaults).',
  ...
  "agents.list[].runtime.acp.agent":
    "Optional ACP harness agent id to use for this RemoteClaw agent (for example codex, claude, cursor, gemini, remoteclaw).",
  ...
  "browser.profiles.*.driver":
    'Per-profile browser driver mode. Use "remoteclaw" (or legacy "clawd") for CDP-based profiles, or use "existing-session" for Chrome DevTools MCP attachment on the selected host or browser node.',
  ...
  "discovery.wideArea.domain":
    "Optional unicast DNS-SD domain for wide-area discovery, such as remoteclaw.internal. Use this when you intentionally publish gateway discovery beyond local mDNS scopes.",
  "discovery.mdns":
  ...
  "tools.toolSearch":
    "Compact large RemoteClaw, MCP, and client tool catalogs behind one search/call surface. Set to true for the default code bridge or use the object form to choose the structured fallback.",
  "tools.toolSearch.enabled":
    "Enables Tool Search. When on, RemoteClaw hides large tool catalogs behind `tool_search_code` or structured search/describe/call tools during PI runs.",
  ...
  "tools.codeMode":
    "Generic RemoteClaw code mode. When enabled, agent runs expose only `exec` and `wait` to the model and hide normal tools behind a QuickJS-WASI catalog bridge.",
  "tools.codeMode.enabled":
    "Enables generic code mode. Default is off. When explicitly enabled, RemoteClaw fails closed if the runtime is unavailable instead of exposing the full tool list.",
  ...
  "agents.defaults.sandbox.browser.network":
    "Docker network for sandbox browser containers (default: remoteclaw-sandbox-browser). Avoid bridge if you need stricter isolation.",
  ...
  "gateway.controlUi.allowExternalEmbedUrls":
    "DANGEROUS toggle that allows hosted embeds to load absolute external http(s) URLs. Keep this off unless your Control UI intentionally embeds trusted third-party pages; hosted /__remoteclaw__/canvas and /__remoteclaw__/a2ui documents do not need it.",
  ...
  "nodeHost.browserProxy.allowProfiles":
    "Optional allowlist of browser profile names exposed through node proxy routing. Leave empty to preserve the default full profile surface, including profile create/delete routes. When set, RemoteClaw enforces least-privilege profile access and blocks persistent profile create/delete through the proxy.",
  media:
  ...
  "audio.transcription.command":
    'Executable + args used to transcribe audio (first token must be a safe binary/path), for example `["whisper-cli", "--model", "small", "{{MediaPath}}"]`. Deprecated `{input}` placeholders are migrated to `{{MediaPath}}` by `remoteclaw doctor --fix`.',
  ...
  "skills.load.extraDirs":
    "Additional shared skill roots to scan at lowest precedence. Use this for sibling repos or shared skill packs that should be available without copying them into the RemoteClaw workspace.",
  ...
  "models.providers.*.agentRuntime":
    "Optional low-level agent runtime policy for this provider. Use provider/model runtime policy instead of agent-wide runtime pins; omitted/default lets RemoteClaw choose the runtime for the selected provider.",
  ...
  "models.providers.*.localService":
    "Optional on-demand local model server process for this provider. RemoteClaw probes healthUrl, starts the command when needed, waits for readiness, and then sends the model request.",
  ...
  "models.providers.*.localService.healthUrl":
    "Readiness URL probed before model requests. If omitted, RemoteClaw uses the provider baseUrl with /models appended.",
  ...
  "models.providers.*.localService.idleStopMs":
    "Milliseconds to keep an RemoteClaw-started local model server alive after the last request finishes. Set 0 to keep it alive until RemoteClaw exits.",
  ...
  proxy:
    "Operator-managed forward proxy routing for RemoteClaw runtime HTTP, HTTPS, WebSocket, and supported raw-egress paths. Use this when central egress control is part of the deployment boundary.",
  "proxy.enabled":
    "Enables operator-managed proxy routing. When enabled, RemoteClaw fails startup if no managed proxy URL is configured.",
  "proxy.proxyUrl":
  ...
  "models.providers.*.models[].mediaInput.image.preferredSidePx":
    "Preferred image resize side for balanced compression. Leave unset to use RemoteClaw's conservative default.",
  ...
  "agents.defaults.experimental.localModelLean":
    "Experimental local-model prompt trim. When enabled, RemoteClaw drops heavyweight default tools like browser, cron, and message for weaker or smaller local-model backends.",
  ...
  "agents.defaults.memorySearch.store.path":
    "Sets where the SQLite memory index is stored on disk for each agent. Keep the default `~/.remoteclaw/memory/{agentId}.sqlite` unless you need custom storage placement or backup policy alignment.",
  ...
  "memory.backend":
    'Selects the global memory engine: "builtin" uses RemoteClaw memory internals, while "qmd" uses the QMD sidecar pipeline. Keep "builtin" unless you intentionally operate QMD.',
  "memory.citations":
  ...
  "agents.defaults.agentRuntime":
    "Legacy whole-agent runtime policy. It is ignored by runtime selection; configure runtime policy on a provider or model instead. Run remoteclaw doctor --fix to remove stale values.",
  ...
  "agents.defaults.embeddedHarness":
    "Legacy whole-agent embedded harness input. Run remoteclaw doctor --fix to remove it and use provider/model runtime policy where needed.",
  ...
  "agents.list.*.agentRuntime":
    "Legacy per-agent runtime policy. It is ignored by runtime selection; configure provider/model runtime policy instead. Run remoteclaw doctor --fix to remove stale values.",
  ...
  "agents.list.*.embeddedHarness":
    "Legacy per-agent embedded harness input. Run remoteclaw doctor --fix to remove it and use provider/model runtime policy where needed.",
  ...
  "agents.defaults.compaction.midTurnPrecheck":
    "Optional Pi tool-loop precheck that detects context pressure after a tool result is appended and before the next model call. When enabled, RemoteClaw reuses existing precheck recovery to truncate tool results or compact before retrying.",
  ...
  "commands.mcp":
    "Allow /mcp chat command to manage RemoteClaw MCP server config under mcp.servers (default: false).",
  "commands.plugins":
  ...
  mcp: "Global MCP server definitions managed by RemoteClaw. Embedded Pi and other runtime adapters can consume these servers without storing them inside Pi-owned project settings.",
  "mcp.servers":
    "Named MCP server definitions. RemoteClaw stores them in its own config and runtime adapters decide which transports are supported at execution time.",
  "mcp.servers.*.codex":
    "RemoteClaw projection metadata for Codex app-server threads only. It does not affect ACP sessions or generic Codex harness config. Omit this block to keep the server available to every Codex app-server agent with Codex's default MCP approval behavior.",
  ...
  "mcp.servers.*.codex.agents":
    "Optional non-empty RemoteClaw agent ids that should receive this MCP server in Codex app-server thread config. Empty, blank, or invalid lists fail closed; when omitted, the server is projected for all Codex app-server agents.",
  ...
  "mcp.servers.*.codex.default_tools_approval_mode":
    "Codex-native spelling for the same per-server MCP tool approval mode. Prefer defaultToolsApprovalMode in RemoteClaw config.",
  ...
  "session.writeLock.acquireTimeoutMs":
    "Milliseconds to wait while acquiring a session transcript write lock before reporting the session as busy. Default: 60000; env override: REMOTECLAW_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS.",
  "session.writeLock.staleMs":
    "Milliseconds before an existing session transcript lock can be treated as stale and reclaimed. Default: 1800000; env override: REMOTECLAW_SESSION_WRITE_LOCK_STALE_MS.",
  "session.writeLock.maxHoldMs":
    "Milliseconds a held in-process session transcript lock may remain held before the watchdog releases it. Default: 300000; env override: REMOTECLAW_SESSION_WRITE_LOCK_MAX_HOLD_MS.",
  ...
  "session.maintenance.rotateBytes":
    'Deprecated and ignored. Do not use for `sessions.json` growth control; RemoteClaw no longer creates automatic rotation backups, and "remoteclaw doctor --fix" removes this key.',
  ...
  "cron.webhook":
    'Deprecated legacy fallback webhook URL used by `remoteclaw doctor --fix` to migrate old jobs with `notify=true`. Runtime delivery uses per-job `delivery.mode="webhook"` plus `delivery.to`, or `delivery.completionDestination` when preserving announce delivery.',
  "cron.webhookToken":
