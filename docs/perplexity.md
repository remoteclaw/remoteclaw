...
RemoteClaw supports Perplexity Search API as a `web_search` provider.
...
`~/.remoteclaw/remoteclaw.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.remoteclaw/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
