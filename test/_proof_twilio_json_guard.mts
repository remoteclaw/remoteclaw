...
async function proofListPayloadMalformed() {
  const { listTwilioIncomingPhoneNumbers } = await import("../extensions/sms/src/twilio.js");
  ...
  check(
    "listPayload malformed: returns [] (fail-safe)",
...
async function proofMessagingServiceMalformed() {
  const { retrieveTwilioMessagingService } = await import("../extensions/sms/src/twilio.js");
  ...
  try {
    ...
    check(
      "messagingService malformed: throws Error",
      err instanceof Error,
      `type=${err instanceof Error ? err.constructor.name : String(err)}`,
    );
    if (err instanceof Error) {
      check(
        "messagingService malformed: mentions malformed JSON",
...
async function proofListPayloadValid() {
  ...
  check(
    "listPayload valid: parsed correctly",
