...
if [[ -n "${OPENCLAW_DOCKER_AUTH_DIRS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(remoteclaw_live_collect_auth_dirs)
  ...
  while IFS= read -r auth_dir; do
    ...
    {
      remoteclaw_live_collect_auth_dirs_from_csv "${OPENCLAW_LIVE_PROVIDERS:-}"
      remoteclaw_live_collect_auth_dirs_from_csv "${OPENCLAW_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
else
  while IFS= read -r auth_dir; do
...
AUTH_DIRS_CSV="$(remoteclaw_live_join_csv "${AUTH_DIRS[@]}")"
