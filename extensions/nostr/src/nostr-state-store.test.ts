...
import { readNostrBusState, writeNostrBusState, computeSinceTimestamp } from "./nostr-state-store.js";
import { setNostrRuntime } from "./runtime.js";

async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.REMOTECLAW_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-nostr-"));
  process.env.REMOTECLAW_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.REMOTECLAW_STATE_DIR?.trim() || stateEnv.CLAWDBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".remoteclaw");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.REMOTECLAW_STATE_DIR = previous;
    }
