...
Plugins extend RemoteClaw with new capabilities: channels, model providers,
...
- For in-repo plugins: repository cloned and `pnpm install` done. Source
  checkout plugin development is pnpm-only because RemoteClaw loads bundled
...
`remoteclaw/plugin-sdk/channel-setup`. It produces a setup adapter + wizard pair
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    must be listed in `contracts.tools` so RemoteClaw can discover the owning
  ...
  <Step title="Test and publish">
    ...
    **External plugins:** validate and publish with ClawHub, then install:
    ...
    remoteclaw plugins install clawhub:@myorg/remoteclaw-my-plugin
    ```
...
RemoteClaw captures and caches the validated descriptor from the registered tool,
...
`api.registerTool(..., { optional: true })` so RemoteClaw can avoid loading that
...
Plugins can add root `remoteclaw` command groups with `api.registerCli`. Provide
`descriptors` for every top-level command root so RemoteClaw can show and route
...
```bash
remoteclaw plugins inspect demo-plugin --runtime --json
