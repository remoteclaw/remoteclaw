...
export function hasConfiguredInternalHooks(config: RemoteClawConfig): boolean {
...
export function resolveConfiguredInternalHookNames(config: RemoteClawConfig): Set<string> | null {
  const internal = config.hooks?.internal;
  if (!internal || internal.enabled === false) {
    return new Set();
  }
