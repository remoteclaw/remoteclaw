...
import { auditRemoteClawPeerDependencyLink } from "../../plugins/plugin-peer-link.js";
...
export async function runPluginPayloadSmokeCheck(params: {
  ...
  for (const [pluginId, record] of Object.entries(params.records).toSorted(([a], [b]) =>
    ...
    if (manifestDeclaresRemoteClawPeer(manifest)) {
      const peerIssue = await auditRemoteClawPeerDependencyLink({
      ...
      if (peerIssue) {
        failures.push({
          ...
          detail: `Plugin declares peerDependency "remoteclaw" but peer link audit failed: ${peerIssue.reason}.`,
        });
      }
    }

    const extensionResolution = resolvePackageExtensionEntries(manifest);
    if (extensionResolution.status === "invalid") {
      failures.push({
        pluginId,
        installPath,
        reason: "missing-extension-entry",
        detail: `Plugin extension entry validation failed: ${extensionResolution.error}`,
      });
      continue;
    }
...
function manifestDeclaresRemoteClawPeer(manifest: PackageManifest): boolean {
  const peerDependencies = (manifest as { peerDependencies?: unknown }).peerDependencies;
  return (
    typeof peerDependencies === "object" &&
    peerDependencies !== null &&
    !Array.isArray(peerDependencies) &&
    typeof (peerDependencies as Record<string, unknown>).remoteclaw === "string"
  );
}
