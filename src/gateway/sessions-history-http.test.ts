...
async function createSessionStoreFile(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-session-history-"));
  ...
  test("returns session history over direct REST", async () => {
    ...
    try {
      ...
      expect(
        (
          body.messages?.[0] as {
            __remoteclaw?: { id?: string; seq?: number };
          }
        )?.__remoteclaw,
      ).toMatchObject({
        seq: 1,
  ...
  test("supports cursor pagination over direct REST while preserving the messages field", async () => {
    ...
    try {
      ...
      const firstBody = (await firstPage.json()) as {
        sessionKey?: string;
        items?: Array<{ content?: Array<{ text?: string }>; __remoteclaw?: { seq?: number } }>;
        messages?: Array<{ content?: Array<{ text?: string }>; __remoteclaw?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(firstBody.messages?.map((message) => message.__remoteclaw?.seq)).toEqual([2, 3]);
      ...
      const secondBody = (await secondPage.json()) as {
        items?: Array<{ content?: Array<{ text?: string }>; __remoteclaw?: { seq?: number } }>;
        messages?: Array<{ __remoteclaw?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(secondBody.messages?.map((message) => message.__remoteclaw?.seq)).toEqual([1]);
  ...
  test("streams session history updates over SSE", async () => {
    ...
    try {
      ...
      expect(
        (
          messageEvent.data as {
            message?: { __remoteclaw?: { id?: string; seq?: number } };
          }
        ).message?.__remoteclaw,
      ).toMatchObject({
