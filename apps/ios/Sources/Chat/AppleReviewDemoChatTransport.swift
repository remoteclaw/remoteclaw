...
enum ScreenshotFixtureMode {
    static let gatewayName = "RemoteClaw Gateway"
...
struct LocalChatFixture {
    ...
    static let appScreenshots = LocalChatFixture(
        ...
        modelID: "gpt-5.5",
        modelName: "GPT-5.5",
        responsePrefix: "RemoteClaw is connected to your gateway.",
        seedMessages: ProcessInfo.processInfo.arguments.contains("--remoteclaw-empty-chat-fixture")
            ? []
...
struct LocalFixtureChatTransport: RemoteClawChatTransport {
    ...
    func createSession(
        ...
        worktree _: Bool?) async throws -> RemoteClawChatCreateSessionResponse
    {
        ...
        archived: Bool) async throws -> RemoteClawChatSessionsListResponse
    {
        ...
        if let search {
            sessions = RemoteClawChatSessionListOrganizer.filter(sessions, search: search)
        }
...
private actor LocalFixtureChatStore {
    ...
    private static var thinkingLevels: [RemoteClawChatThinkingLevelOption] {
        [
            ...
            RemoteClawChatThinkingLevelOption(id: "high", label: "High"),
        ]
    }

    private static func seedMessages(fixture: LocalChatFixture) -> [RemoteClawChatMessage] {
        let now = Date().timeIntervalSince1970 * 1000
