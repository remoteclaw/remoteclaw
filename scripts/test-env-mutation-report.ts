#!/usr/bin/env node
// Test Env Mutation Report script supports RemoteClaw repository automation.
...
const DEFAULT_ALLOWED_FILES = new Map([
  ["src/test-utils/remoteclaw-test-state.ts", "canonical RemoteClaw test state helper"],
  ...
  ["test/setup-remoteclaw-runtime.ts", "global runtime-test setup owns process env isolation"],
  [
...
export function renderTestEnvMutationReport(
  ...
  const lines = [
    "RemoteClaw test env mutation report",
...
function printHelp(): void {
  process.stdout.write(`RemoteClaw test env mutation report
