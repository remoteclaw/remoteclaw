...
const AI_RUNTIME_PACKAGE = "@remoteclaw/ai";
const AI_RUNTIME_BACKUP_DIR = ".remoteclaw-ai-package-backup";
...
function resolvePackedRemoteClawFileName(value) {
  const filename = value.trim();
  if (
    !filename.endsWith(".tgz") ||
    (!filename.startsWith("remoteclaw-") &&
      !filename.includes(":") &&
      !filename.includes("/") &&
      !filename.includes("\\"))
  ) {
  ...
  if (
    ...
    throw new Error(`npm pack reported unsafe RemoteClaw tarball filename: ${filename}`);
  }
  return filename;
}
  ...
  for (const step of PACKAGE_ARTIFACT_BUILD_STEPS) {
    ...
    await runImpl(step.command, step.args, sourceDir, {
      env: {
        ...
        REMOTECLAW_RUN_NODE_SKIP_DTS_BUILD: "0",
      },
...
async function newestRemoteClawTarball(outputDir, packOutput) {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      for (const entry of parsed) {
        ...
        const filename = resolvePackedRemoteClawFileName(entry.filename);
        if (filename) {
          fromOutput = filename;
        }
      }
  ...
  for (const line of packOutput.split(/\r?\n/u)) {
    const filename = resolvePackedRemoteClawFileName(line);
    if (filename) {
      fromOutput = filename;
    }
  }
  ...
  const packed = entries
    .filter((entry) => {
      try {
        return resolvePackedRemoteClawFileName(entry) === entry;
      } catch {
        return false;
      }
    })
...
function isPackedAiRuntimeTarball(filename) {
  return /^remoteclaw-ai-[A-Za-z0-9._-]+\.tgz$/u.test(filename);
}
...
export async function prepareBundledAiRuntimePackage(
  ...
  const aiRuntimePath = path.join(sourceDir, "node_modules", "@remoteclaw", "ai");
  ...
  if (!hasAiRuntimeWorkspace) {
    throw new Error("@remoteclaw/ai dependency requires the packages/ai workspace");
  }
  if (typeof aiRuntimeDependency !== "string") {
    throw new Error("root package.json must declare @remoteclaw/ai as a dependency");
  }
  ...
  try {
    ...
    if (packedAiTarballs.length !== 1) {
      throw new Error(
        `expected one packed @remoteclaw/ai tarball in ${outputDir}, found ${packedAiTarballs.length}`,
      );
    }
    ...
    if (typeof stagedPackageJson.version !== "string" || !stagedPackageJson.version) {
      throw new Error("packed @remoteclaw/ai package must declare a version");
    }
    for (const [name, version] of Object.entries(stagedPackageJson.dependencies ?? {})) {
      if (packageJson.dependencies[name] !== version) {
        throw new Error(
          `root package.json must declare ${name}@${String(version)} to bundle @remoteclaw/ai without duplicate dependencies`,
        );
      }
    }
...
export async function packRemoteClawPackageForDocker(sourceDir, outputDir, options = {}) {
  ...
  let tarball = await newestRemoteClawTarball(outputDir, options.pnpmPack ? "" : packOutput);
  if (options.outputName) {
...
async function main() {
  ...
  const tarball = await packRemoteClawPackageForDocker(sourceDir, outputDir, {
  ...
  await run(
    "node",
    [
      path.join(ROOT_DIR, "scripts/check-remoteclaw-package-tarball.mjs"),
