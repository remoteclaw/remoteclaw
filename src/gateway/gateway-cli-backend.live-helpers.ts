...
export function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("REMOTECLAW_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}

export function shouldRunCliImageProbe(providerId: string): boolean {
  const raw = process.env.REMOTECLAW_LIVE_CLI_BACKEND_IMAGE_PROBE?.trim();
  if (raw) {
...
export function shouldRunCliMcpProbe(providerId: string): boolean {
  const raw = process.env.REMOTECLAW_LIVE_CLI_BACKEND_MCP_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function resolveCliBackendLiveArgs(params: {
  ...
  if (!args || args.length === 0) {
    throw new Error(
      `REMOTECLAW_LIVE_CLI_BACKEND_ARGS is required for provider "${params.providerId}".`,
    );
  }
  const resumeArgs =
    parseJsonStringArray(
      "REMOTECLAW_LIVE_CLI_BACKEND_RESUME_ARGS",
      process.env.REMOTECLAW_LIVE_CLI_BACKEND_RESUME_ARGS,
...
export function shouldRunCliModelSwitchProbe(providerId: string, modelRef: string): boolean {
  const raw = process.env.REMOTECLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
async function callLoopbackJsonRpc(params: {
  ...
  const headers: Record<string, string> = {
    ...
    "x-remoteclaw-sender-is-owner": params.senderIsOwner ? "true" : "false",
  };
  if (params.messageProvider) {
    headers["x-remoteclaw-message-channel"] = params.messageProvider;
  }
  if (params.accountId) {
    headers["x-remoteclaw-account-id"] = params.accountId;
  }
...
export function snapshotCliBackendLiveEnv(): CliBackendLiveEnvSnapshot {
  return {
    ...
    skipProviders: process.env.REMOTECLAW_SKIP_PROVIDERS,
    ...
    skipBrowserControl: process.env.REMOTECLAW_SKIP_BROWSER_CONTROL_SERVER,
    bundledPluginsDir: process.env.REMOTECLAW_BUNDLED_PLUGINS_DIR,
    minimalGateway: process.env.REMOTECLAW_TEST_MINIMAL_GATEWAY,
...
export function restoreCliBackendLiveEnv(snapshot: CliBackendLiveEnvSnapshot): void {
  restoreEnvVar("REMOTECLAW_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("REMOTECLAW_STATE_DIR", snapshot.stateDir);
  restoreEnvVar("REMOTECLAW_GATEWAY_TOKEN", snapshot.token);
  restoreEnvVar("REMOTECLAW_SKIP_CHANNELS", snapshot.skipChannels);
  restoreEnvVar("REMOTECLAW_SKIP_PROVIDERS", snapshot.skipProviders);
  restoreEnvVar("REMOTECLAW_SKIP_GMAIL_WATCHER", snapshot.skipGmail);
  restoreEnvVar("REMOTECLAW_SKIP_CRON", snapshot.skipCron);
  restoreEnvVar("REMOTECLAW_SKIP_CANVAS_HOST", snapshot.skipCanvas);
  restoreEnvVar("REMOTECLAW_SKIP_BROWSER_CONTROL_SERVER", snapshot.skipBrowserControl);
  restoreEnvVar("REMOTECLAW_BUNDLED_PLUGINS_DIR", snapshot.bundledPluginsDir);
  restoreEnvVar("REMOTECLAW_TEST_MINIMAL_GATEWAY", snapshot.minimalGateway);
