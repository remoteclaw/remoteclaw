...
function resolveGatewayConnection() {
  ...
  const urlOverride = readString(process.env.REMOTECLAW_ANDROID_GATEWAY_URL);
  const details = buildGatewayConnectionDetails({
    config: cfg,
    ...(urlOverride ? { url: urlOverride } : {}),
  });
  const tokenOverride = readString(process.env.REMOTECLAW_ANDROID_GATEWAY_TOKEN);
  const passwordOverride = readString(process.env.REMOTECLAW_ANDROID_GATEWAY_PASSWORD);
...
function selectTargetNode(nodes: NodeListNode[]): NodeListNode {
  const nodeIdOverride = readString(process.env.REMOTECLAW_ANDROID_NODE_ID);
  if (nodeIdOverride) {
    const match = nodes.find((node) => node.nodeId === nodeIdOverride);
    if (!match) {
      throw new Error(`REMOTECLAW_ANDROID_NODE_ID not found in node.list: ${nodeIdOverride}`);
    }
    return match;
  }
    ...
    if (!target.connected || !target.paired) {
      ...
      throw new Error(
        [
          ...
          pendingHint,
          "precondition: open app, keep foreground, ensure pairing approved (`remoteclaw nodes pending` / `remoteclaw nodes approve <requestId>`)",
        ].join("\n"),
      );
    }
