...
function resolveGatewayLogPrefix(env: GatewayServiceEnv): string {
  return env.REMOTECLAW_LOG_PREFIX?.trim() || "gateway";
}

function resolveMacLaunchAgentLogPrefix(env: GatewayServiceEnv): string {
  return (
    env.REMOTECLAW_LOG_PREFIX?.trim() ||
    `gateway${resolveGatewayProfileSuffix(env.REMOTECLAW_PROFILE)}`
  );
}
...
export function resolveMacLaunchAgentLogPaths(env: GatewayServiceEnv): GatewayLogPaths {
  ...
  const logDir = path.posix.join(home, "Library", "Logs", "remoteclaw");
...
export function resolveGatewaySupervisorLogPaths(
  ...
  // RemoteClaw state dir so generated service users can create the directory.
...
export function renderCmdRestartLogSetup(env: GatewayServiceEnv): {
  ...
  return {
    ...
    lines: [
      ...
      `>> ${quotedLogPath} 2>&1 echo [%DATE% %TIME%] remoteclaw restart log initialized`,
    ],
  };
}
