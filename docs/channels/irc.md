...
1. Enable IRC config in `~/.remoteclaw/remoteclaw.json`.
2. Set at least:
...
Even if a channel is allowed (via `groupPolicy` + `groups`) and the sender is allowed, RemoteClaw defaults to **mention-gating** in group contexts.
