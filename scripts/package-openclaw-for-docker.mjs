...
async function main() {
  ...
  if (!options.skipBuild) {
    console.error("==> Building RemoteClaw package artifacts");
    await run("node", ["scripts/build-all.mjs"], sourceDir, {
      env: { ...process.env, REMOTECLAW_BUILD_ALL_NO_PNPM: "1" },
    });
    console.error("==> Building RemoteClaw Control UI artifacts");
