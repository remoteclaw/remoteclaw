...
RemoteClaw supports camera capture for agent workflows on paired **iOS**, **Android**, and **macOS** nodes: capture a photo (`jpg`) or a short video clip (`mp4`, with optional audio) via Gateway `node.invoke`.
...
```bash
remoteclaw nodes camera snap --node <id>                 # default: both front + back (2 MEDIA lines)
remoteclaw nodes camera snap --node <id> --facing front
remoteclaw nodes camera clip --node <id> --duration 3000
...
- **Settings → General → Allow Camera** (`remoteclaw.cameraEnabled`).
  - Default: **off**.
  - When off: camera requests return `CAMERA_DISABLED: enable Camera in Settings`.

### CLI helper (node invoke)

Use the main `remoteclaw` CLI to invoke camera commands on the macOS node.
...
- `remoteclaw nodes camera snap` defaults to `maxWidth=1600` unless overridden.
