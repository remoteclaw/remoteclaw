// Bench Gateway Startup script supports RemoteClaw repository automation.
...
function sanitizedEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    LOGNAME: process.env.LOGNAME ?? "remoteclaw-bench",
    NO_COLOR: "1",
...
async function runGatewaySample(options: {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "remoteclaw-gateway-bench-"));
  ...
  const childArgs = [
    ...(options.cpuProfDir
      ? [
          ...
          options.cpuProfDir,
          "--cpu-prof-name",
          `remoteclaw-gateway-${options.benchCase.id}-${options.sampleIndex}-${Date.now()}.cpuprofile`,
        ]
      : []),
    options.entry,
    "gateway",
