...
- REMOTECLAW_IOS_VERSION = exact canonical iOS version
- REMOTECLAW_MARKETING_VERSION = short iOS/App Store version
- REMOTECLAW_BUILD_VERSION = explicit build number or local numeric fallback
EOF
}
...
while IFS='=' read -r key value; do
  case "${key}" in
    REMOTECLAW_IOS_VERSION)
      IOS_VERSION="${value}"
      ;;
    REMOTECLAW_MARKETING_VERSION)
...
REMOTECLAW_IOS_VERSION = ${IOS_VERSION}
REMOTECLAW_MARKETING_VERSION = ${MARKETING_VERSION}
REMOTECLAW_BUILD_VERSION = ${BUILD_NUMBER}
EOF
