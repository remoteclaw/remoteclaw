---
summary: "CLI reference for `remoteclaw proxy`, including operator-managed proxy validation and the local debug proxy capture inspector"
read_when:
...
```bash
remoteclaw proxy validate [--json] [--proxy-url <url>] [--proxy-ca-file <path>] [--allowed-url <url>] [--denied-url <url>] [--apns-reachable] [--apns-authority <url>] [--timeout-ms <ms>]
...
Checks the effective operator-managed proxy URL from `--proxy-url`, config (`proxy.proxyUrl`), or `REMOTECLAW_PROXY_URL`, in that precedence order. Reports a config problem if no proxy is enabled and configured; pass `--proxy-url` for a one-off preflight without touching config.

Managed proxy URLs use `http://` for a plain forward-proxy listener, or `https://` when RemoteClaw must open TLS to the proxy endpoint itself before sending proxy requests. Use `--proxy-ca-file` to trust a private CA for that TLS connection.
...
The debug proxy's direct upstream forwarding opens upstream sockets for diagnostics. When RemoteClaw managed proxy mode is active, direct forwarding for proxy requests and CONNECT tunnels is disabled by default; set `REMOTECLAW_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY=1` only for approved local diagnostics.
