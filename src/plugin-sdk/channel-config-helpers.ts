...
function formatPairingApproveHint(channelId: string): string {
  const listCmd = formatCliCommand(`remoteclaw pairing list ${channelId}`);
  const approveCmd = formatCliCommand(`remoteclaw pairing approve ${channelId} <code>`);
...
type ChannelConfigAccessorParams<Config extends RemoteClawConfig = RemoteClawConfig> = {
  cfg: Config;
  accountId?: string | null;
};
