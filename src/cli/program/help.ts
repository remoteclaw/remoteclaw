...
export function configureProgramHelp(program: Command, ctx: ProgramContext) {
  program
    ...
    .option(
      "--container <name>",
      "Run the CLI inside a running Podman/Docker container named <name> (default: env REMOTECLAW_CONTAINER)",
    )
    .option(
      "--dev",
      "Dev profile: isolate state under ~/.remoteclaw-dev, default gateway port 19001, and shift derived ports (browser/canvas)",
    )
    .option(
      "--profile <name>",
      "Use a named profile (isolates REMOTECLAW_STATE_DIR/REMOTECLAW_CONFIG_PATH under ~/.remoteclaw-<name>)",
    )
    .option(
      "--log-level <level>",
  ...
  program.addHelpText("beforeAll", () => {
    if (hasEmittedCliBanner() || process.env.REMOTECLAW_SUPPRESS_HELP_BANNER === "1") {
      return "";
    }
    const rich = isRich();
  ...
  program.addHelpText("afterAll", ({ command }) => {
    ...
    const docs = formatDocsLink("/cli", "docs.remoteclaw.org/cli");
