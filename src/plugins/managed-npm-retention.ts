...
export const RETAINED_MANAGED_NPM_INSTALL_MARKER = ".remoteclaw-retained-npm-install.json";
const RETAINED_MANAGED_NPM_INSTALL_MARKER_DIR = ".remoteclaw-retained-npm-installs";
...
export async function clearRetainedManagedNpmInstallMarker(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    // Best effort: keep the RemoteClaw-owned marker directory if it is not empty.
  }
  return true;
}
