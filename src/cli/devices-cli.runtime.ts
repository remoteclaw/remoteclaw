...
function buildFallbackStateMismatchError(details: ConnectPairingRequiredDetails): Error {
  return new Error(
    [
      ...
      "The running gateway is probably using a different REMOTECLAW_PROFILE or REMOTECLAW_STATE_DIR than this CLI.",
...
async function approvePairingWithFallback(
  ...
  try {
    ...
    // Local CLI fallback already assumes direct machine access; it approves the
    // pending request directly through the local pairing state.
...
function buildExplicitApproveCommand(opts: DevicesRpcOpts, requestId: string): string {
  const args = ["remoteclaw", "devices", "approve", requestId];
...
function resolveRequiredDeviceRole(
  ...
  defaultRuntime.error(
    `--device and --role are required. Run ${formatCliCommand("remoteclaw devices list")} to choose a paired device.`,
  );
...
export async function runDevicesRemoveCommand(
  ...
  if (!trimmed) {
    defaultRuntime.error(
      `deviceId is required. Run ${formatCliCommand("remoteclaw devices list")} to choose a paired device.`,
    );
    defaultRuntime.exit(1);
    return;
  }
...
export async function runDevicesApproveCommand(
  ...
  if (usingImplicitSelection) {
    ...
    switch (approval.kind) {
      ...
      case "re-approval":
        defaultRuntime.log(
          "  Note:   Already paired. Approval-bound device details changed, so RemoteClaw created a fresh request instead of silently reusing the old approval.",
        );
        break;
      case "new-pairing":
