...
struct TalkProState: Equatable {
    ...
    var title: String {
        ...
        if self.normalizedStatus.contains("thinking") { return "Asking RemoteClaw" }
    ...
    var color: Color {
        ...
        case .requestFailed, .loadFailed:
            return RemoteClawBrand.danger
        case .missingScope, .requestingUpgrade, .upgradeRequested, .apiKeyMissing:
            return RemoteClawBrand.warn
        default:
            if !self.isConfigLoaded { return RemoteClawBrand.warn }
            return self.isEnabled ? RemoteClawBrand.ok : RemoteClawBrand.accentHot
        }
    }
    ...
    var primaryButtonFill: AnyShapeStyle {
        switch self.primaryAction {
        case .stop:
            AnyShapeStyle(RemoteClawBrand.danger)
        case .waiting:
            AnyShapeStyle(RemoteClawBrand.warn.opacity(0.72))
        default:
            AnyShapeStyle(LinearGradient(
                colors: [self.color.opacity(0.95), RemoteClawBrand.accent],
