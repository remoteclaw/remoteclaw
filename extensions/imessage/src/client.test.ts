...
// Fork note: RemoteClaw's client reads stdout through `readline.createInterface`,
// so `stdout` must be a real Readable (upstream buffers stdout by hand and can
// use a bare EventEmitter here). PassThrough satisfies readline and still lets
// the test emit a synthetic `error`.
...
describe("IMessageRpcClient child stream error handling", () => {
  ...
  it.each(["stdout", "stderr", "stdin"] as const)(
    ...
    async (streamName) => {
      ...
      // stdout/stderr are PassThrough and stdin is a bare EventEmitter, so the
      // indexed union has no single callable `emit` signature.
      const stream: EventEmitter = child[streamName];
      expect(() => stream.emit("error", streamError)).not.toThrow();
