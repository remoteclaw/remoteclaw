---
summary: "Use ACP runtime sessions for Claude Code, Cursor, Gemini CLI, explicit Codex ACP fallback, RemoteClaw ACP, and other harness agents"
read_when:
...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let RemoteClaw run external coding harnesses (for example Pi, Claude Code, Cursor, Copilot, RemoteClaw ACP, OpenCode, Gemini CLI, and other supported ACPX harnesses) through an ACP backend plugin.

If you ask RemoteClaw in plain language to bind or control Codex in the current conversation, RemoteClaw should use the native Codex app-server plugin (`/codex bind`, `/codex threads`, `/codex resume`). If you ask for `/acp`, ACP, acpx, or a Codex background child session, RemoteClaw can still route Codex through ACP. Each ACP session spawn is tracked as a [background task](/automation/tasks).

If you ask RemoteClaw in plain language to "start Claude Code in a thread" or use another external harness, RemoteClaw should route that request to the ACP runtime (not the native sub-agent runtime).
...
Usually, yes. Fresh installs ship the bundled `acpx` runtime plugin enabled by default, with a plugin-local pinned `acpx` binary that RemoteClaw probes and self-repairs on startup. Run `/acp doctor` for a readiness check.
...
Native Codex conversation binding is the default chat-control path. RemoteClaw
dynamic tools still execute through RemoteClaw, while Codex-native tools such as
shell/apply-patch execute inside Codex. For Codex-native tool events, RemoteClaw
injects a per-turn native hook relay so plugin hooks can block
`before_tool_call`, observe `after_tool_call`, and route Codex
`PermissionRequest` events through RemoteClaw approvals. The v1 relay is
...
RemoteClaw picks `runtime: "acp"`, resolves the harness `agentId`, binds to the current conversation or thread when supported, and routes follow-ups to that session until close/expiry. Codex only follows this path when ACP is explicit or the requested background runtime still needs ACP.

## ACP versus sub-agents

Use ACP when you want an external harness runtime. Use native Codex app-server for Codex conversation binding/control. Use sub-agents when you want RemoteClaw-native delegated runs.
...
`/acp spawn <harness> --bind here` pins the current conversation to the spawned ACP session — no child thread, same chat surface. RemoteClaw keeps owning transport, auth, safety, and delivery; follow-up messages in that conversation route to the same session; `/new` and `/reset` reset the session in place; `/acp close` removes the binding.
...
- `/acp spawn codex --thread auto` — RemoteClaw may create a child thread/topic and bind there.
...
plugin-tools and RemoteClaw-tools MCP bridges, and ACP permission modes, see
