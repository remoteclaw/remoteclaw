...
`remoteclaw policy` is provided by the bundled Policy plugin. It is an enterprise
conformance layer over existing RemoteClaw settings, not a second configuration
system. You author requirements in `policy.jsonc`; RemoteClaw observes the active
...
observed state is existing RemoteClaw config or workspace metadata.
...
If an `agentIds` entry is not present in `agents.list[]`, RemoteClaw evaluates
...
| `gateway.nodes.denyCommands`            | `gateway.nodes.denyCommands`                   | Require exact node command ids such as `system.run` to be denied in RemoteClaw config. |
...
denied by RemoteClaw config. A deployment that intentionally allows a privileged
...
`~/.remoteclaw/exec-approvals.json` by default, or
`$REMOTECLAW_STATE_DIR/exec-approvals.json` when `REMOTECLAW_STATE_DIR` is set.
...
`remoteclaw doctor --lint` when the Policy plugin is enabled.
...
records the observed RemoteClaw state used by the checks, and
...
`remoteclaw policy watch` re-runs the check and reports when current evidence no
...
| `policy/gateway-node-command-denied`                     | A node command denied by policy is not denied by RemoteClaw config.               |
...
```json
{
  ...
  "severity": "error",
  "message": "Gateway node command 'system.run' is denied by policy but not denied by RemoteClaw config.",
  "source": "policy",
  "path": "remoteclaw config",
  "ocPath": "oc://remoteclaw.config/gateway/nodes/denyCommands",
  "target": "oc://remoteclaw.config/gateway/nodes/denyCommands",
