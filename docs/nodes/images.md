...
- Send media with optional captions via `remoteclaw message send --media`.
- Allow auto-replies from the web inbox to include media alongside text.
- Keep per-type limits sane and predictable.

## CLI Surface

- `remoteclaw message send --media <path-or-url> [--message <caption>]`
...
- Media understanding (if configured via `tools.media.*` or shared `tools.media.models`) runs before templating and can insert `[Image]`, `[Audio]`, and `[Video]` blocks into `Body`.
  ...
  - If the active primary image model already supports vision natively, RemoteClaw skips the `[Image]` summary block and passes the original image to the model instead.
