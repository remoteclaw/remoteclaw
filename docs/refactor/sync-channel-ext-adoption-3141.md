...
| 7   | `clickclack/gateway.ts`     | **Adopted**     | Gateway hunk of `f3e3b6985b0`: a non-Error thrown in ws dispatch reached `reject()` bare. `formatErrorMessage` exists as `src/infra/errors.ts` — upstream's `plugin-sdk/error-runtime` is a repath, not a gap. Restores upstream's `gateway.test.ts` case **minus its trailing `setStatus({running: false})` assertion**, which covers upstream's `finally`-based status clear — behaviour this fork does not have (its `setStatus` call sits after the `while` loop, so a rejection skips it) and that this pass did not adopt. The wrapping behaviour itself is gated: reverting the source hunk turns the restored test red.             |
...
- **Re-check the reclassification instead of inheriting it.** Six of thirteen deltas introduce no
