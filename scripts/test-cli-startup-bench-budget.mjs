...
if (process.argv.slice(2).includes("--help")) {
  console.log(
    [
      ...
      "REMOTECLAW_STARTUP_BENCH_ENFORCE_NONCANONICAL_ARCH=1 to force them.",
...
try {
  opts = parseFlagArgs(
    process.argv.slice(2),
    {
      ...
      report: "",
      entry: "remoteclaw.mjs",
      preset: "all",
      runs: 1,
      warmup: 0,
      timeoutMs: 30_000,
      maxDurationRegressionPct:
        readBudgetEnvNumber("REMOTECLAW_STARTUP_BENCH_MAX_DURATION_REGRESSION_PCT") ?? 20,
      maxFirstOutputRegressionPct:
        readBudgetEnvNumber("REMOTECLAW_STARTUP_BENCH_MAX_FIRST_OUTPUT_REGRESSION_PCT") ?? 20,
      maxRssRegressionPct:
        readBudgetEnvNumber("REMOTECLAW_STARTUP_BENCH_MAX_RSS_REGRESSION_PCT") ?? 20,
      skipBaseline: false,
...
const shouldAutoSkipNonCanonicalBaselineChecks =
  process.arch !== "x64" && process.env.REMOTECLAW_STARTUP_BENCH_ENFORCE_NONCANONICAL_ARCH !== "1";
if (shouldAutoSkipNonCanonicalBaselineChecks && !opts.skipBaseline) {
  console.warn(
    `[test-cli-startup-bench-budget] skipping x64 startup fixture budgets on ${process.arch}; response contracts and sample output validation still ran. Set REMOTECLAW_STARTUP_BENCH_ENFORCE_NONCANONICAL_ARCH=1 to force fixture checks.`,
  );
  opts = {
    ...opts,
    skipBaseline: true,
