...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveRemoteClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("remoteclaw models auth setup-token")}\` or \`${formatCliCommand("remoteclaw configure")}\`.`
          : `Run \`${formatCliCommand("remoteclaw configure")}\` or set an API key env var.`;
