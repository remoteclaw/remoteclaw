...
export async function expectPrivateQaLabRuntimeSurfaceLoad(params: {
  ...
  importRuntime: () => Promise<QaRuntimeModule>;
  loadBundledPluginPublicSurfaceModuleSync: SurfaceLoaderMock;
  resolveRemoteClawPackageRootSync: SurfaceLoaderMock;
}) {
  const sourceRoot = makePrivateQaSourceRoot(params.tempDirs, "remoteclaw-qa-runtime-root-");
  params.resolveRemoteClawPackageRootSync.mockReturnValue(sourceRoot);
