...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["REMOTECLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/remoteclaw")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("remoteclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("remoteclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/remoteclaw/ (rolling or stdout), or nil if none exist.
