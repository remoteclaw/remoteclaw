...
export function shouldUseRootHelpFastPath(
  ...
  const invocation = resolveCliArgvInvocation(argv);
  return (
    env.REMOTECLAW_DISABLE_CLI_STARTUP_HELP_FAST_PATH !== "1" &&
...
export function shouldUseBrowserHelpFastPath(
  ...
  if (env.REMOTECLAW_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1") {
    return false;
  }
  const invocation = resolveCliArgvInvocation(argv);
  return (
...
export function resolveMissingPluginCommandMessage(
  ...
  if (parentPluginId) {
    ...
    if (
      ...
      return (
        `The \`remoteclaw ${normalizedPluginId}\` command is provided by the ` +
        `"${parentPluginId}" plugin, but that bundled plugin is disabled by default. Run ` +
        `\`remoteclaw plugins enable ${parentPluginId}\` to enable that CLI surface.`
      );
    }
    if (commandAlias.kind === "runtime-slash") {
      const cliHint = commandAlias.cliCommand
        ? `Use \`remoteclaw ${commandAlias.cliCommand}\` for related CLI operations, or `
        : "Use ";
      return (
