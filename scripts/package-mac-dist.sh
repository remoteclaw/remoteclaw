...
# Output:
# - dist/RemoteClaw.app
# - dist/RemoteClaw-<version>.zip
# - dist/RemoteClaw-<version>.dmg
...
export BUNDLE_ID="${BUNDLE_ID:-org.remoteclaw.mac}"
...
canonical_sparkle_build() {
  ...
  stderr_file="$(mktemp "${TMPDIR:-/tmp}/remoteclaw-sparkle-build.XXXXXX")" || {
...
APP="$ROOT_DIR/dist/RemoteClaw.app"
if [[ ! -d "$APP" ]]; then
...
ZIP="$ROOT_DIR/dist/RemoteClaw-$VERSION.zip"
DMG="$ROOT_DIR/dist/RemoteClaw-$VERSION.dmg"
NOTARY_ZIP="$ROOT_DIR/dist/RemoteClaw-$VERSION.notary.zip"
DSYM_ZIP="$ROOT_DIR/dist/RemoteClaw-$VERSION.dSYM.zip"
