...
<Tabs>
  <Tab title="DM policy">
    ...
    If you previously relied on pairing-store allowlist files, `remoteclaw doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
...
```
    ...
    - or read `chat.id` from `remoteclaw logs --follow`
...
- DM messages can carry `message_thread_id`; RemoteClaw routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    ...
    If native draft transport is unavailable/rejected, RemoteClaw automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, RemoteClaw retries as plain text.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**:
    ...
    - RemoteClaw pins the spawn confirmation message in-topic after a successful bind.
...
```
    ...
    - `~/.remoteclaw/telegram/sticker-cache.json`
...
```bash
remoteclaw message send --channel telegram --target 123456789 --message "hi"
remoteclaw message send --channel telegram --target @name --message "hi"
```

    Telegram polls use `remoteclaw message poll` and support forum topics:
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat, so only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, RemoteClaw preserves the topic for both the approval prompt and the post-approval follow-up.
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `remoteclaw channels status` warns when config expects unmentioned group messages.
    - `remoteclaw channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `remoteclaw logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, RemoteClaw now retries these as recoverable network errors.
...
```
    ...
    - Environment overrides (temporary):
      - `REMOTECLAW_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `REMOTECLAW_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `REMOTECLAW_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `allowlist` requires at least one sender ID. `open` requires `"*"`. `remoteclaw doctor --fix` can resolve legacy `@username` entries to IDs and can recover allowlist entries from pairing-store files in allowlist migration flows.
...
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `remoteclaw doctor --fix` can resolve legacy `@username` entries to IDs. Non-numeric entries are ignored at auth time. Group auth does not use DM pairing-store fallback (`2026.2.25+`).
- Multi-account precedence:
  ...
  - If neither is set, RemoteClaw falls back to the first normalized account ID and `remoteclaw doctor` warns.
