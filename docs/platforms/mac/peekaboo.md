...
RemoteClaw can host **PeekabooBridge** as a local, permission‑aware UI automation
...
- **UI**: visual overlays stay in Peekaboo.app; RemoteClaw is a thin broker host.
...
When enabled, RemoteClaw starts a local UNIX socket server. If disabled, the host
...
- The bridge validates **caller code signatures**; an allowlist of TeamIDs is
  enforced (Peekaboo host TeamID + RemoteClaw app TeamID).
...
- If `peekaboo` reports “bridge client is not authorized”, ensure the client is
  ...
  in **debug** mode only.
- If no hosts are found, open one of the host apps (Peekaboo.app or RemoteClaw.app)
