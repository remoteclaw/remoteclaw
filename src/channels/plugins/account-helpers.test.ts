...
function cfg(accounts?: Record<string, unknown> | null): RemoteClawConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as RemoteClawConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as RemoteClawConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as RemoteClawConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as RemoteClawConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as RemoteClawConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as RemoteClawConfig)).toBe("default");
    });
  });
});
