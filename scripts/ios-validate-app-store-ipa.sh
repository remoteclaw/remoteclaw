...
usage() {
  cat <<'EOF'
Usage:
  scripts/ios-validate-app-store-ipa.sh --ipa apps/ios/build/app-store/RemoteClaw-<version>.ipa
...
EXPECTED_BUNDLE_ID="org.remoteclaw.app"
EXPECTED_PROFILE_NAME="RemoteClaw App Store org.remoteclaw.app"
EXPECTED_APP_GROUP="group.org.remoteclaw.app.shared"
...
tmp_dir="$(mktemp -d -t remoteclaw-ios-ipa.XXXXXX)"
...
assert_plist_string "${info_plist}" "RemoteClawPushMode" "${EXPECTED_PUSH_MODE}" "push mode mismatch"
assert_plist_empty_or_absent "${info_plist}" "RemoteClawPushRelayBaseURL" "push relay URL override"
assert_plist_key_absent "${info_plist}" "RemoteClawPushTransport" "legacy push transport"
assert_plist_key_absent "${info_plist}" "RemoteClawPushDistribution" "legacy push distribution"
assert_plist_key_absent "${info_plist}" "RemoteClawPushAPNsEnvironment" "legacy APNs environment"
assert_plist_key_absent "${info_plist}" "RemoteClawPushRelayProfile" "legacy relay profile"
assert_plist_key_absent "${info_plist}" "RemoteClawPushProofPolicy" "legacy proof policy"
