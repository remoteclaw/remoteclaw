...
function createSingleAgentAvatarConfig(workspace: string): RemoteClawConfig {
  return {
...
describe("listSessionsFromStore subagent metadata", () => {
  ...
  test("uses persisted active subagent runs when the local worker only has terminal snapshots", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-session-utils-subagent-"));
      ...
      const row = withEnv(
        {
          ...
          REMOTECLAW_TEST_READ_SUBAGENT_RUNS_FROM_DISK: "1",
        },
        () => {
          const result = listSessionsFromStore({
            cfg,
