---
summary: "Expose RemoteClaw diagnostics as Prometheus text metrics through the diagnostics-prometheus plugin"
title: "Prometheus metrics"
sidebarTitle: "Prometheus"
read_when:
  - You want Prometheus, Grafana, VictoriaMetrics, or another scraper to collect RemoteClaw Gateway metrics
...
<Steps>
  ...
  <Step title="Enable the plugin">
    <Tabs>
      ...
      <Tab title="CLI">
        ```bash
        remoteclaw plugins enable diagnostics-prometheus
        ```
      </Tab>
    </Tabs>
  </Step>
  ...
  <Step title="Scrape the protected route">
    ...
    ```bash
    curl -H "Authorization: Bearer $REMOTECLAW_GATEWAY_TOKEN" \
  ...
  <Step title="Wire Prometheus">
    ...
    scrape_configs:
      - job_name: remoteclaw
        ...
        authorization:
          credentials_file: /etc/prometheus/remoteclaw-gateway-token
        static_configs:
          - targets: ["remoteclaw-gateway:18789"]
    ```
  </Step>
</Steps>
...
| Metric                                          | Type      | Labels                                                                                    |
| ----------------------------------------------- | --------- | ----------------------------------------------------------------------------------------- |
| `remoteclaw_run_completed_total`                | counter   | `channel`, `model`, `outcome`, `provider`, `trigger`                                      |
| `remoteclaw_run_duration_seconds`               | histogram | `channel`, `model`, `outcome`, `provider`, `trigger`                                      |
| `remoteclaw_model_call_total`                   | counter   | `api`, `error_category`, `model`, `outcome`, `provider`, `transport`                      |
| `remoteclaw_model_call_duration_seconds`        | histogram | `api`, `error_category`, `model`, `outcome`, `provider`, `transport`                      |
| `remoteclaw_model_tokens_total`                 | counter   | `agent`, `channel`, `model`, `provider`, `token_type`                                     |
| `remoteclaw_gen_ai_client_token_usage`          | histogram | `model`, `provider`, `token_type`                                                         |
| `remoteclaw_model_cost_usd_total`               | counter   | `agent`, `channel`, `model`, `provider`                                                   |
| `remoteclaw_tool_execution_total`               | counter   | `error_category`, `outcome`, `params_kind`, `tool`                                        |
...
| `remoteclaw_telemetry_exporter_total`           | counter   | `exporter`, `reason`, `signal`, `status`                                                  |
| `remoteclaw_prometheus_series_dropped_total`    | counter   | none                                                                                      |
...
<AccordionGroup>
  ...
  <Accordion title="Series cap and overflow accounting">
    The exporter caps retained time series in memory at **2048** series across counters, gauges, and histograms combined. New series beyond that cap are dropped, and `remoteclaw_prometheus_series_dropped_total` increments by one each time.
...
sum by (provider) (rate(remoteclaw_model_tokens_total[1m]))

# Spend (USD) over the last hour, by model
sum by (model) (increase(remoteclaw_model_cost_usd_total[1h]))

# 95th percentile model run duration
histogram_quantile(
  0.95,
  sum by (le, provider, model)
    (rate(remoteclaw_run_duration_seconds_bucket[5m]))
)

# Queue wait time SLO (95p under 2s)
histogram_quantile(
  0.95,
  sum by (le, lane) (rate(remoteclaw_queue_lane_wait_seconds_bucket[5m]))
) < 2

# Dropped Prometheus series (cardinality alarm)
increase(remoteclaw_prometheus_series_dropped_total[15m]) > 0
```

<Tip>
Prefer `gen_ai_client_token_usage` for cross-provider dashboards: it follows the OpenTelemetry GenAI semantic conventions and is consistent with metrics from non-RemoteClaw GenAI services.
</Tip>

## Choosing between Prometheus and OpenTelemetry export

RemoteClaw supports both surfaces independently. You can run either, both, or neither.
...
<Tabs>
  ...
  <Tab title="diagnostics-otel">
    - **Push** model: RemoteClaw sends OTLP/HTTP to a collector or OTLP-compatible backend.
...
<AccordionGroup>
  <Accordion title="Empty response body">
    ...
    - Confirm the plugin is enabled and loaded with `remoteclaw plugins list --enabled`.
  ...
  <Accordion title="`remoteclaw_prometheus_series_dropped_total` is climbing">
