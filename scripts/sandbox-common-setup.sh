...
BASE_IMAGE="${BASE_IMAGE:-remoteclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-remoteclaw-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=remoteclaw.sandbox=1)
NOTE
