...
ASC_KEYCHAIN_SERVICE=remoteclaw-asc-key
...
```bash
ASC_APP_IDENTIFIER=org.remoteclaw.client
# or
...
- confirm `apps/ios/build/beta/RemoteClaw-<version>.ipa` exists
...
- The beta flow regenerates `apps/ios/RemoteClaw.xcodeproj` from `apps/ios/project.yml` before archiving
