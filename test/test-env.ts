...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.REMOTECLAW_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    ...
    { key: "REMOTECLAW_AGENT_DIR", value: process.env.REMOTECLAW_AGENT_DIR },
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.REMOTECLAW_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".remoteclaw");
  const priorIsolatedHome = params.env.REMOTECLAW_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".remoteclaw")
  ) {
    realStateDir = path.join(params.realHome, ".remoteclaw");
  }
  ...
  const realConfigPath = params.env.REMOTECLAW_CONFIG_PATH?.trim()
    ...
    : path.join(realStateDir, "remoteclaw.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "remoteclaw.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const allowRealHome = isTruthyEnvValue(process.env.REMOTECLAW_LIVE_USE_REAL_HOME);
