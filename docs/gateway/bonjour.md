...
RemoteClaw can use Bonjour (mDNS / DNS-SD) to discover an active Gateway (WebSocket endpoint).
...
2. Publish DNS-SD records for `_remoteclaw-gw._tcp` under a dedicated zone
   (example: `remoteclaw.internal.`).
3. Configure Tailscale **split DNS** so your chosen domain resolves via that
   DNS server for clients (including iOS).

RemoteClaw supports any discovery domain; `remoteclaw.internal.` is just an example.
...
- Set `gateway.bind: "tailnet"` in `~/.remoteclaw/remoteclaw.json`.
- Restart the Gateway (or restart the macOS menubar app).

## What advertises

Only the Gateway advertises `_remoteclaw-gw._tcp`. LAN multicast advertising is
...
in-progress states. If the service never reaches `announced`, RemoteClaw eventually
...
invalid DNS-label character, RemoteClaw falls back to `remoteclaw.local`. Set
`REMOTECLAW_MDNS_HOSTNAME=<name>` before starting the Gateway when you need an
explicit host label.
...
discovery plugin for that RemoteClaw config:
...
containers when `REMOTECLAW_DISABLE_BONJOUR` is unset. Docker bridge networks
...
- Disabling Bonjour does not change `gateway.bind`; Docker still defaults to
  `REMOTECLAW_GATEWAY_BIND=lan` so the published host port can work.
...
- Reusing the same `REMOTECLAW_CONFIG_DIR` outside Docker does not persist the
  container auto-disable policy.
- Set `REMOTECLAW_DISABLE_BONJOUR=0` only for host networking, macvlan, or another
...
4. If you deliberately enabled the Bonjour plugin in Docker and forced advertising
   with `REMOTECLAW_DISABLE_BONJOUR=0`, test multicast from the host:
   ...
   cancellations, restore `REMOTECLAW_DISABLE_BONJOUR=1` and use a direct or
   Tailnet route.
...
- **Advertiser stuck in probing/announcing**: hosts with blocked multicast,
  ...
  non-announced state. RemoteClaw retries a few times and then disables Bonjour
...
- **Docker bridge networking**: Bonjour auto-disables in detected containers.
  Set `REMOTECLAW_DISABLE_BONJOUR=0` only for host, macvlan, or another
...
- `remoteclaw plugins enable bonjour` enables the bundled LAN discovery plugin on hosts where it is not default-enabled.
- `remoteclaw plugins disable bonjour` disables LAN multicast advertising by disabling the bundled plugin.
- `REMOTECLAW_DISABLE_BONJOUR=1` disables LAN multicast advertising without changing plugin config; accepted truthy values are `1`, `true`, `yes`, and `on` (legacy: `REMOTECLAW_DISABLE_BONJOUR`).
- `REMOTECLAW_DISABLE_BONJOUR=0` forces LAN multicast advertising on, including inside detected containers; accepted falsy values are `0`, `false`, `no`, and `off`.
- When the Bonjour plugin is enabled and `REMOTECLAW_DISABLE_BONJOUR` is unset, Bonjour advertises on normal hosts and auto-disables inside detected containers.
...
- `REMOTECLAW_TAILNET_DNS` publishes a MagicDNS hint in TXT when mDNS full mode is enabled (legacy: `REMOTECLAW_TAILNET_DNS`).
