...
export const zh_TW = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密碼存入 RemoteClaw 設定",
    ...
    migration: {
      ...
      appliedTitle: "遷移已套用",
      cancelled: "遷移已取消",
      complete: "遷移完成。下一步執行 `remoteclaw doctor`。",
    ...
    remote: {
      ...
      insecureRemoteUrl:
        "遠端主機請使用 wss://，或透過 SSH 通道使用 ws://127.0.0.1/localhost。緊急繞過：可信私有網路可設定 REMOTECLAW_ALLOW_INSECURE_PRIVATE_WS=1。",
      ...
      plaintextPasswordHint: "直接把密碼存入 RemoteClaw 設定",
      plaintextPasswordLabel: "現在輸入密碼",
      plaintextTokenHint: "直接把權杖存入 RemoteClaw 設定",
    ...
    security: {
      ...
      beta: "RemoteClaw 是一個興趣專案，仍處於 beta 階段。請預期會有邊緣問題。",
      confirm: "我理解 RemoteClaw 預設面向個人使用；共享/多使用者使用需要加固。繼續？",
      hardeningRequired: "如果你不熟悉安全加固和存取控制，請不要執行 RemoteClaw。",
      learnMore: "了解更多",
      notMultitenant: "RemoteClaw 預設不是敵對多租戶安全邊界。",
      personalAgent: "預設情況下，RemoteClaw 是個人 agent：一個可信操作者邊界。",
    ...
    skills: {
      ...
      docsLine: "文件：https://docs.remoteclaw.org/skills",
      enterEnv: "輸入 {env}",
      ...
      manualPrereqsDoctorHint: "請先安裝這些工具，然後執行 `remoteclaw doctor` 檢查技能和需求。",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 執行：./occ talk:bot:install "RemoteClaw" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要傳送者/擁有者號碼，以便 RemoteClaw 將你加入允許清單。",
      ...
      separatePhoneLabel: "RemoteClaw 專用獨立手機",
    ...
    irc: {
      ...
      nextStatusCommand: "命令：remoteclaw channels status --probe",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "這是 RemoteClaw 用來向 Chat 傳送回覆的 URL。",
    ...
    signal: {
      ...
      multipleEntries: "多個項目請用逗號分隔。",
      nextLinkDevice: '綁定裝置：signal-cli link -n "RemoteClaw"',
    ...
    twitch: {
      ...
      envPrompt: "偵測到 Twitch 環境變數 REMOTECLAW_TWITCH_ACCESS_TOKEN。使用環境 token？",
      helpCopyToken: "3. 複製 token（以 'oauth:' 開頭）和 Client ID",
      helpCreateApp: "1. 在 https://dev.twitch.tv/console 建立 Twitch 應用",
      helpEnvVars: "支援環境變數：REMOTECLAW_TWITCH_ACCESS_TOKEN",
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "將 Synology NAS Chat 連接到 RemoteClaw，並啟用完整 agent 能力。",
    ...
    search: {
      ...
      configured: "已設定",
      configureLaterHint: "稍後可用 remoteclaw configure --section web 設定",
      docsLine: "文件：{url}",
    ...
    finalize: {
      ...
      controlUiDocs: "文件：https://docs.remoteclaw.org/web/control-ui",
      dashboardCopyPaste: "在本機瀏覽器中複製/貼上這個 URL 來控制 RemoteClaw。",
      dashboardLinkWithToken: "Dashboard 連結（含權杖）：{url}",
      dashboardOpened: "已在瀏覽器中開啟。保留該分頁以控制 RemoteClaw。",
      ...
      daemonRuntimeNodeHint:
        "RemoteClaw 狀態使用 node:sqlite，因此必須使用 Node；Bun 無法執行閘道。",
      ...
      gatewayTokenStored:
        "儲存位置：$REMOTECLAW_CONFIG_PATH（預設 ~/.remoteclaw/remoteclaw.json）中的 gateway.auth.token，或 REMOTECLAW_GATEWAY_TOKEN。",
      ...
      optionalApps: "可選應用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 連結控制 RemoteClaw。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已開啟；保留該分頁以控制 RemoteClaw。",
      ...
      containerSystemdUnavailable:
        "此容器內沒有 systemd 使用者服務。RemoteClaw 只會略過背景服務安裝；請以前景方式執行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
        "在你的電腦上執行 agent 存在風險，請加固設定：https://docs.remoteclaw.org/security",
      ...
      webDocs: "文件：https://docs.remoteclaw.org/tools/web",
      ...
      whatNow: '下一步：https://remoteclaw.org/showcase（"What People Are Building"）。',
      whatNowTitle: "下一步",
      workspaceBackupTitle: "工作區備份",
      workspaceDocs: "文件：https://docs.remoteclaw.org/concepts/agent-workspace",
