...
async function runVitestJsonReport(params) {
  ...
  const result = await spawnText(spawnCommand.command, spawnCommand.args, {
    ...
    env: {
      ...
      REMOTECLAW_VITEST_NO_OUTPUT_TIMEOUT_MS: "0",
      NODE_OPTIONS: [
...
function buildFullSuiteLeafRunPlans() {
  const previousLeafShards = process.env.REMOTECLAW_TEST_PROJECTS_LEAF_SHARDS;
  ...
  try {
    ...
    if (previousLeafShards === undefined) {
      delete process.env.REMOTECLAW_TEST_PROJECTS_LEAF_SHARDS;
    } else {
      process.env.REMOTECLAW_TEST_PROJECTS_LEAF_SHARDS = previousLeafShards;
    }
  }
}
...
export function resolveFullSuiteVitestEnv(args, env = process.env, label = "") {
  if (
    !args.fullSuite ||
    env.REMOTECLAW_VITEST_MAX_WORKERS?.trim() ||
    env.REMOTECLAW_TEST_WORKERS?.trim()
  ) {
    return {};
  }

  return {
    REMOTECLAW_VITEST_MAX_WORKERS: label === "commands" ? "1" : "2",
  };
}
