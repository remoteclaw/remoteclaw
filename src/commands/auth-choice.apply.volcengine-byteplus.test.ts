...
describe.skip("volcengine/byteplus auth choice", () => {
  ...
  async function setupTempState() {
    const env = await setupAuthTestEnv("remoteclaw-volc-byte-");
  ...
  it.each(providerAuthCases)(
    ...
    async (testCase) => {
      ...
      expect((result as any)?.config.auth?.profiles?.[testCase.profileId]).toMatchObject({
