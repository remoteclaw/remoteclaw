...
async function noteNextcloudTalkSecretHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      '2) Run: ./occ talk:bot:install "RemoteClaw" "<shared-secret>" "<webhook-url>" --feature reaction',
  ...
  configure: async ({
    ...
    if (shouldPromptAccountIds && !nextcloudTalkOverride) {
      accountId = await promptAccountId({
        ...
        label: "Nextcloud Talk",
        currentId: accountId,
        listAccountIds: listNextcloudTalkAccountIds as (cfg: RemoteClawConfig) => string[],
        defaultAccountId,
      });
    }
