...
# RemoteClaw macOS Companion (menu bar + gateway broker)

The macOS app is the **menu‑bar companion** for RemoteClaw. It owns permissions,
...
- Installs the global CLI (`remoteclaw`) via npm/pnpm on request (bun not recommended for the Gateway runtime).

## Local vs remote mode

- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `remoteclaw gateway install`.
...
The app registers the `remoteclaw://` URL scheme for local actions.
...
```bash
open 'remoteclaw://agent?message=Hello%20from%20deep%20link'
```
...
1. Install and launch **RemoteClaw.app**.
...
```bash
cd apps/macos
swift run remoteclaw-mac connect --json
swift run remoteclaw-mac discover --timeout 3000 --json
```
...
Tip: compare against `remoteclaw gateway discover --json` to see whether the
