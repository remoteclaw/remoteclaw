/** Inspects installed platform services for extra RemoteClaw or legacy gateway jobs. */
...
function isRemoteClawGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  if (detectLaunchdGatewayExecutionMarker(contents) !== "remoteclaw") {
    return false;
  }
...
async function scanLaunchdDir(params: {
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    const marker =
      hasGatewayServiceMarker(contents) || executionMarker === "remoteclaw"
        ? "remoteclaw"
