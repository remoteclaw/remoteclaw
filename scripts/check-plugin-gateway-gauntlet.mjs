...
export function parseArgs(argv) {
  ...
  const options = {
    ...
    qaTimeoutMs: 900_000,
    allowEmpty: false,
    failOnObservation: process.env.REMOTECLAW_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION === "1",
...
Environment:
  REMOTECLAW_PLUGIN_GATEWAY_GAUNTLET_IDS   Comma-separated plugin ids to include
  REMOTECLAW_PLUGIN_GATEWAY_GAUNTLET_TOTAL Total plugin shards
  REMOTECLAW_PLUGIN_GATEWAY_GAUNTLET_INDEX Zero-based shard index
  REMOTECLAW_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION=1
  REMOTECLAW_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT=1
  REMOTECLAW_PLUGIN_GATEWAY_GAUNTLET_QA_SUMMARY_MAX_BYTES  QA summary read ceiling
`);
}
...
function remoteclawCommand(repoRoot, args) {
  return {
    command: process.execPath,
...
function sourceRemoteclawCommand(repoRoot, args) {
  return {
    command: process.execPath,
...
function buildSlashHelpProbe(params) {
  ...
  return {
    ...
    ...remoteclawCommand(params.repoRoot, [command, "--help"]),
...
async function runPluginLifecycleCommand(params) {
  ...
  params.rows.push(
    await runMeasuredCommand({
      ...
      ...remoteclawCommand(params.repoRoot, ["plugins", ...params.args]),
      label: params.label,
...
async function runQaChunks(params) {
  ...
  for (let index = 0; index < chunks.length; index += 1) {
    ...
    const row = await runMeasuredCommand({
      ...
      ...sourceRemoteclawCommand(params.repoRoot, [
        "qa",
        "suite",
        "--provider-mode",
        "mock-openai",
...
async function main() {
  ...
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-plugin-gauntlet-"));
