...
describe("isVolatileBackupPath", () => {
  ...
  it("does not match paths that escape the anchor via `..`", () => {
    // `/opt/remoteclaw/state/sessions/../config.jsonl` resolves to
    // `/opt/remoteclaw/state/config.jsonl`, which is NOT inside sessions/.
  ...
  it("normalizes Windows-style separators before anchor checks", () => {
    const winStateDir = "C:\\remoteclaw\\state";
  ...
  it("matches tar filter paths when node-tar omits the leading slash", () => {
    expect(
      isVolatileBackupPath("opt/remoteclaw/state/agents/main/sessions/transcript.jsonl", plan),
    ).toBe(true);
  });
