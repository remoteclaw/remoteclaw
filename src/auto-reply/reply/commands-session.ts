...
import { scheduleGatewaySigusr1Restart, triggerRemoteClawRestart } from "../../infra/restart.js";
...
// import ... from "./commands-session-abort.js";
// oxlint-disable-next-line typescript/no-explicit-any
const handleAbortTrigger = (..._args: unknown[]) => undefined as any;
// oxlint-disable-next-line typescript/no-explicit-any
const handleStopCommand = (..._args: unknown[]) => undefined as any;
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting RemoteClaw in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerRemoteClawRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting RemoteClaw via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
