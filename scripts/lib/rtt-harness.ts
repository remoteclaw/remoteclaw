// Rtt Harness script supports RemoteClaw repository automation.
...
const REMOTECLAW_PACKAGE_SPEC_RE =
  /^remoteclaw@(main|alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

const REQUIRED_TELEGRAM_ENV = [
  "REMOTECLAW_QA_TELEGRAM_GROUP_ID",
  "REMOTECLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN",
...
export function validateRemoteClawPackageSpec(spec: string) {
  if (!REMOTECLAW_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `Package spec must be remoteclaw@main, remoteclaw@alpha, remoteclaw@beta, remoteclaw@latest, or an exact RemoteClaw release version; got: ${spec}`,
    );
  }
  return spec;
}
...
export function createHarnessEnv(params: {
  ...
  return {
    ...params.baseEnv,
    REMOTECLAW_NPM_TELEGRAM_PACKAGE_SPEC: params.spec,
    ...
    REMOTECLAW_NPM_TELEGRAM_WARM_SAMPLES: String(params.samples),
    REMOTECLAW_NPM_TELEGRAM_SAMPLE_TIMEOUT_MS: String(params.sampleTimeoutMs),
    REMOTECLAW_QA_TELEGRAM_CANARY_TIMEOUT_MS: String(params.timeoutMs),
    REMOTECLAW_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: String(params.timeoutMs),
  };
}
...
export async function assertHarnessRoot(harnessRoot: string) {
  ...
  try {
    ...
    throw new Error(`Missing RemoteClaw Telegram npm harness: ${scriptPath}`);
  }
}
...
export async function resolveMainVersion(harnessRoot: string) {
  ...
  if (typeof packageJson.version !== "string" || packageJson.version.trim().length === 0) {
    throw new Error("RemoteClaw package.json must contain a non-empty version.");
  }
