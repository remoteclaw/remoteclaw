...
describe("gateway-watch tmux wrapper", () => {
  it("derives stable session names from profile and port", () => {
    expect(resolveGatewayWatchTmuxSessionName({ args: ["gateway", "--force"], env: {} })).toBe(
      "remoteclaw-gateway-watch-main",
    );
    expect(
      resolveGatewayWatchTmuxSessionName({
        args: ["gateway", "--force", "--port", "19001"],
        env: { REMOTECLAW_PROFILE: "Dev Profile" },
      }),
    ).toBe("remoteclaw-gateway-watch-dev-profile-19001");
    expect(
      resolveGatewayWatchTmuxSessionName({
        args: ["--dev", "gateway", "--port=18789"],
        env: {},
      }),
    ).toBe("remoteclaw-gateway-watch-dev");
  });
  ...
  it("builds a login-shell command that runs the raw watcher in the repo", () => {
    const command = buildGatewayWatchTmuxCommand({
      ...
      cwd: "/repo with spaces/remoteclaw",
      env: {
        REMOTECLAW_GATEWAY_PORT: "19001",
        REMOTECLAW_PROFILE: "Dev Profile",
        SHELL: "/bin/zsh",
      },
      nodePath: "/opt/node",
      sessionName: "remoteclaw-gateway-watch-main",
    });

    expect(command).toContain("exec '/bin/zsh' -lc");
    expect(command).toContain("/repo with spaces/remoteclaw");
    expect(command).toContain("'REMOTECLAW_GATEWAY_WATCH_TMUX_CHILD=1'");
    expect(command).toContain("'REMOTECLAW_GATEWAY_WATCH_SESSION=remoteclaw-gateway-watch-main'");
    expect(command).toContain("'\\''-u'\\'' '\\''NO_COLOR'\\''");
    expect(command).toContain("'FORCE_COLOR=1'");
    expect(command).toContain("'REMOTECLAW_GATEWAY_PORT=19001'");
    expect(command).toContain("'REMOTECLAW_PROFILE=Dev Profile'");
  ...
  it("consumes benchmark flags and passes the CPU profile dir to the watched child", () => {
    ...
    expect(command).toContain(
      "'REMOTECLAW_RUN_NODE_CPU_PROF_DIR=.artifacts/gateway-watch-profiles'",
    );
  ...
  it("creates a detached tmux session when none exists", () => {
    ...
    expect(code).toBe(0);
    expect(
      expectSpawn(spawnSync, 0, "tmux", ["has-session", "-t", "remoteclaw-gateway-watch-main"])
        .encoding,
    ).toBe("utf8");
    ...
    expect(
      expectSpawn(spawnSync, 3, "tmux", [
        ...
        "-t",
        "remoteclaw-gateway-watch-main",
        "REMOTECLAW_GATEWAY_WATCH_CWD",
        "/repo",
      ]).encoding,
    ).toBe("utf8");
    expect(stderr.chunks.join("")).toContain(
      "gateway:watch started in tmux session remoteclaw-gateway-watch-main",
    );
    expect(stdout.chunks.join("")).toContain("tmux attach -t remoteclaw-gateway-watch-main");
    expect(stdout.chunks.join("")).toContain(
      "tmux show-options -v -t remoteclaw-gateway-watch-main @remoteclaw.gateway_watch.cwd",
    );
  });
  ...
  it("auto-attaches in an interactive terminal after creating a session", () => {
    ...
    expect(code).toBe(0);
    expect(
      expectSpawn(spawnSync, 4, "tmux", ["attach-session", "-t", "remoteclaw-gateway-watch-main"])
        .stdio,
    ).toBe("inherit");
  ...
  it("switches tmux clients instead of nesting attach when already inside tmux", () => {
    ...
    expect(code).toBe(0);
    expect(
      expectSpawn(spawnSync, 4, "tmux", ["switch-client", "-t", "remoteclaw-gateway-watch-main"])
        .stdio,
    ).toBe("inherit");
  });
  ...
  it("respawns the existing tmux pane on repeated runs", () => {
    ...
    const code = runGatewayWatchTmuxMain({
      args: ["gateway", "--force", "--port=19001"],
      cwd: "/repo",
      env: { REMOTECLAW_PROFILE: "dev", SHELL: "/bin/zsh" },
      nodePath: "/node",
      spawnSync,
      stderr: stderr.stream,
      stdout: stdout.stream,
    ...
    expect(respawnArgs.slice(0, 6)).toEqual([
      ...
      "-k",
      "-t",
      "remoteclaw-gateway-watch-dev-19001",
      "-c",
      "/repo",
    ]);
    ...
    expect(stderr.chunks.join("")).toContain(
      "gateway:watch restarted in tmux session remoteclaw-gateway-watch-dev-19001",
    );
  });
  ...
  it("recreates a stale session when its active pane target is missing", () => {
    ...
    expect(
      expectSpawn(spawnSync, 2, "tmux", ["kill-session", "-t", "remoteclaw-gateway-watch-main"])
        .encoding,
    ).toBe("utf8");
  ...
  it("runs the raw foreground watcher when tmux mode is disabled", () => {
    ...
    const code = runGatewayWatchTmuxMain({
      args: ["gateway", "--force"],
      cwd: "/repo",
      env: { REMOTECLAW_GATEWAY_WATCH_TMUX: "0" },
      nodePath: "/node",
      spawnSync,
    });
