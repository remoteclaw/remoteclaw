...
remoteclaw proxy validate [--json] [--proxy-url <url>] [--allowed-url <url>] [--denied-url <url>] [--apns-reachable] [--apns-authority <url>] [--timeout-ms <ms>]
...
`--proxy-url`, config, or `REMOTECLAW_PROXY_URL`. It reports a config problem when
