...
```bash
openclaw plugins install clawhub:remoteclaw-codex-app-server
...
```bash
openclaw plugins install remoteclaw-codex-app-server
```

Use `npm:` to make npm-only resolution explicit:

```bash
openclaw plugins install npm:remoteclaw-codex-app-server
...
openclaw plugins update @remoteclaw/voice-call
openclaw plugins update remoteclaw-codex-app-server --dangerously-force-unsafe-install
```
