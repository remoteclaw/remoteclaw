...
function remoteAwsMacosJsBootstrap({ packageManager = false, bun = false } = {}) {
  ...
  const bootstrap = [
    ...
    'PATH=*) if [ "$remoteclaw_env_ignore" = "1" ]; then remoteclaw_env_args+=("PATH=$PATH:${1#PATH=}"); else remoteclaw_env_args+=("$1"); fi; remoteclaw_env_path_seen=1; shift ;;',
    '[A-Za-z_]*=*) remoteclaw_env_args+=("$1"); shift ;;',
    '--) remoteclaw_env_args+=("--"); shift; break ;;',
    "*) break ;;",
    "esac;",
    "done;",
    'if [ "$remoteclaw_env_ignore" = "1" ] && [ "$remoteclaw_env_path_seen" = "0" ]; then remoteclaw_env_args+=("PATH=$PATH"); fi;',
