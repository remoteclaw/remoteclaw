...
Bundled plugins ship with RemoteClaw. Some are enabled by default (for example
...
`remoteclaw.install.allowInvalidConfigRecovery`.
...
override, while `remoteclaw skills install` remains a separate ClawHub skill
...
RemoteClaw now also prefers ClawHub for bare npm-safe plugin specs. It only falls
...
RemoteClaw downloads the package archive from ClawHub, checks the advertised
...
inside the cloned marketplace repo. RemoteClaw accepts relative path sources from
...
`remoteclaw gateway run` child, not only a wrapper process.

For runtime hook debugging:

- `remoteclaw plugins inspect <id> --json` shows registered hooks and diagnostics
  from a module-loaded inspection pass.
- `remoteclaw gateway status --deep --require-rpc` confirms the reachable Gateway,
...
or exact version. RemoteClaw resolves that package name back to the tracked plugin
