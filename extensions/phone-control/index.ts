...
import type { OpenClawPluginApi, OpenClawPluginService } from "remoteclaw/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: RemoteClawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: RemoteClawPluginApi["config"]): string[] {
...
export default function register(api: RemoteClawPluginApi) {
  ...
  const timerService: RemoteClawPluginService = {
