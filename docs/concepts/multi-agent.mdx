...
- **State directory** (`agentDir`) for auth profiles and per-agent config.
...
`channels.<channel>.defaultAccount` (optional). When unset, RemoteClaw falls back
...
Split by channel: route WhatsApp to an everyday agent and Telegram to a dedicated deep-work agent.
    ...
    list: [
      {
        id: "daily",
        name: "Everyday",
        workspace: "~/.remoteclaw/workspace-daily",
      },
      {
        id: "deep-work",
        name: "Deep Work",
        workspace: "~/.remoteclaw/workspace-deep-work",
      },
    ],
  },
  bindings: [
...
- To route a single DM/group to the deep-work agent while keeping the rest on daily, add a `match.peer` binding for that peer; peer matches always win over channel-wide rules.

## Example: same channel, one peer to dedicated agent

Keep WhatsApp on the everyday agent, but route one DM to a dedicated agent:
