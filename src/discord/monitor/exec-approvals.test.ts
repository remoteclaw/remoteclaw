...
const STORE_PATH = path.join(os.tmpdir(), "remoteclaw-exec-approvals-test.json");
...
beforeEach(() => {
  ...
  mockResolveGatewayConnectionAuth.mockReset().mockImplementation(
    async (params: {
      ...
      const envToken = params.env.REMOTECLAW_GATEWAY_TOKEN ?? params.env.CLAWDBOT_GATEWAY_TOKEN;
      const envPassword =
        params.env.REMOTECLAW_GATEWAY_PASSWORD ?? params.env.CLAWDBOT_GATEWAY_PASSWORD;
...
describe("DiscordExecApprovalHandler gateway auth", () => {
  ...
  it("prefers REMOTECLAW_GATEWAY_TOKEN when config token is missing", async () => {
    vi.stubEnv("REMOTECLAW_GATEWAY_TOKEN", "env-gateway-token");
    const handler = new DiscordExecApprovalHandler({
      token: "discord-bot-token",
      accountId: "default",
      config: { enabled: true, approvers: ["123"] },
...
describe("DiscordExecApprovalHandler gateway auth resolution", () => {
  ...
  it("passes env URL overrides to shared gateway auth resolver", async () => {
    const previousGatewayUrl = process.env.REMOTECLAW_GATEWAY_URL;
    try {
      process.env.REMOTECLAW_GATEWAY_URL = "wss://gateway-from-env.example/ws";
      const handler = new DiscordExecApprovalHandler({
        token: "test-token",
        accountId: "default",
        config: { enabled: true, approvers: ["123"] },
      ...
      if (typeof previousGatewayUrl === "string") {
        process.env.REMOTECLAW_GATEWAY_URL = previousGatewayUrl;
      } else {
