...
async function resolveChannelPluginForMode(
  ...
  const supportsMode = mode === "login" ? Boolean(plugin?.auth?.login) : Boolean(plugin?.gateway?.logoutAccount);
  if (!supportsMode) {
...
function resolveAccountContext(plugin: ChannelPlugin, opts: ChannelAuthOptions, cfg: RemoteClawConfig) {
  const accountId = opts.account?.trim() || resolveChannelDefaultAccountId({ plugin, cfg });
  return { accountId };
}

export async function runChannelLogin(opts: ChannelAuthOptions, runtime: RuntimeEnv = defaultRuntime) {
...
export async function runChannelLogout(opts: ChannelAuthOptions, runtime: RuntimeEnv = defaultRuntime) {
  const cfg = loadConfig();
