...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “RemoteClaw runs”).
...
- `remoteclaw health` and `remoteclaw status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `remoteclaw gateway status`, `remoteclaw gateway health`, `remoteclaw gateway probe`, and `remoteclaw gateway call` can also target the forwarded URL via `--url` when needed.

Note: replace `18789` with your configured `gateway.port` (or `--port`/`REMOTECLAW_GATEWAY_PORT`).
...
- URL override safety:
  ...
  - Env URL overrides (`REMOTECLAW_GATEWAY_URL`) may use env credentials only (`REMOTECLAW_GATEWAY_TOKEN` / `REMOTECLAW_GATEWAY_PASSWORD`).
- Local mode defaults:
  - token: `REMOTECLAW_GATEWAY_TOKEN` -> `gateway.auth.token` -> `gateway.remote.token` (remote fallback applies only when local auth token input is unset)
  - password: `REMOTECLAW_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password` (remote fallback applies only when local auth password input is unset)
...
- Gateway env overrides use `REMOTECLAW_GATEWAY_*` only.
...
- Plaintext `ws://` is loopback-only by default. For trusted private networks,
  set `REMOTECLAW_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as
  break-glass. There is no `remoteclaw.json` equivalent; this must be process
...
```bash
remoteclaw config set gateway.remote.token "<your-token>"
```

#### Step 4: create the LaunchAgent

Save this as `~/Library/LaunchAgents/org.remoteclaw.ssh-tunnel.plist`:
...
<dict>
    <key>Label</key>
    <string>org.remoteclaw.ssh-tunnel</string>
...
```bash
launchctl bootstrap gui/$UID ~/Library/LaunchAgents/org.remoteclaw.ssh-tunnel.plist
```

The tunnel will start automatically at login, restart on crash, and keep the forwarded port live.

Note: if you have a leftover `com.remoteclaw.ssh-tunnel` LaunchAgent from an older setup, unload and delete it.
...
```bash
launchctl kickstart -k gui/$UID/org.remoteclaw.ssh-tunnel
```

Stop the tunnel:

```bash
launchctl bootout gui/$UID/org.remoteclaw.ssh-tunnel
```
