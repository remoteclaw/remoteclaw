...
by default with a plugin-local pinned `acpx` binary that RemoteClaw probes
...
<AccordionGroup>
  <Accordion title="Binding rules and exclusivity">
    ...
    - On Discord, `spawnAcpSessions` is only required when RemoteClaw needs to create a child thread for `--thread auto|here` — not for `--bind here`.
