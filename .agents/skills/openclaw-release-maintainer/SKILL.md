---
name: remoteclaw-release-maintainer
description: Maintainer workflow for RemoteClaw releases, prereleases, changelog release notes, and publish validation. Use when Codex needs to prepare or verify stable or beta release steps, align version naming, assemble release notes, check release auth requirements, or validate publish-time commands and artifacts.
---

# RemoteClaw Release Maintainer
...
- Core `remoteclaw` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- The production Sparkle feed lives at `https://raw.githubusercontent.com/remoteclaw/remoteclaw/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `remoteclaw` repo.
...
- When cutting a mac release with a beta GitHub prerelease:
  ...
  - create a prerelease titled `remoteclaw YYYY.M.D-beta.N`
...
- Always validate the RemoteClaw npm release path before creating the tag.
...
- RemoteClaw publish uses GitHub trusted publishing.
...
- The `npm-release` environment must be approved by `@remoteclaw/remoteclaw-release-managers` before publish continues.
...
- Do not use `NPM_TOKEN` or the plugin OTP flow for RemoteClaw releases.
- `@remoteclaw/*` plugin publishes use a separate maintainer-only flow.
...
9. Start `.github/workflows/remoteclaw-npm-release.yml` with `preflight_only=true`
...
12. Start `.github/workflows/remoteclaw-npm-release.yml` with the same tag for
    the real publish.
13. Wait for `npm-release` approval from `@remoteclaw/remoteclaw-release-managers`.
...
- Use `remoteclaw-ghsa-maintainer` for GHSA advisory inspection, patch/publish flow, private-fork validation, and GHSA API-specific publish checks.
