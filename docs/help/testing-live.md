...
```bash
pnpm remoteclaw infer tts convert --local --json \
  --text "RemoteClaw live smoke." \
  --output /tmp/remoteclaw-live-smoke.mp3
```

Safe voice-call readiness smoke:

```bash
pnpm remoteclaw voicecall setup --json
pnpm remoteclaw voicecall smoke --to "+15555550123"
```
...
- Optional target overrides:
  ...
  - `REMOTECLAW_ANDROID_GATEWAY_URL` / `REMOTECLAW_ANDROID_GATEWAY_TOKEN` / `REMOTECLAW_ANDROID_GATEWAY_PASSWORD`.
...
- Set `REMOTECLAW_LIVE_MODELS=modern`, `small`, or `all` (alias for modern) to actually run this suite; otherwise it skips to keep `pnpm test:live` focused on gateway smoke
- How to select models:
  - `REMOTECLAW_LIVE_MODELS=modern` to run the modern allowlist (Opus/Sonnet 4.6+, GPT-5.2 + Codex, Gemini 3, DeepSeek V4, GLM 4.7, MiniMax M3, Grok 4.3)
  - `REMOTECLAW_LIVE_MODELS=small` to run the constrained small-model allowlist (Qwen 8B/9B local-compatible routes, Ollama Gemma, OpenRouter Qwen/GLM, and Z.AI GLM)
  - `REMOTECLAW_LIVE_MODELS=all` is an alias for the modern allowlist
  - or `REMOTECLAW_LIVE_MODELS="openai/gpt-5.5,anthropic/claude-opus-4-6,..."` (comma allowlist)
  - Local Ollama small-model runs default to `http://127.0.0.1:11434`; set `REMOTECLAW_LIVE_OLLAMA_BASE_URL` only for LAN, custom, or Ollama Cloud endpoints.
  - Modern/all and small sweeps default to their curated caps; set `REMOTECLAW_LIVE_MAX_MODELS=0` for an exhaustive selected-profile sweep or a positive number for a smaller cap.
  - Exhaustive sweeps use `REMOTECLAW_LIVE_TEST_TIMEOUT_MS` for the whole direct-model test timeout. Default: 60 minutes.
  - Direct-model probes run with 20-way parallelism by default; set `REMOTECLAW_LIVE_MODEL_CONCURRENCY` to override.
...
- Overrides (optional):
  ...
  - `REMOTECLAW_LIVE_CLI_BACKEND_ARGS='["-p","--output-format","json"]'`
  - `REMOTECLAW_LIVE_CLI_BACKEND_IMAGE_PROBE=1` to send a real image attachment (paths are injected into the prompt). Docker recipes default this off unless explicitly requested.
  ...
  - `REMOTECLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE=1` to opt into the Claude Sonnet -> Opus same-session continuity probe when the selected model supports a switch target. Docker recipes default this off for aggregate reliability.
  - `REMOTECLAW_LIVE_CLI_BACKEND_MCP_PROBE=1` to opt into the MCP/tool loopback probe. Docker recipes default this off unless explicitly requested.
...
settings RemoteClaw gives Gemini, then runs `gemini --debug mcp list` to prove a
...
- It resolves CLI smoke metadata from the owning extension, then installs the matching Linux CLI package (`@anthropic-ai/claude-code` or `@google/gemini-cli`) into a cached writable prefix at `REMOTECLAW_DOCKER_CLI_TOOLS_DIR` (default: `~/.cache/remoteclaw/docker-cli-tools`).
...
- Overrides:
  ...
  - `REMOTECLAW_LIVE_ACP_BIND_AGENT=codex`
  - `REMOTECLAW_LIVE_ACP_BIND_AGENT=droid`
  - `REMOTECLAW_LIVE_ACP_BIND_AGENT=gemini`
  - `REMOTECLAW_LIVE_ACP_BIND_AGENT=opencode`
  - `REMOTECLAW_LIVE_ACP_BIND_AGENTS=claude,codex,gemini`
  - `REMOTECLAW_LIVE_ACP_BIND_AGENT_COMMAND='npx -y @agentclientprotocol/claude-agent-acp@<version>'`
  - `REMOTECLAW_LIVE_ACP_BIND_CODEX_MODEL=gpt-5.5`
  - `REMOTECLAW_LIVE_ACP_BIND_OPENCODE_MODEL=opencode/kimi-k2.6`
  - `REMOTECLAW_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1`
  - `REMOTECLAW_LIVE_ACP_BIND_REQUIRE_CRON=1`
  - `REMOTECLAW_LIVE_ACP_BIND_PARENT_MODEL=openai/gpt-5.5`
- Notes:
  - This lane uses the gateway `chat.send` surface with admin-only synthetic originating-route fields so tests can attach message-channel context without pretending to deliver externally.
  - When `REMOTECLAW_LIVE_ACP_BIND_AGENT_COMMAND` is unset, the test uses the embedded `acpx` plugin's built-in agent registry for the selected ACP harness agent.
  - Bound-session cron MCP creation is best-effort by default because external ACP harnesses can cancel MCP calls after the bind/image proof has passed; set `REMOTECLAW_LIVE_ACP_BIND_REQUIRE_CRON=1` to make that post-bind cron probe strict.
...
- Use `REMOTECLAW_LIVE_ACP_BIND_AGENTS=claude`, `REMOTECLAW_LIVE_ACP_BIND_AGENTS=codex`, `REMOTECLAW_LIVE_ACP_BIND_AGENTS=droid`, `REMOTECLAW_LIVE_ACP_BIND_AGENTS=gemini`, or `REMOTECLAW_LIVE_ACP_BIND_AGENTS=opencode` to narrow the matrix.
...
- The OpenCode Docker variant is a strict single-agent regression lane. It writes a temporary `OPENCODE_CONFIG_CONTENT` default model from `REMOTECLAW_LIVE_ACP_BIND_OPENCODE_MODEL` (default `opencode/kimi-k2.6`), and `pnpm test:docker:live-acp-bind:opencode` requires a bound assistant transcript instead of accepting the generic post-bind skip.
- Direct `acpx` CLI calls are only a manual/workaround path for comparing behavior outside the Gateway. The Docker ACP bind smoke exercises RemoteClaw's embedded `acpx` runtime backend.
...
- Goal: validate the plugin-owned Codex harness through the normal gateway
  ...
  - send a second turn to the same RemoteClaw session and verify the app-server
    thread can resume
...
- Enable: `REMOTECLAW_LIVE_CODEX_HARNESS=1`
...
- The smoke forces provider/model `agentRuntime.id: "codex"` so a broken Codex
  harness cannot pass by silently falling back to RemoteClaw.
...
```bash
REMOTECLAW_LIVE_CODEX_HARNESS=1 \
  REMOTECLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE=1 \
  REMOTECLAW_LIVE_CODEX_HARNESS_MCP_PROBE=1 \
  REMOTECLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=1 \
  REMOTECLAW_LIVE_CODEX_HARNESS_MODEL=openai/gpt-5.5 \
...
- Docker enables the image, MCP/tool, and Guardian probes by default. Set
  `REMOTECLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE=0` or
  ...
  run.
- Docker uses the same explicit Codex runtime config, so legacy aliases or RemoteClaw
...
- Single model, direct (no gateway):
  - `REMOTECLAW_LIVE_MODELS="openai/gpt-5.5" pnpm test:live src/agents/models.profiles.live.test.ts`
...
- Single model, gateway smoke:
  - `REMOTECLAW_LIVE_GATEWAY_MODELS="openai/gpt-5.5" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`

- Tool calling across several providers:
  - `REMOTECLAW_LIVE_GATEWAY_MODELS="openai/gpt-5.5,anthropic/claude-opus-4-6,google/gemini-3-flash-preview,deepseek/deepseek-v4-flash,zai/glm-5.1,minimax/MiniMax-M3" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
- Google adaptive thinking smoke:
  - Gemini 3 dynamic default: `pnpm remoteclaw qa manual --provider-mode live-frontier --model google/gemini-3.1-pro-preview --alt-model google/gemini-3.1-pro-preview --message '/think adaptive Reply exactly: GEMINI_ADAPTIVE_OK' --timeout-ms 180000`
  - Gemini 2.5 dynamic budget: `pnpm remoteclaw qa manual --provider-mode live-frontier --model google/gemini-2.5-flash --alt-model google/gemini-2.5-flash --message '/think adaptive Reply exactly: GEMINI25_ADAPTIVE_OK' --timeout-ms 180000`
...
`REMOTECLAW_LIVE_GATEWAY_MODELS="openai/gpt-5.5,anthropic/claude-opus-4-6,google/gemini-3.1-pro-preview,google/gemini-3-flash-preview,google-antigravity/claude-opus-4-6-thinking,google-antigravity/gemini-3-flash,deepseek/deepseek-v4-flash,zai/glm-5.1,minimax/MiniMax-M3" pnpm test:live src/gateway/gateway-models.profiles.live.test.ts`
...
- Per-agent auth profiles: `~/.remoteclaw/agents/<agentId>/agent/auth-profiles.json` (this is what "profile keys" means in the live tests)
- Config: `~/.remoteclaw/remoteclaw.json` (or `REMOTECLAW_CONFIG_PATH`)
- Legacy state dir: `~/.remoteclaw/credentials/` (copied into the staged live home when present, but not the main profile-key store)
...
Docker runners below with an explicit `REMOTECLAW_PROFILE_FILE`.
...
- Enable: `REMOTECLAW_LIVE_TEST=1 COMFY_LIVE_TEST=1 pnpm test:live -- extensions/comfy/comfy.live.test.ts`
- Scope:
...
- Current bundled providers covered:
  ...
  - `xai`
- Optional narrowing:
  - `REMOTECLAW_LIVE_IMAGE_GENERATION_PROVIDERS="openai,google,openrouter,xai"`
  - `REMOTECLAW_LIVE_IMAGE_GENERATION_PROVIDERS="deepinfra"`
  - `REMOTECLAW_LIVE_IMAGE_GENERATION_MODELS="openai/gpt-image-2,google/gemini-3.1-flash-image-preview,openrouter/google/gemini-3.1-flash-image-preview,xai/grok-imagine-image"`
  - `REMOTECLAW_LIVE_IMAGE_GENERATION_CASES="google:flash-generate,google:pro-edit,openrouter:generate,xai:default-generate,xai:default-edit"`
- Optional auth behavior:
  - `REMOTECLAW_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides

For the shipped CLI path, add an `infer` smoke after the provider/runtime live
test passes:

```bash
REMOTECLAW_LIVE_TEST=1 REMOTECLAW_LIVE_INFER_CLI_TEST=1 pnpm test:live -- test/image-generation.infer-cli.live.test.ts
remoteclaw infer image providers --json
remoteclaw infer image generate \
  --model google/gemini-3.1-flash-image-preview \
  --prompt "Minimal flat test image: one blue square on a white background, no text." \
  --output ./remoteclaw-infer-image-smoke.png \
  --json
```
...
- Enable: `REMOTECLAW_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts`
...
- Optional narrowing:
  - `REMOTECLAW_LIVE_MUSIC_GENERATION_PROVIDERS="google,minimax"`
  - `REMOTECLAW_LIVE_MUSIC_GENERATION_MODELS="google/lyria-3-clip-preview,minimax/music-2.6"`
- Optional auth behavior:
  - `REMOTECLAW_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
...
- Enable: `REMOTECLAW_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts`
- Harness: `pnpm test:live:media video`
- Scope:
  - Exercises the shared bundled video-generation provider path
  - Defaults to the release-safe smoke path: non-FAL providers, one text-to-video request per provider, one-second lobster prompt, and a per-provider operation cap from `REMOTECLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` by default)
  - Skips FAL by default because provider-side queue latency can dominate release time; pass `--video-providers fal` or `REMOTECLAW_LIVE_VIDEO_GENERATION_PROVIDERS="fal"` to run it explicitly
  - Uses already-exported provider env vars before probing
  - Uses live/env API keys ahead of stored auth profiles by default, so stale test keys in `auth-profiles.json` do not mask real shell credentials
  - Skips providers with no usable auth/profile/model
  - Runs only `generate` by default
  - Set `REMOTECLAW_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared transform modes when available:
  ...
  - Provider-specific Vydra coverage:
    - `REMOTECLAW_LIVE_TEST=1 REMOTECLAW_LIVE_VYDRA_VIDEO=1 pnpm test:live -- extensions/vydra/vydra.live.test.ts`
...
- Optional narrowing:
  - `REMOTECLAW_LIVE_VIDEO_GENERATION_PROVIDERS="deepinfra,google,openai,runway"`
  - `REMOTECLAW_LIVE_VIDEO_GENERATION_MODELS="google/veo-3.1-fast-generate-preview,openai/sora-2,runway/gen4_aleph"`
  - `REMOTECLAW_LIVE_VIDEO_GENERATION_SKIP_PROVIDERS=""` to include every provider in the default sweep, including FAL
  - `REMOTECLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS=60000` to reduce each provider operation cap for an aggressive smoke run
- Optional auth behavior:
  - `REMOTECLAW_LIVE_REQUIRE_PROFILE_KEYS=1` to force profile-store auth and ignore env-only overrides
