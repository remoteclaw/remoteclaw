...
RemoteClaw routes replies **back to the channel where a message came from**. The
...
RemoteClaw infers a pinned owner from `allowFrom` when all of these are true:
...
Broadcast groups let you run **multiple agents** for the same peer **when RemoteClaw would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.remoteclaw/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.remoteclaw`):

- `~/.remoteclaw/agents/<agentId>/sessions/sessions.json`
