...
const DREAMING_TOKEN = "__remoteclaw_memory_core_short_term_promotion_dream__";
...
describe("migrateLegacyDreamingPayloadShape", () => {
  ...
  it("re-applies missing pieces (e.g. lightContext flag) on partially-migrated jobs", () => {
    ...
    const result = migrateLegacyDreamingPayloadShape(jobs);
    expect(result.rewrittenCount).toBe(1);
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion -- tsgo types jobs[0].payload as unknown; the cast is required to access lightContext
