...
export const MODULE_ATTESTATIONS = {
  maybeWrapCommandWithShellSnapshot: "live",
  resetShellSnapshotCacheForTests: "live",
  resolveShellSnapshotDir: "live",
} as const;

const SNAPSHOT_VERSION = 1;
const SNAPSHOT_REFRESH_MS = 5 * 60 * 1000;
const SNAPSHOT_MAX_AGE_MS = 3 * 24 * 60 * 60 * 1000;
const CAPTURE_MARKER = "__REMOTECLAW_SHELL_SNAPSHOT_CAPTURE__";
const ENV_MARKER = "__REMOTECLAW_SHELL_SNAPSHOT_ENV__";
const EXEC_SHELL_SNAPSHOT_ENV = "REMOTECLAW_EXEC_SHELL_SNAPSHOT";
...
async function captureShellSnapshot(opts: ShellSnapshotWrapOptions): Promise<string | null> {
  ...
  const captureOutputDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-shell-snapshot-"));
...
function buildTrustedSnapshotCaptureEnv(
  ...
  // REMOTECLAW_SHELL is injected by the exec runtime, so startup files can keep
  // their documented exec-specific branches without trusting model input.
  if (runtimeEnv.REMOTECLAW_SHELL === "exec") {
    env.REMOTECLAW_SHELL = "exec";
  }
  return env;
}
...
function buildSnapshotFile(stdout: string): string | null {
  ...
  return [
    "# RemoteClaw exec shell snapshot. Generated; do not edit.",
