...
import type { FastMode } from "@remoteclaw/normalization-core/string-coerce";
...
// SessionGoal is defined locally: the fork's src/config/sessions/types.ts stays
// at fork state (reclassified), and src/config/sessions/goals.ts is upstream-only.
...
export type { FastMode } from "@remoteclaw/normalization-core/string-coerce";
...
// Usage types are not re-exported: ui/src/pages/usage/data-types.ts depends on
// src/infra/provider-usage.types, src/shared/usage-types and
// src/shared/session-usage-timeseries-types, none of which the fork carries.
...
// The fork keeps the Gateway protocol under src/gateway/protocol/ rather than
// upstream's packages/gateway-protocol/ workspace package.
...
// ToolsEffectiveEntry/ToolsEffectiveResult are not in the fork's ProtocolSchemas
// registry and no Control UI surface consumes them.
// ModelAuthStatus* belonged to the gutted model-provider ecosystem.
...
// dropped them; lifted verbatim from ui/src/ui/types.ts.
