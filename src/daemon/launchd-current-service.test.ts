...
describe("isCurrentProcessLaunchdServiceLabel", () => {
  it("matches launchd-provided service labels", () => {
    expect(
      isCurrentProcessLaunchdServiceLabel("org.remoteclaw.gateway", {
        LAUNCH_JOB_LABEL: "org.remoteclaw.gateway",
      }),
    ).toBe(true);
  });

  it("falls back to RemoteClaw service markers when XPC_SERVICE_NAME is inherited", () => {
    expect(
      isCurrentProcessLaunchdServiceLabel("org.remoteclaw.gateway", {
        XPC_SERVICE_NAME: "0",
        REMOTECLAW_SERVICE_MARKER: "remoteclaw",
        REMOTECLAW_SERVICE_KIND: "gateway",
        REMOTECLAW_LAUNCHD_LABEL: "org.remoteclaw.gateway",
      }),
    ).toBe(true);
  });
