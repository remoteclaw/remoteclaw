...
function stubChannelPlugin(params: {
  ...
  inspectAccount?: (cfg: RemoteClawConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: RemoteClawConfig) => string[];
  isConfigured?: (account: unknown, cfg: RemoteClawConfig) => boolean;
  isEnabled?: (account: unknown, cfg: RemoteClawConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
function successfulProbeResult(url: string) {
  return {
    ...
    error: null,
    close: null,
    auth: { role: null, scopes: [], capability: "unknown" as const },
    health: null,
    status: null,
    presence: null,
    configSnapshot: null,
...
`,
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevPassword = process.env.REMOTECLAW_GATEWAY_PASSWORD;
  ...
  it("keeps channel security findings when SecretRef credentials are configured but unavailable", async () => {
    await withChannelSecurityStateDir(async () => {
      ...
      const resolvedConfig: RemoteClawConfig = {
        channels: {
          discord: {
            enabled: true,
            groupPolicy: "allowlist",
      ...
      hooks: {
        internal: {
          installs: {
            "test-hooks": {
              source: "npm",
              spec: "@remoteclaw/test-hooks@1.2.3",
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      path.join(pluginDir, "package.json"),
      JSON.stringify({ name: "@remoteclaw/voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@remoteclaw/test-hooks", version: "8.8.8" }),
      "utf-8",
    );
