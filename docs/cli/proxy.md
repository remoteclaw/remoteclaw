...
RemoteClaw proxy routing. The other commands are debugging tools for
...
`remoteclaw proxy validate` checks the effective operator-managed proxy URL from
`--proxy-url`, config, or `REMOTECLAW_PROXY_URL`. Managed proxy URLs can use
`http://` for a plain forward-proxy listener or `https://` when RemoteClaw must
