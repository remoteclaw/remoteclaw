...
${formatHelpExamples([
  ['remoteclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['remoteclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['remoteclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.remoteclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.remoteclaw.ai/cli/agents")}\n`,
    );
