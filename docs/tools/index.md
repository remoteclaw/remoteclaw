---
summary: "RemoteClaw tools and plugins overview: what the agent can do and how to extend it"
read_when:
  - You want to understand what tools RemoteClaw provides
...
RemoteClaw has three layers that work together:

<Steps>
  <Step title="Tools are what the agent calls">
    A tool is a typed function the agent can invoke (e.g. `exec`, `browser`,
    `web_search`, `message`). RemoteClaw ships a set of **built-in tools** and
  ...
  <Step title="Plugins package everything together">
    ...
    Some plugins are **core** (shipped with RemoteClaw), others are **external**
...
These tools ship with RemoteClaw and are available without installing any plugins:
