...
describe("memory cli", () => {
  ...
  it("documents memory help examples", () => {
    ...
    expect(helpText).toContain("remoteclaw memory status --deep");
    expect(helpText).toContain("Probe embedding provider readiness.");
    expect(helpText).toContain('remoteclaw memory search "meeting notes"');
    expect(helpText).toContain("Quick search using positional query.");
    expect(helpText).toContain('remoteclaw memory search --query "deployment" --max-results 20');
