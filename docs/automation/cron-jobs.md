...
- `sessionTarget: "isolated"` or `sessionTarget: "session:..."` creates a visible task that shows up in `remoteclaw tasks` with delivery notifications.
...
- Jobs persist under `~/.remoteclaw/cron/` so restarts don’t lose schedules.
...
Cron jobs are persisted on the Gateway host at `~/.remoteclaw/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `remoteclaw cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, RemoteClaw applies a
...
Main-session cron jobs create [background task](/automation/tasks) records with `silent` notify policy (no notifications by default). They appear in `remoteclaw tasks list` but do not generate delivery messages.
...
These detached runs create [background task](/automation/tasks) records visible in `remoteclaw tasks` and subject to task audit and maintenance.
...
If `delivery` is omitted for isolated jobs, RemoteClaw defaults to `announce`.
...
- CLI equivalents: `remoteclaw cron add --light-context ...` and `remoteclaw cron edit --light-context`.
...
- `schedule.at` accepts ISO 8601. Tool/API values without a timezone are treated as UTC; the CLI also accepts `remoteclaw cron add|edit --at "<offset-less-iso>" --tz <iana>` for local wall-clock one-shots.
...
- Job store: `~/.remoteclaw/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.remoteclaw/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
...
- For removed run sessions no longer referenced by the session store, RemoteClaw archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `remoteclaw cron runs` and adjust retention before logs become large
...
remoteclaw cron edit <jobId> --agent ops
...
```bash
remoteclaw cron edit <jobId> --exact
```
...
```bash
remoteclaw system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`remoteclaw system event`](/cli/system).

## Troubleshooting

### "Nothing runs"

- Check cron is enabled: `cron.enabled` and `REMOTECLAW_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- RemoteClaw applies exponential retry backoff for recurring jobs after consecutive errors:
