...
export type ToolsConfig = {
  ...
  byProvider?: Record<string, ToolPolicyConfig>;
  /**
   * @deprecated Web search/fetch runtime config. Zod validation gutted
   * (parsed as `unknown`), but the typed shape is kept so in-tree consumers
   * (wizard, onboard-search, audit) still compile until they are removed
   * in a follow-up gut PR.
   */
  web?: {
    search?: {
