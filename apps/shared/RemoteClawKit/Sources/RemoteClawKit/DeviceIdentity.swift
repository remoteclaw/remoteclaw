...
enum DeviceIdentityPaths {
    private static let stateDirEnv = ["REMOTECLAW_STATE_DIR"]
    ...
    private static let appGroupStateDirAvailable =
        DeviceIdentityPaths.hasAppGroupEntitlement(RemoteClawAppGroup.identifier)
        ...
        temporaryDirectory: URL) -> URL
    {
        ...
        return temporaryDirectory.appendingPathComponent("remoteclaw", isDirectory: true)
    }
    ...
    private static func legacyStateDirURL() -> URL? {
        if let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
            return appSupport.appendingPathComponent("RemoteClaw", isDirectory: true)
        }
        return nil
    }
    ...
    private static func appGroupStateDirURL() -> URL? {
        guard
            let containerURL = FileManager.default
                .containerURL(forSecurityApplicationGroupIdentifier: RemoteClawAppGroup.identifier)
        else {
            return nil
        }
        return containerURL.appendingPathComponent("RemoteClaw", isDirectory: true)
    }
    ...
    /// An explicit REMOTECLAW_STATE_DIR override selects a caller-chosen store, not the legacy
