...
let RemoteClaw run external coding harnesses (for example Pi, Claude Code,
...
| Run Claude Code, Gemini CLI, explicit Codex ACP, or another external harness _through_ RemoteClaw | This page: ACP agents                 | Chat-bound sessions, `/acp spawn`, `sessions_spawn({ runtime: "acp" })`, background tasks, runtime controls                                                |
| Expose a RemoteClaw Gateway session _as_ an ACP server for an editor or client                    | [`remoteclaw acp`](/cli/acp)          | Bridge mode. IDE/client talks ACP to RemoteClaw over stdio/WebSocket                                                                                       |
...
| `pi`         | Pi/embedded RemoteClaw runtime                     | Used for RemoteClaw-native harness experiments.                                     |
...
<ParamField path="model" type="string">
  ...
  normalize RemoteClaw Codex refs such as `openai-codex/gpt-5.4` to Codex
...
| ---------------------------- | ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/acp model <id>`            | runtime config key `model`           | For Codex ACP, RemoteClaw normalizes `openai-codex/<model>` to the adapter model id and maps slash reasoning suffixes such as `openai-codex/gpt-5.4/high` to `reasoning_effort`. |
| `/acp set thinking <level>`  | runtime config key `thinking`        | For Codex ACP, RemoteClaw sends the corresponding `reasoning_effort` where the adapter supports one.                                                                             |
