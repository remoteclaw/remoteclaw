#!/usr/bin/env node
// Development runner that rebuilds RemoteClaw, runs runtime postbuild steps, and
...
const createRunNodeProgress = (label, deps) => {
  ...
  const render = () => {
    ...
    frameIndex += 1;
    deps.stderr.write(`\r[remoteclaw] ${frame} ${label} (${elapsedSeconds}s)`);
    visible = true;
  };
...
const closeRunNodeOutputTee = async (deps, exitCode) => {
  ...
  try {
    ...
    deps.stderr.write(
      `[remoteclaw] Failed to write output log: ${error?.message ?? "unknown error"}\n`,
    );
