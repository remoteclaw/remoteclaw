...
Code mode is an experimental, opt-in RemoteClaw agent-runtime feature. When
...
This page documents RemoteClaw code mode, not Codex Code Mode. The two features
...
- RemoteClaw code mode runs in the generic RemoteClaw agent runtime and is
...
- Every other enabled tool (RemoteClaw core, plugin, MCP, client) is hidden from
...
- Provider neutral: works for RemoteClaw, plugin, MCP, and client tools without
...
| Stability           | experimental RemoteClaw surface (Codex Code Mode is a separate, stable Codex harness surface) |
...
- **Code mode**: the RemoteClaw runtime mode that hides normal model tools and
...
If code mode is enabled but QuickJS-WASI cannot load, RemoteClaw fails closed
...
1. Resolve the agent, model, provider, sandbox, channel, sender, and run
   policy.
2. Build the effective RemoteClaw tool list, adding eligible plugin, MCP, and
   client tools.
...
when `tools.codeMode.enabled: true` is configured. Code mode and RemoteClaw Tool
...
- `command` is accepted as an exec-compatible alias for hook policies and
  trusted rewrites (the normal RemoteClaw shell exec tool also uses a `command`
...
4. RemoteClaw delivers nested tool results into the restored VM and drains
...
type ToolCatalogEntry = {
  ...
  label?: string;
  description: string;
  source: "remoteclaw" | "mcp" | "client";
  sourceName?: string;
};
```

Plugin tools use `source: "remoteclaw"` with `sourceName` set to the owning
...
directory. For each code-mode `exec` call, RemoteClaw builds the run-scoped tool
...
catalog for arbitrary enabled RemoteClaw, plugin, and client tools; use `MCP`
...
ordinary RemoteClaw tools, not through the code-mode bridge.

## Tool catalog

The hidden catalog includes tools after effective policy filtering, in this
order: RemoteClaw core tools, bundled plugin tools, external plugin tools, MCP
...
where `<source>` is `remoteclaw`, `mcp`, or `client` (plugin tools use
`remoteclaw` with the plugin id as `<owner>`; core tools use `remoteclaw:core:*`).
Examples:

```text
remoteclaw:core:message
remoteclaw:browser:browser_request
...
Code mode supersedes the RemoteClaw Tool Search model surface for runs where it
is active.
...
- MCP calls use the generated `MCP` namespace and its `$api()` headers instead
  of `tools.call(...)`.
- Nested calls dispatch through the same RemoteClaw executor path that Tool
  Search uses.

See [Tool Search](/tools/tool-search) for the RemoteClaw compact catalog bridge
...
- RemoteClaw caps the number of concurrently suspended runs per process (64) and
...
RemoteClaw loads `quickjs-wasi` as a direct dependency in the owning package; it
...
The runtime executes in a Node.js worker thread, outside RemoteClaw's main
...
breakdown (`remoteclaw`/`mcp`/`client` counts), cumulative search/describe/call
...
tool inputs beyond existing RemoteClaw trajectory policy.
