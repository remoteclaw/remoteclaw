...
const ORIGINAL_GATEWAY_TOKEN_ENV = process.env.REMOTECLAW_GATEWAY_TOKEN;
const OLD_TOKEN = "shared-token-old";
const NEW_TOKEN = "shared-token-new";
const DEFERRED_RESTART_DELAY_MS = 1_000;
const SECRET_REF_TOKEN_ID = "REMOTECLAW_SHARED_AUTH_ROTATION_SECRET_REF";
...
afterAll(() => {
  ...
  if (ORIGINAL_GATEWAY_TOKEN_ENV === undefined) {
    ...
    process.env.REMOTECLAW_GATEWAY_TOKEN = ORIGINAL_GATEWAY_TOKEN_ENV;
  }
});

async function openDeviceTokenWsWithDetails(params: { browserClient?: boolean } = {}): Promise<{
  ws: WebSocket;
  deviceId: string;
  ...
  const identityPath = path.join(os.tmpdir(), `remoteclaw-shared-auth-${process.pid}-${port}.json`);
  ...
  const issuedDeviceToken = rotated?.token ?? "";
...
async function openDeviceTokenWs(): Promise<WebSocket> {
  const { ws } = await openDeviceTokenWsWithDetails();
  return ws;
}
...
// Skipped in the RemoteClaw fork: this describe configures the gateway auth
// token as a `{source:env, provider:default}` SecretRef, but the fork's
// `assertGatewayAuthConfigured` (src/gateway/auth.ts) correctly rejects an
// unresolved provider-ref token at gateway startup — the secrets-provider
// system has not initialised yet at bootstrap, so gateway.auth.token must be a
// plaintext string or come from REMOTECLAW_GATEWAY_TOKEN. SecretRef gateway
// tokens are therefore unsupported at bootstrap by design; this is a
// test-setup mismatch, NOT a disconnect defect. The shared-auth eviction
// behavior these tests target is already covered above with a literal token.
// Do NOT relax assertGatewayAuthConfigured to re-enable this describe — that
// would revert a fork-stricter bootstrap-secret guard.
describe.skip("gateway shared auth rotation with unchanged SecretRefs", () => {
