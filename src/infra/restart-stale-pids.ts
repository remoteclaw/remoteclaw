 ...
 * returning control to the caller (typically `triggerRemoteClawRestart` →
...
/**
 * Parse remoteclaw gateway PIDs from lsof -Fpc stdout, excluding the current
...
/**
 * Windows: find listening PIDs on the port, then verify each is an remoteclaw
...
export function findGatewayPidsOnPortSync(
  ...
  if (process.platform === "win32") {
    ...
    // command-line verification to find only remoteclaw gateway processes.
