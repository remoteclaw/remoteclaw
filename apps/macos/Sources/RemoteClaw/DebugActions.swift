...
enum DebugActions {
    private static let verboseDefaultsKey = "remoteclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "remoteclaw.onboardingSeen"
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "RemoteClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = RemoteClawPaths.configURL
        guard
