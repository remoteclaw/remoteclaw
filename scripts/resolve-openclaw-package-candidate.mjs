...
const DEFAULT_OUTPUT_NAME = "remoteclaw-current.tgz";
...
export const REMOTECLAW_PACKAGE_SPEC_RE =
  /^remoteclaw@(alpha|beta|extended-stable|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-remoteclaw-package-candidate.mjs --source <ref|npm|url|trusted-url|artifact> --output-dir <dir> [options]
...
export function validateRemoteClawPackageSpec(spec) {
  if (!REMOTECLAW_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be remoteclaw@alpha, remoteclaw@beta, remoteclaw@extended-stable, remoteclaw@latest, or an exact RemoteClaw release version; got: ${spec}`,
    );
  }
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `remoteclaw-package-source-${process.pid}`,
  );
