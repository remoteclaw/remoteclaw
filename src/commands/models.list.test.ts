...
const ensureRemoteClawModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/remoteclaw-agent/auth.json" });
const resolveRemoteClawAgentDir = vi.fn().mockReturnValue("/tmp/remoteclaw-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/remoteclaw-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureRemoteClawModelsJson,
}));
...
describe("models list/status", () => {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/remoteclaw-agent");
  });
