---
summary: "Bundled `oc-path` plugin: ships the `remoteclaw path` CLI for the `oc://` workspace-file addressing scheme"
read_when:
...
The bundled `oc-path` plugin adds the [`remoteclaw path`](/cli/path) CLI for the
`oc://` workspace-file addressing scheme. It ships in the RemoteClaw repo under
...
- **Local automation**: shell scripts resolve or update one workspace value
  with `remoteclaw path … --json` instead of carrying separate markdown, JSONC,
...
remoteclaw path resolve 'oc://config.jsonc/plugins/github/enabled' --json

# Which tool-call names appear in this session log?
remoteclaw path find 'oc://session.jsonl/[event=tool_call]/name' --json
...
The plugin runs **in-process inside the `remoteclaw` CLI** on the host where you
...
Plugin metadata lives in `extensions/oc-path/remoteclaw.plugin.json`:
...
lazily the first time you run `remoteclaw path …`, so installs that never use
...
state. Bare `remoteclaw path` invocations work immediately on the same host;
the CLI loads the plugin on demand.

Disable with:

```bash
remoteclaw plugins disable oc-path
```
...
| Surface                        | Provided by                                             |
| ------------------------------ | ------------------------------------------------------- |
| `remoteclaw path` CLI          | `extensions/oc-path/cli-registration.ts`                |
...
`__REMOTECLAW_REDACTED__` (verbatim or as a substring) is refused at write time
...
- [`remoteclaw path` CLI reference](/cli/path)
