...
const a2uiAppDir = path.join(rootDir, "apps", "shared", "RemoteClawKit", "Tools", "CanvasA2UI");
...
export function getBundleHashRepoInputPaths(repoRoot = rootDir) {
  return [
    ...
    path.join(repoRoot, "apps", "shared", "RemoteClawKit", "Tools", "CanvasA2UI"),
  ];
}
...
async function main() {
  ...
  if (!hasRendererDir || !hasAppDir) {
    ...
    if (process.env.REMOTECLAW_SPARSE_PROFILE || process.env.REMOTECLAW_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI sources missing; skipping bundle because REMOTECLAW_A2UI_SKIP_MISSING=1 or REMOTECLAW_SPARSE_PROFILE is set.",
      );
      return;
    }
