...
enum CommandResolver {
    private static let projectRootDefaultsKey = "remoteclaw.gatewayProjectRootPath"
    private static let helperName = "remoteclaw"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let remoteclawEntry = root.appendingPathComponent("remoteclaw.mjs").path
        if FileManager().isReadableFile(atPath: remoteclawEntry) { return remoteclawEntry }
        let binEntry = root.appendingPathComponent("bin/remoteclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/remoteclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let remoteclawPaths = self.remoteclawManagedPaths(home: home)
        if !remoteclawPaths.isEmpty {
            extras.insert(contentsOf: remoteclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + remoteclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func remoteclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".remoteclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func remoteclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectRemoteClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            ...
            root.appendingPathComponent("bin/remoteclaw.js").path,
        ]
    ...
    static func hasAnyRemoteClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.remoteclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func remoteclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let remoteclawPath = self.projectRemoteClawExecutable(projectRoot: root) {
            return [remoteclawPath, subcommand] + extraArgs
        }
        if let remoteclawPath = self.remoteclawExecutable(searchPaths: searchPaths) {
            return [remoteclawPath, subcommand] + extraArgs
        }
        ...
        case .success:
            let missingEntry = """
            remoteclaw entrypoint missing (looked for dist/index.js or remoteclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real remoteclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/remoteclaw"
        ...
        \(cliSection)
        \(projectSection)
        if command -v remoteclaw >/dev/null 2>&1; then
          CLI="$(command -v remoteclaw)"
          remoteclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/remoteclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/remoteclaw.js"
            node "$PRJ/bin/remoteclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent remoteclaw"
          pnpm --silent remoteclaw \(quotedArgs);
        else
          echo "remoteclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? RemoteClawConfigFile.loadDict()
