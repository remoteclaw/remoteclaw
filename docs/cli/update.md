...
read_when:
  ...
  - You are debugging `remoteclaw update` output or options
...
- `--tag <dist-tag|version|spec>`: override the package target for this update only. For package installs, `main` maps to `github:remoteclaw/remoteclaw#main`; GitHub/git source specs are packed into a temporary tarball before the staged global npm install.
...
`remoteclaw update` does not have a `--verbose` flag. Use `--dry-run` to preview
the planned channel/tag/install/restart actions, `--json` for machine-readable
results, and `remoteclaw update status --json` when you only need channel and
...
and the helper runs the normal `remoteclaw update --yes --json` CLI path from
...
not supervising it, RemoteClaw re-bootstraps the LaunchAgent automatically, then
...
manual `remoteclaw gateway restart` hint. With `--no-restart`,
...
- `ok: true`, `result.status: "skipped"`,
  ...
  `remoteclaw update --yes --json` outside the live service process.
- `ok: false`, `result.reason: "managed-service-handoff-unavailable"`, and
  `handoff.status: "unavailable"` mean RemoteClaw could not find a supervising
...
result. `remoteclaw status` and `remoteclaw status --all` show an `Update restart`
...
<Steps>
  ...
  <Step title="Preflight build (dev only)">
    Runs the TypeScript build in a temp worktree. If the tip fails, walks back up to 10 commits to find the newest buildable commit. Set `REMOTECLAW_UPDATE_PREFLIGHT_LINT=1` to also run lint during this preflight; lint runs in constrained serial mode because user update hosts are often smaller than CI runners.
  </Step>
  <Step title="Rebase">
