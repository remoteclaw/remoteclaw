...
vi.mock("remoteclaw/plugin-sdk/provider-setup", async () => {
  const actual = await vi.importActual<object>("remoteclaw/plugin-sdk/provider-setup");
  return {
    ...actual,
...
vi.mock("remoteclaw/plugin-sdk/self-hosted-provider-setup", async () => {
...
vi.mock("remoteclaw/plugin-sdk/ollama-setup", async () => {
  const actual = await vi.importActual<object>("remoteclaw/plugin-sdk/ollama-setup");
  return {
    ...actual,
    buildOllamaProvider: (...args: unknown[]) => buildOllamaProviderMock(...args),
  };
