---
description: "Strict config validation + doctor-only migrations"
read_when:
...
- Plugin manifests (`remoteclaw.plugin.json`) are required for all plugins.

## Plugin schema enforcement

- Each plugin provides a strict JSON Schema for its config (inline in the manifest).
- Plugin load flow:
  1. Resolve plugin manifest + schema (`remoteclaw.plugin.json`).
...
- If config invalid:
  ...
  - Instruct: `remoteclaw doctor --fix`.
...
Everything else must hard-fail with: “Config invalid. Run `remoteclaw doctor --fix`.”
