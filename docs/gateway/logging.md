...
RemoteClaw has two log surfaces:
...
- Default rolling log file is under `/tmp/remoteclaw/` (one file per day): `remoteclaw-YYYY-MM-DD.log`, dated by the gateway host's local timezone. If that directory is unsafe or unwritable (wrong owner, world-writable, a symlink), RemoteClaw falls back to a user-scoped `os.tmpdir()/remoteclaw-<uid>` path instead; on Windows it always uses that OS-tmpdir fallback.
...
- Configure the log file path and level via `~/.remoteclaw/remoteclaw.json`: `logging.file`, `logging.level`.
...
RemoteClaw masks sensitive tokens before log or transcript output leaves the process. This redaction policy applies at console, file-log, OTLP log-record, and session transcript text sinks, so matching secret values are masked before JSONL lines or messages are written to disk.
...
`remoteclaw gateway` supports a per-gateway style switch:
