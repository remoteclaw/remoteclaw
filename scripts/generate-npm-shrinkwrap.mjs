...
function importerKeyForPackageDir(packageDir) {
  const relative = path.relative(ROOT_DIR, packageDir).replaceAll(path.sep, "/");
  return relative === "" ? "." : relative;
}

function readPnpmLockImporterForkedDirectDependencyPins(packageDir) {
  // readShrinkwrapOverrides() skips forked packages (multiple minor lines in the lock, so no single
  // global pin), and readCurrentShrinkwrapOverrides() excludes the package's own declared deps. A
  // dependency the target *directly* declares that is *also* forked therefore falls through both
  // mechanisms, so npm floats it past the lock (e.g. @opentelemetry/resources ^2.7.0 -> 2.9.0 while
  // this importer is locked to 2.8.0). Pin such deps to the exact version this importer resolves to
  // in pnpm-lock.yaml. The pin is target-scoped (only added to this package's generation manifest)
  // and always a version already present in the lock, so it never changes the resolved graph — it
  // only prevents the float. alignDirectDependencySpecsWithOverrides() aligns the direct spec so npm
  // accepts the top-level pin instead of raising EOVERRIDE.
  const lockfile = parseYaml(readFileSync(path.join(ROOT_DIR, "pnpm-lock.yaml"), "utf8"));
  const importer = lockfile?.importers?.[importerKeyForPackageDir(packageDir)];
  ...
  for (const section of ["dependencies", "optionalDependencies", "peerDependencies"]) {
    ...
    for (const [name, entry] of Object.entries(dependencies)) {
      ...
      if (!versions || versions.size < 2 || pnpmLockOverrideVersionForVersions(versions) !== null) {
        continue;
      }
      const rawVersion = isPlainObject(entry) ? entry.version : entry;
      const resolved = exactVersionFromOverrideSpec(String(rawVersion ?? "").replace(/\(.*/u, ""));
      if (resolved && versions.has(resolved)) {
        pins[name] = resolved;
      }
    }
  }
...
function packageJsonForShrinkwrap(packageJson, shrinkwrapOverrides) {
  ...
  alignDirectDependencySpecsWithOverrides(normalized);
  return normalized;
}

function alignDirectDependencySpecsWithOverrides(manifest) {
  const overrides = manifest.overrides;
  if (!isPlainObject(overrides)) {
    return;
  }
  // npm rejects a top-level override for a package the manifest itself directly depends on unless the
  // direct dependency spec is exactly the override spec (EOVERRIDE: "Override for <pkg>@<spec> conflicts
  // with direct dependency"). Pin the direct spec to the (exact) override so npm accepts the pin, which
  // also keeps transitive copies on the locked version. Only bare-name string overrides can collide;
  // scoped parent selectors (name@version -> { child }) and nested-object overrides never do. This edits
  // only the throwaway generation manifest — the real package.json specifier is untouched — and the
  // resolved version is unchanged (it stays the pnpm-locked version the override already targeted).
  for (const [selector, value] of Object.entries(overrides)) {
    if (typeof value !== "string" || parsePnpmPackageKey(selector) !== null) {
      continue;
    }
    for (const section of ["dependencies", "optionalDependencies", "peerDependencies"]) {
      if (isPlainObject(manifest[section]) && selector in manifest[section]) {
        manifest[section] = { ...manifest[section], [selector]: value };
      }
    }
  }
}
...
function generateShrinkwrap(packageDir, options = {}) {
  const tempDir = mkdtempSync(path.join(tmpdir(), "remoteclaw-shrinkwrap-"));
  try {
    ...
    const shrinkwrapOverrides = mergeOverrides(
      ...
      readPnpmLockImporterForkedDirectDependencyPins(packageDir),
    );
