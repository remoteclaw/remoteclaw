...
vi.mock("remoteclaw/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("remoteclaw/plugin-sdk/acp-runtime")>(
    "remoteclaw/plugin-sdk/acp-runtime",
  );
...
vi.mock("remoteclaw/plugin-sdk/json-store", async () => {
  const actual = await vi.importActual<typeof import("remoteclaw/plugin-sdk/json-store")>(
    "remoteclaw/plugin-sdk/json-store",
  );
...
describe("telegram thread bindings", () => {
  ...
  beforeEach(async () => {
    ...
    const acpRuntime = await vi.importActual<typeof import("remoteclaw/plugin-sdk/acp-runtime")>(
      "remoteclaw/plugin-sdk/acp-runtime",
    );
  ...
  it("persists unbinds before restart so removed bindings do not come back", async () => {
    ...
    const bound = await getSessionBindingService().bind({
      targetSessionKey: "plugin-binding:remoteclaw-codex-app-server:abc123",
      targetKind: "session",
      conversation: {
        channel: "telegram",
        accountId: "default",
  ...
  it("keeps plugin-owned bindings when ACP cleanup runs on startup", async () => {
    ...
    await getSessionBindingService().bind({
      targetSessionKey: "plugin-binding:remoteclaw-codex-app-server:still-valid",
      targetKind: "session",
      conversation: {
        channel: "telegram",
        accountId: "default",
    ...
    expect(reloaded.getByConversationId("plugin-binding-convo")?.targetSessionKey).toBe(
      "plugin-binding:remoteclaw-codex-app-server:still-valid",
    );
