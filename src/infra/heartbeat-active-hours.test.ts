...
function cfgWithUserTimezone(userTimezone = "UTC"): RemoteClawConfig {
  return {
    agents: {
      defaults: {
        userTimezone,
...
describe("isWithinActiveHours", () => {
  it("returns true when activeHours is not configured", () => {
    expect(isWithinActiveHours(cfgWithUserTimezone("UTC"), undefined, Date.UTC(2025, 0, 1, 3))).toBe(true);
  });

  it("returns true when activeHours start/end are invalid", () => {
    const cfg = cfgWithUserTimezone("UTC");
    expect(isWithinActiveHours(cfg, heartbeatWindow("bad", "10:00", "UTC"), Date.UTC(2025, 0, 1, 9))).toBe(true);
    expect(isWithinActiveHours(cfg, heartbeatWindow("08:00", "24:30", "UTC"), Date.UTC(2025, 0, 1, 9))).toBe(true);
  });

  it("returns false when activeHours start equals end", () => {
    const cfg = cfgWithUserTimezone("UTC");
    expect(isWithinActiveHours(cfg, heartbeatWindow("08:00", "08:00", "UTC"), Date.UTC(2025, 0, 1, 12, 0, 0))).toBe(
      false,
    );
  });
