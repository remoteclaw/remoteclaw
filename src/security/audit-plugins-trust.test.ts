...
describe("security audit install metadata findings", () => {
  ...
  const runInstallMetadataAudit = async (cfg: RemoteClawConfig, stateDir: string) => {
    return await collectPluginsTrustFindings({ cfg, stateDir });
  };

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-security-install-"));
...
describe("security audit extension tool reachability findings", () => {
  ...
  const runSharedExtensionsAudit = async (config: RemoteClawConfig) => {
  ...
  beforeAll(async () => {
    ...
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-security-extensions-"));
    isolatedHome = path.join(fixtureRoot, "home");
    const isolatedEnv = createPathResolutionEnv(isolatedHome, { REMOTECLAW_HOME: isolatedHome });
