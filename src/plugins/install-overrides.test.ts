  ...
  it("requires an explicit allow flag", () => {
    expect(
      resolvePluginInstallOverride({
        pluginId: "codex",
        env: {
          [PLUGIN_INSTALL_OVERRIDES_ENV]: JSON.stringify({ codex: "npm:@remoteclaw/codex@1.0.0" }),
        },
      }),
    ).toBeNull();
  });
      ...
      resolvePluginInstallOverride({
        ...
        env: {
          ...
          [PLUGIN_INSTALL_OVERRIDES_ENV]: JSON.stringify({
            codex: "npm:@remoteclaw/codex@2026.5.8",
            "demo-plugin": "npm-pack:./demo.tgz",
          }),
        },
      }),
    ).toEqual({ kind: "npm", spec: "@remoteclaw/codex@2026.5.8" });
  });
