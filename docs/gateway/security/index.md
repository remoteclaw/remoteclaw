...
<Warning>
  **Personal assistant trust model.** This guidance assumes one trusted
  ...
  RemoteClaw is **not** a hostile multi-tenant security boundary for multiple
...
RemoteClaw source checkouts use `pnpm-lock.yaml`. The published `remoteclaw` npm
package and RemoteClaw-owned npm plugin packages include `npm-shrinkwrap.json`,
...
at install time. Suitable RemoteClaw-owned npm plugin packages can also publish
...
- the package tarball contains the dependency graph that release validators
  checked;
- suitable RemoteClaw-owned plugin tarballs contain the dependency files from
  that graph;
...
does not replace `remoteclaw security audit`, host isolation, npm provenance,
...
RemoteClaw-owned published plugin package changes its published dependency graph:
...
the root `remoteclaw` package without touching plugin packages.
...
```bash
npm pack remoteclaw@<version> --json --pack-destination /tmp/remoteclaw-pack
tar -tf /tmp/remoteclaw-pack/remoteclaw-<version>.tgz | grep '^package/npm-shrinkwrap.json$'
```

To inspect an RemoteClaw-owned plugin package, replace the package spec and check
the same tar entry:

```bash
npm pack @remoteclaw/discord@<version> --json --pack-destination /tmp/remoteclaw-plugin-pack
tar -tf /tmp/remoteclaw-plugin-pack/remoteclaw-discord-<version>.tgz | grep '^package/npm-shrinkwrap.json$'
tar -tf /tmp/remoteclaw-plugin-pack/remoteclaw-discord-<version>.tgz | grep '^package/node_modules/'
```
...
RemoteClaw uses `@remoteclaw/fs-safe` for root-bounded file access, atomic writes, archive extraction, temp workspaces, and secret-file helpers. RemoteClaw defaults fs-safe's optional POSIX Python helper to **off**; set `REMOTECLAW_FS_SAFE_PYTHON_MODE=auto` or `require` only when you want the extra fd-relative mutation hardening and can support a Python runtime.
...
`remoteclaw security audit` raises `config.insecure_or_dangerous_flags` when
...
RemoteClaw strips common self-hosted LLM chat-template special-token literals from wrapped external content and metadata before they reach the model. Covered marker families include Qwen/ChatML, Llama, Gemma, Mistral, Phi, and GPT-OSS role/turn tokens.
...
RemoteClaw strips common model-family special-token literals from wrapped
...
When the bundled `bonjour` plugin is enabled, the Gateway broadcasts its presence via mDNS (`_remoteclaw-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
5. **Environment variable** (alternative): set `REMOTECLAW_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
`REMOTECLAW_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
This is intentionally process environment only, not an `remoteclaw.json` config
key.
...
RemoteClaw loads workspace-local `.env` files for agents and tools, but never lets those files silently override gateway runtime controls.

- Any key that starts with `REMOTECLAW_*` is blocked from untrusted workspace `.env` files.
...
Why: workspace `.env` files frequently live next to agent code, get committed by accident, or get written by tools. Blocking the whole `REMOTECLAW_*` prefix means adding a new `REMOTECLAW_*` flag later can never regress into silent inheritance from workspace state.
