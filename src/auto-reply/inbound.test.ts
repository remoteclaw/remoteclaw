...
describe("initSessionState BodyStripped", () => {
  it("prefers BodyForAgent over Body for group chats", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-sender-meta-"));
  ...
  it("prefers BodyForAgent over Body for direct chats", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-sender-meta-direct-"));
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } } as RemoteClawConfig;
...
describe("mention helpers", () => {
  it("builds regexes and skips invalid or unsafe patterns", () => {
    const regexes = buildMentionRegexes({
      messages: {
        groupChat: { mentionPatterns: ["\\bremoteclaw\\b", "(invalid", "(a+)+$"] },
      },
    });
    expect(regexes).toHaveLength(1);
    expect(regexes[0]?.test("remoteclaw")).toBe(true);
  });
  ...
  it("matches patterns case-insensitively", () => {
    const regexes = buildMentionRegexes({
      messages: { groupChat: { mentionPatterns: ["\\bremoteclaw\\b"] } },
    });
    expect(matchesMentionPatterns("REMOTECLAW: hi", regexes)).toBe(true);
  });
  ...
  it("strips safe mention patterns and ignores unsafe ones", () => {
    const stripped = stripMentions("remoteclaw " + "a".repeat(28) + "!", {} as MsgContext, {
      messages: {
        groupChat: { mentionPatterns: ["\\bremoteclaw\\b", "(a+)+$"] },
      },
    });
