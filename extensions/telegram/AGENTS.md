...
- Spool completes at turn adoption, not settle. Once the recovery-relevant
  ...
  `REMOTECLAW_TELEGRAM_SPOOLED_HANDLER_TIMEOUT_MS`) is the only ingress
...
- Do not reintroduce `sendMessageDraft` for answer streaming. Telegram drafts
  ...
  requires a separate `sendMessage`. RemoteClaw uses `sendMessage` plus
...
- Reply context comes from RemoteClaw-observed messages. Bot API updates expose
