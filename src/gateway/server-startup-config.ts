...
export function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("remoteclaw doctor --fix")}" to repair, then retry.`
    : "";
...
function hasActiveGatewayAuthSecretRef(config: RemoteClawConfig): boolean {
...
function pruneSkippedStartupSecretSurfaces(config: RemoteClawConfig): RemoteClawConfig {
  const skipChannels =
...
function assertRuntimeGatewayAuthNotKnownWeak(config: RemoteClawConfig): void {
