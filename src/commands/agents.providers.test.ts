...
describe("buildProviderStatusIndex", () => {
  ...
  it("prefers inspectAccount for read-only status surfaces", async () => {
    ...
    const map = await buildProviderStatusIndex({} as RemoteClawConfig);
  ...
  it("records accounts that throw during read-only resolution as not configured", async () => {
    ...
    await expect(buildProviderStatusIndex({} as RemoteClawConfig)).resolves.toEqual(
      new Map([
        [
          "quietchat:default",
          {
  ...
  it("rethrows unexpected read-only account resolution errors", async () => {
    ...
    await expect(buildProviderStatusIndex({} as RemoteClawConfig)).rejects.toThrow("plugin crash");
  });
});
