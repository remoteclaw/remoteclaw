...
/** Result of reading package.json remoteclaw.compat.pluginApi metadata. */
...
export function resolvePackagePluginApiRange(
  ...
  if (typeof pluginApi !== "string") {
    return { ok: false, error: "package.json remoteclaw.compat.pluginApi must be a string" };
  }
  const range = pluginApi.trim();
  if (!range) {
    return { ok: false, error: "package.json remoteclaw.compat.pluginApi must not be empty" };
  }
