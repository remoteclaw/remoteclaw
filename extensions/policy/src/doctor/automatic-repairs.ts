...
function mergeRequiredDenyTools(
  ...
  for (const finding of findings) {
    ...
    if (
      ...
      finding.ocPath === "oc://remoteclaw.config/tools/deny"
    ) {
      warnings.push(
  ...
  return changes.length > 0
    ? { config: next as RemoteClawConfig, changes: uniqueStrings(changes), warnings }
...
function disableElevatedTools(
  ...
  if (
    !findings.some((finding) => finding.ocPath === "oc://remoteclaw.config/tools/elevated/enabled")
  ) {
    return { config: cfg, changes: [] };
  }
  const next = cloneConfig(cfg);
  ...
  return {
    config: next as RemoteClawConfig,
...
function disableInsecureControlUi(
  ...
  const changes: string[] = [];
  const fields = [
    ["allowInsecureAuth", "oc://remoteclaw.config/gateway/controlUi/allowInsecureAuth"],
    [
  ...
  return changes.length > 0
    ? { config: next as RemoteClawConfig, changes }
    : { config: cfg, changes };
}
...
function disableRemoteGatewayMode(
  ...
  if (!findings.some((finding) => finding.ocPath === "oc://remoteclaw.config/gateway/mode")) {
    return { config: cfg, changes: [] };
  }
  const next = cloneConfig(cfg);
  const gateway = ensureRecord(next, "gateway");
...
function enableSensitiveLoggingRedaction(cfg: RemoteClawConfig): RepairPatch {
  const next = cloneConfig(cfg);
...
function disableTelemetryContentCapture(cfg: RemoteClawConfig): RepairPatch {
  const next = cloneConfig(cfg);
...
function cloneConfig(cfg: RemoteClawConfig): ConfigRecord {
...
function configPathSegments(ocPath: string): readonly string[] {
  const prefix = "oc://remoteclaw.config/";
...
function skippedUnsafeScopedRepair(cfg: RemoteClawConfig, warning: string): RepairPatch {
...
function isScopedInheritedChannelDefaultFinding(finding: HealthFinding): boolean {
  return (
    hasScopedPolicyRequirement([finding]) &&
    finding.ocPath?.startsWith("oc://remoteclaw.config/channels/defaults/") === true
  );
}
