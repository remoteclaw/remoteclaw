...
Use these steps to exercise the live RemoteClaw iOS App Review Gateway.

## Demo Account / Setup

Use the RemoteClaw iOS app with the live review Gateway setup code included in
...
9. On `Connected`, tap `Open RemoteClaw`.
10. Confirm the `Control` screen shows `Gateway Online`.
11. Tap `Settings`.
12. Tap `Approvals`.
13. Tap `Open Notifications`.
14. Tap `Enable Notifications`.
15. On `Enable RemoteClaw Hosted Push Relay?`, tap `Continue`.
16. If iOS asks whether RemoteClaw may send notifications, tap `Allow`.
...
command `printf 'RemoteClaw App Review approval demo complete\n'`. Tap
...
9. While Talk is active, send RemoteClaw to the background by returning to the
   Home Screen or locking the iPhone. Do not force quit the app.
10. Continue speaking then wait for assistant audio reply.

Expected result: realtime Talk audio continues while RemoteClaw is backgrounded.
Reopen RemoteClaw, confirm Talk is still active, then tap `Stop Talk`.
...
1. Tap `Settings`.
2. Tap `Reconnect`.
3. Immediately send RemoteClaw to the background by returning to the Home Screen
   or locking the iPhone.
4. Watch the Lock Screen or Dynamic Island while the Gateway reconnects.

Expected result: while reconnecting, iOS can show an `RemoteClaw` Live Activity
with connection status such as `Connecting...` or `Reconnecting...`. On a fast
network this status may be brief because RemoteClaw ends the Live Activity after
...
4. Immediately send RemoteClaw to the background and lock the iPhone. Do not
   force quit the app.

Expected result: the iPhone Lock Screen receives a visible `RemoteClaw`
notification with this body:

```text
RemoteClaw App Review push notification demo
```

Tap the notification and unlock the iPhone if prompted. If RemoteClaw opens on
...
3. Immediately send RemoteClaw to the background and lock the iPhone. Do not
   force quit the app.
4. Wait for the `RemoteClaw` notification on the Lock Screen. It normally appears
   about 10 seconds after the message is sent.
5. Tap the notification and unlock the iPhone if prompted. If RemoteClaw opens on
   `Control`, tap `Chat`.
...
4. Tap `Share`.
5. Tap `RemoteClaw`.
6. Confirm the RemoteClaw share extension appears and shows
   `Edit text, then tap Send.` and `Send to RemoteClaw`.
7. Tap `Send to RemoteClaw`.

Expected result: the RemoteClaw share extension sends the shared Safari page to
the live review Gateway and shows `Sent to RemoteClaw.` Returning to RemoteClaw
