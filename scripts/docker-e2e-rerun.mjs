...
const DEFAULT_WORKFLOW = "remoteclaw-live-and-e2e-checks-reusable.yml";
...
function ghWorkflowCommand(lanes, ref, workflow, reuseInputs = {}) {
  const workflowRef = process.env.REMOTECLAW_DOCKER_E2E_WORKFLOW_REF || process.env.GITHUB_REF_NAME;
...
function main() {
  ...
  if (isLocalJson) {
    ...
    const outputDir =
      options.dir || path.join(os.tmpdir(), `remoteclaw-docker-e2e-rerun-${options.input}`);
