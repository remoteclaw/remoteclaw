---
summary: "Use ACP runtime sessions for Codex, Claude Code, Cursor, Gemini CLI, RemoteClaw ACP, and other harness agents"
read_when:
...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let RemoteClaw run external coding harnesses (for example Pi, Claude Code, Codex, Cursor, Copilot, RemoteClaw ACP, OpenCode, Gemini CLI, and other supported ACPX harnesses) through an ACP backend plugin.

If you ask RemoteClaw in plain language to "run this in Codex" or "start Claude Code in a thread", RemoteClaw should route that request to the ACP runtime (not the native sub-agent runtime). Each ACP session spawn is tracked as a [background task](/automation/tasks).

If you want Codex or Claude Code to connect as an external MCP client directly
to existing RemoteClaw channel conversations, use [`remoteclaw mcp serve`](/cli/mcp)
instead of ACP.
...
| ------------------------------------------------------------------------------------ | ------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Run Codex, Claude Code, Gemini CLI, or another external harness _through_ RemoteClaw | This page: ACP agents                 | Chat-bound sessions, `/acp spawn`, `sessions_spawn({ runtime: "acp" })`, background tasks, runtime controls |
...
- On startup, RemoteClaw probes that binary and self-repairs it if needed.
...
- `/acp spawn codex`: RemoteClaw should be ready to bootstrap `acpx`, but the Codex ACP adapter may still need a first-run fetch.
...
Use ACP when you want an external harness runtime. Use sub-agents when you want RemoteClaw-native delegated runs.
...
- RemoteClaw keeps owning the channel transport, auth, safety, and delivery.
...
- If you spawn to a different ACP agent and do not pass `--cwd`, RemoteClaw inherits the **target agent's** workspace by default, not the requester's.
- If that inherited workspace path is missing (`ENOENT`/`ENOTDIR`), RemoteClaw falls back to the backend default cwd instead of silently reusing the wrong tree.
...
- `/acp spawn codex --thread auto`: RemoteClaw may create a child thread/topic and bind the ACP session there
...
- Generic current-conversation binds use the shared RemoteClaw binding store and survive normal gateway restarts.

Notes:

- `--bind here` and `--thread ...` are mutually exclusive on `/acp spawn`.
- On Discord, `--bind here` binds the current channel or thread in place. `spawnAcpSessions` is only required when RemoteClaw needs to create a child thread for `--thread auto|here`.
- If the active channel does not expose current-conversation ACP bindings, RemoteClaw returns a clear unsupported message.
...
Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, RemoteClaw returns a clear unsupported/unavailable message.
...
- `bindings[].agentId` is the owning RemoteClaw agent id.
...
- If `agentId` is omitted, RemoteClaw uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, RemoteClaw may default to persistent behavior per runtime path
...
`sessions_send` can target another session after spawn. For normal peer sessions, RemoteClaw uses an agent-to-agent (A2A) follow-up path after injecting the message:
...
ACP sessions currently run on the host runtime, not inside the RemoteClaw sandbox.
...
If no target resolves, RemoteClaw returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, RemoteClaw returns a clear unsupported-control error.
...
When RemoteClaw uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal RemoteClaw `agentId` path).
...
```bash
remoteclaw plugins install acpx
...
```bash
remoteclaw plugins install ./path/to/local/acpx-plugin
```
...
- Relative paths resolve from RemoteClaw workspace directory.
...
- RemoteClaw startup remains non-blocking while the backend health check runs.

See [Plugins](/tools/plugin).

### Automatic dependency install

When you install RemoteClaw globally with `npm install -g remoteclaw`, the acpx
...
normally and reports the missing dependency through `remoteclaw acp doctor`.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose RemoteClaw plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
RemoteClaw plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
remoteclaw config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `remoteclaw-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled RemoteClaw
  plugins.
...
These ACPX harness permissions are separate from RemoteClaw exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.
...
```bash
remoteclaw config set plugins.entries.acpx.config.permissionMode approve-all
remoteclaw config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

> **Important:** RemoteClaw currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
