...
export const zh_CN = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密码存入 RemoteClaw 配置",
    ...
    migration: {
      ...
      appliedTitle: "迁移已应用",
      cancelled: "迁移已取消",
      complete: "迁移完成。下一步运行 `remoteclaw doctor`。",
    ...
    remote: {
      ...
      insecureRemoteUrl:
        "远程主机请使用 wss://，或通过 SSH 隧道使用 ws://127.0.0.1/localhost。紧急绕过：可信私有网络可设置 REMOTECLAW_ALLOW_INSECURE_PRIVATE_WS=1。",
      ...
      plaintextPasswordHint: "直接把密码存入 RemoteClaw 配置",
      plaintextPasswordLabel: "现在输入密码",
      plaintextTokenHint: "直接把令牌存入 RemoteClaw 配置",
    ...
    security: {
      ...
      beta: "RemoteClaw 是一个爱好项目，仍处于 beta 阶段。请预期会有边缘问题。",
      confirm: "我理解 RemoteClaw 默认面向个人使用；共享/多用户使用需要加固。继续？",
      hardeningRequired: "如果你不熟悉安全加固和访问控制，请不要运行 RemoteClaw。",
      learnMore: "了解更多",
      notMultitenant: "RemoteClaw 默认不是敌对多租户安全边界。",
      personalAgent: "默认情况下，RemoteClaw 是个人 agent：一个可信操作者边界。",
    ...
    skills: {
      ...
      docsLine: "文档：https://docs.remoteclaw.org/skills",
      enterEnv: "输入 {env}",
      ...
      manualPrereqsDoctorHint: "请先安装这些工具，然后运行 `remoteclaw doctor` 检查技能和要求。",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 运行：./occ talk:bot:install "RemoteClaw" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要发送者/所有者号码，以便 RemoteClaw 将你加入允许列表。",
      ...
      separatePhoneLabel: "RemoteClaw 专用独立手机",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "这是 RemoteClaw 用来向 Chat 发送回复的 URL。",
    ...
    signal: {
      ...
      multipleEntries: "多个条目请用逗号分隔。",
      nextLinkDevice: '绑定设备：signal-cli link -n "RemoteClaw"',
    ...
    twitch: {
      ...
      envPrompt: "检测到 Twitch 环境变量 REMOTECLAW_TWITCH_ACCESS_TOKEN。使用环境 token？",
      helpCopyToken: "3. 复制 token（以 'oauth:' 开头）和 Client ID",
      helpCreateApp: "1. 在 https://dev.twitch.tv/console 创建 Twitch 应用",
      helpEnvVars: "支持环境变量：REMOTECLAW_TWITCH_ACCESS_TOKEN",
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "将 Synology NAS Chat 连接到 RemoteClaw，并启用完整 agent 能力。",
    ...
    search: {
      ...
      configured: "已配置",
      configureLaterHint: "稍后可用 remoteclaw configure --section web 配置",
      docsLine: "文档：{url}",
    ...
    finalize: {
      ...
      controlUiDocs: "文档：https://docs.remoteclaw.org/web/control-ui",
      dashboardCopyPaste: "在本机浏览器中复制/粘贴这个 URL 来控制 RemoteClaw。",
      dashboardLinkWithToken: "Dashboard 链接（含令牌）：{url}",
      dashboardOpened: "已在浏览器中打开。保留该标签页以控制 RemoteClaw。",
      ...
      daemonRuntimeNodeHint:
        "RemoteClaw 状态使用 node:sqlite，因此必须使用 Node；Bun 无法运行网关。",
      ...
      gatewayTokenStored:
        "存储位置：$REMOTECLAW_CONFIG_PATH（默认 ~/.remoteclaw/remoteclaw.json）中的 gateway.auth.token，或 REMOTECLAW_GATEWAY_TOKEN。",
      ...
      optionalApps: "可选应用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 链接控制 RemoteClaw。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已打开；保留该标签页以控制 RemoteClaw。",
      ...
      containerSystemdUnavailable:
        "此容器内没有 systemd 用户服务。RemoteClaw 只会跳过后台服务安装；请以前台方式运行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
        "在你的电脑上运行 agent 存在风险，请加固设置：https://docs.remoteclaw.org/security",
      ...
      webDocs: "文档：https://docs.remoteclaw.org/tools/web",
      ...
      workspaceDocs: "文档：https://docs.remoteclaw.org/concepts/agent-workspace",
