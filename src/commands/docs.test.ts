...
describe("docsSearchCommand", () => {
  ...
  it("invokes the correct lowercase docs MCP tool id", async () => {
    ...
    const toolUrl = argv.find((arg) => arg.includes("docs.remoteclaw.org/mcp."));
    expect(toolUrl).toBe("https://docs.remoteclaw.org/mcp.search_open_claw");
    expect(toolUrl).not.toMatch(/SearchRemoteClaw/);
  });

  it("fails loudly when mcporter returns a JSON-RPC MCP error on stdout with exit 0", async () => {
    runCommandWithTimeout.mockResolvedValueOnce({
      code: 0,
      stdout: "MCP error -32602: Tool SearchRemoteClaw not found",
      stderr: "",
    });
    const runtime = makeRuntime();
