...
export function loadIncludePatternsFromEnv(
  ...
  return loadPatternListFromEnv("REMOTECLAW_VITEST_INCLUDE_FILE", env);
}

export function loadExtraExcludePatternsFromEnv(
  env: Record<string, string | undefined> = process.env,
): string[] {
  return loadPatternListFromEnv("REMOTECLAW_VITEST_EXTRA_EXCLUDE_FILE", env) ?? [];
}
...
export function createUnitVitestConfigWithOptions(
  ...
  return defineProject({
    ...
    test: {
      ...
      setupFiles: [
        ...new Set(
          [...(sharedTest.setupFiles ?? []), "test/setup-remoteclaw-runtime.ts"].map(
            resolveRepoRootPath,
          ),
        ),
      ],
