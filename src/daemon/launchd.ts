...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.REMOTECLAW_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return assertValidLaunchAgentLabel(envLabel);
  }
  return assertValidLaunchAgentLabel(resolveGatewayLaunchAgentLabel(args?.env?.REMOTECLAW_PROFILE));
}
...
export function resolveGatewayLogPaths(env: GatewayServiceEnv): {
  ...
  const prefix = env.REMOTECLAW_LOG_PREFIX?.trim() || "gateway";
  return {
    logDir,
...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromEnv = parseStrictPositiveInteger(env.REMOTECLAW_GATEWAY_PORT ?? "");
  return fromEnv ?? null;
}
...
export async function findLegacyLaunchAgents(env: GatewayServiceEnv): Promise<LegacyLaunchAgent[]> {
  ...
  for (const label of resolveLegacyGatewayLaunchAgentLabels(env.REMOTECLAW_PROFILE)) {
...
async function writeLaunchAgentPlist({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.REMOTECLAW_PROFILE)) {
...
async function activateLaunchAgent(params: { env: GatewayServiceEnv; plistPath: string }) {
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    actionHint: "remoteclaw gateway install --force",
  });
}
...
async function ensureLaunchAgentLoadedAfterFailure(params: {
  ...
  try {
    await bootstrapLaunchAgentOrThrow({
      ...
      plistPath: params.plistPath,
      actionHint: "remoteclaw gateway start",
    });
  } catch {
...
export async function restartLaunchAgent({
  ...
  // `remoteclaw gateway restart` is an explicit operator request to bring the
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    serviceTarget,
    plistPath,
    actionHint: "remoteclaw gateway restart",
  });
