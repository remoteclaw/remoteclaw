---
summary: "CLI reference for `remoteclaw path` (inspect and edit workspace files via the `oc://` addressing scheme)"
read_when:
...
```bash
remoteclaw plugins enable oc-path
```

## Why use it

RemoteClaw state is spread across human-edited markdown, commented JSONC config,
...
`remoteclaw path` gives those callers a stable address instead of a one-off grep,
...
You probably do not need `remoteclaw path` for ordinary whole-file edits, rich
...
```bash
remoteclaw path resolve 'oc://config.jsonc/plugins/github/enabled'
```

Preview a write without touching disk:

```bash
remoteclaw path set 'oc://config.jsonc/plugins/github/enabled' 'true' --dry-run
```

Find matching records in an append-only JSONL log:

```bash
remoteclaw path find 'oc://session.jsonl/[event=tool_call]/name'
```
...
```bash
remoteclaw path resolve 'oc://AGENTS.md/runtime-safety/remoteclaw-gateway'
```

Validate a path in CI or a preflight script before the script reads or writes:

```bash
remoteclaw path validate 'oc://AGENTS.md/tools/$last/risk'
```
...
`remoteclaw path` does four things:
...
# Read a leaf
remoteclaw path resolve 'oc://gateway.jsonc/version'

# Wildcard search
remoteclaw path find 'oc://session.jsonl/*/event' --file ./logs/session.jsonl

# Dry-run a write
remoteclaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run

# Dry-run a write as a unified diff
remoteclaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff

# Apply the write
remoteclaw path set 'oc://gateway.jsonc/version' '2.0'

# Byte-fidelity round-trip (diagnostic)
remoteclaw path emit ./AGENTS.md
```
...
remoteclaw path resolve 'oc://config.jsonc/agents.defaults.models/"anthropic/claude-opus-4-7"/alias'

# Deep JSON/JSONC paths can use slash segments; they normalize to dotted subsegments
remoteclaw path set 'oc://remoteclaw.json/agents/list/0/tools/exec/security' 'allowlist' --dry-run

# Replace a JSONC leaf with a parsed object
remoteclaw path set 'oc://remoteclaw.json/gateway/auth/token' '{"source":"file","provider":"secrets","id":"/test"}' --value-json --dry-run

# Predicate search over JSONC children
remoteclaw path find 'oc://config.jsonc/plugins/[enabled=true]/id'

# Insert into a JSONC array
remoteclaw path set 'oc://config.jsonc/items/+1' '{"id":"new","enabled":true}' --dry-run

# Insert a JSONC object key
remoteclaw path set 'oc://config.jsonc/plugins/+github' '{"enabled":true}' --dry-run

# Append a JSONL event
remoteclaw path set 'oc://session.jsonl/+' '{"event":"checkpoint","ok":true}' --file ./logs/session.jsonl

# Resolve the last JSONL value line
remoteclaw path resolve 'oc://session.jsonl/$last/event' --file ./logs/session.jsonl

# Resolve a YAML workflow step
remoteclaw path resolve 'oc://workflow.yaml/steps/0/id'

# Update a YAML scalar
remoteclaw path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --dry-run

# Address markdown frontmatter
remoteclaw path resolve 'oc://AGENTS.md/[frontmatter]/name'

# Insert markdown frontmatter
remoteclaw path set 'oc://AGENTS.md/[frontmatter]/+description' 'Agent instructions' --dry-run

# Find markdown item fields
remoteclaw path find 'oc://SKILL.md/Tools/*/send_email'

# Validate a session-scoped path
remoteclaw path validate 'oc://AGENTS.md/Tools/$last/risk?session=cron-daily'
```
...
```bash
$ remoteclaw path resolve 'oc://x.md/[frontmatter]/tier' --file frontmatter.md --human
leaf @ L4: "core" (string)

$ remoteclaw path resolve 'oc://x.md/tools/gh/gh' --file frontmatter.md --human
leaf @ L9: "GitHub CLI" (string)

$ remoteclaw path find 'oc://x.md/tools/*' --file frontmatter.md --human
...
```bash
$ remoteclaw path resolve 'oc://config.jsonc/plugins/github/enabled' --file config.jsonc --human
leaf @ L4: "true" (boolean)

$ remoteclaw path set 'oc://config.jsonc/plugins/slack/enabled' 'true' --file config.jsonc --dry-run
...
```bash
$ remoteclaw path find 'oc://session.jsonl/[event=action]/userId' --file session.jsonl --human
1 match for oc://session.jsonl/[event=action]/userId:
  oc://session.jsonl/L2/userId  →  leaf @ L2: "u1" (string)

$ remoteclaw path resolve 'oc://session.jsonl/L2/ts' --file session.jsonl --human
leaf @ L2: "2" (number)
```
...
steps:
  ...
  - id: classify
    command: remoteclaw.invoke
```

```bash
$ remoteclaw path resolve 'oc://workflow.yaml/steps/0/id' --file workflow.yaml --human
leaf @ L3: "fetch" (string)

$ remoteclaw path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --file workflow.yaml --dry-run
...
```bash
remoteclaw path resolve 'oc://AGENTS.md/tools/gh/risk' --human
remoteclaw path resolve 'oc://gateway.jsonc/server/port' --json
```
...
```bash
remoteclaw path find 'oc://AGENTS.md/tools/**/risk'
remoteclaw path find 'oc://session.jsonl/[event=action]/userId'
remoteclaw path find 'oc://config.jsonc/plugins/{github,slack}/enabled'
```
...
remoteclaw path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff
remoteclaw path set 'oc://gateway.jsonc/version' '2.0'
remoteclaw path set 'oc://AGENTS.md/Tools/+gh/risk' 'low'
```
...
```bash
$ remoteclaw path validate 'oc://AGENTS.md/tools/gh' --human
...
```bash
remoteclaw path emit ./AGENTS.md
remoteclaw path emit ./gateway.jsonc --json
```
...
`remoteclaw path` is TTY-aware: human-readable output on a terminal, JSON when
...
- `set` writes bytes through the substrate's emit path, which applies the
  ...
  `__REMOTECLAW_REDACTED__` (verbatim or as a substring) is refused at write
  time.
