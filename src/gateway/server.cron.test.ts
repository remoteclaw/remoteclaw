...
async function getCronSuiteTempRoot(): Promise<string> {
  if (!cronSuiteTempRootPromise) {
    cronSuiteTempRootPromise = fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-gw-cron-suite-"));
  }
...
async function cleanupCronTestRun(params: {
  ...
  process.env.REMOTECLAW_SKIP_CRON = params.prevSkipCron;
}
...
describe("gateway server cron", () => {
  ...
  test("rejects unsafe custom session ids on add and update", async () => {
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "remoteclaw-gw-cron-bad-session-target-",
      cronEnabled: false,
    });
  ...
  test("keeps delivery updates valid for main jobs owned by an explicit default agent", async () => {
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "remoteclaw-gw-cron-main-default-agent-delivery-",
      cronEnabled: false,
    });
  ...
  test("accepts implicit announce delivery when extra configured channels are disabled", async () => {
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "remoteclaw-gw-cron-disabled-channel-ambiguity-",
      cronEnabled: false,
    });
  ...
  test("rejects ambiguous announce delivery on add when multiple channels are configured", async () => {
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "remoteclaw-gw-cron-ambiguous-delivery-add-",
      cronEnabled: false,
    });
  ...
  test("rejects ambiguous announce delivery on update when multiple channels are configured", async () => {
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "remoteclaw-gw-cron-ambiguous-delivery-update-",
      cronEnabled: false,
    });
  ...
  test("rejects target ids mistakenly supplied as delivery.channel providers", async () => {
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "remoteclaw-gw-cron-invalid-delivery-provider-",
      cronEnabled: false,
    });
  ...
  test("writes cron run history and auto-runs due jobs", async () => {
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "remoteclaw-gw-cron-log-",
    });
  ...
  test("fails closed for persisted unsafe custom session ids", async () => {
    const now = Date.now();
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "remoteclaw-gw-cron-persisted-bad-session-target-",
      cronEnabled: false,
      jobs: [
        {
  ...
  test("returns from cron.run immediately while isolated work continues in background", async () => {
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "remoteclaw-gw-cron-run-detached-",
      cronEnabled: false,
    });
  ...
  test("returns already-running without starting background work", async () => {
    ...
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "remoteclaw-gw-cron-run-busy-",
      cronEnabled: false,
      jobs: [
        {
          id: "busy-job",
  ...
  test("returns not-due without starting background work", async () => {
    const now = Date.now();
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "remoteclaw-gw-cron-run-not-due-",
      cronEnabled: false,
      jobs: [
        {
          id: "future-job",
  ...
  test("falls back to the primary delivery channel on job failure and preserves sessionKey", async () => {
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "remoteclaw-gw-cron-failure-primary-fallback-",
      cronEnabled: false,
    });
  ...
  test("ignores non-string cron.webhookToken values without crashing webhook delivery", async () => {
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "remoteclaw-gw-cron-webhook-secretinput-",
      cronEnabled: false,
    });
