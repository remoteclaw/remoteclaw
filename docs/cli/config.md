---
summary: "CLI reference for `remoteclaw config` (get/set/patch/unset/file/schema/validate)"
read_when:
...
Prints the active config file path, resolved from `REMOTECLAW_CONFIG_PATH` or the default location. The path names a regular file, not a symlink; see [Write safety](#write-safety).
...
<Tabs>
  <Tab title="Value mode">
    ```bash
    remoteclaw config set <path> <value>
    ```
  </Tab>
  ...
  <Tab title="Batch mode">
    ```bash
    remoteclaw config set --batch-json '[
      {
...
<Tabs>
  <Tab title="Success example">
    ```json
    {
      "ok": true,
      "operations": 1,
      "configPath": "~/.remoteclaw/remoteclaw.json",
      "inputModes": ["builder"],
      "checks": {
        "schema": false,
        "resolvability": true,
...
Direct editor writes are still allowed, but the running Gateway treats them as untrusted until they validate. Invalid direct edits fail startup or are skipped by hot reload; Gateway does not rewrite `remoteclaw.json`. Run `remoteclaw doctor --fix` to repair prefixed/clobbered config or restore the last-known-good copy. See [Gateway troubleshooting](/gateway/troubleshooting#gateway-rejected-invalid-config).
