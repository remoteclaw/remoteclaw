...
async function createAuditFixture(): Promise<AuditFixture> {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-secrets-audit-"));
  const stateDir = path.join(rootDir, ".remoteclaw");
...
describe("secrets audit", () => {
  ...
  it("does not flag non-sensitive routing headers in remoteclaw config", async () => {
    await writeJsonFile(fixture.configPath, {
      models: {
        providers: {
          openai: {
