...
export function resolveAnthropicVertexProjectId(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return (
...
export function resolveAnthropicVertexRegion(env: NodeJS.ProcessEnv = process.env): string {
  const region =
    normalizeOptionalSecretInput(env.GOOGLE_CLOUD_LOCATION) || normalizeOptionalSecretInput(env.CLOUD_ML_REGION);
...
export function resolveAnthropicVertexClientRegion(params?: { baseUrl?: string; env?: NodeJS.ProcessEnv }): string {
...
function resolveAnthropicVertexAdcCredentialsPath(env: NodeJS.ProcessEnv = process.env): string | undefined {
...
function resolveAnthropicVertexProjectIdFromAdc(env: NodeJS.ProcessEnv = process.env): string | undefined {
...
export function hasAnthropicVertexCredentials(env: NodeJS.ProcessEnv = process.env): boolean {
  return hasAnthropicVertexMetadataServerAdc(env) || resolveAnthropicVertexAdcCredentialsPath(env) !== undefined;
}
