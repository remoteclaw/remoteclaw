...
RemoteClaw can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. RemoteClaw requires a shared password.
- `off`: Default (no Tailscale automation).

Status and audit output use **Tailscale exposure** for this RemoteClaw Serve/Funnel
mode. `off` means RemoteClaw is not managing Serve or Funnel; it does not mean the
...
- `token` (default when `REMOTECLAW_GATEWAY_TOKEN` is set)
...
(`tailscale-user-login`) without supplying a token/password. RemoteClaw verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
RemoteClaw only treats a request as Serve when it arrives from loopback with
...
`https://remoteclaw.<tailnet-name>.ts.net/` instead of the device hostname.
...
- `gateway.tailscale.serviceName` applies only to Serve mode and is passed to
  ...
  `svc:<dns-label>` Service name format, for example `svc:remoteclaw`.
...
- Set `gateway.tailscale.resetOnExit` if you want RemoteClaw to undo `tailscale serve`
...
- Set `gateway.tailscale.preserveFunnel: true` to keep an externally configured
  ...
  gateway runs in `mode: "serve"`, RemoteClaw checks `tailscale funnel status`
  before re-applying Serve and skips it when a Funnel route already covers the
  gateway port. The RemoteClaw-managed Funnel password-only policy is unchanged.
