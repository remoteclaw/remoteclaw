...
import type { ModelsConfig, RemoteClawConfig, ModelProviderConfig } from "../config/types.js";
...
function sanitizeAuthConfig(params: {
  cfg: RemoteClawConfig;
  agentDir: string;
}): RemoteClawConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<RemoteClawConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const hostAgentDir = resolveRemoteClawAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-live-state-"));
  ...
  const toolProbePath = path.join(workspaceDir, `.remoteclaw-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveRemoteClawAgentDir();
  const sanitizedCfg: RemoteClawConfig = {
    ...params.cfg,
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "RemoteClaw live tool probe (local, safe): " +
...
describeLive("gateway live (dev agent, profile keys)", () => {
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.remoteclaw-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
