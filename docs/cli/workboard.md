---
summary: "CLI reference for `remoteclaw workboard` cards, dispatch, and worker runs"
read_when:
...
`remoteclaw workboard` is the terminal surface for the bundled [Workboard plugin](/plugins/workboard). It lets an operator list cards, create a card, inspect one card, and ask the running Gateway to dispatch ready work into subagent worker runs.

Enable the plugin before using the command:

```bash
remoteclaw plugins enable workboard
...
```bash
remoteclaw workboard list [--board <id>] [--status <status>] [--include-archived] [--json]
remoteclaw workboard create <title...> [--notes <text>] [--status <status>] [--priority <priority>] [--agent <id>] [--board <id>] [--labels <items>] [--json]
remoteclaw workboard show <id> [--json]
remoteclaw workboard dispatch [--url <url>] [--token <token>] [--timeout <ms>] [--json]
```
...
```bash
remoteclaw workboard list
remoteclaw workboard list --board default --status ready
remoteclaw workboard list --json
```
...
```bash
remoteclaw workboard create "Fix stale worker heartbeat" --priority high --labels bug,workboard
remoteclaw workboard create "Write Workboard docs" --status ready --agent docs-agent --board docs --notes "Cover CLI, slash command, dispatch, and SQLite state."
```
...
```bash
remoteclaw workboard show 7f4a2c10
remoteclaw workboard show 7f4a2c10 --json
```
...
```bash
remoteclaw workboard dispatch
remoteclaw workboard dispatch --json
remoteclaw workboard dispatch --url http://127.0.0.1:18789 --token "$REMOTECLAW_GATEWAY_TOKEN"
```
...
Local `list`, `create`, and `show` commands operate on the local RemoteClaw state directory used by the current profile. Use `--dev` or `--profile <name>` on the top-level `remoteclaw` command when you need a different state root.
...
```bash
remoteclaw plugins inspect workboard --runtime --json
```
...
Then retry `remoteclaw workboard dispatch`. Data-only fallback is useful for local state cleanup, but worker runs need a live Gateway.
...
```bash
remoteclaw workboard list --status ready
```
