...
const shouldPreferSourceGraph =
  ...
  (process.env.NODE_ENV !== "production" ||
    ...
    process.env.REMOTECLAW_PLUGIN_SDK_SOURCE_IN_TESTS === "1");
...
function listPluginSdkExportedSubpaths() {
  ...
  const cacheKey = `${packageRoot}::privateQa=${process.env.REMOTECLAW_ENABLE_PRIVATE_QA_CLI === "1" ? "1" : "0"}`;
...
function listPrivateLocalOnlyPluginSdkSubpaths() {
  if (process.env.REMOTECLAW_ENABLE_PRIVATE_QA_CLI !== "1") {
    return [];
  }
  try {
...
function getModuleLoader(tryNative) {
  ...
  const moduleLoader = createJiti(__filename, {
    ...
    // so local plugins do not create a second transpiled RemoteClaw core graph.
    tryNative,
