...
# build the runner/functional images with the prepared RemoteClaw package tarball.
...
docker_e2e_resolve_image() {
  ...
  if [ -n "${REMOTECLAW_DOCKER_E2E_IMAGE:-}" ]; then
    printf '%s\n' "$REMOTECLAW_DOCKER_E2E_IMAGE"
    return 0
  fi
...
docker_e2e_build_or_reuse() {
  ...
  if [ "${REMOTECLAW_SKIP_DOCKER_BUILD:-0}" = "1" ] || [ "$skip_build" = "1" ]; then
    ...
    if ! docker_e2e_docker_cmd image inspect "$image_name" >/dev/null 2>&1; then
      ...
      if docker_build_on_missing_enabled; then
        echo "Docker image not available; building because REMOTECLAW_DOCKER_BUILD_ON_MISSING/REMOTECLAW_TESTBOX allows fallback."
      else
  ...
  if [ "$target" = "functional" ]; then
    ...
    if [ -z "${REMOTECLAW_CURRENT_PACKAGE_TGZ:-}" ]; then
    ...
    build_args+=(--build-context "remoteclaw_package=$package_context")
  fi
...
docker_e2e_test_state_shell_b64() {
  ...
  node "$ROOT_DIR/scripts/lib/remoteclaw-test-state.mjs" shell \
    --label "$label" \
    --scenario "$scenario" |
    base64 |
    tr -d '\n'
...
docker_e2e_test_state_function_b64() {
  node "$ROOT_DIR/scripts/lib/remoteclaw-test-state.mjs" shell-function |
    base64 |
    tr -d '\n'
}
