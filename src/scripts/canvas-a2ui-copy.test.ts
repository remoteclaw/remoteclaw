...
const ORIGINAL_SKIP_MISSING = process.env.REMOTECLAW_A2UI_SKIP_MISSING;
const ORIGINAL_SPARSE_PROFILE = process.env.REMOTECLAW_SPARSE_PROFILE;
...
describe("canvas a2ui copy", () => {
  afterEach(() => {
    if (ORIGINAL_SKIP_MISSING === undefined) {
      ...
      process.env.REMOTECLAW_A2UI_SKIP_MISSING = ORIGINAL_SKIP_MISSING;
    }

    if (ORIGINAL_SPARSE_PROFILE === undefined) {
      delete process.env.REMOTECLAW_SPARSE_PROFILE;
    } else {
      process.env.REMOTECLAW_SPARSE_PROFILE = ORIGINAL_SPARSE_PROFILE;
    }
  });

  async function withA2uiFixture(run: (dir: string) => Promise<void>) {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-a2ui-"));
    try {
      await run(dir);
    } finally {
  ...
  it("skips missing assets when REMOTECLAW_A2UI_SKIP_MISSING=1", async () => {
    await withA2uiFixture(async (dir) => {
  ...
  it("skips missing assets when REMOTECLAW_SPARSE_PROFILE is set", async () => {
    await withA2uiFixture(async (dir) => {
      process.env.REMOTECLAW_SPARSE_PROFILE = "core";
      await expect(
        copyA2uiAssets({ srcDir: dir, outDir: path.join(dir, "out") }),
      ).resolves.toBeUndefined();
    });
