...
final class RealtimeTalkRelaySession {
    private static let agentControlToolName = "remoteclaw_agent_control"
    ...
    private func handleToolCall(_ payload: [String: AnyCodable]) async {
        ...
        do {
            ...
            let result: [String: Any] = completion.failed
                ? ["error": "RemoteClaw tool call failed"]
                : ["text": completion.text ?? "RemoteClaw finished with no text."]
                ...
                for await event in stream {
                    ...
                    if chatEvent.state == "final" {
                        return ChatCompletionResult(
                            text: RemoteClawChatEventText.assistantText(from: chatEvent),
                            failed: false)
                    }
