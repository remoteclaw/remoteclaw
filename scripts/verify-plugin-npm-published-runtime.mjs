...
// metadata expected by RemoteClaw.
...
export function collectPluginNpmPublishedRuntimeErrors(params) {
  ...
  const runtimeExtensionsResult = readPackageStringList(
    packageLabel,
    "remoteclaw.runtimeExtensions",
    packageJson.remoteclaw?.runtimeExtensions,
  );
  const setupEntryResult = readOptionalPackageString(
    packageLabel,
    "remoteclaw.setupEntry",
    packageJson.remoteclaw?.setupEntry,
  );
  const runtimeSetupEntryResult = readOptionalPackageString(
    packageLabel,
    "remoteclaw.runtimeSetupEntry",
    packageJson.remoteclaw?.runtimeSetupEntry,
  );
  errors.push(
  ...
  if (!hasPackedFile(packageFiles, "remoteclaw.plugin.json")) {
    errors.push(`${packageLabel} plugin npm package must include remoteclaw.plugin.json`);
    return errors;
  }
  ...
  if (runtimeExtensions.length > 0 && runtimeExtensions.length !== extensions.length) {
    errors.push(
      `${packageLabel} package.json remoteclaw.runtimeExtensions length (${runtimeExtensions.length}) must match remoteclaw.extensions length (${extensions.length})`,
    );
    return errors;
  }
  ...
  if (runtimeSetupEntry && !setupEntry) {
    errors.push(
      `${packageLabel} package.json remoteclaw.runtimeSetupEntry requires remoteclaw.setupEntry`,
    );
    return errors;
  }
...
export function readPluginNpmCommandOptions(env = process.env) {
  return {
    ...
    maxBuffer: readPositiveIntEnv(
      "REMOTECLAW_PLUGIN_NPM_COMMAND_MAX_BUFFER_BYTES",
      DEFAULT_NPM_COMMAND_MAX_BUFFER_BYTES,
      env,
    ),
    stdio: ["ignore", "pipe", "pipe"],
    timeout: readPositiveIntEnv(
      "REMOTECLAW_PLUGIN_NPM_COMMAND_TIMEOUT_MS",
...
async function packPublishedPackage(spec, destinationDir) {
  const attempts = readPositiveIntEnv("REMOTECLAW_PLUGIN_NPM_VERIFY_ATTEMPTS", 90);
  const delayMs = readPositiveIntEnv("REMOTECLAW_PLUGIN_NPM_VERIFY_DELAY_MS", 10000);
  let lastError;
...
async function verifyPublishedPackageReadme(spec) {
  const attempts = readPositiveIntEnv("REMOTECLAW_PLUGIN_NPM_README_VERIFY_ATTEMPTS", 6);
  const delayMs = readPositiveIntEnv("REMOTECLAW_PLUGIN_NPM_README_VERIFY_DELAY_MS", 10000);
  let lastError;
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
...
async function verifyPublishedPluginRuntime(spec) {
  const workingDir = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-plugin-npm-runtime."));
  try {
