...
async function packPublishedPackage(spec, destinationDir) {
  const attempts = Number.parseInt(process.env.REMOTECLAW_PLUGIN_NPM_VERIFY_ATTEMPTS ?? "90", 10);
  const delayMs = Number.parseInt(process.env.REMOTECLAW_PLUGIN_NPM_VERIFY_DELAY_MS ?? "10000", 10);
  let lastError;
