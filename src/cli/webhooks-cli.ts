...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.remoteclaw.org/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + RemoteClaw hooks")
    ...
    .option("--hook-url <url>", "RemoteClaw hook URL")
    .option("--hook-token <token>", "RemoteClaw hook token")
