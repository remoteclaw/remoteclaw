...
if [[ -z "${REMOTECLAW_GATEWAY_TOKEN:-}" ]]; then
  export REMOTECLAW_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "REMOTECLAW_GATEWAY_TOKEN" "$REMOTECLAW_GATEWAY_TOKEN"
  echo "Generated REMOTECLAW_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi
...
PODMAN_USERNS="${REMOTECLAW_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported REMOTECLAW_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (REMOTECLAW_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi
