...
async function waitForRunningRuntime(params: {
  ...
  let lastStatus: string | undefined = "unknown";
...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `remoteclaw-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      REMOTECLAW_LAUNCHD_LABEL: `ai.remoteclaw.launchd-int-${testId}`,
      REMOTECLAW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
