...
struct CommandCenterTab: View {
    ...
    @State private var defaultChatSessionEntry: RemoteClawChatSessionEntry?
    @State private var recentChatSessions: [RemoteClawChatSessionEntry] = []
    var openChat: () -> Void
    ...
    private var header: some View {
        HStack(alignment: .center, spacing: 11) {
            RemoteClawProMark(size: 31, shadowRadius: 9)
            Text("RemoteClaw")
        ...
        HStack(spacing: 8) {
            ...
            if let badgeValue {
                Text(badgeValue)
                    ...
                    .padding(.vertical, 3)
                    .background(RemoteClawBrand.accentHot, in: Capsule())
            }
            Spacer(minLength: 8)
            if let value {
                if let action {
    ...
    private var gatewayStatusColor: Color {
        self.gatewayConnected ? RemoteClawBrand.ok : .secondary
    }
    ...
    private var defaultChatWorkItem: WorkItem {
        ...
        return WorkItem(
            ...
            trailing: "chat",
            color: isOpen ? RemoteClawBrand.accent : RemoteClawBrand.ok,
            progress: nil,
            route: .chat(nil))
    }
    ...
    private static func sessionChoices(
        _ sessions: [RemoteClawChatSessionEntry],
        currentSessionKey: String,
        defaultSessionKey: String) -> [RemoteClawChatSessionEntry]
    {
        ...
        currentSessionKey: String) -> WorkItem
    {
        ...
        return WorkItem(
            ...
            trailing: "chat",
            color: isCurrent ? RemoteClawBrand.accent : RemoteClawBrand.ok,
            progress: nil,
            route: .chat(session.key))
    }

    fileprivate static func sessionTitle(_ session: RemoteClawChatSessionEntry) -> String {
    ...
    fileprivate static func sessionDetail(_ session: RemoteClawChatSessionEntry) -> String {
