...
export async function updateConfig(
  mutator: (cfg: RemoteClawConfig) => RemoteClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: RemoteClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: RemoteClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("remoteclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
