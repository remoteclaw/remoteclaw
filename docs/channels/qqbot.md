...
read_when:
  - You want to connect RemoteClaw to QQ
  - You need QQ Bot credential setup
  - You want QQ Bot group or private chat support
title: QQ bot
---

QQ Bot connects to RemoteClaw via the official QQ Bot API (WebSocket gateway). The
...
```bash
remoteclaw channels add --channel qqbot --token "AppID:AppSecret"
```
...
- `remoteclaw channels add --channel qqbot --token-file ...` provides the
...
Run multiple QQ bots under a single RemoteClaw instance:
...
```bash
remoteclaw channels add --channel qqbot --account bot2 --token "222222222:secret-of-bot-2"
```
...
Old QQBot `toolPolicy` entries are retired. Run `remoteclaw doctor --fix` to migrate them to `tools`.
...
| `/bot-version` | Show the RemoteClaw framework version                                                                    |
...
- Inbound, outbound, and gateway bridge paths share a single media payload root under `~/.remoteclaw/media`, so uploads, downloads, and transcode caches land under one guarded directory instead of a per-subsystem tree.
...
- Credentials can be backed up and restored as part of standard RemoteClaw credential snapshots; the engine re-attaches each account's resource stack on restore without requiring a fresh QR-code pair.

## QR-code onboarding

As an alternative to pasting `AppID:AppSecret` manually, the engine supports a QR-code onboarding flow for linking a QQ Bot to RemoteClaw:

1. Run the QQ Bot setup path (for example `remoteclaw channels add --channel qqbot`) and pick the QR-code flow when prompted.
2. Scan the generated QR code with the phone app tied to the target QQ Bot.
3. Approve the pairing on the phone. RemoteClaw persists the returned credentials into `credentials/` under the right account scope.

Approval prompts generated by the bot itself (for example, "allow this action?" flows exposed by the QQ Bot API) surface as native RemoteClaw prompts that you can accept with `/bot-approve` rather than replying through the raw QQ client.
...
- **Repeated self-replies:** RemoteClaw records QQ outbound ref indexes as
