...
export async function summarizeText(params: {
  ...
  const { text, targetLength } = params;
  if (targetLength < 100 || targetLength > 10_000) {
    throw new Error(`Invalid targetLength: ${targetLength}`);
  }

  // Embedded Pi model resolution and completeSimple were gutted (Middleware Boundary Principle).
  throw new Error(
    `TTS summarization not available: embedded model runner was removed. ` +
      `Text length ${text.length} exceeds target ${targetLength}.`,
  );
}
