---
summary: "Use OpenCode Zen and Go catalogs with RemoteClaw"
read_when:
...
OpenCode exposes two hosted catalogs in RemoteClaw:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. RemoteClaw keeps the runtime provider ids
...
remoteclaw onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
remoteclaw onboard --auth-choice opencode-go
remoteclaw onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
