...
Mattermost ships as a bundled plugin in current RemoteClaw releases, so normal
...
- Command callbacks are validated with the per-command tokens returned by
  Mattermost when RemoteClaw registers `oc_*` commands.
...
- Native slash commands return `Unauthorized: invalid command token.`: RemoteClaw
...
- If `callbackUrl` is omitted and logs warn that the callback resolved to
  ...
  Mattermost runs on the same host/network namespace as RemoteClaw. Set an
