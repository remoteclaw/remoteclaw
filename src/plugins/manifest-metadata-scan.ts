...
const REMOTECLAW_PACKAGE_ROOT = fileURLToPath(new URL("../..", import.meta.url));
const PLUGIN_MANIFEST_FILENAME = "remoteclaw.plugin.json";
...
function resolveUserPath(value: string, env: NodeJS.ProcessEnv): string {
  if (value === "~" || value.startsWith("~/")) {
    const home = env.REMOTECLAW_HOME ?? env.HOME ?? env.USERPROFILE ?? os.homedir();
...
function resolveStateDir(env: NodeJS.ProcessEnv): string {
  const override = normalizeTrimmedString(env.REMOTECLAW_STATE_DIR);
  if (override) {
    return resolveUserPath(override, env);
  }
  const home = env.REMOTECLAW_HOME ?? env.HOME ?? env.USERPROFILE ?? os.homedir();
  return path.join(home, ".remoteclaw");
}

function areBundledPluginsDisabled(env: NodeJS.ProcessEnv): boolean {
  const value = normalizeTrimmedString(env.REMOTECLAW_DISABLE_BUNDLED_PLUGINS)?.toLowerCase();
...
function resolveBundledPluginRoot(env: NodeJS.ProcessEnv): string | undefined {
  ...
  const override = normalizeTrimmedString(env.REMOTECLAW_BUNDLED_PLUGINS_DIR);
  if (override) {
    return resolveUserPath(override, env);
  }

  const sourceRoot = path.join(REMOTECLAW_PACKAGE_ROOT, "extensions");
  const runtimeRoot = path.join(REMOTECLAW_PACKAGE_ROOT, "dist-runtime", "extensions");
  const distRoot = path.join(REMOTECLAW_PACKAGE_ROOT, "dist", "extensions");
...
export function listRemoteClawPluginManifestMetadata(
