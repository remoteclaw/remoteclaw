...
IMAGE_NAME="remoteclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="remoteclaw-net-e2e-$$"
GW_NAME="remoteclaw-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "REMOTECLAW_GATEWAY_TOKEN=$TOKEN" \
  -e "REMOTECLAW_SKIP_CHANNELS=1" \
  -e "REMOTECLAW_SKIP_GMAIL_WATCHER=1" \
  -e "REMOTECLAW_SKIP_CRON=1" \
  -e "REMOTECLAW_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
