...
<AccordionGroup>
  ...
  <Accordion title="Can I use GPT 5.5 for daily tasks and Codex 5.5 for coding?">
    ...
    - **Native Codex coding agent:** set `agents.defaults.model.primary` to `openai/gpt-5.5` and `agents.defaults.agentRuntime.id` to `"codex"`. Sign in with `remoteclaw models auth login --provider openai-codex` when you want ChatGPT/Codex subscription auth.
...
<AccordionGroup>
  <Accordion title="What is an auth profile?">
    ...
    To inspect saved profiles without dumping secrets, run `remoteclaw models auth list` (optionally `--provider <id>` or `--json`). See [Models CLI](/cli/models#remoteclaw-models-auth-list) for details.
