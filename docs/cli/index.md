---
summary: "RemoteClaw CLI index: command list, global flags, and links to per-command pages"
read_when:
  - Finding the right `remoteclaw` subcommand
  - Looking up global flags or output styling rules
title: "CLI reference"
---

`remoteclaw` is the main CLI entry point. Each core command has either a
...
| `--update`              | Shorthand for [`remoteclaw update`](/cli/update) (source installs only) |
...
[`remoteclaw workboard`](/cli/workboard) or `remoteclaw voicecall`.
...
`remoteclaw status --usage` and the Control UI surface provider usage/quota when
