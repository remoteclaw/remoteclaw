...
export const ru: TranslationMap = {
  ...
  nodes: {
    pairing: {
      ...
      title: "RemoteClaw для мобильных устройств",
      ...
      qrAlt: "QR-код сопряжения RemoteClaw для мобильных устройств",
      ...
      review: "Обзор",
      waiting:
        "Официальные мобильные приложения RemoteClaw подключаются автоматически после сканирования.",
  ...
  worktrees: {
    ...
    subtitle: "Изолированные копии репозиториев, принадлежащие RemoteClaw.",
  ...
  subtitles: {
    ...
    config: "Редактировать remoteclaw.json.",
  ...
  overview: {
    ...
    pairing: {
      ...
      mobileHint:
        "На мобильном устройстве? Скопируйте полный URL (включая #token=...) из remoteclaw dashboard --no-open на настольном компьютере.",
    ...
    cards: {
      ...
      modelAuthAttentionExpiredDesc:
        "{providers} — повторно авторизуйтесь с помощью remoteclaw models auth",
  ...
  login: {
    ...
    failure: {
      ...
      authRequired: {
        ...
        stepPaste:
          "Вставьте токен из remoteclaw dashboard --no-open или введите настроенный пароль.",
        stepGenerate:
          "Если токен не настроен, выполните remoteclaw doctor --generate-gateway-token на хосте шлюза.",
      ...
      authFailed: {
        ...
        stepDashboard:
          "Запустите remoteclaw dashboard --no-open и откройте новый URL или вставьте его токен.",
        stepReplace:
      ...
      pairing: {
        ...
        stepList: "Выполните remoteclaw devices list на хосте Gateway.",
        stepApproveId: "Подтвердите этот запрос: remoteclaw devices approve {requestId}.",
      ...
      protocol: {
        ...
        stepDashboard:
          "Повторно откройте обслуживаемую панель командой remoteclaw dashboard, чтобы UI и Gateway были из одной установки.",
        stepDevUi:
          "Если используется pnpm ui:dev, пересоберите или перезапустите dev UI для текущей рабочей копии.",
        stepRestart:
          "Перезапустите Gateway после обновления RemoteClaw, чтобы он обслуживал текущий протокол.",
      },
      network: {
        ...
        stepGateway:
          "Убедитесь, что Gateway запущен, с помощью remoteclaw status или remoteclaw gateway run.",
        stepUrl:
          "Проверьте URL WebSocket и используйте wss://, когда Gateway находится за HTTPS/Tailscale Serve.",
        stepDashboard:
          "Снова откройте панель управления с помощью remoteclaw dashboard --no-open, чтобы повторно скопировать текущий URL и данные аутентификации.",
      },
    },
  },
  connection: {
