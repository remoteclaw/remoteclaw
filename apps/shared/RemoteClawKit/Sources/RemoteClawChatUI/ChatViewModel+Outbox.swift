...
private let outboxLogger = Logger(subsystem: "org.remoteclaw", category: "RemoteClawChatOutbox")

/// Display state for a transcript row backed by a durable outbox command.
public enum RemoteClawChatOutboxMessageState: Equatable, Sendable {
    case queued
    case sending
    case confirming
...
extension RemoteClawChatViewModel {
    ...
    public func outboxState(for messageID: UUID) -> RemoteClawChatOutboxMessageState? {
    ...
    func enqueueOutboxCommand(
        text: String,
        draftInput: String,
        draftAttachments: [RemoteClawPendingAttachment] = [],
        session: SessionSnapshot) async
    {
        ...
        let command = RemoteClawChatOutboxCommand(
            id: UUID().uuidString,
            ...
            attachments: draftAttachments.map {
                RemoteClawChatOutboxAttachment(
                    type: $0.type,
                    mimeType: $0.mimeType,
                    fileName: $0.fileName,
                    data: $0.data,
    ...
    func confirmOutboxCommands(in messages: [RemoteClawChatMessage]) {
    ...
    func confirmOutboxCommandsNow(in messages: [RemoteClawChatMessage]) async {
        self.observeCanonicalOutboxMessageKeys(in: messages)
        guard let outbox else { return }
    ...
    private func persistCanonicalOutboxEvidence(
        _ message: RemoteClawChatMessage,
        for command: RemoteClawChatOutboxCommand) async
    {
        guard let transcriptCache = transcriptCache as? any RemoteClawChatCanonicalTranscriptMerging else { return }
    ...
    private func observeCanonicalOutboxMessageKeys(in messages: [RemoteClawChatMessage]) {
    ...
    private func presentOutboxCommands(_ commands: [RemoteClawChatOutboxCommand]) {
    ...
    private static func outboxUserMessage(for command: RemoteClawChatOutboxCommand) -> RemoteClawChatMessage {
        var content = [
    ...
    private func mapOutboxCommand(_ command: RemoteClawChatOutboxCommand, to messageID: UUID) {
    ...
    private func performOutboxFlush() async {
        ...
        while self.healthOK {
            ...
            do {
                let response = try await routeLease.sendMessage(
                    ...
                    attachments: next.attachments.map {
                        RemoteClawChatAttachmentPayload(
                            type: $0.type,
                            mimeType: $0.mimeType,
                            fileName: $0.fileName,
            ...
            } catch is RemoteClawChatTransportSendError {
                ...
                if error.detailsReason == RemoteClawChatSessionRoutingContract.changedErrorReason {
                    guard await self.parkOutboxCommandForChangedTarget(next, outbox: outbox) else { break }
                    continue
                }
    ...
    private static func needsOutboxDeliveryReconciliation(_ command: RemoteClawChatOutboxCommand) -> Bool {
        command.status == .awaitingConfirmation ||
            (command.status == .failed &&
                command.lastError == RemoteClawChatSQLiteTranscriptCache.outboxUnconfirmedError)
    }

    private static func deliveryTarget(for command: RemoteClawChatOutboxCommand) -> OutboxDeliveryTarget {
    ...
    private func parkOutboxCommandWithUnconfirmedDelivery(
        _ command: RemoteClawChatOutboxCommand,
        outbox: any RemoteClawChatCommandOutbox) async -> RemoteClawChatOutboxUpdateResult
    {
        let update = await outbox.markCommandFailedIfPresent(
            id: command.id,
            retryCount: command.retryCount,
            lastError: RemoteClawChatSQLiteTranscriptCache.outboxUnconfirmedError)
        switch update {
        case .updated:
            self.setOutboxState(
                .failed(reason: RemoteClawChatSQLiteTranscriptCache.outboxUnconfirmedError),
    ...
    private func parkOutboxCommandForChangedTarget(
        _ command: RemoteClawChatOutboxCommand,
        outbox: any RemoteClawChatCommandOutbox) async -> Bool
    {
        let update = await outbox.markCommandFailedIfPresent(
            id: command.id,
            retryCount: command.retryCount,
            lastError: RemoteClawChatSQLiteTranscriptCache.outboxChangedTargetError)
    ...
    private func recordOutboxRejection(
        of command: RemoteClawChatOutboxCommand,
    ...
    private func spliceSentCommandIntoCachedTranscript(_ command: RemoteClawChatOutboxCommand) async {
    ...
    private func outboxRoutingContract(for session: SessionSnapshot) -> String? {
        if !self.transport.outboxRequiresSessionRoutingContract {
            return RemoteClawChatOutboxCommand.legacyUnboundRoutingContract
        }
    ...
    private func setOutboxState(_ state: RemoteClawChatOutboxMessageState, forCommandID commandID: String) {
    ...
    func handleOutboxChange(_ change: RemoteClawChatOutboxChange) {
    ...
    private static func outboxDisplayState(for command: RemoteClawChatOutboxCommand)
        -> RemoteClawChatOutboxMessageState
    {
