...
describe("sessions tools", () => {
  ...
  it("sessions_list filters kinds and includes messages", async () => {
    ...
    const tool = createRemoteClawTools().find((candidate) => candidate.name === "sessions_list");
    expect(tool).toBeDefined();
    if (!tool) {
  ...
  it("sessions_list resolves transcriptPath from agent state dir for multi-store listings", async () => {
    ...
    const tool = createRemoteClawTools().find(
      (candidate: { name: string }) => candidate.name === "sessions_list",
    );
    expect(tool).toBeDefined();
    if (!tool) {
      throw new Error("missing sessions_list tool");
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createRemoteClawTools().find((candidate) => candidate.name === "sessions_history");
    expect(tool).toBeDefined();
    if (!tool) {
