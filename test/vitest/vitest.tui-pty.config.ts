...
export function createTuiPtyVitestConfig(env?: Record<string, string | undefined>) {
  ...
  const includeLocal = configEnv.REMOTECLAW_TUI_PTY_INCLUDE_LOCAL === "1";
  const include = [
    "tui/tui-pty-harness.e2e.test.ts",
    ...(includeLocal ? ["tui/tui-pty-local.e2e.test.ts"] : []),
  ];
  const includeFromEnv = toTuiPtyIncludePatterns(
    loadPatternListFromEnv("REMOTECLAW_VITEST_INCLUDE_FILE", configEnv),
  );
  ...
  return defineConfig({
    ...
    test: {
      ...
      setupFiles: [
        ...new Set(
          [...(baseTest.setupFiles ?? []), "test/setup-remoteclaw-runtime.ts"].map(
            resolveRepoRootPath,
          ),
        ),
      ],
