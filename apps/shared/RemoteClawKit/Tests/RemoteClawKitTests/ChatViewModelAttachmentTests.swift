...
private actor AttachmentSendCapture {
    private(set) var attachments: [RemoteClawChatAttachmentPayload] = []

    func store(_ attachments: [RemoteClawChatAttachmentPayload]) {
    ...
    func first() -> RemoteClawChatAttachmentPayload? {
...
private struct AttachmentProcessingTransport: RemoteClawChatTransport {
        ...
        attachments: [RemoteClawChatAttachmentPayload]) async throws -> RemoteClawChatSendResponse
    {
        ...
        return RemoteClawChatSendResponse(runId: idempotencyKey, status: self.responseStatus)
    }
    ...
    func acquireOutboxRouteLease() async -> RemoteClawChatTransportRouteLeaseResult {
        guard self.durableOutboxAvailable else {
            return .unavailable(reason: RemoteClawChatTransportUpgradeMessage.routingContract)
        }
        let transport = self
...
private func makeAttachmentOutbox() -> RemoteClawChatSQLiteTranscriptCache {
    RemoteClawChatSQLiteTranscriptCache(
...
private func makeDurableAttachmentViewModel(
    ...
    outbox: RemoteClawChatSQLiteTranscriptCache) -> RemoteClawChatViewModel
{
...
final class ChatViewModelAttachmentTests: XCTestCase {
    func testImageAttachmentsAreProcessedBeforeStaging() async throws {
        ...
        let viewModel = await MainActor.run {
            RemoteClawChatViewModel(sessionKey: "main", transport: AttachmentProcessingTransport())
        }
    ...
    func testPartialIdentitySyncPreservesTheOtherDeferredComponent() {
        ...
        let contractViewModel = RemoteClawChatViewModel(
            sessionKey: "main",
            transport: AttachmentProcessingTransport(),
            activeAgentId: "main",
            sessionRoutingContract: oldContract)
        let contractAttachment = RemoteClawPendingAttachment(
            url: nil,
        ...
        let agentViewModel = RemoteClawChatViewModel(
            sessionKey: "main",
            transport: AttachmentProcessingTransport(),
            activeAgentId: "main",
            sessionRoutingContract: oldContract)
        let agentAttachment = RemoteClawPendingAttachment(
            url: nil,
    ...
    func testAttachmentStagingPinsSessionAndIdentityUntilItFinishes() {
        let viewModel = RemoteClawChatViewModel(
            sessionKey: "main",
            transport: AttachmentProcessingTransport(),
            activeAgentId: "main",
    ...
    func testFailedAttachmentSendWithoutOutboxRestoresDraft() async throws {
        ...
        let attachmentID = await MainActor.run {
            let attachment = RemoteClawPendingAttachment(
                url: nil,
                data: attachmentData,
                fileName: "retry.m4a",
                mimeType: "audio/mp4",
    ...
    func testVoiceNoteSendKeepsCapturedDurationWhenDraftChangesDuringHealthCheck() async throws {
        ...
        let (viewModel, draftAttachmentID) = await MainActor.run {
            let viewModel = RemoteClawChatViewModel(sessionKey: "main", transport: transport)
            let draftAttachment = RemoteClawPendingAttachment(
                url: nil,
    ...
    func testAmbiguousVoiceNoteSurvivesViewModelRecreation() async throws {
        let outbox = makeAttachmentOutbox()
        var firstViewModel: RemoteClawChatViewModel? = await MainActor.run {
            let viewModel = makeDurableAttachmentViewModel(
    ...
    func testCanonicalVoiceNoteConfirmationPreservesDurationAndDeletesDurableBytes() async throws {
        ...
        let canonical = try JSONDecoder().decode(
            ...
            from: Data(
                """
                {"role":"user","content":"See attached.","__remoteclaw":{"idempotencyKey":"\(command
    ...
    func testCanonicalVoiceNotePreservesOptimisticDuration() throws {
        let localAudio = RemoteClawChatMessageContent(
            type: "file",
            text: nil,
            mimeType: "audio/mp4",
        ...
        let existing = RemoteClawChatMessage(
            role: "user",
            content: [localAudio],
            timestamp: nil,
        ...
        let incoming = try JSONDecoder().decode(
            RemoteClawChatMessage.self,
            from: Data(
                #"{"role":"user","content":"See attached.","__remoteclaw":{"idempotencyKey":"run:user"},"MediaPaths":["media/inbound/media-1.m4a"],"MediaTypes":["audio/mp4"]}"#
                    .utf8))

        let adopted = RemoteClawChatViewModel.adoptingCanonicalMessage(incoming, over: existing)
