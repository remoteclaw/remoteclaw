...
2. Set at least host, nick, and the channels to join in `~/.remoteclaw/remoteclaw.json`:
...
| `username`                    | nick, else `remoteclaw`       | IRC username                                                |
| `realname`                    | `RemoteClaw`                  | Realname/GECOS field                                        |
| `password` / `passwordFile`   | none                          | Server password; file must be a regular file                |
| `channels`                    | none                          | Channels to join (`["#remoteclaw"]`)                        |
| `accounts` / `defaultAccount` | none                          | Multi-account setup; env vars fill only the default account |

## Security defaults

- IRC uses raw TCP/TLS sockets outside RemoteClaw operator-managed forward proxy routing. In deployments that require all egress through that forward proxy, set `channels.irc.enabled=false` unless direct IRC egress is explicitly approved.
- `channels.irc.dmPolicy` defaults to `"pairing"`: unknown DM senders get a pairing code you approve with `remoteclaw pairing approve irc <code>`.
...
Example (allow anyone in `#remoteclaw` to talk to the bot):
...
Even if a channel is allowed (via `groupPolicy` + `groups`) and the sender is allowed, RemoteClaw defaults to **mention-gating** in group contexts. The bot counts as mentioned when the message contains the connected bot nick or matches your configured mention patterns.
