...
public enum GatewayConnectionProblemMapper {
    ...
    private static func map(_ authError: GatewayConnectAuthError) -> GatewayConnectionProblem {
        ...
        case .authTokenMissing:
            return self.problem(
                ...
                docsURL: self.docsURL(
                    ...
                    fallback: "https://docs.remoteclaw.org/gateway/authentication"),
        ...
        case .authTokenNotConfigured:
            return self.problem(
                ...
                actionCommand: authError.actionCommand ?? "remoteclaw config set gateway.auth.token <new-token>",
                docsURL: self.docsURL(
                    authError.docsURLString,
                    fallback: "https://docs.remoteclaw.org/gateway/authentication"),
                requestId: authError.requestId,
        ...
        case .authPasswordNotConfigured:
            return self.problem(
                ...
                actionLabel: authError.actionLabel ?? "Fix on gateway",
                actionCommand: authError.actionCommand ?? "remoteclaw config set gateway.auth.password <new-password>",
                docsURL: self.docsURL(
                    authError.docsURLString,
                    fallback: "https://docs.remoteclaw.org/gateway/authentication"),
                requestId: authError.requestId,
        ...
        case .authBootstrapTokenInvalid:
            return self.problem(
                ...
                actionCommand: authError.actionCommand,
                docsURL: self.docsURL(authError.docsURLString, fallback: "https://docs.remoteclaw.org/platforms/ios"),
                requestId: authError.requestId,
                retryable: false,
                pauseReconnect: true,
                authError: authError)
        ...
        case .deviceAuthSignatureExpired:
            return self.problem(
                ...
                docsURL: self.docsURL(
                    authError.docsURLString,
                    fallback: "https://docs.remoteclaw.org/gateway/troubleshooting"),
                requestId: authError.requestId,
                retryable: true,
                pauseReconnect: true,
                authError: authError)
        ...
        case .authTailscaleIdentityMissing:
            return self.problem(
                ...
                actionCommand: authError.actionCommand,
                docsURL: self.docsURL(authError.docsURLString, fallback: "https://docs.remoteclaw.org/gateway/tailscale"),
                requestId: authError.requestId,
                retryable: false,
                pauseReconnect: true,
                authError: authError)
    ...
    private static func map(_ tlsError: GatewayTLSValidationError) -> GatewayConnectionProblem {
        ...
        case .pinMismatch:
            ...
            return GatewayConnectionProblem(
                ...
                actionCommand: nil,
                docsURL: URL(string: "https://docs.remoteclaw.org/gateway/troubleshooting"),
                retryable: false,
                pauseReconnect: true,
        ...
        case .certificateUnavailable:
            return GatewayConnectionProblem(
                ...
                message: "RemoteClaw could not read the gateway certificate for \(failure.host).",
                actionLabel: "Retry",
                actionCommand: nil,
                docsURL: URL(string: "https://docs.remoteclaw.org/gateway/troubleshooting"),
                retryable: true,
        ...
        if nsError.domain == URLError.errorDomain {
            ...
            case .cannotFindHost, .dnsLookupFailed, .notConnectedToInternet, .networkConnectionLost,
                ...
                return GatewayConnectionProblem(
                    ...
                    message: "RemoteClaw could not reach the gateway over the current network.",
    ...
    private static func protocolMismatchProblem(for authError: GatewayConnectAuthError) -> GatewayConnectionProblem {
           ...
           clientMax < expected
        {
            ...
            message = authError.userMessageOverride
                ?? "This app is older than the gateway. Update RemoteClaw on this device, then retry."
            owner = .iphone
                  ...
                  clientMin > expected
        {
            ...
            message = authError.userMessageOverride
                ?? "The gateway is older than this app. Update RemoteClaw on the gateway host, then retry."
            owner = .gateway
            actionLabel = authError.actionLabel ?? "Copy update command"
            actionCommand = authError.actionCommand ?? "remoteclaw update"
        } else {
            title = authError.titleOverride ?? "RemoteClaw update required"
            message = authError.userMessageOverride
                ?? "The app and gateway use incompatible protocol versions. Update RemoteClaw on both, then retry."
            owner = .both
            actionLabel = authError.actionLabel ?? "Update RemoteClaw"
            actionCommand = authError.actionCommand
        }
        return self.problem(
    ...
    private static func approvalCommand(requestId: String?) -> String {
        if let requestId = self.nonEmpty(requestId) {
            return "remoteclaw devices approve \(requestId)"
        }
        return "remoteclaw devices list"
    }
