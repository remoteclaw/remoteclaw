...
> **Note:** The Android app has not been publicly released yet. The source code is available in the [RemoteClaw repository](https://github.com/remoteclaw/remoteclaw) under `apps/android`. You can build it yourself using Java 17 and the Android SDK (`./gradlew :app:assemblePlayDebug`). See [apps/android/README.md](https://github.com/remoteclaw/remoteclaw/blob/main/apps/android/README.md) for build instructions.
...
- You can run the CLI (`remoteclaw`) on the gateway machine (or via SSH).
...
- Set `gateway.bind: "tailnet"` in `~/.remoteclaw/remoteclaw.json` on the gateway host.
...
1. Set up a DNS-SD zone (example `remoteclaw.internal.`) on the gateway host and publish `_remoteclaw-gw._tcp` records.
...
- Via Gateway:
  ...
  ```bash
  remoteclaw gateway call node.list --params "{}"
  ```
...
1. Create `~/.remoteclaw/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
remoteclaw nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__remoteclaw__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__remoteclaw__/canvas/`.

This server injects a live-reload client into HTML and reloads on file changes.
The A2UI host lives at `http://<gateway-host>:18789/__remoteclaw__/a2ui/`.
...
Android supports launching RemoteClaw from the system assistant trigger (Google
Assistant). When configured, holding the home button or saying "Hey Google, ask
RemoteClaw..." opens the app and hands the prompt into the chat composer.
...
and whether the user has set RemoteClaw as the default assistant app.
</Note>
