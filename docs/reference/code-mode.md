---
summary: "RemoteClaw code mode: an opt-in exec/wait tool surface backed by QuickJS-WASI and a hidden run-scoped tool catalog"
title: "Code mode"
sidebarTitle: "Code mode"
read_when:
  - You want to enable RemoteClaw code mode for an agent run
...
Code mode is an experimental RemoteClaw agent-runtime feature. It is off by
default. When you enable it, RemoteClaw changes what the model sees for one run:
instead of exposing every enabled tool schema directly, the model sees only
`exec` and `wait`.

This page documents RemoteClaw code mode. It is not Codex Code mode. The two
...
- RemoteClaw code mode is disabled unless `tools.codeMode.enabled: true` is
  configured. It runs in the RemoteClaw generic agent runtime, where the model
...
surfaces. RemoteClaw code mode is an RemoteClaw-owned experimental tool-surface
adapter for generic RemoteClaw runs. It uses `quickjs-wasi`, a hidden RemoteClaw
tool catalog, and the normal RemoteClaw tool executor.

## What is this?

RemoteClaw code mode lets the model write a small JavaScript or TypeScript program
...
- `exec` evaluates model-generated JavaScript or TypeScript in a constrained
  QuickJS-WASI worker.
- Normal RemoteClaw tools are hidden from the model prompt and exposed inside the
  guest program through `ALL_TOOLS` and `tools`.
- Guest code can search the hidden catalog, describe a tool, and call a tool
  through the same RemoteClaw execution path used by normal agent turns.
...
surface. It does not replace RemoteClaw tools, plugin tools, MCP tools, auth,
...
- Provider neutral: it works for RemoteClaw, plugin, MCP, and client tools without
  depending on provider-native code execution.
- Existing policy stays in force: nested tool calls still go through RemoteClaw
...
- Clear failure mode: when code mode is explicitly enabled and the runtime is
  unavailable, RemoteClaw fails closed instead of falling back to broad direct tool
  exposure.
...
```bash
REMOTECLAW_DEBUG_CODE_MODE=1 \
REMOTECLAW_DEBUG_MODEL_TRANSPORT=1 \
REMOTECLAW_DEBUG_MODEL_PAYLOAD=tools \
remoteclaw gateway
```
...
`REMOTECLAW_DEBUG_MODEL_PAYLOAD=full-redacted` for a short debugging session.
...
- Stability: experimental RemoteClaw surface; Codex Code mode is a separate stable
  Codex harness surface.
- Target surface: generic RemoteClaw agent runs.
...
**Code mode** is the RemoteClaw runtime mode that hides normal model tools and
...
If code mode is enabled but QuickJS-WASI cannot load, RemoteClaw fails closed for
...
2. Build the effective RemoteClaw tool list.
...
- If `language` is `"typescript"`, RemoteClaw transpiles before evaluation.
...
final value is JSON-compatible after RemoteClaw's output adapter runs.
...
`wait` exists because nested RemoteClaw tools can be slow, interactive, approval
...
2. On suspension, RemoteClaw snapshots the QuickJS VM and records pending host
   work.
3. When pending work settles, `wait` restores the VM snapshot.
4. RemoteClaw re-registers host callbacks by stable names.
5. RemoteClaw delivers nested tool results into the restored VM.
6. RemoteClaw drains QuickJS pending jobs.
...
type ToolCatalogEntry = {
  ...
  label?: string;
  description: string;
  source: "remoteclaw" | "plugin" | "mcp" | "client";
  sourceName?: string;
};
```
...
const hits = await tools.web_search({ query: "RemoteClaw code mode" });
```
...
state directory. For each code-mode `exec` call, RemoteClaw builds the run-scoped
...
inside the QuickJS program while RemoteClaw still shows only `exec` and `wait` to
the model.
...
7. RemoteClaw auto-drains ready namespace bridge calls inside the active
...
- `globalName` collisions with built-in code-mode globals such as `tools`,
  `namespaces`, `text`, `json`, `yield_control`, or `__remoteclaw*`
...
catalog for arbitrary enabled RemoteClaw, plugin, and client tools; use `MCP` for
...
- images and files travel through ordinary RemoteClaw tools, not through the
  code-mode bridge
...
Code mode supersedes the RemoteClaw Tool Search model surface for runs where it is
active.

When `tools.codeMode.enabled` is true and code mode activates:

- RemoteClaw does not expose `tool_search_code`, `tool_search`, `tool_describe`,
...
- MCP calls use the generated `MCP` namespace and its `$api()` headers instead
  of `tools.call(...)`.
- Nested calls dispatch through the same RemoteClaw executor path that Tool Search
  uses.

The existing [Tool Search](/tools/tool-search) page describes the RemoteClaw compact
catalog bridge. Code mode is the generic RemoteClaw alternative for runs that can
use `exec` and `wait`.

## Tool names and collisions

The model-visible `exec` tool is the code-mode tool. If the normal RemoteClaw
shell `exec` tool is enabled, it is hidden from the model and cataloged like any
other tool.

Inside the guest runtime:

- `tools.call("remoteclaw:core:exec", input)` can call the shell exec tool if
  policy allows it.
...
If two tools normalize to the same safe convenience name, RemoteClaw omits the
convenience function and requires `tools.call(id, input)`.

## Nested tool execution

Every nested tool call crosses the host bridge and re-enters RemoteClaw.
...
RemoteClaw loads `quickjs-wasi` as a direct dependency in the owning package. The
...
The runtime executes outside RemoteClaw's main event loop in a worker. A guest
...
inputs beyond existing RemoteClaw trajectory policy.
...
REMOTECLAW_DEBUG_MODEL_TRANSPORT=1 \
REMOTECLAW_DEBUG_MODEL_PAYLOAD=tools \
REMOTECLAW_DEBUG_SSE=events \
remoteclaw gateway
```

For payload-shape debugging, use `REMOTECLAW_DEBUG_MODEL_PAYLOAD=full-redacted`.
...
For stream debugging, use `REMOTECLAW_DEBUG_SSE=peek` to log the first five
...
- `tools.search`, `tools.describe`, and `tools.call` work for RemoteClaw tools
...
5. Send an agent turn with RemoteClaw, plugin, MCP, and client test tools.
...
8. In `exec`, call RemoteClaw/plugin/client tools through `tools.search`,
