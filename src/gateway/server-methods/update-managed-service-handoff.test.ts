...
async function runHelperWithExistingSentinel(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-handoff-helper-test-"));
  tempDirs.add(tmpDir);
...
describe("managed service update handoff", () => {
  it("strips process supervisor hints while preserving service identity for the CLI handoff", async () => {
    ...
    const serviceIdentityEnv = {
      REMOTECLAW_LAUNCHD_LABEL: "com.example.remoteclaw.test",
      REMOTECLAW_SYSTEMD_UNIT: "remoteclaw-test.service",
    ...
    expect(result.command).toBe("remoteclaw update --yes --timeout 1800");
    ...
    expect(options.env.REMOTECLAW_UPDATE_RUN_HANDOFF).toBe("1");
  ...
  it("launches systemd handoffs through a transient user scope", async () => {
    const { startManagedServiceUpdateHandoff } =
      await import("./update-managed-service-handoff.js");
    const binDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-systemd-run-bin-"));
    tempDirs.add(binDir);
    ...
    const result = await startManagedServiceUpdateHandoff({
      ...
      env: {
        PATH: binDir,
        REMOTECLAW_SYSTEMD_UNIT: "remoteclaw-gateway.service",
    ...
    expect(args.slice(0, 4)).toEqual([
      ...
      "--scope",
      "--collect",
      "--unit=remoteclaw-update-handoff-123.scope",
    ]);
    ...
    expect(options.detached).toBe(true);
    expect(options.env.REMOTECLAW_SYSTEMD_UNIT).toBe("remoteclaw-gateway.service");
  ...
  it("sweeps stale handoff temp directories while keeping fresh handoff logs", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-handoff-cleanup-test-"));
    tempDirs.add(tmpDir);
    ...
    const unrelatedDir = path.join(tmpDir, "remoteclaw-other-temp");
