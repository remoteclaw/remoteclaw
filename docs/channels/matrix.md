...
Matrix is an open, decentralized messaging protocol. RemoteClaw connects as a Matrix **user**
...
1. Install the Matrix plugin:
   - From npm: `remoteclaw plugins install @remoteclaw/matrix`
   - From a local checkout: `remoteclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: RemoteClaw calls the same
     login endpoint, stores the access token in `~/.remoteclaw/credentials/matrix/credentials.json`,
...
- On first connection, RemoteClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  RemoteClaw logs a warning.
...
`~/.remoteclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, RemoteClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
