...
read_when:
  - Setting up Matrix in RemoteClaw
...
Matrix is the Matrix bundled channel plugin for RemoteClaw.
It uses the official `matrix-js-sdk` and supports DMs, rooms, threads, media, reactions, polls, location, and E2EE.

## Bundled plugin

Matrix ships as a bundled plugin in current RemoteClaw releases, so normal
...
```json5
{
  channels: {
    matrix: {
      ...
      deviceName: "RemoteClaw Gateway",
    },
  },
}
```

Matrix stores cached credentials in `~/.remoteclaw/credentials/matrix/`.
The default account uses `credentials.json`; named accounts use `credentials-<account>.json`.
When cached credentials exist there, RemoteClaw treats Matrix as configured for setup, doctor, and channel-status discovery even if current auth is not set directly in config.
...
Set `channels.matrix.streaming` to `"partial"` when you want RemoteClaw to send a single live preview
...
- `streaming: "off"` is the default. RemoteClaw waits for the final reply and sends it once.
...
- If the preview no longer fits in one Matrix event, RemoteClaw stops preview streaming and falls back to normal final delivery.
- Media replies still send attachments normally. If a stale preview can no longer be reused safely, RemoteClaw redacts it before sending the final media reply.
...
- bot user = the RemoteClaw Matrix account that sends the reply
...
1. Configure RemoteClaw to use quiet previews:
...
RemoteClaw rule below.

RemoteClaw marks finalized text-only preview edits with:

```json
{
  "com.remoteclaw.finalized_preview": true
}
```
...
```bash
curl -sS -X PUT \
  "https://matrix.example.org/_matrix/client/v3/pushrules/global/override/remoteclaw-finalized-preview-botname" \
  -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  --data '{
    "conditions": [
      ...
      {
        "kind": "event_property_is",
        "key": "content.com\\.remoteclaw\\.finalized_preview",
        "value": true
      },
...
- `remoteclaw-finalized-preview-botname`: a rule ID unique to this bot for this receiving user
- `@bot:example.org`: your RemoteClaw Matrix bot MXID, not the receiving user's MXID
...
- If one receiving user should notify for multiple RemoteClaw Matrix bot accounts, create one rule per bot with a unique rule ID for each sender match.
- A simple pattern is `remoteclaw-finalized-preview-<botname>`, such as `remoteclaw-finalized-preview-ops` or `remoteclaw-finalized-preview-support`.

The rule is evaluated against the event sender:

- authenticate with the receiving user's token
- match `sender` against the RemoteClaw bot MXID
...
- This only affects text-only preview edits that RemoteClaw can safely finalize in place. Media fallbacks and stale-preview fallbacks still use normal Matrix delivery.
...
- No special `homeserver.yaml` change is required for finalized RemoteClaw preview notifications.
...
By default, Matrix messages from other configured RemoteClaw Matrix accounts are ignored.
...
- RemoteClaw still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix does not expose a native bot flag here; RemoteClaw treats "bot-authored" as "sent by another configured Matrix account on this RemoteClaw gateway".
...
```bash
remoteclaw matrix verify status
```

Verbose status (full diagnostics):

```bash
remoteclaw matrix verify status --verbose
```

Include the stored recovery key in machine-readable output:

```bash
remoteclaw matrix verify status --include-recovery-key --json
```

Bootstrap cross-signing and verification state:

```bash
remoteclaw matrix verify bootstrap
```
...
```bash
remoteclaw matrix verify bootstrap --verbose
```

Force a fresh cross-signing identity reset before bootstrapping:

```bash
remoteclaw matrix verify bootstrap --force-reset-cross-signing
```

Verify this device with a recovery key:

```bash
remoteclaw matrix verify device "<your-recovery-key>"
```

Verbose device verification details:

```bash
remoteclaw matrix verify device "<your-recovery-key>" --verbose
```

Check room-key backup health:

```bash
remoteclaw matrix verify backup status
```

Verbose backup health diagnostics:

```bash
remoteclaw matrix verify backup status --verbose
```

Restore room keys from server backup:

```bash
remoteclaw matrix verify backup restore
```

Verbose restore diagnostics:

```bash
remoteclaw matrix verify backup restore --verbose
```
...
```bash
remoteclaw matrix verify backup reset --yes
```
...
```bash
remoteclaw matrix verify status --account assistant
remoteclaw matrix verify backup restore --account assistant
remoteclaw matrix devices list --account assistant
```
...
RemoteClaw treats this Matrix device as verified only when it is verified by your own cross-signing identity.
In practice, `remoteclaw matrix verify status --verbose` exposes three trust signals:
...
Local trust by itself is not enough for RemoteClaw to treat the device as fully verified.

### What bootstrap does

`remoteclaw matrix verify bootstrap` is the repair and setup command for encrypted Matrix accounts.
...
If the homeserver requires interactive auth to upload cross-signing keys, RemoteClaw tries the upload without auth first, then with `m.login.dummy`, then with `m.login.password` when `channels.matrix.password` is configured.
...
backup baseline for future messages, use `remoteclaw matrix verify backup reset --yes`.
Do this only when you accept that unrecoverable old encrypted history will stay
unavailable and that RemoteClaw may recreate secret storage if the current backup
...
If startup finds broken bootstrap state and `channels.matrix.password` is configured, RemoteClaw can attempt a stricter repair path.
If the current device is already owner-signed, RemoteClaw preserves that identity instead of resetting it automatically.

Upgrading from the previous public Matrix plugin:

- RemoteClaw automatically reuses the same Matrix account, access token, and device identity when possible.
- Before any actionable Matrix migration changes run, RemoteClaw creates or reuses a recovery snapshot under `~/Backups/remoteclaw-migrations/`.
- If you use multiple Matrix accounts, set `channels.matrix.defaultAccount` before upgrading from the old flat-store layout so RemoteClaw knows which account should receive that shared legacy state.
- If the previous plugin stored a Matrix room-key backup decryption key locally, startup or `remoteclaw doctor --fix` will import it into the new recovery-key flow automatically.
...
- If the Matrix access token changes later for the same account, homeserver, and user, RemoteClaw now prefers reusing the most complete existing token-hash storage root instead of starting from an empty Matrix state directory.
- On the next gateway start, backed-up room keys are restored automatically into the new crypto store.
- If the old plugin had local-only room keys that were never backed up, RemoteClaw will warn clearly. Those keys cannot be exported automatically from the previous rust crypto store, so some old encrypted history may remain unavailable until recovered manually.
...
`~/.remoteclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/`.
...
When the token changes but the account identity stays the same, RemoteClaw reuses the best existing
...
RemoteClaw currently provides that in Node by:
...
Under RemoteClaw's security model, the gateway host and local RemoteClaw state directory are already inside the trusted operator boundary, so this is primarily an operational durability concern rather than a separate remote trust boundary.

Planned improvement:

- add SecretRef support for persistent Matrix key material so recovery keys and related store-encryption secrets can be sourced from RemoteClaw secrets providers instead of only local files
...
```bash
remoteclaw matrix profile set --name "RemoteClaw Assistant"
remoteclaw matrix profile set --avatar-url https://cdn.example.org/avatar.png
```

Add `--account <id>` when you want to target a named Matrix account explicitly.

Matrix accepts `mxc://` avatar URLs directly. When you pass an `http://` or `https://` avatar URL, RemoteClaw uploads it to Matrix first and stores the resolved `mxc://` URL back into `channels.matrix.avatarUrl` (or the selected account override).
...
Incoming verification requests from another Matrix client are tracked and auto-accepted by RemoteClaw.
For self-verification flows, RemoteClaw also starts the SAS flow automatically when emoji verification becomes available and confirms its own side.
For verification requests from another Matrix user/device, RemoteClaw auto-accepts the request and then waits for the SAS flow to proceed normally.
...
RemoteClaw does not auto-accept self-initiated duplicate flows blindly. Startup skips creating a new request when a self-verification request is already pending.

Verification protocol/system notices are not forwarded to the agent chat pipeline, so they do not produce `NO_REPLY`.

### Device hygiene

Old RemoteClaw-managed Matrix devices can accumulate on the account and make encrypted-room trust harder to reason about.
List them with:

```bash
remoteclaw matrix devices list
```

Remove stale RemoteClaw-managed devices with:

```bash
remoteclaw matrix devices prune-stale
```

### Direct Room Repair

If direct-message state gets out of sync, RemoteClaw can end up with stale `m.direct` mappings that point at old solo rooms instead of the live DM. Inspect the current mapping for a peer with:

```bash
remoteclaw matrix direct inspect --user-id @alice:example.org
```

Repair it with:

```bash
remoteclaw matrix direct repair --user-id @alice:example.org
```
...
- Same-session DM user-target reuse only kicks in when the current session metadata proves the same DM peer on the same Matrix account; otherwise RemoteClaw falls back to normal user-scoped routing.
- When RemoteClaw sees a Matrix DM room collide with another DM room on the same shared Matrix DM session, it posts a one-time `m.notice` in that room with the `/focus` escape hatch when thread bindings are enabled and the `dm.sessionScope` hint.
...
- `threadBindings.spawnAcpSessions` is only required for `/acp spawn --thread auto|here`, where RemoteClaw needs to create or bind a child Matrix thread.
...
Ack reactions use the standard RemoteClaw resolution order:
...
- Matrix room history is pending-only: RemoteClaw buffers room messages that did not trigger a reply yet, then snapshots that window when a mention or other trigger arrives.
...
```bash
remoteclaw pairing list matrix
remoteclaw pairing approve matrix <CODE>
```

If an unapproved Matrix user keeps messaging you before approval, RemoteClaw reuses the same pending pairing code and may send a reminder reply again after a short cooldown instead of minting a new code.
...
Partial shared auth defaults do not create a separate implicit default account by themselves. RemoteClaw only synthesizes the top-level `default` account when that default has fresh auth (`homeserver` plus `accessToken`, or `homeserver` plus `userId` and `password`); named accounts can still stay discoverable from `homeserver` plus `userId` when cached credentials satisfy auth later.
...
Set `defaultAccount` when you want RemoteClaw to prefer one named Matrix account for implicit routing, probing, and CLI operations.
If you configure multiple named accounts, set `defaultAccount` or pass `--account <id>` for CLI commands that rely on implicit account selection.
Pass `--account <id>` to `remoteclaw matrix verify ...` and `remoteclaw matrix devices ...` when you want to override that implicit selection for one command.
...
RemoteClaw uses the same proxy setting for runtime Matrix traffic and account status probes.

## Target resolution

Matrix accepts these target forms anywhere RemoteClaw asks you for a room or user target:
...
- `allowBots`: allow messages from other configured RemoteClaw Matrix accounts (`true` or `"mentions"`).
