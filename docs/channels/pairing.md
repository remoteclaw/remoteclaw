...
read_when:
  ...
  - Reviewing RemoteClaw security posture
title: "Pairing"
---
...
Supported channels (any installed channel plugin that declares pairing; external plugins such as `remoteclaw-weixin` can add more): `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `signal`, `slack`, `sms`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.
...
3. On your phone, open the RemoteClaw app → **Settings** → **Gateway**.
4. Scan the QR code or paste the setup code, then connect.

Official RemoteClaw iOS and Android apps are approved automatically when their
...
3. On your phone, open the RemoteClaw iOS app → Settings → Gateway.
...
For `gateway.bind=lan` setup URLs, RemoteClaw detects persistent Tailscale Serve
...
<Note>
An already paired device does not get broader access silently. If it reconnects asking for more scopes or a broader role, RemoteClaw keeps the existing approval as-is and creates a fresh pending upgrade request. Use `remoteclaw devices list` to compare the currently approved access with the newly requested access before you approve.
</Note>
...
Stored under `~/.remoteclaw/devices/`:
...
- The legacy `node.pair.*` API (CLI: `remoteclaw nodes pending|approve|reject|remove|rename`) is a
