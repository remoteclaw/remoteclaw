#!/usr/bin/env node
/** ACP stdio server that bridges Agent Client Protocol clients to the RemoteClaw Gateway. */
...
export async function serveAcpGateway(opts: AcpServerOptions = {}): Promise<void> {
  ...
  // Connection auth gutted in RemoteClaw fork — use explicit opts directly.
  const creds = { token: opts.gatewayToken, password: opts.gatewayPassword };
...
function printHelp(): void {
  console.log(`Usage: remoteclaw acp [options]
...
if (isMainModule({ currentFile: fileURLToPath(import.meta.url) })) {
  ...
  if (argv.includes("--token") || argv.includes("--gateway-token")) {
    console.error(
      "Warning: --token can be exposed via process listings. Prefer --token-file or REMOTECLAW_GATEWAY_TOKEN.",
    );
  }
