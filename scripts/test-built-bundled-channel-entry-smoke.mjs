...
process.env.REMOTECLAW_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK ??= "1";

function parseArgs(argv) {
  let packageRoot = process.env.REMOTECLAW_BUNDLED_CHANNEL_SMOKE_ROOT;
...
const installedLayoutEnv = "REMOTECLAW_BUNDLED_CHANNEL_SMOKE_INSTALLED_LAYOUT";

function packageRootLooksInstalled(root) {
  return root.replaceAll("\\", "/").endsWith("/node_modules/remoteclaw");
}
...
function smokeInInstalledLayoutIfNeeded() {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-channel-entry-smoke-"));
  const nodeModulesRoot = path.join(tempRoot, "node_modules");
  const installedPackageRoot = path.join(nodeModulesRoot, "remoteclaw");
...
function collectBundledChannelEntryFiles() {
  ...
  for (const dirent of fs.readdirSync(distExtensionsRoot, { withFileTypes: true })) {
    ...
    if (!packageJson.remoteclaw?.channel) {
      continue;
    }

    const extensionEntries =
      Array.isArray(packageJson.remoteclaw.extensions) &&
      packageJson.remoteclaw.extensions.length > 0
    ...
    const setupEntry = packageJson.remoteclaw.setupEntry;
