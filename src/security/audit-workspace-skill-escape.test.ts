...
describe("security audit workspace skill path escape findings", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-security-audit-workspace-"));
  });
      ...
      !isWindows
        ? (async () => {
            ...
            const findings = await collectWorkspaceSkillSymlinkEscapeFindings({
              cfg: { agents: { defaults: { workspace: workspaceDir } } } satisfies RemoteClawConfig,
            });
