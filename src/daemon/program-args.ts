...
export const REMOTECLAW_WRAPPER_ENV_KEY = "REMOTECLAW_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/remoteclaw -> .pnpm/remoteclaw@X.Y.Z/...
...
function appendNodeModulesBinCandidates(
  ...
  // remoteclaw from node_modules/.bin points at the package root sibling.
...
export async function resolveRemoteClawWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${REMOTECLAW_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveRemoteClawWrapperPath(params.wrapperPath);
  if (wrapperPath) {
