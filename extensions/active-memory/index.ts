...
function resolvePersistentTranscriptBaseDir(api: RemoteClawPluginApi, agentId: string): string {
  return path.join(
...
function resolveToggleStatePath(api: RemoteClawPluginApi): string {
  return path.join(
    api.runtime.state.resolveStateDir(),
    "plugins",
    "active-memory",
...
function isActiveMemoryGloballyEnabled(cfg: RemoteClawConfig): boolean {
...
function resolveActiveMemoryPluginConfigFromConfig(cfg: RemoteClawConfig): unknown {
...
async function runRecallSubagent(params: {
  ...
  const tempDir = params.config.persistTranscripts
    ? undefined
    : await fs.mkdtemp(path.join(resolvePreferredRemoteClawTmpDir(), "remoteclaw-active-memory-"));
