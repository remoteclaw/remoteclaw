...
struct ChatSessionsSheet: View {
    ...
    @State private var scopedSessions: [RemoteClawChatSessionEntry] = []
    @State private var isLoadingScoped = false
    @State private var renameTarget: RemoteClawChatSessionEntry?
    ...
    private var displayedSessions: [RemoteClawChatSessionEntry] {
    ...
    var body: some View {
        NavigationStack {
            ...
            .onAppear {
                self.viewModel.refreshSessions(limit: RemoteClawChatViewModel.sessionListFetchLimit)
            }
            .alert(
                "Rename Session",
                isPresented: Binding(
    ...
    private func sessionRow(_ session: RemoteClawChatSessionEntry) -> some View {
        Button {
            ...
            HStack(spacing: 8) {
                VStack(alignment: .leading, spacing: 4) {
                    Text(session.displayName ?? session.key)
                        .font(RemoteClawChatTypography.mono(size: 17, relativeTo: .body))
                        .lineLimit(1)
        ...
        .swipeActions(edge: .trailing, allowsFullSwipe: false) {
            ...
            .tint(session.isArchived ? RemoteClawChatTheme.accent : RemoteClawChatTheme.danger)
        }
        .contextMenu {
            Button {
