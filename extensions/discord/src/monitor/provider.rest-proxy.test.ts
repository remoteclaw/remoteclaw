...
describe("resolveDiscordRestFetch", () => {
  ...
  function writeTempCa(contents: string): string {
    const dir = mkdtempSync(path.join(os.tmpdir(), "remoteclaw-discord-rest-proxy-ca-"));
    tempDirs.push(dir);
  ...
  it("uses debug proxy env when no discord proxy URL is configured", async () => {
    vi.stubEnv("REMOTECLAW_DEBUG_PROXY_ENABLED", "1");
    vi.stubEnv("REMOTECLAW_DEBUG_PROXY_URL", "http://127.0.0.1:7777");
    const runtime = {
      log: vi.fn(),
      error: vi.fn(),
      exit: vi.fn(),
