...
export const DEFAULT_E2E_BARE_IMAGE = "remoteclaw-docker-e2e-bare:local";
export const DEFAULT_E2E_FUNCTIONAL_IMAGE = "remoteclaw-docker-e2e-functional:local";
...
export function normalizeUpgradeSurvivorBaselineSpec(raw) {
  ...
  const spec = value.startsWith("remoteclaw@") ? value : `remoteclaw@${value}`;
  if (
    !/^remoteclaw@(?:alpha|beta|latest|[0-9]{4}\.[0-9]+\.[0-9]+(?:-(?:[0-9]+|alpha\.[0-9]+|beta\.[0-9]+))?)$/u.test(
      spec,
    )
  ) {
    throw new Error(
      `invalid published upgrade survivor baseline: ${JSON.stringify(
        value,
      )}. Expected remoteclaw@latest, remoteclaw@beta, remoteclaw@alpha, or remoteclaw@YYYY.M.PATCH.`,
    );
  }
  return spec;
}
...
function parsePublishedReleaseVersion(spec) {
  const match = /^remoteclaw@([0-9]{4})\.([0-9]+)\.([0-9]+)/u.exec(String(spec ?? ""));
  if (!match) {
    return null;
  }
  return {
      ...
      matrixScenarios
        .filter(
          (scenario) =>
            ...
            (scenario !== "acpx-remoteclaw-tools-bridge" ||
        ...
        .map((scenario) => {
          ...
          const commandPrefix = [
            `REMOTECLAW_UPGRADE_SURVIVOR_ARTIFACT_DIR="$PWD/.artifacts/upgrade-survivor/${name}"`,
            baselineSpec
              ? `REMOTECLAW_UPGRADE_SURVIVOR_BASELINE_SPEC=${shellQuote(baselineSpec)}`
              : "",
            scenario ? `REMOTECLAW_UPGRADE_SURVIVOR_SCENARIO=${shellQuote(scenario)}` : "",
          ]
            .filter(Boolean)
            .join(" ");
...
export function parseLiveMode(raw) {
  ...
  throw new Error(
    `REMOTECLAW_DOCKER_ALL_LIVE_MODE must be one of: all, skip, only. Got: ${JSON.stringify(raw)}`,
  );
}
...
export function parseProfile(raw) {
  ...
  throw new Error(
    `REMOTECLAW_DOCKER_ALL_PROFILE must be one of: ${DEFAULT_PROFILE}, ${RELEASE_PATH_PROFILE}. Got: ${JSON.stringify(raw)}`,
  );
}
...
export function lanesNeedRemoteClawPackage(poolLanes) {
...
export function findLaneByName(name) {
  return dedupeLanes(
    expandUpgradeSurvivorBaselineLanes(
      ...
      process.env.REMOTECLAW_UPGRADE_SURVIVOR_BASELINE_SPECS,
      process.env.REMOTECLAW_UPGRADE_SURVIVOR_SCENARIOS,
    ),
...
function buildPlanJson(params) {
  ...
  return {
    ...
    needs: {
      ...
      package: lanesNeedRemoteClawPackage(scheduledLanes),
    },
  ...
  const selectedLanes =
    options.selectedLaneNames.length > 0
      ? options.selectedLaneNames.flatMap((selectedName) => {
          ...
          selectNamedLanes(selectableLanes, [selectedName], "REMOTECLAW_DOCKER_ALL_LANES");
          return [];
        })
      : undefined;
