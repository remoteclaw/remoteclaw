...
async function createStagedNpmInstall(
  ...
  const prefix = await fs.mkdtemp(path.join(targetLayout.globalRoot, ".remoteclaw-update-stage-"));
...
async function replaceNpmBinShims(params: {
  ...
  const names = new Set([params.packageName, "remoteclaw"]);
  ...
  const backup: NpmBinShimBackup = {
    backupDir: await fs.mkdtemp(
      path.join(params.targetLayout.globalRoot, ".remoteclaw-shim-backup-"),
    ),
...
async function swapStagedNpmInstall(params: {
  ...
  const backupRoot = path.join(targetLayout.globalRoot, `.remoteclaw-${process.pid}-${Date.now()}`);
