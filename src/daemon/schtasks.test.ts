...
describe("resolveTaskScriptPath", () => {
  it.each([
    ...
    },
    {
      ...
      env: {
        USERPROFILE: "C:\\Users\\test",
        REMOTECLAW_TASK_SCRIPT_NAME: "gateway-node.cmd",
      },
      expected: path.join("C:\\Users\\test", ".remoteclaw", "gateway-node.cmd"),
    },
  ])("$name", ({ env, expected }) => {
  ...
  it.each([
    ...
    expect(() =>
      resolveTaskScriptPath({
        USERPROFILE: "C:\\Users\\test",
        REMOTECLAW_TASK_SCRIPT_NAME: scriptName,
      }),
    ).toThrow("REMOTECLAW_TASK_SCRIPT_NAME must be a file name only");
  });
});
...
describe("readScheduledTaskCommand", () => {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\RemoteClaw Share\\node.exe" "\\\\fileserver\\RemoteClaw Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\RemoteClaw Share\\node.exe",
            "\\\\fileserver\\RemoteClaw Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from REMOTECLAW_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ REMOTECLAW_STATE_DIR: path.join(tmpDir, "custom-state") }),
