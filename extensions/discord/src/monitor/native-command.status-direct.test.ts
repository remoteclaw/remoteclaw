...
vi.mock("remoteclaw/plugin-sdk/reply-dispatch-runtime", async () => {
  const actual = await vi.importActual<typeof import("remoteclaw/plugin-sdk/reply-dispatch-runtime")>(
    "remoteclaw/plugin-sdk/reply-dispatch-runtime",
  );
  return {
    ...actual,
...
vi.mock("remoteclaw/plugin-sdk/command-status-runtime", () => ({
...
function createConfig(params?: { requireMention?: boolean }): RemoteClawConfig {
  return {
    commands: {
...
describe("discord native /status", () => {
  ...
  beforeEach(() => {
    ...
    discordNativeCommandTesting.setDispatchReplyWithDispatcher(
      runtimeModuleMocks.dispatchReplyWithDispatcher as typeof import("remoteclaw/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithDispatcher,
    );
