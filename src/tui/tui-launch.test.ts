...
describe("launchTuiCli", () => {
  beforeEach(() => {
    ...
    process.argv[1] = "/repo/remoteclaw.mjs";
  ...
  it("passes local mode through to the relaunched TUI", async () => {
    ...
    const options = expectSpawned(["/repo/remoteclaw.mjs", "tui", "--local"]);
    expect(options.stdio).toBe("inherit");
  });
  ...
  it("pins the child gateway URL and config auth source through env without adding url argv", async () => {
    ...
    const options = expectSpawned(["/repo/remoteclaw.mjs", "tui"]);
    expect(options.env?.REMOTECLAW_GATEWAY_URL).toBe("ws://127.0.0.1:18789");
    expect(options.env?.REMOTECLAW_TUI_SETUP_AUTH_SOURCE).toBe("config");
  });
});
