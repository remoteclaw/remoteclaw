...
function writeBundledChannelPlugin(root: string, id: string, dependencies: Record<string, string>) {
  ...
  writeJson(path.join(root, "dist", "extensions", id, "remoteclaw.plugin.json"), {
    id,
    channels: [id],
...
describe("doctor bundled plugin runtime deps", () => {
  it("skips source checkouts", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-doctor-bundled-"));
  ...
  it("reports missing deps and conflicts", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-doctor-bundled-"));
    writeJson(path.join(root, "package.json"), { name: "remoteclaw" });
  ...
  it("reports default-enabled bundled plugin deps", () => {
    ...
    writeJson(path.join(root, "dist", "extensions", "openai", "remoteclaw.plugin.json"), {
      id: "openai",
