...
export function resolveTaskScriptPath(env: GatewayServiceEnv): string {
  const override = env.REMOTECLAW_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
...
async function writeTaskXmlTempFile(xml: string): Promise<string> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-task-xml-"));
...
function resolveConfiguredGatewayPort(env: GatewayServiceEnv): number | null {
  const raw = env.REMOTECLAW_GATEWAY_PORT?.trim();
  if (!raw) {
    return null;
  }
...
async function resolveScheduledTaskPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  return (
    ...
    parsePositivePort(command?.environment?.REMOTECLAW_GATEWAY_PORT) ??
...
async function activateScheduledTask(params: {
  ...
  const taskDescription = params.description ?? "RemoteClaw Gateway";
