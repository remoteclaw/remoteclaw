# RemoteClaw ACP Bridge

This document describes how the RemoteClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`remoteclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
RemoteClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing RemoteClaw Gateway
...
- ACP client spawns `remoteclaw acp` and speaks ACP messages over stdio.
...
`remoteclaw acp` resolves the Gateway URL and auth from CLI flags or config:
