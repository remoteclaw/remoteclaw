...
const DEFAULT_REPO = process.env.REMOTECLAW_RELEASE_REPO || "remoteclaw/remoteclaw";
const RELEASE_EVIDENCE_SCHEMA = "remoteclaw.release-validation-evidence/v3";
...
const CHILD_DISPATCHES = [
  ...
  },
  {
    ...
    name: "RemoteClaw Performance",
    parentJobName: "Run product performance evidence",
    suffix: "",
    trustedRef: "parent",
    workflow: "remoteclaw-performance.yml",
  },
];
...
function githubRestJson(pathSuffix, repository = DEFAULT_REPO) {
  const result = execFileSync(
    ...
    [
      ...
      [
        "set -euo pipefail",
        'token="$("$REMOTECLAW_PLAIN_GH_BIN" auth token)"',
    ...
    ],
    {
      ...
      env: {
        ...plainGhEnv(),
        REMOTECLAW_PLAIN_GH_BIN: resolvePlainGhBin(),
        REMOTECLAW_GITHUB_REST_URL: `https://api.github.com/repos/${repository}/${pathSuffix}`,
      },
  ...
  execFileSync(
    ...
    [
      ...
      [
        "set -euo pipefail",
        'token="$("$REMOTECLAW_PLAIN_GH_BIN" auth token)"',
        'curl -fsSL -H "Authorization: Bearer ${token}" -H "Accept: application/vnd.github+json" -H "X-GitHub-Api-Version: 2022-11-28" --output "$REMOTECLAW_GITHUB_ARTIFACT_DESTINATION" "$REMOTECLAW_GITHUB_ARTIFACT_URL"',
      ].join("\n"),
    ],
    {
      env: {
        ...plainGhEnv(),
        REMOTECLAW_GITHUB_ARTIFACT_DESTINATION: destination,
        REMOTECLAW_GITHUB_ARTIFACT_URL: `https://api.github.com/repos/${repository}/actions/artifacts/${artifactId}/zip`,
        REMOTECLAW_PLAIN_GH_BIN: resolvePlainGhBin(),
      },
...
function downloadParentManifestEvidence(runId, runAttempt, repository, manifestPath) {
  ...
  const downloadDir = mkdtempSync(join(tmpdir(), "remoteclaw-release-ci-summary-"));
  try {
