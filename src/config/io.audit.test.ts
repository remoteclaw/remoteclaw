...
function createRenameAuditRecord(home: string) {
  return finalizeConfigWriteAuditRecord({
    base: createAuditRecordBase(path.join(home, ".remoteclaw", "remoteclaw.json")),
    result: "rename",
    nextMetadata: {
      dev: "12",
      ino: "13",
...
describe("config io audit helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "remoteclaw-config-audit-" });
  ...
  it('ignores literal "undefined" home env values when choosing the audit log path', async () => {
    ...
    expect(auditPath).toBe(path.join(home, ".remoteclaw", "logs", "config-audit.jsonl"));
  ...
  it("formats overwrite warnings with hash transition and backup path", () => {
    expect(
      ...
      "Config overwrite: /tmp/remoteclaw.json (sha256 prev-hash -> next-hash, backup=/tmp/remoteclaw.json.bak, changedPaths=3)",
    );
  });
  ...
  it("drops next-file metadata and preserves error details for failed writes", () => {
    const base = createAuditRecordBase("/tmp/remoteclaw.json");
