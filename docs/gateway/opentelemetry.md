...
- `remoteclaw.talk.event` (counter, attrs: `remoteclaw.talk.event_type`, `remoteclaw.talk.mode`, `remoteclaw.talk.transport`, `remoteclaw.talk.brain`, `remoteclaw.talk.provider`)
- `remoteclaw.talk.event.duration_ms` (histogram, attrs: same as `remoteclaw.talk.event`; emitted when a Talk event reports duration)
- `remoteclaw.talk.audio.bytes` (histogram, attrs: same as `remoteclaw.talk.event`; emitted for Talk audio frame events that report byte length)
