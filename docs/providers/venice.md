...
```bash
remoteclaw onboard --auth-choice venice-api-key
```
...
```bash
remoteclaw agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, RemoteClaw shows all available Venice models. Pick based on your needs:
...
```bash
remoteclaw models set venice/kimi-k2-5
remoteclaw models set venice/claude-opus-4-6
```
...
RemoteClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
remoteclaw agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
remoteclaw agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
remoteclaw agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
remoteclaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
remoteclaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
