...
If `remoteclaw config validate` is already failing, start with `remoteclaw configure`
or `remoteclaw doctor --fix` first. `remoteclaw chat` does not bypass the invalid-
config guard.
...
4. Apply narrow changes with `remoteclaw config set` or `remoteclaw configure`, then rerun `!remoteclaw config validate`.
5. If Doctor recommends an automatic migration or repair, review it and run `!remoteclaw doctor --fix`.

Tips:

- Prefer `remoteclaw config set` or `remoteclaw configure` over hand-editing `remoteclaw.json`.
- `remoteclaw docs "<query>"` searches the live docs index from the same machine.
- `remoteclaw config validate --json` is useful when you want structured schema and SecretRef/resolvability errors.
...
- [Config](/cli/config) — inspect, validate, and edit `remoteclaw.json`
