...
<Info>
  The kernel is reached through the injected plugin runtime as `runtime.channel.turn.*`. The plugin runtime type is exported from `remoteclaw/plugin-sdk/core`, so third-party native plugins can use these entry points the same way bundled channel plugins do.
</Info>
...
Use `toInboundMediaFacts(...)` from `remoteclaw/plugin-sdk/channel-inbound` when
