...
bundled LAN discovery plugin for that RemoteClaw config:
...
4. If you deliberately enabled Bonjour in Docker with
   `REMOTECLAW_DISABLE_BONJOUR=0`, test multicast from the host:
...
- `remoteclaw plugins enable bonjour` restores the default LAN discovery plugin.
...
- When `REMOTECLAW_DISABLE_BONJOUR` is unset, Bonjour advertises on normal hosts and auto-disables inside detected containers.
