...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    remoteclaw onboard       # full onboarding flow
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `remoteclaw.json`.
    ...
    ```bash
    remoteclaw config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
