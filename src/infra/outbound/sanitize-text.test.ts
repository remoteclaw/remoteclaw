...
describe("stripInternalRuntimeScaffolding", () => {
  ...
  it("removes internal runtime context blocks", () => {
    expect(
      stripInternalRuntimeScaffolding(
        [
          "before",
          "<<<BEGIN_REMOTECLAW_INTERNAL_CONTEXT>>>",
          "internal metadata",
          "<<<BEGIN_UNTRUSTED_CHILD_RESULT>>>",
          "raw child output",
          "<<<END_UNTRUSTED_CHILD_RESULT>>>",
          "<<<END_REMOTECLAW_INTERNAL_CONTEXT>>>",
          "after",
        ].join("\n"),
      ),
    ).toBe("before\nafter");
  ...
  it("fails closed on unmatched runtime context delimiters", () => {
    expect(
      stripInternalRuntimeScaffolding(
        ["visible", "<<<BEGIN_REMOTECLAW_INTERNAL_CONTEXT>>>", "internal metadata"].join("\n"),
      ),
    ).toBe("visible");
  });
  ...
  it("preserves inline delimiter mentions", () => {
    expect(
      stripInternalRuntimeScaffolding(
        "visible <<<END_REMOTECLAW_INTERNAL_CONTEXT>>> inline mention",
      ),
    ).toBe("visible <<<END_REMOTECLAW_INTERNAL_CONTEXT>>> inline mention");
  ...
  it("removes stray standalone marker lines", () => {
    expect(
      stripInternalRuntimeScaffolding(
        ["visible", "<<<END_REMOTECLAW_INTERNAL_CONTEXT>>>", "after"].join("\n"),
      ),
    ).toBe("visible\nafter");
    expect(
      stripInternalRuntimeScaffolding(
