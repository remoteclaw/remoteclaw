...
describe("canvas documents", () => {
  it("builds entry urls for materialized path documents under managed storage", async () => {
    const stateDir = await mkdtemp(path.join(tmpdir(), "remoteclaw-canvas-documents-"));
    tempDirs.push(stateDir);
    const workspaceDir = await mkdtemp(
      path.join(tmpdir(), "remoteclaw-canvas-documents-workspace-"),
    );
    ...
    expect(document.entryUrl).toContain("/__remoteclaw__/canvas/documents/");
  ...
  it("normalizes nested local entrypoint urls", () => {
    ...
    expect(url).toBe("/__remoteclaw__/canvas/documents/cv_example/collection.media/index.html");
  });
  ...
  it("encodes special characters in hosted entrypoint path segments", () => {
    ...
    expect(url).toBe(
      "/__remoteclaw__/canvas/documents/cv_example/bundle%231/entry%2520point%3F.html",
    );
  });
  ...
  it("materializes inline html bundles as index documents", async () => {
    ...
    expect(document.entryUrl).toBe(`/__remoteclaw__/canvas/documents/${document.id}/index.html`);
  });
    ...
    expect(resolveCanvasDocumentAssets(document, { stateDir })).toEqual([
      {
        ...
        url: `/__remoteclaw__/canvas/documents/${document.id}/collection.media/audio.mp3`,
      },
    ]);
    expect(
    ...
    ).toEqual([
      {
        ...
        url: `http://127.0.0.1:19003/__remoteclaw__/canvas/documents/${document.id}/collection.media/audio.mp3`,
      },
    ]);
  });
  ...
  it("rejects traversal-style document ids in hosted canvas paths", async () => {
    ...
    expect(
      resolveCanvasHttpPathToLocalPath(
        "/__remoteclaw__/canvas/documents/../collection.media/index.html",
        { stateDir },
      ),
    ).toBeNull();
  });
