...
function resolveAgentRuntimeAcpDefaults(params: { cfg: RemoteClawConfig; ownerAgentId: string }): {
  ...
  if (!agent || (agent.runtime as any)?.type !== "acp") {
    return {};
  }
  return {
    acpAgentId: normalizeText((agent.runtime as any)?.acp?.agent),
    mode: normalizeText((agent.runtime as any)?.acp?.mode),
    cwd: normalizeText((agent.runtime as any)?.acp?.cwd),
    backend: normalizeText((agent.runtime as any)?.acp?.backend),
  };
}
