...
describe("extension-relay-auth SecretRef handling", () => {
  const ENV_KEYS = ["REMOTECLAW_GATEWAY_TOKEN", "CLAWDBOT_GATEWAY_TOKEN", "CUSTOM_GATEWAY_TOKEN"];
  ...
  it("resolves file-backed gateway.auth.token SecretRef", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-relay-file-secret-"));
