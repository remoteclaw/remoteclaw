...
`remoteclaw devices list` before approving.
...
- `remoteclaw nodes remove --node <id|name|ip>` removes a node pairing. For a
...
- For direct shell/runtime file executions, RemoteClaw also best-effort binds one concrete local
...
If the node retries with changed auth details, re-run `remoteclaw devices list`
