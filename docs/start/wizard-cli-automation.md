...
<AccordionGroup>
  ...
  <Accordion title="Custom provider example">
    ...
    RemoteClaw marks common vision model IDs as image-capable automatically. Add `--custom-image-input` for unknown custom vision IDs, or `--custom-text-input` to force text-only metadata.
