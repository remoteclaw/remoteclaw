...
```text
refs/remoteclaw/mobile-releases/android/<versionName>-<versionCode>
```

Example:

```text
refs/remoteclaw/mobile-releases/android/2026.6.10-2026061008
```
...
core RemoteClaw release machinery.
...
If `MATCH_PASSWORD` is not set, the existing manual Gradle-property signing path still works: provide `REMOTECLAW_ANDROID_STORE_FILE`, `REMOTECLAW_ANDROID_STORE_PASSWORD`, `REMOTECLAW_ANDROID_KEY_ALIAS`, and `REMOTECLAW_ANDROID_KEY_PASSWORD` through your local Gradle user properties before running release tasks.
