...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for REMOTECLAW_STATE_DIR.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // REMOTECLAW_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
...
function shouldRequireOAuthDir(cfg: RemoteClawConfig, env: NodeJS.ProcessEnv): boolean {
  ...
  if (asNullableObjectRecord(channels.whatsapp)) {
    return true;
  }
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        "- Prefer a local non-synced state dir (for example: ~/.remoteclaw).",
        `  Set locally: REMOTECLAW_STATE_DIR=~/.remoteclaw ${formatCliCommand("remoteclaw doctor")}`,
      ].join("\n"),
    );
  }
    ...
    if (missing.length > 0) {
      warnings.push(
        [
          ...
          `  Prune missing entries: ${formatCliCommand(`remoteclaw sessions cleanup --store "${absoluteStorePath}" --enforce --fix-missing`)}`,
        ].join("\n"),
      );
    }
