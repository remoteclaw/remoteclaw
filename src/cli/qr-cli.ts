...
function readGatewayTokenEnv(env: NodeJS.ProcessEnv): string | undefined {
  const primary =
    typeof env.REMOTECLAW_GATEWAY_TOKEN === "string" ? env.REMOTECLAW_GATEWAY_TOKEN : "";
...
function readGatewayPasswordEnv(env: NodeJS.ProcessEnv): string | undefined {
  const primary =
    typeof env.REMOTECLAW_GATEWAY_PASSWORD === "string" ? env.REMOTECLAW_GATEWAY_PASSWORD : "";
  if (primary.trim().length > 0) {
    return primary.trim();
  }
  const legacy =
    ...
    .action(async (opts: QrCliOptions) => {
      try {
        ...
        const lines: string[] = [
          ...
          "Scan this with the RemoteClaw iOS app (Onboarding -> Scan QR).",
          "",
        ];
        ...
        lines.push(
          ...
          `  ${theme.command("remoteclaw devices list")}`,
          `  ${theme.command("remoteclaw devices approve <requestId>")}`,
        );
