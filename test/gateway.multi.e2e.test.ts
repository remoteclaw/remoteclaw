...
describe("gateway multi-instance e2e", () => {
  ...
  it("spins up two gateways and exercises WS + HTTP + node pairing", { timeout: E2E_TIMEOUT_MS }, async () => {
    ...
    const [hookResA, hookResB] = await Promise.all([
      postJson(
        ...
        { "x-remoteclaw-token": gwA.hookToken },
      ),
      postJson(
        ...
        { "x-remoteclaw-token": gwB.hookToken },
      ),
    ]);
    ...
    const [nodeA, nodeB] = await Promise.all([connectNode(gwA, "node-a"), connectNode(gwB, "node-b")]);
    nodeClients.push(nodeA.client, nodeB.client);

    await Promise.all([waitForNodeStatus(gwA, nodeA.nodeId), waitForNodeStatus(gwB, nodeB.nodeId)]);
  });

  it("delivers final chat event for telegram-shaped session keys", { timeout: E2E_TIMEOUT_MS }, async () => {
