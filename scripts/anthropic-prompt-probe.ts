...
const ENABLE_CAPTURE = process.env.REMOTECLAW_PROMPT_CAPTURE === "1";
const INCLUDE_RAW = process.env.REMOTECLAW_PROMPT_INCLUDE_RAW === "1";
const CLAUDE_BIN = process.env.CLAUDE_BIN?.trim() || "claude";
const NODE_BIN = process.env.REMOTECLAW_NODE_BIN?.trim() || process.execPath;
const TIMEOUT_MS = Number(process.env.REMOTECLAW_PROMPT_TIMEOUT_MS ?? "45000");
const GATEWAY_TIMEOUT_MS = Number(process.env.REMOTECLAW_PROMPT_GATEWAY_TIMEOUT_MS ?? "120000");
...
async function runDirectPrompt(prompt: string): Promise<PromptResult> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-direct-prompt-probe-"));
...
async function runGatewayPrompt(prompt: string): Promise<PromptResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-gateway-prompt-probe-"));
