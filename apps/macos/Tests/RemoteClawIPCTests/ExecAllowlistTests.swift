...
struct ExecAllowlistTests {
    ...
    @Test func `resolve for allowlist splits shell chains`() {
        let command = ["/bin/sh", "-c", "echo allowlisted && /usr/bin/touch /tmp/remoteclaw-allowlist-test"]
    ...
    @Test func `resolve for allowlist uses wrapper argv payload even with canonical raw command`() {
        let command = ["/bin/sh", "-c", "echo allowlisted && /usr/bin/touch /tmp/remoteclaw-allowlist-test"]
        let canonicalRaw = "/bin/sh -c \"echo allowlisted && /usr/bin/touch /tmp/remoteclaw-allowlist-test\""
        let resolutions = ExecCommandResolution.resolveForAllowlist(
            command: command,
    ...
    @Test func `resolve for allowlist fails closed on command substitution`() {
        let command = ["/bin/sh", "-c", "echo $(/usr/bin/touch /tmp/remoteclaw-allowlist-test-subst)"]
        let resolutions = ExecCommandResolution.resolveForAllowlist(
            command: command,
    ...
    @Test func `resolve for allowlist fails closed on quoted command substitution`() {
        let command = ["/bin/sh", "-c", "echo \"ok $(/usr/bin/touch /tmp/remoteclaw-allowlist-test-quoted-subst)\""]
        let resolutions = ExecCommandResolution.resolveForAllowlist(
            command: command,
    ...
    @Test func `resolve for allowlist fails closed on line-continued command substitution`() {
        let command = ["/bin/sh", "-c", "echo $\\\n(/usr/bin/touch /tmp/remoteclaw-allowlist-test-line-cont-subst)"]
        let resolutions = ExecCommandResolution.resolveForAllowlist(
            command: command,
            rawCommand: "echo $\\\n(/usr/bin/touch /tmp/remoteclaw-allowlist-test-line-cont-subst)",
            cwd: nil,
    ...
    @Test func `resolve for allowlist fails closed on chained line-continued command substitution`() {
        let command = [
            "/bin/sh",
            "-c",
            "echo ok && $\\\n(/usr/bin/touch /tmp/remoteclaw-allowlist-test-chained-line-cont-subst)",
        ]
        let resolutions = ExecCommandResolution.resolveForAllowlist(
            command: command,
            rawCommand: "echo ok && $\\\n(/usr/bin/touch /tmp/remoteclaw-allowlist-test-chained-line-cont-subst)",
    ...
    @Test func `resolve for allowlist unwraps env dispatch wrappers inside shell segments`() {
        let command = ["/bin/sh", "-c", "env /usr/bin/touch /tmp/remoteclaw-allowlist-test"]
        let resolutions = ExecCommandResolution.resolveForAllowlist(
            command: command,
            rawCommand: "env /usr/bin/touch /tmp/remoteclaw-allowlist-test",
            cwd: nil,
            env: ["PATH": "/usr/bin:/bin"])
        #expect(resolutions.count == 1)
    ...
    @Test func `resolve for allowlist preserves env assignments inside shell segments`() {
        let command = ["/bin/sh", "-c", "env FOO=bar /usr/bin/touch /tmp/remoteclaw-allowlist-test"]
        let resolutions = ExecCommandResolution.resolveForAllowlist(
            command: command,
            rawCommand: "env FOO=bar /usr/bin/touch /tmp/remoteclaw-allowlist-test",
            cwd: nil,
            env: ["PATH": "/usr/bin:/bin"])
        #expect(resolutions.count == 1)
