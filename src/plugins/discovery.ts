...
function resolveDiscoveryCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.REMOTECLAW_PLUGIN_DISCOVERY_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseDiscoveryCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.REMOTECLAW_DISABLE_PLUGIN_DISCOVERY_CACHE?.trim();
  if (disabled) {
    return false;
  }
...
function readPackageManifest(dir: string, _rejectHardlinks = true): PackageManifest | null {
...
export function discoverRemoteClawPlugins(params: {
  ...
  if (workspaceDir) {
    ...
    const workspaceExtDirs = [path.join(workspaceRoot, ".remoteclaw", "extensions")];
