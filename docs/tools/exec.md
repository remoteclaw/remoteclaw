...
- No-approval host exec is the default for gateway + node. If you want approvals/allowlist behavior, tighten both `tools.exec.*` and the host `~/.remoteclaw/exec-approvals.json`; see [Exec approvals](/tools/exec-approvals#no-approval-yolo-mode).
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  RemoteClaw prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`remoteclaw security audit` and `remoteclaw doctor` also warn when you explicitly add broad-behavior bins such as `jq` back into `safeBins`.
