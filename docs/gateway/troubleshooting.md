...
- `remoteclaw channels status --probe` shows live per-account transport status and,
...
- direct tiny requests succeed, but RemoteClaw agent runs fail with backend/model
  crashes (for example Gemma on some `inferrs` builds) → RemoteClaw transport is
...
2. Set `compat.supportsTools: false` for models/backends that cannot handle
   RemoteClaw's tool schema surface reliably.
3. Lower prompt pressure where possible: smaller workspace bootstrap, shorter
   ...
   support.
4. If tiny direct requests keep passing while RemoteClaw agent turns still crash
...
- `remoteclaw devices rotate --scope ...` can only request operator scopes that
...
- If all validation issues were under `plugins.entries.<id>...`, RemoteClaw would
