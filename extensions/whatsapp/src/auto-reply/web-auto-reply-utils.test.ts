...
describe("isBotMentionedFromTargets", () => {
  const mentionCfg = { mentionRegexes: [/\bremoteclaw\b/i] };
  ...
  it("ignores JID mentions in self-chat mode", () => {
    const cfg = { mentionRegexes: [/\bremoteclaw\b/i], allowFrom: ["+999"] };
    const msg = makeMsg({
      body: "@owner ping",
  ...
  it("honors explicit self-chat overrides without recomputing from allowFrom", () => {
    const cfg = {
      mentionRegexes: [/\bremoteclaw\b/i],
...
describe("web auto-reply util", () => {
  describe("mentions diagnostics", () => {
    it("returns normalized debug fields and mention outcome", () => {
      ...
      const result = debugMention(msg, { mentionRegexes: [/\bremoteclaw\b/i] });
      expect(result.wasMentioned).toBe(true);
      expect(result.details.bodyClean).toBe("remoteclaw ping");
