    ...
    isMessageToolMirror: Bool = false) throws -> AnyCodable
{
    var object: [String: Any] = [
        ...
        "timestamp": timestamp,
        "__remoteclaw": ["id": serverId],
    ]
    if isMessageToolMirror {
        object["remoteclawMessageToolMirror"] = ["toolName": "message"]
    }
...
private final class MockWatchMessagingService: @preconcurrency WatchMessagingServicing, @unchecked Sendable {
    ...
    var sendError: Error?
    var lastSent: (id: String, params: RemoteClawWatchNotifyParams, gatewayStableID: String?)?
    ...
    var sentExecApprovalSnapshots: [RemoteClawWatchExecApprovalSnapshotMessage] = []
    var lastSentAppSnapshot: RemoteClawWatchAppSnapshotMessage?
    var syncExecApprovalSnapshotHandler: ((RemoteClawWatchExecApprovalSnapshotMessage) async throws
        -> WatchNotificationSendResult)?
    var lastSentChatCompletion: RemoteClawWatchChatCompletionMessage?
    ...
    func syncAppSnapshot(
        _ message: RemoteClawWatchAppSnapshotMessage) async throws -> WatchNotificationSendResult
    {
    ...
    func sendChatCompletion(
        _ message: RemoteClawWatchChatCompletionMessage) async throws -> WatchNotificationSendResult
    {
...
@Suite(.serialized) struct NodeAppModelInvokeTests {
    ...
    @Test @MainActor func `failed PTT start restores voice wake suspension`() async {
        ...
        let request = BridgeInvokeRequest(
            id: "ptt-start",
            command: RemoteClawTalkCommand.pttStart.rawValue)
    ...
    @Test @MainActor func `PTT start preserves an active voice note`() async {
        ...
        let recorder = RemoteClawVoiceNoteRecorder(capture: capture)
    ...
    @Test @MainActor func `watch app snapshot uses configured agent avatar`() async throws {
        ...
        appModel.gatewayAgents = [
            AgentSummary(
                ...
                name: "Main",
                identity: [
                    "avatarUrl": AnyCodable("https://example.com/remoteclaw.png"),
                    "emoji": AnyCodable("OC"),
                ],
                workspace: nil,
                workspacegit: nil,
        ...
        let snapshot = try #require(watchService.lastSentAppSnapshot)
        #expect(snapshot.agentAvatarURL == "https://example.com/remoteclaw.png")
    ...
    @Test @MainActor func `watch app commands reject stale gateway targets`() async {
        ...
        for command in [RemoteClawWatchAppCommand.openChat, .startTalk] {
            watchService.emitAppCommand(
                WatchAppCommandEvent(
    ...
    @Test func `watch chat completion bounds reply text`() {
        let message = RemoteClawWatchChatCompletionMessage(
    ...
    @Test @MainActor func `system notify returns unavailable when notifications off`() async throws {
        ...
        center.status = .notDetermined
        let appModel = NodeAppModel(notificationCenter: center)
        let params = RemoteClawSystemNotifyParams(title: "Approval", body: "Review request")
        let paramsData = try JSONEncoder().encode(params)
        let req = BridgeInvokeRequest(
            id: "notify-off",
            command: RemoteClawSystemCommand.notify.rawValue,
    ...
    @Test @MainActor func `chat push without speech returns unavailable when notifications off`() async throws {
        ...
        center.status = .notDetermined
        let appModel = NodeAppModel(notificationCenter: center)
        let params = RemoteClawChatPushParams(text: "Build finished", speak: false)
        let paramsData = try JSONEncoder().encode(params)
        let req = BridgeInvokeRequest(
            id: "chat-push-off",
            command: RemoteClawChatCommand.push.rawValue,
            paramsJSON: String(decoding: paramsData, as: UTF8.self))
    ...
    @Test @MainActor func `watch reply codec preserves prompt gateway owner`() throws {
        ...
        let reply = try #require(WatchMessagingPayloadCodec.parseQuickReplyPayload([
            "type": RemoteClawWatchPayloadType.reply.rawValue,
            "replyId": "reply-a",
            "promptId": "prompt-a",
            "actionId": "approve",
            "gatewayStableID": "gateway-a",
    ...
    @Test @MainActor func `watch exec approval codec preserves gateway owner`() throws {
        let approval = RemoteClawWatchExecApprovalItem(
            id: "approval-a",
            gatewayStableID: "gateway-a",
        ...
        let prompt = WatchMessagingPayloadCodec.encodeExecApprovalPromptPayload(
            RemoteClawWatchExecApprovalPromptMessage(approval: approval))
        ...
        let reply = try #require(WatchMessagingPayloadCodec.parseExecApprovalResolvePayload([
            "type": RemoteClawWatchPayloadType.execApprovalResolve.rawValue,
            "replyId": "reply-a",
            "approvalId": "approval-a",
            "gatewayStableID": "gateway-a",
            "decision": RemoteClawWatchExecApprovalDecision.allowOnce.rawValue,
        ], transport: "sendMessage"))
        #expect(reply.gatewayStableID == "gateway-a")

        let resolved = WatchMessagingPayloadCodec.encodeExecApprovalResolvedPayload(
            RemoteClawWatchExecApprovalResolvedMessage(
                approvalId: "approval-a",
                gatewayStableID: "gateway-a"))
        let expired = WatchMessagingPayloadCodec.encodeExecApprovalExpiredPayload(
            RemoteClawWatchExecApprovalExpiredMessage(
                approvalId: "approval-a",
                gatewayStableID: "gateway-a",
                reason: .notFound))
    ...
    @Test @MainActor func `watch application context retains app and approval snapshots`() throws {
        let appPayload = WatchMessagingPayloadCodec.encodeAppSnapshotPayload(
            RemoteClawWatchAppSnapshotMessage(
        ...
        let approvalPayload = WatchMessagingPayloadCodec.encodeExecApprovalSnapshotPayload(
            RemoteClawWatchExecApprovalSnapshotMessage(
                approvals: [
                    RemoteClawWatchExecApprovalItem(
                        id: "approval-a",
                        gatewayStableID: "gateway-a",
                        commandText: "echo safe",
                        allowedDecisions: [.allowOnce, .deny]),
        ...
        #expect(combined["type"] as? String == RemoteClawWatchPayloadType.execApprovalSnapshot.rawValue)
        let nestedApp = try #require(
            combined[RemoteClawWatchPayloadType.appSnapshot.rawValue] as? [String: Any])
        let nestedApprovals = try #require(
            combined[RemoteClawWatchPayloadType.execApprovalSnapshot.rawValue] as? [String: Any])
    ...
    @Test @MainActor func `operator scopes use the active gateway token`() throws {
        ...
        let previousStateDir = ProcessInfo.processInfo.environment["REMOTECLAW_STATE_DIR"]
        setenv("REMOTECLAW_STATE_DIR", tempDir.path, 1)
        defer {
            if let previousStateDir {
                setenv("REMOTECLAW_STATE_DIR", previousStateDir, 1)
            } else {
                unsetenv("REMOTECLAW_STATE_DIR")
            }
