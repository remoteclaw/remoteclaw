...
function githubWorkflowRef(env = process.env) {
  const explicit = env.REMOTECLAW_DOCKER_E2E_WORKFLOW_REF;
  if (explicit) {
    return explicit;
  }
...
export function githubWorkflowRerunCommand(laneNames, ref, env = process.env) {
  ...
  const releasePath = env.REMOTECLAW_DOCKER_ALL_PROFILE === RELEASE_PATH_PROFILE;
  const allowUnreleasedChangelog = env.REMOTECLAW_DOCKER_E2E_ALLOW_UNRELEASED_CHANGELOG === "true";
  const fields = [
    "gh workflow run",
    shellQuote(env.REMOTECLAW_DOCKER_E2E_WORKFLOW || DEFAULT_GITHUB_WORKFLOW),
  ...
  if (env.GITHUB_RUN_ID) {
    ...
    fields.push(
      ...
      `package_artifact_name=${shellQuote(
        env.REMOTECLAW_DOCKER_E2E_PACKAGE_ARTIFACT_NAME || "docker-e2e-package",
      )}`,
    );
  }
  if (env.REMOTECLAW_UPGRADE_SURVIVOR_BASELINE_SPEC) {
    fields.push(
      "-f",
      `published_upgrade_survivor_baseline=${shellQuote(env.REMOTECLAW_UPGRADE_SURVIVOR_BASELINE_SPEC)}`,
    );
  }
  if (env.REMOTECLAW_UPGRADE_SURVIVOR_BASELINE_SPECS) {
    fields.push(
      "-f",
      `published_upgrade_survivor_baselines=${shellQuote(env.REMOTECLAW_UPGRADE_SURVIVOR_BASELINE_SPECS)}`,
    );
  }
  if (env.REMOTECLAW_UPGRADE_SURVIVOR_SCENARIOS) {
    fields.push(
      "-f",
      `published_upgrade_survivor_scenarios=${shellQuote(env.REMOTECLAW_UPGRADE_SURVIVOR_SCENARIOS)}`,
    );
  }
  if (env.REMOTECLAW_DOCKER_E2E_BARE_IMAGE) {
    fields.push("-f", `docker_e2e_bare_image=${shellQuote(env.REMOTECLAW_DOCKER_E2E_BARE_IMAGE)}`);
  }
  if (env.REMOTECLAW_DOCKER_E2E_FUNCTIONAL_IMAGE) {
    fields.push(
      "-f",
      `docker_e2e_functional_image=${shellQuote(env.REMOTECLAW_DOCKER_E2E_FUNCTIONAL_IMAGE)}`,
    );
  }
  return fields.join(" ");
}
...
function liveDockerHarnessScriptCommand(script) {
  return `bash -c 'harness="\${REMOTECLAW_DOCKER_E2E_TRUSTED_HARNESS_DIR:-${LIVE_DOCKER_DEFAULT_HARNESS_DIR}}"; REMOTECLAW_LIVE_DOCKER_REPO_ROOT="\${REMOTECLAW_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/${script}"'`;
}
...
export async function writeRunSummary(logDir, summary, env = process.env) {
  ...
  const payload = {
    ...summary,
    packageArtifactName: env.REMOTECLAW_DOCKER_E2E_PACKAGE_ARTIFACT_NAME || undefined,
    ...
    github: {
      ...
      runUrl:
        env.GITHUB_SERVER_URL && env.GITHUB_REPOSITORY && env.GITHUB_RUN_ID
          ...
          : undefined,
      selectedSha: env.REMOTECLAW_DOCKER_E2E_SELECTED_SHA || undefined,
...
async function writeFailureIndex(logDir, summary, env) {
  const ref =
    ...
    env.REMOTECLAW_DOCKER_E2E_SELECTED_SHA ||
    summary.github?.sha ||
    summary.github?.ref ||
    env.GITHUB_SHA ||
    "HEAD";
  ...
  const failureIndex = {
    ...
    version: 1,
    workflow: env.REMOTECLAW_DOCKER_E2E_WORKFLOW || DEFAULT_GITHUB_WORKFLOW,
  };
...
async function prepareRemoteClawPackage(baseEnv, logDir) {
  ...
  const packDir = path.join(logDir, "remoteclaw-package");
  await mkdir(packDir, { recursive: true });
  const packageTgz = path.join(packDir, "remoteclaw-current.tgz");
  await runForeground(
    "Prepare RemoteClaw package once",
    `node scripts/package-remoteclaw-for-docker.mjs --allow-unreleased-changelog --output-dir ${shellQuote(packDir)} --output-name remoteclaw-current.tgz`,
    baseEnv,
  );
  ...
  baseEnv.REMOTECLAW_DOCKER_E2E_ALLOW_UNRELEASED_CHANGELOG = "true";
  baseEnv.REMOTECLAW_CURRENT_PACKAGE_TGZ = packageTgz;
  baseEnv.REMOTECLAW_BUNDLED_CHANNEL_HOST_BUILD = "0";
  baseEnv.REMOTECLAW_NPM_ONBOARD_HOST_BUILD = "0";
...
async function main() {
  ...
  const releaseProfile = normalizeReleaseProfile(
    process.env.REMOTECLAW_DOCKER_ALL_RELEASE_PROFILE || process.env.REMOTECLAW_RELEASE_PROFILE,
  );
  const releaseChunk =
