...
handles the RemoteClaw turn. Plugin calls stay in the native Codex transcript;
Codex app-server owns app-backed MCP execution. RemoteClaw does not translate
Codex plugins into synthetic `codex_plugin_*` RemoteClaw dynamic tools.
...
`codexPlugins` has no effect on RemoteClaw-provider runs, ACP conversation
...
`enable`/`disable` write only to `~/.remoteclaw/remoteclaw.json`; they never edit
...
`auth_required`, reauthorize the app in Codex before enabling it in RemoteClaw.
...
| Enabled    | RemoteClaw config allows the plugin for Codex harness turns.                                                                     |
...
- During planning, RemoteClaw reads source Codex `plugin/read` details and
...
RemoteClaw reads Codex app inventory through app-server `app/list`, caches it
...
drops it, and RemoteClaw rebuilds it from the next `app/list` read.
...
get `open_world_enabled: true`. RemoteClaw does not expose a separate
...
- Per-plugin `allow_destructive_actions` overrides the global policy for
  that plugin.
- `false`: RemoteClaw returns a deterministic decline.
- `true`: RemoteClaw auto-accepts only safe schemas it can map to an approval
  response, such as a boolean approve field.
- `"auto"`: RemoteClaw exposes destructive plugin actions to Codex, then
  turns ownership-proven MCP approval elicitations into RemoteClaw plugin
  approvals before returning the Codex approval response.
- `"ask"`: RemoteClaw uses the same Codex write/destructive gating as
  ...
  admitted app using `"ask"`, RemoteClaw selects Codex's human approvals
  reviewer for that app so Codex sends its approval elicitations to
  RemoteClaw; other apps and non-app thread approvals keep their configured
  reviewer and policy.
...
| Code                                              | Meaning                                                                                                                              | Fix                                                                                                                     |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------- |
| `auth_required`                                   | Migration installed the plugin, but one of its apps still needs authentication. The entry is written disabled until you reauthorize. | Reauthorize the app in Codex, then enable the plugin in RemoteClaw.                                                     |
...
| `app_inventory_missing`, `app_inventory_stale`    | App readiness came from an empty or stale cache.                                                                                     | RemoteClaw schedules an async refresh automatically; plugin apps stay excluded until ownership and readiness are known. |
