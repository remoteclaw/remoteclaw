...
describe("dreaming controller", () => {
  ...
  it("patches config to update global dreaming enablement", async () => {
    ...
    state.configSnapshot = {
      ...
      config: {
        plugins: {
          slots: {
            memory: "memos-local-remoteclaw-plugin",
          },
          entries: {
            "memos-local-remoteclaw-plugin": {
              config: {
                dreaming: {
                  enabled: true,
                },
  ...
  it("reads dreaming enabled state from the selected memory slot plugin", () => {
    expect(
      ...
      pluginId: "memos-local-remoteclaw-plugin",
      enabled: true,
    });
  });
  ...
  it("repairs dreaming artifacts and reloads only dreaming status", async () => {
    ...
    request.mockImplementation(async (method: string) => {
      if (method === "doctor.memory.repairDreamingArtifacts") {
        return {
          ...
          changed: true,
          archiveDir: "/tmp/remoteclaw/.remoteclaw-repair/dreaming/2026-04-11T22-10-00-000Z",
    ...
    expect(state.dreamDiaryActionMessage).toEqual({
      kind: "success",
      text: "Dream cache repair complete: archived session corpus, archived ingestion state. Archive: /tmp/remoteclaw/.remoteclaw-repair/dreaming/2026-04-11T22-10-00-000Z",
    });
    expect(state.dreamDiaryActionArchivePath).toBe(
      "/tmp/remoteclaw/.remoteclaw-repair/dreaming/2026-04-11T22-10-00-000Z",
    );
    expect(state.dreamDiaryActionLoading).toBe(false);
  });
