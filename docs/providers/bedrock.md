...
RemoteClaw can use **Amazon Bedrock** models via pi‑ai’s **Bedrock Converse**
...
If AWS credentials are detected, RemoteClaw can automatically discover Bedrock
...
When running RemoteClaw on an EC2 instance with an IAM role attached, the AWS SDK
will automatically use the instance metadata service (IMDS) for authentication.
However, RemoteClaw's credential detection currently only checks for environment
...
remoteclaw config set models.bedrockDiscovery.enabled true
remoteclaw config set models.bedrockDiscovery.region us-east-1
...
- RemoteClaw surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,
