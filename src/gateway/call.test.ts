...
describe("callGateway url resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot.restore();
    delete process.env.REMOTECLAW_ALLOW_INSECURE_PRIVATE_WS;
  ...
  it("allows ws:// hostname remote URLs when REMOTECLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    ...
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
        bind: "loopback",
        remote: { url: "ws://remoteclaw-gateway.org:18789" },
      },
    });
    resolveGatewayPort.mockReturnValue(18789);
    ...
    expect(details.url).toBe("ws://remoteclaw-gateway.org:18789");
    expect(details.urlSource).toBe("config gateway.remote.url");
  });
