...
struct RootTabsSourceGuardTests {
    ...
    @Test func `iOS 26 chrome uses native glass while content cards stay quiet`() throws {
        let rootSource = try String(contentsOf: Self.rootTabsSourceURL(), encoding: .utf8)
        let appSource = try String(contentsOf: Self.remoteClawAppSourceURL(), encoding: .utf8)
        ...
        #expect(rootSource.contains(".remoteClawTabBarBehavior()"))
        ...
        #expect(componentsSource.contains("struct RemoteClawNoticeBanner: View"))
    ...
    @Test func `professional layout avoids nested pills and card stacks`() throws {
        ...
        let gatewayStatus = try Self.extract(
            componentsSource,
            from: "struct RemoteClawGatewayCompactPill: View",
        ...
        #expect(gatewayStatus.contains("RemoteClawStatusBadge(label: self.title, tone: self.tone)"))
        ...
        #expect(settingsList.contains(".font(RemoteClawType.captionSemiBold)"))
    ...
    @Test func `settings about page shows concise public device details`() throws {
        ...
        #expect(aboutDestination.contains("SettingsDetailRow(\"RemoteClaw app version\""))
    ...
    @Test func `routed headers use shared adaptive layout`() throws {
        ...
        #expect(chatSource.contains("RemoteClawSidebarRevealButton(action: headerLeadingAction)"))
    ...
    @Test func `routed feature chrome keeps gateway pill actionable`() throws {
        ...
        #expect(chromeSource.contains(".remoteClawGlassButton()"))
    ...
    @Test func `routed gateway pills open gateway settings`() throws {
        ...
        #expect(docsSource.contains(".remoteClawGlassButton()"))
    ...
    private static func remoteClawAppSourceURL() -> URL {
        URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .appendingPathComponent("Sources/RemoteClawApp.swift")
    }
