---
summary: "Adds Volcengine, Volcengine Plan model provider support to RemoteClaw."
read_when:
  - You are installing, configuring, or auditing the volcengine plugin
title: "Volcengine plugin"
---

# Volcengine plugin

Adds Volcengine, Volcengine Plan model provider support to RemoteClaw.

## Distribution

- Package: `@remoteclaw/volcengine-provider`
- Install route: included in RemoteClaw

## Surface

providers: volcengine, volcengine-plan; contracts: speechProviders

## Related docs

- [volcengine](/providers/volcengine)
