...
The Gateway is RemoteClaw's WebSocket server (channels, nodes, sessions, hooks). Subcommands in this page live under `remoteclaw gateway …`.
...
<CardGroup cols={3}>
  ...
  <Card title="Discovery overview" href="/gateway/discovery">
    How RemoteClaw advertises and finds gateways.
  </Card>
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    - By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.remoteclaw/remoteclaw.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
    - `remoteclaw onboard --mode local` and `remoteclaw setup` are expected to write `gateway.mode=local`. If the file exists but `gateway.mode` is missing, treat that as a broken or clobbered config and repair it instead of assuming local mode implicitly.
...
<ParamField path="--token <token>" type="string">
  Token override (also sets `REMOTECLAW_GATEWAY_TOKEN` for the process).
</ParamField>
...
<ParamField path="--bind custom + gateway.customBindHost" type="string">
  Expects an IPv4 address today. For IPv6-only BYOH, place an IPv4 sidecar or proxy in front of the Gateway and point RemoteClaw at that IPv4 endpoint.
</ParamField>
...
remoteclaw gateway restart --safe
remoteclaw gateway restart --safe --skip-deferral
remoteclaw gateway restart --force
```

`remoteclaw gateway restart --safe` asks the running Gateway to preflight active RemoteClaw work before restarting. If queued operations, reply delivery, embedded runs, or task runs are active, the Gateway reports the blockers, coalesces duplicate safe restart requests, and restarts once the active work drains. Plain `restart` keeps the existing service-manager behavior for compatibility. Use `--force` only when you explicitly want the immediate override path.

`remoteclaw gateway restart --safe --skip-deferral` runs the same RemoteClaw-aware coordinated restart as `--safe`, but bypasses the active-work deferral gate so the Gateway emits the restart immediately even when blockers are reported. Use it as the operator escape hatch when a deferral has been pinned by a stuck task run and `--safe` alone would wait indefinitely. `--skip-deferral` requires `--safe`.
...
- Set `REMOTECLAW_GATEWAY_STARTUP_TRACE=1` to log phase timings during Gateway startup, including per-phase `eventLoopMax` delay and plugin lookup-table timings for installed-index, manifest registry, startup planning, and owner-map work.
- Set `REMOTECLAW_GATEWAY_RESTART_TRACE=1` to log restart-scoped `restart trace:` lines for restart signal handling, active-work drain, shutdown phases, next start, ready timing, and memory metrics.
- Set `REMOTECLAW_DIAGNOSTICS=timeline` with `REMOTECLAW_DIAGNOSTICS_TIMELINE_PATH=<path>` to write a best-effort JSONL startup diagnostics timeline for external QA harnesses. You can also enable the flag with `diagnostics.flags: ["timeline"]` in config; the path is still env-provided. Add `REMOTECLAW_DIAGNOSTICS_EVENT_LOOP=1` to include event-loop samples.
...
remoteclaw gateway health --port 18789
```
...
<ParamField path="--port <port>" type="number">
  Target a local loopback Gateway on this port. This overrides `REMOTECLAW_GATEWAY_URL` and `REMOTECLAW_GATEWAY_PORT` for the health call.
</ParamField>
...
```bash
remoteclaw gateway usage-cost
remoteclaw gateway usage-cost --days 7
remoteclaw gateway usage-cost --agent work --json
remoteclaw gateway usage-cost --all-agents
remoteclaw gateway usage-cost --json
```
...
```bash
remoteclaw gateway stability
remoteclaw gateway stability --type payload.large
remoteclaw gateway stability --bundle latest
remoteclaw gateway stability --bundle latest --export
remoteclaw gateway stability --json
```
...
<AccordionGroup>
  <Accordion title="Privacy and bundle behavior">
    ...
    - On fatal Gateway exits, shutdown timeouts, and restart startup failures, RemoteClaw writes the same diagnostic snapshot to `~/.remoteclaw/logs/stability/remoteclaw-stability-*.json` when the recorder has events. Inspect the newest bundle with `remoteclaw gateway stability --bundle latest`; `--limit`, `--type`, and `--since-seq` also apply to bundle output.
...
remoteclaw gateway diagnostics export --output remoteclaw-diagnostics.zip
remoteclaw gateway diagnostics export --json
```
...
remoteclaw gateway probe --port 18789
```
...
responsible for eventually exec'ing `remoteclaw` or Node with those args.

```bash
cat > ~/.local/bin/remoteclaw-doppler <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
exec doppler run --project my-project --config production -- remoteclaw "$@"
EOF
chmod +x ~/.local/bin/remoteclaw-doppler

remoteclaw gateway install --wrapper ~/.local/bin/remoteclaw-doppler --force
remoteclaw gateway restart
```
...
`REMOTECLAW_WRAPPER` in the service environment for later forced reinstalls, updates, and doctor
repairs.

```bash
REMOTECLAW_WRAPPER="$HOME/.local/bin/remoteclaw-doppler" remoteclaw gateway install --force
remoteclaw doctor
```

To remove a persisted wrapper, clear `REMOTECLAW_WRAPPER` while reinstalling:

```bash
REMOTECLAW_WRAPPER= remoteclaw gateway install --force
remoteclaw gateway restart
```
...
<AccordionGroup>
  ...
  <Accordion title="Lifecycle behavior">
    ...
    - `gateway restart --safe` asks the running Gateway to preflight active RemoteClaw work and defer the restart until reply delivery, embedded runs, and task runs drain. `--safe` cannot be combined with `--force` or `--wait`.
    ...
    - `gateway restart --safe --skip-deferral` runs the RemoteClaw-aware safe restart but bypasses the deferral gate so the Gateway emits the restart immediately even when blockers are reported. Operator escape hatch for stuck-task-run deferrals; requires `--safe`.
  ...
  <Accordion title="Auth and SecretRefs at install time">
    ...
    - For password auth on `gateway run`, prefer `REMOTECLAW_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
    - In inferred auth mode, shell-only `REMOTECLAW_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
...
`gateway discover` scans for Gateway beacons (`_remoteclaw-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `remoteclaw.internal.`) and set up split DNS + a DNS server; see [Bonjour](/gateway/bonjour).
