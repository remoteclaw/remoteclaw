...
export function runTestboxSyncSanity({
  ...
  const result = evaluateTestboxSyncSanity({
    ...
    deletionThreshold: parsePositiveInteger(
      env.REMOTECLAW_TESTBOX_DELETION_THRESHOLD,
      DEFAULT_DELETION_THRESHOLD,
    ),
    allowMassDeletions: parseBooleanEnv(env.REMOTECLAW_TESTBOX_ALLOW_MASS_DELETIONS),
  });
  ...
  if (keyResult.checked) {
    stdout.write(`Testbox local key and RemoteClaw claim ok: ${keyResult.testboxId}\n`);
  }
  stdout.write(
