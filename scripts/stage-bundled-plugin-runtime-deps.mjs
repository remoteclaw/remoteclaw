...
function makePluginOwnedTempDir(pluginDir, label) {
  return makeOwnedTempDir(pluginDir, `.remoteclaw-runtime-deps-${label}-`);
}
...
function replaceDirAtomically(targetPath, sourcePath) {
  ...
  const backupPath = makeTempDir(
    targetParentDir,
    `.remoteclaw-runtime-deps-backup-${sanitizeTempPrefixSegment(path.basename(targetPath))}-`,
  );
...
function writeJsonAtomically(targetPath, value) {
  ...
  const tempDir = makeOwnedTempDir(
    targetParentDir,
    `.remoteclaw-runtime-deps-stamp-${sanitizeTempPrefixSegment(path.basename(targetPath))}-`,
  );
...
function createRuntimeInstallManifest(pluginId, pinnedGroups) {
  const manifest = {
    name: `remoteclaw-runtime-deps-${sanitizeTempPrefixSegment(pluginId)}`,
    private: true,
    version: "0.0.0",
  };
...
function resolveLegacyRuntimeDepsStampPath(pluginDir) {
  return path.join(pluginDir, ".remoteclaw-runtime-deps-stamp.json");
}
...
function removeStaleRuntimeDepsTempDirs(pluginDir) {
  ...
  for (const entry of fs.readdirSync(pluginDir, { withFileTypes: true })) {
    if (entry.name.startsWith(".remoteclaw-runtime-deps-")) {
