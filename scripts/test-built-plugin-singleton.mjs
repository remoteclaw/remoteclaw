...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@remoteclaw/build-smoke-plugin",
      type: "module",
      remoteclaw: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  path.join(distPluginDir, "remoteclaw.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'remoteclaw/plugin-sdk';",
...
const registry = loadRemoteClawPlugins({
  ...
  env: {
    ...process.env,
    REMOTECLAW_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
