...
The plugin is included with RemoteClaw, but it is off by default. When disabled, the route is not registered. When enabled, it adds:
...
Use the WebSocket RPC path for RemoteClaw clients and interactive tools that can keep a Gateway WebSocket connection open.
...
<Tabs>
  <Tab title="CLI">
    ```bash
    remoteclaw plugins enable admin-http-rpc
...
```bash
remoteclaw plugins disable admin-http-rpc
remoteclaw gateway restart
```
...
- For `token` and `password` auth, narrower `x-remoteclaw-scopes` headers are ignored and the normal full operator defaults are restored.
- Trusted identity-bearing HTTP modes honor `x-remoteclaw-scopes` when present.
...
The normal Gateway WebSocket RPC path remains the preferred control-plane API for RemoteClaw clients. Use admin HTTP RPC only for host tooling that needs a request/response HTTP surface.
