...
The Gateway writes records to the shared RemoteClaw state database through a
...
```bash
remoteclaw audit
remoteclaw audit --agent main --status failed
remoteclaw audit --session "agent:main:main" --after 2026-07-01T00:00:00Z
remoteclaw audit --run 8c69f72e-8b11-4c54-98d5-1a3dd67450c3
remoteclaw audit --kind tool_action --limit 50 --json
```
...
```bash
remoteclaw gateway call audit.list --params '{"agentId":"main","status":"failed","limit":50}'
```
