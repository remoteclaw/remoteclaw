...
function canResolveEnvSecretRefInReadOnlyPath(params: {
  ...
  if (!providerConfig) {
    return params.provider === resolveDefaultSecretProviderAlias(params.cfg as any, "env");
  }
  if ((providerConfig as any).source !== "env") {
    return false;
  }
  const allowlist = (providerConfig as any).allowlist;
  return !allowlist || allowlist.includes(params.id);
}

function inspectTokenValue(params: { cfg: RemoteClawConfig; value: unknown }): {
  token: string;
