...
read_when:
  - You maintain an RemoteClaw plugin
  - You see a plugin compatibility warning
  - You are planning a plugin SDK or manifest migration
---

RemoteClaw keeps older plugin contracts wired through named compatibility
...
The plugin inspector should live outside the core RemoteClaw repo as a separate
...
```sh
remoteclaw-plugin-inspector ./my-plugin
```
...
Use `--json` for stable machine-readable output in CI annotations. RemoteClaw
core should expose contracts and fixtures the inspector can consume, but should
not publish the inspector binary from the main `remoteclaw` package.

### Maintainer acceptance lane

Use Crabbox-backed Blacksmith Testbox for the installable-package acceptance
lane when validating the external inspector against RemoteClaw plugin packages.
Run it from a clean RemoteClaw checkout after the package is built:

```sh
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "pnpm install && pnpm build && npm exec --yes @remoteclaw/plugin-inspector@0.1.0 -- ./extensions/telegram --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @remoteclaw/plugin-inspector@0.1.0 -- ./extensions/discord --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @remoteclaw/plugin-inspector@0.1.0 -- <clawhub-plugin-dir> --json"
```
...
RemoteClaw should not remove a documented plugin contract in the same release
...
- legacy broad SDK imports such as `remoteclaw/plugin-sdk/compat`
...
- legacy `activate(api)` plugin entrypoints while plugins migrate to
  `register(api)`
- legacy SDK aliases such as `remoteclaw/extension-api`,
  `remoteclaw/plugin-sdk/channel-runtime`, `remoteclaw/plugin-sdk/command-auth`
  status builders, `remoteclaw/plugin-sdk/test-utils` (replaced by focused
  `remoteclaw/plugin-sdk/*` test subpaths), and the `ClawdbotConfig` /
  `RemoteClawSchemaType` type aliases
...
- legacy channel route key and comparable-target helper aliases while plugins
  move to `remoteclaw/plugin-sdk/channel-route`
...
- channel `showConfigured` / `showInSetup` metadata while channel packages move
  to `remoteclaw.channel.exposure`
...
- persisted plugin registry disable and install-migration env flags while
  repair flows migrate operators to `remoteclaw plugins registry --refresh` and
