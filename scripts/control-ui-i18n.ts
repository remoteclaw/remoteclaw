...
const ENV_PROVIDER = "REMOTECLAW_CONTROL_UI_I18N_PROVIDER";
const ENV_MODEL = "REMOTECLAW_CONTROL_UI_I18N_MODEL";
const ENV_THINKING = "REMOTECLAW_CONTROL_UI_I18N_THINKING";
const ENV_PI_EXECUTABLE = "REMOTECLAW_CONTROL_UI_I18N_PI_EXECUTABLE";
const ENV_PI_ARGS = "REMOTECLAW_CONTROL_UI_I18N_PI_ARGS";
const ENV_PI_PACKAGE_VERSION = "REMOTECLAW_CONTROL_UI_I18N_PI_PACKAGE_VERSION";
const ENV_BATCH_CHAR_BUDGET = "REMOTECLAW_CONTROL_UI_I18N_BATCH_CHAR_BUDGET";
const ENV_PROMPT_TIMEOUT = "REMOTECLAW_CONTROL_UI_I18N_PROMPT_TIMEOUT";
const ENV_AUTH_OPTIONAL = "REMOTECLAW_CONTROL_UI_I18N_AUTH_OPTIONAL";
...
const DEFAULT_GLOSSARY: readonly GlossaryEntry[] = [
  { source: "RemoteClaw", target: "RemoteClaw" },
...
async function resolvePiCommand(): Promise<PiCommand> {
  ...
  if (explicitExecutable) {
    ...
    if (isWindowsCommandShim(explicitExecutable)) {
      throw new Error(
        `${ENV_PI_EXECUTABLE} points to a Windows command shim that cannot safely carry the multiline i18n system prompt. Point it at node with ${ENV_PI_ARGS} set to the Pi package dist/cli.js path, or unset it so RemoteClaw uses the managed Pi runtime.`,
      );
    }
    return {
