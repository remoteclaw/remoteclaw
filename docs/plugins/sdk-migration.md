...
RemoteClaw replaced a broad backwards-compatibility layer with a modern plugin
...
- **`remoteclaw/plugin-sdk/compat`** - re-exported dozens of helpers to keep
  older hook-based plugins working while the new architecture was built.
- **`remoteclaw/plugin-sdk/infra-runtime`** - a broad barrel mixing system
  events, heartbeat state, delivery queues, fetch/proxy helpers, file helpers,
  approval types, and unrelated utilities.
- **`remoteclaw/plugin-sdk/config-runtime`** - a broad config barrel still
  carrying deprecated direct load/write helpers during the migration window.
- **`remoteclaw/extension-api`** - a bridge giving plugins direct access to
...
Each `remoteclaw/plugin-sdk/<subpath>` is now a small, self-contained module with
...
<Steps>
  <Step title="Migrate runtime config load/write helpers">
    ...
    New plugin code should avoid the broad `remoteclaw/plugin-sdk/config-runtime`
  ...
  <Step title="Migrate embedded tool-result extensions to middleware">
    ...
    ```typescript
    // RemoteClaw and Codex runtime dynamic tools
    api.registerAgentToolResultMiddleware(async (event) => {
      return compactToolResult(event);
    }, {
      runtimes: ["remoteclaw", "codex"],
    });
    ```
  ...
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects (clients, tokens, Bolt apps)
      through `remoteclaw/plugin-sdk/channel-runtime-context`.
  ...
  <Step title="Replace broad infra-runtime imports">
    ...
    | In-memory and persistent-backed dedupe caches | `remoteclaw/plugin-sdk/dedupe-runtime` |
  ...
  <Step title="Migrate channel route helpers">
    New channel route code uses `remoteclaw/plugin-sdk/channel-route`. The older
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `RemoteClawSchema` |
...
import the published `@remoteclaw/discord` package directly. Owner-specific
...
<AccordionGroup>
  <Accordion title="command-auth help builders -> command-status">
    **Old (`remoteclaw/plugin-sdk/command-auth`)**: `buildCommandsMessage`,
    `buildCommandsMessagePaginated`, `buildHelpMessage`.

    **New (`remoteclaw/plugin-sdk/command-status`)**: same signatures, same
    exports - just imported from the narrower subpath. `command-auth`
    re-exports them as compat stubs.

    ```typescript
    // Before
    import { buildHelpMessage } from "remoteclaw/plugin-sdk/command-auth";

    // After
    import { buildHelpMessage } from "remoteclaw/plugin-sdk/command-status";
    ```
  ...
  <Accordion title="Mention gating helpers -> resolveInboundMentionDecision">
    **Old**: `resolveMentionGating(params)` and
    ...
    `remoteclaw/plugin-sdk/channel-inbound` or
    `remoteclaw/plugin-sdk/channel-mention-gating`.
  ...
  <Accordion title="Channel runtime shim and channel actions helpers">
    ...
    `channelActions*` helpers in `remoteclaw/plugin-sdk/channel-actions` are
  ...
  <Accordion title="Thinking policy hooks -> resolveThinkingProfile">
    ...
    **New**: a single `resolveThinkingProfile(ctx)` that returns a
    ...
    ranked level list. RemoteClaw downgrades stale stored values by profile rank
    automatically.
...
session controller exported by `remoteclaw/plugin-sdk/realtime-voice`. The
