...
`ask: "always"` in `~/.remoteclaw/exec-approvals.json` keeps prompting even if
session or config defaults request `ask: "on-miss"`.
Use `remoteclaw approvals get`, `remoteclaw approvals get --gateway`, or
`remoteclaw approvals get --node <id|name|ip>` to inspect the requested policy,
host policy sources, and the effective result.
For the local machine, `remoteclaw exec-policy show` exposes the same merged view and
`remoteclaw exec-policy set|preset` can synchronize the local requested policy with the
local host approvals file in one step. When a local scope requests `host=node`,
`remoteclaw exec-policy show` reports that scope as node-managed at runtime instead of
...
- For shell scripts and direct interpreter/runtime file invocations, RemoteClaw also tries to bind
...
- requested exec policy in RemoteClaw config (`tools.exec.*`)
- host-local approvals policy in `~/.remoteclaw/exec-approvals.json`
...
- In YOLO mode, RemoteClaw does not add a separate heuristic command-obfuscation approval gate on top of the configured host exec policy.
...
remotely, continue using `remoteclaw approvals set --gateway` or
...
- allowlist entries live in host-local `~/.remoteclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `remoteclaw approvals allowlist ...`).
- `remoteclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `remoteclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.remoteclaw/exec-approvals.json` directly.

CLI: `remoteclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
After an approved async exec finishes, RemoteClaw sends a followup `agent` turn to the same session.
...
When an async exec approval is denied, RemoteClaw prevents the agent from reusing
