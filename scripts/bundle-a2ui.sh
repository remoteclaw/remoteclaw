...
A2UI_APP_DIR="$ROOT_DIR/apps/shared/RemoteClawKit/Tools/CanvasA2UI"
...
if [[ ! -d "$A2UI_RENDERER_DIR" || ! -d "$A2UI_APP_DIR" ]]; then
  ...
  if [[ -n "${REMOTECLAW_SPARSE_PROFILE:-}" || "${REMOTECLAW_A2UI_SKIP_MISSING:-}" == "1" ]]; then
    echo "A2UI sources missing; skipping bundle because REMOTECLAW_A2UI_SKIP_MISSING=1 or REMOTECLAW_SPARSE_PROFILE is set." >&2
    exit 0
  fi
