...
summary: "Design doc for integrating Kilo Gateway as a first-class RemoteClaw provider"
read_when:
...
This document outlines the design for integrating "Kilo Gateway" as a first-class provider in RemoteClaw, modeled after the existing OpenRouter implementation. Kilo Gateway uses an OpenAI-compatible completions API with a different base URL.
