...
When `channels.matrix.streaming` is `"quiet"`, RemoteClaw streams the reply by editing a single preview event in place. Previews are sent as non-notifying `m.notice` events, and the finalized edit is marked with `content["com.remoteclaw.finalized_preview"] = true`. Matrix clients notify on that final edit only if a per-user push rule matches the marker. This page is for operators who self-host Matrix and want to install that rule for each recipient account.
...
```
    ...
    - `remoteclaw-finalized-preview-botname`: a rule ID unique per bot per recipient (pattern: `remoteclaw-finalized-preview-<botname>`)
    - `@bot:example.org`: your RemoteClaw bot MXID, not the recipient's
...
Push rules are keyed by `ruleId`: re-running `PUT` against the same ID updates a single rule. For multiple RemoteClaw bots notifying the same recipient, create one rule per bot with a distinct sender match.
