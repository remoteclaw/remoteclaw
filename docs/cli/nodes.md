---
summary: "CLI reference for `remoteclaw nodes` (status, pairing, invoke, camera/canvas/screen)"
read_when:
...
remoteclaw nodes list --connected
remoteclaw nodes list --last-connected 24h
...
- `remoteclaw nodes pending` only needs pairing scope.
...
- `remoteclaw nodes approve <requestId>` inherits extra scope requirements from the
  pending request:
...
```bash
remoteclaw nodes invoke --node <id|name|ip> --command <command> --params <json>
```
...
For shell execution on a node, use the `exec` tool with `host=node` instead of `remoteclaw nodes run`.
The `nodes` CLI is now capability-focused: direct RPC via `nodes invoke`, plus pairing, camera,
screen, location, Canvas, and notifications. Canvas commands are implemented by the bundled experimental Canvas plugin; core keeps a compatibility hook so they remain under `remoteclaw nodes canvas`.
