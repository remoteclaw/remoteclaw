...
Lobster is a workflow shell that lets RemoteClaw run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: RemoteClaw runs one Lobster tool call and gets a structured result.
...
RemoteClaw launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | remoteclaw.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In RemoteClaw tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the RemoteClaw Gateway (see the [Lobster repo](https://github.com/remoteclaw/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), RemoteClaw keeps core tools enabled. To restrict core
...
→ remoteclaw calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ remoteclaw calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls RemoteClaw tools that do.
