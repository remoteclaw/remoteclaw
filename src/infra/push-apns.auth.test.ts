...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-push-apns-auth-test-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("push APNs auth and helper coverage", () => {
  ...
  it("prefers inline APNs private key values and unescapes newlines", async () => {
    const resolved = await resolveApnsAuthConfigFromEnv({
      ...
      REMOTECLAW_APNS_PRIVATE_KEY: "ignored",
  ...
  it("falls back to REMOTECLAW_APNS_PRIVATE_KEY when REMOTECLAW_APNS_PRIVATE_KEY_P8 is blank", async () => {
    const resolved = await resolveApnsAuthConfigFromEnv({
      REMOTECLAW_APNS_TEAM_ID: "TEAM123",
      REMOTECLAW_APNS_KEY_ID: "KEY123",
      REMOTECLAW_APNS_PRIVATE_KEY_P8: "   ",
      REMOTECLAW_APNS_PRIVATE_KEY:
        "-----BEGIN PRIVATE KEY-----\\nline-c\\nline-d\\n-----END PRIVATE KEY-----", // pragma: allowlist secret
    } as NodeJS.ProcessEnv);
  ...
  it("reads APNs private keys from REMOTECLAW_APNS_PRIVATE_KEY_PATH", async () => {
    ...
    const resolved = await resolveApnsAuthConfigFromEnv({
      REMOTECLAW_APNS_TEAM_ID: "TEAM123",
      REMOTECLAW_APNS_KEY_ID: "KEY123",
      REMOTECLAW_APNS_PRIVATE_KEY_PATH: keyPath,
    } as NodeJS.ProcessEnv);
  ...
  it("reports missing auth fields and path read failures", async () => {
    ...
    await expect(resolveApnsAuthConfigFromEnv({} as NodeJS.ProcessEnv)).resolves.toEqual({
      ok: false,
      error: "APNs auth missing: set REMOTECLAW_APNS_TEAM_ID and REMOTECLAW_APNS_KEY_ID",
    });

    const missingKey = await resolveApnsAuthConfigFromEnv({
      REMOTECLAW_APNS_TEAM_ID: "TEAM123",
      REMOTECLAW_APNS_KEY_ID: "KEY123",
      REMOTECLAW_APNS_PRIVATE_KEY_PATH: missingPath,
    } as NodeJS.ProcessEnv);

    expect(missingKey.ok).toBe(false);
    if (!missingKey.ok) {
      expect(missingKey.error).toContain(
        `failed reading REMOTECLAW_APNS_PRIVATE_KEY_PATH (${missingPath})`,
      );
    }
  });
