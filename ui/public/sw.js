// RemoteClaw Control – Service Worker
// Handles offline caching and push notifications.

const CACHE_PREFIX = "remoteclaw-control-";
...
const CACHE_VERSION =
  (EMBEDDED_CACHE_VERSION !== "__REMOTECLAW_CONTROL_UI_BUILD_ID__"
...
self.addEventListener("push", (event) => {
  ...
  try {
    ...
    data = { title: "RemoteClaw", body: event.data.text() };
  }

  const title = data.title || "RemoteClaw";
  const options = {
    body: data.body || "",
    icon: "./apple-touch-icon.png",
    badge: "./favicon-32.png",
    tag: data.tag || "remoteclaw-notification",
    data: { url: data.url || "./" },
  };
