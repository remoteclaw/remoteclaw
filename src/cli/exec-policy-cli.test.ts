...
const mocks = vi.hoisted(() => {
  ...
  let configState: RemoteClawConfig = {
    tools: {
      exec: {
        host: "auto",
        security: "allowlist",
  ...
  return {
    ...
    setConfig: (next: RemoteClawConfig) => {
      configState = next;
    },
    ...
    defaultRuntime,
    runtimeErrors,
    mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: RemoteClawConfig) => void }) => {
      ...
      return {
        ...
        previousHash: "hash-1",
        snapshot: { path: "/tmp/remoteclaw.json" },
        nextConfig: draft,
        result: undefined,
      };
    }),
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: RemoteClawConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: RemoteClawConfig }>
    >(async () => ({
      path: "/tmp/remoteclaw.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: RemoteClawConfig) => void }) => {
  ...
  it("shows the local merged exec policy as json", async () => {
    ...
    expect(mocks.defaultRuntime.writeJson).toHaveBeenCalledWith(
      expect.objectContaining({
        configPath: "/tmp/remoteclaw.json",
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/remoteclaw.json");
    ...
    expect(output).not.toContain("/tmp/remoteclaw.json\nforged");
