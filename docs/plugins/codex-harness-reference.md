...
By default, RemoteClaw starts the managed Codex binary shipped with the bundled
plugin:
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after RemoteClaw builds its inherited environment.                                                                                                                                                                                                                                                                   |
| `remoteWorkspaceRoot`                         | unset                                                  | Remote Codex app-server workspace root. When set, RemoteClaw infers the local workspace root from the resolved RemoteClaw workspace, preserves the current cwd suffix under this remote root, and sends only the final app-server cwd to Codex. If the cwd is outside the resolved RemoteClaw workspace root, RemoteClaw fails closed instead of sending a gateway-local path to the remote app-server. |
...
| `postToolRawAssistantCompletionIdleTimeoutMs` | `300000`                                               | Completion-idle and progress guard used after a tool handoff, native tool completion, post-tool raw assistant progress, raw reasoning completion, or reasoning progress while RemoteClaw waits for `turn/completed`. Use this for trusted or heavy workloads where post-tool synthesis can legitimately stay quiet longer than the final assistant release budget.                                      |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start and resume. Active RemoteClaw sandboxes narrow `danger-full-access` turns to Codex `workspace-write`; the turn network flag follows RemoteClaw sandbox egress.                                                                                                                                                                                           |
| `approvalsReviewer`                           | `"user"` or an allowed guardian reviewer               | Use `"auto_review"` to let Codex review native approval prompts when allowed.                                                                                                                                                                                                                                                                                                                           |
| `defaultWorkspaceDir`                         | current process directory                              | Workspace used by `/codex bind` when `--cwd` is omitted.                                                                                                                                                                                                                                                                                                                                                |
| `serviceTier`                                 | unset                                                  | Optional Codex app-server service tier. `"priority"` enables fast-mode routing, `"flex"` requests flex processing, and `null` clears the override. Legacy `"fast"` is accepted as `"priority"`.                                                                                                                                                                                                         |
| `networkProxy`                                | disabled                                               | Opt into Codex permissions-profile networking for app-server commands. RemoteClaw defines the selected `permissions.<profile>.network` config and selects it with `default_permissions` instead of sending `sandbox`.                                                                                                                                                                                   |
...
contract. When enabled, RemoteClaw also sets `features.network_proxy.enabled` and
`default_permissions` in the Codex thread config so the generated permission
profile can start Codex managed networking. By default, RemoteClaw generates a
collision-resistant `remoteclaw-network-<fingerprint>` profile name from the
...
RemoteClaw treats non-loopback WebSocket app-server URLs as remote and requires
...
shorthand before RemoteClaw builds app-server start options, and unresolved
structured SecretRefs fail before any token or header is sent. When native Codex
plugins are configured, RemoteClaw uses the connected app-server's plugin control
plane to install or refresh those plugins and then refreshes app inventory so
plugin-owned apps are visible to the Codex thread. Only connect RemoteClaw to
remote app-servers that are trusted to accept RemoteClaw-managed plugin installs
...
unattended RemoteClaw turns and heartbeats make progress without native approval
prompts that nobody is around to answer.

If Codex's local system requirements file disallows implicit YOLO approval,
reviewer, or sandbox values, RemoteClaw treats the implicit default as guardian
...
When an RemoteClaw sandbox is active, the local Codex app-server process still
runs on the Gateway host. RemoteClaw therefore disables Codex native Code Mode,
user MCP servers, and app-backed plugin execution for that turn instead of
treating Codex host-side sandboxing as equivalent to the RemoteClaw sandbox
backend. Shell access is exposed through RemoteClaw sandbox-backed dynamic tools
...
`workspace-write` without active RemoteClaw sandboxing. If you see
`bwrap: setting up uid map: Permission denied` or
`bwrap: loopback: Failed RTM_NEWADDR: Operation not permitted`, run
`remoteclaw doctor` and fix the reported host namespace policy for the RemoteClaw
...
The stable default is fail-closed: active RemoteClaw sandboxing disables native
...
try Codex's remote environment support with RemoteClaw's sandbox backend. This
...
When the flag is on and the current RemoteClaw session is sandboxed, RemoteClaw
...
RemoteClaw-owned environment. If the app-server cannot register the environment,
...
loopback exec-server unless it is running on the same host, so RemoteClaw rejects
that combination.
...
1. An explicit RemoteClaw Codex auth profile for the agent.
...
Stdio app-server launches inherit RemoteClaw's process environment by default.
RemoteClaw owns the Codex app-server account bridge and sets `CODEX_HOME` to a
per-agent directory under that agent's RemoteClaw state. That keeps Codex config,
accounts, plugin cache/data, and thread state scoped to the RemoteClaw agent
instead of leaking in from the operator's personal `~/.codex` home.

RemoteClaw does not rewrite `HOME` for normal local app-server launches. Codex-run
subprocesses such as `remoteclaw`, `gh`, `git`, cloud CLIs, and shell commands see
...
RemoteClaw plugins and RemoteClaw skill snapshots still flow through RemoteClaw's own
...
part of an RemoteClaw agent, inventory them explicitly:
...
Most remaining RemoteClaw integration tools, such as messaging, media, cron,
browser, nodes, gateway, `heartbeat_respond`, and `web_search`, are available
through Codex tool search under the `remoteclaw` namespace. This keeps the initial
...
surface, while explicit RemoteClaw or ACP delegation is still available through
the `remoteclaw` dynamic tool namespace.
...
Dynamic tool budgets are capped at 600000 ms. On timeout, RemoteClaw aborts the
...
post-tool idle budget: after RemoteClaw returns an `item/tool/call` response, after
...
`turn/completed`, RemoteClaw best-effort interrupts the native turn and releases
...
availability is owned by Codex app-server, so the list can change when RemoteClaw
...
If discovery fails or times out, RemoteClaw uses a bundled fallback catalog for:
...
Codex handles `AGENTS.md` itself through native project-doc discovery. RemoteClaw
...
For RemoteClaw workspace parity, the Codex harness resolves the other bootstrap
files. `SOUL.md`, `IDENTITY.md`, `TOOLS.md`, and `USER.md` are forwarded as
RemoteClaw Codex developer instructions because they define the active agent,
available workspace guidance, and user profile. The compact RemoteClaw skills
...
`BOOTSTRAP.md` when present is forwarded as RemoteClaw turn input reference
context.
