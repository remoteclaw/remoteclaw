 ...
 * Runs as part of `remoteclaw doctor` — config-only, no network calls.
 */
export async function noteMemorySearchHealth(cfg: RemoteClawConfig): Promise<void> {
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("remoteclaw config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("remoteclaw memory status --deep")}`,
        ].join("\n"),
        "Memory search",
      );
      return;
    ...
    note(
      [
        ...
        `- Add credentials: ${formatCliCommand(`remoteclaw auth add --provider ${resolved.provider}`)}`,
        `- To disable: ${formatCliCommand("remoteclaw config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("remoteclaw memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  note(
    [
      ...
      `- Add credentials: ${formatCliCommand("remoteclaw auth add --provider openai")}`,
