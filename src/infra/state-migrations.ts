...
function pickLatestLegacyDirectEntry(store: Record<string, SessionEntryLike>): SessionEntryLike {
  let best: SessionEntryLike = null;
  let bestUpdated = -1;
...
export async function autoMigrateLegacyState(params: {
  ...
  if (env.REMOTECLAW_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim()) {
    return {
