...
- Default rolling log file is under `/tmp/remoteclaw/` (one file per day): `remoteclaw-YYYY-MM-DD.log`
...
- The log file path and level can be configured via `~/.remoteclaw/remoteclaw.json`:
  - `logging.file`
  - `logging.level`
...
RemoteClaw can mask sensitive tokens before log or transcript output leaves the
