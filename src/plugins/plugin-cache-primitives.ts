...
export type ConfigScopedRuntimeCache<T> = WeakMap<RemoteClawConfig, Map<string, T>>;
...
export type ConfigScopedPromiseLoader<T> = {
  load(config?: RemoteClawConfig): Promise<T>;
  clear(): void;
};
...
export function createConfigScopedPromiseLoader<T>(
  load: (config?: RemoteClawConfig) => T | Promise<T>,
): ConfigScopedPromiseLoader<T> {
  let defaultPromise: Promise<T> | undefined;
  let promisesByConfig = new WeakMap<RemoteClawConfig, Promise<T>>();

  const createPromise = (config?: RemoteClawConfig): Promise<T> => {
  ...
  return {
    async load(config?: RemoteClawConfig): Promise<T> {
      if (!config) {
    ...
    clear(): void {
      defaultPromise = undefined;
      promisesByConfig = new WeakMap<RemoteClawConfig, Promise<T>>();
    },
  };
}
