...
async function runTelegramAction(
  ...
  options?: { cfg?: RemoteClawConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as RemoteClawConfig);
...
function createSignalAccountOverrideCfg(): RemoteClawConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as RemoteClawConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as RemoteClawConfig,
        accountId: undefined,
        params: {
