...
struct MacGatewayChatTransport: RemoteClawChatTransport, Sendable {
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> RemoteClawChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                RemoteClawGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    RemoteClawChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    RemoteClawAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<RemoteClawChatView>
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<RemoteClawChatView>) -> NSViewController
    {
