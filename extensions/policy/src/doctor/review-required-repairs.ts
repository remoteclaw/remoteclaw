...
function previewGatewayLoopbackBind(
  ...
  if (
    finding.ocPath !== "oc://remoteclaw.config/gateway/bind" &&
    finding.ocPath !== "oc://remoteclaw.config/gateway/customBindHost"
  ) {
    return [];
  }
  return [
...
function previewGatewayNodeDenyCommand(
  ...
  if (
    command === undefined ||
    command === "" ||
    finding.ocPath !== "oc://remoteclaw.config/gateway/nodes/denyCommands"
  ) {
    return [];
  }
  return [
