---
summary: "Scripted onboarding and agent setup for the RemoteClaw CLI"
read_when:
...
```bash
remoteclaw onboard --non-interactive --accept-risk \
  --mode local \
  --auth-choice apiKey \
...
Anthropic setup-token auth remains supported, but RemoteClaw prefers Claude CLI reuse when a local Claude CLI login is available. For production, prefer an Anthropic API key.
...
- Default workspace (when `--workspace` is omitted in the interactive wizard): `~/.remoteclaw/workspace-<agentId>`.
