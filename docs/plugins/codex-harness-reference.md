...
| `turnCompletionIdleTimeoutMs` | `60000`                                                | Quiet window after a turn-scoped app-server request while RemoteClaw waits for `turn/completed`.                                                                                                |
...
Stdio app-server launches inherit RemoteClaw's process environment by default, but
RemoteClaw owns the Codex app-server account bridge and sets both `CODEX_HOME` and
`HOME` to per-agent directories under that agent's RemoteClaw state. Codex's own
...
skills, plugins, config, accounts, and thread state scoped to the RemoteClaw agent
...
useful Codex CLI skills or plugins that should become part of an RemoteClaw agent,
...
`CODEX_HOME` and `HOME` remain reserved for RemoteClaw's per-agent Codex
...
Remaining RemoteClaw integration tools, such as messaging, sessions, media, cron,
