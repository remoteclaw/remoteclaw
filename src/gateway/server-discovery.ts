...
/** Formats the Bonjour instance name while preserving user-provided RemoteClaw names. */
...
export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  ...
  const envPath = env.REMOTECLAW_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.REMOTECLAW_TAILNET_DNS?.trim();
