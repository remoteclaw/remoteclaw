...
import { AUTO_IMAGE_KEY_PROVIDERS } from "../../../src/media-understanding/defaults.js";
...
export async function describeStickerImage(params: DescribeStickerParams): Promise<string | null> {
  ...
  const hasProviderKey = async (provider: string) => {
    try {
      await resolveApiKeyForProvider({ provider, cfg });
      return true;
    } catch {
      return false;
    }
  ...
  if (!resolved) {
    resolved = resolveAutoImageModel({
      cfg,
      agentDir,
      activeModel,
    }) as unknown as { provider: string; model?: string } | null;
  }
