...
PACKAGE_NAME="org.remoteclaw.app"
RECEIVER="$PACKAGE_NAME/.VoiceE2eReceiver"
RUN_ACTION="org.remoteclaw.app.debug.RUN_VOICE_E2E"
OPEN_ACTION="org.remoteclaw.app.debug.OPEN_VOICE_E2E"
PORT=18789
HOST="127.0.0.1"
MODE="both"
...
ARTIFACT_DIR="/tmp/remoteclaw-android-voice-e2e-$(date +%Y%m%d-%H%M%S)"
...
if [[ "$START_GATEWAY" -eq 1 ]]; then
  if command -v bws_get_secret >/dev/null 2>&1; then
    REMOTECLAW_OPENAI_API_KEY="$(bws_get_secret REMOTECLAW_OPENAI_API_KEY)"
  else
    REMOTECLAW_OPENAI_API_KEY="$(zsh -ic 'bws_get_secret REMOTECLAW_OPENAI_API_KEY')"
  fi
  (
    cd "$ROOT_DIR"
    OPENAI_API_KEY="$REMOTECLAW_OPENAI_API_KEY" \
      pnpm remoteclaw gateway run \
        --port "$PORT" \
        --auth none \
        --bind loopback \
        --force \
  ...
  unset REMOTECLAW_OPENAI_API_KEY
fi
...
adb logcat -d -v time |
  rg -i 'RemoteClaw|TalkMode|MicCapture|AudioRecord|SpeechRecognizer|realtime|talk.session|appendAudio|transcript|Talk failed|Transcription failed|Speech network|VoiceE2E' |
