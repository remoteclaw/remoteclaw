...
noVNC observer access uses VNC auth by default and RemoteClaw emits a short-lived token URL that serves a local bootstrap page; noVNC password is passed via URL fragment (instead of URL query).
