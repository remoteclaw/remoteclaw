...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func prefersRemoteClawBinary() throws {
        ...
        let remoteclawPath = tmp.appendingPathComponent("node_modules/.bin/remoteclaw")
        try self.makeExec(at: remoteclawPath)

        let cmd = CommandResolver.remoteclawCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
    ...
    @Test func fallsBackToNodeAndScript() throws {
        ...
        let scriptPath = tmp.appendingPathComponent("bin/remoteclaw.js")
        try self.makeExec(at: nodePath)
        try "#!/bin/sh\necho v22.0.0\n".write(to: nodePath, atomically: true, encoding: .utf8)
        try FileManager().setAttributes([.posixPermissions: 0o755], ofItemAtPath: nodePath.path)
        try self.makeExec(at: scriptPath)

        let cmd = CommandResolver.remoteclawCommand(
            subcommand: "rpc",
            defaults: defaults,
            configRoot: [:],
    ...
    @Test func prefersRemoteClawBinaryOverPnpm() throws {
        ...
        let remoteclawPath = binDir.appendingPathComponent("remoteclaw")
    ...
    @Test func pnpmKeepsExtraArgsAfterSubcommand() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "remoteclaw", "health", "--json"]))
    ...
    @Test func buildsSSHCommandForRemoteMode() {
        ...
        defaults.set("/srv/remoteclaw", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "remoteclaw@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/remoteclaw'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("remoteclaw"))
    ...
    @Test func configRootLocalOverridesRemoteDefaults() throws {
        ...
        #expect(cmd.first == remoteclawPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
