...
describe("resolveGatewayProgramArguments", () => {
  it("prefers index.js over legacy entry.js when both exist in the same dist directory", async () => {
    const entryPath = path.resolve("/opt/remoteclaw/dist/entry.js");
    const indexPath = path.resolve("/opt/remoteclaw/dist/index.js");
  ...
  it("keeps entry.js when index.js is missing", async () => {
    const entryPath = path.resolve("/opt/remoteclaw/dist/entry.js");
    const indexPath = path.resolve("/opt/remoteclaw/dist/index.js");
    const indexMjsPath = path.resolve("/opt/remoteclaw/dist/index.mjs");
    process.argv = ["node", entryPath];
    fsMocks.realpath.mockResolvedValue(entryPath);
  ...
  it("uses realpath-resolved dist entry when running via npx shim", async () => {
    const argv1 = path.resolve("/tmp/.npm/_npx/63c3/node_modules/.bin/remoteclaw");
    const entryPath = path.resolve("/tmp/.npm/_npx/63c3/node_modules/remoteclaw/dist/entry.js");
  ...
  it("prefers symlinked path over realpath for stable service config", async () => {
    ...
    expect(result.programArguments[1]).toBe(
      path.resolve("/Users/test/Library/pnpm/global/5/node_modules/remoteclaw/dist/index.js"),
    );
  ...
  it("falls back to node_modules package dist when .bin path is not resolved", async () => {
    const argv1 = path.resolve("/tmp/.npm/_npx/63c3/node_modules/.bin/remoteclaw");
    const indexPath = path.resolve("/tmp/.npm/_npx/63c3/node_modules/remoteclaw/dist/index.js");
    process.argv = ["node", argv1];
  ...
  it("uses an executable wrapper when provided", async () => {
    const wrapperPath = path.resolve("/usr/local/bin/remoteclaw-doppler");
  ...
  it("rejects a non-executable wrapper file", async () => {
    ...
    ).rejects.toThrow("REMOTECLAW_WRAPPER must point to an executable file");
  });
});
