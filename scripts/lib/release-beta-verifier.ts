// Release Beta Verifier script supports RemoteClaw repository automation.
...
const DEFAULT_REPO = "remoteclaw/remoteclaw";
const DEFAULT_CLAWHUB_REGISTRY = "https://remoteclaw.org";
...
export function parseReleaseVerifyBetaArgs(argv: string[]): ReleaseVerifyBetaArgs {
  ...
  if (!version || version.startsWith("-")) {
    throw new Error(
      "Usage: pnpm release:verify-beta -- <version> [--workflow-ref REF] [--full-release-validation-run ID] [--remoteclaw-npm-run ID] [--plugin-npm-run ID] [--plugin-clawhub-run ID] [--npm-telegram-run ID] [--skip-clawhub]",
    );
  }
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    switch (arg) {
      ...
      case "--full-release-validation-run":
        ...
        break;
      case "--remoteclaw-npm-run":
        parsed.workflowRuns.remoteclawNpm = next();
        break;
      case "--plugin-npm-run":
...
export async function verifyBetaRelease(
  ...
  const remoteclawNpm = verifyNpmPackage("remoteclaw", args.version, args.distTag);
  lines.push(`remoteclaw npm OK: ${args.version} (${args.distTag})`);
  ...
  if (!args.skipPostpublish) {
    ...
    lines.push("remoteclaw postpublish verifier OK");
  }
  ...
  if (args.workflowRuns.remoteclawNpm !== undefined) {
    workflowRuns.push(
      verifyWorkflowRun({
        id: args.workflowRuns.remoteclawNpm,
        label: "RemoteClaw NPM Release",
        repo: args.repo,
        expectedWorkflowName: "RemoteClaw NPM Release",
        expectedHeadBranch: args.workflowRef,
        rerunFailed: false,
      }),
    );
      ...
      `${JSON.stringify(
        {
          ...
          remoteclawNpmIntegrity: remoteclawNpm.integrity,
