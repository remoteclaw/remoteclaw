...
# RemoteClaw Auth Widget for Termux
...
# Check auth status
termux-toast "Checking RemoteClaw auth..."

STATUS=$(ssh "$SERVER" '$HOME/remoteclaw/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/remoteclaw/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|REMOTECLAW_EXPIRING|CLAWDBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/remoteclaw/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|REMOTECLAW_EXPIRED|CLAWDBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: RemoteClaw won't work until re-auth"
                ;;
        esac
        ;;
