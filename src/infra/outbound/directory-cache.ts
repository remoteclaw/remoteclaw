...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: RemoteClawConfig | null = null;
  ...
  get(key: string, cfg: RemoteClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: RemoteClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: RemoteClawConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: RemoteClawConfig): void {
