...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.REMOTECLAW_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.REMOTECLAW_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
  ...
  const inlineKeyRaw =
    normalizeNonEmptyString(env.REMOTECLAW_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.REMOTECLAW_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.REMOTECLAW_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
  ...
  try {
    ...
    return {
      ok: false,
      error: `failed reading REMOTECLAW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
