...
into core RemoteClaw yet (or you want to keep optional features out of your main
install).
...
1. **Manifest + discovery**
   RemoteClaw finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `remoteclaw.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native RemoteClaw plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of RemoteClaw reads the registry to expose tools, channels, provider
...
- a company plugin should usually own all of that company's RemoteClaw-facing
  surfaces
...
So if RemoteClaw adds a new domain such as video, the first question is not
...
RemoteClaw also recognizes two compatible external bundle layouts:
...
native RemoteClaw plugin sources. RemoteClaw resolves the marketplace entry first,
...
Today, RemoteClaw treats these as **capability packs**, not native runtime
plugins:
...
Native RemoteClaw plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the RemoteClaw process

Compatible bundles are safer by default because RemoteClaw currently treats them
...
Native RemoteClaw plugins are **TypeScript modules** loaded at runtime via jiti.
...
Native RemoteClaw plugins run **in‑process** with the Gateway, so treat them as trusted code.
...
against explicit surfaces that RemoteClaw understands, validates, and can expose
...
`RemoteClawPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   RemoteClaw can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that RemoteClaw knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `RemoteClawPluginApi` or
  `api.runtime`
...
For model/provider plugins, RemoteClaw uses hooks in this rough order:

1. `catalog`
   Publish provider config into `models.providers` during `models.json`
   generation.
2. built-in/discovered model lookup
   RemoteClaw tries the normal registry/catalog path first.
...
RemoteClaw keeps short in-process caches for:
...
- Bundled extension-specific subpaths are also available:
  ...
  `remoteclaw/plugin-sdk/memory-lancedb`,
...
- Set `REMOTECLAW_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `REMOTECLAW_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `REMOTECLAW_PLUGIN_DISCOVERY_CACHE_MS` and
  `REMOTECLAW_PLUGIN_MANIFEST_CACHE_MS`.
...
- If `plugins.allow` is empty and non-bundled plugins are discoverable, RemoteClaw logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. RemoteClaw blocks candidates when:
...
Each native RemoteClaw plugin must include a `remoteclaw.plugin.json` file in its
...
A plugin directory may include a `package.json` with `remoteclaw.extensions`:
...
Security guardrail: every `remoteclaw.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `remoteclaw plugins install` installs plugin dependencies with
...
contain `{ "entries": [ { "name": "@scope/pkg", "remoteclaw": { "channel": {...}, "install": {...} } } ] }`.
...
If a plugin exports `id`, RemoteClaw uses it but warns when it doesn’t match the
configured id.
...
RemoteClaw augments `uiHints` at runtime based on discovered plugins:
...
`remoteclaw plugins list` shows the top-level format as `remoteclaw` or `bundle`.
...
2. **Non-interactive setup**
   `auth[].runNonInteractive(ctx)` handles `remoteclaw onboard --non-interactive`
   without prompts. Use this when the provider needs custom headless setup
   beyond the built-in simple API-key paths.
3. **Wizard integration**
   `wizard.setup` adds an entry to `remoteclaw onboard`.
...
- `configPatch`: optional `remoteclaw.json` changes
...
explicit method or let RemoteClaw synthesize per-method choices.

RemoteClaw validates provider wizard metadata when the plugin registers:
...
- `run` receives a `ProviderAuthContext` with `prompter`, `runtime`,
  ...
  capture, while `remoteclaw models auth` keeps a tighter prompt surface.
...
- `wizard.setup` adds a provider choice to onboarding surfaces such as
  `remoteclaw onboard` / `remoteclaw setup --wizard`.
...
- `deprecatedProfileIds` lets the provider own `remoteclaw doctor` cleanup for
...
- `config`: The current RemoteClaw config
...
- Main package: `remoteclaw` (this repo)
- Plugins: separate npm packages under `@remoteclaw/*` (example: `@remoteclaw/voice-call`)

Publishing contract:

- Plugin `package.json` must include `remoteclaw.extensions` with one or more entry files.
- Optional: `remoteclaw.setupEntry` may point at a lightweight setup-only entry for disabled or still-unconfigured channel setup.
- Optional: `remoteclaw.startup.deferConfiguredChannelFullLoadUntilAfterListen` may opt a channel plugin into using `setupEntry` during pre-listen gateway startup, but only when that setup entry completely covers the plugin's startup-critical surface.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `remoteclaw plugins install <npm-spec>` uses `npm pack`, extracts into `~/.remoteclaw/extensions/<id>/`, and enables it in config.
...
- CLI: `remoteclaw voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `remoteclaw.extensions` points at the built entrypoint (`dist/index.js`).
