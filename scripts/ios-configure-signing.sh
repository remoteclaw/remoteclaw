...
normalize_bundle_id() {
  ...
  if [[ -z "$raw" ]]; then
    raw="org.remoteclaw.app.test.local"
  fi
  printf '%s\n' "$raw"
}
...
if [[ -z "${bundle_base}" ]]; then
  if [[ "${team_id}" == "${CANONICAL_TEAM_ID}" ]]; then
    bundle_base="org.remoteclaw.app"
  else
    bundle_base="org.remoteclaw.app.test.${bundle_suffix}"
  fi
fi
bundle_base="$(normalize_bundle_id "${bundle_base}")"

share_bundle_id="${REMOTECLAW_IOS_SHARE_BUNDLE_ID:-${bundle_base}.share}"
app_group_id="${REMOTECLAW_IOS_APP_GROUP_ID:-group.${bundle_base}.shared}"
...
code_sign_identity="${REMOTECLAW_IOS_CODE_SIGN_IDENTITY:-Apple Development}"
apns_entitlement_environment="${REMOTECLAW_IOS_APNS_ENTITLEMENT_ENVIRONMENT:-development}"
app_profile="${REMOTECLAW_IOS_APP_PROFILE:-}"
share_profile="${REMOTECLAW_IOS_SHARE_PROFILE:-}"
activity_widget_profile="${REMOTECLAW_IOS_ACTIVITY_WIDGET_PROFILE:-}"
watch_app_profile="${REMOTECLAW_IOS_WATCH_APP_PROFILE:-}"

tmp_file="$(mktemp "${TMPDIR:-/tmp}/remoteclaw-ios-configure-signing.XXXXXX")"
cat >"${tmp_file}" <<EOF
...
// REMOTECLAW_IOS_SHARE_BUNDLE_ID / REMOTECLAW_IOS_APP_GROUP_ID
// REMOTECLAW_IOS_ACTIVITY_WIDGET_BUNDLE_ID
// REMOTECLAW_IOS_WATCH_APP_BUNDLE_ID
// REMOTECLAW_IOS_CODE_SIGN_STYLE / REMOTECLAW_IOS_CODE_SIGN_IDENTITY
// REMOTECLAW_IOS_APNS_ENTITLEMENT_ENVIRONMENT / REMOTECLAW_IOS_APP_PROFILE
// REMOTECLAW_IOS_SHARE_PROFILE / REMOTECLAW_IOS_ACTIVITY_WIDGET_PROFILE
