...
struct IOSGatewayChatTransportTests {
    ...
    @Test func `live routing guard permits an identity still loading`() {
        #expect(RemoteClawChatSessionRoutingContract.expectedValue(
            nil,
    ...
    @Test func `routing contract round trips a delimited legacy main key`() throws {
        let contract = try #require(RemoteClawChatSessionRoutingContract.make(
            scope: "per-sender",
            mainKey: "team|primary",
            defaultAgentID: "main"))
        let components = try #require(RemoteClawChatSessionRoutingContract.parse(contract))
...
struct LocalFixtureChatTransportTests {
    @Test func `sent user turn carries gateway idempotency metadata`() async throws {
        ...
        let decoded = try #require(history.messages).compactMap { payload -> RemoteClawChatMessage? in
