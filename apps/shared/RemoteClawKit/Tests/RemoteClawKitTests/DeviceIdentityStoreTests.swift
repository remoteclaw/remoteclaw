...
struct DeviceIdentityStoreTests {
    ...
    func `device auth store reports failed durable writes`() throws {
        ...
        setenv("REMOTECLAW_STATE_DIR", blocker.path, 1)
    ...
    func `secondary profiles use separate identity and auth files`() throws {
        ...
        // getenv, not ProcessInfo: the trait pins REMOTECLAW_STATE_DIR via setenv and
        // ProcessInfo.environment can serve a stale snapshot on Darwin.
        let stateDirPath = try #require(getenv("REMOTECLAW_STATE_DIR").map { String(cString: $0) })
    ...
    func `does not overwrite an existing unrecognized identity file`() throws {
        ...
        let stored = """
        {
          "schema": "future-remoteclaw-device-identity",
    ...
    // Regression: an explicit REMOTECLAW_STATE_DIR override must never import the
