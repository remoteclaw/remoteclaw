...
extension RemoteClawChatViewModel {
    ...
    func replaceMessages(_ messages: [RemoteClawChatMessage]) {
    ...
    func persistSessionsToCache(_ sessions: [RemoteClawChatSessionEntry]) {
        guard let transcriptCache else { return }
        let previous = pendingCacheWriteTask
        pendingCacheWriteTask = Task.detached {
            await previous?.value
    ...
    func paintFromCacheIfNeeded(session: SessionSnapshot) {
        ...
        if sessions.isEmpty, !hasAppliedLiveSessions {
            Task { [weak self] in
                ...
                self.sessions = RemoteClawChatSessionListOrganizer.organize(cached)
            }
        }
