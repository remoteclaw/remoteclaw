...
describe("readConfiguredLogTail", () => {
  ...
  // Skipped: the fork's `readConfiguredLogTail` (src/logging/log-tail.ts) does
  // not yet integrate the `redactSensitiveLines` / `resolveRedactOptions`
  // surface from `./redact.js`. Upstream wires redaction into log tailing;
  // the fork is behind on that integration. Re-enable when log-tail picks up
  // the redaction call.
  it.skip("applies redaction once per request across all returned lines", async () => {
    const { readConfiguredLogTail } = await import("./log-tail.js");
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-log-tail-"));
    const file = path.join(tempDir, "remoteclaw-2026-01-22.log");
