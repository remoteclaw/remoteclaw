...
Channel support depends on whether that channel is wired through the shared RemoteClaw sender-authorization helpers. Current bundled support includes Discord, Feishu, Google Chat, iMessage, LINE, Mattermost, Microsoft Teams, Nextcloud Talk, Nostr, QQBot, Signal, WhatsApp, Zalo, and Zalo Personal. Static `message.senders` groups are designed to be channel-agnostic, so new message channels should support them by using the shared plugin SDK helpers instead of custom allowlist expansion.
...
```typescript
import { resolveAccessGroupAllowFromState } from "remoteclaw/plugin-sdk/security-runtime";
