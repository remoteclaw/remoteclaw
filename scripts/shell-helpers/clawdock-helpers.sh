#!/usr/bin/env bash
# ClawDock - Docker helpers for RemoteClaw
# Inspired by Simon Willison's "Running RemoteClaw in Docker"
# https://til.simonwillison.net/llms/remoteclaw-docker
#
# Installation:
...
# Common paths to check for RemoteClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/remoteclaw"
  "${HOME}/workspace/remoteclaw"
  "${HOME}/projects/remoteclaw"
  "${HOME}/dev/remoteclaw"
  "${HOME}/code/remoteclaw"
  "${HOME}/src/remoteclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found RemoteClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/remoteclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ RemoteClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/remoteclaw/remoteclaw.git ~/remoteclaw"
    echo "  cd ~/remoteclaw && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^REMOTECLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d remoteclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart remoteclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f remoteclaw-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec remoteclaw-gateway \
...
clawdock-cli() {
  _clawdock_compose run --rm remoteclaw-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build remoteclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "REMOTECLAW_GATEWAY_TOKEN=$token" remoteclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" remoteclaw-gateway \
  ...
  local saved_token
  saved_token=$(_clawdock_compose exec remoteclaw-gateway \
  ...
  _clawdock_compose restart remoteclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm remoteclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec remoteclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'remoteclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for RemoteClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (remoteclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to remoteclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.remoteclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'remoteclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'remoteclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.remoteclaw.org${_CLR_RESET}"
  echo ""
}
