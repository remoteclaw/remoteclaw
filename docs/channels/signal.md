...
- RemoteClaw installed on your server (Linux flow below tested on Ubuntu 24).
- One of:
...
3. Choose one setup path:
   - **Path A (QR link):** `signal-cli link -n "RemoteClaw"` and scan with Signal.
   - **Path B (SMS register):** register a dedicated number with captcha + SMS verification.
4. Configure RemoteClaw and restart the gateway.
5. Send a first DM and approve pairing (`remoteclaw pairing approve signal <CODE>`).
...
2. Link a bot account:
   - `signal-cli link -n "RemoteClaw"` then scan the QR in Signal.
...
4. Configure RemoteClaw, restart gateway, verify channel:

```bash
# If you run the gateway as a user systemd service:
systemctl --user restart remoteclaw-gateway.service
...
5. Pair your DM sender:
   ...
   - Approve code on the server: `remoteclaw pairing approve signal <PAIRING_CODE>`.
...
If you want to manage `signal-cli` yourself (slow JVM cold starts, container init, or shared CPUs), run the daemon separately and point RemoteClaw at it:
...
This skips auto-spawn and the startup wait inside RemoteClaw. For slow starts when auto-spawning, set `channels.signal.startupTimeoutMs`.
...
- Register or link your Signal account inside the container before connecting RemoteClaw.
...
The `apiMode` field controls which protocol RemoteClaw uses:
...
When `apiMode` is `"auto"`, RemoteClaw caches the detected mode for 30 seconds to avoid repeated probes. Container receive is only selected for streaming after `/v1/receive/{account}` upgrades to WebSocket, which requires `MODE=json-rpc`.

Container mode supports the same Signal channel operations as native mode where the container exposes matching APIs: sends, receives, attachments, typing indicators, read/viewed receipts, reactions, groups, and styled text. RemoteClaw translates its native Signal RPC calls into the container's REST payloads, including `group.{base64(internal_id)}` group IDs and `text_mode: "styled"` for formatted text.

Operational notes:

- Use `autoStart: false` with container mode. RemoteClaw should not spawn a native daemon when `apiMode: "container"` is selected.
- Use `MODE=json-rpc` for receiving. `MODE=normal` can make `/v1/about` look healthy, but `/v1/receive/{account}` does not WebSocket-upgrade, so RemoteClaw will not select container receive streaming in `auto` mode.
...
- Approve via:
  - `remoteclaw pairing list signal`
  - `remoteclaw pairing approve signal <CODE>`
...
- **Typing indicators**: RemoteClaw sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, RemoteClaw forwards read receipts for allowed DMs.
...
- Config validation errors after edits: run `remoteclaw doctor --fix`.
...
remoteclaw pairing list signal
pgrep -af signal-cli
grep -i "signal" "/tmp/remoteclaw/remoteclaw-$(date +%Y-%m-%d).log" | tail -20
```
