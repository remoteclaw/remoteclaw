...
describe("Slack native command argument menus", () => {
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(
      Array.from(testHarness.actions.keys()).some(
        (key) => key instanceof RegExp && String(key) === String(/^remoteclaw_cmdarg/),
      ),
    ).toBe(true);
    expect(testHarness.options.has("remoteclaw_cmdarg")).toBe(true);
  ...
  it("shows a button menu when required args are omitted", async () => {
    ...
    expect(actions?.elements?.[0]?.action_id).toBe("remoteclaw_cmdarg_0_0");
    expect(actions?.elements?.[1]?.action_id).toBe("remoteclaw_cmdarg_0_1");
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("remoteclaw_cmdarg");
  ...
  it("shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    expect(element?.action_id).toBe("remoteclaw_cmdarg");
    expect(blockId).toContain("remoteclaw_cmdarg_ext:");
    const token = (blockId ?? "").slice("remoteclaw_cmdarg_ext:".length);
