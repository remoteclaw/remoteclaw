...
function createSourceConfig(): RemoteClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createRuntimeConfig(): RemoteClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
describe("runtime config snapshot writes", () => {
  it("returns the source snapshot when runtime snapshot is active", async () => {
    await withTempHome("remoteclaw-config-runtime-source-", async () => {
  ...
  it("skips source projection for non-runtime-derived configs", async () => {
    await withTempHome("remoteclaw-config-runtime-projection-shape-", async () => {
      const sourceConfig: RemoteClawConfig = {
        ...createSourceConfig(),
        gateway: {
          auth: {
            mode: "token",
      ...
      const runtimeConfig: RemoteClawConfig = {
        ...createRuntimeConfig(),
        gateway: {
          auth: {
            mode: "token",
      ...
      const independentConfig: RemoteClawConfig = {
        models: {
          providers: {
            openai: {
              baseUrl: "https://api.openai.com/v1",
  ...
  it("preserves source secret refs when writeConfigFile receives runtime-resolved config", async () => {
    await withTempHome("remoteclaw-config-runtime-write-", async (home) => {
  ...
  it("refreshes the runtime snapshot after writes so follow-up reads see persisted changes", async () => {
    await withTempHome("remoteclaw-config-runtime-write-refresh-", async (home) => {
      const configPath = path.join(home, ".remoteclaw", "remoteclaw.json");
      const sourceConfig: RemoteClawConfig = {
        models: {
          providers: {
      ...
      const nextRuntimeConfig: RemoteClawConfig = {
        ...runtimeConfig,
  ...
  it("keeps the last-known-good runtime snapshot active while a specialized refresh is pending", async () => {
    await withTempHome("remoteclaw-config-runtime-refresh-pending-", async (home) => {
      const configPath = path.join(home, ".remoteclaw", "remoteclaw.json");
      const sourceConfig = createSourceConfig();
      const runtimeConfig = createRuntimeConfig();
      const nextRuntimeConfig: RemoteClawConfig = {
