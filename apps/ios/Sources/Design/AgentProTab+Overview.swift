...
extension AgentProTab {
    var rosterHeader: some View {
        VStack(alignment: .leading, spacing: 10) {
            RemoteClawAdaptiveHeaderRow(
                ...
                titleFont: RemoteClawType.title2SemiBold,
                subtitleFont: RemoteClawType.subheadMedium,
                subtitleLineLimit: 1)
            {
    ...
    var gatewayToolbarButton: some View {
        if let openSettings {
            ...
            .tint(self.gatewayConnected ? RemoteClawBrand.ok : .secondary)
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 10) {
                self.metricTile(
                    ...
                    color: self.gatewayConnected ? RemoteClawBrand.accent : .secondary,
                    route: .skills)
                self.metricTile(
    ...
    func agentAvatar(_ agent: AgentSummary, state: AgentRosterState) -> some View {
        ZStack(alignment: .bottomTrailing) {
            Text(self.agentBadge(for: agent))
                .font(RemoteClawType.avatar(size: self.agentBadge(for: agent).count > 2 ? 14 : 18))
    ...
    var instancesColor: Color {
        ...
        return (overview?.presence.isEmpty == false) ? RemoteClawBrand.accent : .secondary
    }
    ...
    var cronColor: Color {
        guard self.gatewayConnected else { return .secondary }
        return overview?.cronStatus?.enabled == true ? RemoteClawBrand.accent : .secondary
    }
    ...
    var dreamingColor: Color {
        guard self.gatewayConnected else { return .secondary }
        return overview?.dreaming?.enabled == true ? RemoteClawBrand.accent : .secondary
    }
