...
export const MODULE_ATTESTATIONS = {
  collectRuntimeChannelCapabilities: "live",
} as const;
...
export function collectRuntimeChannelCapabilities(params: {
  ...
  // `resolveChannelPromptCapabilities` was gutted from `./channel-tools.js`
  // (the plugin `agentPrompt.messageToolCapabilities` surface + the
  // native-approval-prompt capability it emitted are absent fork-wide), so
  // channel capabilities now derive solely from `resolveChannelCapabilities`.
  return mergeRuntimeCapabilities(resolveChannelCapabilities(params), threadSpawnCapabilities);
}
