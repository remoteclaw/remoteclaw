...
function createHeartbeatTelegramConfig(storePath: string): RemoteClawConfig {
  return {
    agents: {
      // Fork's resolveMainSessionKey hard-fails on an empty agents.list (it does
      // not fall back to a default agent id like upstream does); seed a minimal
      // agent so the heartbeat sandbox resolves the main session key.
      list: [{ id: "main" }],
      defaults: {
        heartbeat: { every: "30m" },
    ...
    // Point the runner's session store at the sandbox path the test seeds; the
    // fork's default (home-based) store path resolves empty in this sandbox.
...
async function seedHeartbeatTelegramSession(storePath: string, cfg: RemoteClawConfig) {
