...
AUTH_KEY_FLAG=""
MODEL_ID=""
INSTALL_URL="https://remoteclaw.org/install.sh"
HOST_PORT="18425"
...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_REMOTECLAW_BIN="/opt/homebrew/bin/remoteclaw"
GUEST_REMOTECLAW_ENTRY="/opt/homebrew/lib/node_modules/remoteclaw/remoteclaw.mjs"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/remoteclaw-parallels-smoke.XXXXXX)"
BUILD_LOCK_DIR="${TMPDIR:-/tmp}/remoteclaw-parallels-build.lock"
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://remoteclaw.org/install.sh
  ...
  --target-package-spec <npm-spec>
                             ...
                             Example: remoteclaw@2026.3.13-beta.1
...
spawn {*}$cmd
send -- "printf '__REMOTECLAW_READY__\\n'\r"
expect "__REMOTECLAW_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/remoteclaw-prl.sh <<'__REMOTECLAW_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__REMOTECLAW_SCRIPT__\r"
send -- "/bin/bash /tmp/remoteclaw-prl.sh; rc=\$?; rm -f /tmp/remoteclaw-prl.sh; printf '__REMOTECLAW_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1

set rc 1
expect {
  -re {__REMOTECLAW_RC__:(-?[0-9]+)} {
...
resolve_latest_version() {
  ...
  npm view remoteclaw version --userconfig "$(mktemp)"
}
...
export REMOTECLAW_NO_ONBOARD=1
curl -fsSL $install_url_q -o /tmp/remoteclaw-install.sh
bash /tmp/remoteclaw-install.sh${version_arg_q}
$GUEST_REMOTECLAW_BIN --version
EOF
)"
}
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_REMOTECLAW_BIN" --version
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/remoteclaw-main-$short_head.tgz"
...
start_server() {
  ...
  ) >/tmp/remoteclaw-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_bundle_permissions() {
  ...
  cmd="$(cat <<EOF
root=\$($npm_q root -g); check_path() { local path="\$1"; [ -e "\$path" ] || return 0; local perm perm_oct; perm=\$(/usr/bin/stat -f '%OLp' "\$path"); perm_oct=\$((8#\$perm)); if (( perm_oct & 0002 )); then echo "world-writable install artifact: \$path (\$perm)" >&2; exit 1; fi; }; check_path "\$root/remoteclaw"; check_path "\$root/remoteclaw/extensions"; if [ -d "\$root/remoteclaw/extensions" ]; then while IFS= read -r -d '' extension_dir; do check_path "\$extension_dir"; done < <(/usr/bin/find "\$root/remoteclaw/extensions" -mindepth 1 -maxdepth 1 -type d -print0); fi
EOF
)"
...
verify_gateway() {
  guest_current_user_cli "$GUEST_REMOTECLAW_BIN" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_current_user_cli "$GUEST_REMOTECLAW_BIN" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_cli "$GUEST_REMOTECLAW_BIN" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_cli "$GUEST_REMOTECLAW_BIN" gateway status --deep
}

verify_turn() {
  guest_current_user_cli "$GUEST_REMOTECLAW_BIN" models set "$MODEL_ID"
  guest_current_user_cli \
    /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" \
    "$GUEST_REMOTECLAW_BIN" agent \
    --agent main \
...
resolve_dashboard_url() {
  local dashboard_url
  dashboard_url="$(
    guest_current_user_cli "$GUEST_REMOTECLAW_BIN" dashboard --no-open \
      | awk '/^Dashboard URL: / { sub(/^Dashboard URL: /, ""); print; exit }'
  )"
  dashboard_url="${dashboard_url//$'\r'/}"
  dashboard_url="${dashboard_url//$'\n'/}"
  [[ -n "$dashboard_url" ]] || {
    echo "failed to resolve dashboard URL from remoteclaw dashboard --no-open" >&2
    return 1
  }
  printf '%s\n' "$dashboard_url"
}
...
while [ \$SECONDS -lt \$deadline ]; do
  if curl -fsSL "\$dashboard_http_url" >/tmp/remoteclaw-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>RemoteClaw Control</title>' /tmp/remoteclaw-dashboard-smoke.html >/dev/null; then
      if grep -F '<remoteclaw-app></remoteclaw-app>' /tmp/remoteclaw-dashboard-smoke.html >/dev/null; then
        dashboard_ready=1
        break
      fi
    fi
...
grep -F '<title>RemoteClaw Control</title>' /tmp/remoteclaw-dashboard-smoke.html >/dev/null
grep -F '<remoteclaw-app></remoteclaw-app>' /tmp/remoteclaw-dashboard-smoke.html >/dev/null
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/remoteclaw-discord-token <<'__REMOTECLAW_TOKEN__'
$DISCORD_TOKEN_VALUE
__REMOTECLAW_TOKEN__
cat >/tmp/remoteclaw-discord-guilds.json <<'__REMOTECLAW_GUILDS__'
$guilds_json
__REMOTECLAW_GUILDS__
token="\$(tr -d '\n' </tmp/remoteclaw-discord-token)"
guilds_json="\$(cat /tmp/remoteclaw-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_REMOTECLAW_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_REMOTECLAW_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_REMOTECLAW_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_REMOTECLAW_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_REMOTECLAW_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_REMOTECLAW_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_REMOTECLAW_ENTRY channels status --probe --json
rm -f /tmp/remoteclaw-discord-token /tmp/remoteclaw-discord-guilds.json
EOF
)"
  prlctl exec "$VM_NAME" --current-user /usr/bin/env \
    PATH=/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin \
...
matches = re.findall(r"RemoteClaw [^\r\n]+ \([0-9a-f]{7,}\)", text)
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "remoteclaw-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "remoteclaw-main-upgrade.tgz"
