...
function isGatewayStartLine(line: string) {
  return line.includes("compose") && line.includes(" up -d") && line.includes("remoteclaw-gateway");
}
...
describe("scripts/docker/setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    expect(log).toContain(
      "run --rm --no-deps --entrypoint node remoteclaw-gateway dist/index.js onboard --mode local --no-install-daemon",
    );
    expect(log).toContain(
  ...
  it("offline mode reuses a preloaded local image without build or pull", async () => {
    ...
    const result = runDockerSetup(
      activeSandbox,
      {
        REMOTECLAW_IMAGE: "ghcr.io/remoteclaw/remoteclaw:latest",
    ...
    expect(result.stdout).toContain(
      "Using preloaded Docker image: ghcr.io/remoteclaw/remoteclaw:latest",
    );

    const lines = await readDockerLogLines(activeSandbox);
    const log = lines.join("\n");
    expect(log).toContain("image inspect ghcr.io/remoteclaw/remoteclaw:latest");
  ...
  it("offline mode fails before setup when the main image is missing", async () => {
    ...
    const result = runDockerSetup(
      activeSandbox,
      {
        REMOTECLAW_IMAGE: "ghcr.io/remoteclaw/remoteclaw:offline",
        DOCKER_STUB_MISSING_IMAGES: "ghcr.io/remoteclaw/remoteclaw:offline",
      },
      ["--offline"],
    );
    ...
    expect(result.stderr).toContain(
      "Offline Docker setup requires preloaded image ghcr.io/remoteclaw/remoteclaw:offline",
    );

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("image inspect ghcr.io/remoteclaw/remoteclaw:offline");
    expect(log).not.toMatch(/^build /m);
    expect(log).not.toMatch(/^pull /m);
    expect(log).not.toContain("up -d remoteclaw-gateway");
  });
  ...
  it("offline sandbox stays disabled when its configured image is missing", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const defaultImage = "registry.example/remoteclaw-sandbox:approved";
      const agentImage = " registry.example/remoteclaw-sandbox:agent ";
      const result = runDockerSetup(
        activeSandbox,
        {
          REMOTECLAW_SANDBOX: "1",
      ...
      const lines = await readDockerLogLines(activeSandbox);
      const log = lines.join("\n");
      expect(log).toContain("image inspect remoteclaw:local");
      expect(log).not.toContain(`image inspect ${defaultImage}`);
      expect(log).toContain(`image inspect ${agentImage} host=unix://${socketPath}`);
      expect(log).not.toContain("image inspect remoteclaw-sandbox:bookworm-slim");
      expect(log).not.toMatch(/^build /m);
      expect(log).not.toMatch(/^pull /m);
  ...
  it("offline sandbox validates only effective Docker and browser images", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const defaultImage = "registry.example/remoteclaw-sandbox:default";
      const browserImage = "registry.example/remoteclaw-sandbox-browser:default";
      const ignoredImages = [
        "registry.example/remoteclaw-sandbox:ssh",
        "registry.example/remoteclaw-sandbox:shared-agent",
        "registry.example/remoteclaw-sandbox-browser:shared-agent",
        "registry.example/remoteclaw-sandbox:off",
        "registry.example/remoteclaw-sandbox-browser:denied",
      ];
      const result = runDockerSetup(
        activeSandbox,
        {
  ...
  it("offline sandbox rejects an incompatible browser image", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const browserImage = "registry.example/remoteclaw-sandbox-browser:stale";
      const result = runDockerSetup(
        activeSandbox,
        {
          REMOTECLAW_SANDBOX: "1",
  ...
  it("keeps offline policy when sandbox config writes fail and the gateway rolls back", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      ...
      expect(log).toContain(
        "run --pull never --rm --no-deps remoteclaw-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("--force-recreate remoteclaw-gateway"));
      expect(forceRecreateLine).toBe(
        `compose compose -f ${join(activeSandbox.rootDir, "docker-compose.yml")} up -d --pull never --no-build --force-recreate remoteclaw-gateway`,
      );
      expect(forceRecreateLine).not.toContain("docker-compose.sandbox.yml");
      expect(log).toContain(
        `image inspect remoteclaw-sandbox:bookworm-slim host=unix://${socketPath}`,
      );
      expectOfflineComposePolicy(lines);
      await expectMissingPath(join(activeSandbox.rootDir, "docker-compose.sandbox.yml"));
    });
  ...
  it("skips onboarding when REMOTECLAW_SKIP_ONBOARDING is set", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("REMOTECLAW_SKIP_ONBOARDING=1");
  });

  it("treats REMOTECLAW_SKIP_ONBOARDING=0 as disabled and runs onboarding", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      REMOTECLAW_SKIP_ONBOARDING: "0",
    });
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toMatch(/REMOTECLAW_SKIP_ONBOARDING=\n/);
  });
  ...
  it("keeps bundled plugin runtime deps on a Docker-managed volume", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(
      compose.match(/REMOTECLAW_PLUGIN_STAGE_DIR: \/var\/lib\/remoteclaw\/plugin-runtime-deps/g),
    ).toHaveLength(2);
    expect(
      compose.match(
        /- remoteclaw-plugin-runtime-deps:\/var\/lib\/remoteclaw\/plugin-runtime-deps/g,
      ),
    ).toHaveLength(2);
    expect(compose).toContain("\nvolumes:\n  remoteclaw-plugin-runtime-deps:\n");
  });
});
