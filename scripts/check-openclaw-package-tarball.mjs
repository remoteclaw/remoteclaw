...
function usage() {
  return "Usage: node scripts/check-remoteclaw-package-tarball.mjs <remoteclaw.tgz>";
}
...
function parseArgs(argv) {
  ...
  if (tarball.startsWith("-")) {
    throw new Error(`Unknown RemoteClaw package tarball check option: ${tarball}`);
  }
  const extraArg = args[1]?.trim();
  if (extraArg) {
    throw new Error(`Unexpected RemoteClaw package tarball check argument: ${extraArg}`);
  }
...
if (!fs.existsSync(tarball)) {
  fail(`RemoteClaw package tarball does not exist: ${tarball}`);
}

const phaseTimingsEnabled = process.env.REMOTECLAW_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
function runPhase(label, action) {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-remoteclaw-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-package-tarball-"));
try {
...
if (errors.length > 0) {
  fs.rmSync(extractDir, { recursive: true, force: true });
  fail(`RemoteClaw package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`RemoteClaw package tarball integrity warning: ${warning}`);
}
fs.rmSync(extractDir, { recursive: true, force: true });
console.log("RemoteClaw package tarball integrity passed.");
