---
summary: "CLI reference for `remoteclaw qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
remoteclaw qr
remoteclaw qr --setup-code-only
remoteclaw qr --json
remoteclaw qr --remote
remoteclaw qr --url wss://gateway.example/ws
```
