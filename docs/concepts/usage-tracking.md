...
- CLI: `remoteclaw status --usage` prints a full per-provider breakdown.
- CLI: `remoteclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
