...
export function resolveRemoteClawUserDataDir(
  profileName = DEFAULT_REMOTECLAW_BROWSER_PROFILE_NAME,
) {
...
export function buildRemoteClawChromeLaunchArgs(params: {
...
export async function launchRemoteClawChrome(
  ...
  const spawnOnce = () => {
    const args = buildRemoteClawChromeLaunchArgs({
      resolved,
      profile,
      userDataDir,
    });
  ...
  if (needsDecorate) {
    try {
      decorateRemoteClawProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
  ...
  try {
    ...
    log.warn(`remoteclaw browser clean-exit prefs failed: ${String(err)}`);
  }
