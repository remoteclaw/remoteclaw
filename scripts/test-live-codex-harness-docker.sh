...
CODEX_HARNESS_SETUP_TIMEOUT_SECONDS="$(remoteclaw_live_read_positive_int_env REMOTECLAW_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS 180)"
CODEX_HARNESS_TARGET_COUNT=1
if [[ -n "${REMOTECLAW_LIVE_CODEX_HARNESS_TARGETS:-}" ]]; then
  IFS=',' read -r -a CODEX_HARNESS_TARGET_ITEMS <<<"$REMOTECLAW_LIVE_CODEX_HARNESS_TARGETS"
...
CODEX_HARNESS_DOCKER_RUN_TIMEOUT="${REMOTECLAW_LIVE_CODEX_HARNESS_DOCKER_RUN_TIMEOUT:-$((2100 * CODEX_HARNESS_TARGET_COUNT))s}"
TEMP_DIRS=()
DOCKER_USER="${REMOTECLAW_DOCKER_USER:-node}"
DOCKER_HOME_MOUNT=()
DOCKER_TRUSTED_HARNESS_MOUNT=()
DOCKER_TRUSTED_HARNESS_CONTAINER_DIR=""
DOCKER_CACHE_CONTAINER_DIR="/tmp/remoteclaw-cache"
DOCKER_CLI_TOOLS_CONTAINER_DIR="/tmp/remoteclaw-npm-global"
DOCKER_EXTRA_ENV_FILES=()
DOCKER_AUTH_PRESTAGED=0

remoteclaw_live_codex_harness_is_ci() {
  remoteclaw_live_is_ci
}
...
remoteclaw_live_prepare_bind_dir_for_container_user "$CLI_TOOLS_DIR"
...
if [[ "$CODEX_HARNESS_AUTH_MODE" == "api-key" ]]; then
  ...
  CONFIG_DIR="$(mktemp -d "${RUNNER_TEMP:-/tmp}/remoteclaw-docker-config.XXXXXX")"
  WORKSPACE_DIR="$(mktemp -d "${RUNNER_TEMP:-/tmp}/remoteclaw-docker-workspace.XXXXXX")"
...
if [[ "$CODEX_HARNESS_AUTH_MODE" != "api-key" ]]; then
  ...
  done < <(remoteclaw_live_collect_auth_files_from_csv "openai")
fi
...
run_setup_command() {
  local timeout_value="${REMOTECLAW_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS:?missing live Codex harness setup timeout seconds}s"
  local timeout_bin=""
...
run_codex_harness_target() {
  ...
  export REMOTECLAW_LIVE_CODEX_HARNESS_MODEL="$model"
  export REMOTECLAW_LIVE_CODEX_HARNESS_THINKING="$thinking"
...
if [ -n "${REMOTECLAW_LIVE_CODEX_HARNESS_TARGETS:-}" ]; then
  IFS=',' read -r -a harness_targets <<<"$REMOTECLAW_LIVE_CODEX_HARNESS_TARGETS"
  ...
  run_codex_harness_target \
    "${REMOTECLAW_LIVE_CODEX_HARNESS_MODEL:-codex/gpt-5.5}" \
    "${REMOTECLAW_LIVE_CODEX_HARNESS_THINKING:-low}"
fi
EOF
...
echo "==> Thinking: ${REMOTECLAW_LIVE_CODEX_HARNESS_THINKING:-low}"
echo "==> Targets: ${REMOTECLAW_LIVE_CODEX_HARNESS_TARGETS:-single model}"
...
DOCKER_RUN_ARGS=()
remoteclaw_live_init_docker_run_args DOCKER_RUN_ARGS "$CODEX_HARNESS_DOCKER_RUN_TIMEOUT"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e REMOTECLAW_LIVE_CODEX_HARNESS_TARGETS="${REMOTECLAW_LIVE_CODEX_HARNESS_TARGETS:-}" \
  -e REMOTECLAW_LIVE_CODEX_HARNESS_THINKING="${REMOTECLAW_LIVE_CODEX_HARNESS_THINKING:-low}" \
  ...
  -e REMOTECLAW_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS="$CODEX_HARNESS_SETUP_TIMEOUT_SECONDS" \
...
remoteclaw_live_append_array DOCKER_RUN_ARGS EXTERNAL_AUTH_MOUNTS
remoteclaw_live_append_array DOCKER_RUN_ARGS PROFILE_MOUNT
DOCKER_RUN_ARGS+=(\
  "$LIVE_IMAGE_NAME" \
  -lc "$LIVE_TEST_CMD")
if [[ "${REMOTECLAW_LIVE_CODEX_HARNESS_DEBUG:-}" == "1" ]]; then
