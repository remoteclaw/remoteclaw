...
import { POSIX_REMOTECLAW_TMP_DIR, resolvePreferredRemoteClawTmpDir } from "./tmp-remoteclaw-dir.js";
...
describe("resolvePreferredRemoteClawTmpDir", () => {
  it("prefers /tmp/remoteclaw when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_REMOTECLAW_TMP_DIR);
  ...
  it("prefers /tmp/remoteclaw when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_REMOTECLAW_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });
