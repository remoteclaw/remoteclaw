...
extension RemoteClawChatViewModel {
    ...
    private func thinkingPickerIsAvailable(
        for session: RemoteClawChatSessionEntry?,
        modelChoice: RemoteClawChatModelChoice?) -> Bool
    {
    ...
    private struct ThinkingLevelOptionsResolution {
        let options: [RemoteClawChatThinkingLevelOption]
        let isGatewayMetadata: Bool
    }

    private func resolvedThinkingLevelOptions(
        for currentSession: RemoteClawChatSessionEntry?) -> ThinkingLevelOptionsResolution
    {
    ...
    private func selectedModelChoice(
        for currentSession: RemoteClawChatSessionEntry?) -> RemoteClawChatModelChoice?
    {
    ...
    private func modelChoice(modelID: String?, provider: String?) -> RemoteClawChatModelChoice? {
    ...
    private static func sessionModelMatchesDefaults(
        _ session: RemoteClawChatSessionEntry,
        defaults: RemoteClawChatSessionsDefaults?) -> Bool
    {
    ...
    private static func normalizedThinkingLevelOptions(
        _ levels: [RemoteClawChatThinkingLevelOption]?) -> [RemoteClawChatThinkingLevelOption]?
    {
        ...
        return Self.dedupedThinkingOptions(
            levels.compactMap { level in
                ...
                return RemoteClawChatThinkingLevelOption(id: id, label: label.isEmpty ? id : label)
            })
    }

    private static func thinkingOptions(from labels: [String]?) -> [RemoteClawChatThinkingLevelOption]? {
        guard let labels else { return nil }
        return Self.dedupedThinkingOptions(
            labels.compactMap { label in
                guard let id = Self.normalizedThinkingLevel(label) else { return nil }
                let trimmed = label.trimmingCharacters(in: .whitespacesAndNewlines)
                return RemoteClawChatThinkingLevelOption(id: id, label: trimmed.isEmpty ? id : trimmed)
            })
    }

    static func withCurrentThinkingOption(
        _ options: [RemoteClawChatThinkingLevelOption],
        current: String) -> [RemoteClawChatThinkingLevelOption]
    {
        guard !options.contains(where: { $0.id == current }) else { return options }
        return options + [RemoteClawChatThinkingLevelOption(id: current, label: current)]
    }

    private static func dedupedThinkingOptions(
        _ options: [RemoteClawChatThinkingLevelOption]) -> [RemoteClawChatThinkingLevelOption]
    {
        var result: [RemoteClawChatThinkingLevelOption] = []
        var seen = Set<String>()
