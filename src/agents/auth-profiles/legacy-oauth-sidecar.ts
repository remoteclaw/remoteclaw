...
const LEGACY_OAUTH_SECRET_KEY_ENV = "REMOTECLAW_AUTH_PROFILE_SECRET_KEY";
const LEGACY_OAUTH_SECRET_KEYCHAIN_SERVICE = "RemoteClaw Auth Profile Secrets";
...
function buildLegacyOAuthSecretKey(seed: string): Buffer {
  ...
  return createHash("sha256").update(`remoteclaw:auth-profile-oauth:${seed}`).digest();
}
...
function resolveLegacyOAuthSecretKeyFileCandidates(env: NodeJS.ProcessEnv): string[] {
  if (process.platform === "win32") {
    ...
    return uniquePaths([
      root ? path.join(root, "RemoteClaw", LEGACY_OAUTH_SECRET_KEY_FILE_NAME) : undefined,
      home
        ? path.join(home, ".remoteclaw-auth-profile-secrets", LEGACY_OAUTH_SECRET_KEY_FILE_NAME)
        : undefined,
    ]);
  }
...
function resolveLegacyOAuthSecretKeySeeds(env: NodeJS.ProcessEnv): string[] {
  ...
  if (env.NODE_ENV === "test" && env.VITEST === "true") {
    addSeed("remoteclaw-test-oauth-profile-secret-key");
  }
...
function emitKeychainOnlyMigrationHintOnce(profileId: string): void {
  ...
  log.warn(
    "Legacy Codex OAuth credentials are stored only in macOS Keychain on this host. " +
      "Headless paths cannot prompt for Keychain access; run `remoteclaw doctor --fix` " +
