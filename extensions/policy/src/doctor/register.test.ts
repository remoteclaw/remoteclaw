...
} from "remoteclaw/plugin-sdk/health";
...
function cfgWithPolicy(settings: Record<string, unknown> = {}): RemoteClawConfig {
  return {
    plugins: {
      entries: {
        policy: {
...
function ctx(configPath: string, cfg: RemoteClawConfig = {}): HealthCheckContext {
  return {
    mode: "lint",
    runtime: {
      log() {},
...
function repairCtx(configPath: string, cfg: RemoteClawConfig = {}): HealthRepairContext {
  return {
    ...ctx(configPath, cfg),
    mode: "fix",
  };
...
describe("registerPolicyDoctorChecks", () => {
    ...
    expect(evidence.toolPosture).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          kind: "alsoAllow",
          entries: ["exec"],
          source: "oc://remoteclaw.config/tools/alsoAllow",
        }),
        expect.objectContaining({
          id: "reviewer-alsoAllow",
          kind: "alsoAllow",
          entries: ["write"],
          source: "oc://remoteclaw.config/agents/list/#0/tools/alsoAllow",
        }),
      ]),
    );
    expect(result.findings).toEqual(
  ...
  it("reports configured channels denied by policy", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        ...
        severity: "error",
        path: "remoteclaw config",
        ocPath: "oc://remoteclaw.config/channels/telegram",
        target: "oc://remoteclaw.config/channels/telegram",
  ...
  it("reports model providers denied by policy", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        ...
        severity: "error",
        ocPath: "oc://remoteclaw.config/models/providers/openrouter",
        requirement: "oc://policy.jsonc/models/providers/deny",
      }),
      expect.objectContaining({
        checkId: "policy/models-denied-provider",
        severity: "error",
        ocPath: "oc://remoteclaw.config/agents/defaults/model",
        requirement: "oc://policy.jsonc/models/providers/deny",
      }),
    ]);
  });
  ...
  it("normalizes model provider refs before deny policy comparison", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/models-denied-provider",
        severity: "error",
        ocPath: "oc://remoteclaw.config/models/providers/aws-bedrock",
        requirement: "oc://policy.jsonc/models/providers/deny",
      }),
      expect.objectContaining({
        checkId: "policy/models-denied-provider",
  ...
  it("reports model refs outside the policy allowlist", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        ...
        severity: "error",
        ocPath: "oc://remoteclaw.config/agents/defaults/model/fallbacks/#0",
  ...
  it("reports model allowlist keys outside the policy allowlist", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/models-unapproved-provider",
        severity: "error",
        ocPath: 'oc://remoteclaw.config/agents/defaults/models/"openrouter/*"',
        requirement: "oc://policy.jsonc/models/providers/allow",
      }),
    ]);
  });
  ...
  it("reports per-agent model allowlist keys outside the policy allowlist", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/models-unapproved-provider",
        severity: "error",
        ocPath: 'oc://remoteclaw.config/agents/list/#0/models/"openrouter/*"',
        requirement: "oc://policy.jsonc/models/providers/allow",
      }),
    ]);
  });
  ...
  it("reports configured model providers outside the policy allowlist", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/models-unapproved-provider",
        severity: "error",
        ocPath: "oc://remoteclaw.config/models/providers/anthropic",
        requirement: "oc://policy.jsonc/models/providers/allow",
      }),
    ]);
  });
  ...
  it("reports non-default agent model refs outside the policy allowlist", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/models-unapproved-provider",
        severity: "error",
        ocPath: "oc://remoteclaw.config/agents/defaults/subagents/model",
        requirement: "oc://policy.jsonc/models/providers/allow",
      }),
    ]);
  });
  ...
  it("reports per-agent model refs outside the policy allowlist", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/models-denied-provider",
        severity: "error",
        ocPath: "oc://remoteclaw.config/agents/list/#0/model/primary",
        requirement: "oc://policy.jsonc/models/providers/deny",
      }),
    ]);
  });
  ...
  it("reports MCP servers denied by policy", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        ...
        severity: "error",
        ocPath: "oc://remoteclaw.config/mcp/servers/untrusted",
  ...
  it("preserves MCP server casing for deny rules", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/mcp-denied-server",
        severity: "error",
        ocPath: "oc://remoteclaw.config/mcp/servers/DocsServer",
        requirement: "oc://policy.jsonc/mcp/servers/deny",
      }),
    ]);
  });
  ...
  it("reports MCP servers outside the policy allowlist", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        ...
        severity: "error",
        ocPath: "oc://remoteclaw.config/mcp/servers/remote",
  ...
  it("quotes MCP server ids with whitespace in policy evidence paths", () => {
    ...
    expect(server).toEqual(
      expect.objectContaining({
        id: "Outlook Graph",
        source: 'oc://remoteclaw.config/mcp/servers/"Outlook Graph"',
      }),
    );
  });
  ...
  it("reports private-network SSRF settings denied by policy", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        ...
        severity: "error",
        ocPath: "oc://remoteclaw.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
        requirement: "oc://policy.jsonc/network/privateNetwork/allow",
      }),
      expect.objectContaining({
        checkId: "policy/network-private-access-enabled",
        severity: "error",
        ocPath: "oc://remoteclaw.config/tools/web/fetch/ssrfPolicy/allowIpv6UniqueLocalRange",
        requirement: "oc://policy.jsonc/network/privateNetwork/allow",
      }),
    ]);
  });
    ...
    expect(evidence.secrets).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          refSource: "exec",
          refProvider: "rogue",
          source: "oc://remoteclaw.config/models/providers/openai/request/auth/token",
        }),
        expect.objectContaining({
          kind: "input",
          provenance: "secretRef",
          refSource: "exec",
          refProvider: "rogue",
          source: "oc://remoteclaw.config/models/providers/openai/request/tls/passphrase",
        }),
        expect.objectContaining({
          kind: "input",
          provenance: "secretRef",
          refSource: "exec",
          refProvider: "rogue",
          source: 'oc://remoteclaw.config/models/providers/"z.ai"/headers/Authorization',
        }),
        expect.objectContaining({
          kind: "input",
          provenance: "secretRef",
          ...
          refProvider: "rogue",
          source:
            "oc://remoteclaw.config/plugins/entries/acpx/config/mcpServers/github/env/GITHUB_TOKEN",
        }),
        expect.objectContaining({
          kind: "input",
          provenance: "secretRef",
          refSource: "exec",
          refProvider: "rogue",
          source: "oc://remoteclaw.config/tools/media/models/#0/request/auth/token",
        }),
        expect.objectContaining({
          kind: "input",
          provenance: "secretRef",
          refSource: "exec",
          refProvider: "rogue",
          source: "oc://remoteclaw.config/tools/media/models/#0/request/tls/key",
        }),
        expect.objectContaining({
          kind: "input",
          provenance: "secretRef",
          refSource: "exec",
          refProvider: "rogue",
          source: "oc://remoteclaw.config/tools/media/audio/request/auth/token",
        }),
        expect.objectContaining({
          kind: "input",
          provenance: "secretRef",
          refSource: "exec",
          refProvider: "rogue",
          source: "oc://remoteclaw.config/tools/media/image/models/#0/request/auth/token",
        }),
      ]),
    );
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          checkId: "policy/secrets-unmanaged-provider",
          ocPath: "oc://remoteclaw.config/models/providers/openai/request/auth/token",
        }),
        expect.objectContaining({
          checkId: "policy/secrets-denied-provider-source",
          ocPath: "oc://remoteclaw.config/models/providers/openai/request/auth/token",
        ...
        expect.objectContaining({
          checkId: "policy/secrets-unmanaged-provider",
          ocPath: "oc://remoteclaw.config/models/providers/openai/request/tls/passphrase",
        }),
        expect.objectContaining({
          checkId: "policy/secrets-denied-provider-source",
          ocPath: "oc://remoteclaw.config/models/providers/openai/request/tls/passphrase",
        ...
        expect.objectContaining({
          checkId: "policy/secrets-unmanaged-provider",
          ocPath: 'oc://remoteclaw.config/models/providers/"z.ai"/headers/Authorization',
        }),
        expect.objectContaining({
          checkId: "policy/secrets-denied-provider-source",
          ocPath:
        ...
        expect.objectContaining({
          checkId: "policy/secrets-unmanaged-provider",
          ocPath: "oc://remoteclaw.config/tools/media/models/#0/request/auth/token",
        }),
        expect.objectContaining({
          checkId: "policy/secrets-denied-provider-source",
          ocPath: "oc://remoteclaw.config/tools/media/audio/request/auth/token",
        }),
        expect.objectContaining({
          checkId: "policy/secrets-unmanaged-provider",
          ocPath: "oc://remoteclaw.config/tools/media/image/models/#0/request/auth/token",
        }),
        expect.objectContaining({
          checkId: "policy/secrets-unmanaged-provider",
          ocPath: "oc://remoteclaw.config/tools/media/models/#0/request/tls/key",
        }),
      ]),
    );
  });
    ...
    expect(evidence.secrets).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          refSource: "env",
          refProvider: "vault",
          source: "oc://remoteclaw.config/models/providers/openai/apiKey",
        }),
      ]),
    );
    expect(result.findings).toEqual([]);
  ...
  it("reports SecretRefs that use a managed provider alias with the wrong source", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/secrets-unmanaged-provider",
        severity: "error",
        ocPath: "oc://remoteclaw.config/models/providers/openai/apiKey",
        requirement: "oc://policy.jsonc/secrets/requireManagedProviders",
      }),
    ]);
  });
  ...
  it("checks configured channel encryptKey SecretRefs", async () => {
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          checkId: "policy/secrets-unmanaged-provider",
          ocPath: "oc://remoteclaw.config/channels/feishu/encryptKey",
        }),
        expect.objectContaining({
          checkId: "policy/secrets-denied-provider-source",
          ocPath: "oc://remoteclaw.config/channels/feishu/encryptKey",
  ...
  it("reports agent workspace posture denied by policy", async () => {
    ...
    expect(evidence.agentWorkspace).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          sandboxMode: "all",
          sandboxEnabled: true,
          source: "oc://remoteclaw.config/agents/defaults/sandbox/workspaceAccess",
        }),
        expect.objectContaining({
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          severity: "error",
          ocPath: "oc://remoteclaw.config/agents/defaults/sandbox/workspaceAccess",
          requirement: "oc://policy.jsonc/agents/workspace/allowedAccess",
        }),
        expect.objectContaining({
          checkId: "policy/agents-tool-not-denied",
          severity: "error",
          ocPath: "oc://remoteclaw.config/tools/deny",
    ...
    expect(result.findings).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          checkId: "policy/agents-tool-not-denied",
          ocPath: "oc://remoteclaw.config/agents/list/#0/tools/deny",
        }),
      ]),
    );
  });
    ...
    expect(evidence.agentWorkspace).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          denied: true,
          source: "oc://remoteclaw.config/tools/sandbox/tools/deny",
        }),
        expect.objectContaining({
          id: "locked-tool-apply_patch",
          denied: true,
          source: "oc://remoteclaw.config/agents/list/#0/tools/sandbox/tools/deny",
        }),
      ]),
    );
    expect(result.findings).toEqual([]);
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          ocPath: "oc://remoteclaw.config/agents/defaults/sandbox/mode",
          requirement: "oc://policy.jsonc/agents/workspace/allowedAccess",
        }),
        expect.objectContaining({
          checkId: "policy/agents-tool-not-denied",
  ...
  it("reports global and agent-scoped workspace claims independently", async () => {
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          checkId: "policy/agents-workspace-access-denied",
          ocPath: "oc://remoteclaw.config/agents/list/#0/sandbox/workspaceAccess",
          requirement: "oc://policy.jsonc/agents/workspace/allowedAccess",
        }),
        expect.objectContaining({
          checkId: "policy/agents-workspace-access-denied",
    ...
    expect(result.findings).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ocPath: "oc://remoteclaw.config/agents/list/#1/sandbox/workspaceAccess",
        }),
      ]),
    );
  });
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          ocPath: "oc://remoteclaw.config/agents/list/#0/tools/exec/host",
  ...
  it("applies main agent-scoped claims to implicit default agent posture", async () => {
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          checkId: "policy/tools-exec-host-unapproved",
          ocPath: "oc://remoteclaw.config/tools/exec/host",
    ...
    expect(evidence.toolPosture).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          kind: "profile",
          value: "coding",
          source: "oc://remoteclaw.config/tools/profile",
        }),
        expect.objectContaining({
          id: "reviewer-exec-security",
          kind: "execSecurity",
          value: "deny",
          source: "oc://remoteclaw.config/agents/list/#0/tools/exec/security",
        }),
        expect.objectContaining({
          id: "tools-elevated-allow-from-whatsapp",
          kind: "elevatedAllowFrom",
          entries: ["+15550000001", "15550000002"],
          source: "oc://remoteclaw.config/tools/elevated/allowFrom/whatsapp",
        }),
      ]),
    );
    expect(result.findings).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          ocPath: "oc://remoteclaw.config/tools/elevated/enabled",
  ...
  it("reports global and agent-scoped tool claims independently", async () => {
    ...
    expect(result.findings).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ocPath: "oc://remoteclaw.config/agents/list/#1/tools/exec/host",
        }),
      ]),
    );
  });
  ...
  it("reports global and agent-scoped alsoAllow drift", async () => {
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          ocPath: "oc://remoteclaw.config/tools/alsoAllow",
        ...
        expect.objectContaining({
          checkId: "policy/tools-also-allow-missing",
          ocPath: "oc://remoteclaw.config/agents/list/#0/tools/alsoAllow",
    ...
    expect(result.findings).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          requirement: "oc://policy.jsonc/scopes/sebby/tools/alsoAllow/expected",
          ocPath: "oc://remoteclaw.config/agents/list/#1/tools/alsoAllow",
        }),
      ]),
    );
  });
    ...
    expect(evidence.toolPosture).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "tools-deny",
          kind: "deny",
          entries: ["group:remoteclaw"],
          source: "oc://remoteclaw.config/tools/deny",
        }),
      ]),
    );
    expect(result.findings).toEqual([]);
    ...
    expect(evidence.toolPosture).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          value: false,
          source: "oc://remoteclaw.config/tools/elevated/enabled",
        }),
      ]),
    );
    expect(result.findings).toEqual([]);
    ...
    expect(evidence.toolPosture).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          kind: "execSecurity",
          value: "deny",
          source: "oc://remoteclaw.config/tools/exec/security",
        }),
      ]),
    );
    expect(result.findings).toEqual([]);
  ...
  it("reports preserved Tailscale Funnel routes when policy denies Funnel exposure", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/gateway-tailscale-funnel",
        severity: "error",
        ocPath: "oc://remoteclaw.config/gateway/tailscale/preserveFunnel",
        requirement: "oc://policy.jsonc/gateway/exposure/allowTailscaleFunnel",
      }),
    ]);
  });
  ...
  it("reports valid non-loopback custom bind hosts", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/gateway-non-loopback-bind",
        severity: "error",
        ocPath: "oc://remoteclaw.config/gateway/customBindHost",
        requirement: "oc://policy.jsonc/gateway/exposure/allowNonLoopbackBind",
      }),
    ]);
  });
  ...
  it("reports configured gateway remote URLs when remote mode is active", async () => {
    ...
    expect(result.findings).toEqual([
      ...
      expect.objectContaining({
        checkId: "policy/gateway-remote-enabled",
        severity: "error",
        ocPath: "oc://remoteclaw.config/gateway/remote/url",
        requirement: "oc://policy.jsonc/gateway/remote/allow",
      }),
    ]);
  });
    ...
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          checkId: "policy/gateway-http-url-fetch-unrestricted",
          severity: "error",
          ocPath: "oc://remoteclaw.config/gateway/http/endpoints/responses/files/allowUrl",
          requirement: "oc://policy.jsonc/gateway/http/requireUrlAllowlists",
        }),
        expect.objectContaining({
          checkId: "policy/gateway-http-url-fetch-unrestricted",
          severity: "error",
          ocPath: "oc://remoteclaw.config/gateway/http/endpoints/responses/images/allowUrl",
          requirement: "oc://policy.jsonc/gateway/http/requireUrlAllowlists",
        }),
      ]),
    );
  ...
  it("reports auth profiles missing required metadata or using unapproved modes", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        ...
        severity: "error",
        ocPath: "oc://remoteclaw.config/auth/profiles/missingMode",
        requirement: "oc://policy.jsonc/auth/profiles/requireMetadata",
      }),
      expect.objectContaining({
        checkId: "policy/auth-profile-unapproved-mode",
        severity: "error",
        ocPath: "oc://remoteclaw.config/auth/profiles/oauth",
