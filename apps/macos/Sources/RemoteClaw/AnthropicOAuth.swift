...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (RemoteClaw token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: RemoteClawOAuthStore.AnthropicOAuthStatus = RemoteClawOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "org.remoteclaw", category: "anthropic-oauth")
...
enum RemoteClawOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let remoteclawOAuthDirEnv = "REMOTECLAW_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "RemoteClaw OAuth token file not found"
            case .unreadableFile: "RemoteClaw OAuth token file not readable"
            case .invalidJSON: "RemoteClaw OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in RemoteClaw OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "RemoteClaw OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.remoteclawOAuthDirEnv]?
        ...
        return home.appendingPathComponent(".remoteclaw", isDirectory: true)
