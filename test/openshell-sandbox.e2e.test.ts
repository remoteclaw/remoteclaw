...
RUN echo "remoteclaw-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && REMOTECLAW_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: REMOTECLAW_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(REMOTECLAW_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-openshell-e2e-"));
      ...
      const gatewayName = `remoteclaw-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `remoteclaw-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        browser: {
          enabled: false,
          image: "remoteclaw-browser",
          containerPrefix: "remoteclaw-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
      ...
      try {
        ...
        expect(stdout).toContain("remoteclaw-openshell-e2e");
