...
const EXPECTED_REPOSITORY_URL = "https://github.com/remoteclaw/remoteclaw";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "remoteclaw") {
    errors.push(`package.json name must be "remoteclaw"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.remoteclaw !== "remoteclaw.mjs") {
    errors.push(
      `package.json bin.remoteclaw must be "remoteclaw.mjs"; found "${pkg.bin?.remoteclaw ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`remoteclaw-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `remoteclaw-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
