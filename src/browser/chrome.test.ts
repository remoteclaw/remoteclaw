...
describe("browser chrome profile decoration", () => {
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateRemoteClawProfile(userDataDir, { color: DEFAULT_REMOTECLAW_BROWSER_COLOR });
    ...
    expect(def.shortcut_name).toBe(DEFAULT_REMOTECLAW_BROWSER_PROFILE_NAME);
    ...
    const marker = await fsp.readFile(
      path.join(userDataDir, ".remoteclaw-profile-decorated"),
      "utf-8",
    );
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    expect(profile.name).toBe(DEFAULT_REMOTECLAW_BROWSER_PROFILE_NAME);
  });
});
...
describe("browser chrome helpers", () => {
    ...
    await stopRemoteClawChrome(
      {
        proc,
        cdpPort: 12345,
      } as unknown as Parameters<typeof stopRemoteClawChrome>[0],
      10,
    );
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopRemoteClawChrome sends SIGTERM and returns once CDP is down", async () => {
  ...
  it("stopRemoteClawChrome escalates to SIGKILL when CDP stays reachable", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
