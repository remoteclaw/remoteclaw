...
export function installBrowserControlServerHooks() {
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.REMOTECLAW_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.REMOTECLAW_GATEWAY_TOKEN;
    } else {
      process.env.REMOTECLAW_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.REMOTECLAW_GATEWAY_PASSWORD;
    } else {
      process.env.REMOTECLAW_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
