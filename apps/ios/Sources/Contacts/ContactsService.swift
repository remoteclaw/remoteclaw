...
final class ContactsService: ContactsServicing {
    ...
    func search(params: RemoteClawContactsSearchParams) async throws -> RemoteClawContactsSearchPayload {
        ...
        return RemoteClawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: RemoteClawContactsAddParams) async throws -> RemoteClawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return RemoteClawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return RemoteClawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> RemoteClawContactPayload {
        RemoteClawContactPayload(
