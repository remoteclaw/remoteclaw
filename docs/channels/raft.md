...
read_when:
  - You want to connect RemoteClaw to a Raft workspace
...
Raft support connects an RemoteClaw agent to a Raft External Agent through the local
...
```bash
remoteclaw plugins install @remoteclaw/raft
...
- The Raft CLI installed on the same host as the RemoteClaw Gateway.
...
```bash
remoteclaw channels setup raft
```
...
7. Starts one serialized RemoteClaw agent turn for each accepted wake.

The bridge owns Raft delivery retries and reconnects. The RemoteClaw turn receives
only a wake notice, not a copied Raft message body. It uses the CLI to read
pending messages and to send its response:

```bash
raft --profile remoteclaw message check
raft --profile remoteclaw message send
```

<Note>
Raft is not a normal push-message transport. RemoteClaw does not automatically
...
Check that RemoteClaw can find the CLI and has a configured profile:

```bash
remoteclaw channels status --probe
remoteclaw plugins inspect raft --runtime --json
```
