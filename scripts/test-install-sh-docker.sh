...
ROOT_DIR="${REMOTECLAW_INSTALL_SMOKE_SOURCE_DIR:-$HARNESS_ROOT}"
...
DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${REMOTECLAW_INSTALL_SMOKE_DOCKER_COMMAND_TIMEOUT:-600s}}"
INSTALL_SMOKE_DOCKER_RUN_TIMEOUT="${REMOTECLAW_INSTALL_SMOKE_DOCKER_RUN_TIMEOUT:-2700s}"
...
resolve_default_smoke_platform() {
  local host_arch
  if [[ -n "${REMOTECLAW_INSTALL_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$REMOTECLAW_INSTALL_SMOKE_PLATFORM"
    return
  fi
  host_arch="$(uname -m)"
...
const parsed = JSON.parse(raw);
const budgetOverride = process.env.REMOTECLAW_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES;
const budgetBytes = budgetOverride ? Number(budgetOverride) : undefined;
if (budgetOverride && !Number.isFinite(budgetBytes)) {
  throw new Error(
    `REMOTECLAW_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES must be numeric, got ${JSON.stringify(
      budgetOverride,
    )}`,
  );
}
...
PACKAGE_NAME="${REMOTECLAW_INSTALL_PACKAGE:-remoteclaw}"
SKIP_NONROOT="${REMOTECLAW_INSTALL_SMOKE_SKIP_NONROOT:-0}"
SKIP_SMOKE_IMAGE_BUILD="${REMOTECLAW_INSTALL_SMOKE_SKIP_IMAGE_BUILD:-0}"
SKIP_NONROOT_IMAGE_BUILD="${REMOTECLAW_INSTALL_NONROOT_SKIP_IMAGE_BUILD:-0}"
SKIP_UPDATE="${REMOTECLAW_INSTALL_SMOKE_SKIP_UPDATE:-0}"
SKIP_NPM_GLOBAL="${REMOTECLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"
SKIP_FRESHNESS="${REMOTECLAW_INSTALL_SMOKE_SKIP_FRESHNESS:-0}"
FRESHNESS_INSTALL_URL="${REMOTECLAW_INSTALL_SMOKE_FRESHNESS_INSTALL_URL:-file:///tmp/remoteclaw-install.sh}"
# npm min-release-age is days; 10000 keeps the control failure independent of normal release cadence.
FRESHNESS_MIN_RELEASE_AGE="${REMOTECLAW_INSTALL_FRESHNESS_MIN_RELEASE_AGE:-10000}"
FRESHNESS_NPM_VERSION="${REMOTECLAW_INSTALL_FRESHNESS_NPM_VERSION:-11.14.1}"
UPDATE_BASELINE_VERSION="${REMOTECLAW_INSTALL_SMOKE_UPDATE_BASELINE:-latest}"
UPDATE_PACKAGE_SPEC="${REMOTECLAW_INSTALL_SMOKE_UPDATE_PACKAGE_SPEC:-}"
UPDATE_DIST_IMAGE="${REMOTECLAW_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"
UPDATE_SKIP_LOCAL_BUILD="${REMOTECLAW_INSTALL_SMOKE_UPDATE_SKIP_LOCAL_BUILD:-0}"
UPDATE_HOST_ALIAS="${REMOTECLAW_INSTALL_SMOKE_UPDATE_HOST:-host.docker.internal}"
UPDATE_PORT="${REMOTECLAW_INSTALL_SMOKE_UPDATE_PORT:-}"
UPDATE_EXPECT_VERSION="${REMOTECLAW_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"
LATEST_DIR="$(mktemp -d)"
...
NPM_CACHE_DIR="${REMOTECLAW_INSTALL_SMOKE_NPM_CACHE_DIR:-}"
NPM_CACHE_OWNED=0
NPM_CACHE_PREPARED=0
NPM_CACHE_DOCKER_ARGS=()
INSTALL_SCRIPT_DOCKER_ARGS=(
  -v "$ROOT_DIR/scripts/install.sh:/tmp/remoteclaw-install.sh:ro"
  -v "$ROOT_DIR/scripts/install-cli.sh:/tmp/remoteclaw-install-cli.sh:ro"
)
SMOKE_RUNNER_ENV_ARGS=()

for env_name in \
  REMOTECLAW_INSTALL_ALLOW_LEGACY_UPDATE_WARNING \
  REMOTECLAW_INSTALL_SELF_UPDATE_WARNING_FIXED_VERSION \
  REMOTECLAW_INSTALL_SMOKE_COMMAND_TIMEOUT \
  REMOTECLAW_INSTALL_SMOKE_HEARTBEAT_INTERVAL \
  REMOTECLAW_INSTALL_SMOKE_PREVIOUS \
  REMOTECLAW_INSTALL_SMOKE_SKIP_PREVIOUS; do
  env_value="${!env_name:-}"
...
prepare_update_tarball() {
  ...
  if [[ -n "$UPDATE_PACKAGE_SPEC" ]]; then
    ...
    package_tgz="$(
      node "$HARNESS_ROOT/scripts/package-remoteclaw-for-docker.mjs" "${package_args[@]}"
    )"
    UPDATE_TGZ_FILE="$(basename "$package_tgz")"
  fi
  if [[ -z "$UPDATE_PACKAGE_SPEC" ]]; then
    node "$HARNESS_ROOT/scripts/check-remoteclaw-package-tarball.mjs" \
...
if [[ "$SKIP_UPDATE" == "1" ]]; then
  echo "==> Skip update smoke (REMOTECLAW_INSTALL_SMOKE_SKIP_UPDATE=1)"
else
  prepare_update_tarball
  prepare_update_host_access
  prepare_npm_cache
  ...
  run_install_smoke_container --rm -t \
    ...
    -e REMOTECLAW_INSTALL_FRESH_VERSION="$UPDATE_EXPECT_VERSION" \
    -e REMOTECLAW_INSTALL_FRESH_TAG_URL="$FRESH_TAG_URL" \
    -e REMOTECLAW_INSTALL_LATEST_OUT="/out/latest" \
  ...
  run_install_smoke_container --rm -t \
    ...
    -e REMOTECLAW_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
    -e REMOTECLAW_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
    -e REMOTECLAW_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
    -e REMOTECLAW_NO_ONBOARD=1 \
    -e REMOTECLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
  ...
  if [[ "$SKIP_NPM_GLOBAL" == "1" ]]; then
    echo "==> Skip direct npm global smoke (REMOTECLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1)"
  else
...
if [[ "$SKIP_FRESHNESS" == "1" ]]; then
  echo "==> Skip installer npm freshness smoke (REMOTECLAW_INSTALL_SMOKE_SKIP_FRESHNESS=1)"
else
  prepare_npm_cache
  ...
  run_install_smoke_container --rm -t \
    ...
    "${INSTALL_SCRIPT_DOCKER_ARGS[@]}" \
    ${SMOKE_RUNNER_ENV_ARGS[@]+"${SMOKE_RUNNER_ENV_ARGS[@]}"} \
    -e REMOTECLAW_INSTALL_URL="$FRESHNESS_INSTALL_URL" \
    -e REMOTECLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e REMOTECLAW_INSTALL_SMOKE_MODE=freshness \
    -e REMOTECLAW_INSTALL_FRESHNESS_VERSION="${REMOTECLAW_INSTALL_FRESHNESS_VERSION:-latest}" \
    -e REMOTECLAW_INSTALL_FRESHNESS_MIN_RELEASE_AGE="$FRESHNESS_MIN_RELEASE_AGE" \
    -e REMOTECLAW_INSTALL_FRESHNESS_NPM_VERSION="$FRESHNESS_NPM_VERSION" \
    -e REMOTECLAW_NO_ONBOARD=1 \
    -e REMOTECLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
...
if [[ "$SKIP_NONROOT" == "1" ]]; then
  echo "==> Skip non-root installer smoke (REMOTECLAW_INSTALL_SMOKE_SKIP_NONROOT=1)"
else
...
if [[ "${REMOTECLAW_INSTALL_SMOKE_SKIP_CLI:-0}" == "1" ]]; then
  echo "==> Skip CLI installer smoke (REMOTECLAW_INSTALL_SMOKE_SKIP_CLI=1)"
  exit 0
fi
