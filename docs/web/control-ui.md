...
reconnect. RemoteClaw keeps the old approval active, blocks the broader reconnect,
...
`/__remoteclaw/control-ui-config.json`. That endpoint is gated by the same
...
- Talk to OpenAI Realtime directly from the browser via WebRTC. The Gateway
  ...
  `remoteclaw_agent_consult` tool calls back through `chat.send` for the larger
  configured RemoteClaw model.
...
- In the Chat composer, the Talk control is the waves button next to the
  ...
  `Asking RemoteClaw...` while a realtime tool call is consulting the configured
