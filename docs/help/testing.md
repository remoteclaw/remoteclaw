...
Convex-managed live Telegram lane as parallel jobs. `RemoteClaw Release Checks`
...
- `pnpm test:docker:npm-telegram-live`
  - Installs a published RemoteClaw package in Docker, runs installed-package
  ...
  - Defaults to `REMOTECLAW_NPM_TELEGRAM_PACKAGE_SPEC=remoteclaw@beta`.
...
- `pnpm remoteclaw qa matrix`
  ...
  - This QA host is repo/dev-only today. Packaged RemoteClaw installs do not ship
    `qa-lab`, so they do not expose `remoteclaw qa`.
  ...
  - Uses the pinned stable Tuwunel image `ghcr.io/matrix-construct/tuwunel:v1.5.1` by default. Override with `REMOTECLAW_QA_MATRIX_TUWUNEL_IMAGE` when you need to test a different image.
  ...
  - Emits progress by default and enforces a hard run timeout with `REMOTECLAW_QA_MATRIX_TIMEOUT_MS` (default 30 minutes). Cleanup is bounded by `REMOTECLAW_QA_MATRIX_CLEANUP_TIMEOUT_MS` and failures include the recovery `docker compose ... down --remove-orphans` command.
...
- the `remoteclaw qa` command root
...
- how `remoteclaw qa <runner>` is mounted beneath the shared `qa` root
...
4. Mount the runner as `remoteclaw qa <runner>` instead of registering a competing root command.
   Runner plugins should declare `qaRunners` in `remoteclaw.plugin.json` and export a matching `qaRunnerCliRegistrations` array from `runtime-api.ts`.
...
- `test:docker:all` builds the live Docker image once via `test:docker:live-build`, then reuses it for the live Docker lanes. It also builds one shared `scripts/e2e/Dockerfile` image via `test:docker:e2e-build` and reuses it for the E2E container smoke runners that exercise the built app. The aggregate uses a weighted local scheduler: `REMOTECLAW_DOCKER_ALL_PARALLELISM` controls process slots, while resource caps keep heavy live, npm-install, and multi-service lanes from all starting at once. Defaults are 10 slots, `REMOTECLAW_DOCKER_ALL_LIVE_LIMIT=6`, `REMOTECLAW_DOCKER_ALL_NPM_LIMIT=8`, and `REMOTECLAW_DOCKER_ALL_SERVICE_LIMIT=7`; tune `REMOTECLAW_DOCKER_ALL_WEIGHT_LIMIT` or `REMOTECLAW_DOCKER_ALL_DOCKER_LIMIT` only when the Docker host has more headroom. The runner performs a Docker preflight by default, removes stale RemoteClaw E2E containers, prints status every 30 seconds, stores successful lane timings in `.artifacts/docker-tests/lane-timings.json`, and uses those timings to start longer lanes first on later runs. Use `REMOTECLAW_DOCKER_ALL_DRY_RUN=1` to print the weighted lane manifest without building or running Docker.
...
- Npm tarball onboarding/channel/agent smoke: `pnpm test:docker:npm-onboard-channel-agent` installs the packed RemoteClaw tarball globally in Docker, configures OpenAI via env-ref onboarding plus Telegram by default, verifies doctor repairs activated plugin runtime deps, and runs one mocked OpenAI agent turn. Reuse a prebuilt tarball with `REMOTECLAW_NPM_ONBOARD_PACKAGE_TGZ=/path/to/remoteclaw-*.tgz`, skip the host rebuild with `REMOTECLAW_NPM_ONBOARD_HOST_BUILD=0`, or switch channel with `REMOTECLAW_NPM_ONBOARD_CHANNEL=discord`.
...
```bash
REMOTECLAW_DOCKER_E2E_IMAGE=remoteclaw-docker-e2e:local pnpm test:docker:e2e-build
REMOTECLAW_DOCKER_E2E_IMAGE=remoteclaw-docker-e2e:local REMOTECLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels
```
