...
- App Store release also switches the app to `RemoteClawPushTransport=relay`, `RemoteClawPushDistribution=official`, `RemoteClawPushAPNsEnvironment=production`, and a production `aps-environment` entitlement.
...
- Release builds default to `https://ios-push-relay.remoteclaw.org`.
- Optional custom relay override: `REMOTECLAW_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
4. Optional: set a custom official relay URL for the build. If unset, the release flow uses `https://ios-push-relay.remoteclaw.org`.

```bash
export REMOTECLAW_PUSH_RELAY_BASE_URL=https://relay.example.com
```
...
- Local/manual builds default to `RemoteClawPushTransport=direct`, `RemoteClawPushDistribution=local`, and a development `aps-environment` entitlement.
...
- Debug builds default to `RemoteClawPushAPNsEnvironment=sandbox`; Release builds default to `production`.
