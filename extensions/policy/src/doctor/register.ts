...
import { normalizeAgentId } from "remoteclaw/plugin-sdk/routing";
import { isRecord, uniqueStrings } from "remoteclaw/plugin-sdk/string-coerce-runtime";
import {
...
const policySecretsUnmanagedProviderCheck: HealthCheck = {
  ...
  kind: "plugin",
  description:
    "RemoteClaw config SecretRefs use configured secret providers when policy requires managed providers.",
  source: "policy",
  async detect(ctx) {
...
const policySecretsDeniedProviderSourceCheck: HealthCheck = {
  ...
  kind: "plugin",
  description:
    "RemoteClaw config secret providers and SecretRefs do not use sources denied by policy.",
  source: "policy",
  async detect(ctx) {
...
const policyAuthProfileInvalidMetadataCheck: HealthCheck = {
  ...
  kind: "plugin",
  description: "RemoteClaw config auth profiles declare required provider and mode metadata.",
  source: "policy",
  async detect(ctx) {
...
const policyAuthProfileUnapprovedModeCheck: HealthCheck = {
  ...
  kind: "plugin",
  description: "RemoteClaw config auth profile modes stay within the policy allowlist.",
  source: "policy",
  async detect(ctx) {
...
async function evaluatePolicyUncached(ctx: HealthCheckContext): Promise<PolicyEvaluation> {
  ...
  if (
    ...
    findings.push({
      ...
      requirement: "oc://remoteclaw.config/plugins/entries/policy/config/expectedHash",
  ...
  return [
    {
      ...
      requirement: "oc://remoteclaw.config/plugins/entries/policy/config/expectedAttestationHash",
    ...
    if (invalidIndex >= 0) {
      findings.push(
        policyShapeFinding(
          ...
          "Use non-empty source names such as env, file, exec, or remoteclaw.",
        ),
      );
    }
  }
  ...
  return [
    ...new Set(
      findings
        ...
        .map((finding) => finding.ocPath?.match(/^oc:\/\/remoteclaw\.config\/channels\/(.+)$/)?.[1])
