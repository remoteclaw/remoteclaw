...
describe("runPluginPayloadSmokeCheck", () => {
  let tmpRoot: string;
  beforeEach(async () => {
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-payload-smoke-"));
  });
  afterEach(async () => {
  ...
  function resolveTestHostRoot(): string {
    const hostRoot = resolveRemoteClawPackageRootSync({
  ...
  async function linkRemoteClawPeerToHost(dir: string): Promise<void> {
    await fs.mkdir(path.join(dir, "node_modules"), { recursive: true });
    await fs.symlink(
      resolveTestHostRoot(),
      path.join(dir, "node_modules", "remoteclaw"),
      "junction",
    );
  }
  ...
  it("reports ok for a record whose package.json + main file exist", async () => {
    ...
    await writePackage(
      dir,
      { name: "@remoteclaw/discord", main: "dist/index.js" },
      "module.exports = {};",
    );
  ...
  it("reports a failure when the main entry file is missing on disk", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, { name: "@remoteclaw/brave", main: "dist/index.js" });
    const result = await runPluginPayloadSmokeCheck({
      records: { brave: { source: "npm", installPath: dir } },
      env: {},
    });
  ...
  it("accepts a manifest with no main field (RemoteClaw plugins commonly use `exports` or `remoteclaw.extensions`)", async () => {
    const dir = path.join(tmpRoot, "matrix");
    await writePackage(dir, { name: "@remoteclaw/plugin-matrix" });
    const result = await runPluginPayloadSmokeCheck({
  ...
  it("accepts a manifest that declares an existing `remoteclaw.extensions` entry and no `main`", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
      name: "@remoteclaw/brave-plugin",
  ...
  it("reports a failure when `remoteclaw.extensions` contains invalid entries", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
      name: "@remoteclaw/brave-plugin",
      remoteclaw: { extensions: ["./index.js", " "] },
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        detail:
          "Plugin extension entry validation failed: package.json remoteclaw.extensions[1] must be a non-empty string",
      },
    ]);
  });
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        reason: "missing-extension-entry",
        detail:
          "Plugin extension entry validation failed: package.json remoteclaw.extensions is empty",
      },
    ]);
  });
  ...
  it("reports a failure when an remoteclaw peer link is missing", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
        name: "@remoteclaw/codex",
        main: "dist/index.js",
        peerDependencies: { remoteclaw: ">=2026.5.18-beta.1" },
      },
      "export default {};\n",
    );
    ...
    expect(result.failures).toStrictEqual([
      {
        pluginId: "codex",
        installPath: dir,
        reason: "missing-remoteclaw-peer-link",
        detail: `Plugin declares peerDependency "remoteclaw" but peer link audit failed: missing ${path.join(
          dir,
          "node_modules",
          "remoteclaw",
        )}.`,
  ...
  it("reports a failure when an remoteclaw peer link is a stale real directory", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    const stalePeerDir = path.join(dir, "node_modules", "remoteclaw");
  ...
  it("reports a failure when an remoteclaw peer link points at the wrong package root", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    const wrongHostRoot = path.join(tmpRoot, "old-remoteclaw");
    await fs.mkdir(wrongHostRoot, { recursive: true });
    await fs.mkdir(path.join(dir, "node_modules"), { recursive: true });
    await fs.symlink(wrongHostRoot, path.join(dir, "node_modules", "remoteclaw"), "junction");
  ...
  it("accepts an remoteclaw peer link when it resolves to the host package root", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    await linkRemoteClawPeerToHost(dir);
  ...
  it("reports a failure when an `remoteclaw.extensions` entry file is missing", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
      name: "@remoteclaw/brave-plugin",
