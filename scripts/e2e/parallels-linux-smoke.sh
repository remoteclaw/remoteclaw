...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/remoteclaw-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/remoteclaw-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  ...
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/remoteclaw-install.sh
  guest_exec /usr/bin/env OPENCLAW_NO_ONBOARD=1 bash /tmp/remoteclaw-install.sh "${version_args[@]}" --no-onboard
  guest_exec remoteclaw --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec remoteclaw --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" remoteclaw onboard \
    --non-interactive \
    --mode local \
...
verify_local_turn() {
  guest_exec /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" remoteclaw agent \
    --local \
    --agent main \
    --message ping \
    --json
