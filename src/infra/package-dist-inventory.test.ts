...
describe("package dist inventory", () => {
  it("tracks missing and stale dist files", async () => {
    await withTempDir({ prefix: "remoteclaw-dist-inventory-" }, async (packageRoot) => {
  ...
  it("keeps npm-omitted dist artifacts out of the inventory", async () => {
    await withTempDir({ prefix: "remoteclaw-dist-inventory-pack-" }, async (packageRoot) => {
  ...
  it("fails closed when the inventory is missing", async () => {
    await withTempDir({ prefix: "remoteclaw-dist-inventory-missing-" }, async (packageRoot) => {
  ...
  it("rejects symlinked dist entries", async () => {
    await withTempDir({ prefix: "remoteclaw-dist-inventory-symlink-" }, async (packageRoot) => {
