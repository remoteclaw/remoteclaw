...
```bash
remoteclaw agent --agent main --message "hi" --model claude-cli/claude-sonnet-4-6
```
    ...
    defaults: {
      cliBackends: {
        "my-cli": {
          ...
          // from bounded raw RemoteClaw transcript history before compaction.
...
3. **Executes the CLI** with a session id (if supported) so history stays consistent.
   ...
   RemoteClaw session and sends follow-up turns over stream-json stdin.
...
resolver for RemoteClaw skills. When the current skills snapshot includes at least
one selected skill with a materialized path, RemoteClaw passes a temporary Claude
Code plugin with `--plugin-dir` and omits the duplicate RemoteClaw skills catalog
from the appended system prompt. If the snapshot has no materialized plugin
skill, RemoteClaw keeps the prompt catalog as a fallback. Skill env/API key
overrides are still applied by RemoteClaw to the child process environment for the
run.

Claude CLI also has its own noninteractive permission mode. RemoteClaw maps that
to the existing exec policy instead of adding Claude-specific policy config.
For RemoteClaw-managed Claude live sessions, the effective RemoteClaw exec policy is
...
Claude launches normalize that flag to match the effective RemoteClaw exec policy.

The bundled Anthropic `claude-cli` backend also maps RemoteClaw `/think` levels
...
Before RemoteClaw can use the bundled `claude-cli` backend, Claude Code itself
...
- `claude-cli` defaults to `liveSession: "claude-stdio"`, `output: "jsonl"`,
  ...
  exits, RemoteClaw resumes from the stored Claude session id. Stored session
...
- Claude live sessions keep bounded JSONL output guards. Defaults allow up to
  ...
  and `maxTurnLines`; RemoteClaw clamps those settings to 64 MiB and 100,000
  lines.
- Stored CLI sessions are provider-owned continuity. The implicit daily session
  ...
  do.
- Fresh CLI sessions normally reseed only from RemoteClaw's compaction summary
  plus post-compaction tail. To recover short sessions that are invalidated
  before compaction, a backend can opt in with
  `reseedFromRawTranscriptWhenUncompacted: true`. RemoteClaw still keeps raw
...
[`agents.defaults.model.fallbacks`](/concepts/model-failover), RemoteClaw seeds
...
provider would start cold because RemoteClaw's own session transcript is empty
for `claude-cli` runs.
...
- For Gemini CLI JSON output, RemoteClaw reads reply text from `response` and usage
...
- If you override Gemini args to `--output-format json`, RemoteClaw normalizes that
...
Some CLI backends run an agent that compacts its **own** transcript, so RemoteClaw must
...
`ownsNativeCompaction: true`, and RemoteClaw returns a no-op from the compaction path.
...
`contextTokens: 1_000_000` purely to keep RemoteClaw's safeguard from firing on a
...
When a fresh CLI session is seeded from a prior RemoteClaw transcript (for
