...
function useLoggingConfig(name: string, logging: Record<string, unknown>): void {
  ...
  setTestEnvValue("REMOTECLAW_CONFIG_PATH", configPath);
}
...
function readMessageSeq(message: unknown): number | undefined {
  ...
  const meta = (message as Record<string, unknown>)["__remoteclaw"];
...
describe("sessions_history redaction", () => {
  ...
  afterAll(() => {
    if (previousConfigPath === undefined) {
      deleteTestEnvValue("REMOTECLAW_CONFIG_PATH");
    } else {
      setTestEnvValue("REMOTECLAW_CONFIG_PATH", previousConfigPath);
    }
    if (tempDir) {
  ...
  it("uses the oldest visible message for pagination after tool messages are filtered", async () => {
    const tool = createSessionsHistoryTool({
      ...
      callGateway: async <T = Record<string, unknown>>(): Promise<T> =>
        ({
          messages: [
            { role: "tool", content: "hidden", __remoteclaw: { seq: 6 } },
            { role: "assistant", content: "visible", __remoteclaw: { seq: 7 } },
            { role: "assistant", content: "latest", __remoteclaw: { seq: 8 } },
          ],
          offset: 0,
          nextOffset: 5,
          hasMore: true,
