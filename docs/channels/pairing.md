...
reconnects asking for more scopes or a broader role, RemoteClaw keeps the
existing approval as-is and creates a fresh pending upgrade request. Use
`remoteclaw devices list` to compare the currently approved access with the newly
...
- The legacy `node.pair.*` API (CLI: `remoteclaw nodes pending|approve|reject|rename`) is a
