...
Goal: let RemoteClaw sit in WhatsApp groups, wake up only when pinged, and keep that thread separate from the personal DM session.
...
Add a `groupChat` block to `~/.remoteclaw/remoteclaw.json` so display-name pings work even when WhatsApp strips the visual `@` in the text body:
    ...
    list: [
      {
        ...
        groupChat: {
          historyLimit: 50,
          mentionPatterns: ["@?remoteclaw", "\\+?15555550123"],
        },
      },
    ],
  },
...
- Manual smoke:
  - Send an `@remoteclaw` ping in the group and confirm a reply that references the sender name.
...
- Session store entries will appear as `agent:<agentId>:whatsapp:group:<jid>` in the session store (`~/.remoteclaw/agents/<agentId>/sessions/sessions.json` by default); a missing entry just means the group hasn't triggered a run yet.
