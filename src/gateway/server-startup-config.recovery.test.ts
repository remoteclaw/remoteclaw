...
vi.mock("../config/config.js", () => ({
  applyConfigOverrides: vi.fn((config: RemoteClawConfig) => config),
  isNixMode: false,
...
const configPath = "/tmp/remoteclaw-startup-recovery.json";
const validConfig = {
  gateway: {
    mode: "local",
  },
...
function buildSnapshot(params: {
  ...
  return buildTestConfigSnapshot({
    ...
    valid: params.valid,
    config: params.config ?? ({} as RemoteClawConfig),
...
describe("gateway startup config recovery", () => {
  ...
  it("keeps startup validation loud when last-known-good recovery is unavailable", async () => {
    ...
    await expect(
      ...
      `Invalid config at ${configPath}.\ngateway.mode: Expected 'local' or 'remote'\nRun "remoteclaw doctor --fix" to repair, then retry.`,
    );
