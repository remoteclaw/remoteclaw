...
5. **Logging**
   - Coordinator emits `.info` logs in subsystem `org.remoteclaw`, categories `voicewake.overlay` and `voicewake.chime`.
...
- Stream logs while reproducing a sticky overlay:
  ...
  ```bash
  sudo log stream --predicate 'subsystem == "org.remoteclaw" AND category CONTAINS "voicewake"' --level info --style compact
  ```
