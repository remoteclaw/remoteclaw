...
import { writeJsonFileAtomically } from "remoteclaw/plugin-sdk/json-store";
import { resolveRequiredHomeDir, resolveStateDir } from "remoteclaw/plugin-sdk/state-paths";

const MATRIX_MIGRATION_SNAPSHOT_DIRNAME = "remoteclaw-migrations";
...
export async function maybeCreateMatrixMigrationSnapshot(params: {
  ...
  outputDir?: string;
  createBackupArchive?: typeof import("remoteclaw/plugin-sdk/runtime").createBackupArchive;
  log?: { info?: (message: string) => void; warn?: (message: string) => void };
}): Promise<MatrixMigrationSnapshotResult> {
  const env = params.env ?? process.env;
  const createBackupArchive =
    params.createBackupArchive ?? (await import("remoteclaw/plugin-sdk/runtime")).createBackupArchive;
