...
import { isMiniMaxModelNotFoundErrorMessage, isModelNotFoundErrorMessage } from "./live-model-errors.js";
...
describe("live model error helpers", () => {
  it("detects generic model-not-found messages", () => {
    ...
    expect(
      isModelNotFoundErrorMessage("HTTP 400 not_found_error: model: claude-3-5-haiku-20241022 (request_id: req_123)"),
    ).toBe(true);
  ...
  it("detects bare minimax 404 page-not-found responses", () => {
    ...
    expect(isMiniMaxModelNotFoundErrorMessage("request ended without sending any chunks")).toBe(false);
  });
});
