...
const workspaceHarness = createCommandWorkspaceHarness("remoteclaw-command-plugins-");

async function createClaudeBundlePlugin(params: { workspaceDir: string; pluginId: string }) {
  const pluginDir = path.join(params.workspaceDir, ".remoteclaw", "extensions", params.pluginId);
...
function buildCfg(): RemoteClawConfig {
  return {
    commands: {
      text: true,
      plugins: true,
...
describe("handleCommands /plugins", () => {
  ...
  it("lists discovered plugins and shows plugin details", async () => {
    await withTempHome("remoteclaw-command-plugins-home-", async () => {
