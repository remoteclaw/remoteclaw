...
- Loopback first: the Gateway WS defaults to `ws://127.0.0.1:18789`. The wizard generates a gateway token by default, even for loopback. For tailnet access, run `remoteclaw gateway --bind tailnet --token ...` because tokens are required for non-loopback binds.
