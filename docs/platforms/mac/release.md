---
summary: "RemoteClaw macOS release checklist (Sparkle feed, packaging, signing)"
read_when:
  - Cutting or validating a RemoteClaw macOS release
  - Updating the Sparkle appcast or feed assets
title: "macOS Release"
---

# RemoteClaw macOS release (Sparkle)
...
- Notary credentials (keychain profile or API key) for `xcrun notarytool` if you want Gatekeeper-safe DMG/zip distribution.
  - We use a Keychain profile named `remoteclaw-notary`, created from App Store Connect API key env vars in your shell profile:
    - `APP_STORE_CONNECT_API_KEY_P8`, `APP_STORE_CONNECT_KEY_ID`, `APP_STORE_CONNECT_ISSUER_ID`
    - `echo "$APP_STORE_CONNECT_API_KEY_P8" | sed 's/\\n/\n/g' > /tmp/remoteclaw-notary.p8`
    - `xcrun notarytool store-credentials "remoteclaw-notary" --key /tmp/remoteclaw-notary.p8 --key-id "$APP_STORE_CONNECT_KEY_ID" --issuer "$APP_STORE_CONNECT_ISSUER_ID"`
...
ditto -c -k --sequesterRsrc --keepParent dist/RemoteClaw.app dist/RemoteClaw-2026.3.12.zip

# Optional: build a styled DMG for humans (drag to /Applications)
scripts/create-dmg.sh dist/RemoteClaw.app dist/RemoteClaw-2026.3.12.dmg
...
ditto -c -k --keepParent apps/macos/.build/release/RemoteClaw.app.dSYM dist/RemoteClaw-2026.3.12.dSYM.zip
```
...
```bash
SPARKLE_PRIVATE_KEY_FILE=/path/to/ed25519-private-key scripts/make_appcast.sh dist/RemoteClaw-2026.3.12.zip https://raw.githubusercontent.com/remoteclaw/remoteclaw/main/appcast.xml
```
...
- Upload `RemoteClaw-2026.3.12.zip` (and `RemoteClaw-2026.3.12.dSYM.zip`) to the GitHub release for tag `v2026.3.12`.
