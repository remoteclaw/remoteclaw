 ...
 *
 * @see https://github.com/remoteclaw/remoteclaw/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /remoteclaw\.json|remoteclaw gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumed = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify remoteclaw.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
