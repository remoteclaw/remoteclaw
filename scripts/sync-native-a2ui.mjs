...
// Keeps the native RemoteClawKit Canvas A2UI resources in sync with the plugin-owned bundle.
...
async function withFreshBundleCheckSource(sourceDir, run) {
  const tempDir = await fs.mkdtemp(path.join(tmpdir(), "remoteclaw-a2ui-native-check-"));
  try {
    ...
    bundleA2ui(rootDir, {
      ...process.env,
      REMOTECLAW_A2UI_BUNDLE_OUT: path.join(checkSourceDir, "a2ui.bundle.js"),
      REMOTECLAW_A2UI_BUNDLE_HASH_FILE: path.join(tempDir, ".bundle.hash"),
    });
