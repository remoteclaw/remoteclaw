...
public struct RemoteClawVoiceNoteRecording: Equatable, Sendable {
...
@MainActor
@Observable
public final class RemoteClawVoiceNoteRecorder {
    public enum State: Equatable {
        case idle
        case requestingPermission
        case recording(startedAt: Date, fileURL: URL)
        case finished(recording: RemoteClawVoiceNoteRecording)
        case staging(recording: RemoteClawVoiceNoteRecording)
    ...
    public convenience init() {
        self.init(capture: RemoteClawVoiceNoteAudioCapture())
    }

    /// Creates a recorder with an injectable audio capture seam.
    public init(
        capture: any VoiceNoteAudioCapture,
        durationLimit: TimeInterval = RemoteClawVoiceNoteRecorder.maximumDurationSeconds,
    ...
    public var completedRecording: RemoteClawVoiceNoteRecording? {
    ...
    func claimCompletedRecording() -> RemoteClawVoiceNoteRecording? {
        guard case let .finished(recording) = self.state else { return nil }
    ...
    func completeStaging(_ recording: RemoteClawVoiceNoteRecording) {
    ...
    @discardableResult
    public func finish() -> RemoteClawVoiceNoteRecording? {
        guard case let .recording(_, fileURL) = self.state else { return nil }

        self.timerTask?.cancel()
        self.timerTask = nil
        let duration = max(0, self.capture.stop())
        let recording = RemoteClawVoiceNoteRecording(fileURL: fileURL, durationSeconds: duration)
...
@MainActor
public final class RemoteClawVoiceNoteAudioCapture: NSObject, VoiceNoteAudioCapture, AVAudioRecorderDelegate {
    ...
    public func start(url: URL) throws {
        do {
            ...
            guard recorder.record() else {
                throw NSError(
                    domain: "RemoteClawVoiceNoteAudioCapture",
                    code: 1,
...
func remoteClawVoiceNoteDurationLabel(_ durationSeconds: TimeInterval) -> String {
