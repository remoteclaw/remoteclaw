...
The bundled ACPX extension is a thin RemoteClaw wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `remoteclaw/acpx`, not in this extension.
...
Use this flow when RemoteClaw needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `remoteclaw/acpx` repo first.
2. In RemoteClaw, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild RemoteClaw and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for RemoteClaw ACP validation.
...
If a change feels like shared ACP runtime behavior instead of RemoteClaw-specific glue, move it to `remoteclaw/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
