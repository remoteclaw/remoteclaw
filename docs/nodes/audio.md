...
When audio understanding is enabled (or auto-detected), RemoteClaw:
...
If you have not configured models and `tools.media.audio.enabled` is not `false`, RemoteClaw auto-detects in this order and stops at the first working option:
...
- CLI `args` should use `{{MediaPath}}` for the local audio file path. Run `remoteclaw doctor --fix` to migrate deprecated `{input}` placeholders from older `audio.transcription.command` configs (retired key: `audio.transcription`, replaced by `tools.media.audio.models`).
...
Lowercase variables take precedence over uppercase; `NO_PROXY`/`no_proxy` entries (hostnames, `*.suffix`, or `host:port`) bypass the proxy. If no proxy env vars are set, direct egress is used. If proxy setup fails (malformed URL), RemoteClaw logs a warning and falls back to direct fetch.

## Mention detection in groups

When `requireMention: true` is set for a group chat, RemoteClaw transcribes audio **before** checking for mentions. This lets voice notes pass the mention gate even when the message has no text body.

**How it works:**

1. If a voice message has no text body and the group requires mentions, RemoteClaw performs a preflight transcription of the first audio attachment.
