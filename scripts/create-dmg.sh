...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "RemoteClaw")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "RemoteClaw")
...
DMG_TEMP="$(mktemp -d /tmp/remoteclaw-dmg.XXXXXX)"
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/remoteclaw-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/remoteclaw-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/remoteclaw-dmg-limits.txt
