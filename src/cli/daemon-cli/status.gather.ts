...
function readGatewayTokenEnv(env: Record<string, string | undefined>): string | undefined {
  return (
    trimToUndefined(env.REMOTECLAW_GATEWAY_TOKEN) ?? trimToUndefined(env.CLAWDBOT_GATEWAY_TOKEN)
  );
}

async function resolveDaemonProbePassword(params: {
  daemonCfg: RemoteClawConfig;
  ...
  const envPassword = trimToUndefined(params.mergedDaemonEnv.REMOTECLAW_GATEWAY_PASSWORD);
  if (envPassword) {
    return envPassword;
  }
...
export async function gatherDaemonStatus(
  ...
  const rpc = opts.probe
    ? await probeGatewayStatus({
        ...
        token:
          opts.rpc.token ||
          mergedDaemonEnv.REMOTECLAW_GATEWAY_TOKEN ||
