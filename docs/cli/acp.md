...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an RemoteClaw Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`remoteclaw acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to RemoteClaw channel
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include RemoteClaw parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                              |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the RemoteClaw gateway or agent instead.                                                                                                                                   |
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run RemoteClaw through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive
`remoteclaw acp` over stdio with an ACP JSON-RPC client. Cover `initialize`,
...
Avoid using `remoteclaw gateway call sessions.list` as the only ACP proof. That
...
it to drive an RemoteClaw Gateway session.
...
If you want ACPX-backed sessions to see RemoteClaw plugin tools or selected
...
[RemoteClaw tools MCP bridge](/tools/acp-agents-setup#remoteclaw-tools-mcp-bridge).
...
pull contextual information from an RemoteClaw agent without scraping a terminal.
...
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`REMOTECLAW_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
