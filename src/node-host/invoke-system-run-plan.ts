...
function resolvePosixShellScriptOperandIndex(argv: string[]): number | null {
  ...
  for (let i = 1; i < argv.length; i += 1) {
    ...
    if (!afterDoubleDash && token.startsWith("-")) {
      const [flag] = token.toLowerCase().split("=", 2);
      if (POSIX_SHELL_OPTIONS_WITH_VALUE.has(flag) && !token.includes("=")) {
        i += 1;
      }
      continue;
    }
...
function hasRubyUnsafeApprovalFlag(argv: string[]): boolean {
  ...
  for (let i = 1; i < argv.length; i += 1) {
    ...
    if (token === "-I" || token === "-r" || token.startsWith("-I") || token.startsWith("-r")) {
      return true;
    }
...
function resolveMutableFileOperandIndex(argv: string[], cwd: string | undefined): number | null {
  ...
  if (executable === "bun") {
    const bunIndex = resolveBunScriptOperandIndex({ argv: unwrapped.argv, cwd });
    if (bunIndex !== null) {
      return unwrapped.baseIndex + bunIndex;
    }
  }
  if (executable === "deno") {
    const denoIndex = resolveDenoRunScriptOperandIndex({ argv: unwrapped.argv, cwd });
    if (denoIndex !== null) {
...
function shellPayloadNeedsStableBinding(shellCommand: string, cwd: string | undefined): boolean {
  ...
  const snapshot = resolveMutableFileOperandSnapshotSync({ argv, cwd, shellCommand: null });
  if (!snapshot.ok) {
    return true;
  }
...
export function resolveMutableFileOperandSnapshotSync(params: {
  ...
  if (!rawOperand) {
    return { ok: false, message: "SYSTEM_RUN_DENIED: approval requires a stable script operand" };
  }
  ...
  if (!stat.isFile()) {
    return { ok: false, message: "SYSTEM_RUN_DENIED: approval requires a file script operand" };
  }
  return {
    ok: true,
    snapshot: { argvIndex, path: realPath, sha256: hashFileContentsSync(realPath) },
  };
}
...
function resolveCanonicalApprovalCwdSync(
  ...
  try {
    ...
    return { ok: false, message: "SYSTEM_RUN_DENIED: approval requires an existing canonical cwd" };
  }
  if (!cwdStat.isDirectory()) {
    return { ok: false, message: "SYSTEM_RUN_DENIED: approval requires cwd to be a directory" };
  }
...
export function buildSystemRunApprovalPlan(params: {
  ...
  const rawCommand = formatExecCommand(hardening.argv);
  const cmdText = commandPreview ?? rawCommand;
  return {
    ok: true,
    plan: {
      argv: hardening.argv,
