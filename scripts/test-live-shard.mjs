#!/usr/bin/env node
// Runs one named live-test shard with REMOTECLAW_LIVE_TEST enabled.
...
const OPTIONAL_LIVE_SHARD_FILE_ENVS = new Map([
  ["src/agents/agent-mcp-style.cache.live.test.ts", ["REMOTECLAW_LIVE_CACHE_TEST"]],
  ["src/agents/cli-runner/bundle-mcp.gemini.live.test.ts", ["REMOTECLAW_LIVE_CLI_MCP_GEMINI"]],
  ["src/agents/embedded-agent-runner.cache.live.test.ts", ["REMOTECLAW_LIVE_CACHE_TEST"]],
  ["src/agents/live-cache-regression.live.test.ts", ["REMOTECLAW_LIVE_CACHE_TEST"]],
  ["src/agents/provider-headers.live.test.ts", ["REMOTECLAW_LIVE_CACHE_TEST"]],
  ["src/agents/subagent-announce.live.test.ts", ["REMOTECLAW_LIVE_SUBAGENT_E2E"]],
  ["src/agents/tools/image-tool.ollama.live.test.ts", ["REMOTECLAW_LIVE_OLLAMA_IMAGE"]],
  ["src/agents/tools/image-tool.providers.live.test.ts", ["REMOTECLAW_LIVE_IMAGE_TOOL_TEST"]],
  ["src/crestodian/rescue-channel.live.test.ts", ["REMOTECLAW_LIVE_CRESTODIAN_RESCUE_CHANNEL"]],
  ["src/gateway/android-node.capabilities.live.test.ts", ["REMOTECLAW_LIVE_ANDROID_NODE"]],
  ["src/gateway/gateway-acp-bind.live.test.ts", ["REMOTECLAW_LIVE_ACP_BIND"]],
  ["src/gateway/gateway-acp-spawn-defaults.live.test.ts", ["REMOTECLAW_LIVE_ACP_SPAWN_DEFAULTS"]],
  ["src/gateway/gateway-cli-backend.live.test.ts", ["REMOTECLAW_LIVE_CLI_BACKEND"]],
  ["src/gateway/gateway-codex-bind.live.test.ts", ["REMOTECLAW_LIVE_CODEX_BIND"]],
  ["src/gateway/gateway-codex-harness.live.test.ts", ["REMOTECLAW_LIVE_CODEX_HARNESS"]],
  ["src/gateway/gateway-trajectory-export.live.test.ts", ["REMOTECLAW_LIVE_CODEX_HARNESS"]],
  ["src/infra/push-apns-http2.live.test.ts", ["REMOTECLAW_LIVE_APNS_REACHABILITY"]],
  ["test/image-generation.infer-cli.live.test.ts", ["REMOTECLAW_LIVE_INFER_CLI_TEST"]],
]);
...
export function buildLiveShardReportPath(shard, env = process.env) {
  const reportDir = env.REMOTECLAW_LIVE_SHARD_REPORT_DIR || ".artifacts/live-shards";
