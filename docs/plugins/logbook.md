...
RemoteClaw-owned state stays on the Gateway under `<state-dir>/logbook/`, but
...
- A connected node that exposes `screen.snapshot` or `logbook.snapshot`. The
  ...
  (`remoteclaw node host run`) gets the plugin-provided `logbook.snapshot`
...
- The bundled Codex plugin enabled and authenticated. Codex currently provides
  ...
  `remoteclaw models auth login --provider openai`; see
...
```bash
remoteclaw plugins enable codex
remoteclaw plugins enable logbook
```
...
remoteclaw plugins inspect logbook --runtime --json
...
1. `remoteclaw plugins list --enabled` includes `logbook`.
