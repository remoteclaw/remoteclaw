...
In RemoteClaw you can authenticate with an API key or a **setup-token**.
...
- Anthropic Claude 4.6 models default to `adaptive` thinking in RemoteClaw when no explicit thinking level is set.
...
RemoteClaw's shared `/fast` toggle also supports direct Anthropic API-key traffic.
...
- This is **API-key only**. Anthropic setup-token / OAuth auth does not honor RemoteClaw fast-mode tier injection.
- RemoteClaw only injects Anthropic service tiers for direct `api.anthropic.com` requests. If you route `anthropic/*` through a proxy or gateway, `/fast` leaves `service_tier` untouched.
...
RemoteClaw includes the `extended-cache-ttl-2025-04-11` beta flag for Anthropic API
requests; keep it if you override provider headers (see [/gateway/configuration](/gateway/configuration)).

## 1M context window (Anthropic beta)

Anthropic's 1M context window is beta-gated. In RemoteClaw, enable it per model
...
RemoteClaw maps this to `anthropic-beta: context-1m-2025-08-07` on Anthropic
requests.
...
OAuth/subscription tokens (`sk-ant-oat-*`). RemoteClaw automatically skips the
