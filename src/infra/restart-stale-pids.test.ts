...
function createRemoteClawBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "remoteclaw-gateway" }]),
    ...overrides,
  });
}
...
function installInitialBusyPoll(
  ...
  mockSpawnSync.mockImplementation(() => {
    call += 1;
    if (call === 1) {
      return createRemoteClawBusyResult(stalePid);
    }
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("verifies argv when lsof reports the node process name instead of remoteclaw", () => {
      ...
      mockSpawnSync.mockImplementation((command: unknown) => {
        if (command === "ps") {
          return {
            error: null,
            status: 0,
            stdout: "node /opt/remoteclaw/dist/entry.js gateway\n",
            stderr: "",
          };
        }
        return {
    ...
    it("excludes ancestor pids so a sidecar cannot kill its parent gateway — regression for #68451", () => {
      // Regression: remoteclaw-weixin sidecar (child of the gateway) invoked
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: parentGatewayPid, cmd: "remoteclaw-gateway" },
          { pid: unrelatedStalePid, cmd: "remoteclaw-gateway" },
        ]),
        stderr: "",
      });
      ...
      () => {
        ...
        mockReadFileSync.mockImplementation((path: unknown): string => {
          if (path === `/proc/${directParentPid}/status`) {
            return `Name:\tremoteclaw-gateway\nPid:\t${directParentPid}\nPPid:\t${grandparentPid}\n`;
          }
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: directParentPid, cmd: "remoteclaw-gateway" },
            { pid: grandparentPid, cmd: "remoteclaw-gateway" },
            { pid: benignStalePid, cmd: "remoteclaw-gateway" },
          ]),
          stderr: "",
        });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: pluginHostPid, cmd: "remoteclaw-gateway" },
            { pid: gatewayGrandparentPid, cmd: "remoteclaw-gateway" },
          ]),
          stderr: "",
        });
    ...
    it("returns verified gateway pids from Windows helpers on win32", () => {
      ...
      try {
        ...
        mockReadWindowsProcessArgs.mockReturnValue(["remoteclaw", "gateway"]);
        expect(findGatewayPidsOnPortSync(18789)).toEqual([stalePid]);
        expect(mockReadWindowsListeningPids).toHaveBeenCalledWith(18789, undefined);
