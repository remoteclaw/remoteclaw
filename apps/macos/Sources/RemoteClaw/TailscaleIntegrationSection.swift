...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.remoteclaw/remoteclaw.json. Prefer REMOTECLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.remoteclaw/remoteclaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.remoteclaw/remoteclaw.json. Restart the gateway to apply."
        }
