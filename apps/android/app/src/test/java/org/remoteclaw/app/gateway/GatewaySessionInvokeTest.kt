...
private class InMemoryDeviceAuthStore : DeviceAuthTokenStore {
  private val entries = mutableMapOf<String, DeviceAuthEntry>()

  private fun key(deviceId: String, role: String): String = "${deviceId.trim()}|${role.trim()}"

  override fun loadEntry(deviceId: String, role: String): DeviceAuthEntry? = entries[key(deviceId, role)]

  override fun saveToken(deviceId: String, role: String, token: String, scopes: List<String>) {
    entries[key(deviceId, role)] =
      DeviceAuthEntry(token = token.trim(), role = role.trim(), scopes = scopes, updatedAtMs = 0L)
  }

  override fun clearToken(deviceId: String, role: String) {
    entries.remove(key(deviceId, role))
  }
}
...
class GatewaySessionInvokeTest {
  ...
  fun refreshNodeCanvasCapability_sendsObjectParamsAndUpdatesScopedUrl() = runBlocking {
                ...
                object : WebSocketListener() {
                  ...
                  override fun onMessage(webSocket: WebSocket, text: String) {
                    ...
                    when (method) {
                      "connect" -> {
                        webSocket.send(
                          """{"type":"res","id":"$id","ok":true,"payload":{"canvasHostUrl":"http://127.0.0.1/__remoteclaw__/cap/old-cap","snapshot":{"sessionDefaults":{"mainSessionKey":"main"}}}}""",
                        )
                      }
    ...
    try {
      ...
      assertEquals(
        "http://127.0.0.1:${server.port}/__remoteclaw__/cap/new-cap",
