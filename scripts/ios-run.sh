...
APP_NAME="${IOS_APP_NAME:-RemoteClaw}"
...
Options:
  ...
  --push-relay-base-url <url>
      ...
      Defaults to https://ios-push-relay-sandbox.remoteclaw.org.
  --simulator-proof-secret-env <name>
      Environment variable that contains the simulator proof secret.
      Defaults to REMOTECLAW_SIMULATOR_PUSH_PROOF_SECRET.
  -h, --help
      Show this help.
EOF
}
...
push_relay_base_url="${REMOTECLAW_PUSH_SANDBOX_RELAY_BASE_URL:-https://ios-push-relay-sandbox.remoteclaw.org}"
simulator_proof_secret_env="${REMOTECLAW_SIMULATOR_PUSH_PROOF_SECRET_ENV:-REMOTECLAW_SIMULATOR_PUSH_PROOF_SECRET}"
...
if [[ "${push_sandbox_simulator}" == "1" ]]; then
  ...
  xcodebuild_overrides+=(
    "REMOTECLAW_PUSH_MODE=simulatorSandbox"
    "REMOTECLAW_PUSH_RELAY_BASE_URL=${push_relay_base_url}"
    "REMOTECLAW_APNS_ENTITLEMENT_ENVIRONMENT=development"
  )
fi

unset "${simulator_proof_secret_env}"
if [[ "${simulator_proof_secret_env}" != "REMOTECLAW_SIMULATOR_PUSH_PROOF_SECRET" ]]; then
  unset REMOTECLAW_SIMULATOR_PUSH_PROOF_SECRET
fi
...
if [[ "${push_sandbox_simulator}" == "1" ]]; then
  # shellcheck disable=SC2086
  SIMCTL_CHILD_REMOTECLAW_SIMULATOR_PUSH_PROOF_SECRET="${simulator_proof_secret}" \
