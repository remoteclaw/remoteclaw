...
Legacy alias for Gateway service management. `remoteclaw daemon ...` maps to the same service-control commands as `remoteclaw gateway ...`. Prefer [`remoteclaw gateway`](/cli/gateway) for current docs and examples.
