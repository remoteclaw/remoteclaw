...
describe("shouldSkipRespawnForArgv", () => {
  it("skips respawn for help/version calls", () => {
    expect(shouldSkipRespawnForArgv(["node", "remoteclaw", "--help"])).toBe(true);
    expect(shouldSkipRespawnForArgv(["node", "remoteclaw", "-V"])).toBe(true);
  });
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "remoteclaw.internal"], {
        from: "user",
      });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("remoteclaw.internal");
    } finally {
      log.mockRestore();
    }
  });
