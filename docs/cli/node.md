...
- `--context-path <path>`: Gateway WebSocket context path (e.g. `/remoteclaw-gw`). Appended to the WebSocket URL.
...
`remoteclaw node run` and `remoteclaw node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `REMOTECLAW_GATEWAY_TOKEN` / `REMOTECLAW_GATEWAY_PASSWORD` are checked first.
...
trusted private-DNS names, set `REMOTECLAW_ALLOW_INSECURE_PRIVATE_WS=1`; without
it, node startup fails closed and asks you to use `wss://`, an SSH tunnel, or
Tailscale. This is a process-environment opt-in, not an `remoteclaw.json` config
key.
`remoteclaw node install` persists it into the supervised node service when it is
...
```bash
remoteclaw node install --host <gateway-host> --port 18789
```
...
info in `node.json` in the RemoteClaw state directory (`~/.remoteclaw` by default,
or `$REMOTECLAW_STATE_DIR` when set).
...
For approved async node exec, RemoteClaw prepares a canonical `systemRunPlan`
