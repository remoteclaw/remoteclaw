...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let payloadMap: [String: RemoteClawProtocol.AnyCodable] = [
            "presence": .init([RemoteClawProtocol.AnyCodable(entry)]),
        ]
        let payload = RemoteClawProtocol.AnyCodable(payloadMap)
