...
has_native_swift_changes() {
  if git rev-parse --verify --quiet origin/main >/dev/null; then
    if git diff --name-only --relative origin/main...HEAD -- apps/macos apps/ios apps/shared/RemoteClawKit | rg -q .; then
      return 0
    fi
  fi

  if git rev-parse --verify --quiet HEAD^ >/dev/null; then
    git diff --name-only --relative HEAD^..HEAD -- apps/macos apps/ios apps/shared/RemoteClawKit | rg -q .
    return $?
  fi

  git show --name-only --relative --pretty='' HEAD -- apps/macos apps/ios apps/shared/RemoteClawKit | rg -q .
}
...
run_macos_ci_mirror() {
  if [[ "${REMOTECLAW_PREPUSH_SKIP_MACOS:-0}" == "1" ]]; then
    log_step "Skipping macOS mirror because REMOTECLAW_PREPUSH_SKIP_MACOS=1"
    return 0
  fi
  ...
  run_step swiftformat --lint apps/macos/Sources --config config/swiftformat --exclude '**/RemoteClawProtocol,**/HostEnvSecurityPolicy.generated.swift'
