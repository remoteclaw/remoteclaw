...
RemoteClaw can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `remoteclaw configure --section web` enables the bundled Firecrawl plugin automatically.
...
- Legacy `tools.web.fetch.firecrawl.*` config is auto-migrated by `remoteclaw doctor --fix`.
...
RemoteClaw always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
