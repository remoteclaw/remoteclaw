...
export function buildLiveCronProbeMessage(params: {
  ...
  if (params.attempt === 0) {
    return (
      "Use the RemoteClaw MCP tool named cron. " +
  ...
  if (family === "claude") {
    return (
      "Return only a tool call for the RemoteClaw MCP tool `cron`. " +
      `Use these exact JSON arguments: ${params.argsJson}. ` +
      "No prose. I will verify externally with the RemoteClaw cron CLI."
    );
  }
  return (
    "Use the RemoteClaw MCP tool named cron. " +
    `Use these exact JSON arguments: ${params.argsJson}. ` +
    "No prose before the tool call. I will verify externally with the RemoteClaw cron CLI."
  );
}

export async function runRemoteClawCliJson<T>(args: string[], env: NodeJS.ProcessEnv): Promise<T> {
  const childEnv = { ...env };
  ...
  if (!trimmed) {
    throw new Error(
      [
        `remoteclaw ${args.join(" ")} produced no JSON stdout`,
  ...
  try {
    ...
    throw new Error(
      [
        `remoteclaw ${args.join(" ")} returned invalid JSON`,
        `stdout: ${trimmed}`,
        stderr.trim() ? `stderr: ${stderr.trim()}` : undefined,
...
export async function assertCronJobVisibleViaCli(params: {
  ...
  const cronList = await runRemoteClawCliJson<CronListCliResult>(
    [
      "cron",
      "list",
      "--all",
