...
- `--model` overrides the default model for that specific run.
...
- Otherwise delivery depends on requester depth:
  ...
  - if a nested requester subagent session is gone, RemoteClaw falls back to that session's requester when available
