...
force `embeddedHarness.runtime: "codex"` or `REMOTECLAW_AGENT_RUNTIME=codex`.
...
RemoteClaw has separate routes for OpenAI and Codex-shaped access:

| Model ref              | Runtime path                                   | Use when                                                                |
| ---------------------- | ---------------------------------------------- | ----------------------------------------------------------------------- |
| `openai/gpt-5.4`       | OpenAI provider through RemoteClaw/PI plumbing | You want direct OpenAI Platform API access with `OPENAI_API_KEY`.       |
...
With fallback disabled, RemoteClaw fails early if the Codex plugin is disabled,
...
`REMOTECLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=1`. It starts the Codex harness in
...
currently selected RemoteClaw `codex/*` model into app-server, and keeps extended
history enabled.
...
**RemoteClaw uses PI instead of Codex:** if no Codex harness claims the run,
RemoteClaw may use PI as the compatibility backend. Set
