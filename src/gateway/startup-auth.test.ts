...
describe("ensureGatewayStartupAuth", () => {
  ...
  it("resolves env-template gateway.auth.token before env-token short-circuiting", async () => {
    const result = await ensureGatewayStartupAuth({
      ...
      env: {
        REMOTECLAW_GATEWAY_TOKEN: "resolved-token",
      } as NodeJS.ProcessEnv,
      persist: true,
    });
    ...
    expect(result.auth.mode).toBe("token");
    expect(result.auth.token).toBe("resolved-token");
    expect(result.cfg.gateway?.auth?.token).toBe("${REMOTECLAW_GATEWAY_TOKEN}");
    expect(mocks.writeConfigFile).not.toHaveBeenCalled();
  });

  it("uses REMOTECLAW_GATEWAY_TOKEN without resolving configured token SecretRef", async () => {
    const result = await ensureGatewayStartupAuth({
      cfg: {
        gateway: {
          auth: {
      ...
      env: {
        REMOTECLAW_GATEWAY_TOKEN: "token-from-env",
      } as NodeJS.ProcessEnv,
      persist: true,
    });
  ...
  it("uses REMOTECLAW_GATEWAY_PASSWORD without resolving configured password SecretRef", async () => {
    const result = await ensureGatewayStartupAuth({
      ...
      env: {
        REMOTECLAW_GATEWAY_PASSWORD: "password-from-env", // pragma: allowlist secret
      } as NodeJS.ProcessEnv,
      persist: true,
    });
  ...
  it("throws when hooks token reuses gateway token resolved from env", async () => {
    await expect(
      ensureGatewayStartupAuth({
        ...
        env: {
          REMOTECLAW_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
        } as NodeJS.ProcessEnv,
      }),
