...
describe("createTelegramBot", () => {
  ...
  // Gutted in RemoteClaw fork — model callback routing requires Pi-embedded dispatch
  it.skip("routes compact model callbacks by inferring provider", async () => {
    onSpy.mockClear();
    replySpy.mockClear();
  ...
  // Gutted in RemoteClaw fork — model callback routing requires Pi-embedded dispatch
  it.skip("rejects ambiguous compact model callbacks and returns provider list", async () => {
    onSpy.mockClear();
    replySpy.mockClear();
    editMessageTextSpy.mockClear();
  ...
  // Gutted in RemoteClaw fork — reply media download refactored upstream (replyMedia param not wired)
  it.skip("includes replied image media in inbound context for text replies", async () => {
    onSpy.mockClear();
    replySpy.mockClear();
  ...
  // Gutted in RemoteClaw fork — reply media download refactored upstream (replyMedia param not wired)
  it.skip("defers reply media download until debounce flush", async () => {
  ...
  // Gutted in RemoteClaw fork — reply media download refactored upstream (replyMedia param not wired)
  it.skip("isolates inbound debounce by DM topic thread id", async () => {
    const DEBOUNCE_MS = 4321;
    onSpy.mockClear();
    replySpy.mockClear();
    loadConfig.mockReturnValue({
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "RemoteClaw" },
        },
      },
      me: { id: 999, username: "remoteclaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
