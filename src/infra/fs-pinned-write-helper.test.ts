...
describe("fs pinned write helper", () => {
  it.runIf(process.platform !== "win32")("writes through a pinned parent directory", async () => {
    const root = await tempDirs.make("remoteclaw-fs-pinned-root-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = await tempDirs.make("remoteclaw-fs-pinned-root-");
      const outside = await tempDirs.make("remoteclaw-fs-pinned-outside-");
  ...
  it.runIf(process.platform !== "win32")("accepts streamed input", async () => {
    const root = await tempDirs.make("remoteclaw-fs-pinned-root-");
    const sourcePath = path.join(await tempDirs.make("remoteclaw-fs-pinned-src-"), "source.txt");
