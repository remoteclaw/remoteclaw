 ...
 *
 * Implements the OpenResponses `/v1/responses` endpoint for RemoteClaw Gateway.
 *
...
import type { ClientToolDefinition, ImageContent } from "../commands/agent/types.js";
...
function applyToolChoice(params: {
  ...
  if (typeof toolChoice === "object" && toolChoice.type === "function") {
    ...
    const matched = tools.filter(
      (tool) => (tool as { function?: { name?: string } }).function?.name === targetName,
    );
    if (matched.length === 0) {
  ...
  unsubscribe = onAgentEvent((evt) => {
    ...
    if (evt.stream === "lifecycle") {
      ...
      if (phase === "end" || phase === "error") {
        const finalText = accumulatedText || "No response from RemoteClaw.";
