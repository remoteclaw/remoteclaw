...
deployments should keep the default RemoteClaw Pi runtime. Use it when a trusted
...
- Environment overrides: `REMOTECLAW_AGENT_RUNTIME=<id|auto|pi>` overrides `id`; `REMOTECLAW_AGENT_HARNESS_FALLBACK=pi|none` overrides fallback for that process.
