...
const OMITTED_DIST_SUBTREE_PATTERNS = [
  ...
  /^dist\/extensions\/[^/]+\/node_modules(?:\/|$)/u,
  /^dist\/extensions\/[^/]+\/\.remoteclaw-runtime-deps-[^/]+(?:\/|$)/u,
  /^dist\/extensions\/qa-matrix(?:\/|$)/u,
  new RegExp(`^dist/plugin-sdk/extensions/${LEGACY_QA_CHANNEL_DIR}(?:/|$)`, "u"),
  new RegExp(`^dist/plugin-sdk/extensions/${LEGACY_QA_LAB_DIR}(?:/|$)`, "u"),
] as const;
const INSTALL_STAGE_DEBRIS_DIR_PATTERN = /^\.remoteclaw-install-stage(?:-[^/]+)?$/iu;
...
function isPackagedDistPath(relativePath: string): boolean {
  ...
  if (relativePath.endsWith("/.remoteclaw-runtime-deps-stamp.json")) {
    return false;
  }
