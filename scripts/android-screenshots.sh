...
boot_emulator() {
  ...
  EMULATOR_LOG="$(mktemp "${TMPDIR:-/tmp}/remoteclaw-android-screenshot-emulator.XXXXXX.log")"
...
for scene in "${SCENES[@]}"; do
  output_path="${OUTPUT_DIR}/remoteclaw-${scene}.png"
  "$ADB_BIN" -s "$ADB_SERIAL" shell am force-stop org.remoteclaw.app >/dev/null
  "$ADB_BIN" -s "$ADB_SERIAL" shell am start -W \
    -n org.remoteclaw.app/.MainActivity \
    --ez remoteclaw.screenshotMode true \
    --es remoteclaw.screenshotScene "$scene" >/dev/null
