...
public struct RemoteClawChatSpeechClip: Equatable, Sendable {
...
public typealias RemoteClawChatSpeechSynthesis =
    @Sendable (_ text: String) async throws -> RemoteClawChatSpeechClip
...
@MainActor
@Observable
public final class RemoteClawChatSpeechController {
    ...
    private let synthesize: RemoteClawChatSpeechSynthesis
    ...
    public convenience init(synthesize: @escaping RemoteClawChatSpeechSynthesis) {
        self.init(
    ...
    init(
        synthesize: @escaping RemoteClawChatSpeechSynthesis,
    ...
    private func run(messageID: UUID, text: String, generation: UInt64) async {
        let clip: RemoteClawChatSpeechClip? = await {
            do {
