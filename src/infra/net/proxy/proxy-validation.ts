...
const DENIED_CANARY_HEADER = "x-remoteclaw-proxy-validation-canary";
...
export type ResolveProxyValidationConfigOptions = {
  config?: ProxyConfig;
  env?: NodeJS.ProcessEnv | Partial<Record<"REMOTECLAW_PROXY_URL", string | undefined>>;
...
function validateProxyUrl(value: string | undefined): string[] {
  if (!value) {
    return ["proxy validation requires proxy.proxyUrl, --proxy-url, or REMOTECLAW_PROXY_URL"];
  }
...
function validateProxyEnabled(source: ProxyValidationConfigSource, enabled: boolean): string[] {
  ...
  if (source === "env") {
    return ["proxy validation requires proxy.enabled to be true for REMOTECLAW_PROXY_URL"];
  }
...
export function resolveProxyValidationConfig(
  ...
  const envUrl = normalizeProxyUrl(options.env?.REMOTECLAW_PROXY_URL);
  if (envUrl) {
    const proxyCaFile = resolveManagedProxyCaFileForUrl({
      proxyUrl: envUrl,
      config: options.config,
