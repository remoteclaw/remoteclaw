...
Tencent Yuanbao is Tencent's AI assistant platform. The RemoteClaw channel plugin
connects Yuanbao bots to RemoteClaw over WebSocket so they can interact with users
...
> **Requires RemoteClaw 2026.4.10 or above.** Run `remoteclaw --version` to check. Upgrade with `remoteclaw update`.
...
3. Ensure the gateway is running: `remoteclaw gateway status`
