...
`;
    ...
    expect(parsed.remoteclaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"remoteclaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"remoteclaw": {"events": ["test"]}}');
  });
...
describe("resolveRemoteClawMetadata", () => {
  it("extracts remoteclaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
  ...
  it("returns undefined when remoteclaw key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        remoteclaw: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with RemoteClaw" },
            { id: "npm", kind: "npm", package: "@remoteclaw/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@remoteclaw/hook");
  });
...
`;
    ...
    expect(remoteclaw?.events).toEqual(["command:new"]);
...
`;
    const frontmatter = parseFrontmatter(content);
    const remoteclaw = resolveRemoteClawMetadata(frontmatter);
    expect(remoteclaw?.emoji).toBe("disk");
    expect(remoteclaw?.events).toEqual(["command:new"]);
  });
});
