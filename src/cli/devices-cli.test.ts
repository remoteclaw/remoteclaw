...
describe("devices cli approve", () => {
  ...
  it("prints selected details and exits when implicit approval is used", async () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      expect.stringContaining("remoteclaw devices approve req-abc"),
    );
  ...
  it.each([
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      expect.stringContaining(`remoteclaw devices approve ${expectedRequestId}`),
    );
  });
  ...
  it("falls back to device id when selected pending display name is blank", async () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      expect.stringContaining("remoteclaw devices approve req-blank"),
    );
    expect(callGateway).not.toHaveBeenCalledWith(
      expect.objectContaining({ method: "device.pair.approve" }),
    );
  ...
  it("includes explicit gateway flags in the rerun approval command", async () => {
    ...
    await runDevicesApprove([
      "--latest",
      "--url",
      "ws://gateway.example:18789/remoteclaw?cluster=qa lab",
      "--timeout",
      "3000",
      "--token",
      "secret-token",
    ...
    expect(errorOutput).toContain(
      "remoteclaw devices approve req-url --url 'ws://gateway.example:18789/remoteclaw?cluster=qa lab' --timeout 3000",
    );
  ...
  it("returns JSON for implicit approval preview in JSON mode", async () => {
    ...
    expect(runtime.writeJson).toHaveBeenCalledWith({
      ...
      approveCommand: "remoteclaw devices approve req-json --url ws://gateway.example:18789 --json",
      requiresAuthFlags: {
        token: false,
        password: false,
      },
