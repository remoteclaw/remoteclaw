...
RemoteClaw can convert outbound replies into audio using ElevenLabs, Microsoft, MiniMax, or OpenAI.
It works anywhere RemoteClaw can send audio.
...
When `messages.tts.provider` is unset, RemoteClaw picks the first configured
speech provider in registry auto-select order.

## Config

TTS config lives under `messages.tts` in `remoteclaw.json`.
...
- If `provider` is **unset**, RemoteClaw uses the first configured speech provider in registry auto-select order.
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, RemoteClaw retries with MP3.

OpenAI/ElevenLabs output formats are fixed per channel (see above).

## Auto-TTS behavior

When enabled, RemoteClaw:
...
Discord note: `/tts` is a built-in Discord command, so RemoteClaw registers
...
/tts limit 2000
/tts summary off
/tts audio Hello from RemoteClaw
```
