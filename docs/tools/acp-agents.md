...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let RemoteClaw run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.
...
By default, the acpx plugin (published as `@remoteclaw/acpx`) uses the plugin-local pinned binary:
