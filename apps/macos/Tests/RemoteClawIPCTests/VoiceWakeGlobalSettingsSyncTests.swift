...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    @Test func appliesVoiceWakeChangedEventToAppState() async {
        ...
        let payload = RemoteClawProtocol.AnyCodable(["triggers": ["remoteclaw", "computer"]])
        let evt = EventFrame(
            type: "event",
        ...
        #expect(updated == ["remoteclaw", "computer"])
    ...
    @Test func ignoresVoiceWakeChangedEventWithInvalidPayload() async {
        ...
        let payload = RemoteClawProtocol.AnyCodable(["unexpected": 123])
        let evt = EventFrame(
            type: "event",
            event: "voicewake.changed",
            payload: payload,
