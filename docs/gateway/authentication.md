...
RemoteClaw supports OAuth and API keys for model providers. For always-on gateway
...
subscription-style auth inside RemoteClaw is the legacy setup-token path and
...
Anthropic setup-token auth is still available in RemoteClaw as a
...
RemoteClaw users that the **RemoteClaw** Claude-login path counts as third-party
...
subscription-style Anthropic path in RemoteClaw, use the legacy setup-token path
...
- If auth exists but RemoteClaw cannot resolve a probeable model candidate for
...
- Use Anthropic API keys for Anthropic traffic in RemoteClaw.
- Anthropic setup-token remains a legacy/manual path and should be used with
  the Extra Usage billing expectation Anthropic communicated to RemoteClaw users.
- `remoteclaw doctor` now detects stale removed Anthropic Claude CLI state. If
...
When you debug order issues, `remoteclaw models status --probe` shows omitted
...
Run `remoteclaw models status` to confirm which profile is expiring. If a legacy
