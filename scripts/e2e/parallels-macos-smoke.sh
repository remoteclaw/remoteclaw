...
INSTALL_URL="https://remoteclaw.ai/install.sh"
HOST_PORT="18425"
...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_OPENCLAW_BIN="/opt/homebrew/bin/remoteclaw"
GUEST_OPENCLAW_ENTRY="/opt/homebrew/lib/node_modules/remoteclaw/remoteclaw.mjs"
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://remoteclaw.ai/install.sh
...
send -- "cat >/tmp/remoteclaw-prl.sh <<'__OPENCLAW_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__OPENCLAW_SCRIPT__\r"
send -- "/bin/bash /tmp/remoteclaw-prl.sh; rc=\$?; rm -f /tmp/remoteclaw-prl.sh; printf '__OPENCLAW_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/remoteclaw-discord-token <<'__OPENCLAW_TOKEN__'
$DISCORD_TOKEN_VALUE
__OPENCLAW_TOKEN__
cat >/tmp/remoteclaw-discord-guilds.json <<'__OPENCLAW_GUILDS__'
$guilds_json
__OPENCLAW_GUILDS__
