...
describe("resolveInitialTuiAgentId", () => {
  const cfg: RemoteClawConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/remoteclaw" },
        { id: "ops", workspace: "/tmp/remoteclaw/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/remoteclaw/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run remoteclaw devices list");
    expect(state.pairingHint).toContain("remoteclaw devices list");
  });
