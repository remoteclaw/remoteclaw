...
Use `remoteclaw onboard` or `remoteclaw setup` for the full guided first-run journey, `remoteclaw setup --baseline` for the baseline config/workspace only, and `remoteclaw channels add` when you only need channel account setup.

<Tip>
`remoteclaw config` with no subcommand opens the same wizard. Use `remoteclaw config get|set|unset` for non-interactive edits.
</Tip>
...
remoteclaw configure --section gateway --section daemon
```
...
<Note>
`remoteclaw configure` requires an interactive terminal (both stdin and stdout must be TTYs). Without one it prints the equivalent non-interactive `remoteclaw config get|set|patch|validate` commands and exits with an error instead of partially running.
</Note>
...
Re-running provider auth from configure preserves an existing `agents.defaults.model.primary`, even when the provider's auth step returns a config patch with its own recommended default model. Adding or reauthing a provider makes its models available without taking over your current primary model. Use `remoteclaw models auth login --provider <id> --set-default` or `remoteclaw models set <model>` to intentionally change the default model.
</Note>
...
`remoteclaw configure --section web` picks a web-search provider and configures its credentials. Some providers show provider-specific follow-ups:
