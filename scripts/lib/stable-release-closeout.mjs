...
function isCloseoutEvidenceAsset(assetName, tag) {
  ...
  return (
    assetName === `remoteclaw-${releaseVersion}-stable-main-closeout.json` ||
    assetName === `remoteclaw-${releaseVersion}-stable-main-closeout.json.sha256`
  );
}
...
export function verifyStableMainCloseout(params) {
  ...
  const expectedMacAssets = [
    `RemoteClaw-${macAssetVersion}.zip`,
    `RemoteClaw-${macAssetVersion}.dmg`,
    `RemoteClaw-${macAssetVersion}.dSYM.zip`,
  ];
