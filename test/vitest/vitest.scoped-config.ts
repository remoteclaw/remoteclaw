...
export function createScopedVitestConfig(
  ...
  options?: {
    ...
    exclude?: string[];
    argv?: string[];
    includeRemoteClawRuntimeSetup?: boolean;
    isolate?: boolean;
    name?: string;
  ...
  const includeFromEnv = loadPatternListFromEnv("REMOTECLAW_VITEST_INCLUDE_FILE", env);
  ...
  const setupFiles = [
    ...new Set([
      ...(baseTest.setupFiles ?? []),
      ...(options?.setupFiles ?? []),
      ...(options?.includeRemoteClawRuntimeSetup === false
        ? []
