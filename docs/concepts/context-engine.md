...
Context engine plugins are installed like any other RemoteClaw plugin. Install
...
remoteclaw plugins install -l ./my-context-engine
```
...
RemoteClaw currently calls one subagent lifecycle hook:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. RemoteClaw uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. RemoteClaw disables Pi's built-in
...
`ownsCompaction: false` does **not** mean RemoteClaw automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `remoteclaw/plugin-sdk/core` to use
  RemoteClaw's built-in compaction behavior.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, RemoteClaw
