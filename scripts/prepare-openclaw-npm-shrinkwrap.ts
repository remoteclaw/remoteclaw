...
const AI_PACKAGE_NAME = "@remoteclaw/ai";
const AI_LOCK_PATH = "node_modules/@remoteclaw/ai";
...
export function prepareRemoteClawNpmShrinkwrap(params: {
  aiIntegrity: string;
  aiManifest: PackageManifest;
  ...
  if (aiVersion !== rootVersion) {
    throw new Error(`AI package version ${aiVersion} does not match RemoteClaw ${rootVersion}`);
  }
  ...
  if (rootLockVersion !== rootVersion) {
    throw new Error(
      `npm shrinkwrap root version ${rootLockVersion} does not match RemoteClaw ${rootVersion}`,
    );
  }
...
export function assertPreparedRemoteClawNpmShrinkwrap(params: {
  aiIntegrity: string;
  aiManifest: PackageManifest;
  rootManifest: PackageManifest;
  shrinkwrap: NpmShrinkwrap;
}): void {
  const expected = prepareRemoteClawNpmShrinkwrap({
    ...params,
  ...
  if (
    ...
    throw new Error(
      `prepared RemoteClaw npm shrinkwrap does not lock the exact ${AI_PACKAGE_NAME} tarball`,
    );
  }
}
...
function main(argv = process.argv.slice(2)): void {
  ...
  if (!aiTarballPath || argv.length > 3) {
    throw new Error(
      "Usage: node --import tsx scripts/prepare-remoteclaw-npm-shrinkwrap.ts <remoteclaw-ai.tgz> [npm-shrinkwrap.json] [package.json]",
    );
  }

  const prepared = prepareRemoteClawNpmShrinkwrap({
