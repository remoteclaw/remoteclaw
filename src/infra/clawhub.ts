  ...
  package:
    | (ClawHubPackageListItem & {
        ...
        compatibility?: {
          ...
          builtWithRemoteClawVersion?: string;
...
export async function downloadClawHubPackageArchive(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-clawhub-package-"));
...
export async function downloadClawHubSkillArchive(params: {
  ...
  const bytes = new Uint8Array(await response.arrayBuffer());
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-clawhub-skill-"));
