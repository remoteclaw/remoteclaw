...
RemoteClaw treats **one direct-chat session per agent** as primary. Direct chats collapse to `agent:<agentId>:<mainKey>` (default `main`), while group/channel chats get their own keys. `session.mainKey` is honored.
...
- You can verify your DM settings with `remoteclaw security audit` (see [security](/cli/security)).
...
- On the **gateway host**:
  - Store file: `~/.remoteclaw/agents/<agentId>/sessions/sessions.json` (per agent).
- Transcripts: `~/.remoteclaw/agents/<agentId>/sessions/<SessionId>.jsonl` (Telegram topic sessions use `.../<SessionId>-topic-<threadId>.jsonl`).
- The store is a map `sessionKey -> { sessionId, updatedAt, ... }`. Deleting entries is safe; they are recreated on demand.
- Group entries may include `displayName`, `channel`, `subject`, `room`, and `space` to label sessions in UIs.
- Session entries include `origin` metadata (label + routing hints) so UIs can explain where a session came from.
- RemoteClaw does **not** read legacy Pi/Tau session folders.

## Maintenance

RemoteClaw applies session-store maintenance to keep `sessions.json` and transcript artifacts bounded over time.
...
Maintenance runs during session-store writes, and you can trigger it on demand with `remoteclaw sessions cleanup`.
...
- many transcript/archive artifacts in `~/.remoteclaw/agents/<agentId>/sessions/`
...
- run `remoteclaw sessions cleanup --dry-run --json` after config changes to verify projected impact before enforcing
...
RemoteClaw trims **old tool results** from the in-memory context right before LLM calls by default.
...
- Send `/status` as a standalone message in chat to see whether the agent is reachable, how much of the session context is used, current verbose toggle, and when your WhatsApp web creds were last refreshed (helps spot relink needs).
