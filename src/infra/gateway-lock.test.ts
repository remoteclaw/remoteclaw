...
describe("gateway lock", () => {
  ...
  it("blocks concurrent acquisition until release", async () => {
    ...
    const pending = acquireForTest(env, {
      timeoutMs: 15,
      readProcessCmdline: () => ["remoteclaw", "gateway", "run"],
    });
  ...
  it("keeps lock when configured port is busy and owner pid is alive", async () => {
    ...
    try {
      const pending = acquireForTest(env, {
        ...
        platform: "darwin",
        port: 18789,
        readProcessCmdline: () => ["/usr/local/bin/remoteclaw", "gateway", "run"],
      });
      await expect(pending).rejects.toBeInstanceOf(GatewayLockError);
    } finally {
      connectSpy.mockRestore();
  ...
  it("returns null when multi-gateway override is enabled", async () => {
    ...
    const lock = await acquireGatewayLock({
      env: { ...env, REMOTECLAW_ALLOW_MULTI_GATEWAY: "1", VITEST: "" },
      lockDir: resolveTestLockDir(),
    });
  ...
  it("keeps lock on darwin when process cmdline is a gateway", async () => {
    ...
    const pending = acquireForTest(env, {
      ...
      platform: "darwin",
      port: 18789,
      readProcessCmdline: () => ["/usr/local/bin/remoteclaw", "gateway", "run", "--port", "18789"],
    });
    await expect(pending).rejects.toBeInstanceOf(GatewayLockError);
