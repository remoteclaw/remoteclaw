...
- Doctor repairs missing bundled plugin runtime dependencies without requiring write access to the installed RemoteClaw package. For root-owned npm installs or hardened systemd units, set `REMOTECLAW_PLUGIN_STAGE_DIR` to a writable directory such as `/var/lib/remoteclaw/plugin-runtime-deps`.
