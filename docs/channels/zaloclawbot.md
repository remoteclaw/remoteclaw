---
summary: "Zalo ClawBot channel setup through the external remoteclaw-zaloclawbot plugin"
read_when:
  - You want a personal Zalo assistant bot with QR-code login
  - You are installing or troubleshooting the remoteclaw-zaloclawbot channel plugin
title: "Zalo ClawBot"
---

RemoteClaw connects to Zalo ClawBot through the catalog-listed external
`@zalo-platforms/remoteclaw-zaloclawbot` plugin. Login uses a Zalo Mini App QR
code.

## Compatibility

| Plugin Version | RemoteClaw Version | npm dist-tag | Status        |
| -------------- | ------------------ | ------------ | ------------- |
...
- Node.js **>= 22**
- [RemoteClaw](https://docs.remoteclaw.org/install) must be installed (`remoteclaw` CLI available).
- A Zalo account on a mobile device to scan the login QR code.

## Install with onboard (recommended)

Run the RemoteClaw onboarding wizard and pick **Zalo ClawBot** from the channel menu:
...
```bash
remoteclaw plugins install "@zalo-platforms/remoteclaw-zaloclawbot@0.1.4"
```

Use the exact pinned version shown above (it matches the official catalog entry), so RemoteClaw verifies the package against the catalog integrity hash during install.

### 2. Enable the plugin in config

```bash
remoteclaw config set plugins.entries.remoteclaw-zaloclawbot.enabled true
```

### 3. Generate QR code and log in

```bash
remoteclaw channels login --channel remoteclaw-zaloclawbot
```
...
The external plugin manages bot credentials under the RemoteClaw state directory.
Treat that directory as sensitive and include it in the same access-control and
backup policy as the rest of your RemoteClaw state.
...
- **Gateway Fails to Load:** Ensure your RemoteClaw host version is `2026.4.10` or higher. Older versions do not support the external npm-plugin installation ledger.
