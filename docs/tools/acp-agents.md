---
summary: "Run external coding harnesses (Claude Code, Cursor, Gemini CLI, explicit Codex ACP, RemoteClaw ACP, OpenCode) through the ACP backend"
read_when:
...
let RemoteClaw run external coding harnesses (for example Claude Code,
Cursor, Copilot, Droid, RemoteClaw ACP, OpenCode, Gemini CLI, and other
...
directly to existing RemoteClaw channel conversations, use
[`remoteclaw mcp serve`](/cli/mcp) instead of ACP.
</Note>
...
| Run Claude Code, Gemini CLI, explicit Codex ACP, or another external harness _through_ RemoteClaw | This page                             | Chat-bound sessions, `/acp spawn`, `sessions_spawn({ runtime: "acp" })`, background tasks, runtime controls                                                                                   |
| Expose an RemoteClaw Gateway session _as_ an ACP server for an editor or client                   | [`remoteclaw acp`](/cli/acp)          | Bridge mode. IDE/client talks ACP to RemoteClaw over stdio/WebSocket                                                                                                                          |
| Reuse a local AI CLI as a text-only fallback model                                                | [CLI Backends](/gateway/cli-backends) | Not ACP. No RemoteClaw tools, no ACP controls, no harness runtime                                                                                                                             |
...
<AccordionGroup>
  <Accordion title="First-run gotchas">
    ...
    - Codex ACP runs with an isolated `CODEX_HOME`; RemoteClaw copies trusted project entries plus safe model/provider routing config from the host Codex config, while auth, notifications, and hooks stay on the host config.
  ...
  <Accordion title="Runtime prerequisites">
    ACP launches a real external harness process. RemoteClaw owns routing,
    ...
    Before blaming RemoteClaw, verify:
...
| `remoteclaw` | RemoteClaw Gateway bridge through `remoteclaw acp` | Lets an ACP-aware harness talk back to an RemoteClaw Gateway session.               |
| `qwen`       | Qwen Code / Qwen CLI                               | Requires Qwen-compatible auth on the host.                                          |

Custom acpx agent aliases can be configured in acpx itself, but RemoteClaw
...
<AccordionGroup>
  <Accordion title="Lifecycle details">
    - Spawn creates or resumes an ACP runtime session, records ACP metadata in the RemoteClaw session store, and may create a background task when the run is parent-owned.
  ...
  <Accordion title="Native Codex routing rules">
    ...
    RemoteClaw dynamic tools still execute through RemoteClaw, while
    Codex-native tools such as shell/apply-patch execute inside Codex.
    For Codex-native tool events, RemoteClaw injects a per-turn native
    hook relay so plugin hooks can block `before_tool_call`, observe
    `after_tool_call`, and route Codex `PermissionRequest` events
    through RemoteClaw approvals. Codex `Stop` hooks are relayed to
    RemoteClaw `before_agent_finalize`, where plugins can request one more
  ...
  <Accordion title="ACP-routing natural-language triggers">
    ...
    RemoteClaw picks `runtime: "acp"`, resolves the harness `agentId`,
    ...
    RemoteClaw config agent id from `agents_list` unless that entry is
    explicitly configured with `agents.list[].runtime.type="acp"`;
    otherwise use the default sub-agent runtime. When an RemoteClaw agent
    is configured with `runtime.type="acp"`, RemoteClaw uses
...
plugin is enabled. Use **sub-agents** when you want RemoteClaw-native
delegated runs.

| Area          | ACP session                           | Sub-agent run                       |
| ------------- | ------------------------------------- | ----------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | RemoteClaw native sub-agent runtime |
...
1. RemoteClaw ACP session control plane.
2. Official `@remoteclaw/acpx` runtime plugin.
...
<AccordionGroup>
  ...
  <Accordion title="Thread-bound sessions">
    ...
    - RemoteClaw binds a thread to a target ACP session.
    ...
    adapter does not support thread bindings, RemoteClaw returns a clear
...
<ParamField path="bindings[].agentId" type="string">
  The owning RemoteClaw agent id.
</ParamField>
...
<Tabs>
  <Tab title="From sessions_spawn">
    ...
    for ACP sessions. If `agentId` is omitted, RemoteClaw uses
...
<ParamField path="mode" type='"run" | "session"' default="run">
  ...
  `mode` is omitted, RemoteClaw may default to persistent behaviour per
...
<ParamField path="model" type="string">
  ...
  `session/set_model`; otherwise RemoteClaw/acpx fails clearly instead of
...
<AccordionGroup>
  <Accordion title="Interactive ACP sessions">
    ...
    What RemoteClaw sends to the harness:
    ...
    - Runtime-generated completion events are materialized per target. RemoteClaw agents get RemoteClaw's internal runtime-context envelope; external ACP harnesses get a plain prompt with the child result and instruction. The raw `<<<BEGIN_REMOTECLAW_INTERNAL_CONTEXT>>>` envelope should never be sent to external harnesses or persisted as ACP user transcript text.
    - ACP transcript entries use the user-visible trigger text or the plain completion prompt. Internal event metadata stays structured in RemoteClaw where possible and is not treated as user-authored chat content.
  ...
  <Accordion title="Parent-owned one-shot ACP sessions">
    ...
    - Completion reports back through the task-completion announce path. RemoteClaw converts internal completion metadata into a plain ACP prompt before sending it to an external harness, so harnesses do not see RemoteClaw-only runtime context markers.
  ...
  <Accordion title="sessions_send and A2A delivery">
    ...
    peer sessions, RemoteClaw uses an agent-to-agent (A2A) follow-up path
    ...
    RemoteClaw skips the A2A follow-up only when the requester is the
...
- RemoteClaw's sandbox policy does **not** wrap ACP harness execution.
- RemoteClaw still enforces ACP feature gates, allowed agents, session ownership, channel bindings, and Gateway delivery policy.
- Use `runtime: "subagent"` for sandbox-enforced RemoteClaw-native work.
...
If no target resolves, RemoteClaw returns a clear error
...
| ---------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `/acp model <id>`            | runtime config key `model`           | For Codex ACP, RemoteClaw normalizes `openai/<model>` to the adapter model id and maps slash reasoning suffixes such as `openai/gpt-5.4/high` to `reasoning_effort`.                                         |
| `/acp set thinking <level>`  | canonical option `thinking`          | RemoteClaw sends the backend-advertised equivalent when present, preferring `thinking`, then `effort`, `reasoning_effort`, or `thought_level`. For Codex ACP, the adapter maps values to `reasoning_effort`. |
| `/acp permissions <profile>` | canonical option `permissionProfile` | RemoteClaw sends the backend-advertised equivalent when present, such as `approval_policy`, `permission_profile`, `permissions`, or `permission_mode`.                                                       |
| `/acp timeout <seconds>`     | canonical option `timeoutSeconds`    | RemoteClaw sends the backend-advertised equivalent when present, such as `timeout` or `timeout_seconds`.                                                                                                     |
...
| Vendor auth error from the harness                                          | RemoteClaw is healthy, but the target CLI/provider is not logged in.                                                   | Log in or provide the required provider key on the Gateway host environment.                                                                                             |
...
| ACP session stalls indefinitely after completing work                       | Harness process finished but ACP session did not report completion.                                                    | Update RemoteClaw; current acpx cleanup reaps RemoteClaw-owned stale wrapper and adapter processes on close and Gateway startup.                                         |
| Harness sees `<<<BEGIN_REMOTECLAW_INTERNAL_CONTEXT>>>`                      | Internal event envelope leaked across the ACP boundary.                                                                | Update RemoteClaw and rerun the completion flow; external harnesses should receive plain completion prompts only.                                                        |
...
native tool call, restart the Codex app-server or RemoteClaw Gateway instead of
