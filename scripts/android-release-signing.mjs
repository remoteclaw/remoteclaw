...
const requiredPropertyNames = [
  "REMOTECLAW_ANDROID_STORE_FILE",
  "REMOTECLAW_ANDROID_STORE_PASSWORD",
  "REMOTECLAW_ANDROID_KEY_ALIAS",
  "REMOTECLAW_ANDROID_KEY_PASSWORD",
];
const sourceRequiredPropertyNames = requiredPropertyNames.filter(
  (name) => name !== "REMOTECLAW_ANDROID_STORE_FILE",
);
...
function parseArgs(argv) {
  const options = {
    ...
    workspace: "",
    materializedDir: "",
    keystorePath: process.env.REMOTECLAW_ANDROID_UPLOAD_KEYSTORE || "",
    propertiesPath: process.env.REMOTECLAW_ANDROID_SIGNING_PROPERTIES || "",
  };
...
function writeMaterializedProperties(materializedDir, sourceProperties) {
  ...
  properties.set("REMOTECLAW_ANDROID_STORE_FILE", keystorePath);
  ...
  const content = [
    ...
    ...requiredPropertyNames.map((name) => `${name}=${String(properties.get(name))}`),
    "",
  ].join("\n");
  try {
...
function validateMaterializedSigning(materializedDir) {
  ...
  requireProperties(properties, requiredPropertyNames, propertiesPath);
  if (properties.get("REMOTECLAW_ANDROID_STORE_FILE") !== keystorePath) {
    throw new Error(
      `${relativePath(propertiesPath)} must point REMOTECLAW_ANDROID_STORE_FILE at ${relativePath(keystorePath)}.`,
    );
  }
}
...
function requirePushSources(options) {
  if (!options.keystorePath) {
    throw new Error(
      "Missing Android upload keystore source. Pass --keystore or set REMOTECLAW_ANDROID_UPLOAD_KEYSTORE.",
    );
  }
  if (!options.propertiesPath) {
    throw new Error(
      "Missing Android signing properties source. Pass --properties or set REMOTECLAW_ANDROID_SIGNING_PROPERTIES.",
    );
  }
