...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: RemoteClawConfig) => unknown;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
function _successfulProbeResult(url: string) {
  return {
    ok: true,
    url,
    connectLatencyMs: 1,
...
describe("security audit", () => {
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    const prevStateDir = process.env.REMOTECLAW_STATE_DIR;
    const tmp = await makeTmpDir("discord");
    process.env.REMOTECLAW_STATE_DIR = tmp;
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    it("prefers env password over remote password", async () => {
      process.env.REMOTECLAW_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: RemoteClawConfig = {
        gateway: {
          mode: "remote",
