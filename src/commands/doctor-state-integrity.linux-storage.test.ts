...
describe("detectLinuxSdBackedStateDir", () => {
  ...
  it("uses resolved state path to select mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/tmp/remoteclaw-state", {
      platform: "linux",
      mountInfo,
      resolveRealPath: () => "/mnt/slow/remoteclaw/.remoteclaw",
    });

    expect(result).toEqual({
      path: "/mnt/slow/remoteclaw/.remoteclaw",
      mountPoint: "/mnt/slow",
      fsType: "ext4",
  ...
  it("returns null outside linux", () => {
    ...
    const result = detectLinuxSdBackedStateDir(path.join("/Users", "tester", ".remoteclaw"), {
      platform: "darwin",
      mountInfo,
    });
  ...
  it("escapes decoded mountinfo control characters in warning output", () => {
    ...
    const stateDir = `${mountRoot}/.remoteclaw`;
