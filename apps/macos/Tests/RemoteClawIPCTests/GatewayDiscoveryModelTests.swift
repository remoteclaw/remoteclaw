...
struct GatewayDiscoveryModelTests {
    ...
    @Test func localGatewayMatchesDisplayName() {
        ...
        #expect(GatewayDiscoveryModel.isLocalGateway(
            lanHost: nil,
            tailnetDns: nil,
            displayName: "Peter's Mac Studio (RemoteClaw)",
            serviceName: nil,
            local: local))
    }
    ...
    @Test func serviceNameDoesNotFalsePositiveOnSubstringHostToken() {
        ...
        #expect(!GatewayDiscoveryModel.isLocalGateway(
            ...
            tailnetDns: nil,
            displayName: nil,
            serviceName: "steipetacstudio (RemoteClaw)",
            local: local))
        #expect(GatewayDiscoveryModel.isLocalGateway(
            lanHost: nil,
            tailnetDns: nil,
            displayName: nil,
            serviceName: "steipete (RemoteClaw)",
            local: local))
    }
    ...
    @Test func parsesGatewayTXTFields() {
        ...
        #expect(parsed.cliPath == "/opt/remoteclaw")
    }
