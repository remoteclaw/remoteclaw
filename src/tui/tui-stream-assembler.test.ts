...
const pairingQr = (terminalText: string) =>
  ({ type: "remoteclaw_pairing_qr", terminalText }) as const;
...
describe("TuiStreamAssembler", () => {
  ...
  it("renders pairing QR terminal text from final assistant content", () => {
    ...
    const finalText = assembler.finalize(
      "run-pair-qr",
      messageWithContent([
        text("Scan this QR code with the RemoteClaw iOS app:"),
        pairingQr("\u001b[47m\u001b[30m█ ▄\u001b[0m"),
      ]),
      false,
    );
    expect(finalText).toContain("Scan this QR code with the RemoteClaw iOS app:");
