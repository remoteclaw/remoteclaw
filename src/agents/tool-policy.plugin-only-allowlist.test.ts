...
const pluginGroups: PluginToolGroups = {
  all: ["crab", "workflow_tool"],
  byPlugin: new Map([["crab", ["crab", "workflow_tool"]]]),
};
...
describe("stripPluginOnlyAllowlist", () => {
  it("strips allowlist when it only targets plugin tools", () => {
    const policy = stripPluginOnlyAllowlist({ allow: ["crab"] }, pluginGroups, coreTools);
  ...
  it("keeps allowlist when it mixes plugin and core entries", () => {
    const policy = stripPluginOnlyAllowlist({ allow: ["crab", "exec"] }, pluginGroups, coreTools);
    expect(policy.policy?.allow).toEqual(["crab", "exec"]);
    expect(policy.unknownAllowlist).toEqual([]);
  });

  it("strips allowlist with unknown entries when no core tools match", () => {
    const emptyPlugins: PluginToolGroups = { all: [], byPlugin: new Map() };
    const policy = stripPluginOnlyAllowlist({ allow: ["crab"] }, emptyPlugins, coreTools);
    expect(policy.policy?.allow).toBeUndefined();
    expect(policy.unknownAllowlist).toEqual(["crab"]);
  });

  it("keeps allowlist with core tools and reports unknown entries", () => {
    const emptyPlugins: PluginToolGroups = { all: [], byPlugin: new Map() };
    const policy = stripPluginOnlyAllowlist({ allow: ["exec", "crab"] }, emptyPlugins, coreTools);
    expect(policy.policy?.allow).toEqual(["exec", "crab"]);
    expect(policy.unknownAllowlist).toEqual(["crab"]);
  });
});
