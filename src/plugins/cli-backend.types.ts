...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before RemoteClaw consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether RemoteClaw should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume RemoteClaw's bundle
   ...
   *
   * Use this when the backend should consume a canonical RemoteClaw auth profile
   ...
   * `profile-only` treats the selected RemoteClaw auth profile as the sole auth
   ...
   * as static config, such as mapping RemoteClaw thinking levels to a backend's
   * native effort flag.
   */
  resolveExecutionArgs?: CliBackendResolveExecutionArgs;
  /**
   * Whether this CLI backend can expose native tools outside RemoteClaw's tool
