...
function listAuthProfileRepairCandidates(
  ...
  const envAgentDir = readNonEmptyString(env.REMOTECLAW_AGENT_DIR);
  if (envAgentDir) {
    ...
    note(
      [
        ...
        ...(unreferencedSidecars.length > 0
          ...
          : []),
        `- ${formatCliCommand("remoteclaw doctor --fix")} migrates active profiles back to inline OAuth credentials and removes only sidecar files it successfully migrated.`,
      ].join("\n"),
      "Auth profiles",
    );
  }
