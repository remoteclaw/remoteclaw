...
read_when:
  ...
  - You want a reference for all registration methods on RemoteClawPluginApi
...
The `register(api)` callback receives an `RemoteClawPluginApi` object with these
methods:
...
<Warning>
  Never import your own plugin through `remoteclaw/plugin-sdk/<your-plugin>`
