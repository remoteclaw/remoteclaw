...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("remoteclaw doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with REMOTECLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with REMOTECLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.REMOTECLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://remoteclaw-gateway.ai:18789",
      onConnectError,
    });
