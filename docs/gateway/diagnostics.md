...
RemoteClaw can create a local diagnostics zip for bug reports. It combines
...
2. RemoteClaw sends the diagnostics preamble and asks for one explicit exec
   approval. The approval runs `remoteclaw gateway diagnostics export --json`.
   Do not approve diagnostics through an allow-all rule.
3. After approval, RemoteClaw replies with a pasteable report containing the local
   bundle path, manifest summary, privacy notes, and relevant session ids.

In group chats, an owner can still run `/diagnostics`, but RemoteClaw does not
...
that the diagnostics flow was sent privately. If RemoteClaw cannot find a private
owner route, the command fails closed and asks the owner to run it from a DM.

When the active RemoteClaw session is using the native OpenAI Codex harness,
the same exec approval also covers an OpenAI feedback upload for the Codex
runtime threads RemoteClaw knows about. That upload is separate from the local
...
the channels, RemoteClaw session ids, Codex thread ids, and local resume commands
for the threads that were sent to OpenAI servers. If you deny or ignore the
approval, RemoteClaw does not run the export, does not send Codex feedback, and
...
Persisted bundles live under `~/.remoteclaw/logs/stability/` when events exist.

## Useful options

```bash
remoteclaw gateway diagnostics export \
  --output remoteclaw-diagnostics.zip \
  --log-lines 5000 \
  --log-bytes 1000000
```
