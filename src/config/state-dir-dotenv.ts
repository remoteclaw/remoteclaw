...
export function isUnresolvedShellReference(value: string): boolean {
  ...
  //   $VAR_NAME          (simple reference, RemoteClaw env-var style)
...
type ParsedStateDirDotEnv = {
   ...
   * reference ($VAR, ${VAR}, or $(cmd)). These are still RemoteClaw-managed keys:
 ...
 * literal references for keys RemoteClaw previously managed.
 */
...
/**
 * Read and parse `~/.remoteclaw/.env` (or `$REMOTECLAW_STATE_DIR/.env`), returning
