...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `remoteclaw.internal.`), **or**
...
- `REMOTECLAW_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine RemoteClaw iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight RemoteClaw builds.
...
```bash
export REMOTECLAW_APNS_TEAM_ID="TEAMID"
export REMOTECLAW_APNS_KEY_ID="KEYID"
export REMOTECLAW_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
The Gateway advertises `_remoteclaw-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `remoteclaw.internal.`) and Tailscale split DNS.
...
```bash
remoteclaw nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__remoteclaw__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__remoteclaw__/canvas/` and `/__remoteclaw__/a2ui/`.
...
```bash
remoteclaw nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__remoteclaw; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
remoteclaw nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
