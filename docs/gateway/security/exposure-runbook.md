---
summary: "Pre-flight and rollback checklist before exposing an RemoteClaw Gateway beyond loopback"
...
- Backup location for `~/.remoteclaw/remoteclaw.json` and credentials.
...
```bash
remoteclaw gateway probe --url ws://127.0.0.1:18789 --token "$REMOTECLAW_GATEWAY_TOKEN"
```
...
- Same-host (loopback-source) reverse proxies cannot satisfy trusted-proxy auth at all, and neither can a proxy connecting from one of the Gateway's own interface addresses. Reach the Gateway from an address the Gateway does not hold, or use `gateway.auth.password` for those callers.
...
1. Re-run `remoteclaw security audit --deep`.
...
5. Re-run `remoteclaw security audit --deep`.
