...
describe("resolveVapidKeys", () => {
  it("generates and persists VAPID keys on first call", async () => {
    ...
    expect(keys.subject).toBe("https://remoteclaw.org");
    const persistedKeys = JSON.parse(
      await fs.readFile(path.join(tmpDir, "push", "vapid-keys.json"), "utf8"),
    ) as { subject?: string };
    expect(persistedKeys.subject).toBe("https://remoteclaw.org");
  ...
  it("prefers env vars over persisted keys", async () => {
    ...
    // Set env overrides.
    process.env.REMOTECLAW_VAPID_PUBLIC_KEY = "env-public";
    process.env.REMOTECLAW_VAPID_PRIVATE_KEY = "env-private";
    process.env.REMOTECLAW_VAPID_SUBJECT = "mailto:env@test.com";
    try {
      const keys = await resolveVapidKeys(tmpDir);
      ...
      delete process.env.REMOTECLAW_VAPID_PUBLIC_KEY;
      delete process.env.REMOTECLAW_VAPID_PRIVATE_KEY;
      delete process.env.REMOTECLAW_VAPID_SUBJECT;
    }
  });
});
