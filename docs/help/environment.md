...
Workspace `.env` files are a lower-trust source: RemoteClaw ignores provider credentials and protected runtime controls from workspace `.env` before applying precedence.
...
3. **Global `.env`** at `~/.remoteclaw/.env` (aka `$REMOTECLAW_STATE_DIR/.env`; recommended for provider API keys; does not override).
...
Do not keep provider API keys only in a workspace `.env`. RemoteClaw ignores provider credential environment variables from workspace `.env` files, including common keys such as `GEMINI_API_KEY`, `GOOGLE_API_KEY`, `XAI_API_KEY`, `MISTRAL_API_KEY`, `GROQ_API_KEY`, `DEEPSEEK_API_KEY`, `PERPLEXITY_API_KEY`, `BRAVE_API_KEY`, `TAVILY_API_KEY`, `EXA_API_KEY`, and `FIRECRAWL_API_KEY`.
...
- The global runtime dotenv file at `~/.remoteclaw/.env` or `$REMOTECLAW_STATE_DIR/.env`.
- The config `env` block in `~/.remoteclaw/remoteclaw.json`.
- Optional login-shell import when `env.shellEnv.enabled` or `REMOTECLAW_LOAD_SHELL_ENV=1` is enabled.

If you previously stored provider keys only in a workspace `.env`, move them to one of the trusted sources above. Workspace `.env` can still provide ordinary project variables that are not credentials, endpoint redirects, host overrides, or `REMOTECLAW_*` runtime controls.
...
Set `REMOTECLAW_EXEC_SHELL_SNAPSHOT=0` in the Gateway process environment to disable this path.
...
- `REMOTECLAW_CLI=1`: set for child processes spawned by the CLI entry point.
...
| `REMOTECLAW_INCLUDE_ROOTS` | Path-list of directories where `$include` directives may resolve files outside the config directory (default: none — `$include` is confined to the config dir). Tilde-expanded.                                                               |
...
| `REMOTECLAW_DEBUG_MODEL_TRANSPORT` | Emit targeted model request/response timing diagnostics at `info` level without enabling global debug logs.                                                                                  |
| `REMOTECLAW_DEBUG_MODEL_PAYLOAD`   | Model payload diagnostics: `summary`, `tools`, or `full-redacted`. `full-redacted` is capped and redacted but may include prompt/message text.                                               |
| `REMOTECLAW_DEBUG_SSE`             | Streaming diagnostics: `events` for first/done timing, `peek` to include the first five redacted SSE events.                                                                                 |
| `REMOTECLAW_DEBUG_CODE_MODE`       | Code-mode model-surface diagnostics, including provider-tool hiding and exec/wait-only enforcement.                                                                                          |
