...
function mockEmbeddedResponse(text: string) {
  vi.mocked(runAgent).mockResolvedValue(makeEmbeddedTextResult(text));
}
...
async function runInFlightVerboseToggleCase(params: {
  ...
  vi.mocked(runAgent).mockImplementation((async (agentParams: Record<string, unknown>) => {
    const shouldEmit = agentParams.shouldEmitToolResult as (() => boolean) | undefined;
    ...
    expect((shouldEmit as (() => boolean) | undefined)?.()).toBe(!params.shouldEmitBefore);
