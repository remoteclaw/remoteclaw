...
function hasPackagePluginMetadata(manifest: Record<string, unknown>): boolean {
  const remoteclaw = manifest.remoteclaw;
  if (!isRecord(remoteclaw)) {
    return false;
  }
  const extensions = remoteclaw.extensions;
...
function readManifestPluginId(packageDir: string): string | undefined {
  const manifest = readJsonObjectFileSync(path.join(packageDir, "remoteclaw.plugin.json"));
