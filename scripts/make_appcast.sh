...
ZIP=${1:?"Usage: $0 RemoteClaw-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/remoteclaw/remoteclaw/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  ...
  if [[ "$ZIP_NAME" =~ ^RemoteClaw-([0-9]+(\.[0-9]+){1,2}([-.][0-9A-Za-z]+([.-][0-9A-Za-z]+)*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/remoteclaw/remoteclaw/releases/download/v${VERSION}/"}
