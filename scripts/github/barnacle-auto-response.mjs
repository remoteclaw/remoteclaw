...
export const rules = [
  ...
  },
  {
    ...
    close: true,
    message:
      "Please use [our support server](https://discord.gg/clawd) and ask in #help or #users-helping-users to resolve this, or follow the stuck FAQ at https://docs.remoteclaw.org/help/faq#im-stuck-whats-the-fastest-way-to-get-unstuck.",
  },
  {
    label: "r: no-ci-pr",
    close: true,
  ...
  },
  {
    ...
    close: true,
    message:
      "Please publish this as a third-party plugin on [ClawHub](https://clawhub.ai) instead of adding it to the core repo. Docs: https://docs.remoteclaw.org/plugin and https://docs.remoteclaw.org/tools/clawhub",
  },
  {
    label: "r: moltbook",
    close: true,
    ...
    message:
      "RemoteClaw is not affiliated with Moltbook, and issues related to Moltbook should not be submitted here.",
  },
];
...
export const managedLabelSpecs = {
  ...
  "r: moltbook": {
    color: "B60205",
    description: "Auto-close and lock: Moltbook is off-topic for RemoteClaw.",
  },
  "r: spam": {
    color: "B60205",
...
export function hasLinkedReference(text) {
  return /(?:#\d+|github\.com\/remoteclaw\/remoteclaw\/(?:issues|pull)\/\d+)/i.test(text);
}
...
export function classifyPullRequestCandidateLabels(pullRequest, files) {
  ...
  const addsPluginManifest = files.some(
    (file) =>
      file.status === "added" &&
      /^extensions\/[^/]+\/remoteclaw\.plugin\.json$/i.test(file.filename),
  );
  if (
    !clearDesignContext &&
    (addsPluginManifest ||
