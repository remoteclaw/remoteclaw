...
RemoteClaw also includes `x_search` for X (formerly Twitter) posts and
...
<Steps>
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "RemoteClaw plugin SDK" });
    ```
...
Codex-capable models can optionally use the provider-native Responses `web_search` tool instead of RemoteClaw's managed `web_search` function.
...
If native Codex search is enabled but the current model is not Codex-capable, RemoteClaw keeps the normal managed `web_search` behavior.
...
<Note>
  ...
  RemoteClaw resolves only the selected provider key -- non-selected SecretRefs
  stay inactive.
</Note>
...
Legacy `tools.web.x_search.*` config is auto-migrated by `remoteclaw doctor --fix`.
When you choose Grok during `remoteclaw onboard` or `remoteclaw configure --section web`,
RemoteClaw can also offer optional `x_search` setup with the same key.
This is a separate follow-up step inside the Grok path, not a separate top-level
web-search provider choice. If you pick another provider, RemoteClaw does not
...
<Tabs>
  <Tab title="Config file">
    Run `remoteclaw configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.remoteclaw/.env`.
...
optional structured filters. RemoteClaw only enables the built-in xAI `x_search`
