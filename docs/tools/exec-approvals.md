...
in `~/.remoteclaw/exec-approvals.json` keeps prompting even if session or config
defaults request `ask: "on-miss"`.
</Note>

## Inspecting the effective policy

- `remoteclaw approvals get`, `... --gateway`, `... --node <id|name|ip>` — show requested policy, host policy sources, and the effective result.
...
- CLI-backed providers that expose their own noninteractive permission mode can follow this policy.
  Claude CLI adds `--permission-mode bypassPermissions` when RemoteClaw's requested exec policy is
