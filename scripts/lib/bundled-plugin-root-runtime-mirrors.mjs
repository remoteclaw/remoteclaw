...
function usesStagedRuntimeDependencies(packageJson) {
  return packageJson?.remoteclaw?.bundle?.stageRuntimeDependencies === true;
}
...
export function collectBundledPluginRootRuntimeMirrorErrors(params) {
  ...
  const declaredMirrorDeps =
    params.rootPackageJson?.remoteclaw?.bundle?.mirroredRootRuntimeDependencies ?? [];
    ...
    if (declaredRootRuntimeDeps.has(dependencyName)) {
      if (!declaredMirrorDepNames.has(dependencyName)) {
        const importerList = Array.from(record.importers)
          ...
          .join(", ");
        errors.push(
          `installed package root mirror '${dependencyName}' for dist importers: ${importerList} is missing from package.json remoteclaw.bundle.mirroredRootRuntimeDependencies. Add it there so packaged runtime installs the mirrored dependency, or keep imports under dist/extensions/${record.pluginIds[0]}/.`,
        );
      }
      continue;
    }
...
export function collectDeclaredRootRuntimeDependencyMetadataErrors(rootPackageJson) {
  ...
  const declaredMirrorDeps =
    rootPackageJson?.remoteclaw?.bundle?.mirroredRootRuntimeDependencies ?? [];
  if (!Array.isArray(declaredMirrorDeps)) {
    return ["package.json remoteclaw.bundle.mirroredRootRuntimeDependencies must be an array."];
  }
  ...
  for (const dependencyName of declaredMirrorDeps) {
    if (typeof dependencyName !== "string" || dependencyName.trim().length === 0) {
      errors.push(
        "package.json remoteclaw.bundle.mirroredRootRuntimeDependencies entries must be non-empty strings.",
      );
      continue;
    }
    if (!declaredRootRuntimeDeps.has(dependencyName)) {
      errors.push(
        `package.json remoteclaw.bundle.mirroredRootRuntimeDependencies declares '${dependencyName}' but package.json dependencies/optionalDependencies do not include it.`,
      );
    }
  }
  return errors.toSorted((left, right) => left.localeCompare(right));
