...
3. Report the result on your plugin's tracking issue in [remoteclaw/remoteclaw](https://github.com/remoteclaw/remoteclaw/issues) after testing, with either `all good` or what broke. Open one if you do not have it yet.
...
5. Open a PR to `main` titled `fix(<plugin-id>): beta blocker - <summary>` and link the issue in the PR. Contributors cannot label PRs, so the title is the PR-side signal for maintainers and automation. Blockers with a PR get merged; blockers without one might ship anyway.
