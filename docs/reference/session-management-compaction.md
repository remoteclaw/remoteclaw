...
RemoteClaw manages sessions end-to-end across these areas:
...
`REMOTECLAW_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS`, `REMOTECLAW_SESSION_WRITE_LOCK_STALE_MS`, and
`REMOTECLAW_SESSION_WRITE_LOCK_MAX_HOLD_MS`.
...
- **Parent fork policy** uses RemoteClaw's active branch when creating a thread or subagent fork. If that branch is too large, RemoteClaw starts the child with isolated context instead of failing or inheriting unusable history. The sizing policy is automatic; legacy `session.parentForkMaxTokens` config is removed by `remoteclaw doctor --fix`.
...
Transcripts are managed by `remoteclaw/plugin-sdk/agent-sessions`'s `SessionManager`.
...
RemoteClaw does not append AGENTS.md excerpts on top of the compaction summary.
...
## When auto-compaction happens (RemoteClaw runtime)

In the embedded RemoteClaw agent, auto-compaction triggers in two cases:
...
exceeded`, and similar provider-shaped variants) → compact → retry.
   When the provider reports the attempted token count, RemoteClaw forwards that
   observed count into overflow recovery compaction. If the provider confirms
   overflow but does not expose a parseable count, RemoteClaw passes a minimally
   over-budget synthetic count to compaction engines and diagnostics.
   If overflow recovery still fails, RemoteClaw surfaces explicit guidance to the
...
These are RemoteClaw runtime semantics.

RemoteClaw can also trigger a preflight local compaction before opening the next
...
reopen cost, not raw archival: RemoteClaw still runs normal semantic compaction,
and it requires `truncateAfterCompaction` so the compacted summary can become a
new successor transcript.

For embedded RemoteClaw runs, `agents.defaults.compaction.midTurnPrecheck.enabled: true`
adds an opt-in tool-loop guard. After a tool result is appended and before the
next model call, RemoteClaw estimates the prompt pressure using the same preflight
budget logic used at turn start. If the context no longer fits, the guard does
not compact inside RemoteClaw runtime's `transformContext` hook. It raises a structured
...
before a turn opens, while mid-turn precheck runs later in the embedded RemoteClaw tool
...
RemoteClaw runtime's compaction settings live in agent settings:
...
- Manual `/compact` honors an explicit `agents.defaults.compaction.keepRecentTokens`
  and keeps RemoteClaw runtime's recent-tail cut point. Without an explicit keep budget,
...
- When `agents.defaults.compaction.truncateAfterCompaction` is enabled,
  RemoteClaw rotates the active transcript to a compacted successor JSONL after
...
- If the provider fails or returns an empty result, RemoteClaw falls back to built-in LLM summarization automatically.
...
2. When it crosses a "soft threshold" (below RemoteClaw runtime's compaction threshold), run a silent
...
- The flush runs only for embedded RemoteClaw sessions (CLI backends skip it).
- The flush is skipped when the session workspace is read-only (`workspaceAccess: "ro"` or `"none"`).
- See [Memory](/concepts/memory) for the workspace file layout and write patterns.

RemoteClaw also exposes a `session_before_compact` hook in the extension API, but RemoteClaw's
