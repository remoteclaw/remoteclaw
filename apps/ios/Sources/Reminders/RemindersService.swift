...
final class RemindersService: RemindersServicing {
    func list(params: RemoteClawRemindersListParams) async throws -> RemoteClawRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[RemoteClawReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return RemoteClawReminderPayload(
        ...
        return RemoteClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: RemoteClawRemindersAddParams) async throws -> RemoteClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = RemoteClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return RemoteClawRemindersAddPayload(reminder: payload)
    }
