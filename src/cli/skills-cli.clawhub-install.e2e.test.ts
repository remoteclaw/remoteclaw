...
async function spawnRemoteClaw(
  args: string[],
...
describe("remoteclaw skills install ClawHub GitHub-backed E2E", () => {
  it("installs from the install resolver and reports install telemetry", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "remoteclaw-clawhub-cli-e2e-"));
    try {
      const result = await spawnRemoteClaw(["skills", "install", "aiq-deploy", "--global"], {
        cwd: process.cwd(),
        env: {
          ...process.env,
          REMOTECLAW_STATE_DIR: stateDir,
          REMOTECLAW_CONFIG_PATH: path.join(stateDir, "remoteclaw.json"),
          REMOTECLAW_CLAWHUB_URL: registry,
          REMOTECLAW_CLAWHUB_TOKEN: "test-token",
          REMOTECLAW_CLAWHUB_GITHUB_CODELOAD_BASE_URL: registry,
          CLAWHUB_DISABLE_TELEMETRY: "",
          CLAWDHUB_DISABLE_TELEMETRY: "",
          REMOTECLAW_DISABLE_BUNDLED_PLUGINS: "1",
        },
      });
