...
private let cacheLogger = Logger(subsystem: "org.remoteclaw", category: "RemoteClawChatTranscriptCache")

private final class OutboxChangeHub: @unchecked Sendable {
    private let lock = NSLock()
    private var continuations: [UUID: AsyncStream<RemoteClawChatOutboxChange>.Continuation] = [:]

    func stream() -> AsyncStream<RemoteClawChatOutboxChange> {
        let id = UUID()
        let pair = AsyncStream<RemoteClawChatOutboxChange>.makeStream()
        self.lock.lock()
    ...
    func yield(_ change: RemoteClawChatOutboxChange) {
        self.lock.lock()
...
public protocol RemoteClawChatTranscriptCache: Sendable {
    func loadSessions() async -> [RemoteClawChatSessionEntry]
    func loadTranscript(sessionKey: String) async -> [RemoteClawChatMessage]
    func loadTranscript(sessionKey: String, agentID: String?) async -> [RemoteClawChatMessage]
    func storeSessions(_ sessions: [RemoteClawChatSessionEntry]) async
    func storeTranscript(sessionKey: String, messages: [RemoteClawChatMessage]) async
    func storeTranscript(sessionKey: String, agentID: String?, messages: [RemoteClawChatMessage]) async
    ...
    func storeCanonicalTranscript(
        sessionKey: String,
        messages: [RemoteClawChatMessage],
...
extension RemoteClawChatTranscriptCache {
    public func loadTranscript(sessionKey: String, agentID: String?) async -> [RemoteClawChatMessage] {
    ...
    public func storeTranscript(
        sessionKey: String,
        agentID: String?,
        messages: [RemoteClawChatMessage]) async
    {
        guard agentID == nil else { return }
...
public protocol RemoteClawChatCanonicalTranscriptMerging: RemoteClawChatTranscriptCache {
    func mergeCanonicalTranscriptMessage(
        sessionKey: String,
        agentID: String?,
        message: RemoteClawChatMessage,
        canonicalMessageIdempotencyKey: String) async
}

/// One attachment captured with a durable chat command.
public struct RemoteClawChatOutboxAttachment: Codable, Hashable, Sendable {
...
public struct RemoteClawChatOutboxCommand: Hashable, Sendable, Identifiable {
    ...
    public let attachments: [RemoteClawChatOutboxAttachment]
    ...
    public init(
        ...
        agentID: String? = nil,
        text: String,
        attachments: [RemoteClawChatOutboxAttachment] = [],
        thinking: String,
        createdAt: Double,
        status: Status,
        retryCount: Int,
...
public enum RemoteClawChatOutboxUpdateResult: Equatable, Sendable {
    case updated
    case confirmed
    case missing
    case unavailable
...
public enum RemoteClawChatOutboxChange: Equatable, Sendable {
...
public protocol RemoteClawChatCommandOutbox: Sendable {
    ...
    func enqueueCommand(_ command: RemoteClawChatOutboxCommand) async -> Bool
    ...
    func claimNextCommand() async -> RemoteClawChatOutboxCommand?
    func markCommandQueued(id: String, retryCount: Int, lastError: String?) async
    func markCommandAwaitingConfirmation(id: String) async -> RemoteClawChatOutboxUpdateResult
    ...
    func markCommandFailedIfPresent(
        id: String,
        retryCount: Int,
        lastError: String?) async -> RemoteClawChatOutboxUpdateResult
    ...
    func markCommandRetriedIfPresent(
        ...
        routingContract: String) async -> RemoteClawChatOutboxUpdateResult
    ...
    func cancelCommand(id: String) async -> RemoteClawChatOutboxUpdateResult
    /// Canonical gateway history may complete any row, including a sending
    /// row whose request ACK was lost.
    func confirmCommand(id: String) async -> RemoteClawChatOutboxUpdateResult
    /// Cross-view-model invalidation.
    func changes() -> AsyncStream<RemoteClawChatOutboxChange>
}

public struct RemoteClawChatSessionRoutingIdentity: Equatable, Sendable {
    ...
    public init?(contract: String?) {
        guard let components = RemoteClawChatSessionRoutingContract.parse(contract) else { return nil }
    ...
    public init?(scope: String?, mainSessionKey: String?, defaultAgentID: String?) {
        guard let contract = RemoteClawChatSessionRoutingContract.make(
            scope: scope,
...
public actor RemoteClawChatSQLiteTranscriptCache: RemoteClawChatTranscriptCache,
    RemoteClawChatCanonicalTranscriptMerging,
    RemoteClawChatCommandOutbox
{
    ...
    public nonisolated static func loadSessionRoutingIdentity(
        databaseURL: URL,
        gatewayID: String) -> RemoteClawChatSessionRoutingIdentity?
    {
        ...
        return RemoteClawChatSessionRoutingIdentity(
    ...
    // MARK: - RemoteClawChatTranscriptCache

    public func loadSessions() async -> [RemoteClawChatSessionEntry] {
        ...
        guard let decoded = try? JSONDecoder().decode(
            [RemoteClawChatSessionEntry].self,
    ...
    public func loadTranscript(sessionKey: String) async -> [RemoteClawChatMessage] {
    ...
    private func readTranscript(
        _ db: OpaquePointer,
        sessionKey: String,
        agentID: String?) -> [RemoteClawChatMessage]
    {
    ...
    public func storeSessions(_ sessions: [RemoteClawChatSessionEntry]) async {
        guard !self.isRetired else { return }
        guard let db = await handle() else { return }
    ...
    public func loadSessionRoutingIdentity() async -> RemoteClawChatSessionRoutingIdentity? {
    ...
    public func storeSessionRoutingIdentity(_ identity: RemoteClawChatSessionRoutingIdentity) async {
    ...
    public func storeTranscript(sessionKey: String, messages: [RemoteClawChatMessage]) async {
    ...
    // MARK: - RemoteClawChatCommandOutbox

    public nonisolated func changes() -> AsyncStream<RemoteClawChatOutboxChange> {
        self.outboxChangeHub.stream()
    }

    public func enqueueCommand(_ command: RemoteClawChatOutboxCommand) async -> Bool {
    ...
    public func loadCommands() async -> [RemoteClawChatOutboxCommand] {
        await self.loadCommandsIfAvailable() ?? []
    }

    public func loadCommandsIfAvailable() async -> [RemoteClawChatOutboxCommand]? {
        guard !self.isRetired, let db = await handle() else { return nil }
    ...
    public func claimNextCommand() async -> RemoteClawChatOutboxCommand? {
        guard !self.isRetired, let db = await handle() else { return nil }
    ...
    public func markCommandAwaitingConfirmation(id: String) async -> RemoteClawChatOutboxUpdateResult {
        ...
        routingContract: String) async -> RemoteClawChatOutboxUpdateResult
    {
        ...
        let normalizedRoutingContract = routingContract.trimmingCharacters(in: .whitespacesAndNewlines)
        let allowsUntargetedAgent = normalizedRoutingContract == RemoteClawChatOutboxCommand
    ...
    public func cancelCommand(id: String) async -> RemoteClawChatOutboxUpdateResult {
        guard !self.isRetired, let db = await handle() else { return .unavailable }
        guard self.execute(db, sql: "BEGIN IMMEDIATE", bindings: []) else { return .unavailable }
        var committed = false
        defer {
        ...
        let result: RemoteClawChatOutboxUpdateResult
        do {
            let isProven = self.canonicalMessageProofHub.lockProofDecision(for: messageKey)
            defer { self.canonicalMessageProofHub.unlockProofDecision() }
            if isProven {
    ...
    public func confirmCommand(id: String) async -> RemoteClawChatOutboxUpdateResult {
        guard !self.isRetired, let db = await handle() else { return .unavailable }
        guard self.execute(
            db,
    ...
    private func emitOutboxChange(_ change: RemoteClawChatOutboxChange) {
    ...
    static func cacheableMessages(_ messages: [RemoteClawChatMessage]) -> [RemoteClawChatMessage] {
    ...
    static func boundedSessions(_ sessions: [RemoteClawChatSessionEntry]) -> [RemoteClawChatSessionEntry] {
    ...
    private static func attachmentByteCount(_ attachments: [RemoteClawChatOutboxAttachment]) -> Int? {
        var total = 0
    ...
    private func readCommands(_ db: OpaquePointer) -> [RemoteClawChatOutboxCommand]? {
        var statement: OpaquePointer?
        let sql = """
        ...
        var commands: [RemoteClawChatOutboxCommand] = []
        ...
        while step == SQLITE_ROW {
               ...
               let text = sqlite3_column_text(statement, 5)
            {
                ...
                guard let attachments = try? JSONDecoder().decode(
                    [RemoteClawChatOutboxAttachment].self,
                ...
                if let status = RemoteClawChatOutboxCommand.Status(rawValue: statusRaw) {
                    commands.append(
                        RemoteClawChatOutboxCommand(
