...
type RemoteClawMcpHttpTransport = "sse" | "streamable-http";

const CLI_MCP_TYPE_TO_REMOTECLAW_TRANSPORT: Record<string, RemoteClawMcpHttpTransport | "stdio"> = {
...
/** Maps CLI-native MCP type aliases to RemoteClaw HTTP transport names. */
export function resolveRemoteClawMcpTransportAlias(
  value: unknown,
): RemoteClawMcpHttpTransport | undefined {
  const mapped = CLI_MCP_TYPE_TO_REMOTECLAW_TRANSPORT[normalizeMcpString(value)];
  return mapped === "sse" || mapped === "streamable-http" ? mapped : undefined;
}

/** Checks whether a raw MCP `type` value is a legacy CLI alias RemoteClaw can rewrite. */
export function isKnownCliMcpTypeAlias(value: unknown): boolean {
  return Object.hasOwn(CLI_MCP_TYPE_TO_REMOTECLAW_TRANSPORT, normalizeMcpString(value));
}
...
export function canonicalizeConfiguredMcpServer(
  ...
  const next = { ...server };
  const transportAlias = resolveRemoteClawMcpTransportAlias(next.type);
  // `transport` is RemoteClaw's canonical field; legacy `type` only fills a gap.
