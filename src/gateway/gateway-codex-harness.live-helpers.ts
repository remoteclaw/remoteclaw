export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current RemoteClaw session status reports the active model as:",
] as const;
...
export function isExpectedCodexStatusCommandText(text: string): boolean {
  ...
  const mentionsRemoteClawStatus =
    ...
    /remoteclaw\s+\S+\s+is running on/u.test(normalized) ||
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from remoteclaw status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live remoteclaw config shows") ||
