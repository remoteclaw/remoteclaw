...
const { makeStorePath } = createCronStoreHarness({ prefix: "remoteclaw-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses REMOTECLAW_HOME for tilde expansion", () => {
    vi.stubEnv("REMOTECLAW_HOME", "/srv/remoteclaw-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/remoteclaw-home", "cron", "jobs.json"));
  });
});
...
describe("cron store", () => {
  ...
  it.skipIf(process.platform === "win32")("writes store and backup files with secure permissions", async () => {
    const store = await makeStorePath();
    const first = makeStore("job-1", true);
    const second = makeStore("job-2", false);
