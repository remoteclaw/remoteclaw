...
Options:
  ...
  --keep                         Keep the synthetic REMOTECLAW_HOME and workspace after the run.
...
function parseArgs(argv) {
  ...
  const options = {
    fileCount: Number(process.env.REMOTECLAW_MEMORY_FD_REPRO_FILES || DEFAULT_FILE_COUNT),
    mode: process.env.REMOTECLAW_MEMORY_FD_REPRO_MODE || "fixed",
    maxWorkspaceRegFds: Number(
      process.env.REMOTECLAW_MEMORY_FD_REPRO_MAX_WORKSPACE_REG_FDS || DEFAULT_MAX_WORKSPACE_REG_FDS,
    ),
    minLeakedFds: undefined,
    invokeTimeoutMs: Number(process.env.REMOTECLAW_MEMORY_FD_REPRO_TIMEOUT_MS || 30_000),
    sampleDelayMs: Number(process.env.REMOTECLAW_MEMORY_FD_REPRO_SAMPLE_DELAY_MS || 1_000),
    settleDelayMs: Number(process.env.REMOTECLAW_MEMORY_FD_REPRO_SETTLE_DELAY_MS || 5_000),
    outputDir: path.resolve(".artifacts", "memory-fd-repro", stamp),
    keep: process.env.REMOTECLAW_MEMORY_FD_REPRO_KEEP === "1",
    allowNonDarwin: process.env.REMOTECLAW_MEMORY_FD_REPRO_ALLOW_NON_DARWIN === "1",
  };
...
async function main() {
  ...
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "remoteclaw-memory-fd-repro-"));
  ...
  const env = {
    ...
    ...SKIP_GATEWAY_ENV,
    HOME: homeDir,
    REMOTECLAW_STATE_DIR: path.join(homeDir, ".remoteclaw"),
  ...
  try {
    ...
    logStep(`files=${options.fileCount} mode=${options.mode} port=${String(port)}`);
