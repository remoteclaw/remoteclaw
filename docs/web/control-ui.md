...
If the browser retries pairing with changed auth details (role/scopes/public key), the previous pending request is superseded and a new `requestId` is created. Re-run `remoteclaw devices list` before approval.

If the browser is already paired and you change it from read access to write/admin access, this is treated as an approval upgrade, not a silent reconnect. RemoteClaw keeps the old approval active, blocks the broader reconnect, and asks you to approve the new scope set explicitly.
...
<AccordionGroup>
  ...
  <Accordion title="Config">
    - View/edit `~/.remoteclaw/remoteclaw.json` (`config.get`, `config.set`).
  ...
  <Accordion title="Cron jobs panel notes">
    ...
    - Deprecated fallback: run `remoteclaw doctor --fix` to migrate stored legacy jobs with `notify: true` from `cron.webhook` to explicit per-job webhook or completion delivery.
...
The dedicated MCP page is an operator view for RemoteClaw-managed MCP servers under `mcp.servers`. It does not start MCP transports by itself; use it to inspect and edit saved config, then use `remoteclaw mcp doctor --probe` when you need live server proof.
...
7. Run `remoteclaw mcp status --verbose`, `remoteclaw mcp doctor --probe`, or `remoteclaw mcp reload` from a terminal when the edited process needs static diagnostics, live proof, or cached-runtime disposal.
...
| `push/vapid-keys.json` (under the RemoteClaw state dir) | Auto-generated VAPID keypair used to sign Web Push payloads.       |
...
If you open the dashboard over plain HTTP (`http://<lan-ip>` or `http://<tailscale-ip>`), the browser runs in a **non-secure context** and blocks WebCrypto. By default, RemoteClaw **blocks** Control UI connections without device identity.
...
- `GET /__remoteclaw__/assistant-media?meta=1&source=<path>` requires the normal Control UI operator auth. The browser sends the gateway token as a bearer header when checking availability.
...
If the browser loads a blank dashboard and DevTools shows no useful error, an extension or early content script may have prevented the JavaScript module app from evaluating. The static page includes a plain HTML recovery panel that appears when `<remoteclaw-app>` is not registered after startup.
