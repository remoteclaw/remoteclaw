...
/**
 * Built-in RemoteClaw commands to register as native slash commands.
...
/**
 * Register all RemoteClaw slash commands for a given team.
...
export async function registerSlashCommands(params: {
  ...
  for (const spec of commands) {
    ...
    if (ownedCommands.length === 0 && foreignCommands.length > 0) {
      log?.(
        `mattermost: trigger /${spec.trigger} already used by non-RemoteClaw command(s); skipping to avoid mutating external integrations`,
      );
      continue;
    }
