...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: RemoteClawOAuthStore.AnthropicOAuthStatus = RemoteClawOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: RemoteClawOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = RemoteClawOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = RemoteClawOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try RemoteClawOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. RemoteClaw can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: RemoteClawOAuthStore.AnthropicOAuthStatus,
