...
interface EmbeddedGatewayRuntime {
  resolveSessionAgentId: (opts: { sessionKey: string; config: RemoteClawConfig }) => string;
  getRuntimeConfig: () => RemoteClawConfig;
  ...
  resolveEffectiveChatHistoryMaxChars: (cfg: RemoteClawConfig) => number;
  ...
  loadCombinedSessionStoreForGateway: (cfg: RemoteClawConfig) => {
    storePath: string;
    store: unknown;
  };
