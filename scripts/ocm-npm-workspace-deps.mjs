...
const WORKSPACE_DIRS_ENV = "REMOTECLAW_OCM_WORKSPACE_DEPENDENCY_DIRS";
const REAL_NPM_ENV = "REMOTECLAW_OCM_REAL_NPM_BIN";
...
export function buildInstallManifest(rootArchive, workspacePackages) {
  return {
    private: true,
    dependencies: {
      remoteclaw: pathToFileURL(rootArchive).href,
      ...Object.fromEntries(
...
function patchRootArchiveWorkspaceDependencies(rootArchive, workspacePackages, outputDir) {
  ...
  const patchedArchive = join(outputDir, "remoteclaw-root-patched.tgz");
...
function main() {
  ...
  const packDir = mkdtempSync(join(tmpdir(), "remoteclaw-ocm-workspace-deps-"));
  try {
