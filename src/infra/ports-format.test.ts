...
const gatewayAlreadyRunningHint = `Gateway already running locally. Stop it (${formatCliCommand("remoteclaw gateway stop")}) or use a different port.`;
...
describe("ports-format", () => {
  it.each([
    ...
    [{ command: "ssh" }, "ssh"],
    [{ commandLine: "node /Users/me/Projects/remoteclaw/dist/entry.js gateway" }, "gateway"],
  ...
  it("treats single-process loopback dual-stack gateway listeners as benign", () => {
    const listeners = [
      { pid: 4242, commandLine: "remoteclaw-gateway", address: "127.0.0.1:18789" },
  ...
  it.each([
    ...
    const listeners = [{ pid: 4242, commandLine: "remoteclaw-gateway", address }];
  ...
  it("keeps Gateway conflict hints for ambiguous Gateway listeners", () => {
    expect(
      buildPortHints(
        [
          { pid: 4242, commandLine: "remoteclaw-gateway", address: "0.0.0.0:18789" },
          { pid: 4243, commandLine: "remoteclaw-gateway", address: "127.0.0.1:18789" },
        ],
        18789,
      ),
