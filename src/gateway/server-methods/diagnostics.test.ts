...
describe("diagnostics gateway methods", () => {
  ...
  it("is registered in coreGatewayHandlers (wired dispatch path, not just the isolated handler)", async () => {
    const { coreGatewayHandlers } = await import("../server-methods.js");
    expect(typeof coreGatewayHandlers["diagnostics.stability"]).toBe("function");
  });
});
