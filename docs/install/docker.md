...
<Steps>
  <Step title="Build the image">
    ...
    [GitHub Container Registry](https://github.com/remoteclaw/remoteclaw/pkgs/container/remoteclaw).
  ...
  <Step title="Complete onboarding">
    ...
    `remoteclaw-gateway` directly. `remoteclaw-cli` is for commands you run after
...
docker compose run --rm --no-deps --entrypoint node remoteclaw-gateway \
...
<Note>
Run `docker compose` from the repo root. If you enabled `REMOTECLAW_EXTRA_MOUNTS`
or `REMOTECLAW_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `remoteclaw-cli` shares `remoteclaw-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d remoteclaw-gateway`, run onboarding
and setup-time config writes through `remoteclaw-gateway` with
...
See the [`ClawDock` Helper README](https://github.com/remoteclaw/remoteclaw/blob/main/scripts/shell-helpers/README.md).
...
<AccordionGroup>
  ...
  <Accordion title="Shared-network security note">
    `remoteclaw-cli` uses `network_mode: "service:remoteclaw-gateway"` so CLI
    ...
    `no-new-privileges` on `remoteclaw-cli`.
  </Accordion>
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export REMOTECLAW_HOME_VOLUME="remoteclaw_home"`
    2. **Bake system deps**: `export REMOTECLAW_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `REMOTECLAW_HOME_VOLUME` or `REMOTECLAW_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/remoteclaw/remoteclaw/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    RemoteClaw runs commands with `sh -lc` (login shell), which sources
