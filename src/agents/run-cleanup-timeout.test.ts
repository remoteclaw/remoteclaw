...
describe("agent cleanup timeout", () => {
  ...
  it("prefers explicit cleanup timeout values over environment overrides", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith(
      "agent cleanup timed out: runId=run-explicit sessionId=session-explicit step=remoteclaw-trajectory-flush timeoutMs=2000",
    );
  });
  ...
  it("keeps explicit zero cleanup timeouts as a one millisecond timeout", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith(
      "agent cleanup timed out: runId=run-zero sessionId=session-zero step=remoteclaw-trajectory-flush timeoutMs=1",
    );
  });
  ...
  it.each([
    ...
    expect(log.warn).toHaveBeenCalledWith(
      `agent cleanup timed out: runId=${runId} sessionId=${sessionId} step=remoteclaw-trajectory-flush timeoutMs=10000`,
    );
  });
