...
function getAssistantToolCallBlocks(messages: AgentMessage[]) {
  ...
  return (assistant.content as unknown[]).filter((block: unknown) => {
...
describe("sanitizeToolCallInputs", () => {
  ...
  it("keeps valid tool calls and preserves text blocks", () => {
    ...
    const types = Array.isArray(assistant.content)
      ? (assistant.content as unknown[]).map((block: unknown) => (block as { type?: unknown }).type)
      : [];
  ...
  it("redacts sessions_spawn attachments for mixed-case and padded tool names", () => {
    ...
    expect(attachments[0]?.content).toBe("__REMOTECLAW_REDACTED__");
  });
