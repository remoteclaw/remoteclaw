...
function resolvePendingPairingIssue(
  ...
  const approveCommand = formatCliArgs(["remoteclaw", "devices", "approve", pending.requestId]);
  const inspectCommand = formatCliArgs(["remoteclaw", "devices", "list"]);
  if (!paired) {
    return {
      kind: "first-time",
      pending,
  ...
  if (paired.publicKey !== pending.publicKey) {
    return {
      ...
      approveCommand,
      inspectCommand,
      removeCommand: formatCliArgs(["remoteclaw", "devices", "remove", pending.deviceId]),
    };
  }
...
function formatPairingStoreReadIssue(error: JsonFileReadError): string {
  ...
  return `- Device pairing store ${error.filePath} ${problem}. RemoteClaw refused to treat it as empty to avoid overwriting approved pairings. Fix the JSON or file permissions, or move it aside and re-pair devices.`;
}
