...
export function shouldSkipAppLintForMissingSwiftlint(options = {}) {
  ...
  return (
    isTruthyEnvFlag(env.REMOTECLAW_TESTBOX_REMOTE_RUN) &&
    platform !== "darwin" &&
    !swiftlintAvailable
  );
}
...
export function shouldDelegateChangedCheckToCrabbox(argv = [], env = process.env) {
  if (!isTruthyEnvFlag(env.REMOTECLAW_TESTBOX)) {
    return false;
  }
  if (isTruthyEnvFlag(env.REMOTECLAW_TESTBOX_REMOTE_RUN)) {
    return false;
  }
...
export function buildChangedCheckCrabboxArgs(argv = []) {
  return [
    ...
    "REMOTECLAW_TEST_PROJECTS_PARALLEL=6",
    "REMOTECLAW_VITEST_MAX_WORKERS=1",
    "REMOTECLAW_VITEST_NO_OUTPUT_TIMEOUT_MS=900000",
    "REMOTECLAW_TESTBOX=1",
    "REMOTECLAW_TESTBOX_REMOTE_RUN=1",
    "pnpm",
    "check:changed",
    ...argv,
  ];
...
export async function runChangedCheckViaCrabbox(argv = [], env = process.env) {
  console.error(
    "[check:changed] REMOTECLAW_TESTBOX=1 set; delegating to Blacksmith Testbox via `pnpm crabbox:run`.",
  );
