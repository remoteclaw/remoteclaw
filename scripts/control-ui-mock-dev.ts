// Control Ui Mock Dev script supports RemoteClaw repository automation.
...
function createChatPickerScenario(): ControlUiMockGatewayScenario {
  ...
  const workspaceListCases = ["main", "alpha", "remoteclaw-mock"].map((agentId) => ({
    match: { agentId },
    response: {
      agentId,
  ...
  const workspaceFileCases = ["main", "alpha", "remoteclaw-mock"].flatMap((agentId) =>
  ...
  <text x="320" y="326" text-anchor="middle" font-family="ui-sans-serif, system-ui" font-size="24" fill="#f6f7f9">remoteclaw session artifact</text>
</svg>`;
  const lobsterArtifact = {
    id: "artifact-remoteclaw-lobster",
    type: "image",
    title: "remoteclaw-lobster-preview.svg",
  ...
  return {
    assistantAgentId: "remoteclaw-mock",
    assistantName: "RemoteClaw mock",
    defaultAgentId: "remoteclaw-mock",
...
function createMockGatewayPlugin(scenario: ControlUiMockGatewayScenario): Plugin {
  ...
  return {
    ...
    name: "remoteclaw-control-ui-mock-gateway",
    transformIndexHtml(html) {
      return html.replace(
        "</head>",
        `    <script data-remoteclaw-control-ui-mock-gateway>\n${initScript}\n    </script>\n  </head>`,
      );
    },
  };
}
...
const server = await createServer({
  ...
  define: {
    REMOTECLAW_CONTROL_UI_BUILD_ID: JSON.stringify("mock"),
  },
  logLevel: "error",
  optimizeDeps: {
