...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: RemoteClawConfig) => string[];
  input: RemoteClawConfig;
  expected: string[];
}) {
  expect(params.resolve(params.input)).toEqual(params.expected);
}

function expectResolvedDefaultAccountCase(input: RemoteClawConfig, expected: string) {
