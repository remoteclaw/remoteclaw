...
final class OnboardingAISetupModel {
    ...
    private(set) var phase: Phase = .idle {
        didSet {
            ...
            OnboardingController.shared.busyReason = self.phase == .testing
                ? "RemoteClaw is testing your AI connection."
                : nil
        }
    }
    ...
    static func friendlyTransportError(_ raw: String) -> String {
        if raw.localizedCaseInsensitiveContains("unknown method") {
            return "The Gateway is running an older RemoteClaw version that doesn’t support " +
                "app-guided setup. Update RemoteClaw on the gateway, then try again."
        }
        return raw
    }
...
private enum OnboardingAISetupError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .providerCatalogUnavailable:
            "The Gateway is running an older RemoteClaw version that doesn’t provide the " +
                "supported provider list. Update RemoteClaw on the gateway, then try again."
        }
    }
}
...
struct OnboardingAISetupView: View {
    ...
    private func candidateRow(_ candidate: OnboardingAISetupModel.Candidate) -> some View {
        ...
        return Button {
            ...
            .remoteClawSelectableRowChrome(selected: selected && status != .failed(message: ""))
        }
        .buttonStyle(.plain)
    ...
    private var manualProviderHelp: String {
        ...
        guard let hint, !hint.isEmpty else {
            return "Paste the key or token here, and RemoteClaw checks it with a real test question."
        }
        return "\(hint). Paste it here, and RemoteClaw checks it with a real test question."
    }
...
/// Every onboarding failure points at a docs.remoteclaw.org page so people are
        ...
        HStack(alignment: .top, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack(spacing: 14) {
                    ...
                    Button("Open help…") {
                        if let url = URL(string: "https://docs.remoteclaw.org/\(self.docsSlug)") {
