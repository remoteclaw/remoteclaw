...
| ----------- | -------------------------- | ------------------------------------ | -------------------------------------------- |
| Legacy mode | `remoteclaw/plugin-sdk/*`  | all existing non-opted-in extensions | current permissive behavior remains          |
| Opt-in mode | `@remoteclaw/plugin-sdk/*` | first-wave extensions only           | package-local `rootDir` + project references |
...
- root `tsconfig.json` maps `remoteclaw/plugin-sdk/*` directly to
  `src/plugin-sdk/*.ts`
...
- moves the SDK toward a workspace package named `@remoteclaw/plugin-sdk`
...
- R2. Name the new package `@remoteclaw/plugin-sdk`.
- R3. Give the new SDK package its own `package.json` and `tsconfig.json`.
- R4. Keep legacy `remoteclaw/plugin-sdk/*` imports working for non-opted-in
...
The initial `@remoteclaw/plugin-sdk` package only needs to cover these:
...
- Introduce `@remoteclaw/plugin-sdk` as a new workspace package while keeping the
  legacy root `remoteclaw/plugin-sdk/*` surface alive during migration.
...
- Keep `@remoteclaw/plugin-sdk` private in the first wave.
...
- Whether the first-wave package can point directly at package-local `src/*.ts`
  ...
  still required for the `@remoteclaw/plugin-sdk` package.
  This is an implementation-owned TS graph validation question.

- Whether the root `remoteclaw` package should proxy first-wave SDK subpaths to
...
- [ ] **Unit 1: Introduce the real `@remoteclaw/plugin-sdk` workspace package**
...
- Add a new workspace package named `@remoteclaw/plugin-sdk`.
...
- The repo contains a valid `@remoteclaw/plugin-sdk` workspace package with a
...
- [ ] **Unit 3: Migrate the first-wave extensions onto `@remoteclaw/plugin-sdk`**
...
- Modify: production and test imports under each of the 10 extension roots that
  currently reference `remoteclaw/plugin-sdk/*`

**Approach:**

- Add `@remoteclaw/plugin-sdk: workspace:*` to the first-wave extension
  `devDependencies`.
- Replace `remoteclaw/plugin-sdk/*` imports in those packages with
  `@remoteclaw/plugin-sdk/*`.
...
- Package dependency shape used by other `@remoteclaw/*` workspace packages
...
- Integration: migrated extensions do not require root `remoteclaw/plugin-sdk/*`
  aliases for typechecking.

**Verification:**

- The first-wave extensions build and test against `@remoteclaw/plugin-sdk`
...
- Keep root `remoteclaw/plugin-sdk/*` as the compatibility surface for legacy
...
- **API surface parity:** first-wave subpaths must remain semantically identical
  through both `remoteclaw/plugin-sdk/*` and `@remoteclaw/plugin-sdk/*` during the
  transition.
...
- Introduce `@remoteclaw/plugin-sdk`
