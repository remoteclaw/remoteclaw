---
summary: "Host RemoteClaw on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up RemoteClaw on Oracle Cloud
  - Looking for free VPS hosting for RemoteClaw
...
Run a persistent RemoteClaw Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@remoteclaw`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=remoteclaw --reset` to re-authenticate.

**Gateway will not start** -- Run `remoteclaw doctor --non-interactive` and check logs with `journalctl --user -u remoteclaw-gateway -n 50`.
...
- [Updating](/install/updating) -- keep RemoteClaw up to date
