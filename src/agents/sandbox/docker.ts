...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "remoteclaw.sandbox=1");
  args.push("--label", `remoteclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `remoteclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `remoteclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "remoteclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`remoteclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`remoteclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("remoteclaw sandbox recreate --all");
}
