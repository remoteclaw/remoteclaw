...
function resolveSkillBinTrustEntries(bins: string[], pathEnv: string): SkillBinTrustEntry[] {
  ...
  for (const bin of bins) {
    ...
    seen.add(key);
    trustEntries.push({ bin: name, name, resolvedPath });
  }
...
export async function resolveNodeHostGatewayCredentials(params: {
  ...
  const token =
    normalizeSecretInputString(env.REMOTECLAW_GATEWAY_TOKEN) ??
  ...
  const shouldResolveConfiguredPassword =
    !normalizeSecretInputString(env.REMOTECLAW_GATEWAY_PASSWORD) &&
    !tokenCanWin &&
