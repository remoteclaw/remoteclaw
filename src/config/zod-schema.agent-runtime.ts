...
/** Fork-specific: CLI runtime identifiers accepted in agents.list[].runtime */
const CliRuntimeIdSchema = z.union([
  z.literal("claude"),
  z.literal("gemini"),
  z.literal("codex"),
  z.literal("opencode"),
...
const AgentRuntimeSchema = z
  .union([
    // Fork-specific: accept simple string runtime identifier (e.g., "claude", "gemini", "codex")
    CliRuntimeIdSchema,
    z
      .object({
...
export const AgentEntrySchema = z
  .object({
    ...
    workspace: z.string().refine((val) => val.trim().length > 0, {
      message: "agents.list[].workspace must be a non-empty string",
    }),
