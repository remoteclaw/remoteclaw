...
module.exports = {
  ...
  configSchema: (require("remoteclaw/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("remoteclaw/plugin-sdk");
  ...
  it("builds plugin-sdk aliases from the module being loaded, not the loader location", () => {
    ...
    expect(fs.realpathSync(sourceAliases["remoteclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(sourceAliases["remoteclaw/plugin-sdk/channel-runtime"] ?? "")).toBe(
    ...
    expect(fs.realpathSync(distAliases["remoteclaw/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(distRootAlias),
    );
    expect(fs.realpathSync(distAliases["remoteclaw/plugin-sdk/channel-runtime"] ?? "")).toBe(
...
`,
    ...
    const withAlias = createJiti(jitiBaseUrl, {
      ...__testing.buildPluginLoaderJitiOptions({
        "remoteclaw/plugin-sdk/infra-runtime": copiedChannelRuntimeShim,
      }),
      tryNative: false,
    });
