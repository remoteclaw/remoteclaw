...
- Every stable RemoteClaw release ships the npm package and macOS app together;
...
9. For beta, tag `vYYYY.M.D-beta.N`, publish with npm dist-tag `beta`, then run
   post-publish package acceptance against the published `remoteclaw@YYYY.M.D-beta.N`
   or `remoteclaw@beta` package. If a pushed or published beta needs a fix, cut
...
- Run the manual `Full Release Validation` workflow before release approval to
  ...
  `RemoteClaw Release Checks` for install smoke, package acceptance, Docker
...
`target_ref=<release-ref>`, dispatches `RemoteClaw Release Checks`, and
optionally dispatches standalone post-publish Telegram E2E when
`npm_telegram_package_spec` is set. `RemoteClaw Release Checks` then fans out
...
6. Run `RemoteClaw NPM Release` again with `preflight_only=false`, the same
