...
- Realtime voice can call back into the full RemoteClaw agent when deeper
...
```bash
remoteclaw googlemeet create --transport chrome-node --mode realtime
```
...
```bash
remoteclaw googlemeet create --access-type OPEN --transport chrome-node --mode realtime
```
...
VM. In both cases the realtime model and `remoteclaw_agent_consult` run on the
...
RemoteClaw tools, it can call `remoteclaw_agent_consult`.
