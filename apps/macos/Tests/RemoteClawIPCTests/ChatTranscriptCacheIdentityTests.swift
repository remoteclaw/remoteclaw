...
struct ChatTranscriptCacheIdentityTests {
    private static let defaultStateDir = URL(fileURLWithPath: "/Users/tester/.remoteclaw", isDirectory: true)
    ...
    @Test @MainActor func `windows share one outbox owner per gateway`() {
        let databaseURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("remoteclaw-cache-owner-\(UUID().uuidString).sqlite")
    ...
    @Test func `local mode keys on state dir so profiles never collide`() {
        ...
        let devProfile = MacChatTranscriptCache.gatewayID(
            mode: .local,
            localStateDir: URL(fileURLWithPath: "/Users/tester/.remoteclaw-dev", isDirectory: true),
            remoteTransport: .ssh,
            directURL: nil,
            sshTarget: "",
            sshRemotePort: 18789)
        #expect(defaultProfile == "local:/Users/tester/.remoteclaw")
        #expect(devProfile == "local:/Users/tester/.remoteclaw-dev")
    ...
    @Test func `local state dir aliases resolve to one identity`() throws {
        ...
        let canonical = try FileManager.default.temporaryDirectory
            .appendingPathComponent("remoteclaw-cache-identity-\(UUID().uuidString)", isDirectory: true)
