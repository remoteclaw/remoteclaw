...
summary: "Step-by-step guide to building a messaging channel plugin for RemoteClaw"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect RemoteClaw to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

# Building Channel Plugins

This guide walks through building a channel plugin that connects RemoteClaw to a
...
Channel plugins do not need their own send/edit/react tools. RemoteClaw keeps one
...
<Steps>
  <Step title="Package and manifest">
    ...
    ```json package.json
    {
      "name": "@myorg/remoteclaw-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "remoteclaw": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect RemoteClaw to Acme Chat."
        }
      }
    }
    ```
  ...
  <Step title="Add a setup entry">
    ...
    import { defineSetupPluginEntry } from "remoteclaw/plugin-sdk/core";
    import { acmeChatPlugin } from "./src/channel.js";

    export default defineSetupPluginEntry(acmeChatPlugin);
    ```

    RemoteClaw loads this instead of the full entry when the channel is disabled
  ...
  <Step title="Handle inbound messages">
    ...
    RemoteClaw. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to RemoteClaw.
...
├── package.json              # remoteclaw.channel metadata
├── remoteclaw.plugin.json      # Manifest with config schema
