...
<Tabs>
  <TabItem label="Interactive wizard">
    ```bash remoteclaw onboard # full setup wizard remoteclaw configure # config wizard ```
  </TabItem>
  <TabItem label="CLI (one-liners)">
    ```bash remoteclaw config get agents.defaults.workspace remoteclaw config set
    agents.defaults.heartbeat.every "2h" remoteclaw config unset tools.web.search.apiKey ```
  </TabItem>
