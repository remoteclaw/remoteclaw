...
final class CLIInstallPrompter {
    ...
    private let logger = Logger(subsystem: "org.remoteclaw", category: "cli.prompt")
    ...
    private func checkAndPromptIfNeededAsync(reason: String) async {
        ...
        let alert = NSAlert()
        alert.messageText = "Install RemoteClaw CLI?"
    ...
    private func installCLI() async {
        ...
        if installed {
            await status.set("Starting RemoteClaw Gateway…")
            let activation = await CLIInstaller.activateLocalGateway()
            let message = switch activation {
            case .ready:
                "RemoteClaw Gateway is ready."
            case .deferred:
                "RemoteClaw is installed. The Gateway will start when This Mac is active and resumed."
            case .failed:
                "RemoteClaw was installed, but the Gateway did not start. Open Settings to retry."
            }
            await status.set(message)
        }
    ...
    private func openSettings(tab: SettingsTab) {
        ...
        DispatchQueue.main.async {
            NotificationCenter.default.post(name: .remoteclawSelectSettingsTab, object: tab)
        }
    }
