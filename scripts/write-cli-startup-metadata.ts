// Write Cli Startup Metadata script supports RemoteClaw repository automation.
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".remoteclaw-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "remoteclaw-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "remoteclaw-build",
    ...
    NO_COLOR: "1",
    REMOTECLAW_BUNDLED_PLUGINS_DIR: bundledPluginsDir,
...
async function renderSourceBrowserHelpText(
  ...
  const inlineModule = [
    ...
    `registerBrowserCli(program, ["node", "remoteclaw", "browser", "--help"]);`,
  ...
  return await spawnText(["--import", "tsx", "--input-type=module", "--eval", inlineModule], {
    ...
    env: {
      ...renderContext.env,
      REMOTECLAW_DISABLE_CLI_STARTUP_HELP_FAST_PATH: "1",
    },
...
async function renderSourceCommandHelpText(
  ...
  return await spawnText(["remoteclaw.mjs", command, "--help"], {
    cwd: rootDir,
    env: {
      ...renderContext.env,
      REMOTECLAW_DISABLE_CLI_STARTUP_HELP_FAST_PATH: "1",
