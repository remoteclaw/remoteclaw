...
export function resolveCompileConcurrency(
  ...
  const raw = env.REMOTECLAW_EXTENSION_BOUNDARY_CONCURRENCY?.trim();
  if (raw) {
    return parsePositiveInt(raw, "REMOTECLAW_EXTENSION_BOUNDARY_CONCURRENCY");
  }
...
async function runCompileCheck(extensionIds) {
  ...
  const verboseFreshLogs = process.env.REMOTECLAW_EXTENSION_BOUNDARY_VERBOSE_FRESH === "1";
