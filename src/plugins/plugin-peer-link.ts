...
export type RemoteClawPeerLinkAuditIssue = {
  packageName: string;
  packageDir: string;
  reason: string;
};
...
type AuditManagedNpmRootResult = {
  checked: number;
  broken: number;
  issues: RemoteClawPeerLinkAuditIssue[];
};

type RemoteClawPeerLinkResult = "linked" | "skipped" | "unchanged";
...
async function auditRemoteClawPeerDependency(params: {
  hostRoot: string;
  packageDir: string;
  npmRoot?: string;
  packageName?: string;
}): Promise<RemoteClawPeerLinkAuditIssue | null> {
  const packageName =
    params.packageName ??
    (params.npmRoot
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      return {
        packageName,
        packageDir: params.packageDir,
        reason: `missing ${path.join(nodeModulesDir, "remoteclaw")}`,
      };
    }
    throw error;
  }

  const linkPath = path.join(nodeModulesDir, "remoteclaw");
...
export async function auditRemoteClawPeerDependencyLink(params: {
  packageDir: string;
  packageName?: string;
}): Promise<RemoteClawPeerLinkAuditIssue | null> {
  ...
  if (!hostRoot) {
    return {
      packageName,
      packageDir: params.packageDir,
      reason: "could not locate remoteclaw package root",
    };
  }
  return await auditRemoteClawPeerDependency({
    hostRoot,
    packageDir: params.packageDir,
    packageName,
  });
...
async function ensureRealNodeModulesDir(params: {
  ...
  try {
    ...
    if (!existing.isDirectory() || existing.isSymbolicLink()) {
      params.logger.warn?.(
        `Skipping remoteclaw peerDependency link because ${nodeModulesDir} is not a real directory.`,
      );
      return null;
    }
...
async function linkRemoteClawPeerDependency(params: {
  hostRoot: string;
  installedDir: string;
  peerName: string;
  logger: PluginPeerLinkLogger;
}): Promise<RemoteClawPeerLinkResult> {
  ...
  try {
    ...
    if (existing) {
      if (!existing.isSymbolicLink()) {
        if (params.peerName === "remoteclaw" && existing.isDirectory()) {
          const existingPackageName = await readPackageName(linkPath);
          if (existingPackageName === "remoteclaw") {
        ...
        params.logger.warn?.(
          `Skipping remoteclaw peerDependency link because ${linkPath} already exists and is not a symlink.`,
        );
        return "skipped";
      }
...
/**
 * Symlink the host remoteclaw package for plugins that declare it as a peer.
 ...
 */
export async function linkRemoteClawPeerDependencies(params: {
  installedDir: string;
  peerDependencies: Record<string, string>;
  logger: PluginPeerLinkLogger;
}): Promise<{ repaired: number; skipped: number }> {
  const peers = Object.keys(params.peerDependencies).filter((name) => name === "remoteclaw");
  if (peers.length === 0) {
  ...
  if (!hostRoot) {
    params.logger.warn?.(
      "Could not locate remoteclaw package root to symlink peerDependencies; plugin may fail to resolve remoteclaw at runtime.",
    );
  ...
  for (const peerName of peers) {
    const result = await linkRemoteClawPeerDependency({
      hostRoot,
      installedDir: params.installedDir,
      peerName,
      logger: params.logger,
...
export async function relinkRemoteClawPeerDependenciesInManagedNpmRoot(params: {
  npmRoot: string;
  logger: PluginPeerLinkLogger;
  ...
  for (const packageDir of await listManagedNpmRootPackageDirs(params.npmRoot)) {
    ...
    if (!Object.hasOwn(peerDependencies, "remoteclaw")) {
      continue;
    }
    checked += 1;
...
export async function auditRemoteClawPeerDependenciesInManagedNpmRoot(params: {
  npmRoot: string;
  ...
  let checked = 0;
  const issues: RemoteClawPeerLinkAuditIssue[] = [];
  for (const packageDir of await listManagedNpmRootPackageDirs(params.npmRoot)) {
    const peerDependencies = await readPackagePeerDependencies(packageDir);
    if (!Object.hasOwn(peerDependencies, "remoteclaw")) {
      continue;
    ...
    checked += 1;
    const issue = await auditRemoteClawPeerDependency({
      hostRoot,
      npmRoot: params.npmRoot,
      packageDir,
    });
