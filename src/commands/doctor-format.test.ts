...
describe("buildGatewayRuntimeHints", () => {
  ...
  it("surfaces suspicious systemd cgroup hygiene with inspection commands", () => {
    expect(
      ...
      "Run: systemctl --user show remoteclaw-gateway.service -p KillMode -p TasksCurrent -p MemoryCurrent -p MainPID",
      "Run: systemd-cgls --user-unit remoteclaw-gateway.service",
      "After reviewing service settings, run: remoteclaw gateway restart",
    ]);
  });
  ...
  it("guides recovery when systemd hit its restart start limit (crash loop)", () => {
    ...
    expect(text).toContain("remoteclaw gateway restart");
