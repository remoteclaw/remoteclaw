...
// Release Beta Smoke script supports RemoteClaw repository automation.
...
function resolveBetaVersion(beta: string): string {
  const value = beta.trim().replace(/^remoteclaw@/, "");
  if (/^\d{4}\.\d+\.\d+-beta\.\d+$/u.test(value)) {
    return value;
  }
  if (value === "beta") {
    return run("npm", ["view", "remoteclaw@beta", "version"], { capture: true }).trim();
  }
  const betaMatch = /^(?:beta)?(\d+)$/u.exec(value);
  if (!betaMatch) {
    return run("npm", ["view", `remoteclaw@${value}`, "version"], { capture: true }).trim();
  }
  ...
  const match = versions
    ...
    .at(-1);
  if (!match) {
    throw new Error(`no remoteclaw registry version found for ${beta}`);
  }
  return match;
}
...
async function main(): Promise<void> {
  ...
  const packageSpec = `remoteclaw@${version}`;
