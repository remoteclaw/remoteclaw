...
Plain `remoteclaw status` stays on the fast read-only path and marks memory as
...
`remoteclaw status --all`, `remoteclaw status --deep`, `remoteclaw security audit`,
and `remoteclaw memory status --deep`.
...
- Session status output separates `Execution:` from `Runtime:`. `Execution`
  ...
  whether the session is using `RemoteClaw Default`, `OpenAI Codex`, a CLI
...
- MiniMax's raw `usage_percent` / `usagePercent` fields are remaining quota,
  so RemoteClaw inverts them before display; count-based fields win when
...
- Update info surfaces in the Overview; if an update is available, status
  prints a hint to run `remoteclaw update` (see [Updating](/install/updating)).
